...
OV-CV-01 role note: v0 is an integrity-only comparator lane (typed input + deterministic pass/fail reason-code surface), not yet a physics-discriminating comparator claim.
...
Name: OV-CV-01 BEC-Bragg comparator v0 (front-door deterministic record)
...
Scope / limits: Integrity-only comparator surface (`comparator_role=integrity_only`); deterministic record only; typed artifacts only; no external truth claim; blocked when pinned domain artifacts are missing; may be BLOCKED by admissibility manifest (CT01/SYM01/CAUS01).

Dependencies: DR-01, BR-01, OV-DR-BR-01, CT-01, SYM-01, CAUS-01

Notes: CV01 v0 is a pipeline-integrity comparator (not physics-discriminating); CV01 v1 design-gate documentation pins the non-tautological upgrade requirement before any role promotion.
