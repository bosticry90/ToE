...
The current Lean aggregate imports the strict scalar/QFT analytic blocker split through A2A15A1A31 raw-IBP-to-Green convergence package, the scalar/QFT handoff capstone, cross-pillar derivation protocol/frontier surfaces, QM-STAT transport plus unified/componentwise transport residual evidence, the QM-STAT post-budget cross-pillar review, the QFT-GR stress-energy expectation source-map package plus residual-only semantic obstruction, the QFT-GR post-budget cross-pillar review, the SR/COSMO regime-transport residual package, and the SR/COSMO global-bridge semantic-map obstruction; `lake build ToeFormal` is green as of 2026-05-02, with no Lean warnings.
A2A15A1 remains retained under the post-capstone anti-loop rule: no further scalar-only drilling unless the master dependency graph changes. The strict architecture target rotated through QM-STAT transport residual semantics, completed the required QM-STAT review, executed the bounded QFT-GR residual-only obstruction slice, completed the QFT-GR post-budget review, constructed a bounded SR/COSMO regime-transport zero-residual package under supplied local/regime alignment, and refuted residual-package-only global SR/COSMO bridge closure without an additional semantic map; Phase 2 is not authorized.
...
The loop-control plane is pinned at `formal/docs/release/LOOP_CONTROL_REGISTRY_v0.json` with focused gate `formal/python/tests/test_loop_control_registry_v0_gate.py`; it centralizes loop/freeze/fresh-delta/attempt-budget/dependency-cycle/promotion-escrow controls without enrolling the governance manifest or authorizing scientific promotion. The QM-STAT component residual evidence satisfies the fresh-delta rule as `stronger_evidence_object`, the QFT-GR residual-only semantic obstruction satisfies it as `counterexample`, the SR/COSMO regime-transport package satisfies it as `new_theorem`, and the SR/COSMO global-bridge semantic-map obstruction satisfies it as `counterexample`; scalar, QM-STAT, QFT-GR, and same-lane SR/COSMO are paused pending the SR/COSMO post-budget review.
