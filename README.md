...
The scalar `taylorWithinEval` order-three coefficient formula is now theorem-derived inside A1A9. The next strict scalar/QFT target is the remaining endpoint-package derivation: left-endpoint Taylor orientation, centered coefficient alignment across endpoint intervals, and full two-sided endpoint package construction. Uniform mesh convergence remains downstream until that endpoint package is derived or explicitly blocked.
