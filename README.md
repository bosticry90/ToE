...
The scalar `taylorWithinEval` order-three coefficient formula, reflected-left endpoint expansion/bound route, and centered coefficient-alignment theorem from mathlib's within-to-global derivative route are now theorem-derived/formalized inside A1A9. The next strict scalar/QFT target is the full two-sided endpoint package construction. Uniform mesh convergence remains downstream until that endpoint package is derived or explicitly blocked.
