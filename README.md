...
The current Lean aggregate imports the strict scalar/QFT analytic blocker split through A2A15A1A31 raw-IBP-to-Green convergence package, the scalar/QFT handoff capstone, cross-pillar derivation protocol/frontier surfaces, QM-STAT transport plus unified/componentwise transport residual evidence, and QFT-GR stress-energy expectation source-map package; `lake build ToeFormal` is green as of 2026-05-01, with existing lint/style warnings only.
A2A15A1 remains retained under the post-capstone anti-loop rule: no further scalar-only drilling unless the master dependency graph changes, the active strict architecture target rotates to QM-STAT transport residual semantics, and Phase 2 is not authorized.
