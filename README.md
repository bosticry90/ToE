...
The current Lean aggregate imports the strict scalar/QFT analytic blocker split, A2A15 raw-IBP and analytic interval-lift channel surfaces, A2A15A1A graph-Laplacian stencil/Taylor/mathlib endpoint-package surfaces through the A2A15A1A9 scalar-coefficient, reflected-left expansion/bound, centered-alignment-from-mathlib, two-sided endpoint-package route, A2A15A1A10 uniform mesh convergence contract/wiring surface, A2A15A1A11 evidence-to-graph-channel theorem surface, A2A15A1A12 order-`h^2` mesh-limit bridge, A2A15A1A13 concrete mesh/zero-error instantiation surface, A2A15A1A14 nonzero stencil-error normal-form bound, A2A15A1A15 endpoint-package stencil-error uniform-bound surface, A2A15A1A16 actual graph stencil-error identification surface, and A1A graph-channel semantic closure review surface, plus `ToeFormal.Bridges.QM_STAT_Transport`; `lake build ToeFormal` is green as of 2026-04-29, with existing lint/style warnings only.
...
The scalar `taylorWithinEval` order-three coefficient formula, reflected-left endpoint expansion/bound route, centered coefficient-alignment theorem from mathlib's within-to-global derivative route, and full two-sided endpoint package into the local stencil-bound route are theorem-derived/formalized inside A1A9. A1A10 records the uniform mesh convergence contract; A1A11 proves the conditional theorem that supplied uniform-mesh evidence fills the A1A graph-channel route; A1A12 proves, in the chosen real-filter convergence mode, that mesh-to-zero plus an eventual order-`h^2` stencil-error bound implies stencil error tends to zero and constructs the A1A11 evidence object; A1A13 instantiates a concrete `1 / (n + 1)` mesh plus zero-error normal form; A1A14 replaces that with a nonzero `h_n^2` error normal form; A1A15 defines the endpoint-package stencil-error sequence and proves its order-`h^2` bound and convergence; A1A16 identifies the actual graph-action stencil-error sequence with that endpoint-package sequence, transferring the order-`h^2` bound, convergence, A1A12 mode, and A1A11 evidence; and the closure review proves the conditional parent-field bridge while recording that the parent interface map is not derived from A1A16 alone. Parent graph-channel semantic closure remains retained under `PHASE1-BLOCKER-003A2A15A1A16_GRAPH_CHANNEL_SEMANTIC_CLOSURE_RETAINED`.
