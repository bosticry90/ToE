...
The scalar `taylorWithinEval` order-three coefficient formula and reflected-left endpoint expansion/bound route are now theorem-derived inside A1A9. The next strict scalar/QFT target is the remaining endpoint-package derivation: original left-endpoint coefficient orientation, centered coefficient alignment across endpoint intervals, and full two-sided endpoint package construction. Uniform mesh convergence remains downstream until that endpoint package is derived or explicitly blocked.
