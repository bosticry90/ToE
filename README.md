...
The scalar `taylorWithinEval` order-three coefficient formula, reflected-left endpoint expansion/bound route, centered coefficient-alignment theorem from mathlib's within-to-global derivative route, and full two-sided endpoint package into the local stencil-bound route are theorem-derived/formalized inside A1A9. A1A10 records the uniform mesh convergence contract, and A1A11 proves the conditional theorem that supplied mesh-to-zero, uniform-bound, refinement-independent-constant, order-`h^2`, stencil-limit, and graph-relation evidence fills the A1A graph-channel route. Deriving that evidence concretely remains unproved.
