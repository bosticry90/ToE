...
The loop-control plane is pinned at `formal/docs/release/LOOP_CONTROL_REGISTRY_v0.json` with focused gates `formal/python/tests/test_loop_control_registry_v0_gate.py` and `formal/python/tests/test_current_target_freshness_gate.py`; it centralizes loop/freeze/fresh-delta/attempt-budget/dependency-cycle/promotion-escrow controls without enrolling the governance manifest or authorizing scientific promotion. The QM-STAT component residual evidence satisfies the fresh-delta rule as `stronger_evidence_object`, the QFT-GR residual-only semantic obstruction satisfies it as `counterexample`, the SR/COSMO regime-transport package satisfies it as `new_theorem`, the SR/COSMO global-bridge semantic-map obstruction satisfies it as `counterexample`, the QM evolution transport-hypotheses adjudication satisfies it as `counterexample`, and the QM evolution semantic bridge theorem satisfies it as `new_theorem`; scalar, QM-STAT, QFT-GR, same-lane SR/COSMO, and same-lane QM evolution are paused, while the single live target is `extract_em_qft_physics_blocker_into_protocol_row`.
