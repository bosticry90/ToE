...
The scalar `taylorWithinEval` order-three coefficient formula, reflected-left endpoint expansion/bound route, centered coefficient-alignment theorem from mathlib's within-to-global derivative route, and full two-sided endpoint package into the local stencil-bound route are now theorem-derived/formalized inside A1A9. A1A10 now records the uniform mesh convergence contract and proves conditional wiring into the smooth Taylor/refinement and graph-channel evidence routes; the actual uniform mesh convergence theorem remains unproved.
