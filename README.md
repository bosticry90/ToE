...
The scalar `taylorWithinEval` order-three coefficient formula, reflected-left endpoint expansion/bound route, centered coefficient-alignment theorem from mathlib's within-to-global derivative route, and full two-sided endpoint package into the local stencil-bound route are now theorem-derived/formalized inside A1A9. Uniform mesh convergence is the next downstream target and remains unproved.
