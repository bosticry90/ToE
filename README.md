...
The loop-control plane is pinned at `formal/docs/release/LOOP_CONTROL_REGISTRY_v0.json` with focused gates `formal/python/tests/test_loop_control_registry_v0_gate.py` and `formal/python/tests/test_current_target_freshness_gate.py`; it centralizes loop/freeze/fresh-delta/attempt-budget/dependency-cycle/promotion-escrow controls without enrolling the governance manifest or authorizing scientific promotion. The QM-STAT component residual evidence satisfies the fresh-delta rule as `stronger_evidence_object`, the QFT-GR residual-only semantic obstruction satisfies it as `counterexample`, the SR/COSMO regime-transport package satisfies it as `new_theorem`, the SR/COSMO global-bridge semantic-map obstruction satisfies it as `counterexample`, the QM evolution transport-hypotheses adjudication satisfies it as `counterexample`, the QM evolution semantic bridge theorem satisfies it as `new_theorem`, the EM-QFT shared-dynamics / residual-unification adjudication satisfies it as `counterexample`, and the EM-QFT interface-alignment adjudication satisfies it as `counterexample`; scalar, QM-STAT, QFT-GR, same-lane SR/COSMO, same-lane QM evolution, and third same-lane EM-QFT bridge drilling are paused, while the single live target is `em_qft_post_budget_cross_pillar_review`.
