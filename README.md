...
The current Lean aggregate imports the strict scalar/QFT analytic blocker split, A2A15 raw-IBP and analytic interval-lift channel surfaces, A2A15A1A graph-Laplacian stencil/Taylor/mathlib endpoint-package surfaces through the A2A15A1A9 scalar-coefficient, reflected-left expansion/bound, and centered-alignment object route, and `ToeFormal.Bridges.QM_STAT_Transport`; `lake build ToeFormal` is green as of 2026-04-29, with existing lint/style warnings only.
...
The scalar `taylorWithinEval` order-three coefficient formula, reflected-left endpoint expansion/bound route, and centered coefficient-alignment object-to-package route are now theorem-derived/formalized inside A1A9. The next strict scalar/QFT target is deriving the centered endpoint coefficient-alignment theorem from mathlib and then the full two-sided endpoint package. Uniform mesh convergence remains downstream until that endpoint package is derived or explicitly blocked.
