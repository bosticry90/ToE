...
- v0.1-alpha dependency remediation status: tranches 001, 002, 003, 005, and 006 are `documented_dependency_nonblocking`; tranche 004 remains `retained_release_blocking_source_map_blocker`; the simple dependency-remediation queue is exhausted; and the retained tranche 004 source-map closure adjudication packet has been prepared from the accepted source-map authorization requirements, with no source-map closure, QFT-GR seam closure, blocker movement, release readiness, release assembly, or release promotion.
...
- `CURRENT_LIVE_TARGET_REPORT_v0: formal/docs/release/V01_ALPHA_RETAINED_TRANCHE_004_SOURCE_MAP_CLOSURE_ADJUDICATION_20260523_v0.json`
