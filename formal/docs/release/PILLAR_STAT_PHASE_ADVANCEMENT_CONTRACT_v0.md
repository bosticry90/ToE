...
1. Once `STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0` is `PRESENT`, `STAT_NEXT_EXECUTION_PHASE_v0` must point to derivation-completeness gate entry work rather than remaining on readiness-packet preparation.
2. `STAT_NEXT_EXECUTION_TOKEN_v0` remains the next unfinished milestone until `STAT_DERIVATION_COMPLETENESS_GATE_ENTRY_STATUS_v0` stops being `NOT_PRESENT_v0`.
...
- Pin the derivation-completeness gate entry status now that the readiness packet is present.
