...
1. Once `STAT_DERIVATION_COMPLETENESS_GATE_ENTRY_STATUS_v0` is pinned, `STAT_NEXT_EXECUTION_PHASE_v0` must point to derivation-completeness discharge-surface entry work rather than remaining on gate-entry status.
2. `STAT_NEXT_EXECUTION_TOKEN_v0` remains the next unfinished milestone until `STAT_DERIVATION_COMPLETENESS_DISCHARGE_SURFACE_STATUS_v0` stops being `NOT_PRESENT_v0`.
...
- Pin the derivation-completeness discharge-surface status now that the gate-entry status is present.
