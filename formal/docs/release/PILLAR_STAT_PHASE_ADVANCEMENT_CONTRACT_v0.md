...
1. Once `STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0` is pinned, `STAT_NEXT_EXECUTION_PHASE_v0` must point to failure-trigger discharge object-surface entry work rather than remaining on failure-trigger theorem-surface status.
2. `STAT_NEXT_EXECUTION_TOKEN_v0` remains the next unfinished milestone until `STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_v0` stops being `NOT_PRESENT_v0`.
...
- Pin the failure-trigger discharge object-surface status now that the failure-trigger theorem-surface status is present.
