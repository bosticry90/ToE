# VPC v6 Linux egress-denied qualification run procedure v3

## Amendment from v2

This procedure supersedes v2 after GitHub run `34165140494` exposed a
content-addressed replay custody error. The qualification entry point created
and replayed the Linux bundle successfully under its content-addressed filename.
The workflow then copied those unchanged bytes to `linux_bundle.json`; the
separate replay correctly rejected that renamed path with `FROZEN_BUNDLE_HASH`.
V3 preserves the bundle's hash-derived basename when copying it into the upload
artifact. The bundle bytes, scientific target, tested commit, sources,
operations, roots, and expected answers are unchanged.
...
11. Copy the Linux bundle into the uploaded evidence tree without changing its
    hash-derived filename, then replay it in two separate Python processes.
