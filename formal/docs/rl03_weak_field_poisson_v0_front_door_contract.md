# RL03 Weak-Field Poisson Front Door Contract v0

Schema ID:
- OV-RL-03_weak_field_poisson_comparator/v0

Input artifact schema:
- RL/weak_field_poisson_front_door_report/v1

Required fields:
- schema
- config_tag
- regime_tag
- params: {kappa, length, nx}
- boundary
- gauge
- x
- rho
- phi
- fingerprint

Blocked conditions:
- missing reference or candidate artifacts

Reason codes (non-exhaustive; v0):
- FAIL_X_GRID_ORDER
- FAIL_X_GRID_ALIGNMENT
- FAIL_DOMAIN_PARAMETER_INCONSISTENT
- FAIL_GAUGE_CONSTRAINT
- FAIL_POISSON_RESIDUAL
- FAIL_NONDETERMINISTIC_FINGERPRINT
- rl03_weak_field_poisson_constraints_satisfied

Scope limits:
- front_door_only
- typed_artifacts_only
- deterministic_record_only
- discriminative_candidate
- within_rep_only
- no_external_truth_claim
