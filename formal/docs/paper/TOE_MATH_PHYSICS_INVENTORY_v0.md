...
| `INV-MATH-POST-SWEEP-THEOREM-QUEUE-v0` | `math` | `work_queue` | First post-sweep theorem queue | `CURRENT` | `formal/toe_formal/ToeFormal/Derivation/PostSweepTheoremQueue.lean` | `formal/docs/lanes/STRICT_PHYSICS_DERIVATION_OBLIGATION_MAP_v0.md` | `formal/toe_formal/lakefile.toml` | `USED` | `T-CONDITIONAL` | `INV-MATH-MASTER-ACTION-DEPENDENCY-FRONTIER-v0` | Ranks the bounded theorem slices by leverage: QM-STAT unified theorem transport residual package, QFT-GR stress-energy expectation source map, and scalar paused after the A1A31 raw-IBP-to-Green conditional bridge; each slice has one retained blocker and one validation target. |
