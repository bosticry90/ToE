# Derivation Target: ToE Master-Action Variant-C Pressure Cycle02 v0

Spec ID:
- `DERIVATION_TARGET_TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE02_v0`

Target ID:
- `TARGET-TOE-MASTER-ACTION-VARIANT-C-PRESSURE-CYCLE02-v0`

Classification:
- `P-POLICY`

Purpose:
- Execute a second bounded pressure cycle on `VARIANT_C_SEAM_TRANSPORT_CROSS_TERM_v0`.
- Hold control/reference lanes stable while refreshing priority-lane evidence inputs.
...
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE02_STATUS_v0: RUN_BOUNDED_v0_NONCLAIM`
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE02_PRIORITY_VARIANT_v0: VARIANT_C_SEAM_TRANSPORT_CROSS_TERM_v0`
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE02_CONTROL_VARIANT_v0: VARIANT_A_BASELINE_v0`
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE02_EXECUTION_GUARD_v0: PACKET02_REQUIRED_FIELDS_AND_M4_SEAM_POINTERS_PRESERVED`

Priority lanes (cycle02):
- `QFT`
- `SR`

Required payload metadata for packet-02 artifacts in this cycle:
- `master_action_variant_cycle_target_v0: VARIANT_C_PRESSURE_CYCLE02_v0`
- `master_action_variant_cycle_role_v0` in `{PRIORITY_LANE_v0, CONTROL_LANE_v0, REFERENCE_LANE_v0}`

Priority-lane evidence refresh rule (cycle02):
- update only bounded evidence-input labels for priority lanes (`QFT`, `SR`) to cycle02-tagged values.
- do not remove required packet fields.
- preserve decision-eligibility guard structure and m4 seam pointers.
...
3. Generate cycle02 execution report artifact:
- `formal/output/master_action_variant_c_pressure_cycle02_execution_report_v0.json`
4. Generate cycle02 drift report vs cycle01:
