# Derivation Target: ToE Master-Action Variant-C Pressure Cycle04 v0

Spec ID:
- `DERIVATION_TARGET_TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE04_v0`

Target ID:
- `TARGET-TOE-MASTER-ACTION-VARIANT-C-PRESSURE-CYCLE04-v0`

Classification:
- `P-POLICY`

Purpose:
- Execute a fourth bounded pressure cycle on `VARIANT_C_SEAM_TRANSPORT_CROSS_TERM_v0`.
- Add explicit decision-flip eligibility criteria tags on priority lanes.
...
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE04_STATUS_v0: RUN_BOUNDED_v0_NONCLAIM`
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE04_PRIORITY_VARIANT_v0: VARIANT_C_SEAM_TRANSPORT_CROSS_TERM_v0`
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE04_CONTROL_VARIANT_v0: VARIANT_A_BASELINE_v0`
- `TOE_MASTER_ACTION_VARIANT_C_PRESSURE_CYCLE04_EXECUTION_GUARD_v0: PACKET02_REQUIRED_FIELDS_AND_M4_SEAM_POINTERS_PRESERVED`

Priority lanes (cycle04):
- `QFT`
- `SR`

Required payload metadata for packet-02 artifacts in this cycle:
- `master_action_variant_cycle_target_v0: VARIANT_C_PRESSURE_CYCLE04_v0`
- `master_action_variant_cycle_role_v0` in `{PRIORITY_LANE_v0, CONTROL_LANE_v0, REFERENCE_LANE_v0}`

Priority-lane flip-eligibility tagging rule (cycle04):
- set `master_action_variant_priority_threshold_profile_v0: STRICT_PRIORITY_THRESHOLD_CYCLE04_v0` on priority lanes.
- set `master_action_variant_flip_eligibility_rule_v0: TWO_CONSECUTIVE_DRIFT_WINDOWS_REQUIRED_v0` on priority lanes.
- set `master_action_variant_flip_eligibility_status_v0` in `{ELIGIBLE_v0, NOT_ELIGIBLE_v0}` on priority lanes.
...
3. Generate cycle04 execution report artifact:
- `formal/output/master_action_variant_c_pressure_cycle04_execution_report_v0.json`
4. Generate cycle04 drift report vs cycle03:
