...
Strict scalar/QFT derivation synchronization checkpoint (2026-05-01):
...
- QFT-GR source-map status: `QFT_GR_StressEnergyExpectationSourceMap.lean` defines the QFT stress-energy object, expectation/state functional, GR source object, covariance/conservation assumptions, and residual/error interface; supplied expectation/source and weak-curvature/source alignments construct a zero-residual source-map package. `QFT_GR_StressEnergySourceMapResidualOnlyObstruction.lean` proves residual-only zero evidence does not close full source-map semantics when the required semantic fields are false.
- Next strict cross-pillar target: QFT-GR has reached its two-slice retained-blocker attempt budget, so same-lane continuation requires post-budget cross-pillar review. A2A15A1/A2A15 closure, QM-STAT seam closure, QFT-GR seam closure, semiclassical gravity, Einstein-equation derivation, master-action promotion, empirical claims, and Phase 2 remain unauthorized.
