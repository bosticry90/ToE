...
- QM-STAT residual package status: `QM_STAT_TransportResidualPackage.lean` defines the source QM evolution, target STAT/entropy, transport map, preserved-quantity, and residual/error interface; existing finite transport theorems construct a zero-residual package only under supplied finite equivalence and alignment data.
