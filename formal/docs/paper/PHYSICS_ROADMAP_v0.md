...
- Next strict scalar/QFT target: finish A2A15A1A9 endpoint-package derivation by discharging or explicitly blocking centered endpoint coefficient alignment from mathlib and then the two-sided endpoint package construction. Uniform mesh convergence remains downstream until that endpoint package is fully derived or explicitly blocked.
