...
State snapshot canonical pointer: formal/output/LATEST_STATE_SNAPSHOT.txt

PAPER-READY-PHYSICS_v0 (governance gate)
- Purpose: define "physics/theory paper completion within power" as an auditable gate.
- Gate semantics: each dependency must be either theorem-level proved, pinned deterministic evidence, or explicitly blocked non-claim.
- Required pass criteria:
  - FN-DERIVE scope statement is explicit and references the three default-quotient lock artifacts.
  - RAC status is explicit: RAC is policy-level.
  - Claim taxonomy exists and is enforced against paper-facing inventory labels.
  - Paper draft includes scope/non-claims, taxonomy labels, and canonical snapshot pointer.
  - ToE claim surface is explicit and includes per-claim proof/derivation plans.
  - Theory spec includes degrees of freedom, action surface, symmetry/conservation map, and units posture.
  - First true derivation target is selected and expansion-freeze policy is explicit.

TOE-COMPLETE-TRACK_v0 (physics/theory completion lane)
- Claim surface document: `formal/docs/paper/TOE_CLAIM_SURFACE_v0.md`.
- Theory specification document: `formal/docs/paper/THEORY_SPEC_v1.md`.
- First true derivation target: `TOE-GR-01` via `formal/docs/paper/DERIVATION_TARGET_NEWTONIAN_LIMIT_v0.md`.
- First target equation: `nabla^2 Phi = kappa * rho` (weak-field scope only).
- TOE-GR-01 theorem surface: `formal/docs/paper/TOE_GR01_THEOREM_SURFACE_v0.md`; `formal/toe_formal/ToeFormal/Variational/WeakFieldPoissonLimit.lean`.
  - Non-vacuity checkpoint: `PoissonEquation1D` is concrete (discrete residual predicate), and theorem input no longer accepts `hPoissonLimit`.
  - Bridge checkpoint: theorem route uses explicit bridge components (`ProjectionMapWellFormed`, `WeakFieldTruncationSound`, `ELImpliesDiscretePoissonResidual`) and constructs `WeakFieldProjectionFromCore` via `mkWeakFieldProjectionFromCore`.
  - Operator transport checkpoint: `OperatorToDiscreteResidual` isolates discrete/operator bookkeeping from modeling assumptions.
- TOE-GR-01 projection bridge spec: `formal/docs/paper/TOE_GR01_PROJECTION_BRIDGE_SPEC_v0.md`.
- TOE-GR-01 derivation scaffolds: `formal/docs/paper/TOE_GR01_WEAK_FIELD_EXPANSION_NOTE_v0.md`; `formal/docs/paper/TOE_GR01_POTENTIAL_IDENTIFICATION_v0.md`; `formal/docs/paper/TOE_GR01_LAPLACIAN_EXTRACTION_v0.md`.
- TOE-GR-01 kappa calibration protocol: `formal/docs/paper/TOE_GR01_KAPPA_CALIBRATION_v0.md`.
  - Deterministic output artifact: `formal/output/toe_gr01_kappa_calibration_v0.json`.
  - Calibration lock: `formal/markdown/locks/functionals/TOE-GR-01_kappa_calibration_v0.md`.
- Freeze policy: No new comparator lanes are authorized until TOE-GR-01 reaches derivation-grade closure.
- Scope note: this lane defines internal completion posture only; no external truth claim.

Canonical claims inventory (paper-facing)
- [T-PROVED] `RAC_Rep32_Cubic_of_obligation_set` and `RACRep32CubicObligationSet_of_RAC` (`formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean`).
- [T-CONDITIONAL] `EL_toe_rep32_equals_Pcubic_under_default_quotient_assumptions` (`formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean`; lock `formal/markdown/locks/functionals/FN-DERIVE_EL_toe_rep32_equals_Pcubic_under_default_quotient_assumptions_v0.md`).
- [T-CONDITIONAL] `ActionRep32FiniteDifferenceRepresentsP_declared_g_of_action_and_obligation_set` (`formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean`; lock `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_bridge_discharge_object_v0.md`).
- [E-REPRO] CT-09 independent external-anchor sound-speed lock (`formal/markdown/locks/observables/CT-09_independent_external_anchor_sound_speed_v0.md`).
- [E-REPRO] CT-10 deterministic pre-activation selection verdict lock (`formal/markdown/locks/observables/OV-SEL-CT10-01_independent_anchor_selection_verdict.md`).
- [E-REPRO] CX-01 update contractivity lock (`formal/markdown/locks/observables/CX-01_update_contractivity_v0.md`).
- [E-REPRO] TOE-GR-01 kappa calibration deterministic artifact + lock (`formal/output/toe_gr01_kappa_calibration_v0.json`; `formal/markdown/locks/functionals/TOE-GR-01_kappa_calibration_v0.md`).
- [P-POLICY] RAC completion policy is explicitly policy-level pending promotion (`State_of_the_Theory.md`).
- [P-POLICY] Comparator expansion remains policy-gated pending FN-DERIVE exit (`State_of_the_Theory.md`).
- [P-POLICY] ToE claim surface and theory spec are governance-frozen planning artifacts (`formal/docs/paper/TOE_CLAIM_SURFACE_v0.md`; `formal/docs/paper/THEORY_SPEC_v1.md`).
- [P-POLICY] TOE-GR-01 projection map and weak-field truncation assumptions are explicit policy-level bridge components (`ProjectionMapWellFormed`; `WeakFieldTruncationSound`; `formal/docs/paper/TOE_GR01_PROJECTION_BRIDGE_SPEC_v0.md`).
- [P-POLICY] TOE-GR-01 modeling bridge `ELImpliesDiscretePoissonResidual` is explicit and policy-scoped in v0 (`formal/docs/paper/TOE_GR01_PROJECTION_BRIDGE_SPEC_v0.md`; `formal/toe_formal/ToeFormal/Variational/WeakFieldPoissonLimit.lean`).
- [B-BLOCKED] Theorem-complete RAC promotion is not yet discharged (`State_of_the_Theory.md`).
- [B-BLOCKED] Full analytic retirement of default-path action/RAC assumptions remains not-yet-discharged (`State_of_the_Theory.md`).
- [B-BLOCKED] TOE-GR-01 weak-field derivation upgrade remains in progress (`formal/docs/paper/DERIVATION_TARGET_NEWTONIAN_LIMIT_v0.md`).
- [B-BLOCKED] TOE-GR-01 theorem surface is non-vacuous and no longer tautological, but analytic weak-field Poisson derivation is not yet discharged (`formal/docs/paper/TOE_GR01_THEOREM_SURFACE_v0.md`; `formal/toe_formal/ToeFormal/Variational/WeakFieldPoissonLimit.lean`).
- [B-BLOCKED] TOE-GR-01 theorem-level promotion of the modeling bridge remains open (future path: promote `ELImpliesDiscretePoissonResidual` from policy to theorem surface).
- [B-BLOCKED] TOE-GR-01 expansion/potential/operator extraction notes are scaffold-only pending formal discharge (`formal/docs/paper/TOE_GR01_WEAK_FIELD_EXPANSION_NOTE_v0.md`; `formal/docs/paper/TOE_GR01_POTENTIAL_IDENTIFICATION_v0.md`; `formal/docs/paper/TOE_GR01_LAPLACIAN_EXTRACTION_v0.md`).
...
Guiding roadmap for post-RL work; non-claim-bearing.
Evidence: formal/docs/programs/INEVITABILITY_ROADMAP_v0.md

Conditional theorems program (post-RL, non-claim-bearing).
Evidence: formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md
...
Status: In progress (surjectivity discharged; pairing contract localized; nondegeneracy proven for grid model; Rep32-first uniqueness uses the Rep32 pairing contract; Field2DRep uniqueness is routed through the explicit quotient pairing contract; Rep32 bridge now includes a non-tautological EL/P derivation route from bridge + finite-difference assumptions; Field2D nondegeneracy remains assumed only in the legacy Field2D lane)
Owner ticket/module: formal/toe_formal/ToeFormal/Variational/PairingContract.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationUniqueness.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationDeclared.lean; formal/toe_formal/ToeFormal/Variational/DiscreteField.lean; formal/toe_formal/ToeFormal/Variational/PairingConcreteGrid.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationRepDefFieldRep.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationUniquenessFieldRep.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationDeclaredFieldRep.lean; formal/toe_formal/ToeFormal/Variational/FieldRepresentationPairingContract.lean; formal/toe_formal/ToeFormal/Variational/ActionToFirstVariationBridgeRep32.lean
Evidence path: formal/toe_formal/ToeFormal/Variational/PairingContract.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationUniqueness.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationDeclared.lean; formal/toe_formal/ToeFormal/Variational/DiscreteField.lean; formal/toe_formal/ToeFormal/Variational/PairingConcreteGrid.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationRepDefFieldRep.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationUniquenessFieldRep.lean; formal/toe_formal/ToeFormal/Variational/FirstVariationDeclaredFieldRep.lean; formal/toe_formal/ToeFormal/Variational/FieldRepresentationPairingContract.lean; formal/toe_formal/ToeFormal/Variational/ActionToFirstVariationBridgeRep32.lean; formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean; formal/markdown/locks/functionals/FN-DERIVE_default_quotient_hAction_provenance_v0.md; formal/markdown/locks/functionals/FN-DERIVE_default_quotient_hRAC_obligation_bundle_v0.md; formal/markdown/locks/functionals/FN-DERIVE_default_quotient_bridge_discharge_object_v0.md; formal/markdown/locks/functionals/FN-DERIVE_EL_toe_rep32_equals_Pcubic_under_default_quotient_assumptions_v0.md; formal/python/tests/test_lean_non_tautological_el_derivation_rep32.py; formal/python/tests/test_lean_non_tautological_el_derivation_rep32_build_guard.py; formal/python/tests/test_fn_derive_default_quotient_discharge_artifacts.py
...
Rep32 non-tautological bridge route (2026-02-12): `ActionRep32ELMatchesP_of_bridge_and_finiteDifferenceRepresentsP` derives EL/P identification from bridge + finite-difference assumptions using `pairingRep32_nondegenerate'` and `variations_surjective_const_rep32`, without using `EL_toe_eq_P_rep32`.
...
Rep32 cubic lane (RAC-scoped): `ActionRep32CubicLane.lean` packages `Rep32CubicLaneAssumptions` (including `ε ≠ 0`, `actionRep32.action = actionRep32Cubic g`, `P_rep32 = P_cubic_rep32_def g`, and `RAC_Rep32_Cubic g`) and provides the default wrapper `Rep32_cubic_lane_declared_g`, yielding `ActionVariationBridgeRep32At ε hε` and `EL_toe_rep32 = P_cubic_rep32_def declared_g_rep32`; default-quotient specialization `ActionRep32FiniteDifferenceRepresentsP_declared_g_of_action_and_RAC` now discharges the `hP` binding via `P_rep32_def`, leaving action/RAC assumptions as the remaining bridge-side obligations.
FN-DERIVE scope statement (paper-facing): default-quotient EL-from-first-variation is represented as a conditional theorem surface only; RAC is policy-level.
FN-DERIVE paper-facing theorem (T-CONDITIONAL): `EL_toe_rep32_equals_Pcubic_under_default_quotient_assumptions` in `formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean`.
FN-DERIVE default-quotient lock set: `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_hAction_provenance_v0.md`; `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_hRAC_obligation_bundle_v0.md`; `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_bridge_discharge_object_v0.md`; `formal/markdown/locks/functionals/FN-DERIVE_EL_toe_rep32_equals_Pcubic_under_default_quotient_assumptions_v0.md`.
...
GapID: RELEASE-PORTFOLIO-01
Layer: Governance
Item: Regime lanes portfolio release note v0
Status: Implemented (portfolio doc + doc-gate)
Owner ticket/module: formal/docs/release/REGIME_LANES_PORTFOLIO_v0.md
Evidence path: formal/docs/release/REGIME_LANES_PORTFOLIO_v0.md; formal/python/tests/test_regime_lanes_portfolio_doc.py
Exit criteria: Portfolio note records RL01-RL12 locks, artifacts, and claim boundary; doc gate is green.
Notes: This is a release-facing summary of evidence objects; no new physics claims.
...
Exit criteria: Deterministic record + blocked-on-missing; reason-coded negative controls; surface contract freeze tests enforced.
Notes: `OV-RL-02` is `discriminative_candidate` and `within_rep_only`; cross-rep promotion is blocked pending non-alias eligibility proofs; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-03
Layer: Governance
Item: Weak-field Poisson limit lane v0
Status: Implemented (front door record + negative controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl03_weak_field_poisson_v0.py
Evidence path: formal/quarantine/physics_target/RL03_weak_field_poisson_v0_SPEC.md; formal/python/tests/test_weak_field_poisson_lane_doc.py; formal/python/toe/comparators/rl03_weak_field_poisson_v0.py; formal/python/tools/rl03_weak_field_poisson_front_door.py; formal/external_evidence/rl03_weak_field_poisson_domain_01/rl03_reference_report.json; formal/external_evidence/rl03_weak_field_poisson_domain_01/rl03_candidate_report.json; formal/python/tests/test_rl03_weak_field_poisson_v0_front_door.py; formal/python/tests/test_rl03_weak_field_poisson_v0_surface_contract_freeze.py; formal/python/tests/test_rl03_weak_field_poisson_v0_pinned_artifacts.py; formal/python/tests/test_rl03_weak_field_poisson_v0_lock.py; formal/markdown/locks/observables/OV-RL-03_weak_field_poisson_v0.md; formal/docs/rl03_weak_field_poisson_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; reason-coded negative controls; surface contract freeze tests enforced.
Notes: This lane asserts a bounded Poisson-type correspondence only; no GR/EFE or external gravity claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-04
Layer: Governance
Item: Continuity equation comparator lane v0
Status: Implemented (front door record + negative controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl04_continuity_v0.py
Evidence path: formal/python/toe/comparators/rl04_continuity_v0.py; formal/python/tools/rl04_continuity_front_door.py; formal/external_evidence/rl04_continuity_domain_01/rl04_reference_report.json; formal/external_evidence/rl04_continuity_domain_01/rl04_candidate_report.json; formal/python/tests/test_rl04_continuity_v0_front_door.py; formal/python/tests/test_rl04_continuity_v0_surface_contract_freeze.py; formal/python/tests/test_rl04_continuity_v0_pinned_artifacts.py; formal/python/tests/test_rl04_continuity_v0_lock.py; formal/markdown/locks/observables/OV-RL-04_continuity_v0.md; formal/docs/rl04_continuity_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; reason-coded negative controls; surface contract freeze tests enforced.
Notes: This lane asserts a bounded continuity-equation correspondence only; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-05
Layer: Governance
Item: Wave equation comparator lane v0
Status: Implemented (front door record + negative controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl05_wave_equation_v0.py
Evidence path: formal/python/toe/comparators/rl05_wave_equation_v0.py; formal/python/tools/rl05_wave_equation_front_door.py; formal/external_evidence/rl05_wave_equation_domain_01/rl05_reference_report.json; formal/external_evidence/rl05_wave_equation_domain_01/rl05_candidate_report.json; formal/python/tests/test_rl05_wave_equation_v0_front_door.py; formal/python/tests/test_rl05_wave_equation_v0_surface_contract_freeze.py; formal/python/tests/test_rl05_wave_equation_v0_pinned_artifacts.py; formal/python/tests/test_rl05_wave_equation_v0_lock.py; formal/markdown/locks/observables/OV-RL-05_wave_equation_v0.md; formal/docs/rl05_wave_equation_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; reason-coded negative controls; surface contract freeze tests enforced.
Notes: This lane asserts a bounded wave-equation correspondence only; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-06
Layer: Governance
Item: Phase winding quantization comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl06_phase_winding_quantization_v0.py
Evidence path: formal/python/toe/comparators/rl06_phase_winding_quantization_v0.py; formal/python/tools/rl06_phase_winding_quantization_front_door.py; formal/external_evidence/rl06_phase_winding_quantization_domain_01/rl06_reference_report.json; formal/external_evidence/rl06_phase_winding_quantization_domain_01/rl06_candidate_report.json; formal/python/tests/test_rl06_phase_winding_quantization_v0_front_door.py; formal/python/tests/test_rl06_phase_winding_quantization_v0_surface_contract_freeze.py; formal/python/tests/test_rl06_phase_winding_quantization_v0_pinned_artifacts.py; formal/python/tests/test_rl06_phase_winding_quantization_v0_lock.py; formal/markdown/locks/observables/OV-RL-06_phase_winding_quantization_v0.md; formal/docs/rl06_phase_winding_quantization_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; negative control detection; surface contract freeze tests enforced.
Notes: This lane asserts bounded integer-winding detection on a periodic ring only; negative control pass is expectation-aware (non-integer winding is a pass); no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-07
Layer: Governance
Item: Energy/Noether conservation comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl07_energy_noether_conservation_v0.py
Evidence path: formal/docs/rl07_energy_noether_conservation_lane_spec.md; formal/python/tests/test_rl07_energy_noether_lane_doc.py; formal/python/toe/comparators/rl07_energy_noether_conservation_v0.py; formal/python/tools/rl07_energy_noether_conservation_front_door.py; formal/external_evidence/rl07_energy_noether_conservation_domain_01/rl07_reference_report.json; formal/external_evidence/rl07_energy_noether_conservation_domain_01/rl07_candidate_report.json; formal/python/tests/test_rl07_energy_noether_conservation_v0_front_door.py; formal/python/tests/test_rl07_energy_noether_conservation_v0_surface_contract_freeze.py; formal/python/tests/test_rl07_energy_noether_conservation_v0_pinned_artifacts.py; formal/python/tests/test_rl07_energy_noether_conservation_v0_lock.py; formal/markdown/locks/observables/OV-RL-07_energy_noether_conservation_v0.md; formal/docs/rl07_energy_noether_conservation_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded discrete energy conservation vs dissipative break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-08
Layer: Governance
Item: Gauge redundancy invariance comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl08_gauge_redundancy_invariance_v0.py
Evidence path: formal/docs/rl08_gauge_redundancy_invariance_lane_spec.md; formal/python/tests/test_rl08_gauge_redundancy_lane_doc.py; formal/python/toe/comparators/rl08_gauge_redundancy_invariance_v0.py; formal/python/tools/rl08_gauge_redundancy_invariance_front_door.py; formal/external_evidence/rl08_gauge_redundancy_invariance_domain_01/rl08_reference_report.json; formal/external_evidence/rl08_gauge_redundancy_invariance_domain_01/rl08_candidate_report.json; formal/python/tests/test_rl08_gauge_redundancy_invariance_v0_front_door.py; formal/python/tests/test_rl08_gauge_redundancy_invariance_v0_surface_contract_freeze.py; formal/python/tests/test_rl08_gauge_redundancy_invariance_v0_pinned_artifacts.py; formal/python/tests/test_rl08_gauge_redundancy_invariance_v0_lock.py; formal/markdown/locks/observables/OV-RL-08_gauge_redundancy_invariance_v0.md; formal/docs/rl08_gauge_redundancy_invariance_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded gauge redundancy invariance vs explicit break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-09
Layer: Governance
Item: Dispersion-topology bridge comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl09_dispersion_topology_bridge_v0.py
Evidence path: formal/docs/rl09_dispersion_topology_bridge_lane_spec.md; formal/python/tests/test_rl09_dispersion_topology_lane_doc.py; formal/python/toe/comparators/rl09_dispersion_topology_bridge_v0.py; formal/python/tools/rl09_dispersion_topology_bridge_front_door.py; formal/external_evidence/rl09_dispersion_topology_bridge_domain_01/rl09_reference_report.json; formal/external_evidence/rl09_dispersion_topology_bridge_domain_01/rl09_candidate_report.json; formal/python/tests/test_rl09_dispersion_topology_bridge_v0_front_door.py; formal/python/tests/test_rl09_dispersion_topology_bridge_v0_surface_contract_freeze.py; formal/python/tests/test_rl09_dispersion_topology_bridge_v0_pinned_artifacts.py; formal/python/tests/test_rl09_dispersion_topology_bridge_v0_lock.py; formal/markdown/locks/observables/OV-RL-09_dispersion_topology_bridge_v0.md; formal/docs/rl09_dispersion_topology_bridge_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded band-topology winding vs dispersion structure only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-10
Layer: Governance
Item: Entropy balance comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl10_entropy_balance_v0.py
Evidence path: formal/docs/rl10_entropy_balance_lane_spec.md; formal/python/tests/test_rl10_entropy_balance_lane_doc.py; formal/python/toe/comparators/rl10_entropy_balance_v0.py; formal/python/tools/rl10_entropy_balance_front_door.py; formal/external_evidence/rl10_entropy_balance_domain_01/rl10_reference_report.json; formal/external_evidence/rl10_entropy_balance_domain_01/rl10_candidate_report.json; formal/python/tests/test_rl10_entropy_balance_v0_front_door.py; formal/python/tests/test_rl10_entropy_balance_v0_surface_contract_freeze.py; formal/python/tests/test_rl10_entropy_balance_v0_pinned_artifacts.py; formal/python/tests/test_rl10_entropy_balance_v0_lock.py; formal/markdown/locks/observables/OV-RL-10_entropy_balance_v0.md; formal/docs/rl10_entropy_balance_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded detailed-balance behavior vs irreversible circulation only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-11
Layer: Governance
Item: Causality signal-cone comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl11_causality_signal_cone_v0.py
Evidence path: formal/docs/rl11_causality_signal_cone_lane_spec.md; formal/python/tests/test_rl11_causality_signal_cone_lane_doc.py; formal/python/toe/comparators/rl11_causality_signal_cone_v0.py; formal/python/tools/rl11_causality_signal_cone_front_door.py; formal/external_evidence/rl11_causality_signal_cone_domain_01/rl11_reference_report.json; formal/external_evidence/rl11_causality_signal_cone_domain_01/rl11_candidate_report.json; formal/python/tests/test_rl11_causality_signal_cone_v0_front_door.py; formal/python/tests/test_rl11_causality_signal_cone_v0_surface_contract_freeze.py; formal/python/tests/test_rl11_causality_signal_cone_v0_pinned_artifacts.py; formal/python/tests/test_rl11_causality_signal_cone_v0_lock.py; formal/markdown/locks/observables/OV-RL-11_causality_signal_cone_v0.md; formal/docs/rl11_causality_signal_cone_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded signal-cone admissibility vs CFL-violating leakage only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-12
Layer: Governance
Item: Lorentz/Poincare invariance proxy comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl12_lorentz_poincare_invariance_proxy_v0.py
Evidence path: formal/docs/rl12_lorentz_poincare_invariance_proxy_lane_spec.md; formal/python/tests/test_rl12_lorentz_poincare_lane_doc.py; formal/python/toe/comparators/rl12_lorentz_poincare_invariance_proxy_v0.py; formal/python/tools/rl12_lorentz_poincare_invariance_proxy_front_door.py; formal/external_evidence/rl12_lorentz_poincare_invariance_proxy_domain_01/rl12_reference_report.json; formal/external_evidence/rl12_lorentz_poincare_invariance_proxy_domain_01/rl12_candidate_report.json; formal/python/tests/test_rl12_lorentz_poincare_invariance_proxy_v0_front_door.py; formal/python/tests/test_rl12_lorentz_poincare_invariance_proxy_v0_surface_contract_freeze.py; formal/python/tests/test_rl12_lorentz_poincare_invariance_proxy_v0_pinned_artifacts.py; formal/python/tests/test_rl12_lorentz_poincare_invariance_proxy_v0_lock.py; formal/markdown/locks/observables/OV-RL-12_lorentz_poincare_invariance_proxy_v0.md; formal/docs/rl12_lorentz_poincare_invariance_proxy_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded Lorentz/Poincare boost invariance vs explicit break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-13
Layer: Governance
Item: Velocity-addition group-law comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl13_velocity_addition_group_law_v0.py
Evidence path: formal/docs/rl13_velocity_addition_group_law_lane_spec.md; formal/python/tests/test_rl13_velocity_addition_lane_doc.py; formal/python/toe/comparators/rl13_velocity_addition_group_law_v0.py; formal/python/tools/rl13_velocity_addition_group_law_front_door.py; formal/external_evidence/rl13_velocity_addition_group_law_domain_01/rl13_reference_report.json; formal/external_evidence/rl13_velocity_addition_group_law_domain_01/rl13_candidate_report.json; formal/python/tests/test_rl13_velocity_addition_group_law_v0_front_door.py; formal/python/tests/test_rl13_velocity_addition_group_law_v0_surface_contract_freeze.py; formal/python/tests/test_rl13_velocity_addition_group_law_v0_pinned_artifacts.py; formal/python/tests/test_rl13_velocity_addition_group_law_v0_lock.py; formal/markdown/locks/observables/OV-RL-13_velocity_addition_group_law_v0.md; formal/docs/rl13_velocity_addition_group_law_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded velocity-addition group-law consistency vs explicit break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-14
Layer: Governance
Item: Time-dilation proxy comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl14_time_dilation_proxy_v0.py
Evidence path: formal/docs/rl14_time_dilation_proxy_lane_spec.md; formal/python/tests/test_rl14_time_dilation_lane_doc.py; formal/python/toe/comparators/rl14_time_dilation_proxy_v0.py; formal/python/tools/rl14_time_dilation_proxy_front_door.py; formal/external_evidence/rl14_time_dilation_proxy_domain_01/rl14_reference_report.json; formal/external_evidence/rl14_time_dilation_proxy_domain_01/rl14_candidate_report.json; formal/python/tests/test_rl14_time_dilation_proxy_v0_front_door.py; formal/python/tests/test_rl14_time_dilation_proxy_v0_surface_contract_freeze.py; formal/python/tests/test_rl14_time_dilation_proxy_v0_pinned_artifacts.py; formal/python/tests/test_rl14_time_dilation_proxy_v0_lock.py; formal/markdown/locks/observables/OV-RL-14_time_dilation_proxy_v0.md; formal/docs/rl14_time_dilation_proxy_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded time-dilation proxy vs no-dilation break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-15
Layer: Governance
Item: Length-contraction proxy comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl15_length_contraction_proxy_v0.py
Evidence path: formal/docs/rl15_length_contraction_proxy_lane_spec.md; formal/python/tests/test_rl15_length_contraction_lane_doc.py; formal/python/toe/comparators/rl15_length_contraction_proxy_v0.py; formal/python/tools/rl15_length_contraction_proxy_front_door.py; formal/external_evidence/rl15_length_contraction_proxy_domain_01/rl15_reference_report.json; formal/external_evidence/rl15_length_contraction_proxy_domain_01/rl15_candidate_report.json; formal/python/tests/test_rl15_length_contraction_proxy_v0_front_door.py; formal/python/tests/test_rl15_length_contraction_proxy_v0_surface_contract_freeze.py; formal/python/tests/test_rl15_length_contraction_proxy_v0_pinned_artifacts.py; formal/python/tests/test_rl15_length_contraction_proxy_v0_lock.py; formal/markdown/locks/observables/OV-RL-15_length_contraction_proxy_v0.md; formal/docs/rl15_length_contraction_proxy_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded length-contraction proxy vs no-contraction break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-RL-16
Layer: Governance
Item: Relativity-of-simultaneity proxy comparator lane v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/rl16_relativity_of_simultaneity_proxy_v0.py
Evidence path: formal/docs/rl16_relativity_of_simultaneity_proxy_lane_spec.md; formal/python/tests/test_rl16_relativity_of_simultaneity_lane_doc.py; formal/python/toe/comparators/rl16_relativity_of_simultaneity_proxy_v0.py; formal/python/tools/rl16_relativity_of_simultaneity_proxy_front_door.py; formal/external_evidence/rl16_relativity_of_simultaneity_proxy_domain_01/rl16_reference_report.json; formal/external_evidence/rl16_relativity_of_simultaneity_proxy_domain_01/rl16_candidate_report.json; formal/python/tests/test_rl16_relativity_of_simultaneity_proxy_v0_front_door.py; formal/python/tests/test_rl16_relativity_of_simultaneity_proxy_v0_surface_contract_freeze.py; formal/python/tests/test_rl16_relativity_of_simultaneity_proxy_v0_pinned_artifacts.py; formal/python/tests/test_rl16_relativity_of_simultaneity_proxy_v0_lock.py; formal/markdown/locks/observables/OV-RL-16_relativity_of_simultaneity_proxy_v0.md; formal/docs/rl16_relativity_of_simultaneity_proxy_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This lane asserts bounded simultaneity-shift proxy vs no-shift break only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed). Hard stop: RL16 closes this lane sequence; further expansion requires explicit governance reset.

GapID: COMP-CT-01
Layer: Governance
Item: Conditional theorem CT-01 no-superluminal propagation v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct01_no_superluminal_propagation_v0.py
Evidence path: formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md; formal/python/tests/test_conditional_theorems_program_doc.py; formal/python/toe/comparators/ct01_no_superluminal_propagation_v0.py; formal/python/tools/ct01_no_superluminal_propagation_front_door.py; formal/external_evidence/ct01_no_superluminal_propagation_domain_01/ct01_reference_report.json; formal/external_evidence/ct01_no_superluminal_propagation_domain_01/ct01_candidate_report.json; formal/python/tests/test_ct01_no_superluminal_propagation_v0_front_door.py; formal/python/tests/test_ct01_no_superluminal_propagation_v0_surface_contract_freeze.py; formal/python/tests/test_ct01_no_superluminal_propagation_v0_pinned_artifacts.py; formal/python/tests/test_ct01_no_superluminal_propagation_v0_lock.py; formal/markdown/locks/observables/CT-01_no_superluminal_propagation_v0.md; formal/docs/ct01_no_superluminal_propagation_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This conditional theorem asserts bounded internal propagation vs signal-cone bound only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-02
Layer: Governance
Item: Conditional theorem CT-02 energy/causality update bounds v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct02_energy_causality_update_bounds_v0.py
Evidence path: formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md; formal/python/tests/test_conditional_theorems_program_doc.py; formal/python/toe/comparators/ct02_energy_causality_update_bounds_v0.py; formal/python/tools/ct02_energy_causality_update_bounds_front_door.py; formal/external_evidence/ct02_energy_causality_update_bounds_domain_01/ct02_reference_report.json; formal/external_evidence/ct02_energy_causality_update_bounds_domain_01/ct02_candidate_report.json; formal/python/tests/test_ct02_energy_causality_update_bounds_v0_front_door.py; formal/python/tests/test_ct02_energy_causality_update_bounds_v0_surface_contract_freeze.py; formal/python/tests/test_ct02_energy_causality_update_bounds_v0_pinned_artifacts.py; formal/python/tests/test_ct02_energy_causality_update_bounds_v0_lock.py; formal/markdown/locks/observables/CT-02_energy_causality_update_bounds_v0.md; formal/docs/ct02_energy_causality_update_bounds_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This conditional theorem asserts bounded internal energy drift plus CFL admissibility only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-03
Layer: Governance
Item: Conditional theorem CT-03 energy/causality representation variant v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct03_energy_causality_rep_variant_v0.py
Evidence path: formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md; formal/python/tests/test_conditional_theorems_program_doc.py; formal/python/toe/comparators/ct03_energy_causality_rep_variant_v0.py; formal/python/tools/ct03_energy_causality_rep_variant_front_door.py; formal/external_evidence/ct03_energy_causality_rep_variant_domain_01/ct03_reference_report.json; formal/external_evidence/ct03_energy_causality_rep_variant_domain_01/ct03_candidate_report.json; formal/python/tests/test_ct03_energy_causality_rep_variant_v0_front_door.py; formal/python/tests/test_ct03_energy_causality_rep_variant_v0_surface_contract_freeze.py; formal/python/tests/test_ct03_energy_causality_rep_variant_v0_pinned_artifacts.py; formal/python/tests/test_ct03_energy_causality_rep_variant_v0_lock.py; formal/markdown/locks/observables/CT-03_energy_causality_rep_variant_v0.md; formal/docs/ct03_energy_causality_rep_variant_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This conditional theorem asserts energy-drift and CFL admissibility under an alternate energy representation only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-04
Layer: Governance
Item: Conditional theorem CT-04 minimality no-go v0
Status: Implemented (front door record + negative control + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct04_minimality_no_go_v0.py
Evidence path: formal/docs/programs/CT04_minimality_no_go_lane_spec.md; formal/python/tests/test_ct04_minimality_no_go_lane_doc.py; formal/python/toe/comparators/ct04_minimality_no_go_v0.py; formal/python/tools/ct04_minimality_no_go_front_door.py; formal/external_evidence/ct04_minimality_no_go_domain_01/ct04_reference_report.json; formal/external_evidence/ct04_minimality_no_go_domain_01/ct04_candidate_report.json; formal/python/tests/test_ct04_minimality_no_go_v0_front_door.py; formal/python/tests/test_ct04_minimality_no_go_v0_surface_contract_freeze.py; formal/python/tests/test_ct04_minimality_no_go_v0_pinned_artifacts.py; formal/python/tests/test_ct04_minimality_no_go_v0_lock.py; formal/markdown/locks/observables/CT-04_minimality_no_go_v0.md; formal/docs/ct04_minimality_no_go_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This conditional theorem asserts a minimality no-go under CFL bound removal only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-05
Layer: Governance
Item: Conditional theorem CT-05 representation-invariant admissibility class v0
Status: Implemented (front door record + negative controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct05_rep_invariant_admissibility_class_v0.py
Evidence path: formal/docs/programs/CT05_rep_invariant_admissibility_class_lane_spec.md; formal/python/tests/test_ct05_rep_invariant_admissibility_lane_doc.py; formal/python/toe/comparators/ct05_rep_invariant_admissibility_class_v0.py; formal/python/tools/ct05_rep_invariant_admissibility_class_front_door.py; formal/external_evidence/ct05_rep_invariant_admissibility_class_domain_01/ct05_reference_report.json; formal/external_evidence/ct05_rep_invariant_admissibility_class_domain_01/ct05_candidate_report.json; formal/python/tests/test_ct05_rep_invariant_admissibility_class_v0_front_door.py; formal/python/tests/test_ct05_rep_invariant_admissibility_class_v0_surface_contract_freeze.py; formal/python/tests/test_ct05_rep_invariant_admissibility_class_v0_front_door_contract_freeze.py; formal/python/tests/test_ct05_rep_invariant_admissibility_class_v0_pinned_artifacts.py; formal/python/tests/test_ct05_rep_invariant_admissibility_class_v0_lock.py; formal/markdown/locks/observables/CT-05_rep_invariant_admissibility_class_v0.md; formal/docs/ct05_rep_invariant_admissibility_class_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics; surface contract freeze tests enforced.
Notes: This conditional theorem asserts representation-invariant admissibility and bound status under CT-02/CT-03 plus RL11 only; negative control pass is expectation-aware; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-06
Layer: Governance
Item: Conditional theorem CT-06 external-anchor dispersion v0
Status: Implemented (front door record + expectation-aware controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct06_external_anchor_dispersion_v0.py
Evidence path: formal/docs/programs/CT06_external_anchor_dispersion_v0.md; formal/python/tests/test_ct06_external_anchor_dispersion_lane_doc.py; formal/python/toe/comparators/ct06_external_anchor_dispersion_v0.py; formal/python/tools/ct06_external_anchor_dispersion_front_door.py; formal/external_evidence/ct06_external_anchor_dispersion_domain_01/dispersion_anchor_steinhauer_fig3a_omega_k.csv; formal/external_evidence/ct06_external_anchor_dispersion_domain_01/dataset_metadata.json; formal/external_evidence/ct06_external_anchor_dispersion_domain_01/source_citation.md; formal/external_evidence/ct06_external_anchor_dispersion_domain_01/ct06_reference_report.json; formal/external_evidence/ct06_external_anchor_dispersion_domain_01/ct06_candidate_report.json; formal/python/tests/test_ct06_external_anchor_dispersion_v0_front_door.py; formal/python/tests/test_ct06_external_anchor_dispersion_v0_surface_contract_freeze.py; formal/python/tests/test_ct06_external_anchor_dispersion_v0_front_door_contract_freeze.py; formal/python/tests/test_ct06_external_anchor_dispersion_v0_pinned_artifacts.py; formal/python/tests/test_ct06_external_anchor_dispersion_v0_lock.py; formal/markdown/locks/observables/CT-06_external_anchor_dispersion_v0.md; formal/docs/ct06_external_anchor_dispersion_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics for positive/negative controls; dataset-conditional tolerance checks enforced.
Notes: This conditional theorem asserts external-anchor dispersion reproducibility under a pinned dataset and preprocessing only; negative control pass is expectation-aware failure detection; no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-07
Layer: Governance
Item: Conditional theorem CT-07 external-anchor dispersion low-k slice v0
Status: Implemented (front door record + expectation-aware controls + low-k slice surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct07_external_anchor_dispersion_lowk_slice_v0.py
Evidence path: formal/docs/programs/CT07_external_anchor_dispersion_lowk_slice_v0.md; formal/python/tests/test_ct07_external_anchor_dispersion_lowk_slice_lane_doc.py; formal/python/toe/comparators/ct07_external_anchor_dispersion_lowk_slice_v0.py; formal/python/tools/ct07_external_anchor_dispersion_lowk_front_door.py; formal/external_evidence/ct07_external_anchor_dispersion_lowk_domain_01/dataset_metadata.json; formal/external_evidence/ct07_external_anchor_dispersion_lowk_domain_01/source_citation.md; formal/external_evidence/ct07_external_anchor_dispersion_lowk_domain_01/ct07_reference_report.json; formal/external_evidence/ct07_external_anchor_dispersion_lowk_domain_01/ct07_candidate_report.json; formal/python/tests/test_ct07_external_anchor_dispersion_lowk_slice_v0_front_door.py; formal/python/tests/test_ct07_external_anchor_dispersion_lowk_slice_v0_surface_contract_freeze.py; formal/python/tests/test_ct07_external_anchor_dispersion_lowk_slice_v0_front_door_contract_freeze.py; formal/python/tests/test_ct07_external_anchor_dispersion_lowk_slice_v0_pinned_artifacts.py; formal/python/tests/test_ct07_external_anchor_dispersion_lowk_slice_v0_lock.py; formal/markdown/locks/observables/CT-07_external_anchor_dispersion_lowk_slice_v0.md; formal/docs/ct07_external_anchor_dispersion_lowk_slice_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics for positive/negative controls on the low-k slice; explicit non-independence from CT-06 enforced in scope.
Notes: This conditional theorem asserts low-k slice external-anchor reproducibility under pinned quantile slicing only; it is explicitly non-independent of CT-06 (`non_independent_of_CT06`); no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-08
Layer: Governance
Item: Conditional theorem CT-08 external-anchor dispersion high-k slice v0
Status: Implemented (front door record + expectation-aware controls + high-k slice surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct08_external_anchor_dispersion_highk_slice_v0.py
Evidence path: formal/docs/programs/CT08_external_anchor_dispersion_highk_slice_v0.md; formal/python/tests/test_ct08_external_anchor_dispersion_highk_slice_lane_doc.py; formal/python/toe/comparators/ct08_external_anchor_dispersion_highk_slice_v0.py; formal/python/tools/ct08_external_anchor_dispersion_highk_front_door.py; formal/external_evidence/ct08_external_anchor_dispersion_highk_domain_01/dataset_metadata.json; formal/external_evidence/ct08_external_anchor_dispersion_highk_domain_01/source_citation.md; formal/external_evidence/ct08_external_anchor_dispersion_highk_domain_01/ct08_reference_report.json; formal/external_evidence/ct08_external_anchor_dispersion_highk_domain_01/ct08_candidate_report.json; formal/python/tests/test_ct08_external_anchor_dispersion_highk_slice_v0_front_door.py; formal/python/tests/test_ct08_external_anchor_dispersion_highk_slice_v0_surface_contract_freeze.py; formal/python/tests/test_ct08_external_anchor_dispersion_highk_slice_v0_front_door_contract_freeze.py; formal/python/tests/test_ct08_external_anchor_dispersion_highk_slice_v0_pinned_artifacts.py; formal/python/tests/test_ct08_external_anchor_dispersion_highk_slice_v0_lock.py; formal/python/tests/test_ct08_files_do_not_contain_ct07_tokens.py; formal/markdown/locks/observables/CT-08_external_anchor_dispersion_highk_slice_v0.md; formal/docs/ct08_external_anchor_dispersion_highk_slice_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics for positive/negative controls on the high-k slice; explicit non-independence from CT-06 enforced in scope.
Notes: This conditional theorem asserts high-k slice external-anchor reproducibility under pinned quantile slicing only; it is explicitly non-independent of CT-06 (`non_independent_of_CT06`); no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-CLUSTER-01
Layer: Governance
Item: CT-06/CT-07/CT-08 external-anchor cluster lock v0
Status: Implemented (cluster posture pinned + non-independence scope explicit)
Owner ticket/module: formal/docs/programs/CT_external_anchor_cluster_v0.md
Evidence path: formal/docs/programs/CT_external_anchor_cluster_v0.md; formal/python/tests/test_ct_external_anchor_cluster_doc.py; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md; formal/python/tests/test_conditional_theorems_program_doc.py; formal/python/tests/test_ct08_files_do_not_contain_ct07_tokens.py; formal/python/tests/test_ct08_file_enumerator_guard_coverage.py; formal/docs/programs/CT06_external_anchor_dispersion_v0.md; formal/docs/programs/CT07_external_anchor_dispersion_lowk_slice_v0.md; formal/docs/programs/CT08_external_anchor_dispersion_highk_slice_v0.md
Exit criteria: CT-06/07/08 cluster is treated as one evidence family with two regime probes (low-k/high-k); CT-07/08 remain explicitly non-independent of CT-06; CT-08 residue and file-list guard tests enforced.
Notes: This lock prevents interpreting CT-07/CT-08 as independent external confirmations; scope remains dataset-conditional and front-door bounded; no external truth claim.

GapID: COMP-CT-09
Layer: Governance
Item: Conditional theorem CT-09 independent external-anchor sound-speed v0
Status: Implemented (front door record + expectation-aware controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/ct09_independent_external_anchor_sound_speed_v0.py
Evidence path: formal/docs/programs/CT09_independent_external_anchor_sound_speed_v0.md; formal/python/tests/test_ct09_independent_external_anchor_sound_speed_lane_doc.py; formal/python/toe/comparators/ct09_independent_external_anchor_sound_speed_v0.py; formal/python/tools/ct09_independent_external_anchor_sound_speed_front_door.py; formal/external_evidence/ct09_independent_sound_speed_domain_01/distance_vs_time_um_ms.csv; formal/external_evidence/ct09_independent_sound_speed_domain_01/dataset_metadata.json; formal/external_evidence/ct09_independent_sound_speed_domain_01/source_citation.md; formal/external_evidence/ct09_independent_sound_speed_domain_01/ct09_reference_report.json; formal/external_evidence/ct09_independent_sound_speed_domain_01/ct09_candidate_report.json; formal/python/tests/test_ct09_independent_external_anchor_sound_speed_v0_front_door.py; formal/python/tests/test_ct09_independent_external_anchor_sound_speed_v0_surface_contract_freeze.py; formal/python/tests/test_ct09_independent_external_anchor_sound_speed_v0_front_door_contract_freeze.py; formal/python/tests/test_ct09_independent_external_anchor_sound_speed_v0_pinned_artifacts.py; formal/python/tests/test_ct09_independent_external_anchor_sound_speed_v0_lock.py; formal/markdown/locks/observables/CT-09_independent_external_anchor_sound_speed_v0.md; formal/docs/ct09_independent_external_anchor_sound_speed_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics for positive/negative controls on independent external-anchor fit reproducibility; dataset-conditional tolerance checks enforced.
Notes: This conditional theorem asserts independent external-anchor sound-speed fit reproducibility under a pinned dataset and preprocessing only; negative control pass is expectation-aware failure detection; scope remains front-door bounded with no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CT-09-PLAN
Layer: Governance
Item: CT-09 independent external-anchor plan v0
Status: Historical provenance artifact (superseded by implemented CT-09 lane)
Owner ticket/module: formal/docs/programs/CT09_independent_external_anchor_plan_v0.md
Evidence path: formal/docs/programs/CT09_independent_external_anchor_plan_v0.md; formal/python/tests/test_ct09_independent_external_anchor_plan_doc.py; formal/external_evidence/ct09_independent_external_anchor_tbd/README.md; formal/external_evidence/ct09_independent_external_anchor_tbd/dataset_metadata.template.json
Exit criteria: Plan artifact remains available for provenance, explicitly marked historical/superseded, and is not treated as an active next-step lane.
Notes: Historical planning object only; active implementation is COMP-CT-09 (`formal/docs/programs/CT09_independent_external_anchor_sound_speed_v0.md`).

GapID: COMP-CT-10-FILTER
Layer: Governance
Item: CT-10 independent external-anchor selection filter v0
Status: Implemented (selection/admission gate + intake packet + deterministic selection-verdict lock + doc gates)
Owner ticket/module: formal/docs/programs/CT10_independent_external_anchor_selection_filter_v0.md
Evidence path: formal/docs/programs/CT10_independent_external_anchor_selection_filter_v0.md; formal/python/tests/test_ct10_independent_external_anchor_selection_filter_doc.py; formal/docs/programs/CONDITIONAL_THEOREMS_PROGRAM_v0.md; formal/external_evidence/ct10_independent_external_anchor_tbd/README.md; formal/external_evidence/ct10_independent_external_anchor_tbd/dataset_metadata.template.json; formal/external_evidence/ct10_independent_external_anchor_tbd/source_citation.md; formal/external_evidence/ct10_independent_external_anchor_tbd/preprocessing_lineage.md; formal/external_evidence/ct10_independent_external_anchor_tbd/dataset_metadata.json; formal/external_evidence/ct10_independent_external_anchor_tbd/candidate_observable_sample.csv; formal/python/toe/observables/ovselct10_independent_anchor_selection_verdict_record.py; formal/python/tools/ovselct10_independent_anchor_selection_verdict_record.py; formal/python/tests/test_ov_sel_ct10_independent_anchor_selection_verdict_record.py; formal/python/tests/test_ov_sel_ct10_independent_anchor_selection_verdict_lock.py; formal/markdown/locks/observables/OV-SEL-CT10-01_independent_anchor_selection_verdict.md
Exit criteria: CT-10 candidate admission requires independent source lineage, preprocessing lineage independence, a different observable surface, and explicit non-BEC provenance; lane activation remains blocked-on-missing until full comparator artifacts exist.
Notes: Governance-only pre-activation filter for external-anchor multiplication pressure; deterministic verdict lock is reproducible; no CT-10 comparator claim yet; scope remains `external_anchor_only` with no external truth claim; governance suite green on 2026-02-12 (383 passed).

GapID: COMP-CX-01
Layer: Governance
Item: Constraint admissibility CX-01 update contractivity v0
Status: Implemented (front door record + expectation-aware controls + surface freeze)
Owner ticket/module: formal/python/toe/comparators/cx01_update_contractivity_v0.py
Evidence path: formal/docs/programs/CONSTRAINT_ADMISSIBILITY_PROGRAM_v0.md; formal/python/tests/test_constraint_admissibility_program_doc.py; formal/docs/programs/CX01_update_contractivity_lane_spec.md; formal/python/tests/test_cx01_update_contractivity_lane_doc.py; formal/python/toe/comparators/cx01_update_contractivity_v0.py; formal/python/tools/cx01_update_contractivity_front_door.py; formal/external_evidence/cx01_update_contractivity_domain_01/cx01_reference_report.json; formal/external_evidence/cx01_update_contractivity_domain_01/cx01_candidate_report.json; formal/python/tests/test_cx01_update_contractivity_v0_front_door.py; formal/python/tests/test_cx01_update_contractivity_v0_surface_contract_freeze.py; formal/python/tests/test_cx01_update_contractivity_v0_front_door_contract_freeze.py; formal/python/tests/test_cx01_update_contractivity_v0_pinned_artifacts.py; formal/python/tests/test_cx01_update_contractivity_v0_lock.py; formal/markdown/locks/observables/CX-01_update_contractivity_v0.md; formal/docs/cx01_update_contractivity_v0_front_door_contract.md; formal/python/toe/comparators/comparator_rep_interpretability_manifest.json
Exit criteria: Deterministic record + blocked-on-missing; expectation-aware pass semantics for positive/negative controls on contractivity admissibility; surface contract freeze tests enforced.
Notes: This constraint lane encodes a stability admissibility predicate (`empirical_lipschitz <= 1`) under pinned within-rep assumptions only; negative control pass is explicit break detection; no external truth claim.
...
Discharge the remaining analytic bridge assumption (`ActionRep32FiniteDifferenceRepresentsP (ε, hε)`) on the default quotient path so the non-tautological EL-to-first-variation route can fully close without legacy shortcuts; keep broader comparator expansion policy-gated until COMP-FN-DERIVE exits.

Remaining default-quotient discharge checklist (COMP-FN-DERIVE)
- `hAction` source: `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_hAction_provenance_v0.md` (obligation `actionRep32.action = actionRep32Cubic declared_g_rep32`; source symbol `Rep32CubicLaneAssumptions.hAction` in `formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean`).
- `hRAC` source: `formal/toe_formal/ToeFormal/Variational/ActionRep32CubicLane.lean` (`RACRep32CubicObligationSet` bundle, bridged by `RAC_Rep32_Cubic_of_obligation_set`) and `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_hRAC_obligation_bundle_v0.md`.
- Bridge-side source: `formal/markdown/locks/functionals/FN-DERIVE_default_quotient_bridge_discharge_object_v0.md` (`ActionRep32FiniteDifferenceRepresentsP_declared_g_of_action_and_obligation_set` discharges `ActionRep32FiniteDifferenceRepresentsP (ε, hε)` from inputs `{ε, hε, hAction, hRAC}` on the default quotient path without legacy EL shortcuts).

RAC completion policy (inevitability plan)
- RAC is currently a declared admissibility principle (policy-level), not a proved theorem-level discharge.
- Plan-complete criterion (policy-level): all residual assumptions are explicitly surfaced and classified as policy-level vs theorem-level, with no hidden assumptions.
- Theorem-complete criterion (future): RAC obligations are promoted to proved discharge artifacts under explicit governance approval.
