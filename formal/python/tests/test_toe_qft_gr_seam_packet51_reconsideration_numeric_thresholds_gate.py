...
THRESHOLD_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_v0.md"
THRESHOLD_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet51_reconsideration_numeric_thresholds_checkpoint_v0.json"
CONVERGENCE_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_convergence_termination_criterion_checkpoint_v0.json"
HOLD_FORK_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet51_hold_fork_decision_checkpoint_v0.json"
...
MEASUREMENT_PROTOCOL_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet51_numeric_threshold_measurement_protocol_checkpoint_v0.json"
...
def test_packet51_reconsideration_numeric_thresholds_document_structure() -> None:
...
def test_packet51_reconsideration_numeric_thresholds_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet51_reconsideration_numeric_thresholds_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2Y_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS"
    assert artifact.get("status") == "PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_ACTIVE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("threshold_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_v0.md"
    ...
    assert payload.get("parent_measurement_protocol_checkpoint_path") == "formal/output/toe_qft_gr_seam_packet51_numeric_threshold_measurement_protocol_checkpoint_v0.json"
    assert payload.get("parent_scorecard_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_SCORECARD_WORKSHEET_v0.md"
    assert payload.get("parent_scorecard_checkpoint_path") == "formal/output/toe_qft_gr_seam_packet51_reconsideration_scorecard_worksheet_checkpoint_v0.json"
    ...
    assert thresholds.get("packet51_reconsideration_release_gate", {}).get("all_numeric_thresholds_required") is True
    assert thresholds.get("packet51_reconsideration_release_gate", {}).get("existing_review_layers_required") is True
    ...
    assert hold_fork.get("status") == "PACKET51_HOLD_FORK_DECISION_COMPLETE_HOLD_v0"
    ...
    assert scorecard.get("status") == "PACKET51_RECONSIDERATION_SCORECARD_WORKSHEET_ACTIVE_v0"


def test_packet51_reconsideration_numeric_thresholds_authority_parity_and_freeze() -> None:
    ...
    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_STATUS_v0")
    ...
    state_outcome = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_OUTCOME_v0"
    )
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET51_RECONSIDERATION_NUMERIC_THRESHOLDS_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_RETAINED_UNTIL_NUMERIC_CLEARANCE_v0"

    assert not PACKET51_AUTH_DOC_PATH.exists(), "Packet49 authorization doc must not exist under numeric-threshold hold"
    assert not PACKET51_AUTH_CHECKPOINT_PATH.exists(), "Packet49 authorization checkpoint must not exist under numeric-threshold hold"
