...
from formal.python.tools.phi_source_theorem_linkage_obligation_packet_report import (
    ...
    WATCH_ITEMS,
    build_phi_source_theorem_linkage_obligation_packet,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_source_theorem_linkage_obligation_packet_report.py"
)
...
def test_phi_source_theorem_linkage_obligation_packet_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_phi_source_theorem_linkage_obligation_packet_scopes_prior_phi_route() -> None:
    ...
    assert build_phi_source_theorem_linkage_obligation_packet() == packet


def test_phi_source_theorem_linkage_obligation_packet_freezes_registry_statement() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["C_source_phi_target_statement"] == C_SOURCE_PHI_TARGET_STATEMENT
    assert packet["source_admissibility_condition"] == (
        C_SOURCE_PHI_SOURCE_ADMISSIBILITY_CONDITION
    )
    assert packet["stress_divergence_target"] == STRESS_DIVERGENCE_TARGET
    assert packet["likely_schematic_route"] == LIKELY_SCHEMATIC_ROUTE
    assert packet["exact_registry_statement_frozen"] is True
    ...
    assert packet["stress_energy_under_selected_policy"] == (
        STRESS_ENERGY_UNDER_SELECTED_POLICY
    )


def test_phi_source_theorem_linkage_obligation_packet_blocks_route_contamination() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert "do not substitute A-sector" in packet["route_contamination_guard"]
    assert "psi-A sourced Maxwell" in packet["route_contamination_guard"]
    assert "QFT-GR source routes" in packet["route_contamination_guard"]


def test_phi_source_theorem_linkage_obligation_packet_preserves_boundaries() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_phi_source_theorem_linkage_obligation_packet_rotates_to_review() -> None:
    ...
    assert consumed["source_admissibility_condition"] == (
        C_SOURCE_PHI_SOURCE_ADMISSIBILITY_CONDITION
    )
    ...
    assert review_row["selected_obligation"] == "C_source^phi theorem-linkage obligation"
    assert review_row["C_source_phi_residual_definition"] == (
        C_SOURCE_PHI_RESIDUAL_DEFINITION
    )
    assert review_row["source_admissibility_condition"] == (
        C_SOURCE_PHI_SOURCE_ADMISSIBILITY_CONDITION
    )
    ...
    assert review_row["C_source_phi_discharged"] == "no"
...
def test_phi_source_theorem_linkage_obligation_packet_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_PACKET_RESULT,
        PACKET_CLASSIFICATION,
        "PhiSourceTheoremLinkageObligationPacket",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        C_SOURCE_PHI_RESIDUAL_DEFINITION,
        ...
        RESIDUAL_IDENTITY_FORM,
        ON_SHELL_IMPLICATION_FORM,
        "PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_PACKET_OUTCOME_v0",
        "PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_PACKET_NONCLAIM_BOUNDARY_v0",
        "no proof execution",
...
def test_phi_source_theorem_linkage_obligation_packet_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_source_theorem_linkage_obligation_packet_gate.py"
    )
