...
from formal.python.tools.qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_report import (
    ...
    SELECTED_ROW_ID,
    SELECTED_ROW_OBJECT,
    build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet,
)
...
from formal.python.tools.qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review_report import (
    DEFAULT_OUT as MR002_RESULT_REVIEW_PATH,
    OUTCOME_ID as MR002_RESULT_REVIEW_OUTCOME,
    RESULT_REVIEW_CLASSIFICATION as MR002_RESULT_REVIEW_CLASSIFICATION,
    REVIEW_ID as MR002_RESULT_REVIEW_ID,
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_DistributionalPairingRegularDomainAssumptionReductionPacket.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_distributional_pairing_regular_domain_packet_files_exist() -> None:
    assert MR002_RESULT_REVIEW_PATH.exists()
...
def test_qft_gr_distributional_pairing_regular_domain_packet_consumes_mr002_review() -> None:
    packet = _json(DEFAULT_OUT)
    review = _json(MR002_RESULT_REVIEW_PATH)
    ...
    assert (
        ...
        == MR002_RESULT_REVIEW_ID
    )
    assert packet["consumed_result_review_outcome_id"] == MR002_RESULT_REVIEW_OUTCOME
    assert packet["consumed_result_review_classification"] == (
        MR002_RESULT_REVIEW_CLASSIFICATION
    )
    assert review["selected_next_target"] == packet["consumed_target"]


def test_qft_gr_distributional_pairing_regular_domain_packet_selects_only_mr003() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["distributional_pairing_regular_domain_assumption"]
    ...
    assert packet["distributional_pairing_regular_domain_status_tokens"] == (
        selected["current_status"]
    )


def test_qft_gr_distributional_pairing_regular_domain_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["distributional_pairing_regular_domain_assumption"]
    assert packet["distributional_pairing_regular_domain"] == SELECTED_ROW_OBJECT
    assert selected["regularity_condition"] == SELECTED_ROW_OBJECT
    assert packet["distributional_pairing_domain_boundaries"] == (
        DISTRIBUTIONAL_PAIRING_DOMAIN_BOUNDARIES
    )
    assert selected["distributional_pairing_domain_boundaries"] == (
        DISTRIBUTIONAL_PAIRING_DOMAIN_BOUNDARIES
    )
...
def test_qft_gr_distributional_pairing_regular_domain_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["distributional_pairing_regular_domain_assumption_discharged"] is False
    assert (
        packet[
            "distributional_pairing_regular_domain_assumption_reduced_or_discharged_by_preparation"
        ]
        is False
    )
...
def test_qft_gr_distributional_pairing_regular_domain_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        ...
        "execute_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt": "deferred",
        "prepare_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_packet": "deferred",
...
def test_qft_gr_distributional_pairing_regular_domain_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet(
