...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qft_m2_canonical_equivalence_scaffold_cycle01_v0.json"

EXPECTED_ARTIFACT_ID = "qft_m2_canonical_equivalence_scaffold_cycle01_v0"
...
def test_qft_m2_canonical_equivalence_scaffold_cycle01_gate() -> None:
    qft_text = _read(QFT_DOC_PATH)
    state_text = _read(STATE_PATH)
    roadmap_text = _read(ROADMAP_PATH)
    inventory_text = _read(INVENTORY_PATH)

    assert ARTIFACT_PATH.exists(), "QFT M2 canonical equivalence scaffold artifact is missing."
    ...
    assert _extract_token(roadmap_text, "QFT_M2_CANONICAL_EQUIVALENCE_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID

    assert _extract_token(qft_text, "QFT_M2_CANONICAL_EQUIVALENCE_SHA256_v0") == expected_sha
    assert _extract_token(state_text, "QFT_M2_CANONICAL_EQUIVALENCE_SHA256_v0") == expected_sha
    assert _extract_token(roadmap_text, "QFT_M2_CANONICAL_EQUIVALENCE_SHA256_v0") == expected_sha

    assert _extract_token(qft_text, "QFT_M2_CANONICAL_EQUIVALENCE_GATE_v0") == EXPECTED_GATE
    assert _extract_token(state_text, "QFT_M2_CANONICAL_EQUIVALENCE_GATE_v0") == EXPECTED_GATE
    assert _extract_token(roadmap_text, "QFT_M2_CANONICAL_EQUIVALENCE_GATE_v0") == EXPECTED_GATE

    assert _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "QFT_M2_CANONICAL_EQUIVALENCE_STATUS_v0"
    ) == "RUN_BOUNDED_v0_NONCLAIM"
    assert _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "QFT_M2_CANONICAL_EQUIVALENCE_ARTIFACT_v0"
    ) == EXPECTED_ARTIFACT_ID
    assert _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "QFT_M2_CANONICAL_EQUIVALENCE_SHA256_v0"
    ) == expected_sha
    assert _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "QFT_M2_CANONICAL_EQUIVALENCE_GATE_v0"
    ) == EXPECTED_GATE

    artifact_rel = "formal/output/qft_m2_canonical_equivalence_scaffold_cycle01_v0.json"
