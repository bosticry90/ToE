...
EXPECTED_ARTIFACT_ID = "stat_der02_bounded_discharge_attempt_execution_packet_v0"
EXPECTED_PACKET_ID = "STAT_DER02_BOUNDED_DISCHARGE_ATTEMPT_EXECUTION_PACKET_v0"
EXPECTED_OUTCOME = "STAT_DER02_BOUNDED_DISCHARGE_ATTEMPT_BLOCKER_UNCHANGED"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_der02_bounded_discharge_attempt_execution_packet_gate.py"
...
def test_stat_der02_bounded_discharge_attempt_execution_packet_gate() -> None:
    ...
    coupling_surface = _read_json(COUPLING_SURFACE_PATH)
    scope_boundary = _read_json(SCOPE_BOUNDARY_PATH)

    execution_packets = sorted(
        path.name
        for path in (REPO_ROOT / "formal" / "output").glob("stat_der02_bounded_discharge_attempt_execution_packet*.json")
    )
    assert execution_packets == ["stat_der02_bounded_discharge_attempt_execution_packet_v0.json"]
    ...
    assert payload["execution_packet_scope"] == "ONE_BOUNDED_DER02_THEOREM_BODY_TO_DISCHARGE_ATTEMPT"

    auth_decision = authorization["authorization_decision"]
    assert auth_decision["one_bounded_der02_discharge_attempt_authorized"] is True
    assert auth_decision["bounded_discharge_attempt_executed_now"] is False
    assert auth_decision["execution_authorization"] == "ONE_BOUNDED_DER02_DISCHARGE_ATTEMPT_ONLY"
    ...
    assert consumed["authorization_report"] == (
        "formal/output/reports/post_recovery_stat_der02_discharge_attempt_authorization_readiness_review_20260424_v0.json"
    )
    ...
    assert consumed["authorization_state_after_packet"] == "CONSUMED_NO_REMAINING_DER02_DISCHARGE_ATTEMPT_AUTHORIZATION"

    source_hashes = {
        "DER02_PINNED_REGIME_CLOSURE_CONSISTENCY_WITNESS": witness["payload_sha256"],
        "DER02_BOUNDED_DISCHARGE_ATTEMPT_SURFACE": surface["payload_sha256"],
        "DER02_THEOREM_BODY_SCAFFOLD": theorem_body["payload_sha256"],
        "DER02_DISCHARGE_SCAFFOLD": discharge["payload_sha256"],
        "DER02_OBJECT_SURFACE_SCAFFOLD": object_surface["payload_sha256"],
        "DER02_REGIME_CLOSURE_COUPLING_SCAFFOLD": coupling_surface["payload_sha256"],
        "DER02_SCOPE_BOUNDARY_SCAFFOLD": scope_boundary["payload_sha256"],
    }
    ...
    assert witness["payload"]["regime_closure_consistency_witness_result"] == payload["pre_attempt_state"]["witness_result"]
    ...
    assert _consumed_input(payload, "DER02_THEOREM_BODY_SCAFFOLD")["observed_status"] == theorem_body["payload"]["status"]
    assert _consumed_input(payload, "DER02_DISCHARGE_SCAFFOLD")["observed_status"] == discharge["payload"]["status"]
    ...
    assert attempt["theorem_body_closure_coupling_constraint_slot_replaced"] is False
    assert attempt["discharge_closure_coupling_consistency_slot_replaced"] is False
    ...
    assert "theorem_body_closure_constraint_slot_still_unreplaced_after_attempt" in attempt["failure_triggered_conditions"]
    assert "discharge_closure_consistency_slot_still_unreplaced_after_attempt" in attempt["failure_triggered_conditions"]
    ...
    assert post_attempt["execution_authorization_before_packet"] == "ONE_BOUNDED_DER02_DISCHARGE_ATTEMPT_ONLY"
    assert post_attempt["execution_authorization_after_packet"] == "CONSUMED_NO_REMAINING_DER02_DISCHARGE_ATTEMPT_AUTHORIZATION"
    ...
    assert post_attempt["further_der02_discharge_attempt_authorization"] == "NONE"
    ...
    for disallowed in (
        "rerun_this_der02_discharge_attempt_without_new_authorization",
    ...
    for required_boundary in (
        "single_bounded_der02_discharge_attempt_readout_only",
        ...
        "no_additional_der02_attempt_authorization",
    ...
    for expected_bundle_member in (
        "formal/output/stat_der02_regime_closure_consistency_witness_binding_v0.json",
        "formal/python/tests/test_stat_der02_regime_closure_consistency_witness_binding_gate.py",
        "formal/output/stat_der02_bounded_discharge_attempt_surface_v0.json",
        "formal/python/tests/test_stat_der02_bounded_discharge_attempt_surface_gate.py",
        "formal/output/stat_der02_bounded_discharge_attempt_execution_packet_v0.json",
        EXPECTED_GATE_REL,
    ):
