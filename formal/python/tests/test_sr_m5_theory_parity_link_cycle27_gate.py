...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "sr_m5_theory_parity_link_cycle27_v0.json"

EXPECTED_ARTIFACT_ID = "sr_m5_theory_parity_link_cycle27_v0"
EXPECTED_GATE = "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_AUDIT_OBJECTIVE = "SINGLE_POINTER_UNIQUENESS_ENFORCED_v0"
...
def test_sr_m5_theory_parity_link_cycle27_gate() -> None:
    ...
    assert ARTIFACT_PATH.exists(), "SR M5 theory-parity-link cycle27 artifact is missing."
    ...
    assert m5_parity.get("artifact_path") == "formal/output/sr_m5_theory_parity_link_cycle27_v0.json"
    assert m5_parity.get("gate_path") == "formal/python/tests/test_sr_m5_theory_parity_link_cycle27_gate.py"
    ...
    # Cycle27 objective: each canonical surface should expose exactly one active SR M5 artifact and gate pointer.
    for text in (target_text, sr_text, state_text, roadmap_text):
