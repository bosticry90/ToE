...
NEGCONTROL_PATTERNS = {
    "premise_removed": re.escape("NEGCTRL_SINGULAR_PREMISE: SINGULAR_SOURCE_LANE_INJECTED_WITHOUT_ASSUMPTION_ID"),
    "breakpoint_unlicensed": re.escape("NEGCTRL_SINGULAR_BREAKPOINT: CLASSICAL_COMMUTING_PARTIALS_ROUTE_UNLICENSED"),
    "outcome_not_licensed": re.escape("NEGCTRL_SINGULAR_OUTCOME: MAXWELL_TO_CONTINUITY_ATTEMPT_NOT_LICENSED"),
}
...
def test_em_cycle020_artifacts_exist() -> None:
    ...
    assert EM_MICRO20_PATH.exists(), "Missing EM U1 Cycle-020 document."
...
def test_em_micro20_contains_required_tokens_and_localized_negcontrol_statements() -> None:
    ...
    assert not missing, "EM Cycle-020 micro document is missing required token(s): " + ", ".join(missing)

    missing_patterns = [label for label, pattern in NEGCONTROL_PATTERNS.items() if re.search(pattern, text) is None]
    assert not missing_patterns, (
        "EM Cycle-020 micro document is missing required neg-control seam statement(s): "
...
def test_em_target_references_cycle020_artifact_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM target document is missing required Cycle-020 token(s): " + ", ".join(missing)
...
def test_state_mentions_cycle020_checkpoint_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "State_of_the_Theory.md is missing required Cycle-020 token(s): " + ", ".join(missing)
...
def test_em_lean_cycle020_tokens_and_negcontrol_harness_stubs_are_pinned() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM U1 Lean scaffold is missing required Cycle-020 token(s): " + ", ".join(missing)


def test_cycle020_negcontrol_statements_are_localized_to_cycle20_artifact() -> None:
    allowed_paths = [EM_MICRO20_PATH]
    scoped_paths = [
        EM_TARGET_PATH,
        EM_MICRO01_PATH,
        EM_MICRO02_PATH,
    ...
    for path in allowed_paths:
        ...
        if not hits:
            violations.append(f"cycle020 artifact {path.relative_to(REPO_ROOT)} is missing localized neg-control statements")
            continue
        ...
        if not (has_source_assumption_id and has_smoothness_assumption_id and has_localization_gate and has_prereq and has_non_selection):
            violations.append(
                f"cycle020 artifact {path.relative_to(REPO_ROOT)} contains neg-control statements {hits} "
    ...
    assert not violations, "Cycle-020 neg-control localization violation:\n- " + "\n- ".join(violations)


def test_cycle020_is_negcontrol_only_and_blocks_promotion_or_import_lane_expansion() -> None:
    text = _read(EM_MICRO20_PATH)
    forbidden_patterns = [
        r"\bwe derive\b",
        r"\bwe prove\b",
    ...
    assert not violations, (
        "Cycle-020 must remain neg-control-only/non-promotional with no import-lane expansion. "
