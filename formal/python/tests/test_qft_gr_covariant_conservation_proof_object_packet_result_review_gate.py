...
from formal.python.tools.qft_gr_covariant_conservation_proof_object_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_covariant_conservation_proof_object_packet_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_CovariantConservationProofObjectPacketResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_covariant_conservation_proof_object_packet_result_review_report.py"
)
...
def test_qft_gr_covariant_conservation_proof_object_packet_result_review_files_exist() -> None:
...
def test_qft_gr_covariant_conservation_proof_object_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_qft_gr_covariant_conservation_proof_object_packet"] == PACKET_ID
    ...
    assert packet["selected_next_target"] == "review_qft_gr_covariant_conservation_proof_object_packet_result"


def test_qft_gr_covariant_conservation_proof_object_packet_result_review_accepts_preparation_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["selected_obstruction"] == SELECTED_OBSTRUCTION
    assert review["target_proof_object"] == TARGET_PROOF_OBJECT
    assert review["proof_object_packet_preparation_accepted"] is True
    assert review["bounded_proof_object_attempt_authorized"] is True
    assert review["proof_object_packet_prepared_only"] is True
...
def test_qft_gr_covariant_conservation_proof_object_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_covariant_conservation_proof_object_packet_result_review_selects_one_execution_target() -> None:
    review = _json(DEFAULT_OUT)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "review_qft_gr_covariant_conservation_proof_object_packet_result": "consumed",
...
def test_qft_gr_covariant_conservation_proof_object_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_covariant_conservation_proof_object_packet_result_review(
