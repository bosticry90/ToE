...
PROTOCOL_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_v0.md"
PROTOCOL_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet43_numeric_threshold_measurement_protocol_checkpoint_v0.json"
...
NUMERIC_THRESHOLDS_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet43_reconsideration_numeric_thresholds_checkpoint_v0.json"
...
def test_packet43_numeric_threshold_measurement_protocol_document_structure() -> None:
    text = _read(PROTOCOL_DOC_PATH)
    required_markers = [
        "Protocol ID:",
        "Parent packet43 reconsideration numeric thresholds:",
        "Parent packet43 reconsideration scorecard worksheet:",
...
def test_packet43_numeric_threshold_measurement_protocol_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet43_numeric_threshold_measurement_protocol_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2Z_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL"
    assert artifact.get("status") == "PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_ACTIVE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("protocol_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_v0.md"
    ...
    assert "formal/output/toe_qft_gr_seam_packet43_reconsideration_numeric_thresholds_checkpoint_v0.json" in admissible
    assert "formal/output/toe_qft_gr_seam_packet43_reconsideration_scorecard_worksheet_checkpoint_v0.json" in admissible
    ...
    assert thresholds.get("status") == "PACKET43_RECONSIDERATION_NUMERIC_THRESHOLDS_ACTIVE_v0"
    assert convergence.get("status") == "SEAM_CONVERGENCE_TERMINATION_CRITERION_ACTIVE_v0"


def test_packet43_numeric_threshold_measurement_protocol_authority_parity_and_freeze() -> None:
    ...
    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_STATUS_v0")
    ...
    state_outcome = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_OUTCOME_v0"
    )
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET43_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_RETAINED_PENDING_MEASURED_CLEARANCE_v0"

    assert not PACKET43_AUTH_DOC_PATH.exists(), "Packet43 authorization doc must not exist under measurement-protocol hold"
    assert not PACKET43_AUTH_CHECKPOINT_PATH.exists(), "Packet43 authorization checkpoint must not exist under measurement-protocol hold"
