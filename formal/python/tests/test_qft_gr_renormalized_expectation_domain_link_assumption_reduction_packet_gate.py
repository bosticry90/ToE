...
from formal.python.tools.qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_report import (
    ...
    SCHEMA_ID,
    SELECTED_ROW_ID,
    build_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet,
)
...
from formal.python.tools.qft_gr_state_expectation_domain_link_assumption_reduction_attempt_result_review_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_report.py"
)
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_consumes_od003_review() -> None:
    ...
    assert (
        packet[
            "consumes_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_result_review"
        ]
        == RESULT_REVIEW_ID
    )
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_records_prior_rows() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert (
        packet["prior_accepted_selected_operator_action_contract"]
        == SELECTED_OPERATOR_ACTION_CONTRACT_ID
    )
    assert (
        packet["prior_accepted_candidate_source_domain_membership_contract"]
        == CANDIDATE_SOURCE_DOMAIN_MEMBERSHIP_CONTRACT_ID
    )
    assert (
        packet["prior_accepted_state_expectation_domain_link_contract"]
        == STATE_EXPECTATION_DOMAIN_LINK_CONTRACT_ID
    )


def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_selects_only_row004() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_expectation_domain_link_assumption"]
    ...
    assert packet["renormalized_expectation_domain_link_status_tokens"] == selected[
        "current_status"
    ]
    assert packet["row_status_enum"] == ROW_STATUS_ENUM


def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_expectation_domain_link_assumption"]
    assert packet["renormalized_expectation_object"] == RENORMALIZED_EXPECTATION_OBJECT
    assert packet["operator_domain_link_condition"] == OPERATOR_DOMAIN_LINK_CONDITION
    assert selected["renormalized_expectation_object"] == RENORMALIZED_EXPECTATION_OBJECT
    assert selected["operator_domain_link_condition"] == OPERATOR_DOMAIN_LINK_CONDITION
    assert selected["required_future_proof_object"] == OPERATOR_DOMAIN_LINK_CONDITION
    for field in [
    ...
    assert packet["renormalized_expectation_domain_link_assumption_discharged"] is False
    assert (
        packet[
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["renormalization_compatibility_with_conservation_claimed"] is False
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet(
