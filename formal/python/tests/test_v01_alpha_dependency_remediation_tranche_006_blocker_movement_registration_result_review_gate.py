...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche006BlockerMovementRegistrationResultReview.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_consumes_registration() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW_20260522_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_registered_true() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["review_scope"] == (
        "REVIEW_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_RESULT_ONLY_ACCEPT_DOCUMENTED_NONBLOCKING_MOVEMENT_NO_RELEASE_PROMOTION"
    )
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_exact_movement() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_preserves_evidence_policy_documentation_chain() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_accepts_formal_movement_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_003_release_blocker_status"] == TRANCHE_003_STATUS
    assert review["tranche_005_release_blocker_status"] == TRANCHE_005_STATUS
    assert review["tranche_006_formal_movement_accepted"] is True
    assert review["tranche_006_release_blocker_status"] == CANDIDATE_BLOCKER_STATUS
    assert review["tranche_006_dependency_policy_remediation_satisfied"] is True
    assert review["tranche_006_cleared_for_global_release_readiness"] is False
    assert review["global_release_readiness_still_blocked"] is True
    assert review["dependency_remediation_queue_exhausted"] is True
    assert review["dependency_remediation_summary_preparation_authorized"] is True
    assert review["release_blocking_obligation_count_after_review"] == 1
    assert review["remaining_release_blocking_obligation_count_after_review"] == 1


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_carries_posture() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_006_status"] == CANDIDATE_BLOCKER_STATUS
    ...
    assert review["tranche_006_moved_or_cleared"] is True
    assert review["tranche_006_selected_yet"] is True


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_remaining_tranche_004_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == (
        "dependency_remediation_summary_preparation_only"
    )
    assert review["selection_count"] == 1
    assert review["next_action_scope"] == (
        "PREPARE_DEPENDENCY_REMEDIATION_SUMMARY_AFTER_TRANCHE_006_MOVEMENT_ONLY_NO_RELEASE_PROMOTION"
    )
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "prepare_v01_alpha_dependency_remediation_summary_after_tranche_006_movement": "selected",
        "assemble_v01_alpha_release_packet": "deferred",
        "pause_v01_alpha_release_readiness_due_to_retained_tranche_004_blocker": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        REVIEW_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW_20260522_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006BlockerMovementRegistrationResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_blocker_movement_registration_result_review_does_not_promote_release"
        in index_text
    )
