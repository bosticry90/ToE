...
from formal.python.tools.ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_result_review_report import (
    ...
    build_ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_result_review_report.py"
)
...
def test_ck_family_selection_after_A_source_closeout_result_review_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_ck_family_selection_after_A_source_closeout_result_review_accepts_phi_selection() -> None:
...
def test_ck_family_selection_after_A_source_closeout_result_review_preserves_phi_registry_boundary() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["phi_source_registry_boundary"] == PHI_SOURCE_REGISTRY_BOUNDARY
    assert (
        review["prior_phi_source_constraint_form"]
        == PHI_SOURCE_REGISTRY_CONSTRAINT_FORM
    )
    assert (
        review["prior_phi_source_constraint_equation"]
        == PHI_SOURCE_REGISTRY_CONSTRAINT_EQUATION
    )
    assert review["forbidden_imported_routes"] == FORBIDDEN_IMPORTED_ROUTES
...
def test_ck_family_selection_after_A_source_closeout_result_review_records_lean_status() -> None:
    review = _json(DEFAULT_OUT)
...
def test_ck_family_selection_after_A_source_closeout_result_review_rotates_to_phi_packet() -> None:
    ...
    if is_current:
        ...
        assert active["prior_phi_source_constraint_form"] == (
            PHI_SOURCE_REGISTRY_CONSTRAINT_FORM
        )
        assert active["prior_phi_source_constraint_equation"] == (
            PHI_SOURCE_REGISTRY_CONSTRAINT_EQUATION
        )
...
def test_ck_family_selection_after_A_source_closeout_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "CKFamilyTheoremLinkageObligationSelectionAfterASourceCloseoutResultReview",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        SELECTED_OBLIGATION,
        ...
        LEAN_STATUS_WORDING_FOR_REVIEW,
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_A_SOURCE_CLOSEOUT_RESULT_REVIEW_OUTCOME_v0",
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_A_SOURCE_CLOSEOUT_RESULT_REVIEW_NONCLAIM_BOUNDARY_v0",
...
def test_ck_family_selection_after_A_source_closeout_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_result_review_gate.py"
    )
