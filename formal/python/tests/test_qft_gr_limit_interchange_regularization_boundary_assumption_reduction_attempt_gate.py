...
from formal.python.tools.qft_gr_limit_interchange_regularization_boundary_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_attempt,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_limit_interchange_regularization_boundary_assumption_reduction_attempt_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_limit_interchange_attempt_files_exist() -> None:
...
def test_qft_gr_limit_interchange_attempt_consumes_review() -> None:
...
def test_qft_gr_limit_interchange_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    review = _json(REVIEW_PATH)
    ...
    assert (
        attempt[
            "limit_interchange_regularization_boundary_assumption_obstruction_identified"
        ]
        is False
    )
    assert (
        attempt["limit_interchange_regularization_boundary_assumption_inconclusive"]
        is False
    )


def test_qft_gr_limit_interchange_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["limit_interchange_regularization_boundary_reduction_contract"]
    ...
    assert attempt["limit_interchange_regularization_boundary"] == SELECTED_ROW_OBJECT
    assert attempt["limit_interchange_boundaries"] == LIMIT_INTERCHANGE_BOUNDARIES
    ...
    assert contract["limit_interchange_boundaries"] == LIMIT_INTERCHANGE_BOUNDARIES
...
def test_qft_gr_limit_interchange_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert (
        attempt[
    ...
    assert (
        attempt["limit_interchange_regularization_boundary_assumption_reduced_by_attempt"]
        is True
    )
    assert (
        attempt["limit_interchange_regularization_boundary_assumption_discharged"]
        is False
    )
    assert (
        attempt[
            "limit_interchange_regularization_boundary_assumption_discharged_by_attempt"
        ]
        is False
    )
    assert attempt["limit_interchange_regularization_boundary_proved"] is False
...
def test_qft_gr_limit_interchange_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        "review_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_attempt_result": "selected",
...
def test_qft_gr_limit_interchange_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_attempt(
