...
from formal.python.tools.qft_gr_stress_energy_conservation_witness_attempt_report import (
    ...
    SCHEMA_ID,
    SCIENTIFIC_QUESTION,
    build_qft_gr_stress_energy_conservation_witness_attempt,
)
...
LEAN_ATTEMPT_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_StressEnergyConservationWitnessAttempt.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_stress_energy_conservation_witness_attempt_report.py"
)
...
def test_qft_gr_stress_energy_conservation_witness_attempt_files_exist() -> None:
...
def test_qft_gr_stress_energy_conservation_witness_attempt_consumes_review_and_executes() -> None:
    ...
    assert (
        attempt[
            "consumes_qft_gr_stress_energy_conservation_witness_packet_result_review"
        ]
        == REVIEW_ID
    )
...
def test_qft_gr_stress_energy_conservation_witness_attempt_records_exactly_one_obstruction_classification() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_stress_energy_conservation_witness_attempt_preserves_nonclaim_boundaries() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["conservation_witness_attempt_executed"] is True
...
def test_qft_gr_stress_energy_conservation_witness_attempt_selects_one_result_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        ...
        "prepare_qft_gr_stress_energy_conservation_obstruction_refinement_packet": "deferred",
...
def test_qft_gr_stress_energy_conservation_witness_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_stress_energy_conservation_witness_attempt(
