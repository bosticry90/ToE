...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_report import (
...
PACKET_PATH = (
    RELEASE_DIR / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_PACKET_20260522_v0.json"
)
DOCUMENTATION_PATH = PAPER_DIR / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_v0.md"
RESULT_REVIEW_PATH = (
    RELEASE_DIR
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche006DocumentationPacketResultReview.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_consumes_packet() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_PACKET_RESULT_REVIEW_20260522_v0"
    )
    ...
    assert review["consumes_packet"] == "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_PACKET_v0"
    assert review["consumes_packet_pointer"] == (
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_PACKET_20260522_v0.json"
    )


def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_preserves_selected_dependency() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_documentation_exists() -> None:
    review = _json(RESULT_REVIEW_PATH)
    documentation = _read(DOCUMENTATION_PATH)
    assert review["documentation_surface"]["surface"] == (
        "formal/docs/paper/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_v0.md"
    )
    assert review["documentation_surface"]["exists"] is True
    assert review["documentation_surface"]["accepted_as_documentation"] is True
    assert review["documentation_surface"]["documentation_text_length"] == len(documentation)
    assert "`[propext, Classical.choice, Quot.sound]`" in documentation
    assert "`project_axioms_used = []`" in documentation
    assert "does not clear `V01-ALPHA-DEP-REM-006`" in documentation
    assert "does not register blocker movement for tranche 006" in documentation
    ...
    assert f"Tranche 005 status: `{TRANCHE_005_STATUS}`" in documentation
    assert f"Tranche 006 status: `{POLICY_CLASSIFICATION}`" in documentation


def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_preserves_evidence_and_policy() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["documentation_result_review_classification"] == RESULT_REVIEW_CLASSIFICATION
    assert review["documentation_accepted_only_as_documentation"] is True


def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_keeps_blocker_uncleared() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["status_adjudication_packet_preparation_authorized"] is True
    assert review["status_adjudication_packet_prepared"] is False
    assert review["status_adjudication_executed"] is False
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_carries_posture() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_006_status"] == POLICY_CLASSIFICATION
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_remaining_blockers_tracked() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert [row["dependency_finding_id"] for row in other] == [TRANCHE_004_FINDING_ID]
    for row in other:
        assert row["modified_by_tranche_006_policy_adjudication"] is False


def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == "status_adjudication_packet_preparation_only"
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "prepare_v01_alpha_dependency_remediation_tranche_006_status_adjudication_packet": "selected",
        "execute_v01_alpha_dependency_remediation_tranche_006_status_adjudication": "deferred",
        "pause_v01_alpha_release_readiness_due_to_retained_tranche_004_blocker": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        REVIEW_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_DOCUMENTATION_PACKET_RESULT_REVIEW_20260522_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_documentation_packet_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006DocumentationPacketResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_documentation_packet_result_review_does_not_clear_blocker"
        in index_text
    )
