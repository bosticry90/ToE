...
REPORT_PATH = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_BOUNDED_ATTACK_20260503_v0.json"
)
SOURCE_REPORT_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QFT_GR_COVARIANT_CONSERVATION_OBLIGATION_SEMANTICS_RESULT_REVIEW_20260503_v0.json"
)
...
SURFACE_ID = "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_v0"
PREVIOUS_TARGET = "prepare_qft_gr_bianchi_compatibility_obligation_semantics_bounded_attack"
SOURCE_REVIEW_TARGET = "review_qft_gr_covariant_conservation_obligation_semantics_result"
RESULT_REVIEW_TARGET = "review_qft_gr_bianchi_compatibility_obligation_semantics_result"
RESULT_TOKEN = "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_SUPPLIED_ONLY"
REPORT_ID = (
    "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_BOUNDED_ATTACK_20260503_v0"
)
RETAINED_BLOCKER = "PHASE1-BLOCKER-QFTGR-BIANCHI-COMPATIBILITY-WITNESS-RETAINED"
...
ACTIVE_LANE = "qft_gr_bianchi_compatibility_obligation_semantics_result_review"
PREPARATION_LANE = "qft_gr_bianchi_compatibility_obligation_semantics_preparation"
...
def test_bianchi_compatibility_obligation_surface_records_supplied_obligation() -> None:
    ...
    for token in {
        ...
        RESULT_TOKEN,
        "QFTGRBianchiCompatibilityObligationSemanticPackage",
        "QFTGRBianchiCompatibilityObligationSemanticData",
        "hasBianchiCompatibilityObligation",
        "bianchiCompatibilitySatisfied",
        "supplied_bianchi_compatibility_obligation_semantics_constructs_package_v0",
        "qft_gr_covariant_conservation_obligation_semantics_does_not_force_bianchi_compatibility_witness_v0",
        "qft_gr_bianchi_compatibility_obligation_semantics_consumes_live_target_v0",
        ...
        "qft_gr_bianchi_compatibility_obligation_semantics_result_token_v0",
        "qft_gr_bianchi_compatibility_obligation_semantics_selected_next_target_v0",
    }:
        assert token in text


def test_bianchi_compatibility_obligation_surface_preserves_boundaries() -> None:
    text = _read(SURFACE_PATH)
...
def test_bianchi_compatibility_obligation_report_records_nonclaim_result() -> None:
    ...
    assert report["supplied_route"]["distinction"] == (
        "has_bianchi_obligation_does_not_imply_has_bianchi_witness_or_satisfaction"
    )
    assert report["refutation"]["status"] == (
        "covariant_conservation_obligation_only_bianchi_witness_derivation_refuted"
    )
    assert not any(report["nonclaim_boundaries"].values())


def test_registry_rotates_to_bianchi_compatibility_obligation_result_review() -> None:
    assert_current_target_consistent()
    assert_frontier_matches_registry()
    assert_forbidden_promotions_closed()
    assert_public_surfaces_match_registry()
    ...
    assert prep["bianchi_compatibility_obligation_semantics_surface"] == (
        SURFACE_EVIDENCE
    )
    assert prep["bianchi_compatibility_obligation_semantics_report"] == REPORT_EVIDENCE
    assert prep["bianchi_compatibility_obligation_semantics_result_token"] == RESULT_TOKEN
    assert prep["supplied_route_available"] == "yes"
    assert prep["covariant_conservation_obligation_only_bianchi_witness_refuted"] == (
        "yes"
    )
    assert prep["bianchi_witness_derived_from_covariant_conservation_obligation_alone"] == (
        "no"
    )
    assert prep["result_review_status"] == "prepared_for_live_result_review"
    assert prep["bianchi_compatibility_witness_authorized"] == "no"
    assert prep["actual_bianchi_compatibility_authorized"] == "no"
    assert prep["einstein_equation_coupling_authorized"] == "no"
    ...
    assert active["bianchi_compatibility_obligation_semantics_surface"] == (
        SURFACE_EVIDENCE
    )
    assert active["bianchi_compatibility_obligation_semantics_result_token"] == (
        RESULT_TOKEN
    )
    assert active["bianchi_compatibility_witness_authorized"] == "no"
    assert active["actual_bianchi_compatibility_authorized"] == "no"
    ...
    assert qft_gr["bianchi_compatibility_obligation_semantics_surface"] == (
        SURFACE_EVIDENCE
    )
    assert qft_gr["bianchi_compatibility_obligation_semantics_result_token"] == (
        RESULT_TOKEN
    )
    assert qft_gr["bianchi_compatibility_obligation_covariant_conservation_only_refuted"] == (
        "yes"
    )
    assert qft_gr["bianchi_witness_derived_from_covariant_conservation_obligation_alone"] == (
        "no"
    )
    assert qft_gr["bianchi_compatibility_witness_authorized"] == "no"
    assert qft_gr["actual_bianchi_compatibility_authorized"] == "no"
...
def test_public_surfaces_track_bianchi_compatibility_obligation_slice() -> None:
    for path in [README_PATH, STATE_PATH, ROADMAP_PATH, STRICT_MAP_PATH]:
        ...
        assert "QFT_GR_BianchiCompatibilityObligationSemantics.lean" in text
    ...
    assert "INV-MATH-QFTGR-BIANCHI-COMPATIBILITY-OBLIGATION-SEMANTICS-v0" in (
        inventory_text
    )
    ...
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_bianchi_compatibility_obligation_semantics_gate.py"
    )
