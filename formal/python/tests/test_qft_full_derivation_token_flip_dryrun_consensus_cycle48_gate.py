...
def test_cycle48_dryrun_consensus_tokens_in_discharge_doc() -> None:
    doc = _read(
        Path("formal/docs/paper/DERIVATION_TARGET_QFT_FULL_DERIVATION_DISCHARGE_v0.md")
    )
    assert (
        "QFT_FULL_DERIVATION_PROGRESS_CYCLE48_v0: "
        "TOKEN_FLIP_DRYRUN_CONSENSUS_LOCK_PINNED" in doc
    )
    assert (
        "QFT_FULL_DERIVATION_TOKEN_FLIP_DRYRUN_CONSENSUS_GATE_v0: "
        "REQUIRE_CONCURRENCE_COMPLETION_AND_MULTI_PARTY_CONSENSUS_NONWRITE" in doc
    )
    assert (
        "QFT_FULL_DERIVATION_TOKEN_FLIP_DRYRUN_CONSENSUS_SCOPE_v0: "
        "CYCLE47_CONCURRENCE_WITH_CYCLE37_46_CHAIN_REVIEW" in doc
    )
    assert (
        "QFT_FULL_DERIVATION_TOKEN_FLIP_DRYRUN_CONSENSUS_ARTIFACT_v0: "
        "qft_full_derivation_token_flip_dryrun_consensus_cycle48_v0" in doc
    )
    assert "formal/output/qft_full_derivation_token_flip_dryrun_consensus_cycle48_v0.json" in doc
...
def test_cycle48_dryrun_consensus_artifact_json_exists_and_is_consistent() -> None:
    artifact = _read(
        Path(
            "formal/output/"
            "qft_full_derivation_token_flip_dryrun_consensus_cycle48_v0.json"
        )
    )
    assert '"artifact_id": "qft_full_derivation_token_flip_dryrun_consensus_cycle48_v0"' in artifact
    assert '"cycle": 48' in artifact
    assert '"status": "consensed"' in artifact
