...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle03_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle02_v0.json"
...
def test_qm_stat_cycle03_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing QM-STAT Cycle03 target doc."
    assert ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle03 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle02 predecessor artifact."


def test_qm_stat_cycle03_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
    ...
    assert not missing, "QM-STAT Cycle03 doc missing required token(s): " + ", ".join(missing)


def test_qm_stat_cycle03_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle03_v0"
    assert artifact["seam_id"] == "SEAM-QM-STAT"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_HIGHER_MOMENT_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle02_v0"
    assert derived["artifact_path"] == "formal/output/qm_stat_class_b_seam_physics_pilot_cycle02_v0.json"


def test_qm_stat_cycle03_blocker_criteria_include_third_moment_parity() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "MASS_MEAN_VARIANCE_THIRD_MOMENT_PARITY_REQUIRED"
    ...
    qm_mu, qm_var, qm_m3 = _moments(xs, qm_p)
    st_mu, st_var, st_m3 = _moments(xs, stat_p)
...
def test_qm_stat_cycle03_higher_moment_exclusion_is_explicit() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    qm_mu, qm_var, qm_m3 = _moments(xs, qm_p)
    st_mu, st_var, st_m3 = _moments(xs, st_p)
    ...
    assert qm_m3 != st_m3 or qm_mu != st_mu or qm_var != st_var
    assert exclusion["classification"] == "NONCOMPATIBLE_EXCLUDED_v0"


def test_qm_stat_cycle03_nonclaim_boundary_and_adjudication() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    assert adjudication["token"] == "QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE03_ADJUDICATION"
