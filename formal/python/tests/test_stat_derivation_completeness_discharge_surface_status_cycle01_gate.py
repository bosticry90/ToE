...
def test_stat_derivation_completeness_discharge_surface_status_cycle01_gate() -> None:
    ...
    for doc_text, doc_label in (
        ...
        theorem_status = _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_v0")
        assert theorem_status in ALLOWED_THEOREM_STATUS_VALUES, (
            f"{doc_label} must keep the theorem-surface status either pre-admission or theorem-surface pinned."
        )
