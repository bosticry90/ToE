...
ASSESSMENT_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet08_assessment_checkpoint_v0.json"
...
def test_qft_gr_seam_packet08_assessment_document_structure() -> None:
    ...
    required_markers = [
        ...
        "did packet08 satisfy its exact bounded target?",
        "target_satisfaction_verdict: YES_EXACT_TARGET_SATISFIED_v0",
        "packet09_authorization_verdict: JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0",
        "packet09_exact_bounded_target:",
        "TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_STATUS_v0: ASSESSED_TARGET_SATISFACTION_VERIFIED_v0",
        "TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_GATE_v0: REQUIRED_PACKET08_ASSESSMENT_SCHEMA_AND_AUTHORITY_PARITY",
        "TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_ARTIFACT_v0: toe_qft_gr_seam_packet08_assessment_checkpoint_v0",
        "QFT_GR_SEAM_FORK_DECISION_STATUS_v0: HOLD_FOR_SCALAR_PUBLICATION_v0",
    ]
    for marker in required_markers:
        assert marker in text, f"Packet08 assessment doc missing marker: {marker}"


def test_qft_gr_seam_packet08_assessment_checkpoint_schema() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet08_assessment_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1G_QFT_GR_SEAM_PACKET08_ASSESSMENT"
    assert artifact.get("status") == "PACKET08_ASSESSMENT_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_v0.md"
    assert payload.get("parent_packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET08_BOUNDED_EXECUTION_v0.md"
    ...
    assert packet_artifact.get("status") == "PACKET08_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"
    assert auth_artifact.get("status") == "PACKET08_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"
    ...
    assert sat.get("authorized_exact_target") == (
        "freeze_residual_risk_classification_map_for_packet07_delta_rows_with_per_row_pass_fail_criterion_and_no_scalar_scope_backflow"
    )

    packet09 = payload.get("packet09_decision", {})
    assert packet09.get("authorized") is True
    assert packet09.get("verdict") == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"

    scalar = payload.get("scalar_compliance", {})
    assert scalar.get("scalar_scope_backflow_status") == "NO_BACKFLOW_DETECTED_v0"


def test_qft_gr_seam_packet08_assessment_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet08 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet08 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_assessment = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_STATUS_v0")
    roadmap_assessment = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_STATUS_v0")
