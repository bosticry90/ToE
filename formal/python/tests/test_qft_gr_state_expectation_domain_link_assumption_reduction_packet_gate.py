...
from formal.python.tools.qft_gr_state_expectation_domain_link_assumption_reduction_packet_report import (
    ...
    build_qft_gr_state_expectation_domain_link_assumption_reduction_packet,
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_StateExpectationDomainLinkAssumptionReductionPacket.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_state_expectation_domain_link_assumption_reduction_packet_report.py"
)
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_files_exist() -> None:
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_consumes_od002_review() -> None:
    ...
    assert (
        packet[
            "consumes_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_result_review"
        ]
        == RESULT_REVIEW_ID
    )
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_records_prior_rows() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_selects_only_row003() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_expectation_domain_link_assumption"]
    ...
    assert packet["state_expectation_domain_link_status_tokens"] == selected[
        "current_status"
    ]
    assert packet["row_status_enum"] == ROW_STATUS_ENUM


def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_expectation_domain_link_assumption"]
    assert packet["state_expectation_object"] == STATE_EXPECTATION_OBJECT
    assert packet["operator_domain_link_condition"] == OPERATOR_DOMAIN_LINK_CONDITION
    assert selected["state_expectation_object"] == STATE_EXPECTATION_OBJECT
    ...
    assert packet["state_expectation_domain_link_assumption_discharged"] is False
    assert (
        packet["state_expectation_domain_link_claimed_as_operator_domain_closed"]
        is False
    )


def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_expectation_domain_link_assumption_reduction_packet(
