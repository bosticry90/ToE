...
from formal.python.tools.qft_gr_state_admissibility_boundary_assumption_reduction_packet_report import (
    ...
    build_qft_gr_state_admissibility_boundary_assumption_reduction_packet,
)
...
def test_qft_gr_state_admissibility_boundary_packet_files_exist() -> None:
...
def test_qft_gr_state_admissibility_boundary_packet_consumes_sd001_review() -> None:
...
def test_qft_gr_state_admissibility_boundary_packet_selects_only_row002() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_admissibility_boundary_assumption"]
    ...
    assert packet["accepted_prior_state_domain_assumption_row"] == ACCEPTED_PRIOR_ROW_ID
    assert packet["accepted_state_domain_assumption_rows"] == [ACCEPTED_PRIOR_ROW_ID]
    ...
    assert packet["state_admissibility_boundary_status_tokens"] == selected[
        "current_status"
    ]


def test_qft_gr_state_admissibility_boundary_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_admissibility_boundary_assumption"]
    assert packet["state_admissibility_boundary_condition"] == (
        STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
    assert selected["state_admissibility_boundary_condition"] == (
        STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
    assert packet["state_admissibility_boundary"] == (
        STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
    assert selected["state_admissibility_boundary"] == (
        STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
...
def test_qft_gr_state_admissibility_boundary_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["state_admissibility_boundary_satisfied"] is False
    assert packet["state_admissibility_boundary_discharged"] is False
...
def test_qft_gr_state_admissibility_boundary_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        ...
        "execute_qft_gr_state_admissibility_boundary_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_state_admissibility_boundary_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_admissibility_boundary_assumption_reduction_packet(
