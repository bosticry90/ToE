...
INCREMENT_ASSESS_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_ASSESSMENT_NOTE_v0.md"
)
PARENT_PACKET_PATH = (
    REPO_ROOT
    / "formal"
...
def test_sliceb_increment01_artifacts_exist() -> None:
    for p in (INCREMENT_PACKET_PATH, INCREMENT_ASSESS_PATH):
        assert p.exists(), f"Missing Slice B Increment01 artifact: {p}"


def test_sliceb_increment01_anchor_parity() -> None:
    objective = _read(OBJECTIVE_PATH)
    parent = _read(PARENT_PACKET_PATH)
    packet = _read(INCREMENT_PACKET_PATH)
    assess = _read(INCREMENT_ASSESS_PATH)
...
def test_sliceb_increment01_invariance_and_nonclaim_markers() -> None:
    packet = _read(INCREMENT_PACKET_PATH)
    assess = _read(INCREMENT_ASSESS_PATH)
...
def test_sliceb_increment01_status_tokens_present() -> None:
    packet = _read(INCREMENT_PACKET_PATH)
    assess = _read(INCREMENT_ASSESS_PATH)

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_STATUS_v0: EXECUTED_BOUNDED_v0" in packet
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_ASSESSMENT_STATUS_v0: ASSESSED_BOUNDED_v0" in assess
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_OBJECTIVE_ADVANCEMENT_v0: YES" in assess
