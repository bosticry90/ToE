...
DOC = ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET29_ASSESSMENT_v0.md"
CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet29_assessment_checkpoint_v0.json"
)
PARENT_EXEC_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET29_BOUNDED_EXECUTION_v0.md"
)
PARENT_EXEC_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet29_bounded_execution_checkpoint_v0.json"
)
PARENT_AUTH_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET29_AUTHORIZATION_v0.md"
)
PARENT_AUTH_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet29_authorization_checkpoint_v0.json"
)
...
def test_packet29_assessment_files_exist() -> None:
    assert DOC.exists(), f"Missing packet29 assessment doc: {DOC}"
    assert CHECKPOINT.exists(), f"Missing packet29 assessment checkpoint: {CHECKPOINT}"


def test_packet29_assessment_doc_required_markers() -> None:
    text = _read_text(DOC)
    required_markers = [
        "# TOE QFT-GR Seam Packet29 Assessment v0",
        "## Assessment Questions",
        "## Objective Progress Snapshot",
        "## Scalar Freeze Compliance",
        "## Decision Rule Projection (for packet30 authorization)",
        "TOE_QFT_GR_SEAM_PACKET29_ASSESSMENT_STATUS_v0: ASSESSED_TARGET_SATISFACTION_VERIFIED_v0",
        "PACKET29_ASSESSMENT_COMPLETE_v0",
...
def test_packet29_assessment_checkpoint_schema_and_lineage() -> None:
    data = _load_json(CHECKPOINT)
    assert data["artifact_id"] == "toe_qft_gr_seam_packet29_assessment_checkpoint_v0"
    assert data["phase"] == "PHASE_2B_QFT_GR_SEAM_PACKET29_ASSESSMENT"
    assert data["scope"] == "packet29_target_satisfaction_assessment"
    assert data["status"] == "PACKET29_ASSESSMENT_COMPLETE_v0"

    payload = data["payload"]
    assert payload["assessment_doc_path"].endswith(
        "TOE_QFT_GR_SEAM_PACKET29_ASSESSMENT_v0.md"
    )
...
def test_packet29_assessment_target_satisfaction_and_packet30_rule() -> None:
    ...
    packet30 = payload["packet30_decision"]
    assert packet30["authorized"] is True
    assert packet30["progress_verdict"] == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet30["ladder_extension_verdict"] == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet30["verdict"] == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet30["exact_bounded_target"] == EXPECTED_PACKET30_TARGET
    assert packet30["hold_refine_trigger_status"] == "NOT_HIT_v0"


def test_packet29_assessment_scalar_and_guardrail_invariants() -> None:
    payload = _load_json(CHECKPOINT)["payload"]
    ...
    assert guardrails["assessment_scope"] == "ASSESS_PACKET29_BEFORE_PACKET30_v0"


def test_packet29_assessment_parent_chain_exists() -> None:
