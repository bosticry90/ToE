...
def test_synthesis_0110_exists_and_is_anchored() -> None:
    synthesis = _read(SYNTHESIS_0110_PATH)
    objective = _read(OBJECTIVE_PATH)
    increment10 = _read(INCREMENT10_PACKET_PATH)

    assert REQUIRED_QUESTION in objective
    assert REQUIRED_QUESTION in increment10
...
def test_synthesis_0110_contains_required_sections() -> None:
    synthesis = _read(SYNTHESIS_0110_PATH)
...
def test_synthesis_0110_tokens_present() -> None:
    synthesis = _read(SYNTHESIS_0110_PATH)

    assert REQUIRED_HOLD_TOKEN in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT11_DECISION_RULE_v0: "
        "REQUIRE_NEW_INCOMPATIBILITY_OR_DEPENDENCY_CRITERION_BEYOND_ORIGIN_PROVENANCE_EPOCH_BRANCH_IRREVERSIBILITY_FALLBACK_COMPLETENESS_WITNESS_STACK"
    ) in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_10_SYNTHESIS_STATUS_v0: "
        "SYNTHESIZED_BOUNDED_v0"
    ) in synthesis


def test_synthesis_0110_nonclaim_markers_present() -> None:
    synthesis = _read(SYNTHESIS_0110_PATH)
