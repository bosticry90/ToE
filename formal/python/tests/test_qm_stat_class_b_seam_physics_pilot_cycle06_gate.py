...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle06_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle05_v0.json"
...
def test_qm_stat_cycle06_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing QM-STAT Cycle06 target doc."
    assert ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle06 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle05 predecessor artifact."


def test_qm_stat_cycle06_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
    ...
    assert not missing, "QM-STAT Cycle06 doc missing required token(s): " + ", ".join(missing)


def test_qm_stat_cycle06_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle06_v0"
    assert artifact["seam_id"] == "SEAM-QM-STAT"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_EIGHTH_MOMENT_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle05_v0"
    assert derived["artifact_path"] == "formal/output/qm_stat_class_b_seam_physics_pilot_cycle05_v0.json"


def test_qm_stat_cycle06_blocker_criteria_include_eighth_moment_parity() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "MASS_MEAN_VARIANCE_THIRD_FOURTH_SIXTH_AND_EIGHTH_MOMENT_PARITY_REQUIRED"
    ...
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8 = _moments(xs, qm_p)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8 = _moments(xs, stat_p)
...
def test_qm_stat_cycle06_eighth_moment_exclusion_is_explicit() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8 = _moments(xs, qm_p)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8 = _moments(xs, st_p)
...
def test_qm_stat_cycle06_nonclaim_boundary_and_adjudication() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    assert adjudication["token"] == "QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE06_ADJUDICATION"
