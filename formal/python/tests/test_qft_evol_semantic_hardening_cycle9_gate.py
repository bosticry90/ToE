...
def test_qft_cycle9_gate_is_pinned_in_authority_surfaces() -> None:
    ...
    assert CYCLE9_GATE_PATH in state_text or CYCLE9_GATE_PATH in inventory_text, (
        f"State/Inventory authority surface must pin `{CYCLE9_GATE_PATH}`."
    )
    assert CYCLE9_MILESTONE_TOKEN in state_text or CYCLE9_MILESTONE_TOKEN in inventory_text, (
        "State/Inventory authority surface missing cycle-9 milestone token."
    )
    assert CYCLE9_PROGRESS_TOKEN in state_text or CYCLE9_PROGRESS_TOKEN in inventory_text, (
        "State/Inventory authority surface missing cycle-9 progress token."
    )
