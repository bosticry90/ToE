...
from formal.python.tools.qft_gr_state_domain_assumption_reduction_closeout_packet_report import (
    ...
    build_qft_gr_state_domain_assumption_reduction_closeout_packet,
)
...
def test_qft_gr_state_domain_assumption_reduction_closeout_packet_files_exist() -> None:
...
def test_qft_gr_state_domain_assumption_reduction_closeout_packet_consumes_result_review() -> None:
...
def test_qft_gr_state_domain_assumption_reduction_closeout_packet_records_all_three_rows() -> None:
    packet = _json(DEFAULT_OUT)
    rows = _accepted_rows()
    assert packet["accepted_state_domain_assumption_rows"] == rows
    assert packet["accepted_state_domain_assumption_row_count"] == 3
    assert packet["state_domain_assumption_row_count"] == 3
    assert packet["state_domain_assumptions_row_sequence_completed"] is True
    assert packet["state_domain_assumption_rows"] == [
        {
            "row_id": row,
            "status": "accepted",
            "source": source,
    ...
    assert packet["state_domain_assumptions_reduced_for_this_lane"] is True
    assert packet["state_domain_assumption_reduction_family_reduced"] is True
    assert packet["state_domain_assumption_reduction_row_inventory_exhausted"] is True
    assert packet["no_remaining_state_domain_assumption_row_in_current_inventory"] is True
    assert packet["next_state_domain_assumption_row"] is None


def test_qft_gr_state_domain_assumption_reduction_closeout_packet_preserves_blocker_and_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["state_domain_assumptions_family_closeout_prepared"] is True
    assert packet["state_domain_assumption_reduction_closeout_packet_prepared"] is True
    assert packet["state_domain_assumption_reduction_closeout_prepared"] is True
    assert packet["state_domain_assumption_reduction_closeout_status"] == (
        "prepared_pending_result_review"
    )
    assert packet["state_domain_assumption_reduction_closeout_result_review_required"] is True
    assert packet["state_domain_assumption_reduction_closeout_preparation_only"] is True
    assert packet["state_admissibility_claimed"] is False
    assert packet["state_admissibility_discharged"] is False
    assert packet["state_expectation_compatibility_claimed"] is False
    assert packet["state_expectation_compatibility_satisfied"] is False
    assert packet["state_domain_assumptions_discharged"] is False
    assert packet["state_domain_assumptions_discharged_by_closeout"] is False
    assert packet["state_domain_assumptions_reduced_or_discharged_by_closeout"] is False
...
def test_qft_gr_state_domain_assumption_reduction_closeout_packet_selects_result_review_only() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == NEXT_TARGET_KIND
    assert packet["state_domain_assumption_reduction_closeout_packet_selected_next_target"] == NEXT_TARGET
...
def test_qft_gr_state_domain_assumption_reduction_closeout_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_domain_assumption_reduction_closeout_packet(
    ...
    for token in [
        ...
        OUTCOME_ID,
        CLOSEOUT_CLASSIFICATION,
        "state_domain_assumptions_row_sequence_completed",
