...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_report.py"
)
...
LEAN_REGISTRATION_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche005BlockerMovementRegistration.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_consumes_result_review() -> None:
    registration = _json(REGISTRATION_PATH)
    assert registration["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_BLOCKER_MOVEMENT_REGISTRATION_20260515_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_selected_dependency() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_exact_movement() -> None:
    registration = _json(REGISTRATION_PATH)
    ...
    assert movement["movement_scope"] == "tranche_005_only"
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_preserves_evidence_policy_documentation_chain() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_registers_only_pending_review() -> None:
    registration = _json(REGISTRATION_PATH)
    movement = registration["registered_movement"]
    ...
    assert registration["tranche_005_formal_movement_accepted"] is False
    assert registration["tranche_005_cleared_for_global_release_readiness"] is False
    assert registration["tranche_005_release_blocker_status"] == (
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_carries_posture() -> None:
    registration = _json(REGISTRATION_PATH)
    ...
    assert registration["tranche_006_status"] == TRANCHE_006_STATUS
    ...
    assert registration["tranche_006_moved_or_cleared"] is False


def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_remaining_blockers_tracked() -> None:
    registration = _json(REGISTRATION_PATH)
    rows = registration["release_blocking_obligations_carry_forward"]
    assert registration["release_blocking_obligation_count"] == 3
    ...
    assert registration["other_release_blocking_obligation_count"] == 2
    assert (
        registration["remaining_release_blocking_obligation_count_excluding_tranche_005_candidate"]
        == 2
    )
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_forbidden_effects_false() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_next_target() -> None:
    registration = _json(REGISTRATION_PATH)
    ...
    assert {row["target"]: row["decision"] for row in registration["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_acceptance_and_determinism() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        EXECUTION_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_BLOCKER_MOVEMENT_REGISTRATION_20260515_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche005BlockerMovementRegistration" in index_text
    assert (
        "v01_dependency_remediation_tranche_005_blocker_movement_registration_does_not_promote_release"
        in index_text
    )
