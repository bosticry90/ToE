...
from formal.python.tools.qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt,
)
...
def test_qft_gr_derivative_exchange_regular_boundary_attempt_files_exist() -> None:
...
def test_qft_gr_derivative_exchange_regular_boundary_attempt_consumes_review() -> None:
...
def test_qft_gr_derivative_exchange_regular_boundary_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    review = _json(REVIEW_PATH)
    ...
    assert (
        attempt["derivative_exchange_regular_boundary_assumption_obstruction_identified"]
        is False
    )
    assert attempt["derivative_exchange_regular_boundary_assumption_inconclusive"] is False


def test_qft_gr_derivative_exchange_regular_boundary_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["derivative_exchange_regular_boundary_reduction_contract"]
    ...
    assert attempt["completed_prior_assumption_families"] == PRIOR_COMPLETED_FAMILIES
    assert attempt["operator_domain_assumptions_completed"] is True
    assert attempt["renormalization_assumptions_completed"] is True
    assert attempt["state_domain_assumptions_completed"] is True
    assert attempt["selected_mathematical_regularity_assumption_row"] == (
        SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW
    )
    assert attempt["selected_bounded_mathematical_regularity_assumption_row"] == (
        SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW
    )
    assert attempt["selected_row_count"] == 1
    assert attempt["derivative_exchange_regular_boundary"] == (
        DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY
    )
    assert contract["contract_id"] == DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_CONTRACT_ID
    assert contract["assumption_id"] == SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW
    assert contract["assumption_family"] == SELECTED_ASSUMPTION_FAMILY
    assert contract["regularity_condition"] == DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY
    assert contract["derivative_exchange_regular_boundary"] == (
        DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY
    )
    ...
    assert (
        contract["required_future_proof_object"]
        == "regularity_boundary_supports_covariant_derivative_exchange"
    )
    assert len(attempt["execution_findings"]) == 4


def test_qft_gr_derivative_exchange_regular_boundary_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert (
        attempt["derivative_exchange_regular_boundary_assumption_reduction_attempt_started"]
        is True
    )
    assert (
        attempt["derivative_exchange_regular_boundary_assumption_reduction_attempt_executed"]
        is True
    )
    assert (
        attempt["derivative_exchange_regular_boundary_assumption_reduced_by_attempt"]
        is True
    )
    assert attempt["derivative_exchange_regular_boundary_assumption_discharged"] is False
    assert (
        attempt["derivative_exchange_regular_boundary_assumption_discharged_by_attempt"]
        is False
    )
    assert (
        attempt["derivative_exchange_regular_boundary_reduced_or_discharged_by_attempt"]
        is False
    )
    assert (
        attempt["derivative_exchange_regular_boundary_reduced_or_discharged_by_implication"]
        is False
    )
    assert attempt["derivative_exchange_regular_boundary_globally_solved"] is False
    assert (
        attempt["derivative_exchange_regular_boundary_globally_solved_by_attempt"]
        is False
    )
    assert (
        attempt["derivative_exchange_regular_boundary_claimed_as_conservation_proof"]
        is False
    )
    assert (
        attempt["derivative_exchange_regular_boundary_claimed_as_state_admissibility"]
        is False
    )
    assert (
        attempt["derivative_exchange_regular_boundary_claimed_as_source_admissibility"]
        is False
    )
    assert (
        attempt["derivative_exchange_regular_boundary_claimed_as_bianchi_compatibility"]
        is False
    )
    assert attempt["mathematical_regularity_assumptions_discharged"] is False
    assert (
        attempt["mathematical_regularity_assumptions_reduced_or_discharged_by_attempt"]
        is False
    )
    ...
    assert attempt["state_admissibility_claimed"] is False
...
def test_qft_gr_derivative_exchange_regular_boundary_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        ...
        "discharge_qft_gr_derivative_exchange_regular_boundary_assumption": "not_authorized",
        "execute_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt": (
            "not_authorized_current_row"
        ),
...
def test_qft_gr_derivative_exchange_regular_boundary_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt(
