...
def test_qft_gr_seam_packet21_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet21 authorization pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet21 authorization pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_auth = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET21_AUTHORIZATION_STATUS_v0")
    roadmap_auth = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET21_AUTHORIZATION_STATUS_v0")
