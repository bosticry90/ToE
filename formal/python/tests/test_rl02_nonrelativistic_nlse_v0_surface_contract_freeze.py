...
from formal.python.toe.comparators.rl02_nonrelativistic_nlse_v0 import rl02_nonrelativistic_nlse_v0_record
...
def test_rl02_surface_contract_top_level_keys_frozen():
    rec = rl02_nonrelativistic_nlse_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-02_nonrelativistic_nlse_comparator/v0"
    assert payload["observable_id"] == "OV-RL-02"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-02"
...
def test_rl02_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl02_nonrelativistic_nlse_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 1
...
def test_rl02_scope_limits_include_discriminative_candidate():
    rec = rl02_nonrelativistic_nlse_v0_record(date="2026-02-09", tolerance_profile="pinned")
