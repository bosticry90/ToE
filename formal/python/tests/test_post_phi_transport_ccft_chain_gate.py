...
from formal.python.tools.scalar_nonzero_curvature_background_reports import (
    EXECUTION_OUTCOME as SCALAR_NONZERO_CURVATURE_EXECUTION_OUTCOME,
    EXECUTION_STRICT_OUTCOME as SCALAR_NONZERO_CURVATURE_EXECUTION_STRICT_OUTCOME,
    EXECUTION_TARGET as SCALAR_NONZERO_CURVATURE_EXECUTION_TARGET,
    HIGHER_DIMENSIONAL_CURVED_BACKGROUND_GUARDRAIL_TARGET,
    REVIEW_OUTCOME as SCALAR_NONZERO_CURVATURE_REVIEW_OUTCOME,
    REVIEW_STRICT_OUTCOME as SCALAR_NONZERO_CURVATURE_REVIEW_STRICT_OUTCOME,
    REVIEW_TARGET as SCALAR_NONZERO_CURVATURE_REVIEW_TARGET,
)
from formal.python.tools.scalar_higher_dimensional_curved_background_reports import (
    EXECUTION_TARGET as SCALAR_HIGHER_DIMENSIONAL_EXECUTION_TARGET,
    REVIEW_TARGET as SCALAR_HIGHER_DIMENSIONAL_REVIEW_TARGET,
)
from formal.python.tools.scalar_higher_dimensional_curved_background_result_review import (
    REVIEW_OUTCOME as SCALAR_HIGHER_DIMENSIONAL_REVIEW_OUTCOME,
    REVIEW_REPORT_PATH as SCALAR_HIGHER_DIMENSIONAL_REVIEW_REPORT_PATH,
    REVIEW_STRICT_OUTCOME as SCALAR_HIGHER_DIMENSIONAL_REVIEW_STRICT_OUTCOME,
...
from formal.python.tools.scalar_multi_background_robustness_reports import (
    EXECUTION_OUTCOME as SCALAR_MULTI_BACKGROUND_EXECUTION_OUTCOME,
    EXECUTION_REPORT_PATH as SCALAR_MULTI_BACKGROUND_EXECUTION_REPORT_PATH,
    EXECUTION_STRICT_OUTCOME as SCALAR_MULTI_BACKGROUND_EXECUTION_STRICT_OUTCOME,
    EXECUTION_TARGET as SCALAR_MULTI_BACKGROUND_EXECUTION_TARGET,
    REVIEW_TARGET as SCALAR_MULTI_BACKGROUND_REVIEW_TARGET,
    REVIEW_TARGET_KIND as SCALAR_MULTI_BACKGROUND_REVIEW_TARGET_KIND,
)


FINAL_LIVE_TARGET = SCALAR_MULTI_BACKGROUND_REVIEW_TARGET
FINAL_PREVIOUS_TARGET = SCALAR_MULTI_BACKGROUND_EXECUTION_TARGET
...
OPEN_SYSTEM_DECOHERENCE_SUPERCONDUCTING_CIRCUIT_QED_PLATFORM_RELEVANT_CANDIDATE_PLATFORM_SPECIFIC_LITERATURE_REVIEW_REVIEW_TARGET = (
...
BASELINE_COMPONENT_EQUATION_SOURCE_APPLICABILITY_OPEN_SYSTEM_DECOHERENCE_SUPERCONDUCTING_CIRCUIT_QED_PLATFORM_RELEVANT_CANDIDATE_PLATFORM_SPECIFIC_LITERATURE_REVIEW_PACKET_EVIDENCE = (
    str(lean_path(
        STAGES[
...
BASELINE_COMPONENT_EQUATION_SOURCE_APPLICABILITY_OPEN_SYSTEM_DECOHERENCE_SUPERCONDUCTING_CIRCUIT_QED_PLATFORM_RELEVANT_CANDIDATE_PLATFORM_SPECIFIC_LITERATURE_REVIEW_PACKET_REPORT = (
    str(release_path(
        STAGES[
            "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet"
        ]
...
BASELINE_COMPONENT_EQUATION_SOURCE_APPLICABILITY_OPEN_SYSTEM_DECOHERENCE_SUPERCONDUCTING_CIRCUIT_QED_PLATFORM_RELEVANT_CANDIDATE_PLATFORM_SPECIFIC_LITERATURE_REVIEW_PACKET_STRICT_OUTCOME = (
    STAGES[
        "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet"
    ].strict_outcome_id
)
...
CCFT_SCQED_LITERATURE_APPLICABILITY_MATRIX_CALCULATION_EXECUTION_EVIDENCE = (
    str(lean_path(
        STAGES[
...
FINAL_REPORT = str(
    SCALAR_MULTI_BACKGROUND_EXECUTION_REPORT_PATH.relative_to(REPO_ROOT)
).replace("\\", "/")
FINAL_OUTCOME = SCALAR_MULTI_BACKGROUND_EXECUTION_OUTCOME
FINAL_STRICT_OUTCOME = SCALAR_MULTI_BACKGROUND_EXECUTION_STRICT_OUTCOME
...
FINAL_KIND = SCALAR_MULTI_BACKGROUND_REVIEW_TARGET_KIND
NEXT_PACKET_OUTCOME = (
...
WRAPPER_BY_STAGE = {
    ...
    "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet": (
        "formal/python/tools/"
        "selected_ccft_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet_report.py"
    ),
    "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_review": (
        "formal/python/tools/"
...
WRAPPER_BUILD_FUNCTION_BY_STAGE = {
    ...
    "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet": (
        "formal.python.tools."
        "selected_ccft_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet_report",
        "build_selected_ccft_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet",
    ),
    "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_review": (
        "formal.python.tools."
...
def test_ccft_scqed_literature_applicability_matrix_calculation_execution_contract() -> None:
    report = build_stage_payload(
        "ccft_scqed_literature_applicability_matrix_calculation_execution"
    )

    assert report["calculation_sprint_guardrail_packet_consumed"] is True
    assert report["calculation_sprint_guardrail_enforced"] is True
    assert report["calculation_execution_completed"] is True
    assert report["safe_matrix_counts_calculation_executed"] is True
    assert report["calculation_output_generated"] is True
    assert report["calculation_manifest_generated"] is True
    assert report["reproducibility_metadata_generated"] is True
    assert report["reproducibility_hashes_verified"] is True
    assert report["calculation_input_sha256"] == (
        "4c26a1195e528748c79f0dbd0b9ef0653c26c4b92f765d8d95cac1ed512c58fc"
    )
    assert report["calculation_script_sha256"] == (
        "c5641407c07e3d271f158881b88fecbb2d0ba8d771a6921d4a2f999587f3e059"
    )
    assert report["calculation_output_sha256"] == (
        "0d738e72ef7caf187cd819595b9d6dcdf9bb0770d2be20fb01eb09d30427b685"
    )
    assert report["calculation_matrix_dimensions"]["total_rows"] == 48
    assert report["calculation_matrix_dimensions"][
        "complete_cartesian_matrix"
    ] is True
    assert report["calculation_status_distribution"] == {
        "blocked_missing_requirement_binding": 2,
        "not_applicable_for_requirement": 4,
        "partially_relevant_unvalidated": 23,
        "platform_relevant_unvalidated": 12,
    ...
    assert report["calculation_missing_variable_counts"][
        "total_occurrences"
    ] == 92
    assert report["calculation_missing_unit_counts"]["total_occurrences"] == 64
    assert report["calculation_missing_assumption_counts"][
        "total_occurrences"
    ] == 52
    assert report["calculation_blocked_missing_requirement_binding_count"] == 2
    assert report["calculation_unclear_requires_review_count"] == 7
    assert report["calculation_not_applicable_classification_count"] == 4
    assert report["e_repro_evidence_generated"] is True
    assert report["e_repro_evidence_claimed"] is True
    assert report["e_repro_claim_label"] == "E-REPRO"
    assert report["e_repro_claim_status"] == "generated_pending_result_review"
    assert report["calculation_result_review_pending"] is True
    assert report["calculation_result_review_completed"] is False
    assert report["selected_next_target"] == (
        CCFT_SCQED_LITERATURE_APPLICABILITY_MATRIX_CALCULATION_RESULT_REVIEW_TARGET
    )
    assert report["source_validated"] is False
    assert report["equation_adopted"] is False
    assert report["tau_baseline_value_computed"] is False
    assert report["r_tau_empirical_value_computed"] is False
    assert report["ccft_validated"] is False
    assert report["master_action_promoted"] is False
...
def test_post_phi_transport_ccft_registry_rotation_and_stage_rows() -> None:
    ...
    assert active["safe_matrix_counts_calculation_executed"] == "yes"
    assert active["calculation_output_generated"] == "yes"
    assert active["calculation_manifest_generated"] == "yes"
    assert active["reproducibility_metadata_generated"] == "yes"
    ...
    assert active["selected_next_target"] == SCIENCE_FIRST_REBASE_PREPARE_TARGET
    assert active["selected_next_target_kind"] == (
        "science_first_pillar_seam_dependency_rebase_packet"
    )
    assert active["suggested_next_packet_target"] == SCIENCE_FIRST_REBASE_PREPARE_TARGET
    assert active["suggested_next_packet_kind"] == (
        "science_first_pillar_seam_dependency_rebase_packet"
    )
    assert (
        active[
    ...
    assert active["platform_specific_literature_review_packet_prepared"] == "yes"
    assert active["platform_specific_literature_review_executed"] == "yes"
    assert active["platform_specific_literature_review_executed_as_locator_review_only"] == "yes"
    assert active["measurement_control_review_executed"] == "no"
    assert active["literature_review_executed"] == "yes"
    assert active["literature_review_executed_as_platform_specific_locator_review_only"] == "yes"
    assert active["platform_specific_literature_review_row_count"] == 4
    assert active["platform_specific_literature_review_relevant_unvalidated_row_count"] == 4
    assert active["platform_specific_literature_review_candidate_count"] == 2
    assert active["platform_specific_literature_review_requirement_count"] == 2
    assert active["platform_specific_literature_review_locator_count"] == 4
    assert active["platform_specific_literature_review_supporting_locator_count"] == 4
    assert active["platform_specific_literature_review_not_validated_row_count"] == 4
    assert active["platform_specific_literature_review_not_adopted_row_count"] == 4
    assert active["platform_specific_literature_review_tau_baseline_not_computed_row_count"] == 4
    assert active["platform_specific_literature_review_source_validation_statuses"] == [
        "not_validated"
    ]
    assert active["platform_specific_literature_review_equation_adoption_statuses"] == [
        "not_adopted"
    ]
    assert active["platform_specific_literature_review_tau_baseline_statuses"] == [
        "not_computed"
    ]
    assert active["platform_specific_literature_review_source_validated_count"] == 0
    assert active["platform_specific_literature_review_source_adopted_count"] == 0
    assert active["platform_specific_literature_review_source_replaced_count"] == 0
    assert active["platform_specific_literature_review_equation_import_count"] == 0
    assert active["platform_specific_literature_review_equation_adoption_count"] == 0
    assert active["platform_specific_literature_review_lindblad_import_count"] == 0
    assert active["platform_specific_literature_review_tau_baseline_computation_count"] == 0
    assert active["platform_specific_literature_review_reviewed_rows_source_validation_status"] == "not_validated"
    assert active["platform_specific_literature_review_reviewed_rows_equation_adoption_status"] == "not_adopted"
    assert active["platform_specific_literature_review_reviewed_rows_tau_baseline_status"] == "not_computed"
    assert active["platform_specific_literature_review_result_review_selected"] == "yes"
    assert (
        active[
            "measurement_control_review_route_retained_unresolved_after_platform_specific_literature_review_packet"
        ]
        == "yes"
    )
    ...
    assert active["selected_next_platform_specific_literature_review_result_review_target"] == (
        OPEN_SYSTEM_DECOHERENCE_SUPERCONDUCTING_CIRCUIT_QED_PLATFORM_RELEVANT_CANDIDATE_PLATFORM_SPECIFIC_LITERATURE_REVIEW_REVIEW_TARGET
    )
    assert active["selected_next_platform_specific_literature_review_result_review_kind"] == (
        BASELINE_COMPONENT_EQUATION_SOURCE_APPLICABILITY_OPEN_SYSTEM_DECOHERENCE_SUPERCONDUCTING_CIRCUIT_QED_PLATFORM_RELEVANT_CANDIDATE_PLATFORM_SPECIFIC_LITERATURE_REVIEW_REVIEW_KIND
    )
    assert (
        active[
            "baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_relevant_candidate_platform_specific_literature_review_packet_result_review_consumed"
        ]
        == "yes"
    )
    assert active["platform_specific_literature_review_packet_accepted"] == "yes"
    assert active["platform_specific_literature_review_accepted_only"] == "yes"
    assert (
        active[
    ...
    assert active["platform_specific_literature_review_relevance_support_accepted_only"] == "yes"
    assert active["platform_specific_literature_review_locator_rows_accepted"] == "yes"
    assert active["platform_specific_literature_review_locator_rows_accepted_only"] == "yes"
    assert active["accepted_platform_specific_literature_review_row_count"] == 4
    assert active["accepted_platform_specific_literature_review_candidate_count"] == 2
    assert active["accepted_platform_specific_literature_review_requirement_count"] == 2
    assert active["accepted_platform_specific_literature_review_locator_count"] == 4
    assert active["accepted_platform_specific_literature_review_supporting_locator_count"] == 4
    assert (
        active["accepted_platform_specific_literature_review_source_validation_status"]
        == "not_validated"
    )
    assert (
        active["accepted_platform_specific_literature_review_equation_adoption_status"]
        == "not_adopted"
    )
    assert (
        active["accepted_platform_specific_literature_review_tau_baseline_status"]
        == "not_computed"
    )
    assert active["accepted_platform_specific_literature_review_not_validated_row_count"] == 4
    assert active["accepted_platform_specific_literature_review_not_adopted_row_count"] == 4
    assert (
        active[
            "accepted_platform_specific_literature_review_tau_baseline_not_computed_row_count"
        ]
        == 4
    )
    ...
    assert active["e_repro_evidence_claimed"] == "yes"
    assert active["platform_specific_literature_applicability_crosswalk_packet_accepted"] == "yes"
    assert active["platform_specific_literature_applicability_crosswalk_accepted_only"] == "yes"
    assert (
        active[
    ...
    assert active["accepted_platform_specific_literature_applicability_crosswalk_row_count"] == 48
    assert active["accepted_platform_specific_literature_applicability_crosswalk_literature_locator_count"] == 4
    assert active["accepted_platform_specific_literature_applicability_crosswalk_requirement_count"] == 12
    assert active["accepted_platform_specific_literature_applicability_crosswalk_status_counts"] == {
        "platform_relevant_unvalidated": 12,
        "partially_relevant_unvalidated": 23,
        "unclear_requires_review": 7,
        "blocked_missing_requirement_binding": 2,
    ...
    assert active["accepted_platform_specific_literature_applicability_crosswalk_summary_counts"] == {
        "total_rows": 48,
        "platform_relevant_unvalidated": 12,
        "partially_relevant_unvalidated": 23,
        "unclear_requires_review": 7,
    ...
    assert active["platform_relevant_unvalidated_interpreted_as_validation"] == "no"
    assert active["partially_relevant_unvalidated_interpreted_as_adoption"] == "no"
    assert active["unclear_requires_review_interpreted_as_rejection"] == "no"
    assert active["not_applicable_for_requirement_interpreted_as_source_rejection"] == "no"
    assert active["calculation_readiness_metadata_accepted"] == "yes"
    assert active["calculation_readiness_metadata_accepted_as_future_input_only"] == "yes"
    assert active["accepted_calculation_readiness_metadata"] == {
        "calculation_role": "applicability_crosswalk_input",
        "calculation_allowed": "no",
        "future_calculation_candidate": "CALC-CCFT-SCQED-LITERATURE-APPLICABILITY-MATRIX-v0",
        "calculation_forbidden_reason": "source_not_validated_no_equation_adoption_no_tau_baseline_authorization",
    ...
    assert active["accepted_calculation_allowed"] == "no"
    assert active["accepted_calculation_executed"] == "no"
    ...
    assert (
        active[
            "residual_formula_changed_by_baseline_component_equation_source_applicability_gap_resolution_open_system_decoherence_superconducting_circuit_qed_platform_specific_literature_applicability_crosswalk_review"
        ]
        == "no"
    )
    assert (
    ...
    assert active["calculation_scaffold_started"] == "yes"
    assert active["calculation_matrix_executed"] == "yes"
    assert active["reproducible_calculation_executed"] == "yes"
