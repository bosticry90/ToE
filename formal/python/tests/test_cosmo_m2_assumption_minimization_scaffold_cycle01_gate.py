...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_m2_assumption_minimization_scaffold_cycle01_v0.json"

EXPECTED_ARTIFACT_ID = "cosmo_m2_assumption_minimization_scaffold_cycle01_v0"
...
def test_cosmo_m2_assumption_minimization_scaffold_cycle01_gate() -> None:
    cosmo_text = _read(COSMO_DOC_PATH)
    state_text = _read(STATE_PATH)
    roadmap_text = _read(ROADMAP_PATH)

    assert ARTIFACT_PATH.exists(), "COSMO M2 assumption minimization scaffold artifact is missing."
    ...
    assert _extract_token(roadmap_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID

    assert _extract_token(cosmo_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_SHA256_v0") == expected_sha
    assert _extract_token(state_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_SHA256_v0") == expected_sha
    assert _extract_token(roadmap_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_SHA256_v0") == expected_sha

    assert _extract_token(cosmo_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_GATE_v0") == EXPECTED_GATE
    assert _extract_token(state_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_GATE_v0") == EXPECTED_GATE
    assert _extract_token(roadmap_text, "COSMO_M2_ASSUMPTION_MINIMIZATION_GATE_v0") == EXPECTED_GATE

    artifact_rel = "formal/output/cosmo_m2_assumption_minimization_scaffold_cycle01_v0.json"
