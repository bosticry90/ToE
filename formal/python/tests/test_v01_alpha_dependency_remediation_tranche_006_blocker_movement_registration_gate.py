...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_report import (
...
RESULT_REVIEW_PATH = (
    RELEASE_DIR
    / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_PACKET_RESULT_REVIEW_20260522_v0.json"
)
REGISTRATION_PATH = (
    RELEASE_DIR
    / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_20260522_v0.json"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_report.py"
)
...
LEAN_REGISTRATION_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche006BlockerMovementRegistration.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_consumes_result_review() -> None:
    registration = _json(REGISTRATION_PATH)
    assert registration["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_20260522_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_selected_dependency() -> None:
    registration = _json(REGISTRATION_PATH)
    assert registration["execution_scope"] == (
        "EXECUTE_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_ONLY_NO_RELEASE_PROMOTION_OR_GLOBAL_DEBT_DISCHARGE"
    )
    assert registration["selected_tranche_id"] == SELECTED_TRANCHE_ID
    assert registration["selected_remediation_finding_id"] == SELECTED_FINDING_ID
    ...
    assert registration["lean_audit_target"]["command"] == LEAN_AUDIT_COMMAND


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_exact_movement() -> None:
    registration = _json(REGISTRATION_PATH)
    movement = registration["registered_movement"]
    assert movement["selected_tranche_id"] == SELECTED_TRANCHE_ID
    assert movement["selected_remediation_finding_id"] == SELECTED_FINDING_ID
    assert movement["selected_dependency"] == SELECTED_DEPENDENCY
    assert movement["previous_status"] == CURRENT_BLOCKER_STATUS
    assert movement["registered_status"] == CANDIDATE_BLOCKER_STATUS
    assert movement["registered_movement"] == PROPOSED_MOVEMENT
    assert movement["registered_movement_token"] == PROPOSED_MOVEMENT_TOKEN
    assert movement["movement_scope"] == "tranche_006_only"
    ...
    assert registration["registered_blocker_status"] == CANDIDATE_BLOCKER_STATUS
    assert registration["registered_movement_name"] == PROPOSED_MOVEMENT
    assert registration["registered_movement_token"] == PROPOSED_MOVEMENT_TOKEN


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_preserves_evidence_policy_documentation_chain() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_registers_only_pending_review() -> None:
    registration = _json(REGISTRATION_PATH)
    movement = registration["registered_movement"]
    ...
    assert registration["tranche_006_formal_movement_accepted"] is False
    assert registration["tranche_006_cleared_for_global_release_readiness"] is False
    assert registration["tranche_006_release_blocker_status"] == (
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_carries_posture() -> None:
    registration = _json(REGISTRATION_PATH)
    assert registration["tranche_001_status"] == TRANCHE_001_STATUS
    assert registration["tranche_002_status"] == TRANCHE_002_STATUS
    assert registration["tranche_003_status"] == TRANCHE_003_STATUS
    assert registration["tranche_004_status"] == TRANCHE_004_STATUS

    retained = registration["retained_tranche_004_carry_forward"]
    ...
    assert registration["release_readiness_blocked_by_tranche_004"] is True
    assert registration["tranche_004_moved_to_documented_dependency_nonblocking"] is False
    assert registration["tranche_004_retained_blocker_discharged"] is False

    assert registration["tranche_005_status"] == TRANCHE_005_STATUS
    assert registration["tranche_005_dependency"] == TRANCHE_005_DEPENDENCY
    assert registration["tranche_006_status"] == CANDIDATE_BLOCKER_STATUS
    tranche_006 = registration["tranche_006_obligation_carry_forward"]
    assert tranche_006["dependency_finding_id"] == SELECTED_FINDING_ID
    assert tranche_006["dependency"] == SELECTED_DEPENDENCY
    assert registration["tranche_006_moved_or_cleared"] is True


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_remaining_blockers_tracked() -> None:
    registration = _json(REGISTRATION_PATH)
    rows = registration["release_blocking_obligations_carry_forward"]
    assert registration["release_blocking_obligation_count"] == 2
    ...
    assert registration["other_release_blocking_obligation_count"] == 1
    assert (
        registration["remaining_release_blocking_obligation_count_excluding_tranche_006_candidate"]
        == 1
    )
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_forbidden_effects_false() -> None:
    registration = _json(REGISTRATION_PATH)
    ...
    assert registration["release_readiness_pause_registered"] is False
    assert registration["release_readiness_adjudication_prepared"] is False
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_next_target() -> None:
    registration = _json(REGISTRATION_PATH)
    ...
    assert registration["next_action_scope"] == (
        "REVIEW_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_RESULT_ONLY_NO_RELEASE_PROMOTION"
    )
    assert {row["target"]: row["decision"] for row in registration["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_result": "selected",
        "prepare_v01_alpha_dependency_remediation_next_tranche_selection_packet_after_tranche_006_movement": "deferred",
        "pause_v01_alpha_release_readiness_due_to_retained_tranche_004_blocker": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_acceptance_and_determinism() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        EXECUTION_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_BLOCKER_MOVEMENT_REGISTRATION_20260522_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_blocker_movement_registration_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006BlockerMovementRegistration" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_blocker_movement_registration_does_not_promote_release"
        in index_text
    )
