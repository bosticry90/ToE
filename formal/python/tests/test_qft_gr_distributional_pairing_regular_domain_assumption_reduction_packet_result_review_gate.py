...
from formal.python.tools.qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review_report.py"
)
LEAN_REVIEW_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_DistributionalPairingRegularDomainAssumptionReductionPacketResultReview.lean"
)
SURFACES_PATH = (
...
def test_qft_gr_distributional_pairing_packet_result_review_files_exist() -> None:
...
def test_qft_gr_distributional_pairing_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review[
            "consumes_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet"
        ]
        == PACKET_ID
    )
...
def test_qft_gr_distributional_pairing_packet_result_review_accepts_packet_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["packet_preparation_only_confirmed_by_review"] is True
    assert (
        review[
            "distributional_pairing_regular_domain_assumption_reduction_packet_reviewed"
        ]
        is True
    )
    assert (
        review[
            "distributional_pairing_regular_domain_assumption_reduction_packet_accepted"
        ]
        is True
    )
    assert (
        review[
            "distributional_pairing_regular_domain_assumption_reduction_packet_rejected"
        ]
        is False
    )
    assert (
        review[
            "distributional_pairing_regular_domain_assumption_reduction_packet_preparation_only"
        ]
        is True
    )
    assert review["mr_assump_003_attempt_executed_by_review"] is False
    assert (
        review[
...
def test_qft_gr_distributional_pairing_packet_result_review_preserves_selection() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["distributional_pairing_regular_domain"] == SELECTED_ROW_OBJECT
    assert review["distributional_pairing_domain_boundaries"] == (
        DISTRIBUTIONAL_PAIRING_DOMAIN_BOUNDARIES
    )
...
def test_qft_gr_distributional_pairing_packet_result_review_selects_attempt_target() -> None:
    review = _json(DEFAULT_OUT)
    packet = _json(PACKET_PATH)
    assert packet["selected_next_target"] == review["consumed_target"]
    ...
    assert (
        review[
            "distributional_pairing_regular_domain_assumption_reduction_packet_selected_next_target"
        ]
        == packet["selected_next_target"]
    )
    assert (
        review[
            "distributional_pairing_regular_domain_assumption_reduction_packet_result_review_selected_next_target"
        ]
        == NEXT_TARGET
    )
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        ...
        "review_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result": "completed_consumed_live_target",
...
def test_qft_gr_distributional_pairing_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_distributional_pairing_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review(
