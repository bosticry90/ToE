...
from formal.python.tools import (
    qm_stat_rl10_discrete_transition_bridge_interpretation_scope_uplift_gate_artifact_report as tool,
)
...
def _write_declaration(path: Path) -> None:
    _write_json(
        ...
        {
            "required_inputs": {
                "bridge_interpretation_scope_uplift_evidence_object_report": "formal/output/reports/qm_stat_rl10_discrete_transition_bridge_interpretation_scope_uplift_evidence_object_20260422_v0.json"
            },
            "seam_scope": {
                "external_comparator_id": "OV-RL-10",
                "bridge_quantity_id": "RL10_BRIDGE_SIGMA_DB_OBSERVABLE_v0",
            },
            "uplift_gate_artifact_policy": {
                "required_evidence_object_outcome": "INTERPRETATION_SCOPE_UPLIFT_EVIDENCE_OBJECT_DECLARED_AND_GATE_READY",
                ...
                "uplift_gate_artifact_id": "RL10_INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_v0",
                "uplift_gate_artifact_question": "What single bounded uplift-gate run can test for post-acceptance interpretation-scope uplift without reopening branch execution?",
                "uplift_gate_run_mode": "ONE_BOUNDED_UPLIFT_GATE_RUN_ONLY",
                "uplift_gate_success_condition": "DECLARED_SINGLE_RUN_RETURNS_DECLARED_POST_ACCEPTANCE_UPLIFT_STATE_CHANGE_SIGNAL",
            ...
            "uplift_gate_artifact_contract": {
                "allowed_outcomes": [
                    "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_DECLARED_AND_SINGLE_RUN_READY",
                    "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_DECLARATION_INVALID",
                    "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_SCOPE_VIOLATION",
                ],
                "single_terminal_outcome_rule": "EXACTLY_ONE_ALLOWED_INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_OUTCOME",
                "no_loop_rule": "DECLARATION_ONLY_NO_EXECUTION_REOPEN_IN_THIS_PACKET",
                "default_outcome": "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_DECLARATION_INVALID",
            },
        },
    )


def _seed_evidence_report(
    root: Path,
    *,
    outcome: str = "INTERPRETATION_SCOPE_UPLIFT_EVIDENCE_OBJECT_DECLARED_AND_GATE_READY",
    ...
    _write_json(
        ...
        / "qm_stat_rl10_discrete_transition_bridge_interpretation_scope_uplift_evidence_object_20260422_v0.json",
        {
            ...
            "objective_quality": {
                "inputs": {
                    "uplift_gate_id": gate_id,
                    "uplift_gate_contract": gate_contract,
                }
            },
        },
    )
...
def test_declared_and_single_run_ready_when_all_preconditions_and_guards_hold(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_evidence_report(tmp_path)

    report = tool.build_report(declaration_path=declaration_path, captured_at_utc=None)

    assert report["summary"]["review_outcome"] == "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_DECLARED_AND_SINGLE_RUN_READY"
...
def test_declaration_invalid_when_branch_execution_reopened_true(tmp_path: Path, monkeypatch) -> None:
    ...
    payload["uplift_gate_artifact_policy"]["branch_execution_reopened"] = True
    _write_json(declaration_path, payload)
    _seed_evidence_report(tmp_path)

    report = tool.build_report(declaration_path=declaration_path, captured_at_utc=None)

    assert report["summary"]["review_outcome"] == "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_DECLARATION_INVALID"
    assert report["criteria"]["branch_execution_reopened_is_false"] is False


def test_declaration_invalid_when_evidence_gate_id_mismatch(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(tool, "REPO_ROOT", tmp_path)
    declaration_path = (
        tmp_path
        / "formal"
    ...
    _write_declaration(declaration_path)
    _seed_evidence_report(tmp_path, gate_id="wrong_gate_id")
...
def test_scope_violation_when_input_scope_mismatches(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_declaration(declaration_path)
    _seed_evidence_report(tmp_path, comparator_id="OV-RL-10-ALT")

    report = tool.build_report(declaration_path=declaration_path, captured_at_utc=None)

    assert report["summary"]["review_outcome"] == "INTERPRETATION_SCOPE_UPLIFT_GATE_ARTIFACT_SCOPE_VIOLATION"
...
def test_declaration_invalid_when_required_text_field_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_declaration(declaration_path)
    payload = json.loads(declaration_path.read_text(encoding="utf-8"))
    payload["uplift_gate_artifact_policy"]["falsification_condition"] = ""
    _write_json(declaration_path, payload)
    _seed_evidence_report(tmp_path)
