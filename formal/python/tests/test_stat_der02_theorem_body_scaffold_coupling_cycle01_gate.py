...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_der02_regime_closure_theorem_body_scaffold_cycle01_v0.json"

EXPECTED_ARTIFACT_ID = "stat_der02_regime_closure_theorem_body_scaffold_cycle01_v0"
EXPECTED_GATE = "ARTIFACT_HASH_ROW_LABEL_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_ROW_BINDING = "TOE_STAT_DER_02_P_POLICY_THEOREM_BODY_SCAFFOLD_PINNED_NONCLAIM"
EXPECTED_ROW_ID = "TOE-STAT-DER-02"
EXPECTED_ROW_LABEL = "B-BLOCKED"
EXPECTED_ARTIFACT_REL = "formal/output/stat_der02_regime_closure_theorem_body_scaffold_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_der02_theorem_body_scaffold_coupling_cycle01_gate.py"
...
def test_stat_der02_theorem_body_scaffold_coupling_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT DER02 theorem-body scaffold coupling gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), "PILLAR-STAT matrix row must exist for DER02 theorem-body scaffold gate."
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT DER02 theorem-body scaffold payload_sha256 does not match canonical payload hash."
    )

    for token_name, expected in (
        ("STAT_DER02_THEOREM_BODY_SCAFFOLD_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_DER02_THEOREM_BODY_SCAFFOLD_CYCLE01_GATE_v0", EXPECTED_GATE),
        ("STAT_DER02_THEOREM_BODY_ROW_BINDING_v0", EXPECTED_ROW_BINDING),
    ):
    ...
    stat_sha = _extract_token(stat_text, "STAT_DER02_THEOREM_BODY_SCAFFOLD_CYCLE01_ARTIFACT_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_DER02_THEOREM_BODY_SCAFFOLD_CYCLE01_ARTIFACT_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_DER02_THEOREM_BODY_SCAFFOLD_CYCLE01_ARTIFACT_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert EXPECTED_ARTIFACT_REL in doc_text, f"{doc_label} must pin DER02 theorem-body scaffold artifact path."
        assert EXPECTED_GATE_REL in doc_text, f"{doc_label} must pin DER02 theorem-body scaffold gate path."
    ...
    assert "STAT_DER02_THEOREM_BODY_SCAFFOLD_CYCLE01_GATE_v0" in row_line
    assert "STAT_DER02_THEOREM_BODY_ROW_BINDING_v0" in row_line
    ...
    assert payload.get("status") == "regime_closure_theorem_body_scaffold_placeholder_nonclaim"
    ...
    assert payload.get("theorem_body_scope") == [
        "regime_validity_and_closure_coupling_theorem_body_scaffold_only",
        "regime_assumption_binding_structure_placeholder",
        "closure_coupling_structure_slots_placeholder",
        "dependency_on_der01_entropy_balance_scaffolds_required",
        "no_discharge_or_adequacy_claim",
    ]
    assert payload.get("required_theorem_body_components") == [
        "regime_validity_predicate_slot_placeholder",
        "closure_coupling_constraint_slot_placeholder",
        "assumption_freeze_linkage_slot_placeholder",
        "der01_entropy_balance_dependency_slot_placeholder",
        "future_discharge_adjudication_slot_placeholder",
    ]
