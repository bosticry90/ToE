...
REVIEW_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_REVIEW_v0.md"
REVIEW_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet52_targeted_justification_review_checkpoint_v0.json"
...
def test_qft_gr_seam_packet52_targeted_justification_review_document_structure() -> None:
    ...
    required_markers = [
        ...
        "## Review Outcome",
        "targeted_justification_verdict: INSUFFICIENT_FOR_PACKET52_AUTHORIZATION_v0",
        "hold_alignment_status: CONSISTENT_WITH_PACKET52_ELIGIBILITY_HOLD_v0",
        ...
        "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_GATE_v0: REQUIRED_PACKET52_TARGETED_JUSTIFICATION_SCHEMA_AND_HOLD_ALIGNMENT",
        "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_ARTIFACT_v0: toe_qft_gr_seam_packet52_targeted_justification_review_checkpoint_v0",
    ]
    for marker in required_markers:
        assert marker in text, f"Packet49 targeted justification review doc missing marker: {marker}"


def test_qft_gr_seam_packet52_targeted_justification_review_checkpoint_schema_and_outcome() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet52_targeted_justification_review_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2V_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_REVIEW"
    assert artifact.get("status") == "PACKET52_TARGETED_JUSTIFICATION_REVIEW_COMPLETE_INSUFFICIENT_v0"

    payload = artifact.get("payload", {})
    assert payload.get("review_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_REVIEW_v0.md"
    ...
    assert outcome.get("targeted_justification_verdict") == "INSUFFICIENT_FOR_PACKET52_AUTHORIZATION_v0"
    assert outcome.get("hold_alignment_status") == "CONSISTENT_WITH_PACKET52_ELIGIBILITY_HOLD_v0"
...
def test_qft_gr_seam_packet52_targeted_justification_review_authority_parity_and_freeze() -> None:
    ...
    for ref in refs:
        assert ref in state_text or ref in inventory_text, (
            f"Missing packet52 targeted justification pointer in compact-State or central inventory: {ref}"
        )
        assert ref in roadmap_text, f"Missing packet52 targeted justification pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_STATUS_v0")
    ...
    state_outcome = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_OUTCOME_v0"
    )
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET52_TARGETED_JUSTIFICATION_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_RETAINED_v0"

    assert not PACKET52_AUTH_DOC_PATH.exists(), "Packet49 authorization doc must not exist during targeted-justification hold"
    assert not PACKET52_AUTH_CHECKPOINT_PATH.exists(), "Packet49 authorization checkpoint must not exist during targeted-justification hold"
