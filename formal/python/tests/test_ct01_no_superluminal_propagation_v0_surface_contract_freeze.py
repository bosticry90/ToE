...
def test_ct01_surface_contract_top_level_keys_frozen():
    rec = ct01_no_superluminal_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "CT-01_no_superluminal_propagation_comparator/v0"
    assert payload["observable_id"] == "CT-01"
    assert payload["domain_id"] == "CT-DOMAIN-01"
...
def test_ct01_surface_contract_row_keys_and_reason_codes_frozen():
    rec = ct01_no_superluminal_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_ct01_scope_limits_include_discriminative_candidate():
    rec = ct01_no_superluminal_v0_record(date="2026-02-09", tolerance_profile="pinned")
