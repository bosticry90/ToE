...
from formal.python.tools.phi_ck_source_bridge_transport_rule_family_synthesis_result_review_report import (
    ...
    build_phi_ck_source_bridge_transport_rule_family_synthesis_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_ck_source_bridge_transport_rule_family_synthesis_result_review_report.py"
)
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_files_exist() -> None:
    for path in [
        SYNTHESIS_PACKET_PATH,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_accepts_packet() -> None:
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_preserves_triad() -> None:
    review = _json(DEFAULT_OUT)
    assert review["phi_ck_admissibility_rule_family_count"] == 3
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_blocks_claims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_validation_policy() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_rotates_to_closeout() -> None:
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        RECOMMENDED_AFTER_CLOSEOUT_SELECTOR_TARGET,
        ALTERNATE_AFTER_CLOSEOUT_SELECTOR_TARGET,
        "PhiCKSourceBridgeTransportRuleFamilySynthesisResultReview",
        "CURRENT_LIVE_NEXT_TARGET_v0: prepare_phi_ck_source_bridge_transport_rule_family_closeout",
        SOURCE_RULE_DISPLAY_FORM,
        SOURCE_CANDIDATE_CONSTRAINT_EQUATION,
        BRIDGE_ADMISSIBILITY_CONSTRAINT_FORM,
        TRANSPORT_ADMISSIBILITY_CONSTRAINT_FORM,
...
def test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_ck_source_bridge_transport_rule_family_synthesis_result_review_gate.py"
    )
