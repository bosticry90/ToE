...
def test_ov01_multi_fit_stability_dr04d_set_curved_retains_linear_prunes():
    base = REPO_ROOT / "formal" / "external_evidence" / "bec_bragg_steinhauer_2001"

    # Linear DR windows (upgraded set)
    ...
    # Curved DR windows (upgraded set)
    ...
    rep_lin = ov01_multi_fit_stability(fn, [dr02, dr03, dr04d_lin, dr05])
    rep_cur = ov01_multi_fit_stability_curved(fn, [dr02c, dr03c, dr04d, dr05c])

    # Determinism
    assert rep_lin.fingerprint() == ov01_multi_fit_stability(fn, [dr02, dr03, dr04d_lin, dr05]).fingerprint()
    assert rep_cur.fingerprint() == ov01_multi_fit_stability_curved(fn, [dr02c, dr03c, dr04d, dr05c]).fingerprint()

    # Qualitative locks only.
    assert rep_cur.r_max < rep_lin.r_max

    # Under tau=0.10, curved retains but linear prunes for this set.
    assert rep_cur.retain is True
    assert rep_lin.retain is False
