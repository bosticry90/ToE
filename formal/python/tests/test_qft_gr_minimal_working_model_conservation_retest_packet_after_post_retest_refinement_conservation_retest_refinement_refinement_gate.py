...
from formal.python.tools.qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement_report import (
    ...
    RETEST_CONDITION_ID,
    SCHEMA_ID,
    build_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement_report.py"
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    / "formal"
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_files_exist() -> None:
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_consumes_review() -> None:
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_defines_newest_delta() -> None:
    packet = _json(DEFAULT_OUT)
    delta = packet["newest_refinement_delta"]
    assert delta["delta_id"] == (
        "post_retest_refinement_conservation_retest_refinement_refined_toy_candidate_delta_v4"
    )
    assert delta["refinement_objective"] == REFINEMENT_OBJECTIVE
    assert delta["obstruction_class"] == OBSTRUCTION_CLASS
    changes = {
        row["component"]: row
    ...
    assert changes["scope_restriction"]["component_id"] == (
        "bounded_toy_weak_pairing_scope_v4"
    )
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_defines_retest_condition() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert "toy_weak_pairing_domain_v4_candidate" in condition["statement_template"]
    assert "toy_regular_context_v4_candidate" in condition["statement_template"]


def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_defines_outcomes() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    for key in [
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_validation_policy() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert (
        packet["validation_posture"]["release_index_lean_path"]
        == "not_freshly_validated_preserved_caveat"
    )


def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_selects_result_review_only() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_updates_live_target() -> None:
    ...
    assert packet_workstream["test_function_class_id"] == (
        "toy_conservation_test_function_class_v3_candidate"
    )
    assert packet_workstream["candidate_source_definition_id"] == (
        "toy_source_candidate_definition_v4_candidate"
    )
    ...
    assert active_workstream["post_retest_refinement_conservation_retest_refinement_refinement_packet_consumed"] == "yes"
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_deterministic() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement(
...
def test_post_retest_refinement_conservation_retest_refinement_refinement_packet_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement_gate.py"
    )
