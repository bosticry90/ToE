...
from formal.python.tools.v01_alpha_retained_tranche_004_source_map_authorization_adjudication_result_review_report import (
    ...
    OUTCOME_ID as AUTHORIZATION_RESULT_REVIEW_OUTCOME,
    RESULT_REVIEW_CLASSIFICATION as AUTHORIZATION_RESULT_REVIEW_CLASSIFICATION,
)
from formal.python.tools.v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_report import (
    ...
    build_source_map_closure_adjudication_packet,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_report.py"
)
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_files_exist() -> None:
    assert DEFAULT_AUTHORIZATION_RESULT_REVIEW_PATH.exists()
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_consumes_authorization_result_review_only() -> None:
    ...
    assert packet[
        "consumes_source_map_authorization_adjudication_result_review_pointer"
    ] == (
        "formal/docs/release/"
        "V01_ALPHA_RETAINED_TRANCHE_004_SOURCE_MAP_AUTHORIZATION_ADJUDICATION_RESULT_REVIEW_20260523_v0.json"
    )
    review = _json(DEFAULT_AUTHORIZATION_RESULT_REVIEW_PATH)
    assert review["outcome_id"] == AUTHORIZATION_RESULT_REVIEW_OUTCOME
    assert review["result_review_classification"] == AUTHORIZATION_RESULT_REVIEW_CLASSIFICATION
    assert review["selected_next_target"] == (
        "prepare_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet"
    )


def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_prepares_question_only() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["source_map_closure_adjudication_packet_prepared"] is True
    assert packet["source_map_closure_adjudication_packet_preparation_only"] is True
    assert packet["source_map_closure_adjudication_question_prepared"] is True
    assert packet["source_map_closure_adjudication_question_answered"] is False
    assert packet["source_map_closure_adjudication_execution_authorized_by_packet"] is False
    assert packet["source_map_closure_adjudication_execution_authorized"] is False
    assert packet["source_map_closure_adjudication_executed"] is False
    assert packet["source_map_closure_adjudication_result_review_authorized"] is False
    assert packet["closure_adjudication_question"] == (
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_carries_accepted_authorization_requirements() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["source_map_authorization_adjudication_result_reviewed"] is True
    assert packet["source_map_authorization_adjudication_result_accepted"] is True
    assert packet["requirements_satisfied_status_accepted_by_review"] is True
    assert packet["source_map_authorization_requirements_satisfied_accepted_by_review"] is True
    assert (
        packet[
    ...
    assert packet["source_map_authorization_result_accepted_as_closure_evidence"] is False
    assert packet["accepted_authorization_requirement_count"] == 7
    assert packet["reviewed_authorization_requirement_count"] == 7
    assert packet["closure_adjudication_requirement_count"] == 7
    assert {
        row["packet_status"] for row in packet["closure_adjudication_requirements"]
    ...
    assert packet["closure_adjudication_success_criteria_count"] == 4
    assert packet["closure_adjudication_failure_criteria_count"] == 4
    assert packet["closure_adjudication_execution_boundary_count"] == 5


def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_preserves_retained_blocker_and_release_hold() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_no_closure_or_promotion() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_forbidden_effects_false() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_selects_exactly_one_next_target() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_determinism() -> None:
    packet = _json(DEFAULT_OUT)
    for key, value in packet["acceptance_criteria"].items():
        assert value is True, f"Acceptance criterion failed: {key}"

    generated_1 = build_source_map_closure_adjudication_packet(
        review_path=DEFAULT_AUTHORIZATION_RESULT_REVIEW_PATH,
        captured_at_utc=DEFAULT_CAPTURED_AT_UTC,
    )
    generated_2 = build_source_map_closure_adjudication_packet(
        review_path=DEFAULT_AUTHORIZATION_RESULT_REVIEW_PATH,
        captured_at_utc=DEFAULT_CAPTURED_AT_UTC,
    )
...
def test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_is_pinned() -> None:
    roadmap_text = _read(ROADMAP_PATH)
    refs = [
        PACKET_ID,
        "formal/docs/release/V01_ALPHA_RETAINED_TRANCHE_004_SOURCE_MAP_CLOSURE_ADJUDICATION_PACKET_20260523_v0.json",
        "formal/python/tools/v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_report.py",
        "formal/python/tests/test_v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_gate.py",
        OUTCOME_ID,
        PACKET_CLASSIFICATION,
        AUTHORIZATION_RESULT_REVIEW_CLASSIFICATION,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01RetainedTranche004SourceMapClosureAdjudicationPacket" in index_text
    assert (
        "v01_alpha_retained_tranche_004_source_map_closure_adjudication_packet_prepares_packet_only"
        in index_text
    )
