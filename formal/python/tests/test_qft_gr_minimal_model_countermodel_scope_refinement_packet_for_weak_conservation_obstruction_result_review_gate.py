...
from formal.python.tools.qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction_result_review,
)
...
def test_countermodel_scope_refinement_packet_result_review_files_exist() -> None:
...
def test_countermodel_scope_refinement_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_scope_refinement_packet_id"] == PACKET_ID
...
def test_countermodel_scope_refinement_packet_result_review_accepts_packet_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["scope_refinement_packet_result_review_accepted"] is True
    assert review["scope_refinement_packet_consumed"] is True
    assert review["scope_refinement_packet_accepted"] is True
    assert review["countermodel_scope_refinement_packet_result_reviewed"] is True
    assert review["countermodel_scope_refinement_packet_result_review_pending"] is False
    assert review["countermodel_scope_refinement_packet_prepared"] is True
    assert review["countermodel_scope_refinement_packet_preparation_only"] is True
    assert review["scope_refinement_packet_is_not_strict_toy_witness_refutation"] is True


def test_countermodel_scope_refinement_packet_result_review_authorizes_scope_refinement_attempt_only() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["scope_refinement_attempt_authorized"] is True
    assert review["bounded_scope_refinement_attempt_authorized_only"] is True
    assert review["scope_refinement_attempt_executed"] is False
    assert review["countermodel_attempt_authorized_by_review"] is False
    assert review["countermodel_attempt_executed_by_review"] is False
    assert {
        row["target"]: row["decision"] for row in review["candidate_next_targets"]
    } == {
        NEXT_TARGET: "selected",
        CONSUMED_TARGET: "completed_consumed_live_target",
        "execute_qft_gr_minimal_model_countermodel_attempt_for_weak_conservation_obstruction_after_scope_refinement": (
...
def test_countermodel_scope_refinement_packet_result_review_preserves_pinned_scope() -> None:
    review = _json(DEFAULT_OUT)
    assert review["countermodel_search_space_refined"] is True
    ...
    assert review["scope_refinement_row_count"] == 3
    assert review["future_attempt_decision_criteria_count"] == 3


def test_countermodel_scope_refinement_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_countermodel_scope_refinement_packet_result_review_validation_policy() -> None:
    review = _json(DEFAULT_OUT)
...
def test_countermodel_scope_refinement_packet_result_review_updates_live_target() -> None:
    ...
    assert review_workstream["scope_refinement_packet_result_review_accepted"] == "yes"
    assert review_workstream["selected_next_target"] == NEXT_TARGET
    assert review_workstream["scope_refinement_attempt_authorized"] == "yes"
    assert review_workstream["scope_refinement_attempt_executed"] == "no"
    ...
    assert active_workstream["scope_refinement_packet_result_review_accepted"] == "yes"
    assert active_workstream["scope_refinement_attempt_authorized"] == "yes"
    assert active_workstream["bounded_scope_refinement_attempt_authorized_only"] == "yes"
    assert active_workstream["scope_refinement_attempt_executed"] == "no"
    assert active_workstream["countermodel_attempt_authorized_by_review"] == "no"
    assert active_workstream["countermodel_attempt_executed_by_review"] == "no"
...
def test_countermodel_scope_refinement_packet_result_review_deterministic() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction_result_review(
...
def test_countermodel_scope_refinement_packet_result_review_lean_and_surface_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        PINNED_WEAK_PAIRING_CONTRACT_ID,
        PINNED_EVALUATION_SCOPE_ID,
        "countermodelScopeRefinementPacketResultReviewAccepted",
        "boundedScopeRefinementAttemptAuthorizedOnly",
...
def test_countermodel_scope_refinement_packet_result_review_gate_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction_result_review_gate.py"
    )
