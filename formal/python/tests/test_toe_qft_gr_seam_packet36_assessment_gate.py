...
DOC = ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET36_ASSESSMENT_v0.md"
CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet36_assessment_checkpoint_v0.json"
)
PARENT_EXEC_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET36_BOUNDED_EXECUTION_v0.md"
)
PARENT_EXEC_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet36_bounded_execution_checkpoint_v0.json"
)
PARENT_AUTH_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET36_AUTHORIZATION_v0.md"
)
PARENT_AUTH_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet36_authorization_checkpoint_v0.json"
)
...
def test_packet36_assessment_files_exist() -> None:
    assert DOC.exists(), f"Missing packet36 assessment doc: {DOC}"
    assert CHECKPOINT.exists(), f"Missing packet36 assessment checkpoint: {CHECKPOINT}"


def test_packet36_assessment_doc_required_markers() -> None:
    text = _read_text(DOC)
    required_markers = [
        "# TOE QFT-GR Seam Packet36 Assessment v0",
        "## Assessment Questions",
        "## Objective Progress Snapshot",
        "## Scalar Freeze Compliance",
        "## Decision Rule Projection (for packet37 authorization)",
        "TOE_QFT_GR_SEAM_PACKET36_ASSESSMENT_STATUS_v0: ASSESSED_TARGET_SATISFACTION_VERIFIED_v0",
        "PACKET36_ASSESSMENT_COMPLETE_v0",
...
def test_packet36_assessment_checkpoint_schema_and_lineage() -> None:
    data = _load_json(CHECKPOINT)
    assert data["artifact_id"] == "toe_qft_gr_seam_packet36_assessment_checkpoint_v0"
    assert data["phase"] == "PHASE_2K_QFT_GR_SEAM_PACKET36_ASSESSMENT"
    assert data["scope"] == "packet36_target_satisfaction_assessment"
    assert data["status"] == "PACKET36_ASSESSMENT_COMPLETE_v0"

    payload = data["payload"]
    assert payload["assessment_doc_path"].endswith(
        "TOE_QFT_GR_SEAM_PACKET36_ASSESSMENT_v0.md"
    )
...
def test_packet36_assessment_target_satisfaction_and_packet37_rule() -> None:
    ...
    packet37 = payload["packet37_decision"]
    assert packet37["authorized"] is True
    assert packet37["progress_verdict"] == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet37["ladder_extension_verdict"] == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet37["verdict"] == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet37["exact_bounded_target"] == EXPECTED_PACKET37_TARGET
    assert packet37["hold_refine_trigger_status"] == "NOT_HIT_v0"


def test_packet36_assessment_scalar_and_guardrail_invariants() -> None:
    payload = _load_json(CHECKPOINT)["payload"]
    ...
    assert guardrails["assessment_scope"] == "ASSESS_PACKET36_BEFORE_PACKET37_v0"


def test_packet36_assessment_authority_pointer_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet36 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet36 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"


def test_packet36_assessment_parent_chain_exists() -> None:
