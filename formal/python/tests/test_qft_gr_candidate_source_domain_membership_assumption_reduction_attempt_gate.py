...
from formal.python.tools.qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_report.py"
)
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_consumes_review() -> None:
    ...
    assert (
        attempt[
            "consumes_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review"
        ]
        == REVIEW_ID
    )
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["candidate_source_domain_membership_assumption_inconclusive"] is False


def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["candidate_source_domain_membership_reduction_contract"]
    ...
    assert attempt["prior_accepted_operator_domain_assumption_row"] == PRIOR_ACCEPTED_ROW
    assert (
        attempt["prior_accepted_selected_operator_action_contract"]
        == SELECTED_OPERATOR_ACTION_CONTRACT_ID
    )
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["candidate_source_domain_membership_assumption_discharged"] is False
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt(
