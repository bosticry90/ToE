...
TARGET_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_EM_M4_SEAM_CLOSURE_PROMOTION_v0.md"
...
def test_em_m4_seam_closure_promotion_cycle01_gate() -> None:
    ...
    m4_completion = em_row.get("m4_completion", {})
    assert m4_completion.get("target_id") == "TARGET-EM-M4-SEAM-CLOSURE-PROMOTION-v0"
    assert m4_completion.get("doc_path") == "formal/docs/paper/DERIVATION_TARGET_EM_M4_SEAM_CLOSURE_PROMOTION_v0.md"
    assert m4_completion.get("artifact_path") == "formal/output/em_m4_seam_closure_promotion_cycle01_v0.json"
    assert m4_completion.get("gate_path") == "formal/python/tests/test_em_m4_seam_closure_promotion_cycle01_gate.py"

    for text in (target_text, em_text, state_text, roadmap_text):
        assert _extract_token(text, "EM_M4_STATUS_v0") == "COMPLETE_BOUNDED_v0"
        assert _extract_token(text, "EM_M4_SEAM_CLOSURE_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID
        assert _extract_token(text, "EM_M4_SEAM_CLOSURE_SHA256_v0") == artifact_hash
        assert _extract_token(text, "EM_M4_SEAM_CLOSURE_GATE_v0") == EXPECTED_GATE
        assert _extract_token(text, "EM_M4_PROMOTION_READINESS_v0") == "CROSS_PILLAR_SEAM_BUNDLE_PINNED_v0"
