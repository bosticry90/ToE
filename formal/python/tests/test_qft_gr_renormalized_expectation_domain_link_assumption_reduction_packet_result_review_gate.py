...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_RenormalizedExpectationDomainLinkAssumptionReductionPacketResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_report.py"
)
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review[
            "consumes_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet"
        ]
        == PACKET_ID
    )
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_confirms_row004() -> None:
    review = _json(DEFAULT_OUT)
    selected = review["renormalized_expectation_domain_link_assumption"]
    ...
    assert review["prior_rows001_002_003_remain_accepted"] is True
    ...
    assert review["renormalized_expectation_object"] == RENORMALIZED_EXPECTATION_OBJECT
    assert review["operator_domain_link_condition"] == OPERATOR_DOMAIN_LINK_CONDITION
    assert review["renormalized_expectation_domain_link_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert all(
        ...
        for token in review["renormalized_expectation_domain_link_status_tokens"]
    )


def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["packet_preparation_only_confirmed"] is True
    assert review["renormalized_expectation_domain_link_packet_accepted_by_review"] is True
    assert review["renormalized_expectation_domain_link_analysis_accepted"] is True
    assert review["renormalized_expectation_domain_link_assumption_reduced_by_review"] is False
    assert review["renormalized_expectation_domain_link_assumption_discharged"] is False
    assert (
        review["renormalized_expectation_domain_link_claimed_as_operator_domain_closed"]
        is False
    )
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_renormalized_expectation_domain_link_assumption_reduction_packet_result_review(
