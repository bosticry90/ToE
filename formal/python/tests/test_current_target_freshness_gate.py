...
ACTIVE_LANE = "prepare_ck_family_theorem_linkage_priority_selection_after_index"
ATTEMPT_TARGET = (
...
PREVIOUS_LIVE_TARGET = "review_ck_family_theorem_linkage_obligation_selection_after_index_result"
...
LIVE_TARGET = "prepare_ck_family_theorem_linkage_priority_selection_after_index"
...
def test_single_live_target_is_machine_pinned_after_samplerep32_audit_selector() -> None:
    ...
    assert payload["CURRENT_LIVE_TARGET_REPORT_v0"] == (
        "formal/docs/release/"
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_INDEX_RESULT_REVIEW_20260626_v0.json"
    )
    ...
    ck_family_theorem_linkage_obligation_selection_after_index_result_review_result = (
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_INDEX_RESULT_REVIEW_"
        "ACCEPTS_PRIORITY_SELECTION_PACKET_HANDOFF_NO_PROOF_EXECUTION_OR_"
        "MASTER_ACTION_PROMOTION"
    )
    ...
    assert interaction_active_workstream["priority_selection_packet_handoff_accepted"] == "yes"
    assert interaction_active_workstream["priority_selection_packet_selected"] == "yes"
    assert interaction_active_workstream[
    ...
    assert interaction_active_workstream["priority_selection_packet_prepared"] == "no"
    assert interaction_active_workstream["priority_selection_packet_executed"] == "no"
    assert interaction_active_workstream["rows_ranked"] == "no"
    assert interaction_active_workstream["likely_priority_candidate_count"] == "4"
    assert interaction_active_workstream["likely_first_priority_candidate"] == (
        "C_exchange theorem-linkage gap"
    )
    assert interaction_active_workstream["recommended_first_priority_row"] == (
        "C_exchange^{Apsi}"
    )
    assert interaction_active_workstream["selected_proof_target"] == "NONE_SELECTED"
    assert interaction_active_workstream["selected_theorem_row"] == "NONE_SELECTED"
    assert interaction_active_workstream["theorem_linkage_obligation_index_reviewed"] == "yes"
    assert interaction_active_workstream["obligation_index_reviewed"] == "yes"
    assert interaction_active_workstream["proof_obligation_rows_indexed"] == "yes"
    assert interaction_active_workstream["proof_target_selected"] == "no"
    assert interaction_active_workstream["priority_row_selected"] == "no"
    assert interaction_active_workstream["theorem_row_selected"] == "no"
    assert interaction_active_workstream["priority_selection_prepared"] == "no"
    assert interaction_active_workstream["priority_selection_executed"] == "no"
    assert interaction_active_workstream["proof_debt_target_selected"] == "no"
    ...
    assert interaction_active_workstream["no_C_k_functionalization_occurs"] == "yes"
    ...
    assert interaction_active_workstream["empirical_prediction_claimed"] == "no"
