...
def test_single_live_target_is_machine_pinned_after_samplerep32_audit_selector() -> None:
    ...
    assert (
        a_source_ck_active_workstream["authorized_next_strict_target"]
        == LIVE_TARGET
    )
    assert a_source_ck_active_workstream["authorized_target"] == LIVE_TARGET
    ...
    assert a_source_ck_active_workstream["packet_result"] == "REVIEW_ACCEPTED"
    ...
    assert a_source_ck_active_workstream["A_field_domain_policy"] == (
        "smooth real 1-form A on the selected spacetime domain"
    )
    assert a_source_ck_active_workstream["F_definition_policy"] == (
    ...
    consumed_a_source_ck_candidate_review = _workstream(payload, PREVIOUS_LIVE_TARGET)
    ...
    assert consumed_a_source_ck_candidate_review["selected_next_target"] == LIVE_TARGET
