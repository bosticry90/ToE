...
LIVE_TARGET_EVIDENCE_PATH = (
    ...
    / "ToeFormal"
    / "Derivation"
    / "QFTGRMinimalWorkingModelConservationRetestAttemptAfterPostRetestRefinement.lean"
)
...
def test_single_live_target_is_machine_pinned_after_samplerep32_audit_selector() -> None:
    ...
    post_retest_retest_attempt_result_review_active_workstream = active_workstream(payload)
    assert (
        post_retest_retest_attempt_result_review_active_workstream["workstream_id"]
        == ACTIVE_LANE
    )
    assert (
        post_retest_retest_attempt_result_review_active_workstream[
    ...
    assert post_retest_retest_attempt_result_review_active_workstream["report"] == (
        "formal/docs/release/"
        "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
        "RETEST_REFINEMENT_20260613_v0.json"
    )
    assert post_retest_retest_attempt_result_review_active_workstream["outcome_id"] == (
        "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
        "RETEST_REFINEMENT_EXECUTED_WITH_NO_CONSERVATION_PROOF_OR_SOURCE_"
        "ADMISSIBILITY"
    )
    assert (
        post_retest_retest_attempt_result_review_active_workstream["claim_level"]
        == "Level 3 minimal working model post-retest conservation retest attempt executed inconclusive pending review"
    )
    assert (
        post_retest_retest_attempt_result_review_active_workstream["claim_ceiling"]
        == "post-retest conservation retest attempt result review only"
    )
    assert (
        "no source admissibility"
        in post_retest_retest_attempt_result_review_active_workstream[
            "non_claim_boundary"
        ]
    )
    assert post_retest_retest_attempt_result_review_active_workstream[
        "consumed_target"
    ] == (
        PREVIOUS_LIVE_TARGET
    )
    assert (
        post_retest_retest_attempt_result_review_active_workstream[
            "post_retest_refinement_conservation_retest_attempt_consumed"
        ]
        == "yes"
    )
    assert (
    ...
    assert (
        post_retest_retest_attempt_result_review_active_workstream[
            "conservation_retest_result_recorded"
        ]
        == "yes"
    )
    assert (
    ...
    assert (
        post_retest_retest_attempt_result_review_active_workstream["retest_condition_id"]
