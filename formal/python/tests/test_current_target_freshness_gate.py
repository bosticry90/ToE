...
ACTIVE_LANE = (
    "qft_gr_covariant_derivative_operator_domain_packet_result_review"
)
PREVIOUS_TARGET = (
...
def test_single_live_target_is_machine_pinned_after_samplerep32_audit_selector() -> None:
    ...
    assert current_active_workstream["output_token"] == (
        "QFT_GR_COVARIANT_DERIVATIVE_OPERATOR_DOMAIN_PACKET_RESULT_REVIEW_"
        "ACCEPTS_OPERATOR_DOMAIN_PREPARATION_AND_AUTHORIZES_NEXT_BOUNDED_CONSERVATION_STATEMENT_PACKET_ONLY"
    )
    ...
    assert current_active_workstream["selected_route"] == (
        "qft_gr_covariant_conservation_statement_with_operator_domain_packet_preparation_after_operator_domain_result_review"
    )
    ...
    assert current_active_workstream["track2_started"] == (
        "covariant_derivative_operator_domain_packet_result_reviewed_statement_packet_pending"
    )
    ...
    assert current_active_workstream["covariant_conservation_statement_formulated"] == "no"
