...
ACTIVE_LANE = (
    "qft_gr_covariant_conservation_proof_object_packet"
)
PREVIOUS_TARGET = (
...
def test_single_live_target_is_machine_pinned_after_samplerep32_audit_selector() -> None:
    ...
    assert current_active_workstream["selected_route"] == (
        "qft_gr_covariant_conservation_proof_object_packet_result_review_after_packet_preparation"
    )
    ...
    assert current_active_workstream["track2_started"] == (
        "covariant_conservation_proof_object_packet_prepared_result_review_pending"
    )
    ...
    assert current_active_workstream["claim_ceiling"] == (
        "proof_object_packet_only_no_proof_construction_or_qft_gr_closure"
    )
