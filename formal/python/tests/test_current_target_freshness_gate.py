...
ACTIVE_LANE = (
    "qft_gr_covariant_conservation_proof_object_packet_result_review"
)
PREVIOUS_TARGET = (
...
def test_single_live_target_is_machine_pinned_after_samplerep32_audit_selector() -> None:
    ...
    assert current_active_workstream["selected_route"] == (
        "qft_gr_covariant_conservation_proof_object_attempt_execution_after_packet_result_review"
    )
    ...
    assert current_active_workstream["track2_started"] == (
        "covariant_conservation_proof_object_packet_result_reviewed_bounded_attempt_authorized"
    )
