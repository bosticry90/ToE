...
from formal.python.tools.qft_gr_selected_operator_action_assumption_reduction_attempt_report import (
    ...
    build_qft_gr_selected_operator_action_assumption_reduction_attempt,
)
...
LEAN_ATTEMPT_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_SelectedOperatorActionAssumptionReductionAttempt.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_selected_operator_action_assumption_reduction_attempt_report.py"
)
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_consumes_review() -> None:
    ...
    assert (
        attempt[
            "consumes_qft_gr_selected_operator_action_assumption_reduction_packet_result_review"
        ]
        == REVIEW_ID
    )
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["operator_action_assumption_reduced_pending_result_review"] is True
    assert attempt["operator_action_assumption_obstruction_identified"] is False
    assert attempt["operator_action_assumption_inconclusive"] is False


def test_qft_gr_selected_operator_action_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["selected_operator_action_reduction_contract"]
    ...
    assert (
        attempt["selected_operator_domain_assumption_row"]
        == SELECTED_OPERATOR_DOMAIN_ASSUMPTION_ROW
    )
    assert contract["contract_id"] == SELECTED_OPERATOR_ACTION_CONTRACT_ID
    assert contract["assumption_id"] == SELECTED_OPERATOR_DOMAIN_ASSUMPTION_ROW
    assert contract["assumption_family"] == PRIMARY_ASSUMPTION_FAMILY
    assert "covariant_divergence_operator_action" in contract["selected_operator_action"]
    assert len(attempt["execution_findings"]) == 4


def test_qft_gr_selected_operator_action_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["operator_action_assumption_discharged"] is False
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_selected_operator_action_assumption_reduction_attempt(
