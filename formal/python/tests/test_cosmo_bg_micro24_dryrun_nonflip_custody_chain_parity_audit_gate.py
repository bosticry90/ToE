...
COSMO_MICRO24_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_24_DRYRUN_NONFLIP_CUSTODY_CHAIN_PARITY_AUDIT_v0.md"
COSMO_MICRO24_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro24_dryrun_nonflip_custody_chain_parity_audit_cycle01_v0.json"
...
def test_cosmo_micro24_doc_contains_required_headers_and_tokens() -> None:
    ...
    assert not missing_headers, "COSMO micro-24 document is missing required header(s): " + ", ".join(missing_headers)
    ...
    assert not missing_tokens, "COSMO micro-24 document is missing required token(s): " + ", ".join(missing_tokens)


def test_cosmo_micro24_artifact_schema_and_scope_boundary() -> None:
    payload = _read_json(COSMO_MICRO24_ARTIFACT_PATH)

    assert payload.get("artifact_id") == "cosmo_bg_micro24_dryrun_nonflip_custody_chain_parity_audit_cycle01_v0"
    ...
    assert boundary_statement == "DRYRUN_NONFLIP_LANE_ONLY_CUSTODY_CHAIN_PARITY_AUDIT_NO_ADJUDICATION_FLIP_NO_COMPARATOR_AUTHORIZATION"


def test_cosmo_micro24_forbidden_tokens_not_present() -> None:
    doc_text = _read(COSMO_MICRO24_PATH)
    artifact_text = _read(COSMO_MICRO24_ARTIFACT_PATH)
    forbidden_tokens = [
...
def test_cosmo_micro24_cross_surface_pointers_are_complete() -> None:
    matrix = _read_json(MATRIX_PATH)
    cosmo = matrix.get("pillars", {}).get("PILLAR-COSMO")
    assert isinstance(cosmo, dict), "PILLAR-COSMO matrix row must exist."

    assert cosmo.get("dryrun_nonflip_custody_chain_parity_audit_doc") == "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_24_DRYRUN_NONFLIP_CUSTODY_CHAIN_PARITY_AUDIT_v0.md"
    assert cosmo.get("dryrun_nonflip_custody_chain_parity_audit_gate") == "formal/python/tests/test_cosmo_bg_micro24_dryrun_nonflip_custody_chain_parity_audit_gate.py"
    assert cosmo.get("dryrun_nonflip_custody_chain_parity_audit_policy") == "CYCLE08_09_10_11_12_13_14_15_16_17_18_19_20_21_22_23_DRYRUN_NONFLIP_CUSTODY_CHAIN_PARITY_AUDIT_LOCK_REQUIRED_NO_STATUS_FLIP_NO_COMPARATOR_AUTHORIZATION"
    ...
    assert not missing_state, "State missing dryrun nonflip custody-chain parity audit token(s): " + ", ".join(missing_state)
    ...
    assert not missing_target, "COSMO target missing dryrun nonflip custody-chain parity audit token(s): " + ", ".join(missing_target)
    ...
    assert not missing_rollup, "Rollup gate missing dryrun nonflip custody-chain parity audit cross-pin token(s): " + ", ".join(missing_rollup)
