...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche004SourceMapAuthorizationRemediationPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_consumes_result_review() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_scope() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["source_map_authorization_remediation_packet_prepared"] is True


def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_preserves_blocker_and_lean_posture() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_defines_required_evidence() -> None:
    packet = _json(PACKET_PATH)
    scope = packet["planning_scope"]
    assert scope["scope_kind"] == "SOURCE_MAP_AUTHORIZATION_REMEDIATION_PLANNING_ONLY"
    assert scope["current_blocker"] == CURRENT_BLOCKER
    assert scope["blocker_reason"] == BLOCKER_REASON
    assert packet["required_witness_chain_evidence"] == REQUIRED_WITNESS_CHAIN_EVIDENCE
    assert len(packet["required_witness_chain_evidence"]) == 10
    assert "qft_gr_source_map_closure_witness" in packet["required_witness_chain_evidence"]
    assert packet["required_source_map_semantic_closure_evidence"] == (
        REQUIRED_SOURCE_MAP_SEMANTIC_CLOSURE_EVIDENCE
    )
    assert "positive_full_source_map_semantic_closure_authorization_readout" in packet[
        "required_source_map_semantic_closure_evidence"
    ]
    assert packet["authorization_conditions"] == AUTHORIZATION_CONDITIONS
    assert packet["failure_conditions"] == FAILURE_CONDITIONS


def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_defines_policy_boundaries() -> None:
    packet = _json(PACKET_PATH)
    assert packet["documentation_role"]["documentation_can_help"] is True
    assert packet["documentation_role"]["documentation_alone_can_clear_blocker"] is False
    assert packet["documentation_role"]["documentation_packet_prepared_by_this_packet"] is False
    assert packet["lean_theorem_work_requirement"]["lean_theorem_work_required"] == (
        "likely_required_for_positive_source_map_authorization"
    )
    assert packet["lean_theorem_work_requirement"]["lean_theorem_work_executed_by_this_packet"] is False
    assert packet["expert_re_review_requirement"]["expert_re_review_required"] is True
    assert packet["expert_re_review_requirement"]["expert_re_review_executed_by_this_packet"] is False
    assert packet["post_remediation_adjudication_target"] == (
        "review_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_result"
    )


def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_keeps_blockers_tracked() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_no_closure_or_movement() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    assert packet["post_packet_review_target"] == NEXT_TARGET
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == (
        "tranche_004_source_map_authorization_remediation_packet_result_review_only"
    )
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_result": "selected",
        "execute_v01_alpha_dependency_remediation_tranche_004_bounded_source_map_witness_chain_evidence_construction": "deferred",
        "prepare_v01_alpha_dependency_remediation_tranche_004_documentation_only_retained_blocker_packet": "deferred",
        "prepare_v01_alpha_release_readiness_blocker_declaration_packet": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche004SourceMapAuthorizationRemediationPacket" in index_text
    assert (
        "v01_dependency_remediation_tranche_004_source_map_authorization_remediation_packet_does_not_claim_closure"
        in index_text
    )
