...
COSMO_MICRO27_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_27_DRYRUN_NONFLIP_CUSTODY_BOUNDARY_RECERTIFICATION_AUDIT_v0.md"
COSMO_MICRO27_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro27_dryrun_nonflip_custody_boundary_recertification_audit_cycle01_v0.json"
...
def test_cosmo_micro27_artifacts_exist() -> None:
    ...
    assert COSMO_MICRO27_ARTIFACT_PATH.exists(), "Missing COSMO background Cycle-027 artifact payload."
...
def test_cosmo_micro27_doc_contains_required_headers_and_tokens() -> None:
    ...
    assert not missing_headers, "COSMO micro-27 document is missing required header(s): " + ", ".join(missing_headers)
    ...
    assert not missing_tokens, "COSMO micro-27 document is missing required token(s): " + ", ".join(missing_tokens)


def test_cosmo_micro27_artifact_schema_and_scope_boundary() -> None:
    payload = _read_json(COSMO_MICRO27_ARTIFACT_PATH)

    assert payload.get("artifact_id") == "cosmo_bg_micro27_dryrun_nonflip_custody_boundary_recertification_audit_cycle01_v0"
    ...
    assert boundary_statement == "DRYRUN_NONFLIP_LANE_ONLY_CUSTODY_BOUNDARY_RECERTIFICATION_NO_ADJUDICATION_FLIP_NO_COMPARATOR_AUTHORIZATION"


def test_cosmo_micro27_forbidden_tokens_not_present() -> None:
    doc_text = _read(COSMO_MICRO27_PATH)
    artifact_text = _read(COSMO_MICRO27_ARTIFACT_PATH)
    forbidden_tokens = [
...
def test_cosmo_micro27_cross_surface_pointers_are_complete() -> None:
    matrix = _read_json(MATRIX_PATH)
    cosmo = matrix.get("pillars", {}).get("PILLAR-COSMO")
    assert isinstance(cosmo, dict), "PILLAR-COSMO matrix row must exist."

    assert cosmo.get("dryrun_nonflip_custody_boundary_recertification_audit_doc") == "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_27_DRYRUN_NONFLIP_CUSTODY_BOUNDARY_RECERTIFICATION_AUDIT_v0.md"
    assert cosmo.get("dryrun_nonflip_custody_boundary_recertification_audit_gate") == "formal/python/tests/test_cosmo_bg_micro27_dryrun_nonflip_custody_boundary_recertification_audit_gate.py"
    assert cosmo.get("dryrun_nonflip_custody_boundary_recertification_audit_policy") == "CYCLE08_09_10_11_12_13_14_15_16_17_18_19_20_21_22_23_24_25_26_DRYRUN_NONFLIP_CUSTODY_BOUNDARY_RECERTIFICATION_LOCK_REQUIRED_NO_STATUS_FLIP_NO_COMPARATOR_AUTHORIZATION"
    ...
    assert not missing_state, "State missing dryrun nonflip custody-boundary recertification token(s): " + ", ".join(missing_state)
    ...
    assert not missing_target, "COSMO target missing dryrun nonflip custody-boundary recertification token(s): " + ", ".join(missing_target)
    ...
    assert not missing_rollup, "Rollup gate missing dryrun nonflip custody-boundary recertification cross-pin token(s): " + ", ".join(missing_rollup)
