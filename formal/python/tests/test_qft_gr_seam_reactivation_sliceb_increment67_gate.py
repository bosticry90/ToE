...
DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT67_v0.md"
)


def _read() -> str:
    assert DOC_PATH.exists(), f"Missing Increment67 doc: {DOC_PATH}"
    return DOC_PATH.read_text(encoding="utf-8")


def test_increment67_doc_exists_and_anchors() -> None:
    text = _read()
...
def test_increment67_status_and_nonclaim_tokens_present() -> None:
    text = _read()

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT67_STATUS_v0: T-CONDITIONAL_BOUNDED_NONCLAIM" in text
    assert f"QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT67_DELTA_v0: {REQUIRED_DELTA}" in text
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT67_ADJUDICATION: DISCHARGED_v0_BOUNDED" in text
...
def test_increment67_template_and_governance_pointers_present() -> None:
    text = _read()
