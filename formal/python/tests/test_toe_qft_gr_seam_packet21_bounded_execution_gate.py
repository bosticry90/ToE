...
def test_qft_gr_seam_packet21_bounded_execution_document_structure() -> None:
    ...
    required_markers = [
        ...
        "packet21 is genuine physics progress rather than another ladder rung",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet21 execution doc missing marker: {marker}"


def test_qft_gr_seam_packet21_bounded_execution_checkpoint_schema_and_alignment() -> None:
    artifact = _read_json(PACKET_CHECKPOINT_PATH)
    auth_artifact = _read_json(AUTH_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet21_bounded_execution_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2A_QFT_GR_SEAM_PACKET21_BOUNDED_EXECUTION"
    assert artifact.get("status") == "PACKET21_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"

    payload = artifact.get("payload", {})
    assert payload.get("packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET21_BOUNDED_EXECUTION_v0.md"
    ...
    assert auth_payload.get("authorization_decision", {}).get("decision_outcome") == "AUTHORIZE_PACKET21_BOUNDED_TARGET_v0"

    binding = payload.get("authorized_target_binding", {})
    assert binding.get("authorization_decision_outcome") == "AUTHORIZE_PACKET21_BOUNDED_TARGET_v0"
    ...
    assert binding.get("authorized_exact_target") == auth_payload.get("packet21_bounded_target", {}).get("exact_target")
    assert binding.get("execution_target_match") == "EXACT_MATCH_CONFIRMED_v0"

    discr = payload.get("post_completion_closure_finalization_discriminator", {})
    assert discr.get("closure_finalization_witness_token") == "HANDOFF_CLOSURE_FINALIZATION_WITNESS_MET_v0"

    non_repetition = payload.get("non_repetition_validation", {})
    assert non_repetition.get("non_repetition_clause_status") == "ENFORCED_v0"
    assert non_repetition.get("result") == "PACKET21_ADDS_NEW_POST_COMPLETION_CLOSURE_FINALIZATION_DISCRIMINATOR_v0"

    ac = payload.get("acceptance_criteria", {})
    assert ac.get("ac1_input_state_binding_to_packet20_closure_completion_witness") == "PASS_v0"
    assert ac.get("ac2_discriminator_rule_explicitness") == "PASS_v0"
    assert ac.get("ac3_closure_finalization_witness_token_pinned") == "PASS_v0"
...
def test_qft_gr_seam_packet21_bounded_execution_authority_parity_and_invariance() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet21 execution pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet21 execution pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_packet21 = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET21_STATUS_v0")
    roadmap_packet21 = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET21_STATUS_v0")
    assert state_packet21 == roadmap_packet21 == "EXECUTED_BOUNDED_TARGET_STEP_v0"
