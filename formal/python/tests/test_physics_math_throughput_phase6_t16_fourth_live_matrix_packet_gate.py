...
DECLARATION_PATH = (
    ...
    / "docs"
    / "release"
    / "PHYS_MATH_THROUGHPUT_IMPLEMENTATION_TRANCHE_16_PHASE6_FOURTH_LIVE_MATRIX_PACKET_20260407_v0.md"
)
CHECKPOINT_PATH = (
    REPO_ROOT
    / "formal"
...
def test_phase6_t16_files_exist() -> None:
    assert PROGRAM_PATH.exists()
    assert DECLARATION_PATH.exists()
    assert CHECKPOINT_PATH.exists()


def test_phase6_t16_program_tokens_present() -> None:
...
def test_phase6_t16_checkpoint_contract() -> None:
    payload = _read_json(CHECKPOINT_PATH)
    assert payload.get("schema_id") == "PHYS_MATH_THROUGHPUT_PHASE6_T16_FOURTH_LIVE_MATRIX_PACKET_v0"
    assert payload.get("status") == "PHASE6_T16_FOURTH_LIVE_MATRIX_PACKET_EXECUTED_NONLIVE_NONCLAIM"

    coverage = payload.get("coverage_contract", {})
    assert coverage.get("declared_tranche_count") == 17
    assert len(coverage.get("tranche_ids", [])) == 17
