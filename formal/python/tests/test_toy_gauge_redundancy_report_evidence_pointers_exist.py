...
def test_toy_gauge_redundancy_report_artifact_matches_generator() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    report_path = repo_root / Path(DEFAULT_OUT_PATH)
    assert report_path.exists(), "Expected pinned gauge-redundancy report artifact to exist"
    ...
    assert payload.get("candidate_id") == "H1_phase_gauge"
