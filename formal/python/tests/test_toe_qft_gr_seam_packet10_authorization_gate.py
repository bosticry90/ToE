...
def test_qft_gr_seam_packet10_authorization_document_structure() -> None:
    ...
    required_markers = [
        ...
        "Parent objective:",
        "## Decision Branches",
        "branch_a_authorize_packet10: ACTIVE",
        "branch_b_hold_and_refine_objective: INACTIVE",
        "## Decision Preconditions (from packet09 assessment)",
        ...
        "decision_outcome: AUTHORIZE_PACKET10_BOUNDED_TARGET_v0",
        "TOE_QFT_GR_SEAM_PACKET10_AUTHORIZATION_STATUS_v0: AUTHORIZED_WITH_SINGLE_BOUNDED_TARGET_v0",
        "TOE_QFT_GR_SEAM_PACKET10_AUTHORIZATION_GATE_v0: REQUIRED_PACKET10_AUTHORIZATION_SCHEMA_AND_AUTHORITY_PARITY",
        "TOE_QFT_GR_SEAM_PACKET10_AUTHORIZATION_ARTIFACT_v0: toe_qft_gr_seam_packet10_authorization_checkpoint_v0",
        "QFT_GR_SEAM_FORK_DECISION_STATUS_v0: HOLD_FOR_SCALAR_PUBLICATION_v0",
    ]
    for marker in required_markers:
        assert marker in text, f"Packet10 authorization doc missing marker: {marker}"


def test_qft_gr_seam_packet10_authorization_checkpoint_schema_and_preconditions() -> None:
    artifact = _read_json(AUTH_CHECKPOINT_PATH)
    assessment_artifact = _read_json(ASSESSMENT_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet10_authorization_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1K_QFT_GR_SEAM_PACKET10_AUTHORIZATION"
    assert artifact.get("status") == "PACKET10_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET10_AUTHORIZATION_v0.md"
    assert payload.get("parent_assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET09_ASSESSMENT_v0.md"
    ...
    assessment_decision = assessment_artifact.get("payload", {}).get("packet10_decision", {})
    ...
    pre = payload.get("preconditions_from_packet09_assessment", {})
    ...
    assert decision.get("decision_outcome") == "AUTHORIZE_PACKET10_BOUNDED_TARGET_v0"
    assert decision.get("status") == "AUTHORIZED_WITH_SINGLE_BOUNDED_TARGET_v0"

    bounded = payload.get("packet10_bounded_target", {})
...
def test_qft_gr_seam_packet10_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet10 authorization pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet10 authorization pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_auth = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET10_AUTHORIZATION_STATUS_v0")
    roadmap_auth = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET10_AUTHORIZATION_STATUS_v0")
