...
def test_rl01_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "OV-RL-01_relativistic_dispersion_v0.md"

    rec = rl01_relativistic_dispersion_v0_record(date="2026-02-08", tolerance_profile="pinned")
    expected = render_rl01_lock_markdown(rec)
