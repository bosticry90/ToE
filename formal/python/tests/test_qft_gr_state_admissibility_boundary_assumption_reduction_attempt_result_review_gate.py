...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_state_admissibility_boundary_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_state_admissibility_boundary_attempt_result_review_consumes_attempt() -> None:
...
def test_qft_gr_state_admissibility_boundary_attempt_result_review_accepts_row002() -> None:
    review = _json(DEFAULT_OUT)
    contract = review["state_admissibility_boundary_reduction_contract"]
    ...
    assert review["accepted_state_domain_assumption_row_count"] == 2
    assert review["accepted_contract_id"] == STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_ID
    assert (
        review["bounded_state_admissibility_boundary_contract_status"]
        == BOUNDED_STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_STATUS
    )
    ...
    assert contract["accepted_prior_state_domain_assumption_row"] == ACCEPTED_PRIOR_ROW_ID
    assert (
        contract["state_admissibility_boundary"]
        == STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
    assert (
        contract["state_admissibility_boundary_condition"]
        == STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
    ...
    assert review["state_admissibility_boundary_assumption_reduction_accepted"] is True
    assert review["state_admissibility_boundary_assumption_reduction_rejected"] is False


def test_qft_gr_state_admissibility_boundary_attempt_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert (
        review[
    ...
    assert review["state_admissibility_boundary_assumption_accepted"] is True
    ...
    assert review["state_admissibility_boundary_assumption_discharged"] is False
    assert review["state_admissibility_boundary_assumption_discharged_by_review"] is False
    assert (
        review[
...
def test_qft_gr_state_admissibility_boundary_attempt_result_review_selects_next_state_domain_row() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["state_expectation_compatibility_assumption_packet_target"] == NEXT_TARGET
...
def test_qft_gr_state_admissibility_boundary_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review(
