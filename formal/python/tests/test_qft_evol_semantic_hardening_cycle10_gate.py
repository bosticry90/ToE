...
def test_qft_cycle10_gate_is_pinned_in_authority_surfaces() -> None:
    ...
    assert CYCLE10_GATE_PATH in state_text, f"State authority surface must pin `{CYCLE10_GATE_PATH}`."
    assert CYCLE10_MILESTONE_TOKEN in state_text, "State authority surface missing cycle-10 milestone token."
    assert CYCLE10_PROGRESS_TOKEN in state_text, "State authority surface missing cycle-10 progress token."
