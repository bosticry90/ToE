...
def test_cosmo_micro08_artifact_schema_and_token_alignment() -> None:
    ...
    assert payload.get("artifact_id") == "cosmo_bg_micro08_locked_queue_unlock_transition_packet_cycle01_v0"
    assert payload.get("artifact_version") == "v0"
    assert payload.get("placeholder_template") is True

    body = payload.get("payload")
    assert isinstance(body, dict)
    assert body.get("checkpoint") == "cosmo_bg_micro08_locked_queue_unlock_transition_packet_cycle01"
