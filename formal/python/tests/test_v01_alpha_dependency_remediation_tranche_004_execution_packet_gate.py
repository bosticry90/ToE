...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_004_execution_packet_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_004_execution_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche004ExecutionPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_consumes_selection_result_review() -> None:
    packet = _json(PACKET_PATH)
    assert packet["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_004_EXECUTION_PACKET_20260515_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_preserves_prior_tranches() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["tranche_004_status"] == "selected_for_execution_packet_preparation"
    assert packet["tranche_004_cleared_for_global_release_readiness"] is False
    assert packet["global_release_readiness_still_blocked"] is True


def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_selects_only_tranche_004() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_tracks_three_blockers() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["nonselected_release_blocking_obligation_count"] == 2
    assert [
        row["dependency_finding_id"]
        for row in packet["nonselected_release_blocking_obligations"]
    ] == NONSELECTED_BLOCKER_IDS
    for row in rows:
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_defines_audit_surface() -> None:
    packet = _json(PACKET_PATH)
    scope = packet["execution_scope"]
    required = packet["required_evidence_surface"]
    lean_target = packet["lean_dependency_audit_target"]
    source_map_target = packet["source_map_authorization_audit_target"]
    assert scope["scope_kind"] == (
        "QFT_GR_SOURCE_MAP_AUTHORIZATION_AND_DEPENDENCY_AUDIT_PREPARATION_ONLY"
    )
    ...
    assert required["surface_kind"] == "source_map_authorization_and_lean_axiom_audit_readout"
    ...
    assert required["negative_authorization_marker_expected"] is True
    ...
    assert required["authorization_status_readout_required"] is True
    assert lean_target["executed_by_this_packet"] is False
    assert source_map_target["executed_by_this_packet"] is False


def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_defines_review_requirements_and_criteria() -> None:
    packet = _json(PACKET_PATH)
    assert packet["audit_requirements"]["lean_audit_required"] is True
    assert packet["audit_requirements"]["authorization_surface_audit_required"] is True
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_prepares_without_execution() -> None:
    packet = _json(PACKET_PATH)
    assert packet["tranche_004_execution_packet_prepared"] is True
    ...
    assert packet["source_map_authorization_audit_executed"] is False
    assert packet["lean_dependency_evidence_captured"] is False
    assert packet["authorization_evidence_captured"] is False
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == (
        "tranche_004_execution_packet_result_review_only"
    )
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_004_execution_packet_result": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_004_execution_packet_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_004_execution_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_004_execution_packet_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche004ExecutionPacket" in index_text
    assert (
        "v01_dependency_remediation_tranche_004_execution_packet_does_not_execute_remediation"
        in index_text
    )
