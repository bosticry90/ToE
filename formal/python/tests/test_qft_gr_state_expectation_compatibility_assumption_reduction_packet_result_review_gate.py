...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_state_expectation_compatibility_assumption_reduction_packet_result_review_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_state_expectation_compatibility_packet_result_review_files_exist() -> None:
...
def test_qft_gr_state_expectation_compatibility_packet_result_review_consumes_packet() -> None:
...
def test_qft_gr_state_expectation_compatibility_packet_result_review_confirms_row003() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert (
        review["accepted_prior_state_domain_assumption_rows"]
        == ACCEPTED_PRIOR_ROW_IDS
    )
    assert review["accepted_state_domain_assumption_rows"] == ACCEPTED_PRIOR_ROW_IDS
    assert review["selected_state_domain_assumption_row"] == SELECTED_ROW_ID
    assert review["selected_row_count"] == 1
    assert review["state_expectation_compatibility_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert review["state_expectation_compatibility_condition"] == (
        STATE_EXPECTATION_COMPATIBILITY_CONDITION
    )
    assert review["packet_preparation_only_confirmed"] is True


def test_qft_gr_state_expectation_compatibility_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["state_expectation_compatibility_assumption_reduced_by_review"] is False
    assert (
        review[
...
def test_qft_gr_state_expectation_compatibility_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_state_expectation_compatibility_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_state_expectation_compatibility_assumption_reduction_packet_result_review(
