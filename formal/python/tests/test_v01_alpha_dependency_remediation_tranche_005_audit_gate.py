...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_005_audit_report import (
    AUDIT_ID,
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_005_audit_report.py"
)
...
LEAN_AUDIT_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche005Audit.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_consumes_result_review() -> None:
    ...
    assert audit["consumes_tranche_005_execution_packet_result_review"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_RESULT_REVIEW_v0"
    )
    assert audit["consumes_tranche_005_execution_packet_result_review_pointer"] == (
        "formal/docs/release/"
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0.json"
    )


def test_v01_alpha_dependency_remediation_tranche_005_audit_scope_is_single_dependency() -> None:
    audit = _json(AUDIT_PATH)
    ...
    assert audit["tranche_006_status"] == "tracked_unresolved"
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_carries_tranche_004_and_tranche_006() -> None:
    audit = _json(AUDIT_PATH)
    ...
    tranche_006 = audit["tranche_006_obligation_carry_forward"]
    assert tranche_006["dependency_finding_id"] == "V01-ALPHA-DEP-REM-006"
    assert tranche_006["status_carry_forward"] == (
        "tracked_unmodified_not_audited_in_tranche_004"
    )


def test_v01_alpha_dependency_remediation_tranche_005_audit_captures_exact_lean_evidence() -> None:
    audit = _json(AUDIT_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_classifies_project_axioms_separately() -> None:
    audit = _json(AUDIT_PATH)
    evidence = audit["lean_evidence"]
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_evidence_surfaces() -> None:
    audit = _json(AUDIT_PATH)
    surfaces = audit["evidence_surfaces_produced_or_updated"]
    assert {row["kind"]: row["status"] for row in surfaces} == {
        "tranche_005_audit_result_packet": "produced",
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_carries_forward_blockers() -> None:
    audit = _json(AUDIT_PATH)
    ...
    assert other[1]["status_carry_forward"] == (
        "tracked_unmodified_not_audited_in_tranche_004"
    )
    for row in other:
        assert row["modified_by_tranche_005"] is False


def test_v01_alpha_dependency_remediation_tranche_005_audit_no_broader_remediation_or_movement() -> None:
    audit = _json(AUDIT_PATH)
    assert audit["tranche_005_audit_result_classification"] == (
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_forbidden_effects_false() -> None:
    audit = _json(AUDIT_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_next_target() -> None:
    audit = _json(AUDIT_PATH)
    assert audit["post_audit_adjudication_target"] == NEXT_TARGET
    assert audit["selected_next_target"] == NEXT_TARGET
    assert audit["selected_next_target_kind"] == "tranche_005_audit_result_review_only"
    ...
    assert {row["target"]: row["decision"] for row in audit["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_005_audit_result": "selected",
        "prepare_v01_alpha_dependency_remediation_tranche_005_release_policy_adjudication_packet": "deferred",
        "pause_v01_alpha_release_readiness_due_to_retained_tranche_004_blocker": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_005_audit_acceptance_and_determinism() -> None:
    audit = _json(AUDIT_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_audit_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_005_audit_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_005_audit_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche005Audit" in index_text
    assert "v01_dependency_remediation_tranche_005_audit_does_not_move_blocker" in index_text
