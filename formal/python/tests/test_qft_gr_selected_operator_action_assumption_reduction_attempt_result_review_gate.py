...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_SelectedOperatorActionAssumptionReductionAttemptResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_report.py"
)
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_consumes_attempt() -> None:
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_accepts_contract_only() -> None:
    review = _json(DEFAULT_OUT)
    contract = review["selected_operator_action_reduction_contract"]
    ...
    assert review["accepted_contract_id"] == SELECTED_OPERATOR_ACTION_CONTRACT_ID
    assert contract["contract_id"] == SELECTED_OPERATOR_ACTION_CONTRACT_ID
    assert contract["assumption_id"] == SELECTED_ROW_ID
    assert review["operator_action_assumption_reduction_attempt_result_reviewed"] is True
    assert review["operator_action_assumption_reduction_accepted"] is True
    assert review["operator_action_assumption_reduction_rejected"] is False
    assert review["operator_action_assumption_reduced_pending_result_review_accepted"] is True
    assert review["operator_action_assumption_discharged"] is False


def test_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["operator_action_assumption_discharged_by_review"] is False
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_selects_next_operator_domain_row() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review(
