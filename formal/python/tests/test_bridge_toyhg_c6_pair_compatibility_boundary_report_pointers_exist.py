...
def test_bridge_toyhg_c6_pair_compatibility_boundary_report_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    report_rel = Path("formal/quarantine/bridge_tickets/BRIDGE_TOYHG_C6_pair_compatibility_BOUNDARY_REPORT.json")
    report_path = repo_root / report_rel
    assert report_path.exists(), "Expected Toy-H/Toy-G C6 pair-compatibility boundary report artifact to exist"
