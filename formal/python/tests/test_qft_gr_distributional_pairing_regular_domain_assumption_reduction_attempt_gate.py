...
from formal.python.tools.qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt,
)
...
LEAN_ATTEMPT_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_DistributionalPairingRegularDomainAssumptionReductionAttempt.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_distributional_pairing_attempt_files_exist() -> None:
...
def test_qft_gr_distributional_pairing_attempt_consumes_review() -> None:
...
def test_qft_gr_distributional_pairing_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    review = _json(REVIEW_PATH)
    ...
    assert (
        attempt[
            "distributional_pairing_regular_domain_assumption_obstruction_identified"
        ]
        is False
    )
    assert (
        attempt["distributional_pairing_regular_domain_assumption_inconclusive"]
        is False
    )


def test_qft_gr_distributional_pairing_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["distributional_pairing_regular_domain_reduction_contract"]
    ...
    assert attempt["accepted_prior_mathematical_regularity_assumption_rows"] == (
        ACCEPTED_PRIOR_ROW_IDS
    )
    ...
    assert attempt["distributional_pairing_regular_domain"] == SELECTED_ROW_OBJECT
    assert attempt["distributional_pairing_domain_boundaries"] == (
        DISTRIBUTIONAL_PAIRING_DOMAIN_BOUNDARIES
    )
    assert attempt["required_future_proof_object"] == REQUIRED_FUTURE_PROOF_OBJECT
    assert attempt["failure_mode_if_unresolved"] == FAILURE_MODE_IF_UNRESOLVED
    assert contract["contract_id"] == DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN_CONTRACT_ID
    ...
    assert contract["distributional_pairing_domain_boundaries"] == (
        DISTRIBUTIONAL_PAIRING_DOMAIN_BOUNDARIES
    )
...
def test_qft_gr_distributional_pairing_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert (
        attempt[
    ...
    assert (
        attempt["distributional_pairing_regular_domain_assumption_reduced_by_attempt"]
        is True
    )
    assert attempt["distributional_pairing_regular_domain_assumption_discharged"] is False
    assert (
        attempt[
            "distributional_pairing_regular_domain_assumption_discharged_by_attempt"
        ]
        is False
    )
    assert attempt["distributional_pairing_regular_domain_proved"] is False
...
def test_qft_gr_distributional_pairing_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        "review_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_result": "selected",
        "prepare_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_packet": "not_authorized_current_row",
        "prove_qft_gr_distributional_pairing_regularity": "not_authorized",
...
def test_qft_gr_distributional_pairing_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt(
