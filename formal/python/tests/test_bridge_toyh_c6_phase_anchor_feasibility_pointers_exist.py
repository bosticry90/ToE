...
def test_bridge_toyh_c6_phase_anchor_feasibility_artifact_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    artifact_rel = Path("formal/quarantine/feasibility/BRIDGE_TOYH_C6_phase_anchor_feasibility.json")
    artifact_path = repo_root / artifact_rel
    assert artifact_path.exists(), "Expected Toy-H C6 phase-anchor feasibility artifact to exist"

    payload = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert payload.get("schema_version") == 1
    assert payload.get("bridge_id") == "BRIDGE_TICKET_TOYH_0003"
    ...
    assert nodes, "Missing pytest_nodes evidence in Toy-H phase-anchor feasibility artifact"
    for node in nodes:
