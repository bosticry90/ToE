...
from formal.python.tools.qft_gr_renormalized_expectation_domain_assumption_reduction_packet_report import (
    ...
    SCHEMA_ID,
    SELECTED_ROW_ID,
    build_qft_gr_renormalized_expectation_domain_assumption_reduction_packet,
)
...
def test_qft_gr_renormalized_expectation_domain_packet_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_domain_packet_consumes_rn002_result_review() -> None:
...
def test_qft_gr_renormalized_expectation_domain_packet_selects_only_row003() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_expectation_domain_assumption"]
    ...
    assert packet["accepted_prior_renormalization_assumption_rows"] == [
        ACCEPTED_PRIOR_OBJECT_ROW,
        ACCEPTED_PRIOR_SCOPE_ROW,
    ]
    assert packet["accepted_prior_row_count"] == 2
    assert packet["accepted_prior_renormalization_assumption_row"] == (
        ACCEPTED_PRIOR_SCOPE_ROW
    )
    ...
    assert packet["renormalized_expectation_domain_status_tokens"] == selected[
        "current_status"
    ]


def test_qft_gr_renormalized_expectation_domain_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_expectation_domain_assumption"]
    ...
    assert packet["renormalized_expectation_domain_object"] == (
        RENORMALIZED_EXPECTATION_DOMAIN
    )
    assert packet["renormalized_expectation_domain_status"] == (
        RENORMALIZED_EXPECTATION_DOMAIN_STATUS
    )
    assert packet["domain_boundary"] == DOMAIN_BOUNDARY
    assert selected["renormalized_expectation_domain_object"] == (
        RENORMALIZED_EXPECTATION_DOMAIN
    )
    assert selected["renormalized_expectation_domain_status"] == (
        RENORMALIZED_EXPECTATION_DOMAIN_STATUS
    )
    assert selected["domain_boundary"] == DOMAIN_BOUNDARY
...
def test_qft_gr_renormalized_expectation_domain_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["renormalized_expectation_domain_assumption_discharged"] is False
    assert (
        packet[
...
def test_qft_gr_renormalized_expectation_domain_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_renormalized_expectation_domain_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_domain_assumption_reduction_packet(
