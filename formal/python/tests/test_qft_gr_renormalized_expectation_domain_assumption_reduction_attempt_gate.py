...
from formal.python.tools.qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt,
)
...
def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_consumes_review() -> None:
...
def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["renormalized_expectation_domain_assumption_inconclusive"] is False


def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["renormalized_expectation_domain_reduction_contract"]
    ...
    assert contract["renormalized_expectation_domain_object"] == (
        RENORMALIZED_EXPECTATION_DOMAIN
    )
    assert contract["domain_boundary"] == DOMAIN_BOUNDARY
    assert contract["contract_status"] == BOUNDED_DOMAIN_CONTRACT_STATUS
    assert contract["source_packet_domain_status"] == RENORMALIZED_EXPECTATION_DOMAIN_STATUS
...
def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["renormalized_expectation_domain_assumption_reduced_by_attempt"] is True
    assert attempt["renormalized_expectation_domain_assumption_discharged"] is False
    assert (
        attempt["renormalized_expectation_domain_assumption_discharged_by_attempt"]
        is False
    )
    assert (
        attempt[
    ...
    assert attempt["renormalized_expectation_domain_discharged_by_implication"] is False
    assert attempt["renormalized_expectation_domain_claimed_as_conservation_proof"] is False
    assert attempt["renormalized_expectation_domain_claimed_as_conservation_source"] is False
    assert (
        attempt["renormalized_expectation_domain_claimed_as_source_admissibility"]
        is False
    )
    assert (
        attempt["renormalized_expectation_domain_claimed_as_bianchi_compatibility"]
        is False
    )
...
def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt(
