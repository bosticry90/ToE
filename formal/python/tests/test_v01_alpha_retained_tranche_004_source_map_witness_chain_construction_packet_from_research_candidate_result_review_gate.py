...
from formal.python.tools.v01_alpha_retained_tranche_004_source_map_witness_chain_construction_packet_from_research_candidate_report import (
    ...
    OUTCOME_ID as CONSTRUCTION_PACKET_OUTCOME,
    PACKET_ID as CONSTRUCTION_PACKET_ID,
    RESULT_REVIEW_CLASSIFICATION as ACCEPTED_INPUT_CLASSIFICATION,
)
from formal.python.tools.v01_alpha_retained_tranche_004_source_map_witness_chain_construction_packet_from_research_candidate_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_construction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_retained_tranche_004_source_map_witness_chain_construction_packet_from_research_candidate_result_review_report.py"
)
...
def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_files_exist() -> None:
    assert DEFAULT_CONSTRUCTION_PACKET_PATH.exists()
...
def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_consumes_packet_only() -> None:
    ...
    assert review["consumes_construction_packet_from_research_candidate"] == (
        CONSTRUCTION_PACKET_ID
    )
    assert review["consumes_construction_packet_from_research_candidate_pointer"] == (
        "formal/docs/release/"
        "V01_ALPHA_RETAINED_TRANCHE_004_SOURCE_MAP_WITNESS_CHAIN_CONSTRUCTION_PACKET_FROM_RESEARCH_CANDIDATE_20260523_v0.json"
    )
    packet = _json(DEFAULT_CONSTRUCTION_PACKET_PATH)
    assert packet["outcome_id"] == CONSTRUCTION_PACKET_OUTCOME
    assert packet["selected_next_target"] == EXPECTED_PACKET_SELECTED_TARGET


def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_accepts_bounded_execution_only() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["construction_packet_result_reviewed"] is True
    assert review["construction_packet_result_accepted"] is True
    assert (
        review["construction_packet_accepted_for_bounded_construction_execution_only"]
        is True
    )
    assert review["result_review_classification"] == RESULT_REVIEW_CLASSIFICATION
    assert review["result_classification_count"] == 1
    assert review["accepted_input_classification"] == ACCEPTED_INPUT_CLASSIFICATION
    assert review["construction_packet_prepared"] is True
    assert review["construction_packet_prepared_only"] is True
    assert review["construction_execution_authorized_by_packet"] is False
    assert review["bounded_construction_execution_authorized"] is True
    assert review["bounded_construction_execution_authorized_by_review"] is True
    assert review["source_map_witness_chain_construction_execution_authorized"] is True
    assert review["source_map_witness_chain_construction_execution_authorized_by_review"] is True
    assert review["construction_execution_target"] == NEXT_TARGET


def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_preserves_candidate_material() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert {row["construction_packet_status"] for row in review["candidate_witness_chain_components"]} == {
        "candidate_input_only_not_constructed_by_packet"
    }
    assert "semantic_transport_link_map" in {
        row["evidence_id"] for row in review["required_evidence_surfaces"]
    }


def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_preserves_release_hold() -> None:
    review = _json(DEFAULT_OUT)
...
def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_no_execution_closure_or_status_movement() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["construction_packet_claimed_as_closure"] is False
...
def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_forbidden_effects_false() -> None:
    review = _json(DEFAULT_OUT)
...
def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_selects_exactly_one_next_target() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        ...
        POST_CONSTRUCTION_RESULT_REVIEW_TARGET: "deferred",
        REFINED_RESEARCH_TARGET: "deferred",
        ASSEMBLE_RELEASE_PACKET_TARGET: "not_authorized",
    }


def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_determinism() -> None:
    review = _json(DEFAULT_OUT)
    for key, value in review["acceptance_criteria"].items():
        assert value is True, f"Acceptance criterion failed: {key}"

    generated_1 = build_construction_packet_result_review(
        construction_packet_path=DEFAULT_CONSTRUCTION_PACKET_PATH,
        captured_at_utc=DEFAULT_CAPTURED_AT_UTC,
    )
    generated_2 = build_construction_packet_result_review(
        construction_packet_path=DEFAULT_CONSTRUCTION_PACKET_PATH,
        captured_at_utc=DEFAULT_CAPTURED_AT_UTC,
    )
...
def test_v01_alpha_retained_tranche_004_construction_packet_from_research_candidate_result_review_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_retained_tranche_004_source_map_witness_chain_construction_packet_from_research_candidate_result_review_report.py",
        "formal/python/tests/test_v01_alpha_retained_tranche_004_source_map_witness_chain_construction_packet_from_research_candidate_result_review_gate.py",
        OUTCOME_ID,
        RESULT_REVIEW_CLASSIFICATION,
        NEXT_TARGET,
    ]
    ...
    assert (
        "V01RetainedTranche004SourceMapWitnessChainConstructionPacketFromResearchCandidateResultReview"
        in index_text
    )
    assert (
        "v01_alpha_retained_tranche_004_source_map_witness_chain_construction_packet_from_research_candidate_result_review_authorizes_bounded_construction_execution_only"
        in index_text
    )
