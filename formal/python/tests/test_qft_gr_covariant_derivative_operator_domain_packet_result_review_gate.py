...
from formal.python.tools.qft_gr_covariant_derivative_operator_domain_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_covariant_derivative_operator_domain_packet_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_CovariantDerivativeOperatorDomainPacketResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_covariant_derivative_operator_domain_packet_result_review_report.py"
)
...
def test_qft_gr_covariant_derivative_operator_domain_packet_result_review_files_exist() -> None:
...
def test_qft_gr_covariant_derivative_operator_domain_packet_result_review_consumes_packet() -> None:
    ...
    assert review["primary_blocker"] == PRIMARY_BLOCKER == PRIMARY_MISSING_CONDITION


def test_qft_gr_covariant_derivative_operator_domain_packet_result_review_accepts_preparation_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["operator_domain_preparation_only_confirmed"] is True
    assert review["operator_domain_structure_prepared"] is True
    assert review["covariant_conservation_statement_formulated"] is False
    assert review["covariant_conservation_statement_attempted"] is False


def test_qft_gr_covariant_derivative_operator_domain_packet_result_review_preserves_nonclaim_boundaries() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_covariant_derivative_operator_domain_packet_result_review_selects_one_statement_packet_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_covariant_derivative_operator_domain_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_covariant_derivative_operator_domain_packet_result_review(
