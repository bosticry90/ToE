...
def test_toy_gauge_redundancy_negative_control_wrong_transform() -> None:
    inp = ToyHInput(
    ...
    wrong_gauge = [[x + 0.5, y] for x, y in state_after]
...
def test_toy_gauge_redundancy_negative_control_noninvariant_observable() -> None:
    inp = ToyHInput(
        state=GaugeStateInput(grid=[[1.0, 0.2], [2.0, -0.3], [0.5, 0.4]]),
        params=ToyHParams(dt=0.1, steps=2, theta=0.3, epsilon=1e-9, gauge_kind="phase_rotate"),
        candidate_id="H1_phase_gauge",
