...
def test_bridge_toyg_c6_mode_index_boundary_report_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    report_rel = Path("formal/quarantine/bridge_tickets/BRIDGE_TOYG_C6_mode_index_BOUNDARY_REPORT.json")
    report_path = repo_root / report_rel
    assert report_path.exists(), "Expected Toy-G C6 mode-index boundary report artifact to exist"
