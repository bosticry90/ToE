...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle08_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle07_v0.json"
...
def test_qm_stat_cycle08_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing QM-STAT Cycle08 target doc."
    assert ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle08 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle07 predecessor artifact."


def test_qm_stat_cycle08_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
    ...
    assert not missing, "QM-STAT Cycle08 doc missing required token(s): " + ", ".join(missing)


def test_qm_stat_cycle08_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle08_v0"
    assert artifact["seam_id"] == "SEAM-QM-STAT"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_TWELFTH_MOMENT_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle07_v0"
    assert derived["artifact_path"] == "formal/output/qm_stat_class_b_seam_physics_pilot_cycle07_v0.json"


def test_qm_stat_cycle08_blocker_criteria_include_twelfth_moment_parity() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "MASS_MEAN_VARIANCE_THIRD_FOURTH_SIXTH_EIGHTH_TENTH_AND_TWELFTH_MOMENT_PARITY_REQUIRED"
    ...
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8, qm_m10, qm_m12 = _moments12(xs, qm_p)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8, st_m10, st_m12 = _moments12(xs, stat_p)
...
def test_qm_stat_cycle08_twelfth_moment_exclusion_is_explicit() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8, qm_m10, qm_m12 = _moments12(xs, qm_p)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8, st_m10, st_m12 = _moments12(xs, st_p)
    ...
    assert qm_m12 == Fraction(exclusion["twelfth_central_moment"]["qm_m12"])
    assert st_m12 == Fraction(exclusion["twelfth_central_moment"]["stat_m12"])

    assert exclusion["classification"] == "NONCOMPATIBLE_EXCLUDED_v0"


def test_qm_stat_cycle08_nonclaim_boundary_and_adjudication() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    assert adjudication["token"] == "QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE08_ADJUDICATION"
