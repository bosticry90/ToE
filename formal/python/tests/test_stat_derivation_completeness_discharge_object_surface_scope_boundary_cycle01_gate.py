...
EXPECTED_ARTIFACT_ID = "stat_derivation_completeness_discharge_object_surface_scope_boundary_cycle01_v0"
...
def test_stat_derivation_completeness_discharge_object_surface_scope_boundary_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT derivation-completeness discharge object-surface scope-boundary gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for derivation-completeness discharge object-surface scope-boundary gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT derivation-completeness discharge object-surface scope-boundary cycle-01 payload_sha256 does not match canonical payload hash."
    )

    for token_name, expected in (
        ("STAT_DERIVATION_COMPLETENESS_DISCHARGE_OBJECT_SURFACE_SCOPE_BOUNDARY_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_DERIVATION_COMPLETENESS_DISCHARGE_OBJECT_SURFACE_SCOPE_BOUNDARY_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
    ):
    ...
    stat_sha = _extract_token(
        stat_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_OBJECT_SURFACE_SCOPE_BOUNDARY_CYCLE01_SHA256_v0"
    )
    state_sha = _extract_token(
        state_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_OBJECT_SURFACE_SCOPE_BOUNDARY_CYCLE01_SHA256_v0"
    )
    roadmap_sha = _extract_token(
        roadmap_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_OBJECT_SURFACE_SCOPE_BOUNDARY_CYCLE01_SHA256_v0"
    )
    ...
    for doc_text, doc_label in (
        ...
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin STAT derivation-completeness discharge object-surface scope-boundary artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin STAT derivation-completeness discharge object-surface scope-boundary gate path."
        )

    payload = artifact_json["payload"]
    assert payload.get("checkpoint") == "stat_derivation_completeness_discharge_object_surface_scope_boundary_cycle01"
