...
def test_qft_m2_completion_promotion_cycle01_gate() -> None:
    ...
    assert qft_row.get("m2_status") == "COMPLETE_BOUNDED_v0"

    m2_rows = qft_row.get("m2_exit_rows", {})
    for key in ROW_KEYS:
        row = m2_rows.get(key)
        assert isinstance(row, dict), f"Missing QFT M2 row `{key}`."
        assert row.get("token_value") == "RUN_BOUNDED_v0_NONCLAIM", f"QFT M2 row `{key}` must be active bounded-run."

    assert ARTIFACT_PATH.exists(), "QFT M2 completion promotion artifact is missing."
    ...
    assert _extract_token(qft_text, "QFT_M2_STATUS_v0") == "COMPLETE_BOUNDED_v0"
    assert _extract_token(state_text, "QFT_M2_STATUS_v0") == "COMPLETE_BOUNDED_v0"
    assert _extract_token(roadmap_text, "QFT_M2_STATUS_v0") == "COMPLETE_BOUNDED_v0"

    assert _extract_token(qft_text, "QFT_M2_COMPLETION_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID
    assert _extract_token(state_text, "QFT_M2_COMPLETION_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID
    assert _extract_token(roadmap_text, "QFT_M2_COMPLETION_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID

    assert _extract_token(qft_text, "QFT_M2_COMPLETION_SHA256_v0") == expected_sha
    assert _extract_token(state_text, "QFT_M2_COMPLETION_SHA256_v0") == expected_sha
    assert _extract_token(roadmap_text, "QFT_M2_COMPLETION_SHA256_v0") == expected_sha

    assert _extract_token(qft_text, "QFT_M2_COMPLETION_GATE_v0") == EXPECTED_GATE
