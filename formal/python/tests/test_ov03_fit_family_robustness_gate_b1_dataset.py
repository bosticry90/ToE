...
def test_ov03_fit_family_robustness_gate_runs_on_b1_dataset_when_artifacts_present():
    base = REPO_ROOT / "formal" / "external_evidence" / "bec_bragg_b1_second_dataset_TBD"

    # This test becomes active once the B1 dataset is populated and artifacts are generated.
    required = [
    ...
    if not all(p.exists() for p in required):
        pytest.skip("B1 dataset DR artifacts not yet generated")
    ...
    rep = ov03_fit_family_robustness_gate(
        fn,
    ...
    # Determinism
    assert rep.fingerprint() == ov03_fit_family_robustness_gate(
        fn,
        [dr02, dr03, dr04d, dr05],
        [dr02c, dr03c, dr04dc, dr05c],
        adequacy_policy="DQ-01_v1",
