...
def test_bridge_boundary_report_evidence_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    report_rel = Path("formal/quarantine/bridge_tickets/BRIDGE_BOUNDARY_REPORT.json")
    report_path = repo_root / report_rel
    assert report_path.exists(), "Expected bridge boundary report artifact to exist"
