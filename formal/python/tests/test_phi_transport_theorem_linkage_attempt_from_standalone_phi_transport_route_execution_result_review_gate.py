...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_result_review_report.py"
)
...
def test_phi_transport_execution_result_review_files_exist() -> None:
    for path in [
        EXECUTION_OUT,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_phi_transport_execution_result_review_accepts_local_linkage() -> None:
...
def test_phi_transport_execution_result_review_records_route_and_boundaries() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_transport_execution_result_review_records_lean_status() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_transport_execution_result_review_rotates_to_closeout_preparation() -> None:
...
def test_phi_transport_execution_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "PhiTransportTheoremLinkageAttemptFromStandalonePhiTransportRouteExecutionResultReview",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        CLOSEOUT_OUTCOME,
        ...
        MAIN_BOUNDARY,
        *LEAN_STATUS_WORDING_LINES_FOR_REVIEW,
        "PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_TRANSPORT_ROUTE_EXECUTION_RESULT_REVIEW_OUTCOME_v0",
        "PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_TRANSPORT_ROUTE_EXECUTION_RESULT_REVIEW_NONCLAIM_BOUNDARY_v0",
...
def test_phi_transport_execution_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_result_review_gate.py"
    )
