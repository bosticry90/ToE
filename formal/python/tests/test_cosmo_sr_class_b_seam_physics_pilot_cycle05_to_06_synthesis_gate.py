...
SYNTH_DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_COSMO_SR_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE05_TO_06_SYNTHESIS_v0.md"
)
CYCLE05_DOC_PATH = (
    REPO_ROOT
    / "formal"
...
def test_cosmo_sr_cycle05_to_06_synthesis_artifacts_exist() -> None:
    for path in (SYNTH_DOC_PATH, CYCLE05_DOC_PATH, CYCLE06_DOC_PATH, CYCLE05_ARTIFACT_PATH, CYCLE06_ARTIFACT_PATH):
        assert path.exists(), f"Missing required file: {path}"


def test_cosmo_sr_cycle05_to_06_synthesis_doc_tokens() -> None:
    ...
    assert not missing, "COSMO-SR Cycle05-to-06 synthesis doc missing required token(s): " + ", ".join(missing)


def test_cosmo_sr_cycle05_to_06_additive_delta_is_material() -> None:
    ...
    for s5, s6 in zip(c5["samples"], c6["samples"]):
        z5 = Fraction(s5["z"])
        z6 = Fraction(s6["z"])
        assert z5 == z6
        ...
        assert beta_exact5 == beta_exact6 == _sr_beta_exact(z5)

        assert Fraction(s5["beta_series8"]) == _series8(z5)
        assert Fraction(s6["beta_series8"]) == _series8(z6)
        ...
        assert abs_delta_s10 <= abs_delta_s8


def test_cosmo_sr_cycle05_to_06_decic_exclusion_and_blockers() -> None:
    cycle05 = _json(CYCLE05_ARTIFACT_PATH)
    cycle06 = _json(CYCLE06_ARTIFACT_PATH)
    ...
    z = Fraction(c6_ex["z"])
    beta_exact = Fraction(c6_ex["beta_exact"])
    beta_series10 = Fraction(c6_ex["beta_series10"])
    abs_delta = Fraction(c6_ex["abs_delta"])
...
def test_cosmo_sr_cycle05_to_06_nonclaim_boundary_preserved() -> None:
    for artifact in (_json(CYCLE05_ARTIFACT_PATH), _json(CYCLE06_ARTIFACT_PATH)):
