...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_failure_trigger_discharge_object_surface_status_cycle01_v0.json"
THEOREM_STATUS_ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "stat_failure_trigger_discharge_theorem_surface_status_cycle01_v0.json"
)

EXPECTED_ARTIFACT_ID = "stat_failure_trigger_discharge_object_surface_status_cycle01_v0"
EXPECTED_THEOREM_STATUS_ARTIFACT_ID = "stat_failure_trigger_discharge_theorem_surface_status_cycle01_v0"
EXPECTED_COUPLING_GATE = "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_STATUS_VALUE = "OBJECT_SURFACE_SCAFFOLD_PINNED_NONCLAIM"
ALLOWED_COHERENCE_STATUS_VALUES = {"NOT_PRESENT_v0", "COHERENCE_SCAFFOLD_PINNED_NONCLAIM"}
EXPECTED_ARTIFACT_REL = "formal/output/stat_failure_trigger_discharge_object_surface_status_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_failure_trigger_discharge_object_surface_status_cycle01_gate.py"
...
def test_stat_failure_trigger_discharge_object_surface_status_cycle01_gate() -> None:
    stat_text = _read(STAT_DOC_PATH)
    state_text = _read(STATE_PATH)
    roadmap_text = _read(ROADMAP_PATH)
    matrix = _read_json(MATRIX_PATH)
    artifact_json = _read_json(ARTIFACT_PATH)
    theorem_status_artifact_json = _read_json(THEOREM_STATUS_ARTIFACT_PATH)

    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT failure-trigger discharge object-surface status gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for failure-trigger discharge object-surface status gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT failure-trigger discharge object-surface status cycle-01 payload_sha256 does not match canonical payload hash."
    )

    assert theorem_status_artifact_json.get("artifact_id") == EXPECTED_THEOREM_STATUS_ARTIFACT_ID

    for token_name, expected in (
        ("STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
        ("STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_v0", EXPECTED_STATUS_VALUE),
    ):
    ...
    for doc_text, doc_label in (
        ...
        coherence_status = _extract_token(doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_v0")
        assert coherence_status in ALLOWED_COHERENCE_STATUS_VALUES, (
            f"{doc_label} must keep the failure-trigger coherence-status token either pre-admission or coherence-status pinned."
        )

    stat_sha = _extract_token(stat_text, "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_CYCLE01_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_CYCLE01_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_CYCLE01_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0") == "PRESENT", (
            f"{doc_label} must preserve the readiness packet before failure-trigger object-surface status admission."
        )
        assert _extract_token(
            ...
            f"{doc_label} must preserve the gate entry-status token before failure-trigger object-surface status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_v0"
        ) == "ENTRY_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger discharge surface-status token before object-surface status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0"
        ) == "THEOREM_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger theorem-surface status token before object-surface status admission."
        )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the failure-trigger discharge object-surface status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the failure-trigger discharge object-surface status gate path."
        )

    for token_name, expected in (
        (
            "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_CYCLE01_ARTIFACT_v0",
            EXPECTED_THEOREM_STATUS_ARTIFACT_ID,
        ),
        (
    ...
    assert payload.get("checkpoint") == "stat_failure_trigger_discharge_object_surface_status_cycle01"
    assert payload.get("status") == "object_surface_status_non_promotional"
    assert payload.get("object_surface_entry_scope") == [
        "failure_trigger_discharge_object_surface_status_placeholder_only",
        "theorem_surface_status_verified_before_object_surface_status",
        "coherence_scope_boundary_pinned_before_object_surface_status",
        "no_failure_trigger_object_discharge_claim",
        "no_external_truth_claim",
    ]
    assert payload.get("required_object_surface_status_inputs") == [
        ...
        "failure_trigger_discharge_coherence_scope_boundary_pinned",
    ...
    assert payload.get("emitted_status_tokens") == [
        ...
        "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_v0: NOT_PRESENT_v0",
    ]
    ...
    assert "- dependency ladder back to the theorem-surface status artifact is pinned before object-surface status admission." in stat_text
