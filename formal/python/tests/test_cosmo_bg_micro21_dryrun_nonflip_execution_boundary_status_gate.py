...
COSMO_MICRO21_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_21_DRYRUN_NONFLIP_EXECUTION_BOUNDARY_STATUS_v0.md"
COSMO_MICRO21_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro21_dryrun_nonflip_execution_boundary_status_cycle01_v0.json"
...
def test_cosmo_micro21_doc_contains_required_headers_and_tokens() -> None:
    ...
    assert not missing_headers, "COSMO micro-21 document is missing required header(s): " + ", ".join(missing_headers)
    ...
    assert not missing_tokens, "COSMO micro-21 document is missing required token(s): " + ", ".join(missing_tokens)


def test_cosmo_micro21_artifact_schema_and_scope_boundary() -> None:
    payload = _read_json(COSMO_MICRO21_ARTIFACT_PATH)

    assert payload.get("artifact_id") == "cosmo_bg_micro21_dryrun_nonflip_execution_boundary_status_cycle01_v0"
    ...
    assert boundary_statement == "DRYRUN_NONFLIP_LANE_ONLY_NO_ADJUDICATION_FLIP_NO_COMPARATOR_AUTHORIZATION"


def test_cosmo_micro21_forbidden_tokens_not_present() -> None:
    doc_text = _read(COSMO_MICRO21_PATH)
    artifact_text = _read(COSMO_MICRO21_ARTIFACT_PATH)
    forbidden_tokens = [
...
def test_cosmo_micro21_cross_surface_pointers_are_complete() -> None:
    matrix = _read_json(MATRIX_PATH)
    cosmo = matrix.get("pillars", {}).get("PILLAR-COSMO")
    assert isinstance(cosmo, dict), "PILLAR-COSMO matrix row must exist."

    assert cosmo.get("dryrun_nonflip_execution_boundary_status_doc") == "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_21_DRYRUN_NONFLIP_EXECUTION_BOUNDARY_STATUS_v0.md"
    assert cosmo.get("dryrun_nonflip_execution_boundary_status_gate") == "formal/python/tests/test_cosmo_bg_micro21_dryrun_nonflip_execution_boundary_status_gate.py"
    assert cosmo.get("dryrun_nonflip_execution_boundary_status_policy") == "CYCLE08_09_10_11_12_13_14_15_16_17_18_19_20_DRYRUN_NONFLIP_EXECUTION_BOUNDARY_LOCK_REQUIRED_NO_STATUS_FLIP_NO_COMPARATOR_AUTHORIZATION"
    ...
    assert not missing_state, "State missing dryrun nonflip execution-boundary token(s): " + ", ".join(missing_state)
    ...
    assert not missing_target, "COSMO target missing dryrun nonflip execution-boundary token(s): " + ", ".join(missing_target)
    ...
    assert not missing_rollup, "Rollup gate missing dryrun nonflip execution-boundary cross-pin token(s): " + ", ".join(missing_rollup)