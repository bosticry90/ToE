...
from formal.python.tools.phi_bridge_theorem_linkage_obligation_closeout_result_review_report import (
    ...
    TARGET_CONCLUSION,
    build_phi_bridge_theorem_linkage_obligation_closeout_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_bridge_theorem_linkage_obligation_closeout_result_review_report.py"
)
...
def test_phi_bridge_closeout_result_review_files_exist() -> None:
    for path in [
        CLOSEOUT_PATH,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_phi_bridge_closeout_result_review_accepts_local_closeout() -> None:
...
def test_phi_bridge_closeout_result_review_preserves_route_and_boundary() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["linkage_route"] == LOCAL_CLOSEOUT_ROUTE
    assert review["phi_bridge_theorem_linkage_obligation_closeout_accepted"] is True
    assert review["standalone_componentwise_route_match_preserved"] is True
    ...
    assert review["C_bridge_phi_zero_locally_linked"] is True
    assert review["C_bridge_phi_zero_constructed"] is True
    assert review["C_bridge_phi_zero_derived"] is True
    assert review["C_bridge_phi_discharged"] is True
...
def test_phi_bridge_closeout_result_review_records_lean_status() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_bridge_closeout_result_review_rotates_to_selector() -> None:
    ...
    assert consumed["likely_next_obligation"] == LIKELY_NEXT_OBLIGATION
    assert consumed["C_bridge_phi_zero_locally_linked"] == "yes"
...
def test_phi_bridge_closeout_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "PhiBridgeTheoremLinkageObligationCloseoutResultReview",
        CONSUMED_TARGET,
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        SELECTOR_QUESTION,
        ...
        *LOCAL_CLOSEOUT_ROUTE,
        ...
        "PHI_BRIDGE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_RESULT_REVIEW_OUTCOME_v0",
...
def test_phi_bridge_closeout_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_bridge_theorem_linkage_obligation_closeout_result_review_gate.py"
    )
