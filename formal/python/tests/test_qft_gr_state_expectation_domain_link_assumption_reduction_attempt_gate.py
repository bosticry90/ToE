...
from formal.python.tools.qft_gr_state_expectation_domain_link_assumption_reduction_attempt_report import (
    ...
    build_qft_gr_state_expectation_domain_link_assumption_reduction_attempt,
)
...
LEAN_ATTEMPT_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_StateExpectationDomainLinkAssumptionReductionAttempt.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_state_expectation_domain_link_assumption_reduction_attempt_report.py"
)
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_consumes_review() -> None:
    ...
    assert (
        attempt[
            "consumes_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review"
        ]
        == REVIEW_ID
    )
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["state_expectation_domain_link_assumption_inconclusive"] is False


def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["state_expectation_domain_link_reduction_contract"]
    ...
    assert contract["contract_id"] == STATE_EXPECTATION_DOMAIN_LINK_CONTRACT_ID
    assert contract["assumption_id"] == SELECTED_ROW_ID
    assert contract["assumption_family"] == PRIMARY_ASSUMPTION_FAMILY
    assert contract["state_expectation_object"] == STATE_EXPECTATION_OBJECT
    assert contract["operator_domain_link_condition"] == OPERATOR_DOMAIN_LINK_CONDITION
    assert len(attempt["execution_findings"]) == 4


def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["state_expectation_domain_link_assumption_discharged"] is False
    assert (
        attempt["state_expectation_domain_link_claimed_as_operator_domain_closed"]
        is False
    )
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_state_expectation_domain_link_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_expectation_domain_link_assumption_reduction_attempt(
