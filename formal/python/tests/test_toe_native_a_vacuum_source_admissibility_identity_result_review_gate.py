...
from formal.python.tools.toe_native_a_vacuum_source_admissibility_identity_result_review_report import (
    ...
    build_toe_native_a_vacuum_source_admissibility_identity_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "toe_native_a_vacuum_source_admissibility_identity_result_review_report.py"
)
...
def test_a_vacuum_source_identity_result_review_files_exist() -> None:
    for path in [
        A_SOURCE_IDENTITY_PACKET_PATH,
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
        LEAN_VALIDATION_POLICY_PATH,
...
def test_a_vacuum_source_identity_result_review_packet_shape() -> None:
    prior = _json(A_SOURCE_IDENTITY_PACKET_PATH)
    packet = _json(DEFAULT_OUT)
    assert prior["outcome_id"] == A_SOURCE_IDENTITY_PACKET_OUTCOME
    ...
    assert build_toe_native_a_vacuum_source_admissibility_identity_result_review() == packet


def test_a_vacuum_source_identity_result_review_accepts_identity_only() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["retry_reason"] == RETRY_REASON
...
def test_a_vacuum_source_identity_result_review_rotates_to_retry() -> None:
    ...
    assert consumed["divergence_identity_accepted"] == "yes"
    assert consumed["on_shell_vanishing_route_accepted"] == "yes"
    ...
    assert active_row["divergence_identity_accepted"] == "yes"
    assert active_row["on_shell_vanishing_route_accepted"] == "yes"
...
def test_a_vacuum_source_identity_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        CONSUMED_TARGET,
        NEXT_TARGET,
        "ToeNativeAVacuumSourceAdmissibilityIdentityResultReview",
        "TOE_NATIVE_A_VACUUM_SOURCE_ADMISSIBILITY_IDENTITY_RESULT_REVIEW_OUTCOME_v0",
        "HISTORICAL_TOE_NATIVE_A_VACUUM_SOURCE_ADMISSIBILITY_IDENTITY_RESULT_REVIEW_CURRENT_LIVE_NEXT_TARGET_v0: "
...
def test_a_vacuum_source_identity_result_review_validation_policy_is_bounded() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_a_vacuum_source_identity_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_toe_native_a_vacuum_source_admissibility_identity_result_review_gate.py"
    )
