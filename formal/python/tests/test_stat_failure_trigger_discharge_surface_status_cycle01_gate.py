...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_failure_trigger_discharge_surface_status_cycle01_v0.json"

EXPECTED_ARTIFACT_ID = "stat_failure_trigger_discharge_surface_status_cycle01_v0"
...
ALLOWED_THEOREM_STATUS_VALUES = {"NOT_PRESENT_v0", "THEOREM_SURFACE_SCAFFOLD_PINNED_NONCLAIM"}
EXPECTED_ARTIFACT_REL = "formal/output/stat_failure_trigger_discharge_surface_status_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_failure_trigger_discharge_surface_status_cycle01_gate.py"
...
def test_stat_failure_trigger_discharge_surface_status_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT failure-trigger discharge surface-status gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for failure-trigger discharge surface-status gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT failure-trigger discharge surface-status cycle-01 payload_sha256 does not match canonical payload hash."
    )

    for token_name, expected in (
        ("STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
        ("STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_v0", EXPECTED_STATUS_VALUE),
    ):
    ...
    for doc_text, doc_label in (
        ...
        theorem_status = _extract_token(doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0")
        assert theorem_status in ALLOWED_THEOREM_STATUS_VALUES, (
            f"{doc_label} must keep the failure-trigger theorem-surface status either pre-admission or theorem-surface pinned."
        )

    stat_sha = _extract_token(stat_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_CYCLE01_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_CYCLE01_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_CYCLE01_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0") == "PRESENT", (
            f"{doc_label} must preserve the readiness packet before failure-trigger surface-status admission."
        )
        assert _extract_token(
            ...
            f"{doc_label} must preserve the gate entry-status token before failure-trigger surface-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_COHERENCE_STATUS_v0"
        ) == "COHERENCE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the derivation-completeness coherence-status token before failure-trigger surface-status admission."
        )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the failure-trigger discharge surface-status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the failure-trigger discharge surface-status gate path."
        )

    for token_name, expected in (
        (
            "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_SCOPE_BOUNDARY_CYCLE01_ARTIFACT_v0",
    ...
    assert payload.get("checkpoint") == "stat_failure_trigger_discharge_surface_status_cycle01"
    assert payload.get("status") == "surface_status_non_promotional"
    assert payload.get("discharge_surface_entry_scope") == [
        "failure_trigger_discharge_surface_status_placeholder_only",
        "derivation_completeness_coherence_status_verified_before_surface_status",
        "theorem_and_object_surface_scope_boundaries_pinned_before_surface_status",
    ...
    assert payload.get("required_surface_status_inputs") == [
        ...
        "derivation_completeness_discharge_coherence_status_pinned",
        "failure_trigger_discharge_surface_scope_boundary_pinned",
    ...
    assert payload.get("emitted_status_tokens") == [
        ...
        "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0: NOT_PRESENT_v0",
    ]
    ...
    assert "- bounded failure-trigger discharge surface entry scope only; no theorem-surface execution claim and no external truth claim." in stat_text
    assert "- failure-trigger discharge surface-status packet remains non-promotional and does not authorize `TOE-STAT-DER-*` label promotion." in stat_text
