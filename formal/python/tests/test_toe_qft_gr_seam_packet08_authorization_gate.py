...
def test_qft_gr_seam_packet08_authorization_document_structure() -> None:
    ...
    for marker in required_markers:
        assert marker in text, f"Packet08 authorization doc missing marker: {marker}"


def test_qft_gr_seam_packet08_authorization_checkpoint_schema_and_preconditions() -> None:
    artifact = _read_json(AUTH_CHECKPOINT_PATH)
    assessment_artifact = _read_json(ASSESSMENT_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet08_authorization_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1E_QFT_GR_SEAM_PACKET08_AUTHORIZATION"
    assert artifact.get("status") == "PACKET08_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET08_AUTHORIZATION_v0.md"
    assert payload.get("parent_assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET07_ASSESSMENT_v0.md"
    ...
    assessment_decision = assessment_artifact.get("payload", {}).get("packet08_decision", {})
    ...
    pre = payload.get("preconditions_from_packet07_assessment", {})
    ...
    assert pre.get("scalar_drift_trigger_status") == "NOT_TRIGGERED_v0"
    ...
    assert decision.get("decision_outcome") == "AUTHORIZE_PACKET08_BOUNDED_TARGET_v0"
    assert decision.get("status") == "AUTHORIZED_WITH_SINGLE_BOUNDED_TARGET_v0"

    bounded = payload.get("packet08_bounded_target", {})
...
def test_qft_gr_seam_packet08_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet08 authorization pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet08 authorization pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_auth = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET08_AUTHORIZATION_STATUS_v0")
    roadmap_auth = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET08_AUTHORIZATION_STATUS_v0")
