...
DOC = ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET27_ASSESSMENT_v0.md"
CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet27_assessment_checkpoint_v0.json"
)
PARENT_EXEC_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET27_BOUNDED_EXECUTION_v0.md"
)
PARENT_EXEC_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet27_bounded_execution_checkpoint_v0.json"
)
PARENT_AUTH_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET27_AUTHORIZATION_v0.md"
)
PARENT_AUTH_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet27_authorization_checkpoint_v0.json"
)
...
def test_packet27_assessment_files_exist() -> None:
    assert DOC.exists(), f"Missing packet27 assessment doc: {DOC}"
    assert CHECKPOINT.exists(), f"Missing packet27 assessment checkpoint: {CHECKPOINT}"


def test_packet27_assessment_doc_required_markers() -> None:
    text = _read_text(DOC)
    required_markers = [
        "# TOE QFT-GR Seam Packet27 Assessment v0",
        "## Assessment Questions",
        "## Objective Progress Snapshot",
        "## Scalar Freeze Compliance",
        "## Decision Rule Projection (for packet28 authorization)",
        "TOE_QFT_GR_SEAM_PACKET27_ASSESSMENT_STATUS_v0: ASSESSED_TARGET_SATISFACTION_VERIFIED_v0",
        "PACKET27_ASSESSMENT_COMPLETE_v0",
...
def test_packet27_assessment_checkpoint_schema_and_lineage() -> None:
    data = _load_json(CHECKPOINT)
    assert data["artifact_id"] == "toe_qft_gr_seam_packet27_assessment_checkpoint_v0"
    assert data["phase"] == "PHASE_2B_QFT_GR_SEAM_PACKET27_ASSESSMENT"
    assert data["scope"] == "packet27_target_satisfaction_assessment"
    assert data["status"] == "PACKET27_ASSESSMENT_COMPLETE_v0"

    payload = data["payload"]
    assert payload["assessment_doc_path"].endswith(
        "TOE_QFT_GR_SEAM_PACKET27_ASSESSMENT_v0.md"
    )
...
def test_packet27_assessment_target_satisfaction_and_packet28_rule() -> None:
    ...
    packet28 = payload["packet28_decision"]
    assert packet28["authorized"] is True
    assert packet28["progress_verdict"] == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet28["ladder_extension_verdict"] == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet28["verdict"] == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet28["exact_bounded_target"] == EXPECTED_PACKET28_TARGET
    assert packet28["hold_refine_trigger_status"] == "NOT_HIT_v0"


def test_packet27_assessment_scalar_and_guardrail_invariants() -> None:
    payload = _load_json(CHECKPOINT)["payload"]
    ...
    assert guardrails["assessment_scope"] == "ASSESS_PACKET27_BEFORE_PACKET28_v0"


def test_packet27_assessment_parent_chain_exists() -> None:
