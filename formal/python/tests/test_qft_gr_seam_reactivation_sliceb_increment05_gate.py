...
INCREMENT05_PACKET_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_EXECUTION_PACKET_v0.md"
)
INCREMENT05_ASSESS_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_ASSESSMENT_NOTE_v0.md"
)
DECISION_PATH = (
    REPO_ROOT
    / "formal"
...
def test_sliceb_increment05_artifacts_exist() -> None:
    for p in (INCREMENT05_PACKET_PATH, INCREMENT05_ASSESS_PATH):
        assert p.exists(), f"Missing Slice B Increment05 artifact: {p}"


def test_sliceb_increment05_anchor_and_delta_parity() -> None:
    objective = _read(OBJECTIVE_PATH)
    parent = _read(PARENT_PACKET_PATH)
    decision = _read(DECISION_PATH)
    packet = _read(INCREMENT05_PACKET_PATH)
    assess = _read(INCREMENT05_ASSESS_PATH)
    ...
    assert "formal/docs/release/QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_SEMANTIC_DELTA_DECISION_NOTE_v0.md" in packet
    assert "mixed-origin" in packet
    assert "explicitly invalid" in packet


def test_sliceb_increment05_invariance_and_nonclaim_markers() -> None:
    packet = _read(INCREMENT05_PACKET_PATH)
    assess = _read(INCREMENT05_ASSESS_PATH)
...
def test_sliceb_increment05_status_tokens_present() -> None:
    packet = _read(INCREMENT05_PACKET_PATH)
    assess = _read(INCREMENT05_ASSESS_PATH)

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_STATUS_v0: EXECUTED_BOUNDED_v0" in packet
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_ASSESSMENT_STATUS_v0: ASSESSED_BOUNDED_v0" in assess
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_OBJECTIVE_ADVANCEMENT_v0: YES" in assess
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT05_NEGATIVE_PATH_EXCLUSION_AND_CIRCULARITY_v0: ENFORCED" in assess
