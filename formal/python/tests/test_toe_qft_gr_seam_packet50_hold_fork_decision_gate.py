...
DECISION_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_v0.md"
DECISION_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet50_hold_fork_decision_checkpoint_v0.json"
...
TARGETED_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET50_TARGETED_JUSTIFICATION_REVIEW_v0.md"
TARGETED_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet50_targeted_justification_review_checkpoint_v0.json"
...
def test_qft_gr_seam_packet50_hold_fork_decision_document_structure() -> None:
    ...
    required_markers = [
        ...
        "decision_outcome: HOLD_PACKET50_AUTHORIZATION_v0",
        ...
        "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_GATE_v0: REQUIRED_PACKET50_HOLD_FORK_DECISION_SCHEMA_AND_DISPOSITION_ALIGNMENT",
        "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_ARTIFACT_v0: toe_qft_gr_seam_packet50_hold_fork_decision_checkpoint_v0",
    ]
    for marker in required_markers:
        assert marker in text, f"Packet49 hold-fork decision doc missing marker: {marker}"


def test_qft_gr_seam_packet50_hold_fork_decision_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet50_hold_fork_decision_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2W_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION"
    assert artifact.get("status") == "PACKET50_HOLD_FORK_DECISION_COMPLETE_HOLD_v0"

    payload = artifact.get("payload", {})
    assert payload.get("decision_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_v0.md"
    ...
    assert payload.get("parent_targeted_justification_review_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET50_TARGETED_JUSTIFICATION_REVIEW_v0.md"
    ...
    assert output.get("decision_outcome") == "HOLD_PACKET50_AUTHORIZATION_v0"
    assert output.get("packet50_authorization_freeze_status") == "ENFORCED_v0"

    assert eligibility_artifact.get("status") == "PACKET50_ELIGIBILITY_REVIEW_COMPLETE_HOLD_v0"
    assert targeted_artifact.get("status") == "PACKET50_TARGETED_JUSTIFICATION_REVIEW_COMPLETE_INSUFFICIENT_v0"
    assert convergence_artifact.get("status") == "SEAM_CONVERGENCE_TERMINATION_CRITERION_ACTIVE_v0"


def test_qft_gr_seam_packet50_hold_fork_decision_authority_parity_and_freeze() -> None:
    ...
    for ref in refs:
        assert ref in state_text or ref in inventory_text, (
            f"Missing packet50 hold-fork decision pointer in compact-State or central inventory: {ref}"
        )
        assert ref in roadmap_text, f"Missing packet50 hold-fork decision pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_STATUS_v0")
    assert state_status == roadmap_status == "DECISION_COMPLETE_HOLD_v0"

    state_outcome = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_OUTCOME_v0"
    )
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET50_HOLD_FORK_DECISION_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_v0"

    assert not PACKET50_AUTH_DOC_PATH.exists(), "Packet49 authorization doc must not exist during hold-fork HOLD disposition"
    assert not PACKET50_AUTH_CHECKPOINT_PATH.exists(), "Packet49 authorization checkpoint must not exist during hold-fork HOLD disposition"
