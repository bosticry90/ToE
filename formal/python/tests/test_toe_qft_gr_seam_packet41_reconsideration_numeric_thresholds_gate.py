...
THRESHOLD_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_v0.md"
THRESHOLD_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet41_reconsideration_numeric_thresholds_checkpoint_v0.json"
...
SCORECARD_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet41_reconsideration_scorecard_worksheet_checkpoint_v0.json"
...
def test_packet41_reconsideration_numeric_thresholds_document_structure() -> None:
    ...
    required_markers = [
        ...
        "NUMERIC_THRESHOLD_PACKET41_RELEASE_REQUIRES_ALL_NUMERIC_AND_EXISTING_BINDINGS_v0",
...
def test_packet41_reconsideration_numeric_thresholds_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet41_reconsideration_numeric_thresholds_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2Y_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS"
    assert artifact.get("status") == "PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_ACTIVE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("threshold_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_v0.md"
    ...
    assert payload.get("parent_measurement_protocol_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET41_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_v0.md"
    assert payload.get("parent_measurement_protocol_checkpoint_path") == "formal/output/toe_qft_gr_seam_packet41_numeric_threshold_measurement_protocol_checkpoint_v0.json"
    assert payload.get("parent_scorecard_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_SCORECARD_WORKSHEET_v0.md"
    assert payload.get("parent_scorecard_checkpoint_path") == "formal/output/toe_qft_gr_seam_packet41_reconsideration_scorecard_worksheet_checkpoint_v0.json"
    ...
    assert thresholds.get("packet41_reconsideration_release_gate", {}).get("all_numeric_thresholds_required") is True
    assert thresholds.get("packet41_reconsideration_release_gate", {}).get("existing_review_layers_required") is True
    ...
    assert protocol.get("status") == "PACKET41_NUMERIC_THRESHOLD_MEASUREMENT_PROTOCOL_ACTIVE_v0"
    assert scorecard.get("status") == "PACKET41_RECONSIDERATION_SCORECARD_WORKSHEET_ACTIVE_v0"


def test_packet41_reconsideration_numeric_thresholds_authority_parity_and_freeze() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing numeric-threshold pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing numeric-threshold pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_status = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_STATUS_v0")
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_STATUS_v0")
    assert state_status == roadmap_status == "ACTIVE_HOLD_GATED_NUMERIC_CRITERIA_v0"

    state_outcome = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_OUTCOME_v0")
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_RETAINED_UNTIL_NUMERIC_CLEARANCE_v0"

    assert not PACKET41_AUTH_DOC_PATH.exists(), "Packet41 authorization doc must not exist under numeric-threshold hold"
    assert not PACKET41_AUTH_CHECKPOINT_PATH.exists(), "Packet41 authorization checkpoint must not exist under numeric-threshold hold"