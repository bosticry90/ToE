...
def test_qft_gr_seam_packet29_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert (ref in state_text) or (ref in inventory_text) or (ref in roadmap_text), (
            f"Missing packet29 authorization pointer across authority surfaces: {ref}"
        )
    ...
    state_auth = _extract_token_from_surfaces(
        ...
        "TOE_QFT_GR_SEAM_PACKET29_AUTHORIZATION_STATUS_v0",
    )
    roadmap_auth = _extract_token_from_surfaces([roadmap_text], "TOE_QFT_GR_SEAM_PACKET29_AUTHORIZATION_STATUS_v0")
