...
def test_qft_cycle15_gate_is_pinned_in_authority_surfaces() -> None:
    ...
    assert CYCLE15_GATE_PATH in state_text, f"State authority surface must pin `{CYCLE15_GATE_PATH}`."
    assert CYCLE15_MILESTONE_TOKEN in state_text, "State authority surface missing cycle-15 milestone token."
    assert CYCLE15_PROGRESS_TOKEN in state_text, "State authority surface missing cycle-15 progress token."
