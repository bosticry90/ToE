...
def test_bridge_toyh_c6_orthogonality_report_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    report_rel = Path("formal/quarantine/bridge_tickets/BRIDGE_TOYH_C6_ORTHOGONALITY_REPORT.json")
    report_path = repo_root / report_rel
    assert report_path.exists(), "Expected Toy-H orthogonality report artifact to exist"

    payload = json.loads(report_path.read_text(encoding="utf-8"))
    assert payload.get("schema_version") == 1
    assert payload.get("report_id") == "BRIDGE_TOYH_C6_ORTHOGONALITY_REPORT"

    evidence = payload.get("evidence", {})
    nodes = list(evidence.get("pytest_nodes", []))
    assert nodes, "Missing pytest_nodes evidence in orthogonality report"
    for node in nodes:
