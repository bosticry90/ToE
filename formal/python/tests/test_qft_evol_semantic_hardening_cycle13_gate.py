...
def test_qft_cycle13_gate_is_pinned_in_authority_surfaces() -> None:
    ...
    assert CYCLE13_GATE_PATH in state_text, f"State authority surface must pin `{CYCLE13_GATE_PATH}`."
    assert CYCLE13_MILESTONE_TOKEN in state_text, "State authority surface missing cycle-13 milestone token."
    assert CYCLE13_PROGRESS_TOKEN in state_text, "State authority surface missing cycle-13 progress token."
