...
CYCLE13_GATE_PATH = "formal/python/tests/test_qft_evol_semantic_hardening_cycle13_gate.py"
...
def test_qft_cycle13_hardening_artifacts_exist() -> None:
...
def test_qft_cycle13_tokens_are_pinned_in_qft_docs() -> None:
    evol_text = _read(QFT_EVOL_TARGET_PATH)
    discharge_text = _read(QFT_FULL_DISCHARGE_TARGET_PATH)

    for token in [CYCLE13_MILESTONE_TOKEN, CYCLE13_GATE_PATH]:
        assert token in evol_text, f"QFT evolution umbrella target missing cycle-13 token `{token}`."
        assert token in discharge_text, f"QFT full-derivation discharge target missing cycle-13 token `{token}`."

    assert CYCLE13_PROGRESS_TOKEN in discharge_text, "QFT full-derivation discharge target missing cycle-13 progress token."


def test_qft_cycle13_gate_is_pinned_in_authority_surfaces() -> None:
    roadmap_text = _read(ROADMAP_PATH)
    state_text = _read(STATE_PATH)
    inventory_text = _read(INVENTORY_PATH)

    assert CYCLE13_GATE_PATH in roadmap_text, f"Roadmap authority surface must pin `{CYCLE13_GATE_PATH}`."
    assert CYCLE13_GATE_PATH in state_text or CYCLE13_GATE_PATH in inventory_text, (
        f"State/Inventory authority surface must pin `{CYCLE13_GATE_PATH}`."
    )
    assert CYCLE13_MILESTONE_TOKEN in state_text or CYCLE13_MILESTONE_TOKEN in inventory_text, (
        "State/Inventory authority surface missing cycle-13 milestone token."
    )
    assert CYCLE13_PROGRESS_TOKEN in state_text or CYCLE13_PROGRESS_TOKEN in inventory_text, (
        "State/Inventory authority surface missing cycle-13 progress token."
    )


def test_qft_cycle13_alignment_symmetry_witness_coherence_normalization_theorem_token_is_present() -> None:
    text = _read(QFT_EVOL_OBJECT_SCAFFOLD_LEAN_PATH)
    assert "theorem qft_evol_generator_unitarity_route_alignment_symmetry_witness_coherence_normalization_v0" in text, (
        "QFT evolution Lean scaffold missing cycle-13 alignment-symmetry witness coherence-normalization theorem token."
    )


def test_qft_cycle13_alignment_symmetry_witness_coherence_normalization_reuses_cycle12_via_symmetry() -> None:
    text = _read(QFT_EVOL_OBJECT_SCAFFOLD_LEAN_PATH)
    ...
    for token in [
        ...
        assert token in theorem_block, f"Cycle-13 theorem missing expected token `{token}`."

    assert "trivial" not in theorem_block, (
        "Cycle-13 theorem must not be implemented as a trivial placeholder proof."
    )
