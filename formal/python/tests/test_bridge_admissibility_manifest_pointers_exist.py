...
def test_bridge_admissibility_manifest_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    manifest_rel = Path("formal/quarantine/bridge_tickets/BRIDGE_ADMISSIBILITY_MANIFEST.json")
    manifest_path = repo_root / manifest_rel
    assert manifest_path.exists(), "Expected bridge admissibility manifest artifact to exist"
    ...
    for _, rel in artifacts.items():
        assert (repo_root / str(rel)).exists(), f"Missing artifact path: {rel}"

    required = payload.get("required_evidence", {})
    nodes = list(required.get("pytest_nodes", []))
    assert nodes, "Missing required_evidence.pytest_nodes"
    for node in nodes:
