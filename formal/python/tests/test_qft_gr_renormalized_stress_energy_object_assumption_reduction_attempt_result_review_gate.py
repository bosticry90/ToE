...
from formal.python.tools.qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_consumes_attempt() -> None:
...
def test_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_accepts_row001_only() -> None:
    review = _json(DEFAULT_OUT)
    contract = review["renormalized_stress_energy_object_reduction_contract"]
    ...
    assert review["accepted_renormalization_assumption_rows"] == [SELECTED_ROW_ID]
    assert review["accepted_contract_id"] == RENORMALIZED_STRESS_ENERGY_OBJECT_CONTRACT_ID
    ...
    assert review["renormalized_stress_energy_object_assumption_reduction_accepted"] is True
    assert review["renormalized_stress_energy_object_assumption_reduction_rejected"] is False


def test_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert (
        review[
    ...
    assert review["renormalized_stress_energy_object_assumption_discharged"] is False
    assert review["renormalized_stress_energy_object_assumption_discharged_by_review"] is False
    assert (
        review["renormalized_stress_energy_object_final_definition_claimed_by_review"]
        is False
    )
    assert review["renormalized_stress_energy_object_defined_as_final_by_review"] is False
    assert (
        review[
            "renormalized_stress_energy_object_final_definition_or_discharge_claimed_by_review"
        ]
        is False
    )
    assert review["renormalized_stress_energy_object_final_definition_claimed"] is False
    assert review["renormalized_stress_energy_object_defined_as_final"] is False
...
def test_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_selects_scope_packet() -> None:
    review = _json(DEFAULT_OUT)
    assert review["next_renormalization_assumption_row"] == NEXT_ROW_ID
    assert review["next_renormalization_assumption_row_object"] == RENORMALIZATION_SCOPE
    assert (
    ...
    assert {row["target"]: row["status"] for row in review["candidate_next_targets"]} == {
        ...
        "define_final_qft_gr_renormalized_stress_energy_object": "not_authorized",
        "discharge_qft_gr_renormalized_stress_energy_object_assumption": "not_authorized",
        ...
        "authorize_qft_gr_public_submission": "not_authorized",
    }


def test_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review(
