...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_CovariantConservationStatementObstructionRefinementPacket.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_covariant_conservation_statement_obstruction_refinement_packet_report.py"
)
...
def test_qft_gr_covariant_conservation_statement_obstruction_refinement_packet_files_exist() -> None:
...
def test_qft_gr_covariant_conservation_statement_obstruction_refinement_packet_consumes_review() -> None:
...
def test_qft_gr_covariant_conservation_statement_obstruction_refinement_packet_rows_and_primary() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_qft_gr_covariant_conservation_statement_obstruction_refinement_packet_preserves_nonclaim_boundaries() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["prepares_refinement_only"] is True
    assert packet["identifies_covariant_conservation_obstruction_more_narrowly"] is True
...
def test_qft_gr_covariant_conservation_statement_obstruction_refinement_packet_selects_one_next_target() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        ...
        "prepare_qft_gr_covariant_conservation_statement_form_selection_packet": "deferred",
        "close_qft_gr_seam": "not_authorized",
        "authorize_public_submission": "not_authorized",
    }


def test_qft_gr_covariant_conservation_statement_obstruction_refinement_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_covariant_conservation_statement_obstruction_refinement_packet(
