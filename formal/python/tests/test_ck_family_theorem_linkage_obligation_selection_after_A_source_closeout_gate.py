...
from formal.python.tools.ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_report import (
    ...
    build_ck_family_theorem_linkage_obligation_selection_after_A_source_closeout,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_report.py"
)
...
def test_ck_family_selection_after_A_source_closeout_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_ck_family_selection_after_A_source_closeout_selects_C_source_phi() -> None:
...
def test_ck_family_selection_after_A_source_closeout_preserves_phi_registry_boundary() -> None:
    selection = _json(DEFAULT_OUT)

    assert selection["plain_meaning"] == PLAIN_MEANING
    assert selection["route_boundary"] == ROUTE_BOUNDARY
    assert selection["phi_source_registry_boundary"] == PHI_SOURCE_REGISTRY_BOUNDARY
    assert (
        selection["prior_phi_source_constraint_form"]
        == PHI_SOURCE_REGISTRY_CONSTRAINT_FORM
    )
    assert (
        selection["prior_phi_source_constraint_equation"]
        == PHI_SOURCE_REGISTRY_CONSTRAINT_EQUATION
    )
    assert selection["forbidden_imported_routes"] == FORBIDDEN_IMPORTED_ROUTES
    ...
    for key in [
        ...
        "later_A_source_route_imported",
        "psi_A_sourced_route_imported",
        "psi_A_sourced_Maxwell_substitution",
...
def test_ck_family_selection_after_A_source_closeout_records_lean_status() -> None:
    selection = _json(DEFAULT_OUT)
...
def test_ck_family_selection_after_A_source_closeout_rotates_to_review() -> None:
    ...
    assert selector["psi_A_sourced_route_imported"] == "no"
    assert selector["QFT_GR_source_route_imported"] == "no"
    ...
    if is_current:
        ...
        assert active["psi_A_sourced_route_imported"] == "no"
...
def test_ck_family_selection_after_A_source_closeout_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_SELECTION_RESULT,
        PACKET_CLASSIFICATION,
        "CKFamilyTheoremLinkageObligationSelectionAfterASourceCloseout",
        CONSUMED_TARGET,
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        FOLLOW_ON_TARGET_AFTER_REVIEW,
        ...
        ROUTE_BOUNDARY,
        LEAN_STATUS_WORDING_FOR_SELECTION,
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_A_SOURCE_CLOSEOUT_OUTCOME_v0",
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_A_SOURCE_CLOSEOUT_NONCLAIM_BOUNDARY_v0",
...
def test_ck_family_selection_after_A_source_closeout_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_ck_family_theorem_linkage_obligation_selection_after_A_source_closeout_gate.py"
    )
