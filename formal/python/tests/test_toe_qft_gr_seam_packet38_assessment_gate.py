...
def test_qft_gr_seam_packet38_assessment_authority_parity_and_invariance() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet38 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet38 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_assessment = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET38_ASSESSMENT_STATUS_v0")
    roadmap_assessment = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET38_ASSESSMENT_STATUS_v0")
    assert state_assessment == roadmap_assessment == "BOUNDED_TARGET_CONFIRMED_v0"

    state_readiness = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET39_AUTHORIZATION_READINESS_v0")
    roadmap_readiness = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET39_AUTHORIZATION_READINESS_v0")
