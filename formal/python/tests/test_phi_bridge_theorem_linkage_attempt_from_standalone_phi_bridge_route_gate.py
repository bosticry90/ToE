...
from formal.python.tools.phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route_report import (
    ...
    TARGET_CONCLUSION,
    WATCH_ITEMS,
    build_phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route_report.py"
)
...
def consumed_target() -> str:
    return "prepare_phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route"


def test_phi_bridge_standalone_attempt_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_phi_bridge_standalone_attempt_prepares_componentwise_zero_route() -> None:
...
def test_phi_bridge_standalone_attempt_preserves_bridge_tuple_and_components() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["exact_tuple_definition_preserved"] is True
    assert packet["target_C_bridge_phi_zero_preserved"] is True
    ...
    assert packet["field_equation_match"] == FIELD_EQUATION_MATCH
    assert packet["stress_energy_match"] == STRESS_ENERGY_MATCH
    assert packet["source_residual_match"] == SOURCE_RESIDUAL_MATCH
    assert packet["field_equation_zero_component"] == FIELD_EQUATION_ZERO_COMPONENT
    assert packet["stress_energy_zero_component"] == STRESS_ENERGY_ZERO_COMPONENT
    assert packet["source_residual_zero_component"] == SOURCE_RESIDUAL_ZERO_COMPONENT
    assert packet["bridge_route_field_equation_match"] == BRIDGE_ROUTE_FIELD_EQUATION_MATCH
    assert packet["bridge_route_stress_energy_match"] == BRIDGE_ROUTE_STRESS_ENERGY_MATCH
    assert packet["bridge_route_source_residual_match"] == (
        BRIDGE_ROUTE_SOURCE_RESIDUAL_MATCH
    )
    assert packet["componentwise_zero_route"] == COMPONENTWISE_ZERO_ROUTE
    assert packet["bridge_tuple_zero"] == BRIDGE_TUPLE_ZERO
    ...
    assert packet["route_kind"] == "standalone_phi_bridge_componentwise_zero_preparation"


def test_phi_bridge_standalone_attempt_blocks_execution_and_route_contamination() -> None:
    packet = _json(DEFAULT_OUT)
    route_text = " ".join(packet["componentwise_zero_route"])

    assert packet["master_witness_route_match_target_prepared"] is True
    assert packet["master_witness_route_match_promoted_to_master_action"] is False
    assert packet["same_standalone_phi_bridge_registry_tuple"] is True
    assert packet["same_E_phi_master_witness_component"] is True
    assert packet["same_T_phi_master_witness_component"] is True
    assert packet["same_C_source_phi_residual_component"] is True
    ...
    for flag in [
        ...
        "C_bridge_phi_linkage_constructed",
        "C_bridge_phi_zero_derived",
...
def test_phi_bridge_standalone_attempt_rotates_to_result_review() -> None:
    ...
    prior_review = _workstream(
        registry, "review_phi_bridge_theorem_linkage_obligation_packet_result"
    )
    ...
    assert consumed["C_bridge_phi_discharged"] == "no"
...
def test_phi_bridge_standalone_attempt_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_ATTEMPT_PREPARATION_RESULT,
        PACKET_CLASSIFICATION,
        "PhiBridgeTheoremLinkageAttemptFromStandalonePhiBridgeRoute",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        SUGGESTED_REVIEW_OUTCOME,
        ...
        TARGET_CONCLUSION,
        *LEAN_STATUS_WORDING_LINES_FOR_PACKET,
        "PHI_BRIDGE_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_BRIDGE_ROUTE_OUTCOME_v0",
...
def test_phi_bridge_standalone_attempt_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route_gate.py"
    )
