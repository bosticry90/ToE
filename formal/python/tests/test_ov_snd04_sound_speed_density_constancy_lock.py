...
def test_ov_snd04_lock_matches_computed_record() -> None:
    rec = ovsnd04_sound_speed_density_constancy_record(date="2026-01-24")
    ...
    lock_path = (
        ...
        / "locks"
        / "observables"
        / "OV-SND-04_sound_speed_density_constancy.md"
    )
    ...
    assert locked["observable_id"] == "OV-SND-04"
    assert locked["status"]["blocked"] is False
    assert locked["status"]["n_pairs"] >= 2
    assert locked["summary"].get("metrics") is not None
    assert isinstance(locked.get("condition_rows"), list)
    assert len(locked.get("condition_rows") or []) >= 2
