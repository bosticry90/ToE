...
from formal.python.tools.qft_gr_state_admissibility_boundary_assumption_reduction_attempt_report import (
    ...
    build_qft_gr_state_admissibility_boundary_assumption_reduction_attempt,
)
...
def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_consumes_review() -> None:
...
def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert (
        attempt["state_admissibility_boundary_assumption_obstruction_identified"]
        is False
    )
    assert attempt["state_admissibility_boundary_assumption_inconclusive"] is False


def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["state_admissibility_boundary_reduction_contract"]
    ...
    assert attempt["accepted_prior_state_domain_assumption_row"] == ACCEPTED_PRIOR_ROW_ID
    assert attempt["accepted_state_domain_assumption_rows"] == [ACCEPTED_PRIOR_ROW_ID]
    assert attempt["selected_state_domain_assumption_row"] == SELECTED_ROW_ID
    assert attempt["selected_row_count"] == 1
    assert contract["contract_id"] == STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_ID
    assert contract["assumption_id"] == SELECTED_ROW_ID
    assert contract["assumption_family"] == SELECTED_ASSUMPTION_FAMILY
    assert contract["state_admissibility_boundary"] == (
        STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
    assert contract["state_admissibility_boundary_condition"] == (
        STATE_ADMISSIBILITY_BOUNDARY_CONDITION
    )
...
def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["state_admissibility_boundary_assumption_reduced_by_attempt"] is True
    assert attempt["state_admissibility_boundary_assumption_discharged"] is False
    assert (
        attempt["state_admissibility_boundary_assumption_discharged_by_attempt"]
        is False
    )
    assert (
        attempt[
    ...
    assert attempt["state_admissibility_boundary_satisfied"] is False
    assert attempt["state_admissibility_boundary_discharged"] is False
...
def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_admissibility_boundary_assumption_reduction_attempt(
