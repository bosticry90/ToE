...
from formal.python.tools import post_plan_stat_packet05_lane_eligibility_review_report as tool
...
def _write_declaration(path: Path) -> None:
    _write_json(
        path,
        {
            ...
            "outcome_contract": {
                "single_terminal_outcome_rule": "EXACTLY_ONE_ALLOWED_POST_PLAN_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_OUTCOME",
                "no_loop_rule": "ONE_POST_PLAN_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_LAYER_ONLY",
                "allowed_outcomes": [
                    ...
                    "POST_PLAN_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_EVIDENCE_INCOMPLETE"
                ],
                "default_outcome": "POST_PLAN_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_EVIDENCE_INCOMPLETE"
            }
        },
    )
...
def test_packet05_lane_eligibility_review_confirms_stat_is_not_enabled_under_current_bootstrap(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(tool, "REPO_ROOT", tmp_path)
    declaration_path = tmp_path / "formal" / "docs" / "release" / "STAT_PACKET05_REVIEW.json"
    ...
    assert report["summary"]["eligible_for_packet05_bootstrap"] is False


def test_packet05_lane_eligibility_review_detects_visible_stat_packet05_bootstrap_path(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(tool, "REPO_ROOT", tmp_path)
    declaration_path = tmp_path / "formal" / "docs" / "release" / "STAT_PACKET05_REVIEW.json"
    ...
    assert report["summary"]["eligible_for_packet05_bootstrap"] is True


def test_live_packet05_lane_eligibility_review_is_mirrored_and_bound_into_the_dossier() -> None:
    ...
    required_refs = [
        "formal/docs/release/POST_PLAN_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_20260420_v0.json",
        "formal/output/reports/post_plan_stat_packet05_lane_eligibility_review_20260420_v0.json",
        "formal/python/tools/post_plan_stat_packet05_lane_eligibility_review_report.py",
        "formal/python/tests/test_post_plan_stat_packet05_lane_eligibility_review_report.py",
    ]
    ...
    assert (
        dossier["summary"]["additional_bound_surfaces"]["stat_packet05_lane_eligibility_review_report"]
        == "POST_PLAN_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_NOT_ELIGIBLE_UNDER_CURRENT_BOOTSTRAP"
    )