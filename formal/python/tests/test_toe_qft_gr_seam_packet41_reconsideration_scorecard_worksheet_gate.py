...
SCORECARD_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_SCORECARD_WORKSHEET_v0.md"
...
def test_packet41_reconsideration_scorecard_document_structure() -> None:
    ...
    required_markers = [
        "Worksheet ID:",
        "Parent packet41 numeric-threshold measurement protocol:",
        "## Canonical Inputs",
...
def test_packet41_reconsideration_scorecard_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet41_reconsideration_scorecard_worksheet_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2ZA_QFT_GR_SEAM_PACKET41_RECONSIDERATION_SCORECARD_WORKSHEET"
    assert artifact.get("status") == "PACKET41_RECONSIDERATION_SCORECARD_WORKSHEET_ACTIVE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("scorecard_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_SCORECARD_WORKSHEET_v0.md"
    ...
    assert payload.get("parent_numeric_thresholds_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET41_RECONSIDERATION_NUMERIC_THRESHOLDS_v0.md"
...
def test_packet41_reconsideration_scorecard_authority_parity_and_freeze() -> None:
    ...
    assert not PACKET41_AUTH_DOC_PATH.exists(), "Packet41 authorization doc must not exist under scorecard-governed hold"
    assert not PACKET41_AUTH_CHECKPOINT_PATH.exists(), "Packet41 authorization checkpoint must not exist under scorecard-governed hold"