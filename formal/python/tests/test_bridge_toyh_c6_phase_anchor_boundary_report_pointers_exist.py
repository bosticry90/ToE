...
def test_bridge_toyh_c6_phase_anchor_boundary_report_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    report_rel = Path("formal/quarantine/bridge_tickets/BRIDGE_TOYH_C6_phase_anchor_BOUNDARY_REPORT.json")
    report_path = repo_root / report_rel
    assert report_path.exists(), "Expected Toy-H C6 phase-anchor boundary report artifact to exist"
