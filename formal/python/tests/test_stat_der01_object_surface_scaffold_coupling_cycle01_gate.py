...
EXPECTED_ARTIFACT_ID = "stat_der01_entropy_balance_object_surface_scaffold_cycle01_v0"
EXPECTED_GATE = "ARTIFACT_HASH_ROW_LABEL_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_ROW_BINDING = "TOE_STAT_DER_01_T_PROVED_OBJECT_SURFACE_SCAFFOLD_PINNED_NONCLAIM"
EXPECTED_ROW_ID = "TOE-STAT-DER-01"
EXPECTED_ROW_LABEL = "T-PROVED"
EXPECTED_ARTIFACT_REL = "formal/output/stat_der01_entropy_balance_object_surface_scaffold_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_der01_object_surface_scaffold_coupling_cycle01_gate.py"
...
def test_stat_der01_object_surface_scaffold_coupling_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT DER01 object-surface scaffold coupling gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), "PILLAR-STAT matrix row must exist for DER01 object-surface scaffold gate."
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT DER01 object-surface scaffold payload_sha256 does not match canonical payload hash."
    )

    for token_name, expected in (
        ("STAT_DER01_OBJECT_SURFACE_SCAFFOLD_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_DER01_OBJECT_SURFACE_SCAFFOLD_CYCLE01_GATE_v0", EXPECTED_GATE),
        ("STAT_DER01_OBJECT_SURFACE_ROW_BINDING_v0", EXPECTED_ROW_BINDING),
    ):
    ...
    stat_sha = _extract_token(stat_text, "STAT_DER01_OBJECT_SURFACE_SCAFFOLD_CYCLE01_ARTIFACT_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_DER01_OBJECT_SURFACE_SCAFFOLD_CYCLE01_ARTIFACT_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_DER01_OBJECT_SURFACE_SCAFFOLD_CYCLE01_ARTIFACT_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert EXPECTED_ARTIFACT_REL in doc_text, f"{doc_label} must pin DER01 object-surface scaffold artifact path."
        assert EXPECTED_GATE_REL in doc_text, f"{doc_label} must pin DER01 object-surface scaffold gate path."
    ...
    assert payload.get("status") == "object_surface_scaffold_placeholder_nonclaim"
    ...
    assert payload.get("object_surface_scope") == [
        "classical_entropy_balance_object_surface_scaffold_only",
        "entropy_density_or_control_volume_quantity_slot_placeholder",
        "flux_and_source_term_slots_placeholder",
        "no_constitutive_closure_claim",
        "no_theorem_body_discharge_claim",
    ]
    assert payload.get("required_object_surface_components") == [
        "entropy_quantity_symbol_surface",
        "entropy_flux_placeholder_surface",
        "entropy_production_source_placeholder_surface",
        "state_variable_pointer_surface",
