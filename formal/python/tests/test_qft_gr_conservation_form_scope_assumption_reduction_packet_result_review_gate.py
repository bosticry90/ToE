...
from formal.python.tools.qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_report.py"
)
...
def test_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_files_exist() -> None:
...
def test_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review["consumes_qft_gr_conservation_form_scope_assumption_reduction_packet"]
        == PACKET_ID
    )
...
def test_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_confirms_row005() -> None:
    review = _json(DEFAULT_OUT)
    selected = review["conservation_form_scope_assumption"]
    ...
    assert review["prior_rows001_002_003_004_remain_accepted"] is True
    ...
    assert review["conservation_form_options"] == CONSERVATION_FORM_OPTIONS
    assert review["selected_bounded_conservation_form"] == (
        SELECTED_BOUNDED_CONSERVATION_FORM
    )
    assert review["required_future_proof_object"] == REQUIRED_FUTURE_PROOF_OBJECT
    assert review["conservation_form_scope_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert all(
        ...
        for token in review["conservation_form_scope_status_tokens"]
    )


def test_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["packet_preparation_only_confirmed"] is True
    assert review["conservation_form_scope_packet_accepted_by_review"] is True
    assert review["conservation_form_scope_analysis_accepted"] is True
    assert review["conservation_form_scope_assumption_reduced_by_review"] is False
...
def test_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review(
