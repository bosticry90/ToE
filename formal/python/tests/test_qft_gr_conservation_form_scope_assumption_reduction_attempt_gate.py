...
from formal.python.tools.qft_gr_conservation_form_scope_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_conservation_form_scope_assumption_reduction_attempt,
)
...
LEAN_ATTEMPT_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_ConservationFormScopeAssumptionReductionAttempt.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_conservation_form_scope_assumption_reduction_attempt_report.py"
)
...
def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_consumes_review() -> None:
    ...
    assert (
        attempt[
            "consumes_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review"
        ]
        == REVIEW_ID
    )
...
def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["conservation_form_scope_assumption_reduced_pending_result_review"] is True
    assert attempt["conservation_form_scope_assumption_obstruction_identified"] is False
    assert attempt["conservation_form_scope_assumption_inconclusive"] is False


def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["conservation_form_scope_reduction_contract"]
    ...
    assert contract["selected_bounded_conservation_form"] == (
        SELECTED_BOUNDED_CONSERVATION_FORM
    )
...
def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["conservation_form_scope_assumption_discharged"] is False
    assert attempt["conservation_form_scope_claimed_as_conservation_proof"] is False
...
def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_qft_gr_conservation_form_scope_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_conservation_form_scope_assumption_reduction_attempt(
