...
ASSESSMENT_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_v0.md"
ASSESSMENT_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet21_assessment_checkpoint_v0.json"
PACKET_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET21_BOUNDED_EXECUTION_v0.md"
PACKET_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet21_bounded_execution_checkpoint_v0.json"
...
def test_qft_gr_seam_packet21_assessment_document_structure() -> None:
    ...
    required_markers = [
        ...
        "did packet21 satisfy its exact bounded target?",
        "target_satisfaction_verdict: YES_EXACT_TARGET_SATISFIED_v0",
        "physics_tightening_beyond_packet20_verdict: YES_REAL_TIGHTENING_VERIFIED_v0",
        ...
        "packet22_progress_verdict: GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0",
        "packet22_ladder_extension_verdict: REJECT_IF_MOMENTUM_EXTENSION_v0",
        "packet22_authorization_verdict: JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0",
        "packet22_exact_bounded_target:",
        ...
        "TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_GATE_v0: REQUIRED_PACKET21_ASSESSMENT_SCHEMA_AND_AUTHORITY_PARITY",
        "TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_ARTIFACT_v0: toe_qft_gr_seam_packet21_assessment_checkpoint_v0",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet21 assessment doc missing marker: {marker}"


def test_qft_gr_seam_packet21_assessment_checkpoint_schema() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet21_assessment_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2B_QFT_GR_SEAM_PACKET21_ASSESSMENT"
    assert artifact.get("status") == "PACKET21_ASSESSMENT_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_v0.md"
    assert payload.get("parent_packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET21_BOUNDED_EXECUTION_v0.md"
    ...
    assert payload.get("parent_authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET21_AUTHORIZATION_v0.md"
    ...
    assert packet_artifact.get("status") == "PACKET21_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"
    assert auth_artifact.get("status") == "PACKET21_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"
    ...
    assert summary.get("physics_tightening_beyond_packet20_verdict") == "YES_REAL_TIGHTENING_VERIFIED_v0"
    assert summary.get("residual_ambiguity_reduction_verdict") == "YES_RESIDUAL_AMBIGUITY_REDUCED_v0"

    packet22 = payload.get("packet22_decision", {})
    assert packet22.get("authorized") is True
    assert packet22.get("progress_verdict") == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet22.get("ladder_extension_verdict") == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet22.get("verdict") == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet22.get("hold_refine_trigger_status") == "NOT_HIT_v0"
...
def test_qft_gr_seam_packet21_assessment_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet21 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet21 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_assessment = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_STATUS_v0")
    roadmap_assessment = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_STATUS_v0")
