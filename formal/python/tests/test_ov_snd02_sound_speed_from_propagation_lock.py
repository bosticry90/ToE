...
def test_ov_snd02_lock_matches_computed_record() -> None:
    rec = ovsnd02_sound_speed_from_propagation_record(date="2026-01-24")
    ...
    lock_path = (
        ...
        / "locks"
        / "observables"
        / "OV-SND-02_sound_speed_from_propagation.md"
    )
    ...
    assert locked["observable_id"] == "OV-SND-02"
    assert locked["results"]["primary"]["c_m_per_s"] > 0
    assert locked["results"]["primary"]["se_m_per_s"] >= 0
