...
LOCALIZATION_TOKEN = "EM_U1_DISTRIBUTIONAL_SEMANTICS_MAPPING_LOCALIZATION_GATE_v0: CYCLE22_ARTIFACTS_ONLY"
PREREQUISITE_TOKENS = [
...
CLASSIFICATION_PATTERNS = {
    "class_delta": re.escape(
        "SEMANTICS_CLASS_DELTA_SOURCE_v0: DIRAC_DELTA|DELTA_FUNCTION|DELTA_X|POINT_CHARGE|SINGULAR_SOURCE"
    ),
    "class_weak": re.escape(
        "SEMANTICS_CLASS_WEAK_LANGUAGE_v0: IN_SENSE_OF_DISTRIBUTIONS|WEAK_DERIVATIVE|WEAK_SOLUTION|TEMPERED_DISTRIBUTION"
    ),
    "class_pairing": re.escape(
        "SEMANTICS_CLASS_TEST_PAIRING_v0: TEST_FUNCTION|PAIRING_DUALITY_BRACKET|ACTION_ON_TEST_FUNCTIONS"
    ),
    "class_measure": re.escape(
        "SEMANTICS_CLASS_MEASURE_SIGNAL_v0: DISTRIBUTION_VALUED_SOURCE|MEASURE_VALUED_SOURCE|RADON_MEASURE_SIGNAL"
    ),
}
...
def test_em_cycle022_artifacts_exist() -> None:
    ...
    assert EM_MICRO22_PATH.exists(), "Missing EM U1 Cycle-022 document."
...
def test_em_micro22_contains_required_tokens_and_localized_classification_statements() -> None:
    ...
    assert not missing, "EM Cycle-022 micro document is missing required token(s): " + ", ".join(missing)

    missing_patterns = [label for label, pattern in CLASSIFICATION_PATTERNS.items() if re.search(pattern, text) is None]
    assert not missing_patterns, (
        "EM Cycle-022 micro document is missing required classification seam statement(s): "
...
def test_em_target_references_cycle022_artifact_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM target document is missing required Cycle-022 token(s): " + ", ".join(missing)
...
def test_state_mentions_cycle022_checkpoint_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "State_of_the_Theory.md is missing required Cycle-022 token(s): " + ", ".join(missing)


def test_assumption_registry_still_contains_distributional_lane_authorization_id() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "Assumption registry is missing Cycle-021 distributional lane assumption token(s): " + ", ".join(missing)
...
def test_em_lean_cycle022_tokens_and_mapping_harness_stubs_are_pinned() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM U1 Lean scaffold is missing required Cycle-022 token(s): " + ", ".join(missing)


def test_cycle022_classification_statements_are_localized_to_cycle22_artifact() -> None:
    allowed_paths = [EM_MICRO22_PATH]
    scoped_paths = [
        EM_TARGET_PATH,
        EM_MICRO01_PATH,
        EM_MICRO02_PATH,
    ...
    for path in allowed_paths:
        text = _read(path)
        hits = [label for label, pattern in CLASSIFICATION_PATTERNS.items() if re.search(pattern, text)]
        if not hits:
            violations.append(f"cycle022 artifact {path.relative_to(REPO_ROOT)} is missing localized classification statements")
            continue
        ...
        if not (
            ...
            violations.append(
                f"cycle022 artifact {path.relative_to(REPO_ROOT)} contains classification statements {hits} "
    ...
    for path in scoped_paths:
        ...
        for label, pattern in CLASSIFICATION_PATTERNS.items():
            if re.search(pattern, text):
                violations.append(
                    f"{label} classification statement leaked into non-authorized artifact "
                    f"{path.relative_to(REPO_ROOT)}"
                )

    assert not violations, "Cycle-022 classification localization violation:\n- " + "\n- ".join(violations)


def test_cycle022_is_mapping_only_and_blocks_promotion_or_new_physics_selection() -> None:
    text = _read(EM_MICRO22_PATH)
    forbidden_patterns = [
        r"\bwe derive\b",
        r"\bwe prove\b",
    ...
    assert not violations, (
        "Cycle-022 must remain mapping-only/non-promotional and non-selecting. "
