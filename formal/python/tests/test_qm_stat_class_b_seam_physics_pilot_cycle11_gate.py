...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle11_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "qm_stat_class_b_seam_physics_pilot_cycle10_v0.json"
...
def test_qm_stat_cycle11_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing QM-STAT Cycle11 target doc."
    assert ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle11 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing QM-STAT Cycle10 predecessor artifact."


def test_qm_stat_cycle11_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
        "DERIVATION_TARGET_QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE11_v0",
        "TARGET-QM-STAT-CLASS-B-SEAM-PHYSICS-PILOT-CYCLE11-v0",
        ...
        "QM_STAT_CYCLE11_INCOMPATIBILITY_EXCLUSION_v0: EIGHTEENTH_MOMENT_MISMATCH_FLAGGED_AS_NONCOMPATIBLE",
        "QM_STAT_CYCLE11_SCOPE_v0: FINITE_STATE_DISCRETE_HIGHER_MOMENT_AUDIT_ONLY_NONCLAIM",
        "QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE11_STATUS_v0: CRITERIA_AND_EIGHTEENTH_MOMENT_EXCLUSION_PINNED_NONCLAIM",
    ...
    assert not missing, "QM-STAT Cycle11 doc missing required token(s): " + ", ".join(missing)


def test_qm_stat_cycle11_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle11_v0"
    assert artifact["seam_id"] == "SEAM-QM-STAT"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_EIGHTEENTH_MOMENT_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "qm_stat_class_b_seam_physics_pilot_cycle10_v0"
    assert derived["artifact_path"] == "formal/output/qm_stat_class_b_seam_physics_pilot_cycle10_v0.json"


def test_qm_stat_cycle11_blocker_criteria_include_eighteenth_moment_parity() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "MASS_MEAN_VARIANCE_THIRD_FOURTH_SIXTH_EIGHTH_TENTH_TWELFTH_FOURTEENTH_SIXTEENTH_AND_EIGHTEENTH_MOMENT_PARITY_REQUIRED"
    ...
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8, qm_m10, qm_m12, qm_m14, qm_m16, qm_m18 = _moments18(xs, qm_p)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8, st_m10, st_m12, st_m14, st_m16, st_m18 = _moments18(xs, stat_p)
    ...
    assert qm_m18 == st_m18 == Fraction(criteria["eighteenth_central_moment"]["qm_m18"])
    assert st_m18 == Fraction(criteria["eighteenth_central_moment"]["stat_m18"])


def test_qm_stat_cycle11_eighteenth_moment_exclusion_is_explicit() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8, qm_m10, qm_m12, qm_m14, qm_m16, qm_m18 = _moments18(xs, qm_p)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8, st_m10, st_m12, st_m14, st_m16, st_m18 = _moments18(xs, st_p)
    ...
    assert qm_m18 == Fraction(exclusion["eighteenth_central_moment"]["qm_m18"])
    assert st_m18 == Fraction(exclusion["eighteenth_central_moment"]["stat_m18"])

    assert exclusion["classification"] == "NONCOMPATIBLE_EXCLUDED_v0"


def test_qm_stat_cycle11_nonclaim_boundary_and_adjudication() -> None:
    artifact = _json(ARTIFACT_PATH)
    ...
    assert adjudication["token"] == "QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE11_ADJUDICATION"
