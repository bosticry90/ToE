...
REQUIRED_DELTA = (
    ...
    "STABILITY_CURVATURE_FLUX_COHERENCE_DEPENDENCY"
)
...
DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT66_TO_67_SYNTHESIS_v0.md"
)


def _read() -> str:
    assert DOC_PATH.exists(), f"Missing Increment66-to-67 synthesis doc: {DOC_PATH}"
    return DOC_PATH.read_text(encoding="utf-8")


def test_increment66_to_67_synthesis_exists_and_anchors() -> None:
    text = _read()
    ...
    assert "formal/docs/paper/DERIVATION_TARGET_QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT66_v0.md" in text
    assert "stress_energy_to_weak_curvature_handoff_strengthening" in text


def test_increment66_to_67_synthesis_tokens_and_nonclaim_present() -> None:
    text = _read()

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT66_TO_67_SYNTHESIS_STATUS_v0: LOCKED_BOUNDED_REOPEN_READY_v0" in text
    assert f"QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT66_TO_67_DELTA_v0: {REQUIRED_DELTA}" in text
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT66_TO_67_ADJUDICATION: DISCHARGED_v0_BOUNDED" in text
...
def test_increment66_to_67_template_and_governance_pointers_present() -> None:
    text = _read()
