...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review_files_exist() -> None:
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review[
            "consumes_qft_gr_candidate_source_domain_membership_assumption_reduction_packet"
        ]
        == PACKET_ID
    )
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review_confirms_row002() -> None:
    review = _json(DEFAULT_OUT)
    selected = review["candidate_source_domain_membership_assumption"]
    ...
    assert review["prior_accepted_operator_domain_assumption_row"] == PRIOR_ACCEPTED_ROW
    assert (
        review["prior_accepted_selected_operator_action_contract"]
        == SELECTED_OPERATOR_ACTION_CONTRACT_ID
    )
    assert review["prior_row001_contract_remains_accepted"] is True
    ...
    assert review["candidate_source_object"] == CANDIDATE_SOURCE_OBJECT
    assert review["operator_domain_membership_condition"] == (
        OPERATOR_DOMAIN_MEMBERSHIP_CONDITION
    )
    assert review["candidate_source_domain_membership_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert all(
        ...
        for token in review["candidate_source_domain_membership_status_tokens"]
    )


def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["packet_preparation_only_confirmed"] is True
    assert review["candidate_source_domain_membership_packet_accepted_by_review"] is True
    assert review["candidate_source_domain_membership_analysis_accepted"] is True
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_result_review(
