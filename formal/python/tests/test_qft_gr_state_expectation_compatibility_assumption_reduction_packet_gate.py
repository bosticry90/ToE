...
from formal.python.tools.qft_gr_state_expectation_compatibility_assumption_reduction_packet_report import (
    ...
    build_qft_gr_state_expectation_compatibility_assumption_reduction_packet,
)
...
def test_qft_gr_state_expectation_compatibility_packet_files_exist() -> None:
...
def test_qft_gr_state_expectation_compatibility_packet_consumes_sd002_review() -> None:
...
def test_qft_gr_state_expectation_compatibility_packet_selects_only_row003() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_expectation_compatibility_assumption"]
    ...
    assert packet["accepted_prior_state_domain_assumption_rows"] == ACCEPTED_PRIOR_ROW_IDS
    assert packet["accepted_state_domain_assumption_rows"] == ACCEPTED_PRIOR_ROW_IDS
    assert packet["selected_state_domain_assumption_row"] == SELECTED_ROW_ID
    ...
    assert packet["state_expectation_compatibility_status_tokens"] == selected[
        "current_status"
    ]


def test_qft_gr_state_expectation_compatibility_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_expectation_compatibility_assumption"]
    assert packet["state_expectation_compatibility_condition"] == (
        STATE_EXPECTATION_COMPATIBILITY_CONDITION
    )
    assert selected["state_expectation_compatibility_condition"] == (
        STATE_EXPECTATION_COMPATIBILITY_CONDITION
    )
    assert packet["state_expectation_compatibility"] == (
        STATE_EXPECTATION_COMPATIBILITY_CONDITION
    )
    assert selected["state_expectation_compatibility"] == (
        STATE_EXPECTATION_COMPATIBILITY_CONDITION
    )
...
def test_qft_gr_state_expectation_compatibility_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["state_expectation_compatibility_claimed"] is False
    assert packet["state_expectation_compatibility_discharged"] is False
    ...
    assert packet["state_admissibility_claimed_as_source_admissibility"] is False
    assert packet["state_domain_assumptions_discharged"] is False
    assert (
        packet["state_domain_assumptions_reduced_or_discharged_by_preparation"]
        is False
    )
...
def test_qft_gr_state_expectation_compatibility_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        ...
        "execute_qft_gr_state_expectation_compatibility_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_state_expectation_compatibility_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_expectation_compatibility_assumption_reduction_packet(
