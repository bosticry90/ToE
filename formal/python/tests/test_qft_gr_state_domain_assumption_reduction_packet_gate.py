...
from formal.python.tools.qft_gr_state_domain_assumption_reduction_packet_report import (
    ...
    build_qft_gr_state_domain_assumption_reduction_packet,
)
...
def test_qft_gr_state_domain_assumption_reduction_packet_files_exist() -> None:
...
def test_qft_gr_state_domain_assumption_reduction_packet_consumes_renormalization_closeout_review() -> None:
    ...
    assert (
        packet[
            "consumes_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review"
        ]
        == RESULT_REVIEW_ID
    )
...
def test_qft_gr_state_domain_assumption_reduction_packet_selects_family_and_preserves_prior_family_closeouts() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["completed_prior_assumption_family_count"] == 2
    assert packet["prior_completed_operator_domain_assumption_row_count"] == 6
    assert packet["prior_completed_renormalization_assumption_row_count"] == 5
...
def test_qft_gr_state_domain_assumption_reduction_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["state_domain_object"] == STATE_DOMAIN_OBJECT
    assert packet["state_admissibility_boundary"] == STATE_ADMISSIBILITY_BOUNDARY
    assert packet["state_expectation_compatibility"] == STATE_EXPECTATION_COMPATIBILITY
    ...
    assert packet["candidate_reducible_assumption_count"] == 3
    assert packet["not_reducible_in_current_lane"]
    assert packet["not_reducible_in_current_lane_count"] == 8
    assert packet["state_domain_assumption_reduction_analysis_prepared"] is True
...
def test_qft_gr_state_domain_assumption_reduction_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["state_domain_assumptions_discharged"] is False
    assert packet["state_domain_assumptions_reduced_or_discharged_by_preparation"] is False
...
def test_qft_gr_state_domain_assumption_reduction_packet_selects_one_review_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_state_domain_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_state_domain_assumption_reduction_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_domain_assumption_reduction_packet(
