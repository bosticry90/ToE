...
from formal.python.tools.qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_DistributionalPairingRegularDomainAssumptionReductionAttemptResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_result_review_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_distributional_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_distributional_attempt_result_review_consumes_attempt() -> None:
...
def test_qft_gr_distributional_attempt_result_review_accepts_mr003() -> None:
    review = _json(DEFAULT_OUT)
    contract = review["distributional_pairing_regular_domain_reduction_contract"]
    ...
    assert review["accepted_contract_id"] == DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN_CONTRACT_ID
    assert (
        review["bounded_distributional_pairing_regular_domain_contract_status"]
        == BOUNDED_DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN_CONTRACT_STATUS
    )
    assert contract["contract_id"] == DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN_CONTRACT_ID
    assert contract["assumption_id"] == "MR-ASSUMP-003-distributional_pairing_regular_domain"
    assert (
        contract["regularity_condition"]
    ...
    assert review["distributional_pairing_regular_domain_assumption_reduction_accepted"] is True
    assert review["distributional_pairing_regular_domain_assumption_reduction_rejected"] is False
    assert (
        review[
...
def test_qft_gr_distributional_attempt_result_review_selects_mr004_from_inventory() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["limit_interchange_regularization_boundary"] == (
        LIMIT_INTERCHANGE_REGULARIZATION_BOUNDARY
    )
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        ...
        "discharge_qft_gr_distributional_pairing_regular_domain_assumption": "not_authorized",
...
def test_qft_gr_distributional_attempt_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["distributional_pairing_regular_domain_assumption_discharged"] is False
    assert (
        review["distributional_pairing_regular_domain_assumption_discharged_by_review"]
        is False
    )
    assert review["distributional_pairing_regular_domain_globally_solved"] is False
    assert review["distributional_pairing_regular_domain_globally_solved_by_review"] is False
...
def test_qft_gr_distributional_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt_result_review(
