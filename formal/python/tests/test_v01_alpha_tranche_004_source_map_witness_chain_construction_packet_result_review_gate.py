...
from formal.python.tools.v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_report import (
    BLOCKER_REASON,
    CURRENT_BLOCKER,
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01Tranche004SourceMapWitnessChainConstructionPacketResultReview.lean"
)
...
FORBIDDEN_TRUE_KEYS = [
    ...
    "source_map_witness_chain_construction_executed_by_review",
    "evidence_construction_executed_by_review",
    "remediation_executed_by_review",
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_files_exist() -> None:
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_construction_packet"] == (
        "V01_ALPHA_TRANCHE_004_SOURCE_MAP_WITNESS_CHAIN_CONSTRUCTION_PACKET_v0"
    )
    assert review["consumes_construction_packet_pointer"] == (
        "formal/docs/release/"
        "V01_ALPHA_TRANCHE_004_SOURCE_MAP_WITNESS_CHAIN_CONSTRUCTION_PACKET_20260515_v0.json"
    )


def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_scope() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["review_scope"] == (
        "REVIEW_TRANCHE_004_SOURCE_MAP_WITNESS_CHAIN_CONSTRUCTION_PACKET_RESULT_"
        "ONLY_NO_CONSTRUCTION_EXECUTION_CLOSURE_BLOCKER_MOVEMENT_OR_RELEASE_PROMOTION"
    )
    ...
    assert review["tranche_004_status"] == (
        "construction_route_accepted_source_map_closure_still_unauthorized_pending_"
        "bounded_construction_attempt"
    )
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_preserves_blocker_and_lean_posture() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_accepts_route_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["review_classification"] == RESULT_REVIEW_CLASSIFICATION
    assert review["construction_route_accepted"] is True
    assert review["construction_packet_scope"]["scope_kind"] == (
    ...
    assert len(review["candidate_construction_route"]) == 5
    assert len(review["required_lean_theory_surfaces"]) >= 6
    assert len(review["required_documentation_surfaces"]) >= 5
    assert len(review["success_criteria"]) >= 6
    assert len(review["failure_criteria"]) >= 6


def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_selects_bounded_attempt_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
    basis = review["route_review_basis"]
    assert basis["route_sufficient_for_bounded_attempt"] is True
    assert basis["attempt_execution_authorized_as_next_target"] is True
    assert basis["attempt_result_review_required_before_status_change"] is True
    assert basis["source_map_closure_authorized_by_review"] is False
    assert basis["blocker_movement_authorized_by_review"] is False
    assert review["source_map_witness_chain_construction_attempt_authorized"] is True
    assert review["source_map_witness_chain_construction_attempt_executed_by_review"] is False
    assert review["source_map_witness_chain_construction_executed_by_review"] is False
    assert review["route_decision"]["selected_branch"] == (
        "bounded_source_map_witness_chain_construction_attempt"
    )
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_keeps_blockers_tracked() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_no_execution_closure_seam_or_movement() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["remediation_executed_by_review"] is False
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == (
        "bounded_tranche_004_source_map_witness_chain_construction_attempt_execution_only"
    )
    assert review["selection_count"] == 1
    assert review["next_action_scope"] == (
        "EXECUTE_TRANCHE_004_SOURCE_MAP_WITNESS_CHAIN_CONSTRUCTION_ATTEMPT_ONLY_"
        "NO_SOURCE_MAP_CLOSURE_BLOCKER_MOVEMENT_RELEASE_PROMOTION_OR_READINESS_MARKING"
    )
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "execute_v01_alpha_tranche_004_source_map_witness_chain_construction_attempt": "selected",
        "prepare_v01_alpha_tranche_004_retained_source_map_blocker_declaration": "deferred",
    }


def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        REVIEW_ID,
        "formal/docs/release/V01_ALPHA_TRANCHE_004_SOURCE_MAP_WITNESS_CHAIN_CONSTRUCTION_PACKET_RESULT_REVIEW_20260515_v0.json",
        "formal/python/tools/v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_report.py",
        "formal/python/tests/test_v01_alpha_tranche_004_source_map_witness_chain_construction_packet_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01Tranche004SourceMapWitnessChainConstructionPacketResultReview" in index_text
    assert (
        "v01_tranche_004_source_map_witness_chain_construction_packet_result_review_does_not_claim_closure"
        in index_text
    )
