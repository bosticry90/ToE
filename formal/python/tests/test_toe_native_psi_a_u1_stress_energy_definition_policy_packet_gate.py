...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "toe_native_psi_a_u1_stress_energy_definition_policy_packet_report.py"
)
...
def test_psi_a_u1_stress_energy_definition_policy_packet_files_exist() -> None:
    for path in [
        STRESS_ENERGY_OBLIGATION_PACKET_PATH,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_psi_a_u1_stress_energy_definition_policy_packet_builds() -> None:
    consumed = _json(STRESS_ENERGY_OBLIGATION_PACKET_PATH)
    packet = _json(DEFAULT_OUT)
    assert consumed["outcome_id"] == STRESS_ENERGY_OBLIGATION_OUTCOME
    ...
    assert packet["consumed_stress_energy_and_exchange_obligation_packet_result"] == (
        STRESS_ENERGY_OBLIGATION_OUTCOME
    )
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == NEXT_TARGET_KIND
    assert build_toe_native_psi_a_u1_stress_energy_definition_policy_packet() == packet


def test_psi_a_u1_stress_energy_definition_policy_packet_indexes_definitions() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert packet["exchange_targets_preserved"] is True
    assert packet["stress_energy_definition_policy_count"] == 3
    assert packet["stress_energy_policy_ids"] == ["P1", "P2", "P3"]
    for row in packet["stress_energy_definition_policies"]:
        assert row["status"] == "selected_policy_not_derived"
    for key in [
...
def test_psi_a_u1_stress_energy_definition_policy_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    for phrase in [
        "stress-energy definition policy packet only",
        "bounded symmetric Dirac matter stress-energy candidate policy",
...
def test_psi_a_u1_stress_energy_definition_policy_packet_rotates_to_result_review() -> None:
    ...
    assert consumed["stress_energy_definition_policy_packet_result"] == OUTCOME_ID
    assert consumed["gauge_stress_energy_policy"] == GAUGE_STRESS_ENERGY_POLICY
    assert consumed["matter_stress_energy_policy"] == MATTER_STRESS_ENERGY_POLICY
    assert consumed["total_stress_energy_policy"] == TOTAL_STRESS_ENERGY_POLICY
    assert consumed["stress_energy_metric_variation_derived"] == "no"
    assert consumed["stress_energy_tetrad_variation_derived"] == "no"
    ...
    if is_current:
        ...
        assert active_row["consumed_stress_energy_definition_policy_packet_result"] == (
            OUTCOME_ID
        )
        assert active_row["packet_result"] == "PENDING"
        assert active_row["stress_energy_definition_policy_packet_result"] == OUTCOME_ID
        assert active_row[
        ...
        assert active_row["matter_stress_energy_policy"] == MATTER_STRESS_ENERGY_POLICY
        assert active_row["total_stress_energy_policy"] == TOTAL_STRESS_ENERGY_POLICY
...
def test_psi_a_u1_stress_energy_definition_policy_packet_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        OUTCOME_ID,
        PACKET_CLASSIFICATION,
        "ToeNativePsiAU1StressEnergyDefinitionPolicyPacket",
        NEXT_TARGET,
        "CURRENT_LIVE_NEXT_TARGET_v0: review_toe_native_psi_A_u1_stress_energy_definition_policy_packet_result",
        "PREVIOUS_LIVE_NEXT_TARGET_v0: prepare_toe_native_psi_A_u1_stress_energy_definition_policy_packet",
        GAUGE_STRESS_ENERGY_POLICY,
        GAUGE_STRESS_ENERGY_LOWER_INDEX_POLICY,
        MATTER_STRESS_ENERGY_POLICY,
        TOTAL_STRESS_ENERGY_POLICY,
...
def test_psi_a_u1_stress_energy_definition_policy_packet_validation_policy_is_bounded() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_psi_a_u1_stress_energy_definition_policy_packet_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_toe_native_psi_a_u1_stress_energy_definition_policy_packet_gate.py"
    )
