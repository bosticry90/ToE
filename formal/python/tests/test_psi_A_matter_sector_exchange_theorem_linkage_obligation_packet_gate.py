...
from formal.python.tools.psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_report import (
    ...
    WATCH_ITEMS,
    build_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_report.py"
)
...
def test_psi_A_matter_sector_exchange_packet_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_psi_A_matter_sector_exchange_packet_scopes_dirac_target() -> None:
    ...
    assert build_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet() == packet


def test_psi_A_matter_sector_exchange_packet_preserves_boundary() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_psi_A_matter_sector_exchange_packet_records_lean_status() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_psi_A_matter_sector_exchange_packet_rotates_to_result_review() -> None:
...
def test_psi_A_matter_sector_exchange_packet_mirrors() -> None:
    ...
    for token in [
        ...
        STRICT_PACKET_RESULT,
        PACKET_CLASSIFICATION,
        "PsiAMatterSectorExchangeTheoremLinkageObligationPacket",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        LIKELY_FOLLOW_ON_TARGET_AFTER_REVIEW,
        ...
        WATCH_ITEMS[-1],
        LEAN_STATUS_WORDING_FOR_PACKET,
        "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_OUTCOME_v0",
        "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_NONCLAIM_BOUNDARY_v0",
        "no proof execution",
...
def test_psi_A_matter_sector_exchange_packet_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_gate.py"
    )
