...
REPORT_PATH = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_WEAK_CURVATURE_SOURCE_IDENTIFICATION_OBLIGATION_SEMANTICS_BOUNDED_ATTACK_20260503_v0.json"
)
SOURCE_REPORT_PATH = (
    REPO_ROOT
    / "formal"
...
SURFACE_ID = "QFT_GR_WEAK_CURVATURE_SOURCE_IDENTIFICATION_OBLIGATION_SEMANTICS_v0"
PREVIOUS_TARGET = (
    "prepare_qft_gr_weak_curvature_source_identification_obligation_semantics_bounded_attack"
)
SOURCE_REVIEW_TARGET = "review_qft_gr_einstein_coupling_obligation_semantics_result"
...
REPORT_ID = (
    "QFT_GR_WEAK_CURVATURE_SOURCE_IDENTIFICATION_OBLIGATION_SEMANTICS_BOUNDED_ATTACK_20260503_v0"
)
RETAINED_BLOCKER = (
...
def test_weak_curvature_source_identification_surface_records_supplied_obligation() -> None:
    ...
    for token in {
        ...
        "QFTGRWeakCurvatureSourceIdentificationObligationSemanticData",
...
def test_weak_curvature_source_identification_surface_preserves_boundaries() -> None:
    text = _read(SURFACE_PATH)
...
def test_weak_curvature_source_identification_report_records_nonclaim_result() -> None:
    ...
    assert report["supplied_route"]["distinction"] == (
        "has_weak_curvature_source_identification_obligation_does_not_imply_has_source_identification_witness_or_satisfaction"
    )
    assert report["refutation"]["status"] == (
        "einstein_coupling_obligation_only_weak_curvature_source_identification_witness_derivation_refuted"
    )
    assert not any(report["nonclaim_boundaries"].values())


def test_registry_rotates_to_weak_curvature_source_identification_result_review() -> None:
    assert_current_target_consistent()
    assert_frontier_matches_registry()
    assert_forbidden_promotions_closed()
    assert_public_surfaces_match_registry()
    ...
    assert prep["weak_curvature_source_identification_obligation_semantics_surface"] == (
        SURFACE_EVIDENCE
    )
    assert prep["weak_curvature_source_identification_obligation_semantics_report"] == (
        REPORT_EVIDENCE
    )
    assert prep["weak_curvature_source_identification_obligation_semantics_result_token"] == (
        RESULT_TOKEN
    )
    assert prep["supplied_route_available"] == "yes"
    assert prep["einstein_obligation_only_source_identification_witness_refuted"] == (
        "yes"
    )
    assert prep["source_identification_witness_derived_from_einstein_obligation_alone"] == (
        "no"
    )
    assert prep["result_review_status"] == "prepared_for_live_result_review"
    assert prep["weak_curvature_source_identification_witness_authorized"] == "no"
    assert prep["actual_weak_curvature_source_identification_authorized"] == "no"
    ...
    assert active["weak_curvature_source_identification_obligation_semantics_surface"] == (
        SURFACE_EVIDENCE
    )
    assert active["weak_curvature_source_identification_obligation_semantics_result_token"] == (
        RESULT_TOKEN
    )
    assert active["weak_curvature_source_identification_witness_authorized"] == "no"
    assert active["actual_weak_curvature_source_identification_authorized"] == "no"
    ...
    assert qft_gr["weak_curvature_source_identification_obligation_semantics_surface"] == (
        SURFACE_EVIDENCE
    )
    assert qft_gr["weak_curvature_source_identification_obligation_semantics_result_token"] == (
        RESULT_TOKEN
    )
    assert qft_gr["weak_curvature_source_identification_einstein_only_refuted"] == (
        "yes"
    )
    assert qft_gr["source_identification_witness_derived_from_einstein_obligation_alone"] == (
        "no"
    )
...
def test_public_surfaces_track_weak_curvature_source_identification_obligation() -> None:
    for path in [README_PATH, STATE_PATH, ROADMAP_PATH, STRICT_MAP_PATH]:
        ...
        assert "QFT_GR_WeakCurvatureSourceIdentificationObligationSemantics.lean" in text
    ...
    assert (
        "INV-MATH-QFTGR-WEAK-CURVATURE-SOURCE-IDENTIFICATION-OBLIGATION-v0"
        in inventory_text
    )
    ...
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_weak_curvature_source_identification_obligation_semantics_gate.py"
    )
