...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_audit_result_review_report.py"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_consumes_audit() -> None:
    ...
    assert review["consumes_audit"] == "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_AUDIT_v0"
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_selected_dependency() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_preserves_audit_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_preserves_exact_evidence() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_carries_tranche_004() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_classifies_conservatively() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["tranche_006_audit_result_classification"] == TRANCHE_CLASSIFICATION
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_keeps_blockers_tracked() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert rows[0]["retained_blocker_reason"] == TRANCHE_004_RETAINED_REASON
    assert rows[1]["status_carry_forward"] == (
        "release_blocking_pending_tranche_006_release_policy_adjudication_packet_preparation"
    )
    for row in rows:
        assert row["modified_by_tranche_006_audit_result_review"] is False
    ...
    assert other[0]["modified_by_tranche_006_audit_result_review"] is False


def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "prepare_v01_alpha_dependency_remediation_tranche_006_release_policy_adjudication_packet": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_audit_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_audit_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006AuditResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_audit_result_review_does_not_move_blocker"
        in index_text
    )
