...
LOCALIZATION_TOKEN = "EM_U1_CONTINUITY_LOCALIZATION_GATE_v0: CYCLE15_ARTIFACTS_ONLY"
PREREQUISITE_TOKENS = [
...
CONTINUITY_SURFACE_PATTERNS = {
    "tensor_continuity_surface": re.escape("∂_μ J^μ = 0"),
    "split_continuity_surface": re.escape("∂_t ρ + ∇·j = 0"),
}
...
def test_em_micro15_contains_required_tokens_and_localized_continuity_statements() -> None:
    text = _read(EM_MICRO15_PATH)
    required_tokens = [
    ...
    assert not missing, "EM Cycle-015 micro document is missing required token(s): " + ", ".join(missing)

    missing_patterns = [
        label for label, pattern in CONTINUITY_SURFACE_PATTERNS.items() if re.search(pattern, text) is None
    ]
    assert not missing_patterns, (
        "EM Cycle-015 micro document is missing continuity statement seam(s): " + ", ".join(missing_patterns)
    )


def test_em_target_references_cycle015_artifact_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM target document is missing required Cycle-015 token(s): " + ", ".join(missing)


def test_em_roadmap_references_cycle015_target_and_artifact_with_single_row() -> None:
    ...
    assert "TARGET-EM-U1-MICRO-15-CONTINUITY-SURFACE-COMPATIBILITY-SEAM-v0" in targets
    assert (
        "formal/docs/paper/DERIVATION_TARGET_EM_U1_MICRO_15_CONTINUITY_SURFACE_COMPATIBILITY_SEAM_v0.md"
        in artifacts
    )
...
def test_state_mentions_cycle015_checkpoint_and_tokens() -> None:
    text = _read(STATE_PATH)
    required_tokens = [
        "EM Cycle-015 continuity-surface compatibility seam checkpoint",
        "TARGET-EM-U1-MICRO-15-CONTINUITY-SURFACE-COMPATIBILITY-SEAM-v0",
        "EM_U1_PROGRESS_CYCLE15_v0: CONTINUITY_SURFACE_COMPATIBILITY_SEAM_TOKEN_PINNED",
        "EM_U1_CONTINUITY_TENSOR_SURFACE_v0: DIVERGENCE_CURRENT_STATEMENT_PINNED",
        "EM_U1_CONTINUITY_SPLIT_SURFACE_v0: DT_RHO_PLUS_DIVJ_STATEMENT_PINNED",
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "State_of_the_Theory.md is missing required Cycle-015 token(s): " + ", ".join(missing)


def test_em_lean_cycle015_tokens_and_harness_stubs_are_pinned() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM U1 Lean scaffold is missing required Cycle-015 token(s): " + ", ".join(missing)


def test_cycle015_continuity_statements_are_localized_to_cycle15_artifact() -> None:
    allowed_paths = [EM_MICRO15_PATH]
    scoped_paths = [
        EM_TARGET_PATH,
        EM_MICRO01_PATH,
        EM_MICRO02_PATH,
    ...
    for path in allowed_paths:
        text = _read(path)
        hits = [label for label, pattern in CONTINUITY_SURFACE_PATTERNS.items() if re.search(pattern, text)]
        if not hits:
            violations.append(f"cycle015 artifact {path.relative_to(REPO_ROOT)} is missing localized continuity statements")
            continue
        ...
        if not (has_assumption_id and has_localization_gate and has_prereq and has_non_selection):
            violations.append(
                f"cycle015 artifact {path.relative_to(REPO_ROOT)} contains continuity statements {hits} "
                "without source assumption ID + localization token + prerequisites + non-selection tokens."
            )

    for path in scoped_paths:
        text = _read(path)
        for label, pattern in CONTINUITY_SURFACE_PATTERNS.items():
            ...
            if not (has_assumption_id and has_localization_gate and has_prereq and has_non_selection):
                violations.append(
                    f"{label} continuity statement leaked into non-authorized artifact {path.relative_to(REPO_ROOT)} "
                    "without source assumption ID + localization token + prerequisites + non-selection tokens."
                )

    assert not violations, "Cycle-015 continuity-surface localization violation:\n- " + "\n- ".join(violations)


def test_cycle015_is_statement_only_and_blocks_derivation_or_new_physics_selection_language() -> None:
    text = _read(EM_MICRO15_PATH)
    forbidden_patterns = [
        r"\bwe derive\b",
        r"\bwe prove\b",
    ...
    assert not violations, (
        "Cycle-015 must remain statement-only/non-derivational with no import-lane selections. "
