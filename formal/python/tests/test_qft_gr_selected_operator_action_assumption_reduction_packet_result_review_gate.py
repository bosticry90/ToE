...
from formal.python.tools.qft_gr_selected_operator_action_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_selected_operator_action_assumption_reduction_packet_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_SelectedOperatorActionAssumptionReductionPacketResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_selected_operator_action_assumption_reduction_packet_result_review_report.py"
)
...
def test_qft_gr_selected_operator_action_assumption_reduction_packet_result_review_files_exist() -> None:
...
def test_qft_gr_selected_operator_action_assumption_reduction_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review["consumes_qft_gr_selected_operator_action_assumption_reduction_packet"]
        == PACKET_ID
    )
...
def test_qft_gr_selected_operator_action_assumption_reduction_packet_result_review_confirms_selected_row() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert (
        review["selected_operator_domain_assumption_row"]
        == SELECTED_OPERATOR_DOMAIN_ASSUMPTION_ROW
    )
    assert review["selected_operator_action_assumption_status_tokens"] == [
        "required",
        "supplied",
        "missing",
        "candidate_reducible",
    ]
    assert review["selected_operator_action_assumption"]["assumption_id"] == (
        SELECTED_OPERATOR_DOMAIN_ASSUMPTION_ROW
    )
    assert review["selected_operator_action_analysis_accepted"] is True
    assert review["packet_preparation_only_confirmed"] is True
    assert all(
        token in ROW_STATUS_ENUM
        for token in review["selected_operator_action_assumption_status_tokens"]
    )


def test_qft_gr_selected_operator_action_assumption_reduction_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_selected_operator_action_assumption_reduction_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_selected_operator_action_assumption_reduction_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_selected_operator_action_assumption_reduction_packet_result_review(
