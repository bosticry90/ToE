...
SURFACE_ID = "qft_gr_bianchi_compatibility_obligation_semantics_result_review_v0"
SOURCE_SURFACE_ID = "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_v0"
PREVIOUS_TARGET = "review_qft_gr_bianchi_compatibility_obligation_semantics_result"
SELECTED_NEXT_TARGET = (
    "prepare_qft_gr_einstein_coupling_obligation_semantics_bounded_attack"
)
SOURCE_RESULT_TOKEN = "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_SUPPLIED_ONLY"
REVIEW_RESULT_TOKEN = (
    "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_RESULT_REVIEW_CONSUMED_SUPPLIED_ONLY"
)
REPORT_ID = (
    "QFT_GR_BIANCHI_COMPATIBILITY_OBLIGATION_SEMANTICS_RESULT_REVIEW_20260503_v0"
)
RETAINED_BLOCKER = (
    "PHASE1-BLOCKER-QFTGR-EINSTEIN-COUPLING-OBLIGATION-SEMANTICS-RETAINED"
)
SOURCE_RETAINED_BLOCKER = "PHASE1-BLOCKER-QFTGR-BIANCHI-COMPATIBILITY-WITNESS-RETAINED"
ACTIVE_LANE = "qft_gr_einstein_coupling_obligation_semantics_preparation"
REVIEW_LANE = "qft_gr_bianchi_compatibility_obligation_semantics_result_review"
...
def test_bianchi_compatibility_obligation_result_review_surface_consumes_result() -> None:
    ...
    for token in {
        ...
        "QFTGRBianchiCompatibilityObligationResultReviewDecision",
        "acceptSuppliedOnlyAndPrepareEinsteinCouplingObligationSemantics",
        "qft_gr_bianchi_compatibility_obligation_result_review_consumes_live_target_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_completed_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_accepts_supplied_only_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_covariant_conservation_only_refuted_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_retained_as_supplied_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_token_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_selected_decision_v0",
        "qft_gr_bianchi_compatibility_obligation_result_review_selected_next_target_v0",
    }:
        assert token in text


def test_bianchi_compatibility_obligation_result_review_preserves_boundaries() -> None:
    text = _read(SURFACE_PATH)

    for theorem in {
        "qft_gr_bianchi_compatibility_obligation_result_review_same_lane_not_authorized_v0",
...
def test_bianchi_compatibility_obligation_result_review_report_records_selection() -> None:
    ...
    assert report["review_result"] == (
        "supplied_only_bianchi_compatibility_obligation_result_consumed"
    )
...
def test_registry_rotates_to_einstein_coupling_obligation_preparation() -> None:
    assert_current_target_consistent()
    assert_frontier_matches_registry()
    assert_forbidden_promotions_closed()
    assert_public_surfaces_match_registry()
    ...
    assert review["bianchi_compatibility_witness_authorized"] == "no"
    assert review["actual_bianchi_compatibility_authorized"] == "no"
    assert review["conservation_witness_authorized"] == "no"
    assert review["actual_covariant_conservation_authorized"] == "no"
    assert review["einstein_equation_coupling_authorized"] == "no"
    ...
    assert active["bianchi_compatibility_obligation_semantics_result_review_token"] == (
        REVIEW_RESULT_TOKEN
    )
    assert active["preparation_scope"] == (
        "einstein_coupling_obligation_semantics_surface_only"
    )
    assert active["einstein_coupling_obligation_semantics_authorized"] == (
        "preparation_only"
    )
    ...
    assert qft_gr["bianchi_compatibility_obligation_semantics_result_review_status"] == (
        "completed"
    )
    assert qft_gr["bianchi_compatibility_obligation_semantics_result_review_evidence"] == (
        SURFACE_EVIDENCE
    )
    assert qft_gr["einstein_coupling_obligation_semantics_preparation_target"] == (
        SELECTED_NEXT_TARGET
    )
    assert qft_gr["einstein_coupling_obligation_semantics_authorized"] == (
        "preparation_only"
    )
...
def test_public_surfaces_track_bianchi_result_review() -> None:
    for path in [README_PATH, STATE_PATH, ROADMAP_PATH, STRICT_MAP_PATH]:
        ...
        assert "QFT_GR_BianchiCompatibilityObligationSemanticsResultReview.lean" in text
    ...
    assert (
        "INV-MATH-QFTGR-BIANCHI-COMPATIBILITY-OBLIGATION-RESULT-REVIEW-v0"
        in inventory_text
    )
    ...
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_bianchi_compatibility_obligation_semantics_result_review_gate.py"
    )
