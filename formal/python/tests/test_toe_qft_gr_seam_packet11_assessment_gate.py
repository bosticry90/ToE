...
def test_qft_gr_seam_packet11_assessment_document_structure() -> None:
    ...
    required_markers = [
        ...
        "did packet11 satisfy its exact bounded target?",
        "target_satisfaction_verdict: YES_EXACT_TARGET_SATISFIED_v0",
        "packet12_authorization_verdict: JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0",
        "packet12_exact_bounded_target:",
        "TOE_QFT_GR_SEAM_PACKET11_ASSESSMENT_STATUS_v0: ASSESSED_TARGET_SATISFACTION_VERIFIED_v0",
        "TOE_QFT_GR_SEAM_PACKET11_ASSESSMENT_GATE_v0: REQUIRED_PACKET11_ASSESSMENT_SCHEMA_AND_AUTHORITY_PARITY",
        "TOE_QFT_GR_SEAM_PACKET11_ASSESSMENT_ARTIFACT_v0: toe_qft_gr_seam_packet11_assessment_checkpoint_v0",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet11 assessment doc missing marker: {marker}"


def test_qft_gr_seam_packet11_assessment_checkpoint_schema() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet11_assessment_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1P_QFT_GR_SEAM_PACKET11_ASSESSMENT"
    assert artifact.get("status") == "PACKET11_ASSESSMENT_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET11_ASSESSMENT_v0.md"
    assert payload.get("parent_packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET11_BOUNDED_EXECUTION_v0.md"
    ...
    assert packet_artifact.get("status") == "PACKET11_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"
    assert auth_artifact.get("status") == "PACKET11_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"
    ...
    packet12 = payload.get("packet12_decision", {})
    assert packet12.get("authorized") is True
    assert packet12.get("verdict") == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
...
def test_qft_gr_seam_packet11_assessment_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet11 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet11 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_assessment = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET11_ASSESSMENT_STATUS_v0")
    roadmap_assessment = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET11_ASSESSMENT_STATUS_v0")
