...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_sr_class_b_seam_physics_pilot_cycle06_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_sr_class_b_seam_physics_pilot_cycle05_v0.json"
...
def test_cosmo_sr_cycle06_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing COSMO-SR Cycle06 target doc."
    assert ARTIFACT_PATH.exists(), "Missing COSMO-SR Cycle06 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing COSMO-SR Cycle05 predecessor artifact."


def test_cosmo_sr_cycle06_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
    ...
    assert not missing, "COSMO-SR Cycle06 doc missing required token(s): " + ", ".join(missing)


def test_cosmo_sr_cycle06_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "cosmo_sr_class_b_seam_physics_pilot_cycle06_v0"
    assert artifact["seam_id"] == "SEAM-COSMO-SR"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_DECIC_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "cosmo_sr_class_b_seam_physics_pilot_cycle05_v0"
    assert derived["artifact_path"] == "formal/output/cosmo_sr_class_b_seam_physics_pilot_cycle05_v0.json"


def test_cosmo_sr_cycle06_low_z_decic_improvement() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "EXACT_SR_DOPPLER_MATCH_DECIC_IMPROVEMENT_REQUIRED"
    ...
    for sample in criteria["samples"]:
        ...
        assert abs_delta_series10 <= abs_delta_series8
        assert abs_delta_series10 <= eps


def test_cosmo_sr_cycle06_high_z_exclusion_and_nonclaim_boundary() -> None:
    artifact = _json(ARTIFACT_PATH)
    exclusion = artifact["bounded_incompatibility_exclusion"]

    z = Fraction(exclusion["z"])
    beta_exact = Fraction(exclusion["beta_exact"])
    beta_series10 = Fraction(exclusion["beta_series10"])
    abs_delta = Fraction(exclusion["abs_delta"])

    assert beta_exact == _sr_beta_exact(z)
    assert beta_series10 == _series10(z)
    assert abs(beta_exact - beta_series10) == abs_delta
    ...
    assert adjudication["token"] == "COSMO_SR_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE06_ADJUDICATION"
