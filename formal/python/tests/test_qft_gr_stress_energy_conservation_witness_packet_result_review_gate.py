...
from formal.python.tools.qft_gr_stress_energy_conservation_witness_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_stress_energy_conservation_witness_packet_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_StressEnergyConservationWitnessPacketResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_stress_energy_conservation_witness_packet_result_review_report.py"
)
...
def test_qft_gr_stress_energy_conservation_witness_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_qft_gr_stress_energy_conservation_witness_packet"] == PACKET_ID
    ...
    assert packet["post_packet_review_target"] == (
        "review_qft_gr_stress_energy_conservation_witness_packet_result"
    )


def test_qft_gr_stress_energy_conservation_witness_packet_result_review_gate_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["primary_missing_condition"] == "conservation"
    assert review["primary_obstruction_preserved"] is True
...
def test_qft_gr_stress_energy_conservation_witness_packet_result_review_selects_attempt() -> None:
    review = _json(DEFAULT_OUT)
    assert review["bounded_conservation_witness_attempt_authorized"] is True
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == (
        "qft_gr_stress_energy_conservation_witness_attempt_execution"
    )
...
def test_qft_gr_stress_energy_conservation_witness_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_stress_energy_conservation_witness_packet_result_review(
    ...
    for path in [REGISTRY_PATH, CURRENT_SURFACES_PATH]:
        ...
        assert OUTCOME_ID in text
        assert NEXT_TARGET in text
        assert "review_qft_gr_stress_energy_conservation_witness_packet_result" in text
