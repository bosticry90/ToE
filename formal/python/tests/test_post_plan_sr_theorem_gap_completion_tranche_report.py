...
from formal.python.tools import post_plan_sr_theorem_gap_completion_tranche_report as tool
    ...
    _write_json(
        path,
        {
            "required_inputs": {
                ...
                "sr_target_doc": "formal/docs/paper/DERIVATION_TARGET_SR_EMPIRICAL_COMPARISON_PACKET_05_v0.md",
                "sr_artifact": "formal/output/sr_empirical_comparison_packet_05_v0.json",
                "sr_gate": "formal/python/tests/test_sr_empirical_comparison_packet_05_gate.py"
            },
            "execution_policy": {
                "required_target_row": "ROW-PILLAR-SR-001",
                ...
                "required_queue_follow_on_row": "ROW-PILLAR-SR-001",
                "required_prior_em_outcome": "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_NONPROMOTED",
                "required_program_state_conversion_review_basis": "THREE_UPSTREAM_EXPLANATIONS_EXHAUSTED_NON_MOVEMENT_PERSISTS",
                "required_target_decision": "RETAIN_v0",
                "required_target_status": "RUN_BOUNDED_v0_NONCLAIM",
                "required_target_evidence_tier": "INTERMEDIATE_v0"
            },
            "outcome_contract": {
                "single_terminal_outcome_rule": "EXACTLY_ONE_ALLOWED_POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_OUTCOME",
                "no_loop_rule": "ONE_POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_LAYER_ONLY",
                "allowed_outcomes": [
                    "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED",
                    "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_NONPROMOTED",
                    "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED",
                    "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE",
                    "HOLD_PENDING_POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_REPAIR"
                ],
                "default_outcome": "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE"
            }
        },
    )


def _seed_inputs(root: Path, *, decision: str = "RETAIN_v0") -> None:
    _write_json(
        root / "formal" / "output" / "reports" / "post_plan_objective_quality_physics_completion_queue_20260418_v0.json",
        {
            ...
            "completion_queue": [{"row_id": "ROW-PILLAR-SR-001", "queue_rank": 6}],
        },
    )
    _write_json(
    ...
    _write_json(
        ...
        {"routed_rows": [{"row_id": "ROW-PILLAR-SR-001", "route_class": "THEOREM_GAP_PROGRAM"}]},
    )
    _write_text(
    ...
    _write_json(
        ...
        {"modeled_observations": [{"row_id": "ROW-PILLAR-SR-001", "observation_type": "PILLAR_M4_QUALIFIED_BY_LIVE_THEOREM_GAP"}]},
    )
    _write_json(
    ...
    _write_text(
        root / "formal" / "docs" / "paper" / "DERIVATION_TARGET_SR_EMPIRICAL_COMPARISON_PACKET_05_v0.md",
        "\n".join(
            [
    ...
    _write_json(
        root / "formal" / "output" / "sr_empirical_comparison_packet_05_v0.json",
        {
            "artifact_id": "sr_empirical_comparison_packet_05_v0",
            "payload": {"status": "RUN_BOUNDED_v0_NONCLAIM", "decision": decision, "evidence_tier": "INTERMEDIATE_v0"},
        },
    )
    _write_text(root / "formal" / "python" / "tests" / "test_sr_empirical_comparison_packet_05_gate.py", "def test_gate():\n    assert True\n")


def test_sr_tranche_reports_nonpromoted_from_live_shape(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(tool, "REPO_ROOT", tmp_path)
    declaration_path = tmp_path / "formal" / "docs" / "release" / "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_20260418_v0.json"
    _write_declaration(declaration_path)
    _seed_inputs(tmp_path)

    report = tool.build_report(declaration_path=declaration_path, captured_at_utc=None)

    assert report["summary"]["terminal_outcome"] == "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_NONPROMOTED"
    assert report["summary"]["target_row_id"] == "ROW-PILLAR-SR-001"
    assert report["summary"]["queue_rank"] == 6
    assert report["summary"]["row_truth_change_detected"] is False


def test_sr_tranche_reports_exhausted_when_pruned(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(tool, "REPO_ROOT", tmp_path)
    declaration_path = tmp_path / "formal" / "docs" / "release" / "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_20260418_v0.json"
    _write_declaration(declaration_path)
    ...
    assert report["summary"]["terminal_outcome"] == "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED"


def test_live_sr_tranche_registered_in_mirrors() -> None:
    ...
    required_refs = [
        "formal/docs/release/POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_20260418_v0.json",
        "formal/output/reports/post_plan_sr_theorem_gap_completion_tranche_20260418_v0.json",
        "formal/python/tools/post_plan_sr_theorem_gap_completion_tranche_report.py",
        "formal/python/tests/test_post_plan_sr_theorem_gap_completion_tranche_report.py",
    ]
    ...
    report = _read_json(REPO_ROOT / "formal" / "output" / "reports" / "post_plan_sr_theorem_gap_completion_tranche_20260418_v0.json")
    assert report["summary"]["terminal_outcome"] == "POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_NONPROMOTED"
    assert report["summary"]["target_row_id"] == "ROW-PILLAR-SR-001"
    assert report["summary"]["queue_rank"] == 6
    assert report["summary"]["row_truth_change_detected"] is False