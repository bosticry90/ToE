...
def test_packet03_matrix_surface_is_pinned() -> None:
    matrix = _read_json(MATRIX_PATH)
    roadmap_text = _read(ROADMAP_PATH)
    inventory_text = _read(INVENTORY_PATH)
    state_text = _read(STATE_PATH)

    assert matrix.get("matrix_id") == "FOUNDATIONAL_EMPIRICAL_COMPARISON_PACKET03_MATRIX_v0"
...
def test_packet03_rows_match_docs_and_artifacts() -> None:
    matrix = _read_json(MATRIX_PATH)
    ...
    assert _extract_token(protocol_text, "FOUNDATIONAL_EMPIRICAL_PACKET_03_BASELINE_DECISION_v0") == (
        "INCONCLUSIVE_ONLY_UNTIL_PACKET04_OR_HIGHER"
    )
    ...
    for lane, row in sorted(rows.items()):
        ...
        assert artifact.get("artifact_id", "").endswith("_packet_03_v0")
        ...
        assert f"{lane}_EMPIRICAL_PACKET_03_STATUS_v0" in doc_text
        assert f"{lane}_EMPIRICAL_PACKET_03_ARTIFACT_v0" in doc_text
        assert f"{lane}_EMPIRICAL_PACKET_03_GATE_v0" in doc_text
        assert f"{lane}_EMPIRICAL_PACKET_03_DECISION_v0" in doc_text
        assert f"{lane}_EMPIRICAL_PACKET_03_EVIDENCE_TIER_v0" in doc_text
