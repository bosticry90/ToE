...
COSMO_MICRO29_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_29_DRYRUN_NONFLIP_EXECUTION_CUSTODY_RECERTIFICATION_CONTINUITY_AUDIT_v0.md"
COSMO_MICRO29_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro29_dryrun_nonflip_execution_custody_recertification_continuity_audit_cycle01_v0.json"
...
def test_cosmo_micro29_artifacts_exist() -> None:
    assert COSMO_TARGET_PATH.exists(), "Missing COSMO background target document."
    assert COSMO_MICRO29_PATH.exists(), "Missing COSMO background Cycle-029 micro document."
    assert COSMO_MICRO29_ARTIFACT_PATH.exists(), "Missing COSMO background Cycle-029 artifact payload."


def test_cosmo_target_references_micro29_and_gate() -> None:
    ...
    assert not missing, "COSMO parent target is missing required micro-29 token(s): " + ", ".join(missing)


def test_cosmo_micro29_doc_contains_required_headers_and_tokens() -> None:
    text = _read(COSMO_MICRO29_PATH)
    required_headers = [
        "Spec ID:",
        "Target ID:",
        "Classification:",
    ...
    assert not missing_headers, "COSMO micro-29 document is missing required header(s): " + ", ".join(missing_headers)
    ...
    assert not missing_tokens, "COSMO micro-29 document is missing required token(s): " + ", ".join(missing_tokens)


def test_cosmo_micro29_artifact_schema_and_scope_boundary() -> None:
    payload = _read_json(COSMO_MICRO29_ARTIFACT_PATH)

    assert payload.get("artifact_id") == "cosmo_bg_micro29_dryrun_nonflip_execution_custody_recertification_continuity_audit_cycle01_v0"
    ...
    assert boundary_statement == "DRYRUN_NONFLIP_LANE_ONLY_EXECUTION_CUSTODY_RECERTIFICATION_CONTINUITY_NO_ADJUDICATION_FLIP_NO_COMPARATOR_AUTHORIZATION"


def test_cosmo_micro29_forbidden_tokens_not_present() -> None:
    doc_text = _read(COSMO_MICRO29_PATH)
    artifact_text = _read(COSMO_MICRO29_ARTIFACT_PATH)
    forbidden_tokens = [
...
def test_cosmo_micro29_cross_surface_pointers_are_complete() -> None:
    matrix = _read_json(MATRIX_PATH)
    cosmo = matrix.get("pillars", {}).get("PILLAR-COSMO")
    assert isinstance(cosmo, dict), "PILLAR-COSMO matrix row must exist."

    assert cosmo.get("dryrun_nonflip_execution_custody_recertification_continuity_audit_doc") == "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_29_DRYRUN_NONFLIP_EXECUTION_CUSTODY_RECERTIFICATION_CONTINUITY_AUDIT_v0.md"
    assert cosmo.get("dryrun_nonflip_execution_custody_recertification_continuity_audit_gate") == "formal/python/tests/test_cosmo_bg_micro29_dryrun_nonflip_execution_custody_recertification_continuity_audit_gate.py"
    assert cosmo.get("dryrun_nonflip_execution_custody_recertification_continuity_audit_policy") == "CYCLE08_09_10_11_12_13_14_15_16_17_18_19_20_21_22_23_24_25_26_27_28_DRYRUN_NONFLIP_EXECUTION_CUSTODY_RECERTIFICATION_CONTINUITY_LOCK_REQUIRED_NO_STATUS_FLIP_NO_COMPARATOR_AUTHORIZATION"
    ...
    assert not missing_state, "State missing dryrun nonflip execution-custody recertification continuity token(s): " + ", ".join(missing_state)
    ...
    assert not missing_target, "COSMO target missing dryrun nonflip execution-custody recertification continuity token(s): " + ", ".join(missing_target)
    ...
    assert not missing_rollup, "Rollup gate missing dryrun nonflip execution-custody recertification continuity cross-pin token(s): " + ", ".join(missing_rollup)
