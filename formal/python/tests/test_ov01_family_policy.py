...
from formal.python.toe.observables.ov01_family_policy import ov01_preferred_family_for_pruning
...
def test_ov01_family_policy_defaults_to_v1_and_selects_curved_for_dr04d_min_n5_set():
    ...
    sel = ov01_preferred_family_for_pruning(fn, [dr02, dr03, dr04d_lin, dr05], [dr02c, dr03c, dr04d, dr05c])

    assert sel.gate_report.adequacy_policy == "DQ-01_v1"
    assert sel.preferred_family == "curved"
    assert sel.gate_report.prefer_curved is True
    assert sel.gate_report.robustness_failed is False


def test_ov01_family_policy_v2_selects_curved_for_dr04d_min_n5_set():
    base = REPO_ROOT / "formal" / "external_evidence" / "bec_bragg_steinhauer_2001"
    ...
    sel = ov01_preferred_family_for_pruning(
        fn,
    ...
    assert sel.preferred_family == "curved"
    assert sel.gate_report.adequacy_policy == "DQ-01_v2"
    assert sel.gate_report.prefer_curved is True
    assert sel.gate_report.robustness_failed is False


def test_ov01_family_policy_v1_is_undecided_for_legacy_dr04a_set():
    base = REPO_ROOT / "formal" / "external_evidence" / "bec_bragg_steinhauer_2001"
    ...
    assert sel.preferred_family == "undecided"
    assert sel.gate_report.prefer_curved is False
    assert sel.gate_report.robustness_failed is True


def test_ov01_family_policy_variantA_selects_curved_for_legacy_dr04a_set_when_dr04_is_admitted():
    base = REPO_ROOT / "formal" / "external_evidence" / "bec_bragg_steinhauer_2001"
    ...
    assert sel.gate_report.adequacy_policy == "DQ-01_variantA"
    assert sel.preferred_family == "curved"
    assert sel.gate_report.prefer_curved is True
    assert sel.gate_report.robustness_failed is False
