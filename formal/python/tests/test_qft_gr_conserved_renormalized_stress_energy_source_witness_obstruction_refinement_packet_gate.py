...
from formal.python.tools.qft_gr_conserved_renormalized_stress_energy_source_witness_obstruction_refinement_packet_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_conserved_renormalized_stress_energy_source_witness_obstruction_refinement_packet,
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_ConservedRenormalizedStressEnergySourceWitnessObstructionRefinementPacket.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_conserved_renormalized_stress_energy_source_witness_obstruction_refinement_packet_report.py"
)
...
def test_qft_gr_source_witness_obstruction_refinement_packet_files_exist() -> None:
...
def test_qft_gr_source_witness_obstruction_refinement_packet_consumes_review() -> None:
    ...
    assert packet["accepted_obstruction_classification"] == OBSTRUCTION_CLASS


def test_qft_gr_source_witness_obstruction_refinement_packet_rows_and_primary() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["missing_condition_menu"] == MISSING_CONDITION_CANDIDATES
    ...
    assert [row["missing_condition"] for row in packet["refinement_rows"]] == (
        MISSING_CONDITION_CANDIDATES
    )
    required = {
        "obstruction_id",
...
def test_qft_gr_source_witness_obstruction_refinement_packet_preserves_nonclaim_boundaries() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["witness_constructed"] is False
    assert packet["completed_witness_constructed"] is False
...
def test_qft_gr_source_witness_obstruction_refinement_packet_selects_one_next_target() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_qft_gr_source_witness_obstruction_refinement_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_conserved_renormalized_stress_energy_source_witness_obstruction_refinement_packet(
    ...
    joined = "\n".join(
        _read(path)
        for path in [SURFACES_PATH, REGISTRY_PATH, ROADMAP_PATH, LEAN_PACKET_PATH]
    )
