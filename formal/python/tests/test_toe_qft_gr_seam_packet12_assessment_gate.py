...
def test_qft_gr_seam_packet12_assessment_document_structure() -> None:
    ...
    required_markers = [
        ...
        "did packet12 satisfy its exact bounded target?",
        "target_satisfaction_verdict: YES_EXACT_TARGET_SATISFIED_v0",
        "material_advancement_verdict: YES_MATERIAL_ADVANCEMENT_v0",
        "packet13_authorization_verdict: JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0",
        "packet13_exact_bounded_target:",
        ...
        "TOE_QFT_GR_SEAM_PACKET12_ASSESSMENT_GATE_v0: REQUIRED_PACKET12_ASSESSMENT_SCHEMA_AND_AUTHORITY_PARITY",
        "TOE_QFT_GR_SEAM_PACKET12_ASSESSMENT_ARTIFACT_v0: toe_qft_gr_seam_packet12_assessment_checkpoint_v0",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet12 assessment doc missing marker: {marker}"


def test_qft_gr_seam_packet12_assessment_checkpoint_schema() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet12_assessment_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1S_QFT_GR_SEAM_PACKET12_ASSESSMENT"
    assert artifact.get("status") == "PACKET12_ASSESSMENT_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET12_ASSESSMENT_v0.md"
    assert payload.get("parent_packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET12_BOUNDED_EXECUTION_v0.md"
    ...
    assert packet_artifact.get("status") == "PACKET12_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"
    assert auth_artifact.get("status") == "PACKET12_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"
    ...
    packet13 = payload.get("packet13_decision", {})
    assert packet13.get("authorized") is True
    assert packet13.get("verdict") == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet13.get("hold_refine_trigger_status") == "NOT_HIT_v0"
...
def test_qft_gr_seam_packet12_assessment_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet12 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet12 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_assessment = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET12_ASSESSMENT_STATUS_v0")
    roadmap_assessment = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET12_ASSESSMENT_STATUS_v0")
