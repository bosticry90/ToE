...
DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_COSMO_SR_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE08_v0.md"
)
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_sr_class_b_seam_physics_pilot_cycle08_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_sr_class_b_seam_physics_pilot_cycle07_v0.json"
...
def _series12(z: Fraction) -> Fraction:
    return _series10(z) - (z**12) / 64


def _series14(z: Fraction) -> Fraction:
    return _series12(z) - (z**14) / 128


def test_cosmo_sr_cycle08_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing COSMO-SR Cycle08 target doc."
    assert ARTIFACT_PATH.exists(), "Missing COSMO-SR Cycle08 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing COSMO-SR Cycle07 predecessor artifact."


def test_cosmo_sr_cycle08_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
    ...
    assert not missing, "COSMO-SR Cycle08 doc missing required token(s): " + ", ".join(missing)


def test_cosmo_sr_cycle08_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "cosmo_sr_class_b_seam_physics_pilot_cycle08_v0"
    assert artifact["seam_id"] == "SEAM-COSMO-SR"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_TETRADECIC_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "cosmo_sr_class_b_seam_physics_pilot_cycle07_v0"
    assert derived["artifact_path"] == "formal/output/cosmo_sr_class_b_seam_physics_pilot_cycle07_v0.json"


def test_cosmo_sr_cycle08_low_z_tetradecic_improvement() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "EXACT_SR_DOPPLER_MATCH_TETRADECIC_IMPROVEMENT_REQUIRED"
    ...
    for sample in criteria["samples"]:
        ...
        beta_series14 = Fraction(sample["beta_series14"])
        abs_delta_series12 = Fraction(sample["abs_delta_series12"])
        abs_delta_series14 = Fraction(sample["abs_delta_series14"])

        assert z_min <= z <= z_max
        assert beta_exact == _sr_beta_exact(z)
        assert beta_series12 == _series12(z)
        assert beta_series14 == _series14(z)

        assert abs(beta_exact - beta_series12) == abs_delta_series12
        assert abs(beta_exact - beta_series14) == abs_delta_series14

        assert abs_delta_series14 <= abs_delta_series12
        assert abs_delta_series14 <= eps


def test_cosmo_sr_cycle08_high_z_exclusion_and_nonclaim_boundary() -> None:
    artifact = _json(ARTIFACT_PATH)
    exclusion = artifact["bounded_incompatibility_exclusion"]

    z = Fraction(exclusion["z"])
    beta_exact = Fraction(exclusion["beta_exact"])
    beta_series14 = Fraction(exclusion["beta_series14"])
    abs_delta = Fraction(exclusion["abs_delta"])

    assert beta_exact == _sr_beta_exact(z)
    assert beta_series14 == _series14(z)
    assert abs(beta_exact - beta_series14) == abs_delta
    ...
    assert adjudication["token"] == "COSMO_SR_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE08_ADJUDICATION"
