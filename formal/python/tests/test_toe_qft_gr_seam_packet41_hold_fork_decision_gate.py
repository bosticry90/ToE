...
TARGETED_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet41_targeted_justification_review_checkpoint_v0.json"
...
def test_qft_gr_seam_packet41_hold_fork_decision_document_structure() -> None:
    ...
    for marker in required_markers:
        assert marker in text, f"Packet41 hold-fork decision doc missing marker: {marker}"


def test_qft_gr_seam_packet41_hold_fork_decision_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("status") == "PACKET41_HOLD_FORK_DECISION_COMPLETE_HOLD_v0"
...
def test_qft_gr_seam_packet41_hold_fork_decision_authority_parity_and_freeze() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet41 hold-fork decision pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet41 hold-fork decision pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_status = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET41_HOLD_FORK_DECISION_STATUS_v0")
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET41_HOLD_FORK_DECISION_STATUS_v0")
    assert state_status == roadmap_status == "DECISION_COMPLETE_HOLD_v0"

    state_outcome = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET41_HOLD_FORK_DECISION_OUTCOME_v0")
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET41_HOLD_FORK_DECISION_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_v0"

    assert not PACKET41_AUTH_DOC_PATH.exists(), "Packet41 authorization doc must not exist during hold-fork HOLD disposition"
    assert not PACKET41_AUTH_CHECKPOINT_PATH.exists(), "Packet41 authorization checkpoint must not exist during hold-fork HOLD disposition"
