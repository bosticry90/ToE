...
def test_bridge_program_orthogonality_report_is_deterministic() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    r1 = build_bridge_program_orthogonality_report(repo_root=repo_root)
    r2 = build_bridge_program_orthogonality_report(repo_root=repo_root)

    assert r1 == r2
    assert r1.get("schema_version") == 1
    assert r1.get("report_id") == "BRIDGE_PROGRAM_ORTHOGONALITY_REPORT"
