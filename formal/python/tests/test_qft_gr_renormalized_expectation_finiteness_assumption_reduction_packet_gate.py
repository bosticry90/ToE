...
from formal.python.tools.qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_report import (
    ...
    SCHEMA_ID,
    SELECTED_ROW_ID,
    build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet,
)
...
def test_qft_gr_renormalized_expectation_finiteness_packet_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_finiteness_packet_consumes_rn003_result_review() -> None:
...
def test_qft_gr_renormalized_expectation_finiteness_packet_selects_only_row004() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_expectation_finiteness_assumption"]
    ...
    assert packet["accepted_prior_row_count"] == 3
    ...
    assert packet["renormalized_expectation_finiteness_status_tokens"] == selected[
        "current_status"
    ]


def test_qft_gr_renormalized_expectation_finiteness_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_expectation_finiteness_assumption"]
    ...
    assert packet["finiteness_regular_boundary"] == FINITENESS_REGULARITY_BOUNDARY
    assert packet["finiteness_regular_boundary_condition"] == (
        FINITENESS_REGULARITY_BOUNDARY
    )
    assert selected["finiteness_regular_boundary_condition"] == (
        FINITENESS_REGULARITY_BOUNDARY
    )
    assert packet["finiteness_regular_boundary_status"] == (
        FINITENESS_REGULARITY_CONDITION_STATUS
    )
    assert selected["finiteness_regular_boundary_status"] == (
        FINITENESS_REGULARITY_CONDITION_STATUS
    )
    assert packet["scope_boundaries"] == FINITENESS_REGULARITY_BOUNDARY_SCOPE
    assert selected["scope_boundaries"] == FINITENESS_REGULARITY_BOUNDARY_SCOPE
...
def test_qft_gr_renormalized_expectation_finiteness_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["finiteness_regular_boundary_discharged"] is False
    assert packet["finiteness_regular_boundary_reduced_or_discharged_by_preparation"] is False
...
def test_qft_gr_renormalized_expectation_finiteness_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_renormalized_expectation_finiteness_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet(
