...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_nonflip_execution_custody_status_cycle01_v0.json"
BOUNDARY_STATUS_ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "stat_nonflip_execution_boundary_status_cycle01_v0.json"
)

EXPECTED_ARTIFACT_ID = "stat_nonflip_execution_custody_status_cycle01_v0"
EXPECTED_BOUNDARY_STATUS_ARTIFACT_ID = "stat_nonflip_execution_boundary_status_cycle01_v0"
EXPECTED_COUPLING_GATE = "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_STATUS_VALUE = "NONFLIP_EXECUTION_CUSTODY_SCAFFOLD_PINNED_NONCLAIM"
ALLOWED_NONFLIP_CUSTODY_ATTESTATION_STATUS_VALUES = {
    "NOT_PRESENT_v0",
    "NONFLIP_EXECUTION_CUSTODY_ATTESTATION_SCAFFOLD_PINNED_NONCLAIM",
}
EXPECTED_ARTIFACT_REL = "formal/output/stat_nonflip_execution_custody_status_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_nonflip_execution_custody_status_cycle01_gate.py"
...
def test_stat_nonflip_execution_custody_status_cycle01_gate() -> None:
    stat_text = _read(STAT_DOC_PATH)
    state_text = _read(STATE_PATH)
    roadmap_text = _read(ROADMAP_PATH)
    matrix = _read_json(MATRIX_PATH)
    artifact_json = _read_json(ARTIFACT_PATH)
    boundary_status_artifact_json = _read_json(BOUNDARY_STATUS_ARTIFACT_PATH)

    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT nonflip-execution-custody status gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for nonflip-execution-custody status gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT nonflip-execution-custody status cycle-01 payload_sha256 does not match canonical payload hash."
    )

    assert boundary_status_artifact_json.get("artifact_id") == EXPECTED_BOUNDARY_STATUS_ARTIFACT_ID

    for token_name, expected in (
        ("STAT_NONFLIP_EXECUTION_CUSTODY_STATUS_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_NONFLIP_EXECUTION_CUSTODY_STATUS_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
        ("STAT_NONFLIP_EXECUTION_CUSTODY_STATUS_v0", EXPECTED_STATUS_VALUE),
    ):
    ...
    for doc_text, doc_label in (
        ...
        nonflip_custody_attestation_status = _extract_token(
            doc_text, "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_STATUS_v0"
        )
        assert nonflip_custody_attestation_status in ALLOWED_NONFLIP_CUSTODY_ATTESTATION_STATUS_VALUES, (
            f"{doc_label} must keep the nonflip-execution-custody-attestation status either pre-admission or attestation-status pinned."
        )

    stat_sha = _extract_token(stat_text, "STAT_NONFLIP_EXECUTION_CUSTODY_STATUS_CYCLE01_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_NONFLIP_EXECUTION_CUSTODY_STATUS_CYCLE01_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_NONFLIP_EXECUTION_CUSTODY_STATUS_CYCLE01_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert _extract_token(doc_text, "EVIDENCE_ADEQUACY_STAT_5X5_JUSTIFICATION_v0") == "PRESENT", (
            f"{doc_label} must preserve the adequacy 5x5 justification token before nonflip-execution-custody status admission."
        )
        assert _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0") == "PRESENT", (
            f"{doc_label} must preserve the readiness packet before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            ...
            f"{doc_label} must preserve the gate entry-status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_v0"
        ) == "ENTRY_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger discharge surface-status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0"
        ) == "THEOREM_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger theorem-surface status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_v0"
        ) == "OBJECT_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger object-surface status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_v0"
        ) == "COHERENCE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger coherence-status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_v0"
        ) == "DISCHARGE_COMPLETION_TRANSITION_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the discharge-completion transition-status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_ADJUDICATION_TRANSITION_STATUS_v0"
        ) == "ADJUDICATION_TRANSITION_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the adjudication-transition status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_INEVITABILITY_TRANSITION_STATUS_v0"
        ) == "INEVITABILITY_TRANSITION_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the inevitability-transition status token before nonflip-execution-custody status admission."
        )
        assert _extract_token(
            doc_text, "STAT_NONFLIP_EXECUTION_BOUNDARY_STATUS_v0"
        ) == "NONFLIP_EXECUTION_BOUNDARY_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the nonflip-execution-boundary status token before nonflip-execution-custody status admission."
        )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the nonflip-execution-custody status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the nonflip-execution-custody status gate path."
        )

    for token_name, expected in (
        (
            "STAT_NONFLIP_EXECUTION_BOUNDARY_STATUS_CYCLE01_ARTIFACT_v0",
            EXPECTED_BOUNDARY_STATUS_ARTIFACT_ID,
        ),
        (
    ...
    assert payload.get("checkpoint") == "stat_nonflip_execution_custody_status_cycle01"
    assert payload.get("status") == "nonflip_execution_custody_status_non_promotional"
    assert payload.get("custody_entry_scope") == [
        "nonflip_execution_custody_status_placeholder_only",
        "nonflip_execution_boundary_status_verified_before_nonflip_execution_custody_status",
        "nonflip_execution_custody_attestation_scope_boundary_pinned_before_nonflip_execution_custody_status",
        "no_nonflip_execution_custody_claim",
        "no_external_truth_claim",
    ]
    assert payload.get("required_custody_status_inputs") == [
        ...
        "nonflip_execution_custody_scope_boundary_pinned",
    ...
    assert payload.get("emitted_status_tokens") == [
        ...
        "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_STATUS_v0: NOT_PRESENT_v0",
    ]
