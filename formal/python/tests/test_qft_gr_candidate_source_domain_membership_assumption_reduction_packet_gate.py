...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_CandidateSourceDomainMembershipAssumptionReductionPacket.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_candidate_source_domain_membership_assumption_reduction_packet_report.py"
)
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_files_exist() -> None:
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_consumes_row001_review() -> None:
    ...
    assert (
        packet[
            "consumes_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review"
        ]
        == RESULT_REVIEW_ID
    )
    ...
    assert packet["prior_accepted_operator_domain_assumption_row"] == PRIOR_ACCEPTED_ROW
    assert (
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_selects_only_row002() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["candidate_source_domain_membership_assumption"]
    ...
    assert packet["candidate_source_domain_membership_status_tokens"] == selected[
        "current_status"
    ]
    assert packet["row_status_enum"] == ROW_STATUS_ENUM


def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["candidate_source_domain_membership_assumption"]
    assert packet["candidate_source_object"] == CANDIDATE_SOURCE_OBJECT
    assert packet["operator_domain_membership_condition"] == (
        OPERATOR_DOMAIN_MEMBERSHIP_CONDITION
    )
    assert selected["candidate_source_object"] == CANDIDATE_SOURCE_OBJECT
    assert selected["operator_domain_membership_condition"] == (
        OPERATOR_DOMAIN_MEMBERSHIP_CONDITION
    )
    for field in [
    ...
    assert packet["candidate_source_domain_membership_assumption_discharged"] is False


def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "prepare_qft_gr_state_expectation_domain_link_assumption_reduction_packet": "deferred",
...
def test_qft_gr_candidate_source_domain_membership_assumption_reduction_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_candidate_source_domain_membership_assumption_reduction_packet(
