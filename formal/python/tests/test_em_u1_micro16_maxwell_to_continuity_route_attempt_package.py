...
LOCALIZATION_TOKEN = "EM_U1_MAXWELL_CONTINUITY_LOCALIZATION_GATE_v0: CYCLE16_ARTIFACTS_ONLY"
PREREQUISITE_TOKENS = [
...
ROUTE_PATTERNS = {
    "divergence_applied_to_inhom_surface": re.escape("∂_ν(∂_μ F^{μν}) = ∂_ν J^ν"),
    "antisymmetry_commuting_partials_seam": re.escape("∂_ν∂_μ F^{μν} = 0"),
    "continuity_target_surface": re.escape("∂_ν J^ν = 0"),
}
...
def test_em_micro16_contains_required_tokens_and_localized_route_statements() -> None:
    text = _read(EM_MICRO16_PATH)
    required_tokens = [
    ...
    assert not missing, "EM Cycle-016 micro document is missing required token(s): " + ", ".join(missing)

    missing_patterns = [label for label, pattern in ROUTE_PATTERNS.items() if re.search(pattern, text) is None]
    assert not missing_patterns, (
        "EM Cycle-016 micro document is missing required route seam expression(s): "
        + ", ".join(missing_patterns)
    )


def test_em_target_references_cycle016_artifact_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM target document is missing required Cycle-016 token(s): " + ", ".join(missing)


def test_em_roadmap_references_cycle016_target_and_artifact_with_single_row() -> None:
    ...
    assert "TARGET-EM-U1-MICRO-16-MAXWELL-TO-CONTINUITY-ROUTE-ATTEMPT-PACKAGE-v0" in targets
    assert (
        "formal/docs/paper/DERIVATION_TARGET_EM_U1_MICRO_16_MAXWELL_TO_CONTINUITY_ROUTE_ATTEMPT_PACKAGE_v0.md"
        in artifacts
    )
...
def test_state_mentions_cycle016_checkpoint_and_tokens() -> None:
    text = _read(STATE_PATH)
    required_tokens = [
        "EM Cycle-016 Maxwell-to-continuity route attempt-package checkpoint",
        "TARGET-EM-U1-MICRO-16-MAXWELL-TO-CONTINUITY-ROUTE-ATTEMPT-PACKAGE-v0",
        "EM_U1_PROGRESS_CYCLE16_v0: MAXWELL_TO_CONTINUITY_ROUTE_TOKEN_PINNED",
        "EM_U1_MAXWELL_CONTINUITY_ROUTE_v0: DIVERGENCE_OF_INHOM_SURFACE_ROUTE_PINNED",
        LOCALIZATION_TOKEN,
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "State_of_the_Theory.md is missing required Cycle-016 token(s): " + ", ".join(missing)


def test_em_lean_cycle016_tokens_and_route_harness_stubs_are_pinned() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM U1 Lean scaffold is missing required Cycle-016 token(s): " + ", ".join(missing)


def test_cycle016_route_statements_are_localized_to_cycle16_artifact_only() -> None:
    allowed_paths = [EM_MICRO16_PATH]
    scoped_paths = [
        EM_TARGET_PATH,
        EM_MICRO01_PATH,
        EM_MICRO02_PATH,
    ...
    for path in allowed_paths:
        text = _read(path)
        hits = [label for label, pattern in ROUTE_PATTERNS.items() if re.search(pattern, text)]
        if not hits:
            violations.append(f"cycle016 artifact {path.relative_to(REPO_ROOT)} is missing localized route expressions")
            continue
        ...
        if not (has_assumption_id and has_localization_gate and has_prereq and has_non_selection):
            violations.append(
                f"cycle016 artifact {path.relative_to(REPO_ROOT)} contains route expressions {hits} "
    ...
    for path in scoped_paths:
        ...
        for label, pattern in ROUTE_PATTERNS.items():
            if re.search(pattern, text):
                violations.append(
                    f"{label} route expression leaked into non-authorized artifact "
                    f"{path.relative_to(REPO_ROOT)}"
                )

    assert not violations, "Cycle-016 route localization violation:\n- " + "\n- ".join(violations)


def test_cycle016_is_attempt_package_only_and_blocks_derivation_or_new_physics_selection_language() -> None:
    text = _read(EM_MICRO16_PATH)
    forbidden_patterns = [
        r"\bwe derive\b",
        r"\bwe prove\b",
    ...
    assert not violations, (
        "Cycle-016 must remain attempt-package-only/non-derivational with no import-lane selections. "
