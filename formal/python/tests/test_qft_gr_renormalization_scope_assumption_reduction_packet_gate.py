...
from formal.python.tools.qft_gr_renormalization_scope_assumption_reduction_packet_report import (
    ...
    SCOPE_BOUNDARIES,
    SELECTED_ROW_ID,
    build_qft_gr_renormalization_scope_assumption_reduction_packet,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_renormalization_scope_assumption_reduction_packet_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_renormalization_scope_packet_files_exist() -> None:
...
def test_qft_gr_renormalization_scope_packet_consumes_rn001_result_review() -> None:
...
def test_qft_gr_renormalization_scope_packet_selects_only_row002() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalization_scope_assumption"]
    ...
    assert packet["prior_completed_family"] == PRIOR_COMPLETED_FAMILY
    assert packet["prior_operator_domain_assumptions_accepted"] is True
    assert packet["accepted_prior_renormalization_assumption_row"] == ACCEPTED_PRIOR_ROW
    ...
    assert packet["renormalization_scope_status_tokens"] == selected["current_status"]


def test_qft_gr_renormalization_scope_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalization_scope_assumption"]
    assert packet["candidate_stress_energy_object"] == RENORMALIZED_STRESS_ENERGY_OBJECT
    assert packet["renormalization_scope"] == RENORMALIZATION_SCOPE
    assert packet["renormalization_scope_object"] == RENORMALIZATION_SCOPE
    assert packet["renormalization_scope_status"] == RENORMALIZATION_SCOPE_STATUS
    assert packet["scope_boundaries"] == SCOPE_BOUNDARIES
    assert selected["renormalization_scope_object"] == RENORMALIZATION_SCOPE
    assert selected["renormalization_scope_status"] == RENORMALIZATION_SCOPE_STATUS
    assert selected["scope_boundaries"] == SCOPE_BOUNDARIES
    ...
    assert packet["renormalization_scope_assumption_reduction_analysis_prepared"] is True
    assert packet["prepares_reduction_analysis_only"] is True


def test_qft_gr_renormalization_scope_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["renormalization_scope_assumption_discharged"] is False
    assert (
        packet["renormalization_scope_assumption_reduced_or_discharged_by_preparation"]
        is False
    )
...
def test_qft_gr_renormalization_scope_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_renormalization_scope_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_renormalization_scope_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalization_scope_assumption_reduction_packet(
