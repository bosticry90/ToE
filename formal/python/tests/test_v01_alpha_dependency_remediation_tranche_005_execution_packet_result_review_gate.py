...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_report import (
...
RESULT_REVIEW_PATH = (
    RELEASE_DIR
    / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0.json"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche005ExecutionPacketResultReview.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_consumes_packet() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0"
    )
    ...
    assert review["consumes_tranche_005_execution_packet"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_v0"
    )
    assert review["consumes_tranche_005_execution_packet_pointer"] == (
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_20260515_v0.json"
    )


def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_preserves_posture() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_005_status"] == "lean_dependency_audit_scope_accepted_pending_execution"
    assert review["tranche_005_cleared_for_global_release_readiness"] is False
    assert review["tranche_006_status"] == "tracked_unresolved"
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_selects_tranche_005_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_accepts_exact_audit_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    target = review["lean_dependency_audit_target"]
    required = review["required_evidence_surface"]
    assert review["tranche_005_execution_packet_accepted"] is True
    assert review["tranche_005_lean_dependency_audit_scope_accepted"] is True
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_tracks_blockers_and_tranche_006() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_006_obligation_carry_forward"]["dependency_finding_id"] == (
        "V01-ALPHA-DEP-REM-006"
    )
    assert review["tranche_006_obligation_carry_forward"]["status_carry_forward"] == (
        "tracked_unmodified_not_audited_in_tranche_004"
    )


def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_authorizes_audit_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["execution_packet_result_review_classification"] == (
        "lean_dependency_audit_scope_accepted_audit_execution_authorized_only"
    )
    assert review["tranche_005_audit_execution_authorized"] is True
    ...
    assert review["audit_execution_authorized_scope"]["does_not_authorize_blocker_movement"] is True


def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == "tranche_005_audit_execution_only"
    assert review["next_action_scope"] == (
        "EXECUTE_TRANCHE_005_LEAN_AUDIT_ONLY_NO_REMEDIATION_CLOSURE_OR_RELEASE_PROMOTION"
    )
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "execute_v01_alpha_dependency_remediation_tranche_005_audit": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        REVIEW_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_005_execution_packet_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche005ExecutionPacketResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_005_execution_packet_result_review_does_not_execute_audit"
        in index_text
    )
