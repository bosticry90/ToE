...
INCREMENT30_ASSESS_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_ASSESSMENT_NOTE_v0.md"
)
DECISION_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_SEMANTIC_DELTA_DECISION_NOTE_v0.md"
)
PARENT_PACKET_PATH = (
    REPO_ROOT
    / "formal"
...
def test_sliceb_increment30_artifacts_exist() -> None:
    for p in (INCREMENT30_PACKET_PATH, INCREMENT30_ASSESS_PATH):
        assert p.exists(), f"Missing Slice B Increment30 artifact: {p}"


def test_sliceb_increment30_anchor_and_delta_parity() -> None:
    objective = _read(OBJECTIVE_PATH)
    parent = _read(PARENT_PACKET_PATH)
    decision = _read(DECISION_PATH)
    packet = _read(INCREMENT30_PACKET_PATH)
    assess = _read(INCREMENT30_ASSESS_PATH)
    ...
    assert "formal/docs/release/QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_SEMANTIC_DELTA_DECISION_NOTE_v0.md" in packet
    assert "fixed final admissibility input union" in packet
    assert "admissible minimal normal-form completion routes" in packet
    assert "prefix-invariance dependency" in packet


def test_sliceb_increment30_invariance_and_nonclaim_markers() -> None:
    packet = _read(INCREMENT30_PACKET_PATH)
    assess = _read(INCREMENT30_ASSESS_PATH)
...
def test_sliceb_increment30_status_tokens_present() -> None:
    packet = _read(INCREMENT30_PACKET_PATH)
    assess = _read(INCREMENT30_ASSESS_PATH)

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_STATUS_v0: EXECUTED_BOUNDED_v0" in packet
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_ASSESSMENT_STATUS_v0: ASSESSED_BOUNDED_v0" in assess
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_OBJECTIVE_ADVANCEMENT_v0: YES" in assess
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT30_PREFIX_INVARIANCE_DEPENDENCY_OVER_ADMISSIBLE_MINIMAL_NORMAL_FORM_COMPLETION_ROUTES_v0: ENFORCED"
    ) in assess


def test_sliceb_increment30_packet_assessment_semantic_delta_alignment() -> None:
    packet = _read(INCREMENT30_PACKET_PATH)
    assess = _read(INCREMENT30_ASSESS_PATH)

    assert "prefix-invariance dependency" in packet
    assert "prefix-invariance dependency" in assess
    assert "fixed final admissibility input union" in packet
...
def test_sliceb_increment30_forbidden_overclaim_and_drift_phrases_absent() -> None:
    packet = _read(INCREMENT30_PACKET_PATH)
    assess = _read(INCREMENT30_ASSESS_PATH)
