...
from formal.python.tools.psi_A_gauge_sector_exchange_theorem_linkage_obligation_closeout_report import (
    ...
    T_A_POLICY,
    WATCH_ITEMS,
    build_psi_A_gauge_sector_exchange_theorem_linkage_obligation_closeout,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "psi_A_gauge_sector_exchange_theorem_linkage_obligation_closeout_report.py"
)
...
def test_psi_A_gauge_exchange_closeout_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_psi_A_gauge_exchange_closeout_accepts_local_sourced_maxwell_linkage() -> None:
    ...
    assert closeout["likely_next_synthesis_target_after_review"] == (
        LIKELY_NEXT_SYNTHESIS_TARGET_AFTER_REVIEW
    )
...
def test_psi_A_gauge_exchange_closeout_records_claims_and_nonclaims() -> None:
    closeout = _json(DEFAULT_OUT)
    ...
    assert closeout["T_A_policy"] == T_A_POLICY
    assert closeout["field_strength_object"] == FIELD_STRENGTH_OBJECT
    assert closeout["current_object"] == CURRENT_OBJECT
    assert closeout["accepted_sourced_maxwell_route"] == ACCEPTED_SOURCED_MAXWELL_ROUTE
    assert closeout["accepted_gauge_stress_energy_divergence_identity"] == (
        ACCEPTED_GAUGE_STRESS_ENERGY_DIVERGENCE_IDENTITY
    )
    assert closeout["watch_items"] == WATCH_ITEMS
    assert closeout["route_statement"] == ROUTE_STATEMENT
    assert closeout["gauge_exchange_route_constructed"] is True
    assert closeout["gauge_exchange_derived"] is True
    assert closeout["gauge_exchange_linked_to_sourced_maxwell_route"] is True
    assert closeout["gauge_sector_exchange_obligation_locally_closed"] is True
    assert closeout["local_psi_A_gauge_sector_exchange_obligation_closed"] is True
    assert closeout["same_F_and_J_objects_preserved"] is True
    assert closeout["sourced_maxwell_route_used"] is True
    assert closeout["gauge_stress_energy_divergence_identity_used"] is True
...
def test_psi_A_gauge_exchange_closeout_records_lean_status() -> None:
    closeout = _json(DEFAULT_OUT)
...
def test_psi_A_gauge_exchange_closeout_rotates_to_result_review() -> None:
    ...
    assert consumed["gauge_sector_exchange_obligation_locally_closed"] == "yes"
    ...
    if is_current:
        ...
        assert active["gauge_sector_exchange_obligation_locally_closed"] == "yes"
        assert active["gauge_exchange_linked_to_sourced_maxwell_route"] == "yes"
        assert active["same_F_and_J_objects_preserved"] == "yes"
...
def test_psi_A_gauge_exchange_closeout_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_CLOSEOUT_RESULT,
        PACKET_CLASSIFICATION,
        "PsiAGaugeSectorExchangeTheoremLinkageObligationCloseout",
        CONSUMED_TARGET,
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        LIKELY_NEXT_SYNTHESIS_TARGET_AFTER_REVIEW,
        ...
        ROUTE_STATEMENT,
        LEAN_STATUS_WORDING_FOR_CLOSEOUT,
        "PSI_A_GAUGE_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_OUTCOME_v0",
...
def test_psi_A_gauge_exchange_closeout_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_psi_A_gauge_sector_exchange_theorem_linkage_obligation_closeout_gate.py"
    )
