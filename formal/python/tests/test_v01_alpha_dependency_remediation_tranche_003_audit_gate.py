...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_003_audit_report import (
    AUDIT_ID,
...
AUDIT_PATH = (
    RELEASE_DIR / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_AUDIT_20260515_v0.json"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_003_audit_report.py"
)
...
LEAN_AUDIT_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche003Audit.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_consumes_result_review() -> None:
    ...
    assert audit["consumes_tranche_003_execution_packet_result_review"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_EXECUTION_PACKET_RESULT_REVIEW_v0"
    )
    assert audit["consumes_tranche_003_execution_packet_result_review_pointer"] == (
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0.json"
    )


def test_v01_alpha_dependency_remediation_tranche_003_audit_scope_is_single_dependency() -> None:
    audit = _json(AUDIT_PATH)
    assert audit["audit_scope"] == (
        "EXECUTE_TRANCHE_003_LEAN_DEPENDENCY_AUDIT_ONLY_NO_REMEDIATION_OR_RELEASE_PROMOTION"
    )
    ...
    assert audit["tranche_002_global_release_readiness_still_not_marked"] is True
    ...
    assert audit["selected_obligation_status_after_audit"] == (
        "release_blocking_pending_tranche_003_audit_result_review"
    )
    assert audit["audit_status"] == "executed_evidence_captured"
    assert audit["evidence_surface_exists"] is True


def test_v01_alpha_dependency_remediation_tranche_003_audit_captures_exact_lean_evidence() -> None:
    audit = _json(AUDIT_PATH)
    ...
    assert evidence["raw_output"] == (
        ...
        "finite_transport_theorems_construct_residual_package_v0' depends on axioms: "
        "[propext,\n Classical.choice,\n Quot.sound]"
    )
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_classifies_project_axioms_separately() -> None:
    audit = _json(AUDIT_PATH)
    evidence = audit["lean_evidence"]
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_evidence_surfaces() -> None:
    audit = _json(AUDIT_PATH)
    surfaces = audit["evidence_surfaces_produced_or_updated"]
    assert {row["kind"]: row["status"] for row in surfaces} == {
        "tranche_003_audit_result_packet": "produced",
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_carries_forward_blockers() -> None:
    audit = _json(AUDIT_PATH)
    rows = audit["release_blocking_obligations_carry_forward"]
    assert audit["release_blocking_obligation_count"] == 4
    assert [row["dependency_finding_id"] for row in rows] == RELEASE_BLOCKER_IDS

    other = audit["other_release_blocking_obligations"]
    assert audit["other_release_blocking_obligation_count"] == 3
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_no_broader_remediation_or_movement() -> None:
    audit = _json(AUDIT_PATH)
    assert audit["tranche_003_audit_result_classification"] == (
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_forbidden_effects_false() -> None:
    audit = _json(AUDIT_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_next_target() -> None:
    audit = _json(AUDIT_PATH)
    assert audit["post_audit_adjudication_target"] == NEXT_TARGET
    assert audit["selected_next_target"] == NEXT_TARGET
    assert audit["selected_next_target_kind"] == "tranche_003_audit_result_review_only"
    assert audit["selection_count"] == 1
    assert audit["next_action_scope"] == (
        "REVIEW_TRANCHE_003_LEAN_AUDIT_EVIDENCE_ONLY_NO_REMEDIATION_CLOSURE_OR_RELEASE_PROMOTION"
    )
    assert {row["target"]: row["decision"] for row in audit["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_003_audit_result": "selected",
        "prepare_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet": "deferred",
        "prepare_v01_alpha_release_readiness_adjudication_packet": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_003_audit_acceptance_and_determinism() -> None:
    audit = _json(AUDIT_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_audit_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        AUDIT_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_AUDIT_20260515_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_003_audit_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_003_audit_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche003Audit" in index_text
    assert "v01_dependency_remediation_tranche_003_audit_does_not_move_blocker" in index_text
