...
from formal.python.tools.qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_WeakStrongConservationComparisonScopeAssumptionReductionAttemptResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_weak_strong_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_weak_strong_attempt_result_review_consumes_attempt() -> None:
...
def test_qft_gr_weak_strong_attempt_result_review_accepts_mr002() -> None:
    review = _json(DEFAULT_OUT)
    contract = review["weak_strong_conservation_comparison_scope_reduction_contract"]
    ...
    assert review["accepted_contract_id"] == WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_ID
    assert (
        review["bounded_weak_strong_conservation_comparison_scope_contract_status"]
        == BOUNDED_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_STATUS
    )
    assert contract["contract_id"] == WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_ID
    assert contract["assumption_id"] == "MR-ASSUMP-002-weak_strong_conservation_comparison_scope"
    assert contract["regularity_condition"] == "weak_strong_conservation_comparison_scope_for_future_conservation_proof_object"
    assert (
        review[
    ...
    assert review["weak_strong_conservation_comparison_scope_assumption_reduction_accepted"] is True
    assert review["weak_strong_conservation_comparison_scope_assumption_reduction_rejected"] is False
    assert (
        review[
...
def test_qft_gr_weak_strong_attempt_result_review_selects_mr003_from_inventory() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        ...
        "discharge_qft_gr_weak_strong_conservation_comparison_scope_assumption": "not_authorized",
...
def test_qft_gr_weak_strong_attempt_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["weak_strong_conservation_comparison_scope_assumption_discharged"] is False
    assert review["weak_strong_conservation_comparison_scope_assumption_discharged_by_review"] is False
    assert review["weak_strong_conservation_scope_globally_solved"] is False
    assert review["weak_strong_conservation_scope_globally_solved_by_review"] is False
...
def test_qft_gr_weak_strong_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review(
