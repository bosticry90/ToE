from __future__ import annotations

import hashlib
import json
from pathlib import Path

from formal.python.meta.repo_environment import find_repo_root


REPO_ROOT = find_repo_root(Path(__file__))
REPORT_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_recovery_non_stat_frontier_phase2_candidate_intake_20260424_v0.json"
)
PHASE1_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_recovery_non_stat_frontier_phase1_baseline_lock_20260424_v0.json"
)
EXHAUSTION_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_recovery_stat_post_der_cycle_exhaustion_readout_review_20260424_v0.json"
)
NON_REPLAY_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_recovery_stat_non_replay_frontier_reassessment_20260424_v0.json"
)
COSMO_SR_BASELINE_STOP_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_recovery_cosmo_sr_negative_baseline_frontier_stop_20260424_v0.json"
)
RL10_LIMITATION_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "qm_stat_rl10_discrete_transition_bridge_limitation_review_20260422_v2.json"
)
GR_TRANCHE_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_plan_gr_dormant_new_structure_completion_tranche_20260418_v0.json"
)
QUEUE_PACKET_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QUEUE_PRIORITY_NON_REPLAY_FRONTIER_RECONSTRUCTION_PACKET_20260423_v0.json"
)
QUEUE_SELECTION_PATH = (
    REPO_ROOT
    / "formal"
    / "docs"
    / "release"
    / "QUEUE_RECONSTRUCTION_DECLARED_NON_REPLAY_FRONTIER_SELECTION_20260423_v0.json"
)
PILLAR_REASSESSMENT_PATH = (
    REPO_ROOT
    / "formal"
    / "output"
    / "reports"
    / "post_recovery_pillar_frontier_reassessment_20260424_v0.json"
)


def _read_json(path: Path) -> dict:
    assert path.exists(), f"Missing required file: {path}"
    return json.loads(path.read_text(encoding="utf-8"))


def _canonical_hash(payload: dict) -> str:
    canonical = json.dumps(payload, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(canonical.encode("utf-8")).hexdigest()


def test_post_recovery_non_stat_frontier_phase2_candidate_intake_gate() -> None:
    report = _read_json(REPORT_PATH)
    phase1 = _read_json(PHASE1_PATH)
    exhaustion = _read_json(EXHAUSTION_PATH)
    non_replay = _read_json(NON_REPLAY_PATH)
    cosmo_sr_baseline_stop = _read_json(COSMO_SR_BASELINE_STOP_PATH)
    rl10_limitation = _read_json(RL10_LIMITATION_PATH)
    gr_tranche = _read_json(GR_TRANCHE_PATH)
    queue_packet = _read_json(QUEUE_PACKET_PATH)
    queue_selection = _read_json(QUEUE_SELECTION_PATH)
    pillar_reassessment = _read_json(PILLAR_REASSESSMENT_PATH)

    assert report["schema_id"] == "POST_RECOVERY_NON_STAT_FRONTIER_PHASE2_CANDIDATE_INTAKE_20260424_v0"
    assert report["artifact_id"] == "post_recovery_non_stat_frontier_phase2_candidate_intake_20260424_v0"
    assert report["status"] == "DECLARATION_ONLY_NON_STAT_FRONTIER_PHASE2_CANDIDATE_INTAKE_NONCLAIM"

    trigger = report["trigger"]
    assert trigger["source"] == "POST_RECOVERY_NON_STAT_FRONTIER_PHASE1_BASELINE_LOCK"
    assert trigger["source_report"] == (
        "formal/output/reports/post_recovery_non_stat_frontier_phase1_baseline_lock_20260424_v0.json"
    )

    boundary = report["frontier_boundary"]
    assert boundary["mode"] == "DECLARATION_ONLY_PHASE2_NON_STAT_CANDIDATE_INTAKE"
    assert boundary["execution_authorization"] == "NONE"
    assert boundary["records_readout_only"] is True
    assert boundary["candidate_intake_only"] is True
    assert boundary["review_counts_theorem_gap_delta"] is False
    assert boundary["packet05_bootstrap_allowed"] is False
    assert boundary["seam_execution_allowed"] is False
    assert boundary["gr_work_allowed"] is False
    assert boundary["rl10_work_allowed"] is False
    assert boundary["master_action_allowed"] is False
    assert boundary["promotion_or_closure_language_allowed"] is False

    carryforward = report["baseline_token_carryforward"]
    assert carryforward["phase1_entry_condition"] == "EXACT_MATCH_WITH_LOCKED_BASELINE_INVARIANTS"
    assert carryforward["phase1_token_set"] == phase1["phase_lock_contract"]["baseline_token_set"]
    assert carryforward["token_set_match_phase1"] is True
    assert carryforward["baseline_drift_detected"] is False

    # Phase 1 lock and post-recovery anchors remain unchanged.
    assert phase1["decision"]["phase1_baseline_lock_complete"] is True
    assert phase1["decision"]["execution_authorization"] == "NONE"
    assert exhaustion["readout_decision"]["stat_execution_held"] is True
    assert exhaustion["execution_hold_state"]["der_execution_authorization_available_now"] == "NONE"
    assert non_replay["decision"]["execution_held"] is True
    assert non_replay["decision"]["non_replay_execution_frontier_opened"] is False

    assert cosmo_sr_baseline_stop["cycle08_negative_baseline"]["terminal_outcome"] == "BLOCKER_UNCHANGED"
    assert (
        cosmo_sr_baseline_stop["alternative_hypothesis_review"][
            "machine_pinned_materially_different_cosmo_sr_candidate_found"
        ]
        is False
    )
    assert rl10_limitation["summary"]["review_outcome"] == "LIMITATION_INTERPRETATION_SCOPE_HOLD"
    assert rl10_limitation["summary"]["one_more_bounded_comparator_cycle_justified"] is False
    assert (
        gr_tranche["summary"]["terminal_outcome"]
        == "POST_PLAN_GR_DORMANT_NEW_STRUCTURE_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED"
    )

    assert queue_packet["reconstruction_policy"]["execution_authorization"] == "NONE"
    assert queue_selection["selection_policy"]["required_reconstruction_execution_authorization"] == "NONE"
    assert (
        pillar_reassessment["frontier_decision"]["terminal_outcome"]
        == "NO_PILLAR_SIDE_EXECUTION_PATH_AUTHORIZED"
    )

    intake_scope = report["candidate_intake_scope"]
    assert intake_scope["intake_pool_class"] == "NON_STAT_ONLY"
    assert intake_scope["bounded_fallback_cap"] == 2
    assert intake_scope["primary_lane_preference"] == "COSMO-SR"

    candidates = intake_scope["candidates"]
    assert len(candidates) == 3

    cosmo = candidates[0]
    assert cosmo["candidate_id"] == "SEAM-COSMO-SR::POST_RECOVERY_FRESH_HYPOTHESIS_DESIGN"
    assert cosmo["lane_id"] == "SEAM-COSMO-SR"
    assert cosmo["target_row_id"] == "ROW-SEAM-COSMO-SR-001"
    assert cosmo["intake_priority"] == 1
    assert cosmo["current_readiness"] == "NOT_READY_MATERIAL_DIFFERENCE_UNPINNED"

    rl10 = candidates[1]
    assert rl10["candidate_id"] == "ROW-PILLAR-QM-001::RL10_BRIDGE_SCOPE_REASSESSMENT"
    assert rl10["lane_id"] == "RL10"
    assert rl10["target_row_id"] == "ROW-PILLAR-QM-001"
    assert rl10["intake_priority"] == 2
    assert rl10["current_readiness"] == "HOLD_LIMITATION_SCOPE"

    gr = candidates[2]
    assert gr["candidate_id"] == "ROW-PILLAR-GR-001::DORMANT_NEW_STRUCTURE_NON_REPLAY_REASSESSMENT"
    assert gr["lane_id"] == "GR"
    assert gr["target_row_id"] == "ROW-PILLAR-GR-001"
    assert gr["intake_priority"] == 3
    assert gr["current_readiness"] == "EXHAUSTED_RETRY_PATH_NONAUTHORITATIVE"

    exclusions = report["candidate_exclusions"]
    assert exclusions["excluded_from_phase2_non_stat_pool"][0]["candidate_id"].startswith("ROW-PILLAR-STAT-001")
    assert "PACKET05_BOOTSTRAP" in exclusions["global_exclusions"]
    assert "SEAM_EXPANSION" in exclusions["global_exclusions"]
    assert "MASTER_ACTION" in exclusions["global_exclusions"]

    decision = report["intake_decision"]
    assert decision["terminal_outcome"] == "POST_RECOVERY_NON_STAT_FRONTIER_PHASE2_CANDIDATE_INTAKE_COMPLETE"
    assert decision["phase2_candidate_intake_complete"] is True
    assert decision["selected_primary_lane"] == "COSMO-SR"
    assert decision["fallback_lane_order"] == ["RL10", "GR"]
    assert decision["execution_authorization"] == "NONE"

    for disallowed in (
        "rerun_der01_attempt_without_new_authorization",
        "rerun_der02_attempt_without_new_authorization",
        "count_theorem_gap_delta_without_machine_pinned_negative_delta",
        "open_packet05",
        "open_seam_work",
        "open_gr_work",
        "open_rl10_work",
        "invoke_master_action",
        "claim_promotion_or_closure",
    ):
        assert disallowed in report["disallowed_next_actions"]

    validation = report["validation"]
    assert validation["source_consistency_check"] == "GREEN"
    assert "test_post_recovery_non_stat_frontier_phase2_candidate_intake_gate.py" in validation["targeted_gate_command"]

    assert report["payload_sha256"] == _canonical_hash({k: v for k, v in report.items() if k != "payload_sha256"})
