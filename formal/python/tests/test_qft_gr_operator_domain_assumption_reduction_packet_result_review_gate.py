...
from formal.python.tools.qft_gr_operator_domain_assumption_reduction_packet_result_review_report import (
    ...
    build_qft_gr_operator_domain_assumption_reduction_packet_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_OperatorDomainAssumptionReductionPacketResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_operator_domain_assumption_reduction_packet_result_review_report.py"
)
...
def test_qft_gr_operator_domain_assumption_reduction_packet_result_review_files_exist() -> None:
...
def test_qft_gr_operator_domain_assumption_reduction_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_qft_gr_operator_domain_assumption_reduction_packet"] == PACKET_ID
...
def test_qft_gr_operator_domain_assumption_reduction_packet_result_review_confirms_rows() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["operator_domain_assumption_rows_confirmed"] is True
    assert review["operator_domain_assumption_row_count"] == 6
    assert review["row_status_enum"] == ROW_STATUS_ENUM
    assert (
        review["selected_operator_domain_assumption_row"]
        == SELECTED_OPERATOR_DOMAIN_ASSUMPTION_ROW
    )
    assert review["selected_operator_domain_assumption_row_status"] == [
        "required",
        "supplied",
        "missing",
        "candidate_reducible",
...
def test_qft_gr_operator_domain_assumption_reduction_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_operator_domain_assumption_reduction_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_operator_domain_assumption_reduction_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_operator_domain_assumption_reduction_packet_result_review(
