...
def test_qft_cycle8_hardening_artifacts_exist() -> None:
...
def test_qft_cycle8_tokens_are_pinned_in_qft_docs() -> None:
    evol_text = _read(QFT_EVOL_TARGET_PATH)
    discharge_text = _read(QFT_FULL_DISCHARGE_TARGET_PATH)

    for token in [CYCLE8_MILESTONE_TOKEN, CYCLE8_GATE_PATH]:
        assert token in evol_text, f"QFT evolution umbrella target missing cycle-8 token `{token}`."
        assert token in discharge_text, f"QFT full-derivation discharge target missing cycle-8 token `{token}`."

    assert CYCLE8_PROGRESS_TOKEN in discharge_text, "QFT full-derivation discharge target missing cycle-8 progress token."


def test_qft_cycle8_gate_is_pinned_in_authority_surfaces() -> None:
    roadmap_text = _read(ROADMAP_PATH)
    state_text = _read(STATE_PATH)
    inventory_text = _read(INVENTORY_PATH)

    assert CYCLE8_GATE_PATH in roadmap_text, f"Roadmap authority surface must pin `{CYCLE8_GATE_PATH}`."
    assert CYCLE8_GATE_PATH in state_text or CYCLE8_GATE_PATH in inventory_text, (
        f"State/Inventory authority surface must pin `{CYCLE8_GATE_PATH}`."
    )
    assert CYCLE8_MILESTONE_TOKEN in state_text or CYCLE8_MILESTONE_TOKEN in inventory_text, (
        "State/Inventory authority surface missing cycle-8 milestone token."
    )
    assert CYCLE8_PROGRESS_TOKEN in state_text or CYCLE8_PROGRESS_TOKEN in inventory_text, (
        "State/Inventory authority surface missing cycle-8 progress token."
    )


def test_qft_cycle8_alignment_theorem_token_is_present() -> None:
    text = _read(QFT_EVOL_OBJECT_SCAFFOLD_LEAN_PATH)
    assert "theorem qft_evol_generator_unitarity_route_normalization_coherence_alignment_v0" in text, (
        "QFT evolution Lean scaffold missing cycle-8 normalization/coherence alignment theorem token."
    )


def test_qft_cycle8_alignment_theorem_reuses_cycle7_and_cycle6_routes() -> None:
    text = _read(QFT_EVOL_OBJECT_SCAFFOLD_LEAN_PATH)
    ...
    for token in [
        ...
        assert token in theorem_block, f"Cycle-8 alignment theorem missing expected token `{token}`."

    assert "trivial" not in theorem_block, (
        "Cycle-8 alignment theorem must not be implemented as a trivial placeholder proof."
    )
