...
def test_qft_cycle8_gate_is_pinned_in_authority_surfaces() -> None:
    ...
    assert CYCLE8_GATE_PATH in state_text, f"State authority surface must pin `{CYCLE8_GATE_PATH}`."
    assert CYCLE8_MILESTONE_TOKEN in state_text, "State authority surface missing cycle-8 milestone token."
    assert CYCLE8_PROGRESS_TOKEN in state_text, "State authority surface missing cycle-8 progress token."
