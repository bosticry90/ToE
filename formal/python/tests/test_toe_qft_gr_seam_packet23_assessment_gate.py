...
DOC = ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET23_ASSESSMENT_v0.md"
CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet23_assessment_checkpoint_v0.json"
)
PARENT_EXEC_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET23_BOUNDED_EXECUTION_v0.md"
)
PARENT_EXEC_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet23_bounded_execution_checkpoint_v0.json"
)
PARENT_AUTH_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET23_AUTHORIZATION_v0.md"
)
PARENT_AUTH_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet23_authorization_checkpoint_v0.json"
)
...
def test_packet23_assessment_files_exist() -> None:
    assert DOC.exists(), f"Missing packet23 assessment doc: {DOC}"
    assert CHECKPOINT.exists(), f"Missing packet23 assessment checkpoint: {CHECKPOINT}"


def test_packet23_assessment_doc_required_markers() -> None:
    text = _read_text(DOC)
    required_markers = [
        ...
        "PACKET23_ASSESSMENT_COMPLETE_v0",
...
def test_packet23_assessment_checkpoint_schema_and_lineage() -> None:
    data = _load_json(CHECKPOINT)
    assert data["artifact_id"] == "toe_qft_gr_seam_packet23_assessment_checkpoint_v0"
    assert data["phase"] == "PHASE_2B_QFT_GR_SEAM_PACKET23_ASSESSMENT"
    assert data["scope"] == "packet23_target_satisfaction_assessment"
    assert data["status"] == "PACKET23_ASSESSMENT_COMPLETE_v0"

    payload = data["payload"]
    assert payload["assessment_doc_path"].endswith(
        "TOE_QFT_GR_SEAM_PACKET23_ASSESSMENT_v0.md"
    )
...
def test_packet23_assessment_target_satisfaction_and_packet24_rule() -> None:
    ...
    packet24 = payload["packet24_decision"]
    assert packet24["authorized"] is True
    assert packet24["progress_verdict"] == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet24["ladder_extension_verdict"] == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet24["verdict"] == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet24["exact_bounded_target"] == EXPECTED_PACKET24_TARGET
    assert packet24["hold_refine_trigger_status"] == "NOT_HIT_v0"


def test_packet23_assessment_scalar_and_guardrail_invariants() -> None:
    payload = _load_json(CHECKPOINT)["payload"]
    ...
    assert guardrails["assessment_scope"] == "ASSESS_PACKET23_BEFORE_PACKET24_v0"


def test_packet23_assessment_parent_chain_exists() -> None:
