...
def test_ov01_multi_fit_stability_lock_and_degenerate_g0_case():
    ...
    rep0 = ov01_multi_fit_stability(fn0, [dr02, dr03, dr04], epsilon=1e-12, tau_obs=0.10)
    rep = ov01_multi_fit_stability(fn, [dr02, dr03, dr04], epsilon=1e-12, tau_obs=0.10)

    # Fingerprint propagation
    assert rep.fn_fingerprint == fn.fingerprint()
    assert rep.dr_fingerprints == (dr02.fingerprint(), dr03.fingerprint(), dr04.fingerprint())

    # Determinism of report fingerprint
    assert rep.fingerprint() == ov01_multi_fit_stability(fn, [dr02, dr03, dr04], epsilon=1e-12, tau_obs=0.10).fingerprint()

    # Degenerate g=0 case should yield obs=0 for all and thus all residuals 0.
    assert rep0.obs_values == (0.0, 0.0, 0.0)
    assert rep0.r_max == 0.0
    assert rep0.r_rms == 0.0
    assert all(r == 0.0 for (_, _, r) in rep0.pairwise_r)

    # Non-degeneracy: with g>0 and distinct DR fits, residuals should be positive.
    if rep.dr_fingerprints[0] != rep.dr_fingerprints[2] and rep.obs_values[0] != 0.0:
        assert rep.r_max > 0.0

    # Lock expected summary statistics for this specific DR02/DR03/DR04 trio.
    # (This prevents silent weakening of the discriminator.)
    assert abs(rep.r_max - 0.29003318302945985) < 1e-15
    assert abs(rep.r_rms - 0.21906491457608293) < 1e-15
