...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_result_review_report.py"
)
...
def test_phi_transport_standalone_attempt_result_review_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_phi_transport_standalone_attempt_result_review_accepts_preparation() -> None:
...
def test_phi_transport_standalone_attempt_result_review_preserves_component_route() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["componentwise_zero_target_prepared"] is True
...
def test_phi_transport_standalone_attempt_result_review_preserves_boundaries() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["action_to_regime_transport_match_target_prepared"] is True
    assert review["action_to_regime_transport_match_promoted_to_master_action"] is False
...
def test_phi_transport_standalone_attempt_result_review_rotates_to_execution() -> None:
...
def test_phi_transport_standalone_attempt_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "PhiTransportTheoremLinkageAttemptFromStandalonePhiTransportRouteResultReview",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        SUGGESTED_EXECUTION_OUTCOME,
        ...
        *[row["component_form"] for row in TRANSPORT_COMPONENTS],
        ...
        TARGET_CONCLUSION,
        *LEAN_STATUS_WORDING_LINES_FOR_PACKET,
        "PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_TRANSPORT_ROUTE_RESULT_REVIEW_OUTCOME_v0",
...
def test_phi_transport_standalone_attempt_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_result_review_gate.py"
    )
