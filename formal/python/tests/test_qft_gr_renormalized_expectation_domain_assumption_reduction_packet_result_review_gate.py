...
def test_qft_gr_renormalized_expectation_domain_packet_result_review_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_domain_packet_result_review_consumes_packet() -> None:
...
def test_qft_gr_renormalized_expectation_domain_packet_result_review_confirms_row003() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["accepted_prior_row_count"] == 2
    assert review["selected_renormalization_assumption_row"] == SELECTED_ROW_ID
    assert review["selected_row_count"] == 1
    assert review["renormalized_expectation_domain_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert review["renormalized_expectation_domain"] == RENORMALIZED_EXPECTATION_DOMAIN
    assert review["renormalized_expectation_domain_object"] == (
        RENORMALIZED_EXPECTATION_DOMAIN
    )
    assert review["renormalized_expectation_domain_status"] == (
        RENORMALIZED_EXPECTATION_DOMAIN_STATUS
    )
    assert review["domain_boundary"] == DOMAIN_BOUNDARY
...
def test_qft_gr_renormalized_expectation_domain_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["renormalized_expectation_domain_assumption_discharged"] is False
    assert review["renormalized_expectation_domain_assumption_reduced_by_review"] is False
    assert (
        review[
...
def test_qft_gr_renormalized_expectation_domain_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_renormalized_expectation_domain_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_domain_assumption_reduction_packet_result_review(
