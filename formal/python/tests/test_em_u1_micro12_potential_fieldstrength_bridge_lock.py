...
LOCALIZATION_TOKEN = "EM_U1_AF_BRIDGE_LOCALIZATION_GATE_v0: CYCLE12_ARTIFACTS_ONLY"
PREREQUISITE_TOKENS = [
...
AF_BRIDGE_PATTERNS = {
    "forms_bridge": re.escape("F := dA"),
    "tensor_bridge": re.escape("F_{μν} := ∂_μ A_ν − ∂_ν A_μ"),
}
...
def test_em_micro12_contains_required_tokens_and_bridge_definitions() -> None:
    text = _read(EM_MICRO12_PATH)
    required_tokens = [
    ...
    assert not missing, "EM Cycle-012 micro document is missing required token(s): " + ", ".join(missing)

    missing_patterns = [label for label, pattern in AF_BRIDGE_PATTERNS.items() if re.search(pattern, text) is None]
    assert not missing_patterns, (
        "EM Cycle-012 micro document is missing required A↔F bridge definition(s): "
        + ", ".join(missing_patterns)
    )


def test_em_target_references_cycle012_artifact_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM target document is missing required Cycle-012 token(s): " + ", ".join(missing)


def test_em_roadmap_references_cycle012_target_and_artifact_with_single_row() -> None:
    ...
    assert "TARGET-EM-U1-MICRO-12-POTENTIAL-FIELDSTRENGTH-BRIDGE-LOCK-v0" in targets
    assert (
        "formal/docs/paper/DERIVATION_TARGET_EM_U1_MICRO_12_POTENTIAL_FIELDSTRENGTH_BRIDGE_LOCK_v0.md"
        in artifacts
    )
...
def test_state_mentions_cycle012_checkpoint_and_tokens() -> None:
    text = _read(STATE_PATH)
    required_tokens = [
        "EM Cycle-012 potential/field-strength bridge-lock checkpoint",
        "TARGET-EM-U1-MICRO-12-POTENTIAL-FIELDSTRENGTH-BRIDGE-LOCK-v0",
        "EM_U1_PROGRESS_CYCLE12_v0: POTENTIAL_FIELDSTRENGTH_BRIDGE_TOKEN_PINNED",
        "EM_U1_AF_BRIDGE_FORMS_v0: F_EQUALS_DA_SEAM_PINNED",
        "EM_U1_AF_BRIDGE_TENSOR_v0: F_MUNU_FROM_A_SEAM_PINNED",
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "State_of_the_Theory.md is missing required Cycle-012 token(s): " + ", ".join(missing)


def test_em_lean_cycle012_tokens_and_bridge_stubs_are_pinned() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM U1 Lean scaffold is missing required Cycle-012 token(s): " + ", ".join(missing)


def test_cycle012_bridge_definitions_are_localized_to_cycle12_artifact() -> None:
    allowed_paths = [EM_MICRO12_PATH]
    scoped_paths = [
        EM_TARGET_PATH,
        EM_MICRO01_PATH,
        EM_MICRO02_PATH,
    ...
    for path in allowed_paths:
        text = _read(path)
        hits = [label for label, pattern in AF_BRIDGE_PATTERNS.items() if re.search(pattern, text)]
        if not hits:
            violations.append(f"cycle012 artifact {path.relative_to(REPO_ROOT)} is missing localized bridge definitions")
            continue
        ...
        if not (has_assumption_id and has_localization_gate and has_prereq and has_non_selection):
            violations.append(
                f"cycle012 artifact {path.relative_to(REPO_ROOT)} contains bridge definitions {hits} "
                "without source assumption ID + localization token + prerequisites + non-selection tokens."
            )

    for path in scoped_paths:
        text = _read(path)
        for label, pattern in AF_BRIDGE_PATTERNS.items():
            ...
            if not (has_assumption_id and has_localization_gate and has_prereq and has_non_selection):
                violations.append(
                    f"{label} bridge definition leaked into non-authorized artifact {path.relative_to(REPO_ROOT)} "
                    "without source assumption ID + localization token + prerequisites + non-selection tokens."
                )

    assert not violations, "Cycle-012 A↔F bridge localization violation:\n- " + "\n- ".join(violations)


def test_cycle012_is_definition_only_and_blocks_derivation_or_new_physics_selection_language() -> None:
    text = _read(EM_MICRO12_PATH)
    forbidden_patterns = [
        r"\bwe derive\b",
        r"\bwe prove\b",
    ...
    assert not violations, (
        "Cycle-012 must remain definition-only/non-derivational with no import-lane selections. "
