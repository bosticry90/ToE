...
def test_synthesis_0115_exists_and_is_anchored() -> None:
    synthesis = _read(SYNTHESIS_0115_PATH)
    synthesis_0114 = _read(SYNTHESIS_0114_PATH)
    objective = _read(OBJECTIVE_PATH)
    increment15 = _read(INCREMENT15_PACKET_PATH)

    assert REQUIRED_QUESTION in objective
    assert REQUIRED_QUESTION in synthesis_0114
    assert REQUIRED_QUESTION in increment15
    ...
    assert "formal/docs/release/QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_14_SYNTHESIS_NOTE_v0.md" in synthesis


def test_synthesis_0115_contains_required_sections() -> None:
    synthesis = _read(SYNTHESIS_0115_PATH)
...
def test_synthesis_0115_tokens_present() -> None:
    synthesis = _read(SYNTHESIS_0115_PATH)

    assert REQUIRED_HOLD_TOKEN in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT16_DECISION_RULE_v0: "
        "REQUIRE_NEW_INCOMPATIBILITY_OR_DEPENDENCY_CRITERION_BEYOND_ORIGIN_PROVENANCE_EPOCH_BRANCH_IRREVERSIBILITY_FALLBACK_COMPLETENESS_WITNESS_CONSISTENCY_MINIMALITY_UNIQUENESS_REEVALUATION_STABILITY_STRENGTHENING_MONOTONICITY_STACK"
    ) in synthesis
    assert (
...
def test_synthesis_0115_nonclaim_markers_present() -> None:
    synthesis = _read(SYNTHESIS_0115_PATH)
