...
def test_qft_cycle12_gate_is_pinned_in_authority_surfaces() -> None:
    ...
    assert CYCLE12_GATE_PATH in state_text, f"State authority surface must pin `{CYCLE12_GATE_PATH}`."
    assert CYCLE12_MILESTONE_TOKEN in state_text, "State authority surface missing cycle-12 milestone token."
    assert CYCLE12_PROGRESS_TOKEN in state_text, "State authority surface missing cycle-12 progress token."
