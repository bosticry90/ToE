...
def test_ov_sel_snd05_lock_matches_computed_record() -> None:
    rec = ovsel_snd05_multi_density_constancy_audit_record(status_date="2026-01-24")
    ...
    lock_path = (
        ...
        / "locks"
        / "observables"
        / "OV-SEL-SND-05_multi_density_constancy_audit.md"
    )
    ...
    assert "OV-SND-04" in locked["checks"]
    assert "OV-BR-SND-02" in locked["checks"]
    assert "OV-SND-03N-COV" in locked["checks"]
