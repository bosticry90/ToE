...
ASSESSMENT_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet35_assessment_checkpoint_v0.json"
...
def test_qft_gr_seam_packet36_authorization_document_structure() -> None:
    ...
    required_markers = [
        ...
        "packet36_physics_quantity_tightened:",
        "packet36_discriminator_strengthening_requirement:",
        "packet36_ambiguity_reduction_requirement:",
        "packet36_non_repetition_clause:",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet36 authorization doc missing marker: {marker}"


def test_qft_gr_seam_packet36_authorization_checkpoint_schema_and_preconditions() -> None:
    artifact = _read_json(AUTH_CHECKPOINT_PATH)
    assessment_artifact = _read_json(ASSESSMENT_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet36_authorization_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1AL_QFT_GR_SEAM_PACKET36_AUTHORIZATION"
    assert artifact.get("status") == "PACKET36_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET36_AUTHORIZATION_v0.md"
    assert payload.get("parent_assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET35_ASSESSMENT_v0.md"
    ...
    assessment_decision = assessment_artifact.get("payload", {}).get("packet36_decision", {})
    ...
    pre = payload.get("preconditions_from_packet35_assessment", {})
    ...
    assert decision.get("decision_outcome") == "AUTHORIZE_PACKET36_BOUNDED_TARGET_v0"
    assert decision.get("status") == "AUTHORIZED_WITH_SINGLE_BOUNDED_TARGET_v0"

    bounded = payload.get("packet36_bounded_target", {})
    ...
    assert bounded.get("discriminator_strengthening_requirement") == (
        "criterion_level_post_persistence_closure_continuity_discrimination_beyond_packet35_closure_persistence_witness_qualification_required"
    )
    assert bounded.get("ambiguity_reduction_requirement") == (
        "packet36_valid_only_if_residual_interface_ambiguity_reduced_via_new_closure_continuity_discriminative_content"
    )
    assert bounded.get("non_repetition_clause") == (
        "packet36_invalid_if_only_packet35_closure_persistence_witness_reencoded_without_new_closure_continuity_discriminative_content"
    )


def test_qft_gr_seam_packet36_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet36 authorization pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet36 authorization pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_auth = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET36_AUTHORIZATION_STATUS_v0")
    roadmap_auth = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET36_AUTHORIZATION_STATUS_v0")
