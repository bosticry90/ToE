...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_CovariantConservationStatementWitnessAttemptResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_covariant_conservation_statement_witness_attempt_result_review_report.py"
)
...
def test_qft_gr_covariant_conservation_statement_witness_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_covariant_conservation_statement_witness_attempt_result_review_consumes_obstruction_attempt() -> None:
...
def test_qft_gr_covariant_conservation_statement_witness_attempt_result_review_accepts_obstruction_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["attempt_result_reviewed"] is True
    assert review["covariant_conservation_obstruction_result_accepted"] is True
    ...
    assert review["refinement_packet_preparation_authorized"] is True
    assert review["refinement_candidates"] == REFINEMENT_CANDIDATES


def test_qft_gr_covariant_conservation_statement_witness_attempt_result_review_preserves_nonclaim_boundaries() -> None:
    review = _json(DEFAULT_OUT)
    assert review["covariant_conservation_statement_witness_constructed"] is False
...
def test_qft_gr_covariant_conservation_statement_witness_attempt_result_review_selects_one_refinement_packet_target() -> None:
    review = _json(DEFAULT_OUT)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_covariant_conservation_statement_refinement_attempt": "deferred",
...
def test_qft_gr_covariant_conservation_statement_witness_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_covariant_conservation_statement_witness_attempt_result_review(
