...
PACKET_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET22_BOUNDED_EXECUTION_v0.md"
PACKET_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet22_bounded_execution_checkpoint_v0.json"
AUTH_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET22_AUTHORIZATION_v0.md"
AUTH_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet22_authorization_checkpoint_v0.json"
...
def test_qft_gr_seam_packet22_bounded_execution_document_structure() -> None:
    ...
    required_markers = [
        ...
        "packet22 is genuine physics progress rather than another ladder rung",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet22 execution doc missing marker: {marker}"


def test_qft_gr_seam_packet22_bounded_execution_checkpoint_schema_and_alignment() -> None:
    artifact = _read_json(PACKET_CHECKPOINT_PATH)
    auth_artifact = _read_json(AUTH_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet22_bounded_execution_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2A_QFT_GR_SEAM_PACKET22_BOUNDED_EXECUTION"
    assert artifact.get("status") == "PACKET22_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"

    payload = artifact.get("payload", {})
    assert payload.get("packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET22_BOUNDED_EXECUTION_v0.md"
    assert payload.get("parent_authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET22_AUTHORIZATION_v0.md"
    ...
    assert auth_payload.get("authorization_decision", {}).get("decision_outcome") == "AUTHORIZE_PACKET22_BOUNDED_TARGET_v0"

    binding = payload.get("authorized_target_binding", {})
    assert binding.get("authorization_decision_outcome") == "AUTHORIZE_PACKET22_BOUNDED_TARGET_v0"
    assert binding.get("authorized_exact_target") == (
        "freeze_one_bounded_handoff_post_finalization_closure_stability_discriminator_that_maps_packet21_closure_finalization_witness_to_a_single_non_scalar_expanding_closure_stability_witness"
    )
    assert binding.get("authorized_exact_target") == auth_payload.get("packet22_bounded_target", {}).get("exact_target")
    assert binding.get("execution_target_match") == "EXACT_MATCH_CONFIRMED_v0"

    discr = payload.get("post_finalization_closure_stability_discriminator", {})
    ...
    assert non_repetition.get("result") == "PACKET22_ADDS_NEW_POST_FINALIZATION_CLOSURE_STABILITY_DISCRIMINATOR_v0"

    ac = payload.get("acceptance_criteria", {})
    assert ac.get("ac1_input_state_binding_to_packet21_closure_finalization_witness") == "PASS_v0"
...
def test_qft_gr_seam_packet22_bounded_execution_authority_parity_and_invariance() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet22 execution pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet22 execution pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_packet22 = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET22_STATUS_v0")
    roadmap_packet22 = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET22_STATUS_v0")
    assert state_packet22 == roadmap_packet22 == "EXECUTED_BOUNDED_TARGET_STEP_v0"
