...
def test_qft_gr_seam_packet22_authorization_document_structure() -> None:
    ...
    required_markers = [
        ...
        "packet22_physics_quantity_tightened:",
        "packet22_discriminator_strengthening_requirement:",
        "packet22_ambiguity_reduction_requirement:",
        "packet22_non_repetition_clause:",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet22 authorization doc missing marker: {marker}"


def test_qft_gr_seam_packet22_authorization_checkpoint_schema_and_preconditions() -> None:
    artifact = _read_json(AUTH_CHECKPOINT_PATH)
    assessment_artifact = _read_json(ASSESSMENT_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet22_authorization_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1AG_QFT_GR_SEAM_PACKET22_AUTHORIZATION"
    assert artifact.get("status") == "PACKET22_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET22_AUTHORIZATION_v0.md"
    assert payload.get("parent_assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET21_ASSESSMENT_v0.md"
    ...
    assessment_decision = assessment_artifact.get("payload", {}).get("packet22_decision", {})
    ...
    pre = payload.get("preconditions_from_packet21_assessment", {})
    ...
    assert decision.get("decision_outcome") == "AUTHORIZE_PACKET22_BOUNDED_TARGET_v0"
    assert decision.get("status") == "AUTHORIZED_WITH_SINGLE_BOUNDED_TARGET_v0"

    bounded = payload.get("packet22_bounded_target", {})
    ...
    assert bounded.get("discriminator_strengthening_requirement") == (
        "criterion_level_post_finalization_closure_stability_discrimination_beyond_packet21_closure_finalization_witness_qualification_required"
    )
    assert bounded.get("ambiguity_reduction_requirement") == (
        "packet22_valid_only_if_residual_interface_ambiguity_reduced_via_new_closure_stability_discriminative_content"
    )
    assert bounded.get("non_repetition_clause") == (
        "packet22_invalid_if_only_packet21_closure_finalization_witness_reencoded_without_new_closure_stability_discriminative_content"
    )


def test_qft_gr_seam_packet22_authorization_chain_consistency_and_authority_parity() -> None:
