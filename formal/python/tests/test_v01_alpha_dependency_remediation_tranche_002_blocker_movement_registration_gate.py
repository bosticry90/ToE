...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_report.py"
)
...
LEAN_REGISTRATION_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche002BlockerMovementRegistration.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_consumes_result_review() -> None:
    registration = _json(REGISTRATION_PATH)
    assert registration["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_002_BLOCKER_MOVEMENT_REGISTRATION_20260515_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_selected_dependency() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_exact_movement() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_preserves_evidence_policy_documentation_chain() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_registers_only_pending_review() -> None:
    registration = _json(REGISTRATION_PATH)
    movement = registration["registered_movement"]
    ...
    assert registration["tranche_002_formal_movement_accepted"] is False
    assert registration["tranche_002_cleared_for_global_release_readiness"] is False
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_remaining_blockers_tracked() -> None:
    registration = _json(REGISTRATION_PATH)
    rows = registration["release_blocking_obligations_carry_forward"]
    assert registration["release_blocking_obligation_count"] == 5
    ...
    assert registration["other_release_blocking_obligation_count"] == 4
    assert (
        registration["remaining_release_blocking_obligation_count_excluding_tranche_002_candidate"]
        == 4
    )
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_forbidden_effects_false() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_next_target() -> None:
    registration = _json(REGISTRATION_PATH)
    ...
    assert {row["target"]: row["decision"] for row in registration["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_result": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_acceptance_and_determinism() -> None:
    registration = _json(REGISTRATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche002BlockerMovementRegistration" in index_text
    assert (
        "v01_dependency_remediation_tranche_002_blocker_movement_registration_does_not_promote_release"
        in index_text
    )
