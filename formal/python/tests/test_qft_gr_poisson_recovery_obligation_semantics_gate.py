...
SOURCE_REPORT_PATH = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_WEAK_CURVATURE_SOURCE_IDENTIFICATION_OBLIGATION_SEMANTICS_RESULT_REVIEW_20260503_v0.json"
)
...
SURFACE_ID = "QFT_GR_POISSON_RECOVERY_OBLIGATION_SEMANTICS_v0"
PREVIOUS_TARGET = "prepare_qft_gr_poisson_recovery_obligation_semantics_bounded_attack"
SOURCE_REVIEW_TARGET = (
    "review_qft_gr_weak_curvature_source_identification_obligation_semantics_result"
)
RESULT_REVIEW_TARGET = "review_qft_gr_poisson_recovery_obligation_semantics_result"
RESULT_TOKEN = "QFT_GR_POISSON_RECOVERY_OBLIGATION_SEMANTICS_SUPPLIED_ONLY"
REPORT_ID = "QFT_GR_POISSON_RECOVERY_OBLIGATION_SEMANTICS_BOUNDED_ATTACK_20260503_v0"
RETAINED_BLOCKER = "PHASE1-BLOCKER-QFTGR-POISSON-RECOVERY-WITNESS-RETAINED"
...
ACTIVE_LANE = "qft_gr_poisson_recovery_obligation_semantics_result_review"
PREPARATION_LANE = "qft_gr_poisson_recovery_obligation_semantics_preparation"
...
def test_poisson_recovery_obligation_surface_records_supplied_obligation() -> None:
    ...
    for token in {
        ...
        "QFTGRPoissonRecoveryObligationSemanticData",
        "hasPoissonRecoveryObligation",
        "poissonRecoverySatisfied",
        "supplied_poisson_recovery_obligation_semantics_constructs_package_v0",
        "qft_gr_weak_curvature_source_identification_obligation_semantics_does_not_force_poisson_recovery_witness_v0",
        "qft_gr_poisson_recovery_obligation_semantics_consumes_live_target_v0",
        ...
        "qft_gr_poisson_recovery_obligation_semantics_result_token_v0",
        "qft_gr_poisson_recovery_obligation_semantics_selected_next_target_v0",
    }:
        assert token in text


def test_poisson_recovery_obligation_surface_preserves_boundaries() -> None:
    text = _read(SURFACE_PATH)
...
def test_poisson_recovery_obligation_report_records_nonclaim_result() -> None:
    ...
    assert report["supplied_route"]["distinction"] == (
        "has_poisson_recovery_obligation_does_not_imply_has_poisson_recovery_witness_or_satisfaction"
    )
...
def test_registry_rotates_to_poisson_recovery_obligation_result_review() -> None:
    assert_current_target_consistent()
    assert_frontier_matches_registry()
    assert_forbidden_promotions_closed()
    assert_public_surfaces_match_registry()
    ...
    assert prep["poisson_recovery_obligation_semantics_surface"] == SURFACE_EVIDENCE
    assert prep["poisson_recovery_obligation_semantics_report"] == REPORT_EVIDENCE
    assert prep["poisson_recovery_obligation_semantics_result_token"] == RESULT_TOKEN
    assert prep["supplied_route_available"] == "yes"
    assert prep["weak_curvature_obligation_only_poisson_recovery_witness_refuted"] == (
        "yes"
    )
    assert prep["poisson_recovery_witness_derived_from_weak_curvature_obligation_alone"] == (
        "no"
    )
    assert prep["result_review_status"] == "prepared_for_live_result_review"
    assert prep["poisson_recovery_witness_authorized"] == "no"
    assert prep["actual_poisson_recovery_authorized"] == "no"
    assert prep["newtonian_limit_recovery_authorized"] == "no"
    assert prep["weak_field_recovery_proof_authorized"] == "no"
    ...
    assert active["poisson_recovery_obligation_semantics_surface"] == SURFACE_EVIDENCE
    assert active["poisson_recovery_obligation_semantics_result_token"] == RESULT_TOKEN
    assert active["poisson_recovery_witness_authorized"] == "no"
    assert active["actual_poisson_recovery_authorized"] == "no"
    assert active["newtonian_limit_recovery_authorized"] == "no"
    assert active["weak_field_recovery_proof_authorized"] == "no"
    ...
    assert qft_gr["poisson_recovery_obligation_semantics_surface"] == SURFACE_EVIDENCE
    assert qft_gr["poisson_recovery_obligation_semantics_result_token"] == RESULT_TOKEN
    assert qft_gr["poisson_recovery_weak_curvature_obligation_only_refuted"] == "yes"
    assert qft_gr["poisson_recovery_witness_derived_from_weak_curvature_obligation_alone"] == (
        "no"
    )
...
def test_public_surfaces_track_poisson_recovery_obligation() -> None:
    for path in [README_PATH, STATE_PATH, ROADMAP_PATH, STRICT_MAP_PATH]:
        ...
        assert "QFT_GR_PoissonRecoveryObligationSemantics.lean" in text
    ...
    assert "INV-MATH-QFTGR-POISSON-RECOVERY-OBLIGATION-v0" in inventory_text
    ...
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_poisson_recovery_obligation_semantics_gate.py"
    )
