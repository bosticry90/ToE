...
from formal.python.tools import science_phase_i_untouched_lane_signal_refinement_packet_report as tool
...
def _write_declaration(
    ...
    *,
    ...
    signal_refinement_valid_without_movement: bool = True,
    ...
    _write_json(
        path,
        {
            ...
            "packet_contract": {
                "single_terminal_outcome_rule": "EXACTLY_ONE_ALLOWED_SCIENCE_PHASE_I_UNTOUCHED_LANE_SIGNAL_REFINEMENT_PACKET_OUTCOME",
                "no_loop_rule": "ONE_SCIENCE_PHASE_I_UNTOUCHED_LANE_SIGNAL_REFINEMENT_PACKET_LAYER_ONLY",
                "allowed_outcomes": [
                    "UNTOUCHED_LANE_SIGNAL_PRODUCED",
                    "UNTOUCHED_LANE_VALID_BUT_NONMOVING",
                    "UNTOUCHED_LANE_REFINEMENT_REQUIRES_UNDECLARED_STRUCTURE",
...
def _seed_inputs(
    root: Path,
    *,
    phase_h_outcome: str = "UNTOUCHED_LANE_SIGNAL_REFINEMENT_JUSTIFIED",
) -> None:
    _write_json(
    ...
    _write_json(
        ...
        {
            "summary": {
                "terminal_outcome": phase_h_outcome,
                "selected_attack_class": "neutrino_interface_alignment_boundary_probe",
            }
        },
    )
...
def test_reports_requires_undeclared_structure(tmp_path: Path, monkeypatch) -> None:
    ...
    report = tool.build_report(declaration_path=declaration_path, captured_at_utc=None)
    assert report["summary"]["terminal_outcome"] == "UNTOUCHED_LANE_REFINEMENT_REQUIRES_UNDECLARED_STRUCTURE"
...
def test_reports_path_falsified_when_phase_h_precondition_fails(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(tool, "REPO_ROOT", tmp_path)
    declaration_path = (
        tmp_path
        / "formal"
    ...
    _write_declaration(declaration_path)
    _seed_inputs(tmp_path, phase_h_outcome="UNTOUCHED_LANE_HOLD_AND_DO_NOT_CONTINUE")
