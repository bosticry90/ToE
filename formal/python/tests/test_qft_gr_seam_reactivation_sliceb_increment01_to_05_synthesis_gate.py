...
def test_synthesis_0105_exists_and_is_anchored() -> None:
    synthesis = _read(SYNTHESIS_0105_PATH)
    objective = _read(OBJECTIVE_PATH)
    increment05 = _read(INCREMENT05_PACKET_PATH)

    assert REQUIRED_QUESTION in objective
    assert REQUIRED_QUESTION in increment05
...
def test_synthesis_0105_contains_required_sections() -> None:
    synthesis = _read(SYNTHESIS_0105_PATH)
...
def test_synthesis_0105_tokens_present() -> None:
    synthesis = _read(SYNTHESIS_0105_PATH)

    assert REQUIRED_HOLD_TOKEN in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT06_TARGET_DECISION_v0: "
        "REQUIRE_NEW_ADDITIVE_CRITERION_BEYOND_MIXED_ORIGIN_EXCLUSION"
    ) in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_05_SYNTHESIS_STATUS_v0: "
        "SYNTHESIZED_BOUNDED_v0"
    ) in synthesis


def test_synthesis_0105_nonclaim_markers_present() -> None:
    synthesis = _read(SYNTHESIS_0105_PATH)
