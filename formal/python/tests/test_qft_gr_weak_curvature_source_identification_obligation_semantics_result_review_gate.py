...
SOURCE_SURFACE_ID = "QFT_GR_WEAK_CURVATURE_SOURCE_IDENTIFICATION_OBLIGATION_SEMANTICS_v0"
PREVIOUS_TARGET = (
    "review_qft_gr_weak_curvature_source_identification_obligation_semantics_result"
)
SELECTED_NEXT_TARGET = "prepare_qft_gr_poisson_recovery_obligation_semantics_bounded_attack"
SOURCE_RESULT_TOKEN = (
...
REPORT_ID = (
    "QFT_GR_WEAK_CURVATURE_SOURCE_IDENTIFICATION_OBLIGATION_SEMANTICS_RESULT_REVIEW_20260503_v0"
)
RETAINED_BLOCKER = "PHASE1-BLOCKER-QFTGR-POISSON-RECOVERY-OBLIGATION-SEMANTICS-RETAINED"
SOURCE_RETAINED_BLOCKER = (
    "PHASE1-BLOCKER-QFTGR-WEAK-CURVATURE-SOURCE-IDENTIFICATION-WITNESS-RETAINED"
)
ACTIVE_LANE = "qft_gr_poisson_recovery_obligation_semantics_preparation"
REVIEW_LANE = (
...
def test_weak_curvature_obligation_result_review_surface_consumes_result() -> None:
    ...
    for token in {
        ...
        "qft_gr_weak_curvature_source_identification_obligation_result_review_token_v0",
...
def test_weak_curvature_obligation_result_review_preserves_boundaries() -> None:
    text = _read(SURFACE_PATH)
...
def test_weak_curvature_obligation_result_review_report_records_selection() -> None:
    ...
    assert report["review_result"] == (
        "supplied_only_weak_curvature_source_identification_obligation_result_consumed"
    )
...
def test_registry_rotates_to_poisson_recovery_obligation_preparation() -> None:
    assert_current_target_consistent()
    assert_frontier_matches_registry()
    assert_forbidden_promotions_closed()
    assert_public_surfaces_match_registry()
    ...
    assert review["poisson_recovery_obligation_semantics_authorized"] == (
        "preparation_only"
    )
    assert review["poisson_recovery_obligation_semantics_constructed"] == "no"
    assert review["weak_curvature_source_identification_witness_authorized"] == "no"
    assert review["actual_weak_curvature_source_identification_authorized"] == "no"
    ...
    assert active[
        "weak_curvature_source_identification_obligation_semantics_result_review_token"
    ] == REVIEW_RESULT_TOKEN
    assert active["preparation_scope"] == "poisson_recovery_obligation_semantics_surface_only"
    assert active["poisson_recovery_obligation_semantics_authorized"] == (
        "preparation_only"
    )
    assert active["poisson_recovery_obligation_semantics_constructed"] == "no"
    ...
    assert qft_gr["weak_curvature_source_identification_obligation_result_review_status"] == (
        "completed"
    )
    assert qft_gr["weak_curvature_source_identification_obligation_result_review_evidence"] == (
        SURFACE_EVIDENCE
    )
    ...
    assert qft_gr["poisson_recovery_obligation_semantics_constructed"] == "no"
...
def test_public_surfaces_track_weak_curvature_result_review() -> None:
    ...
    assert (
        "INV-MATH-QFTGR-WEAK-CURVATURE-SOURCE-IDENTIFICATION-OBLIGATION-RESULT-REVIEW-v0"
        in inventory_text
    )
    ...
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_weak_curvature_source_identification_obligation_semantics_result_review_gate.py"
    )
