...
def test_stat_nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_status_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT nonflip-execution-custody-attestation-confirmation-attestation-confirmation-attestation status gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for nonflip-execution-custody-attestation-confirmation-attestation-confirmation-attestation status gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT nonflip-execution-custody-attestation-confirmation-attestation-confirmation-attestation status cycle-01 payload_sha256 does not match canonical payload hash."
    )
    ...
    for token_name, expected in (
        ...
        (
            "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_ATTESTATION_CONFIRMATION_ATTESTATION_STATUS_CYCLE01_GATE_v0",
    ...
    for doc_text, doc_label in (
        ...
        assert next_status in ALLOWED_NEXT_STATUS_VALUES, (
            f"{doc_label} must keep the downstream confirmation-attestation-confirmation-attestation-confirmation status either pre-admission or pinned."
        )

    stat_sha = _extract_token(
        stat_text,
        "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_ATTESTATION_CONFIRMATION_ATTESTATION_STATUS_CYCLE01_SHA256_v0",
    )
    ...
    for doc_text, doc_label in (
        ...
        for token_name, expected in (
            ...
            assert _extract_token(doc_text, token_name) == expected, (
                f"{doc_label} must preserve `{token_name}` before nonflip-execution-custody-attestation-confirmation-attestation-confirmation-attestation status admission."
            )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the nonflip-execution-custody-attestation-confirmation-attestation-confirmation-attestation status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the nonflip-execution-custody-attestation-confirmation-attestation-confirmation-attestation status gate path."
        )
    ...
    assert payload.get("checkpoint") == (
        "stat_nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_status_cycle01"
    )
    assert payload.get("status") == (
        "nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_status_non_promotional"
    )
    assert payload.get("confirmation_attestation_confirmation_attestation_entry_scope") == [
        "nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_status_placeholder_only",
        "nonflip_execution_custody_attestation_confirmation_attestation_confirmation_status_verified_before_nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_status",
        "nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_confirmation_scope_boundary_pinned_before_nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_status",
        "no_nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_claim",
        "no_external_truth_claim",
    ]
    assert payload.get("required_confirmation_attestation_confirmation_attestation_status_inputs") == [
        ...
        "nonflip_execution_custody_attestation_confirmation_attestation_confirmation_attestation_scope_boundary_pinned",
    ...
    assert payload.get("emitted_status_tokens") == [
        ...
        "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_ATTESTATION_CONFIRMATION_ATTESTATION_CONFIRMATION_STATUS_v0: NOT_PRESENT_v0",
    ]
