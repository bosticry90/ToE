...
REVIEW_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_REVIEW_v0.md"
REVIEW_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet41_targeted_justification_review_checkpoint_v0.json"
...
def test_qft_gr_seam_packet41_targeted_justification_review_document_structure() -> None:
    ...
    for marker in required_markers:
        assert marker in text, f"Packet41 targeted justification review doc missing marker: {marker}"


def test_qft_gr_seam_packet41_targeted_justification_review_checkpoint_schema_and_outcome() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet41_targeted_justification_review_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2V_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_REVIEW"
    assert artifact.get("status") == "PACKET41_TARGETED_JUSTIFICATION_REVIEW_COMPLETE_INSUFFICIENT_v0"

    payload = artifact.get("payload", {})
    assert payload.get("review_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_REVIEW_v0.md"
    ...
    assert checks.get("seam_gap_still_measurably_shrinking_check") == "PASS_v0_CYCLE02_THRESHOLD1_CLEARED"
    assert checks.get("expected_marginal_gain_above_threshold_check") == "PASS_v0_CYCLE02_THRESHOLD2_CLEARED"
    assert checks.get("stagnation_clearance_check") == "PASS_v0_CYCLE02_THRESHOLD3_CLEARED"
    assert checks.get("remaining_gap_narrower_than_objective_check") == "PASS_v0"

    outcome = payload.get("review_outcome", {})
    assert outcome.get("targeted_justification_verdict") == "INSUFFICIENT_FOR_PACKET41_AUTHORIZATION_REVIEW_LAYER_STACK_NOT_CLEARED_v0"
    assert outcome.get("hold_alignment_status") == "CONSISTENT_WITH_PACKET41_ELIGIBILITY_HOLD_v0"
...
def test_qft_gr_seam_packet41_targeted_justification_review_authority_parity_and_freeze() -> None:
    ...
    for ref in refs:
        assert ref in state_text or ref in inventory_text, (
            f"Missing packet41 targeted justification pointer in compact-State or central inventory: {ref}"
        )
        assert ref in roadmap_text, f"Missing packet41 targeted justification pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_STATUS_v0")
    ...
    state_outcome = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_OUTCOME_v0"
    )
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET41_TARGETED_JUSTIFICATION_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_RETAINED_v0"

    assert not PACKET41_AUTH_DOC_PATH.exists(), "Packet41 authorization doc must not exist during targeted-justification hold"
    assert not PACKET41_AUTH_CHECKPOINT_PATH.exists(), "Packet41 authorization checkpoint must not exist during targeted-justification hold"
