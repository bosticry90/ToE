...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "ck_family_theorem_linkage_obligation_selection_after_phi_source_closeout_result_review_report.py"
)
...
def test_ck_family_selection_after_phi_source_closeout_result_review_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_ck_family_selection_after_phi_source_closeout_result_review_accepts_bridge_selection() -> None:
...
def test_ck_family_selection_after_phi_source_closeout_result_review_preserves_bridge_registry_boundary() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["phi_bridge_registry_boundary"] == PHI_BRIDGE_REGISTRY_BOUNDARY
    assert review["prior_phi_bridge_candidate_id"] == BRIDGE_CANDIDATE_ID
    assert review["prior_phi_bridge_candidate_type"] == BRIDGE_CANDIDATE_TYPE
    assert review["prior_phi_bridge_constraint_form"] == BRIDGE_CONSTRAINT_FORM
    assert (
        review["prior_phi_bridge_constraint_equation"]
        == BRIDGE_CONSTRAINT_EQUATION
    )
    assert (
        review["prior_phi_bridge_admissibility_constraint_form"]
        == BRIDGE_ADMISSIBILITY_CONSTRAINT_FORM
    )
...
def test_ck_family_selection_after_phi_source_closeout_result_review_records_lean_status() -> None:
    review = _json(DEFAULT_OUT)
...
def test_ck_family_selection_after_phi_source_closeout_result_review_rotates_to_bridge_packet() -> None:
    ...
    assert consumed["prior_phi_bridge_constraint_form"] == BRIDGE_CONSTRAINT_FORM
    assert consumed["prior_phi_bridge_constraint_equation"] == BRIDGE_CONSTRAINT_EQUATION
    ...
    if is_current:
        ...
        assert active["prior_phi_bridge_constraint_form"] == BRIDGE_CONSTRAINT_FORM
        assert active["prior_phi_bridge_constraint_equation"] == BRIDGE_CONSTRAINT_EQUATION
...
def test_ck_family_selection_after_phi_source_closeout_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "CKFamilyTheoremLinkageObligationSelectionAfterPhiSourceCloseoutResultReview",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        LIKELY_PACKET_OUTCOME,
        ...
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PHI_SOURCE_CLOSEOUT_RESULT_REVIEW_OUTCOME_v0",
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PHI_SOURCE_CLOSEOUT_RESULT_REVIEW_NONCLAIM_BOUNDARY_v0",
...
def test_ck_family_selection_after_phi_source_closeout_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_ck_family_theorem_linkage_obligation_selection_after_phi_source_closeout_result_review_gate.py"
    )
