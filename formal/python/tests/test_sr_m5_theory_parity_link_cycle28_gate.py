...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "sr_m5_theory_parity_link_cycle28_v0.json"

EXPECTED_ARTIFACT_ID = "sr_m5_theory_parity_link_cycle28_v0"
EXPECTED_GATE = "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_AUDIT_OBJECTIVE = "LEGACY_LEAKAGE_ZERO_AND_SINGLE_ACTIVE_POINTER_v0"
...
def test_sr_m5_theory_parity_link_cycle28_gate() -> None:
    ...
    assert ARTIFACT_PATH.exists(), "SR M5 theory-parity-link cycle28 artifact is missing."
    ...
    assert m5_parity.get("artifact_path") == "formal/output/sr_m5_theory_parity_link_cycle28_v0.json"
    assert m5_parity.get("gate_path") == "formal/python/tests/test_sr_m5_theory_parity_link_cycle28_gate.py"
    ...
    # Cycle28 objective: exactly one active pointer and no legacy leakage from cycle27.
    for text in (target_text, sr_text, state_text, roadmap_text):
        assert text.count("formal/output/sr_m5_theory_parity_link_cycle28_v0.json") == 1
        assert text.count("formal/python/tests/test_sr_m5_theory_parity_link_cycle28_gate.py") == 1
        assert "formal/output/sr_m5_theory_parity_link_cycle27_v0.json" not in text
        assert "formal/python/tests/test_sr_m5_theory_parity_link_cycle27_gate.py" not in text
