...
def test_qft_legacy_not_yet_adjudication_token_is_retired_from_active_authority_surfaces() -> None:
    ...
    inventory_text, _ = split_active_and_archived(_read(INVENTORY_PATH), INVENTORY_PATH)
    ...
    assert LEGACY_QFT_ADJUDICATION_TOKEN not in inventory_text, "Legacy QFT adjudication token present in inventory authority surface."
    assert LEGACY_QFT_INEVITABILITY_TOKEN not in inventory_text, "Legacy QFT inevitability token present in inventory authority surface."
