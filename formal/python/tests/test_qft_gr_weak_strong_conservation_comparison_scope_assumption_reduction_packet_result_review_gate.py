...
from formal.python.tools.qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review_report.py"
)
LEAN_REVIEW_PATH = (
    REPO_ROOT
    / "formal"
...
def test_qft_gr_weak_strong_packet_result_review_files_exist() -> None:
...
def test_qft_gr_weak_strong_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review[
            "consumes_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet"
        ]
        == PACKET_ID
    )
...
def test_qft_gr_weak_strong_packet_result_review_accepts_packet_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["packet_preparation_only_confirmed_by_review"] is True
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_packet_reviewed"
        ]
        is True
    )
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_packet_accepted"
        ]
        is True
    )
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_packet_rejected"
        ]
        is False
    )
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_packet_preparation_only"
        ]
        is True
    )
    assert review["mr_assump_002_attempt_executed_by_review"] is False
    assert (
        review[
    ...
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_attempt_executed"
        ]
        is False
    )


def test_qft_gr_weak_strong_packet_result_review_preserves_selection() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["weak_strong_conservation_comparison_scope"] == SELECTED_ROW_OBJECT
    assert review["weak_conservation_scope"] == WEAK_CONSERVATION_SCOPE
    assert review["strong_conservation_scope"] == STRONG_CONSERVATION_SCOPE
    assert review["comparison_scope_boundaries"] == COMPARISON_SCOPE_BOUNDARIES
...
def test_qft_gr_weak_strong_packet_result_review_selects_attempt_target() -> None:
    review = _json(DEFAULT_OUT)
    packet = _json(PACKET_PATH)
    assert packet["selected_next_target"] == review["consumed_target"]
    ...
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_packet_selected_next_target"
        ]
        == packet["selected_next_target"]
    )
    assert (
        review[
            "weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review_selected_next_target"
        ]
        == NEXT_TARGET
    )
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        ...
        "review_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result": "completed_consumed_live_target",
        "prepare_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet": "not_authorized_current_row",
...
def test_qft_gr_weak_strong_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["weak_conservation_proved"] is False
    assert review["strong_conservation_proved"] is False
    assert review["weak_conservation_claimed"] is False
    assert review["strong_conservation_claimed"] is False
    assert review["weak_strong_conservation_equivalence_claimed"] is False
...
def test_qft_gr_weak_strong_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review(
