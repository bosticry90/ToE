...
ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "stat_nonflip_execution_custody_attestation_confirmation_scope_boundary_cycle01_v0.json"
)

EXPECTED_ARTIFACT_ID = "stat_nonflip_execution_custody_attestation_confirmation_scope_boundary_cycle01_v0"
EXPECTED_COUPLING_GATE = "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_ARTIFACT_REL = "formal/output/stat_nonflip_execution_custody_attestation_confirmation_scope_boundary_cycle01_v0.json"
EXPECTED_GATE_REL = (
...
def test_stat_nonflip_execution_custody_attestation_confirmation_scope_boundary_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT nonflip execution custody attestation confirmation scope-boundary gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for nonflip execution custody attestation confirmation scope-boundary gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT nonflip execution custody attestation confirmation scope-boundary cycle-01 payload_sha256 does not match canonical payload hash."
    )
    ...
    for token_name, expected in (
        ...
        (
            "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_SCOPE_BOUNDARY_CYCLE01_GATE_v0",
    ...
    stat_sha = _extract_token(
        stat_text, "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_SCOPE_BOUNDARY_CYCLE01_SHA256_v0"
    )
    state_sha = _extract_token(
        state_text, "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_SCOPE_BOUNDARY_CYCLE01_SHA256_v0"
    )
    roadmap_sha = _extract_token(
        roadmap_text, "STAT_NONFLIP_EXECUTION_CUSTODY_ATTESTATION_CONFIRMATION_SCOPE_BOUNDARY_CYCLE01_SHA256_v0"
    )
    ...
    for doc_text, doc_label in (
        ...
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin STAT nonflip execution custody attestation confirmation scope-boundary artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin STAT nonflip execution custody attestation confirmation scope-boundary gate path."
        )

    payload = artifact_json["payload"]
    assert payload.get("checkpoint") == "stat_nonflip_execution_custody_attestation_confirmation_scope_boundary_cycle01"
    assert payload.get("status") == "placeholder_non_promotional"
    assert payload.get("nonflip_execution_custody_attestation_confirmation") == [
    ...
    assert payload.get("required_nonflip_custody_attestation_confirmation_inputs") == [
