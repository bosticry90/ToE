...
DOC = REPO_ROOT / "formal" / "docs" / "second_empirical_comparator_domain_bec_bragg_b1.md"
ARTIFACT_DIR = REPO_ROOT / "formal" / "external_evidence" / "bec_bragg_b1_second_dataset_TBD"


def test_second_empirical_comparator_domain_is_pinned_to_bec_bragg_b1():
    text = DOC.read_text(encoding="utf-8")
    assert "Domain ID: `DRBR-DOMAIN-02`" in text
    assert "Selected domain: BEC Bragg secondary-source" in text
    assert "no status upgrade" in text


def test_second_empirical_comparator_domain_references_typed_fingerprinted_artifacts():
    text = DOC.read_text(encoding="utf-8")
...
def test_second_empirical_domain_artifacts_exist_and_fingerprint_checks_hold():
