...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route_result_review_report.py"
)
...
def test_phi_bridge_standalone_attempt_result_review_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_phi_bridge_standalone_attempt_result_review_accepts_preparation() -> None:
...
def test_phi_bridge_standalone_attempt_result_review_preserves_component_route() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["route_kind"] == "standalone_phi_bridge_componentwise_zero_preparation"


def test_phi_bridge_standalone_attempt_result_review_preserves_boundaries() -> None:
    review = _json(DEFAULT_OUT)
    route_text = " ".join(review["execution_route_to_authorize"])

    assert review["route_purity_watch_items"] == ROUTE_PURITY_WATCH_ITEMS
    assert review["componentwise_zero_route_prepared"] is True
    assert review["master_witness_route_match_target_prepared"] is True
    assert review["master_witness_route_match_promoted_to_master_action"] is False
...
def test_phi_bridge_standalone_attempt_result_review_rotates_to_execution() -> None:
...
def test_phi_bridge_standalone_attempt_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "PhiBridgeTheoremLinkageAttemptFromStandalonePhiBridgeRouteResultReview",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        SUGGESTED_EXECUTION_OUTCOME,
        ...
        TARGET_CONCLUSION,
        *LEAN_STATUS_WORDING_LINES_FOR_PACKET,
        "PHI_BRIDGE_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_BRIDGE_ROUTE_RESULT_REVIEW_OUTCOME_v0",
...
def test_phi_bridge_standalone_attempt_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_bridge_theorem_linkage_attempt_from_standalone_phi_bridge_route_result_review_gate.py"
    )
