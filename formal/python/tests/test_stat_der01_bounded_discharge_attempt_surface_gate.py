...
READINESS_REPORT_PATH = (
    ...
    / "output"
    / "reports"
    / "post_recovery_stat_der01_post_witness_discharge_readiness_review_20260424_v0.json"
)
WITNESS_PATH = (
    REPO_ROOT
    / "formal"
...
WITNESS_GATE_PATH = (
    ...
    / "python"
    / "tests"
    / "test_stat_der01_entropy_production_sign_definiteness_witness_binding_gate.py"
)
...
EXPECTED_ARTIFACT_ID = "stat_der01_bounded_discharge_attempt_surface_v0"
EXPECTED_SURFACE_ID = "STAT_DER01_BOUNDED_DISCHARGE_ATTEMPT_SURFACE_v0"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_der01_bounded_discharge_attempt_surface_gate.py"
EXPECTED_WITNESS_REL = "formal/output/stat_der01_entropy_production_sign_definiteness_witness_binding_v0.json"
...
def test_stat_der01_bounded_discharge_attempt_surface_gate() -> None:
    ...
    dependencies = {
        "formal/output/stat_der01_entropy_balance_theorem_body_scaffold_cycle01_v0.json": _read_json(
            THEOREM_BODY_PATH
        )["payload"],
        "formal/output/stat_der01_entropy_balance_discharge_scaffold_cycle01_v0.json": _read_json(
            DISCHARGE_PATH
        )["payload"],
        "formal/output/stat_der01_entropy_balance_object_surface_scaffold_cycle01_v0.json": _read_json(
            OBJECT_SURFACE_PATH
        )["payload"],
        "formal/output/stat_der01_entropy_balance_theorem_surface_scaffold_cycle01_v0.json": _read_json(
            THEOREM_SURFACE_PATH
        )["payload"],
    }
    ...
    assert (
        ...
        == "STAT_DER01_BOUNDED_DISCHARGE_ATTEMPT_SURFACE_PINNED_NONCLAIM_NOT_EXECUTED"
    )
    assert payload["source_missing_component_id"] == EXPECTED_SURFACE_ID
    assert payload["terminal_outcome"] == "STAT_DER01_BOUNDED_DISCHARGE_ATTEMPT_SURFACE_AUTHORED_NONCLAIM_NOT_EXECUTED"
    ...
    assert readiness_decision["terminal_outcome"] == (
        "STAT_DER01_POST_WITNESS_DISCHARGE_READINESS_NOT_READY_ATTEMPT_SURFACE_MISSING"
    )
    ...
    assert witness["artifact_id"] == "stat_der01_entropy_production_sign_definiteness_witness_binding_v0"
    assert witness["payload_sha256"] == _payload_hash(witness_payload)
    assert witness_payload["status"] == "sign_definiteness_witness_binding_pinned_nonclaim"
    assert witness_payload["sign_definiteness_witness_result"] == "SIGN_DEFINITENESS_WITNESS_PINNED_NONCLAIM"
    ...
    assert attempt_object["object_id"] == "STAT_DER01_BOUNDED_DISCHARGE_ATTEMPT_OBJECT_v0"
    ...
    witness_input = required_inputs["DER01_PINNED_SIGN_DEFINITENESS_WITNESS"]
    assert witness_input["artifact"] == EXPECTED_WITNESS_REL
    assert witness_input["required_status"] == witness_payload["status"]
    assert witness_input["required_result"] == witness_payload["sign_definiteness_witness_result"]

    for input_id, item in required_inputs.items():
        if input_id == "DER01_PINNED_SIGN_DEFINITENESS_WITNESS":
            continue
        ...
        assert dependency_payload["results_row_id"] == "TOE-STAT-DER-01"
        assert _has_slot(dependency_payload, item["required_slot"])

    assert "formal/output/reports/post_recovery_stat_der01_post_witness_discharge_readiness_review_20260424_v0.json" in payload["required_gate_inputs"]
    ...
    assert success["success_event_id"] == "STAT_DER01_BOUNDED_DISCHARGE_ATTEMPT_SUCCESS_EVENT_v0"
    assert success["evaluated_now"] is False
    assert success["current_success_state"] is False
    assert "theorem_body_entropy_production_source_placeholder_replaced_by_claim_checkable_witness_binding" in success["future_success_requires"]
    assert "discharge_future_sign_definiteness_derivation_slot_replaced_by_matching_claim_checkable_derivation" in success["future_success_requires"]
    assert "theorem_gap_delta_less_than_zero_is_machine_pinned_by_a_later_authorized_surface" in success["future_success_requires"]

    failure = payload["failure_discriminator"]
    assert failure["failure_event_id"] == "STAT_DER01_BOUNDED_DISCHARGE_ATTEMPT_FAILURE_EVENT_v0"
    assert failure["evaluated_now"] is False
    assert failure["current_failure_state"] is False
    assert "attempt_requires_packet05_der02_seam_master_action_or_promotion_surface" in failure["future_failure_conditions"]
