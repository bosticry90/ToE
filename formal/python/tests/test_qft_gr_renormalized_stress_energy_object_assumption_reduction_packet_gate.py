...
from formal.python.tools.qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_report import (
    ...
    SCHEMA_ID,
    SELECTED_ROW_ID,
    build_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet,
)
...
def test_qft_gr_renormalized_stress_energy_object_packet_files_exist() -> None:
...
def test_qft_gr_renormalized_stress_energy_object_packet_consumes_result_review() -> None:
...
def test_qft_gr_renormalized_stress_energy_object_packet_selects_only_row001() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_stress_energy_object_assumption"]
    ...
    assert packet["renormalized_stress_energy_object_status_tokens"] == selected[
        "current_status"
    ]


def test_qft_gr_renormalized_stress_energy_object_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["renormalized_stress_energy_object_assumption"]
    assert packet["candidate_stress_energy_object"] == CANDIDATE_STRESS_ENERGY_OBJECT
    assert packet["renormalized_stress_energy_object"] == CANDIDATE_STRESS_ENERGY_OBJECT
    assert packet["renormalized_stress_energy_object_definition_status"] == (
        RENORMALIZED_STRESS_ENERGY_OBJECT_DEFINITION_STATUS
    )
    assert selected["candidate_stress_energy_object"] == CANDIDATE_STRESS_ENERGY_OBJECT
    assert selected["renormalized_stress_energy_object_definition_status"] == (
        RENORMALIZED_STRESS_ENERGY_OBJECT_DEFINITION_STATUS
    )
...
def test_qft_gr_renormalized_stress_energy_object_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["renormalized_stress_energy_object_final_definition_claimed"] is False
    assert packet["renormalized_stress_energy_object_defined_as_final"] is False
    assert packet["renormalized_stress_energy_object_assumption_discharged"] is False
...
def test_qft_gr_renormalized_stress_energy_object_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_renormalized_stress_energy_object_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet(
