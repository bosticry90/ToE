...
LOCALIZATION_TOKEN = "EM_U1_DISTRIBUTIONAL_REFERENCE_SURFACE_LOCALIZATION_GATE_v0: CYCLE23_ARTIFACTS_ONLY"
PREREQUISITE_TOKENS = [
...
REFERENCE_PATTERNS = {
    "ref_in_sense": re.escape("DISTRIB_REF_VOCAB_v0: IN_SENSE_OF_DISTRIBUTIONS_REFERENCE_ONLY"),
    "ref_pairing": re.escape("DISTRIB_REF_VOCAB_v0: TEST_FUNCTION_PAIRING_REFERENCE_ONLY"),
    "ref_delta": re.escape("DISTRIB_REF_VOCAB_v0: DELTA_SOURCE_REFERENCE_ONLY"),
    "ref_weak_derivative": re.escape("DISTRIB_REF_VOCAB_v0: WEAK_DERIVATIVE_REFERENCE_ONLY"),
    "ref_rule": re.escape("REF_RULE_v0: REFERENCE_VOCAB_REQUIRES_DISTRIB_ASSUMPTION_AND_CYCLE23_LOCALIZATION"),
}
...
def test_em_cycle023_artifacts_exist() -> None:
    ...
    assert EM_MICRO23_PATH.exists(), "Missing EM U1 Cycle-023 document."
...
def test_em_micro23_contains_required_tokens_and_localized_reference_statements() -> None:
    ...
    assert not missing, "EM Cycle-023 micro document is missing required token(s): " + ", ".join(missing)

    missing_patterns = [label for label, pattern in REFERENCE_PATTERNS.items() if re.search(pattern, text) is None]
    assert not missing_patterns, (
        "EM Cycle-023 micro document is missing required reference seam statement(s): "
...
def test_em_target_references_cycle023_artifact_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM target document is missing required Cycle-023 token(s): " + ", ".join(missing)
...
def test_state_mentions_cycle023_checkpoint_and_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "State_of_the_Theory.md is missing required Cycle-023 token(s): " + ", ".join(missing)
...
def test_em_lean_cycle023_tokens_and_reference_harness_stubs_are_pinned() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "EM U1 Lean scaffold is missing required Cycle-023 token(s): " + ", ".join(missing)


def test_cycle023_reference_statements_are_localized_to_cycle23_artifact() -> None:
    allowed_paths = [EM_MICRO23_PATH]
    scoped_paths = [
        EM_TARGET_PATH,
        EM_MICRO01_PATH,
        EM_MICRO02_PATH,
    ...
    for path in allowed_paths:
        text = _read(path)
        hits = [label for label, pattern in REFERENCE_PATTERNS.items() if re.search(pattern, text)]
        if not hits:
            violations.append(f"cycle023 artifact {path.relative_to(REPO_ROOT)} is missing localized reference statements")
            continue
        ...
        if not (
            ...
            violations.append(
                f"cycle023 artifact {path.relative_to(REPO_ROOT)} contains reference statements {hits} "
    ...
    for path in scoped_paths:
        ...
        for label, pattern in REFERENCE_PATTERNS.items():
            if re.search(pattern, text):
                violations.append(
                    f"{label} reference statement leaked into non-authorized artifact "
                    f"{path.relative_to(REPO_ROOT)}"
                )

    assert not violations, "Cycle-023 reference localization violation:\n- " + "\n- ".join(violations)


def test_cycle023_is_reference_only_and_blocks_distributional_math_or_promotion() -> None:
    text = _read(EM_MICRO23_PATH)
    forbidden_patterns = [
        r"\bwe derive\b",
        r"\bwe prove\b",
        ...
        r"\bT-PROVED\b",
        r"\bDISCHARGED_v0\b",
        r"\bdistributional derivative equals\b",
        r"\bweak form identity\b",
    ...
    assert not violations, (
        "Cycle-023 must remain reference-only/non-promotional and non-selecting. "
