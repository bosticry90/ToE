...
from formal.python.tools.qft_gr_state_domain_object_assumption_reduction_attempt_report import (
    ...
    build_qft_gr_state_domain_object_assumption_reduction_attempt,
)
...
def test_qft_gr_state_domain_object_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_state_domain_object_assumption_reduction_attempt_consumes_review() -> None:
...
def test_qft_gr_state_domain_object_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["state_domain_object_assumption_reduced_pending_result_review"] is True
    assert attempt["state_domain_object_assumption_obstruction_identified"] is False
    assert attempt["state_domain_object_assumption_inconclusive"] is False


def test_qft_gr_state_domain_object_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["state_domain_object_reduction_contract"]
    ...
    assert attempt["completed_prior_assumption_families"] == [
    ...
    assert contract["contract_status"] == BOUNDED_STATE_DOMAIN_OBJECT_CONTRACT_STATUS
...
def test_qft_gr_state_domain_object_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["state_domain_object_assumption_reduced_by_attempt"] is True
    assert attempt["state_domain_object_assumption_discharged"] is False
    assert attempt["state_domain_object_assumption_discharged_by_attempt"] is False
    assert (
        attempt["state_domain_object_assumption_reduced_or_discharged_by_implication"]
        is False
    )
    assert attempt["state_domain_assumptions_discharged"] is False
    assert attempt["state_domain_assumptions_discharged_by_attempt"] is False
    assert (
        attempt["state_domain_assumptions_reduced_or_discharged_by_implication"]
        is False
    )
...
def test_qft_gr_state_domain_object_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        ...
        "discharge_qft_gr_state_domain_object_assumption": "not_authorized",
        "prepare_qft_gr_state_admissibility_boundary_assumption_reduction_packet": "deferred",
...
def test_qft_gr_state_domain_object_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_domain_object_assumption_reduction_attempt(
