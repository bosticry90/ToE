...
from formal.python.tools.qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_report import (
    ...
    DEFAULT_OPERATOR_DOMAIN_CLOSEOUT_REVIEW_PATH,
    DEFAULT_OUT,
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review,
)
...
def test_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_files_exist() -> None:
    assert PACKET_PATH.exists()
    assert DEFAULT_FAMILY_MAP_PATH.exists()
    assert DEFAULT_OPERATOR_DOMAIN_CLOSEOUT_REVIEW_PATH.exists()
    assert DEFAULT_OUT.exists()
    assert TOOL_PATH.exists()
    assert LEAN_REVIEW_PATH.exists()


def test_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_consumes_packet() -> None:
...
def test_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_accepts_rows_for_lane_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["accepted_renormalization_assumption_rows"] == _accepted_rows()
    assert review["accepted_renormalization_assumption_row_count"] == 5
    assert review["renormalization_assumptions_closed_for_this_lane"] is True
    assert review["renormalization_assumptions_reduced_for_this_lane"] is True
    assert review["renormalization_assumption_reduction_family_reduced"] is True
    assert review["renormalization_assumption_reduction_closeout_packet_reviewed"] is True
    assert review["renormalization_assumption_reduction_closeout_accepted"] is True
    assert review["renormalization_assumption_reduction_closeout_rejected"] is False
    assert review["renormalization_assumption_reduction_row_inventory_exhausted"] is True
    assert (
        review["no_remaining_renormalization_assumption_row_in_current_inventory"]
        is True
    )
...
def test_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_selects_state_domain_from_family_map() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["state_domain_assumption_reduction_packet_authorized"] is True
    assert review["mathematical_regularity_assumption_reduction_packet_deferred"] is True
    assert (
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        ...
        "prepare_qft_gr_mathematical_regularity_assumption_reduction_packet": "deferred",
...
def test_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_preserves_packet_vs_review_target_split() -> None:
    review = _json(DEFAULT_OUT)
    packet = _json(PACKET_PATH)
    ...
    assert (
        review["renormalization_assumption_reduction_closeout_packet_selected_next_target"]
    ...
    assert (
        active["renormalization_assumption_reduction_closeout_packet_selected_next_target"]
        == packet["selected_next_target"]
    )
    assert (
        active[
    ...
    assert active["selected_next_target"] == review["selected_next_target"]
    assert active["current_live_next_target"] == review["selected_next_target"]
...
def test_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalization_assumption_reduction_closeout_packet_result_review(
        packet_path=PACKET_PATH,
        family_map_path=DEFAULT_FAMILY_MAP_PATH,
        operator_domain_closeout_review_path=DEFAULT_OPERATOR_DOMAIN_CLOSEOUT_REVIEW_PATH,
