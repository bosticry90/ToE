...
from formal.python.tools.qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement_result_review_report import (
    ...
    build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement_result_review,
)
...
TOOL_PATH = (
    ...
    / (
        ...
        "retest_refinement_conservation_retest_refinement_refinement_result_review_report.py"
    )
)
LEAN_REVIEW_PATH = (
    REPO_ROOT
    ...
    / (
        ...
        "RefinementConservationRetestRefinementRefinementResultReview.lean"
    )
)
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_result_review_selects_one_next_target() -> None:
    ...
    assert review["obstruction_class_stabilization_packet_authorized"] is True
    assert review["obstruction_class_stabilization_packet_prepared_by_review"] is False
    assert review["ordinary_model_refinement_packet_authorized"] is False
    assert review["model_refinement_packet_authorized"] is False
    ...
    assert review["immediate_conservation_retest_authorized"] is False
    assert review["countermodel_packet_authorized"] is False
    assert review["countermodel_packet_prepared_by_review"] is False
    assert review["selected_dominant_obstruction_candidate"] == (
        DOMINANT_OBSTRUCTION_CANDIDATE
    )
    assert review["selected_dominant_obstruction_candidate_count"] == 1
    assert review["canonical_obstruction_id"] == CANONICAL_OBSTRUCTION_ID
    assert review["obstruction_status"] == OBSTRUCTION_STATUS
    assert review["selected_stabilization_route"] == NEXT_TARGET
    assert review["selected_stabilization_route_count"] == 1
    assert review["pattern_stabilization_signal"] == PATTERN_STABILIZATION_SIGNAL
    assert review["repeated_inconclusive_pattern_recorded"] is True
    assert review["repeated_inconclusive_attempt_count"] == 5
    assert all(row["inconclusive"] for row in review["repeated_inconclusive_chain"])
    assert sum(1 for row in review["route_rows"] if row["selected"]) == 1
    assert sum(1 for row in review["obstruction_target_rows"] if row["selected"]) == 1
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        CONSUMED_TARGET: "completed_consumed_live_target",
        ORDINARY_MODEL_REFINEMENT_TARGET: "not_authorized_repeated_inconclusive_loop",
        COUNTERMODEL_TARGET: "retained_follow_on_after_stabilization",
        POSITIVE_WITNESS_TARGET: "recommended_after_stabilization_review",
        SOURCE_MAP_LADDER_TARGET: "retained_follow_on_after_countermodel_pressure",
        (
            "execute_qft_gr_minimal_working_model_conservation_retest_attempt_"
            "after_post_retest_refinement_conservation_retest_refinement_refinement"
        ): "not_authorized_immediate_retest_forbidden",
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_result_review_updates_live_target() -> None:
    ...
    assert review_workstream["repeated_inconclusive_pattern_recorded"] == "yes"
    assert (
        review_workstream["obstruction_class_stabilization_packet_authorized"] == "yes"
    )
    assert review_workstream["ordinary_model_refinement_packet_authorized"] == "no"
    assert review_workstream["model_refinement_packet_authorized"] == "no"
    assert review_workstream["countermodel_packet_authorized"] == "no"
    assert review_workstream["immediate_conservation_retest_authorized"] == "no"
    ...
    assert active_workstream["obstruction_class_stabilization_packet_authorized"] == "yes"
    assert active_workstream["obstruction_class_stabilization_packet_prepared"] == "no"
    assert active_workstream["dominant_obstruction_candidate"] == (
        DOMINANT_OBSTRUCTION_CANDIDATE
    )
    assert active_workstream["canonical_obstruction_id"] == CANONICAL_OBSTRUCTION_ID
    assert active_workstream["obstruction_status"] == OBSTRUCTION_STATUS
    assert active_workstream["ordinary_model_refinement_packet_authorized"] == "no"
    assert active_workstream["model_refinement_packet_authorized"] == "no"
    assert active_workstream["countermodel_packet_authorized"] == "no"
    assert active_workstream["positive_witness_lane_recommended_after_stabilization"] == "yes"
    assert active_workstream["conservation_retest_rerun_authorized"] == "no"
    assert active_workstream["immediate_conservation_retest_authorized"] == "no"
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_result_review_deterministic() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement_result_review(
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement_result_review_gate.py"
    )
