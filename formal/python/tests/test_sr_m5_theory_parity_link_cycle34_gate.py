...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "sr_m5_theory_parity_link_cycle34_v0.json"

EXPECTED_ARTIFACT_ID = "sr_m5_theory_parity_link_cycle34_v0"
...
def test_sr_m5_theory_parity_link_cycle34_gate() -> None:
    ...
    assert ARTIFACT_PATH.exists(), "SR M5 theory-parity-link cycle34 artifact is missing."
    ...
    assert m5_parity.get("artifact_path") == "formal/output/sr_m5_theory_parity_link_cycle34_v0.json"
    assert m5_parity.get("gate_path") == "formal/python/tests/test_sr_m5_theory_parity_link_cycle34_gate.py"
    ...
    # Cycle34 objective: exactly one active pointer, no legacy leakage from cycle33, and stable token ordering.
    for text in (target_text, sr_text, state_text, roadmap_text):
        assert text.count("formal/output/sr_m5_theory_parity_link_cycle34_v0.json") == 1
        assert text.count("formal/python/tests/test_sr_m5_theory_parity_link_cycle34_gate.py") == 1
        assert "formal/output/sr_m5_theory_parity_link_cycle33_v0.json" not in text
        assert "formal/python/tests/test_sr_m5_theory_parity_link_cycle33_gate.py" not in text
