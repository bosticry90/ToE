...
from formal.python.tools.qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction,
)
...
def test_countermodel_scope_refinement_packet_files_exist() -> None:
...
def test_countermodel_scope_refinement_packet_consumes_result_review() -> None:
    ...
    assert packet["consumes_countermodel_attempt_result_review"] == RESULT_REVIEW_ID
...
def test_countermodel_scope_refinement_packet_pins_missing_search_space() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["countermodel_search_space_refined"] is True
    assert packet["source_test_instantiation_pinned"] is True
    assert packet["weak_pairing_semantics_pinned"] is True
    assert packet["broader_divergence_boundary_evaluation_scope_pinned"] is True
    ...
    assert packet["scope_refinement_row_count"] == 3
    assert {
        row["requirement_id"] for row in packet["scope_refinement_rows"]
    } == {
...
def test_countermodel_scope_refinement_packet_does_not_execute_attempt_or_claim_result() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["countermodel_scope_refinement_packet_prepared"] is True
    assert packet["countermodel_scope_refinement_packet_preparation_only"] is True
    assert packet["countermodel_scope_refinement_packet_result_review_pending"] is True
    assert packet["countermodel_scope_refinement_packet_result_reviewed"] is False
    assert packet["countermodel_attempt_authorized_by_packet"] is False
    assert packet["countermodel_attempt_executed_by_packet"] is False
    assert packet["countermodel_attempt_reexecuted"] is False
    for key in [
...
def test_countermodel_scope_refinement_packet_preserves_strict_toy_witness_and_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_countermodel_scope_refinement_packet_selects_result_review_only() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        ...
        "execute_qft_gr_minimal_model_countermodel_attempt_for_weak_conservation_obstruction_after_scope_refinement": (
            "not_authorized_until_packet_result_review"
        ),
...
def test_countermodel_scope_refinement_packet_updates_live_target() -> None:
    ...
    assert packet_workstream["countermodel_scope_refinement_packet_prepared"] == "yes"
    assert packet_workstream["selected_next_target"] == NEXT_TARGET
    assert packet_workstream["countermodel_result_claimed"] == "no"
    assert packet_workstream["no_go_result_claimed"] == "no"
    ...
    assert active_workstream["countermodel_scope_refinement_packet_prepared"] == "yes"
    assert (
        active_workstream["countermodel_scope_refinement_packet_result_review_pending"]
        == "yes"
    )
    assert active_workstream["countermodel_scope_refinement_packet_result_reviewed"] == "no"
    assert active_workstream["countermodel_attempt_authorized_by_packet"] == "no"
    assert active_workstream["countermodel_attempt_executed_by_packet"] == "no"
...
def test_countermodel_scope_refinement_packet_deterministic() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction(
...
def test_countermodel_scope_refinement_packet_lean_and_surface_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        PINNED_WEAK_PAIRING_CONTRACT_ID,
        PINNED_EVALUATION_SCOPE_ID,
        "countermodelScopeRefinementPacketPrepared",
        "sourceTestInstantiationPinned",
        "weakPairingSemanticsPinned",
...
def test_countermodel_scope_refinement_packet_gate_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_model_countermodel_scope_refinement_packet_for_weak_conservation_obstruction_gate.py"
    )
