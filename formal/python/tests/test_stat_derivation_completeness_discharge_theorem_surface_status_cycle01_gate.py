...
ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "stat_derivation_completeness_discharge_theorem_surface_status_cycle01_v0.json"
)

EXPECTED_ARTIFACT_ID = "stat_derivation_completeness_discharge_theorem_surface_status_cycle01_v0"
...
EXPECTED_ARTIFACT_REL = "formal/output/stat_derivation_completeness_discharge_theorem_surface_status_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_derivation_completeness_discharge_theorem_surface_status_cycle01_gate.py"
...
def test_stat_derivation_completeness_discharge_theorem_surface_status_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT derivation-completeness discharge theorem-surface status gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for derivation-completeness discharge theorem-surface status gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT derivation-completeness discharge theorem-surface status cycle-01 payload_sha256 does not match canonical payload hash."
    )

    for token_name, expected in (
        ("STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
        ("STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_v0", EXPECTED_STATUS_VALUE),
    ):
    ...
    for doc_text, doc_label in (
        ...
        object_status = _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_OBJECT_SURFACE_STATUS_v0")
        assert object_status in ALLOWED_OBJECT_STATUS_VALUES, (
            f"{doc_label} must keep the object-surface status either pre-admission or object-surface pinned."
        )

    stat_sha = _extract_token(stat_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_CYCLE01_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_CYCLE01_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_THEOREM_SURFACE_STATUS_CYCLE01_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0") == "PRESENT", (
            f"{doc_label} must preserve the readiness packet before theorem-surface status admission."
        )
        assert _extract_token(
            ...
            f"{doc_label} must preserve the gate entry-status token before theorem-surface status admission."
        )
        assert _extract_token(
            doc_text, "STAT_DERIVATION_COMPLETENESS_DISCHARGE_SURFACE_STATUS_v0"
        ) == "ENTRY_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the discharge-surface status token before theorem-surface status admission."
        )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the derivation-completeness discharge theorem-surface status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the derivation-completeness discharge theorem-surface status gate path."
        )
    ...
    assert payload.get("checkpoint") == "stat_derivation_completeness_discharge_theorem_surface_status_cycle01"
    assert payload.get("status") == "theorem_surface_status_non_promotional"
    assert payload.get("theorem_surface_entry_scope") == [
        "derivation_completeness_discharge_theorem_surface_status_placeholder_only",
        "discharge_surface_status_verified_before_theorem_surface_status",
    ...
    assert "- bounded derivation-completeness discharge theorem-surface entry scope only; no object-surface execution claim and no external truth claim." in stat_text
    assert "- theorem-surface status packet remains non-promotional and does not authorize `TOE-STAT-DER-*` label promotion." in stat_text
