...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_failure_trigger_discharge_coherence_status_cycle01_v0.json"
OBJECT_STATUS_ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "stat_failure_trigger_discharge_object_surface_status_cycle01_v0.json"
)

EXPECTED_ARTIFACT_ID = "stat_failure_trigger_discharge_coherence_status_cycle01_v0"
EXPECTED_OBJECT_STATUS_ARTIFACT_ID = "stat_failure_trigger_discharge_object_surface_status_cycle01_v0"
...
ALLOWED_TRANSITION_STATUS_VALUES = {
    "NOT_PRESENT_v0",
    "DISCHARGE_COMPLETION_TRANSITION_SCAFFOLD_PINNED_NONCLAIM",
}
EXPECTED_ARTIFACT_REL = "formal/output/stat_failure_trigger_discharge_coherence_status_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_failure_trigger_discharge_coherence_status_cycle01_gate.py"
...
def test_stat_failure_trigger_discharge_coherence_status_cycle01_gate() -> None:
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT failure-trigger discharge coherence-status cycle-01 payload_sha256 does not match canonical payload hash."
    )

    assert object_status_artifact_json.get("artifact_id") == EXPECTED_OBJECT_STATUS_ARTIFACT_ID

    for token_name, expected in (
        ("STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
        ("STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_v0", EXPECTED_STATUS_VALUE),
    ):
    ...
    for doc_text, doc_label in (
        ...
        transition_status = _extract_token(doc_text, "STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_v0")
        assert transition_status in ALLOWED_TRANSITION_STATUS_VALUES, (
            f"{doc_label} must keep the discharge-completion transition status either pre-admission or transition-status pinned."
        )

    stat_sha = _extract_token(stat_text, "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_CYCLE01_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_CYCLE01_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_CYCLE01_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0") == "PRESENT", (
            f"{doc_label} must preserve the readiness packet before failure-trigger coherence-status admission."
        )
        assert _extract_token(
            ...
            f"{doc_label} must preserve the gate entry-status token before failure-trigger coherence-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_v0"
        ) == "ENTRY_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger discharge surface-status token before coherence-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0"
        ) == "THEOREM_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger theorem-surface status token before coherence-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_v0"
        ) == "OBJECT_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger object-surface status token before coherence-status admission."
        )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the failure-trigger discharge coherence-status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the failure-trigger discharge coherence-status gate path."
        )

    for token_name, expected in (
        (
            "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_CYCLE01_ARTIFACT_v0",
            EXPECTED_OBJECT_STATUS_ARTIFACT_ID,
        ),
        (
    ...
    assert payload.get("checkpoint") == "stat_failure_trigger_discharge_coherence_status_cycle01"
    assert payload.get("status") == "coherence_status_non_promotional"
    assert payload.get("coherence_entry_scope") == [
        "failure_trigger_discharge_coherence_status_placeholder_only",
        "surface_theorem_object_status_bundle_verified_before_coherence_status",
        "discharge_completion_transition_scope_boundary_pinned_before_coherence_status",
        "no_failure_trigger_coherence_discharge_claim",
        "no_external_truth_claim",
    ]
    assert payload.get("required_coherence_status_inputs") == [
    ...
    assert payload.get("emitted_status_tokens") == [
        ...
        "STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_v0: NOT_PRESENT_v0",
    ]
    ...
    assert "- dependency ladder back to the object-surface status artifact is pinned before coherence-status admission." in stat_text
