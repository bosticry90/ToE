...
from formal.python.tools.qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_renormalized_expectation_finiteness_packet_result_review_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_finiteness_packet_result_review_consumes_packet() -> None:
...
def test_qft_gr_renormalized_expectation_finiteness_packet_result_review_confirms_row004() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["accepted_prior_renormalization_assumption_rows"] == [
    ...
    assert review["accepted_prior_row_count"] == 3
    assert review["selected_renormalization_assumption_row"] == SELECTED_ROW_ID
    assert review["selected_row_count"] == 1
    assert review["renormalized_expectation_finiteness_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert review["renormalized_expectation_domain"] == RENORMALIZED_EXPECTATION_DOMAIN
    assert review["finiteness_regular_boundary"] == FINITENESS_REGULARITY_BOUNDARY
    assert review["finiteness_regular_boundary_condition"] == (
        FINITENESS_REGULARITY_BOUNDARY
    )
    assert review["finiteness_regular_boundary_status"] == (
        FINITENESS_REGULARITY_CONDITION_STATUS
    )
    assert review["scope_boundaries"] == FINITENESS_REGULARITY_BOUNDARY_SCOPE
...
def test_qft_gr_renormalized_expectation_finiteness_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["finiteness_regular_boundary_discharged"] is False
    assert review["finiteness_regular_boundary_reduced_by_review"] is False
    assert (
        review["finiteness_regular_boundary_reduced_or_discharged_by_review"]
        is False
    )
...
def test_qft_gr_renormalized_expectation_finiteness_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert target_decisions[
        "discharge_qft_gr_renormalized_expectation_finiteness_assumption"
    ] == "not_authorized"
...
def test_qft_gr_renormalized_expectation_finiteness_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review(
