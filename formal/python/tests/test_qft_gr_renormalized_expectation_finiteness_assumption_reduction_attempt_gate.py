...
from formal.python.tools.qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt,
)
...
def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_consumes_review() -> None:
...
def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert (
        attempt["finiteness_regular_boundary_assumption_obstruction_identified"]
        is False
    )
    assert attempt["finiteness_regular_boundary_assumption_inconclusive"] is False


def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["finiteness_regular_boundary_reduction_contract"]
    ...
    assert attempt["accepted_prior_renormalization_assumption_rows"] == [
    ...
    assert contract["contract_id"] == FINITENESS_REGULARITY_CONTRACT_ID
    ...
    assert contract["finiteness_regular_boundary"] == FINITENESS_REGULARITY_BOUNDARY
    assert contract["finiteness_regular_boundary_condition"] == (
        FINITENESS_REGULARITY_BOUNDARY
    )
    assert contract["scope_boundaries"] == FINITENESS_REGULARITY_BOUNDARY_SCOPE
    assert contract["contract_status"] == BOUNDED_FINITENESS_REGULARITY_CONTRACT_STATUS
    assert (
        contract["source_packet_finiteness_regular_boundary_status"]
        == FINITENESS_REGULARITY_CONDITION_STATUS
    )
...
def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["finiteness_regular_boundary_assumption_reduced_by_attempt"] is True
    assert attempt["finiteness_regular_boundary_assumption_discharged"] is False
    assert (
        attempt["finiteness_regular_boundary_assumption_discharged_by_attempt"]
        is False
    )
    assert (
        attempt["finiteness_regular_boundary_reduced_or_discharged_by_attempt"]
        is False
    )
    assert (
        attempt["finiteness_regular_boundary_reduced_or_discharged_by_implication"]
        is False
    )
    assert attempt["finiteness_regular_boundary_discharged"] is False
    assert attempt["finiteness_regular_boundary_discharged_by_implication"] is False
    assert attempt["finiteness_regular_boundary_claimed_as_conservation_proof"] is False
    assert attempt["finiteness_regular_boundary_claimed_as_conservation_source"] is False
    assert (
        attempt["finiteness_regular_boundary_claimed_as_source_admissibility"]
        is False
    )
    assert (
        attempt["finiteness_regular_boundary_claimed_as_bianchi_compatibility"]
        is False
    )
...
def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        ...
        "discharge_qft_gr_renormalized_expectation_finiteness_assumption": "not_authorized",
...
def test_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt(
