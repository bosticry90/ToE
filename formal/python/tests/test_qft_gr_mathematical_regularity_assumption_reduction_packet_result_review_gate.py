...
from formal.python.tools.qft_gr_mathematical_regularity_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_mathematical_regularity_assumption_reduction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_mathematical_regularity_assumption_reduction_packet_result_review_report.py"
)
LEAN_REVIEW_PATH = (
    REPO_ROOT
    / "formal"
...
def test_qft_gr_mathematical_regularity_packet_result_review_files_exist() -> None:
...
def test_qft_gr_mathematical_regularity_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_qft_gr_mathematical_regularity_assumption_reduction_packet"] == (
        PACKET_ID
    )
...
def test_qft_gr_mathematical_regularity_packet_result_review_accepts_packet_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["packet_preparation_only_confirmed_by_review"] is True
    assert review["mathematical_regularity_assumption_reduction_packet_reviewed"] is True
    assert review["mathematical_regularity_assumption_reduction_packet_accepted"] is True
    assert review["mathematical_regularity_assumption_reduction_packet_rejected"] is False
    assert (
        review["mathematical_regularity_assumption_reduction_packet_preparation_only"]
        is True
    )
    assert review["mr_assump_001_attempt_executed_by_review"] is False
    assert (
        review[
...
def test_qft_gr_mathematical_regularity_packet_result_review_preserves_selection() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["selected_mathematical_regularity_assumption_row"] == (
        SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW
    )
    ...
    assert review["selected_row_is_first_repo_authoritative_row"] is True
    assert review["derivative_exchange_regular_boundary"] == (
        DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY
    )
...
def test_qft_gr_mathematical_regularity_packet_result_review_selects_attempt_target() -> None:
    review = _json(DEFAULT_OUT)
    packet = _json(PACKET_PATH)
    assert packet["selected_next_target"] == review["consumed_target"]
...
def test_qft_gr_mathematical_regularity_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_mathematical_regularity_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_mathematical_regularity_assumption_reduction_packet_result_review(
