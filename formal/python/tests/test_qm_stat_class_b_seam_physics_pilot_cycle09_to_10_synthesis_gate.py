...
SYNTH_DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE09_TO_10_SYNTHESIS_v0.md"
)
CYCLE09_DOC_PATH = (
    REPO_ROOT
    / "formal"
...
def test_qm_stat_cycle09_to_10_synthesis_artifacts_exist() -> None:
    for path in (SYNTH_DOC_PATH, CYCLE09_DOC_PATH, CYCLE10_DOC_PATH, CYCLE09_ARTIFACT_PATH, CYCLE10_ARTIFACT_PATH):
        assert path.exists(), f"Missing required file: {path}"


def test_qm_stat_cycle09_to_10_synthesis_doc_tokens() -> None:
    text = _read(SYNTH_DOC_PATH)
    required_tokens = [
        "DERIVATION_TARGET_QM_STAT_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE09_TO_10_SYNTHESIS_v0",
        "TARGET-QM-STAT-CLASS-B-SEAM-PHYSICS-PILOT-CYCLE09-TO-10-SYNTHESIS-v0",
        ...
        "formal/python/tests/test_qm_stat_class_b_seam_physics_pilot_cycle09_to_10_synthesis_gate.py",
    ]
    missing = [tok for tok in required_tokens if tok not in text]
    assert not missing, "QM-STAT Cycle09-to-10 synthesis doc missing required token(s): " + ", ".join(missing)


def test_qm_stat_cycle09_to_10_additive_delta_is_material() -> None:
    ...
    assert "sixteenth_central_moment" not in c9
    assert "sixteenth_central_moment" in c10

    xs10 = [Fraction(x) for x in c10["shared_support"]]
    qm10 = [Fraction(x) for x in c10["qm_probability_mass"]]
    st10 = [Fraction(x) for x in c10["stat_probability_mass"]]
    _, _, qm_m3_10, qm_m4_10, qm_m6_10, qm_m8_10, qm_m10_10, qm_m12_10, qm_m14_10, qm_m16_10 = _moments16(xs10, qm10)
    _, _, st_m3_10, st_m4_10, st_m6_10, st_m8_10, st_m10_10, st_m12_10, st_m14_10, st_m16_10 = _moments16(xs10, st10)
    assert qm_m3_10 == st_m3_10
    assert qm_m4_10 == st_m4_10
    assert qm_m6_10 == st_m6_10
    assert qm_m8_10 == st_m8_10
    assert qm_m10_10 == st_m10_10
    assert qm_m12_10 == st_m12_10
    assert qm_m14_10 == st_m14_10
    assert qm_m16_10 == st_m16_10


def test_qm_stat_cycle09_to_10_exclusion_strengthening_present() -> None:
    ...
    assert "sixteenth_central_moment" not in cycle09_ex
    assert "sixteenth_central_moment" in cycle10_ex

    xs = [Fraction(x) for x in cycle10_ex["shared_support"]]
    qm = [Fraction(x) for x in cycle10_ex["qm_probability_mass"]]
    st = [Fraction(x) for x in cycle10_ex["stat_probability_mass"]]
    qm_mu, qm_var, qm_m3, qm_m4, qm_m6, qm_m8, qm_m10, qm_m12, qm_m14, qm_m16 = _moments16(xs, qm)
    st_mu, st_var, st_m3, st_m4, st_m6, st_m8, st_m10, st_m12, st_m14, st_m16 = _moments16(xs, st)
...
def test_qm_stat_cycle09_to_10_promotion_still_blocked() -> None:
    cycle09 = _json(CYCLE09_ARTIFACT_PATH)
    cycle10 = _json(CYCLE10_ARTIFACT_PATH)
    ...
    for artifact in (cycle09, cycle10):
