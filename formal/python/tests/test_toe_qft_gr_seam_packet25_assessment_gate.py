...
DOC = ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET25_ASSESSMENT_v0.md"
CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet25_assessment_checkpoint_v0.json"
)
PARENT_EXEC_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET25_BOUNDED_EXECUTION_v0.md"
)
PARENT_EXEC_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet25_bounded_execution_checkpoint_v0.json"
)
PARENT_AUTH_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET25_AUTHORIZATION_v0.md"
)
PARENT_AUTH_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet25_authorization_checkpoint_v0.json"
)
...
def test_packet25_assessment_files_exist() -> None:
    assert DOC.exists(), f"Missing packet25 assessment doc: {DOC}"
    assert CHECKPOINT.exists(), f"Missing packet25 assessment checkpoint: {CHECKPOINT}"


def test_packet25_assessment_doc_required_markers() -> None:
    text = _read_text(DOC)
    required_markers = [
        "# TOE QFT-GR Seam Packet25 Assessment v0",
        "## Assessment Questions",
        "## Objective Progress Snapshot",
        "## Scalar Freeze Compliance",
        "## Decision Rule Projection (for packet26 authorization)",
        "TOE_QFT_GR_SEAM_PACKET25_ASSESSMENT_STATUS_v0: ASSESSED_TARGET_SATISFACTION_VERIFIED_v0",
        "PACKET25_ASSESSMENT_COMPLETE_v0",
...
def test_packet25_assessment_checkpoint_schema_and_lineage() -> None:
    data = _load_json(CHECKPOINT)
    assert data["artifact_id"] == "toe_qft_gr_seam_packet25_assessment_checkpoint_v0"
    assert data["phase"] == "PHASE_2B_QFT_GR_SEAM_PACKET25_ASSESSMENT"
    assert data["scope"] == "packet25_target_satisfaction_assessment"
    assert data["status"] == "PACKET25_ASSESSMENT_COMPLETE_v0"

    payload = data["payload"]
    assert payload["assessment_doc_path"].endswith(
        "TOE_QFT_GR_SEAM_PACKET25_ASSESSMENT_v0.md"
    )
...
def test_packet25_assessment_target_satisfaction_and_packet26_rule() -> None:
    ...
    packet26 = payload["packet26_decision"]
    assert packet26["authorized"] is True
    assert packet26["progress_verdict"] == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet26["ladder_extension_verdict"] == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet26["verdict"] == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet26["exact_bounded_target"] == EXPECTED_PACKET26_TARGET
    assert packet26["hold_refine_trigger_status"] == "NOT_HIT_v0"


def test_packet25_assessment_scalar_and_guardrail_invariants() -> None:
    payload = _load_json(CHECKPOINT)["payload"]
    ...
    assert guardrails["assessment_scope"] == "ASSESS_PACKET25_BEFORE_PACKET26_v0"


def test_packet25_assessment_parent_chain_exists() -> None:
