...
COSMO_MICRO12_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_12_DRYRUN_CLOSURE_PACKET_v0.md"
COSMO_MICRO12_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro12_dryrun_closure_packet_cycle01_v0.json"
...
def test_cosmo_micro12_artifacts_exist() -> None:
    assert COSMO_TARGET_PATH.exists(), "Missing COSMO background target document."
    assert COSMO_MICRO12_PATH.exists(), "Missing COSMO background Cycle-012 micro document."
    assert COSMO_MICRO12_ARTIFACT_PATH.exists(), "Missing COSMO background Cycle-012 artifact payload."
...
def test_cosmo_target_references_micro12_and_gate() -> None:
    ...
    assert not missing, "COSMO parent target is missing required micro-12 token(s): " + ", ".join(missing)
...
def test_cosmo_micro12_doc_contains_required_tokens() -> None:
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "COSMO micro-12 document is missing required token(s): " + ", ".join(missing)


def test_cosmo_dryrun_closure_keeps_locked_queue_status_and_bundle_coherence() -> None:
    ...
    assert not missing, "State missing dryrun closure policy token(s): " + ", ".join(missing)
...
def test_cosmo_micro12_artifact_schema_and_token_alignment() -> None:
    ...
    pointers = body.get("required_closure_bundle_pointers")
    assert isinstance(pointers, list) and len(pointers) == 4

    assert body.get("required_closure_bundle_hash") == "c08_c09_c10_c11_pointer_bundle_cycle01_v0"
