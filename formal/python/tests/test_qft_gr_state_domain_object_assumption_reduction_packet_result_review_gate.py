...
from formal.python.tools.qft_gr_state_domain_object_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_state_domain_object_assumption_reduction_packet_result_review,
)
...
def test_qft_gr_state_domain_object_packet_result_review_files_exist() -> None:
...
def test_qft_gr_state_domain_object_packet_result_review_consumes_packet() -> None:
    ...
    assert review["consumes_qft_gr_state_domain_object_assumption_reduction_packet"] == (
        PACKET_ID
    )
...
def test_qft_gr_state_domain_object_packet_result_review_confirms_row001() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["state_domain_object_status_tokens"] == [
        "required",
        "supplied",
        "candidate_reducible",
    ]
...
def test_qft_gr_state_domain_object_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["state_domain_object_assumption_discharged"] is False
    assert review["state_domain_object_assumption_reduced_by_review"] is False
    assert review["state_domain_object_assumption_reduced_or_discharged_by_review"] is False
...
def test_qft_gr_state_domain_object_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_state_domain_object_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = (
        build_qft_gr_state_domain_object_assumption_reduction_packet_result_review(
