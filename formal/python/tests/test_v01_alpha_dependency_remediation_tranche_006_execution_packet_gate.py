...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_execution_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche006ExecutionPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_consumes_selection_result_review() -> None:
    packet = _json(PACKET_PATH)
    assert packet["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_EXECUTION_PACKET_20260515_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_preserves_prior_tranches_and_tranche_004() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["tranche_006_status"] == "selected_for_execution_packet_preparation"
    assert packet["tranche_006_cleared_for_global_release_readiness"] is False
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_selects_only_tranche_006() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_tracks_current_ledger() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert [row["dependency_finding_id"] for row in selectable] == [SELECTED_FINDING_ID]
    assert selectable[0]["dependency"] == SELECTED_DEPENDENCY
    assert selectable[0]["dependency_class"] == SELECTED_DEPENDENCY_CLASS
    assert packet["tranche_006_obligation_carry_forward"]["dependency_finding_id"] == (
        SELECTED_FINDING_ID
    )


def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_defines_lean_audit_surface() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_defines_review_requirements_and_criteria() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_prepares_without_execution() -> None:
    packet = _json(PACKET_PATH)
    assert packet["tranche_006_execution_packet_prepared"] is True
    ...
    assert packet["tranche_006_audit_executed"] is False
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == (
        "tranche_006_execution_packet_result_review_only"
    )
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_006_execution_packet_result": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_execution_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_execution_packet_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006ExecutionPacket" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_execution_packet_does_not_execute_remediation"
        in index_text
    )
