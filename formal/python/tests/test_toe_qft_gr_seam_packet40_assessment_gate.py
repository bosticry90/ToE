...
def test_qft_gr_seam_packet40_assessment_authority_parity_and_invariance() -> None:
    ...
    for ref in refs:
        assert any(ref in text for text in (state_text, inventory_text, roadmap_text)), (
            f"Missing packet40 assessment pointer across authority surfaces: {ref}"
        )
    ...
    state_assessment = _extract_token_from_surfaces(
        ...
        "TOE_QFT_GR_SEAM_PACKET40_ASSESSMENT_STATUS_v0",
    )
    roadmap_assessment = _extract_token_from_surfaces([roadmap_text], "TOE_QFT_GR_SEAM_PACKET40_ASSESSMENT_STATUS_v0")
    assert state_assessment == roadmap_assessment == "BOUNDED_TARGET_CONFIRMED_v0"

    state_readiness = _extract_token_from_surfaces(
        [state_text, inventory_text, roadmap_text],
        "TOE_QFT_GR_SEAM_PACKET41_AUTHORIZATION_READINESS_v0",
    )
