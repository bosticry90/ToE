...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_der02_theorem_body_scope_boundary_cycle01_v0.json"

EXPECTED_ARTIFACT_ID = "stat_der02_theorem_body_scope_boundary_cycle01_v0"
EXPECTED_GATE = "ARTIFACT_HASH_ROW_LABEL_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_ROW_BINDING = "TOE_STAT_DER_02_T_PROVED_THEOREM_BODY_SCOPE_BOUNDARY_PINNED_NONCLAIM"
...
EXPECTED_DER02_DISCHARGE_ARTIFACT_REL = "formal/output/stat_der02_regime_closure_discharge_scaffold_cycle01_v0.json"
...
EXPECTED_DER02_OBJECT_ARTIFACT_REL = "formal/output/stat_der02_regime_closure_object_surface_scaffold_cycle01_v0.json"
...
def test_stat_der02_theorem_body_scope_boundary_cycle01_gate() -> None:
    ...
    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT DER02 theorem-body scope-boundary gate applies only after `PILLAR-STAT` activation."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT DER02 theorem-body scope-boundary payload_sha256 does not match canonical payload hash."
    )

    for token_name, expected in (
        ("STAT_DER02_THEOREM_BODY_SCOPE_BOUNDARY_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_DER02_THEOREM_BODY_SCOPE_BOUNDARY_CYCLE01_GATE_v0", EXPECTED_GATE),
        ("STAT_DER02_THEOREM_BODY_SCOPE_BOUNDARY_ROW_BINDING_v0", EXPECTED_ROW_BINDING),
    ):
    ...
    stat_sha = _extract_token(stat_text, "STAT_DER02_THEOREM_BODY_SCOPE_BOUNDARY_CYCLE01_ARTIFACT_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_DER02_THEOREM_BODY_SCOPE_BOUNDARY_CYCLE01_ARTIFACT_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_DER02_THEOREM_BODY_SCOPE_BOUNDARY_CYCLE01_ARTIFACT_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert EXPECTED_ARTIFACT_REL in doc_text, f"{doc_label} must pin DER02 theorem-body scope-boundary artifact path."
    ...
    assert payload.get("sibling_der02_regime_closure_scaffold_dependency_artifact_id") == EXPECTED_DER02_REGIME_CLOSURE_ARTIFACT_ID
    assert payload.get("sibling_der02_regime_closure_scaffold_dependency_artifact_path") == EXPECTED_DER02_REGIME_CLOSURE_ARTIFACT_REL
    assert payload.get("sibling_der02_regime_closure_scaffold_dependency_gate") == EXPECTED_DER02_REGIME_CLOSURE_GATE_REL
    assert payload.get("sibling_der02_theorem_body_scaffold_dependency_artifact_id") == EXPECTED_DER02_THEOREM_BODY_ARTIFACT_ID
    assert payload.get("sibling_der02_theorem_body_scaffold_dependency_artifact_path") == EXPECTED_DER02_THEOREM_BODY_ARTIFACT_REL
    assert payload.get("sibling_der02_theorem_body_scaffold_dependency_gate") == EXPECTED_DER02_THEOREM_BODY_GATE_REL
    assert payload.get("sibling_der02_discharge_scaffold_dependency_artifact_id") == EXPECTED_DER02_DISCHARGE_ARTIFACT_ID
    assert payload.get("sibling_der02_discharge_scaffold_dependency_artifact_path") == EXPECTED_DER02_DISCHARGE_ARTIFACT_REL
    assert payload.get("sibling_der02_discharge_scaffold_dependency_gate") == EXPECTED_DER02_DISCHARGE_GATE_REL
    assert payload.get("sibling_der02_object_surface_scaffold_dependency_artifact_id") == EXPECTED_DER02_OBJECT_ARTIFACT_ID
    assert payload.get("sibling_der02_object_surface_scaffold_dependency_artifact_path") == EXPECTED_DER02_OBJECT_ARTIFACT_REL
    assert payload.get("sibling_der02_object_surface_scaffold_dependency_gate") == EXPECTED_DER02_OBJECT_GATE_REL

    assert payload.get("allowed_operations") == [
        "regime_validity_symbolic_scope_declaration",
        "closure_coupling_placeholder_slot_binding",
        "assumption_slot_linkage",
        "cross_surface_dependency_consistency_checks",
    ...
    assert payload.get("required_dependency_slots") == [
        ...
        "der02_regime_closure_scaffold_artifact",
        "der02_theorem_body_scaffold_artifact",
        "der02_discharge_scaffold_artifact",
        "der02_object_surface_scaffold_artifact",
