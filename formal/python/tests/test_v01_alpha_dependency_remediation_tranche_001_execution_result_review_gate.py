...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_001_execution_result_review_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_001_execution_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche001ExecutionResultReview.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_consumes_execution() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_EXECUTION_RESULT_REVIEW_20260515_v0"
    )
    ...
    assert review["consumes_execution"] == (
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_selected_dependency() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["review_scope"] == (
        "DEPENDENCY_REMEDIATION_TRANCHE_001_EXECUTION_RESULT_REVIEW_ONLY_NO_RELEASE_PROMOTION"
    )
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_exact_evidence() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_classifies_conservatively() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["tranche_001_status_classification"] == TRANCHE_CLASSIFICATION
    assert review["tranche_001_release_blocker_status"] == (
        "still_blocking_pending_release_policy_adjudication"
    )
    assert review["remediation_evidence_accepted"] is True
    assert review["remediation_fully_satisfied"] is False
    assert review["blocker_movement_authorized"] is False
    assert review["expert_re_review_required"] is True
    assert review["release_policy_adjudication_required"] is True
    assert TRANCHE_CLASSIFICATION in review["classification_options_considered"]


def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_other_five_unmodified() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == (
        "tranche_001_release_policy_adjudication_packet_preparation_only"
    )
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "prepare_v01_alpha_dependency_remediation_tranche_001_release_policy_adjudication_packet": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_001_execution_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_001_execution_result_review_gate.py",
        OUTCOME_ID,
    ...
    assert "V01DependencyRemediationTranche001ExecutionResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_001_execution_result_review_does_not_promote_release"
        in index_text
    )
