...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche005BlockerMovementRegistrationResultReview.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_consumes_registration() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW_20260515_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_registered_true() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_exact_movement() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_preserves_evidence_policy_documentation_chain() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_accepts_formal_movement_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_005_formal_movement_accepted"] is True
    assert review["tranche_005_release_blocker_status"] == CANDIDATE_BLOCKER_STATUS
    assert review["tranche_005_dependency_policy_remediation_satisfied"] is True
    assert review["tranche_005_cleared_for_global_release_readiness"] is False
    assert review["global_release_readiness_still_blocked"] is True
    assert review["release_blocking_obligation_count_after_review"] == 2
    assert review["remaining_release_blocking_obligation_count_after_review"] == 2


def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_carries_posture() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_006_selected_yet"] is False


def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_remaining_two_unmodified() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert other[1]["status_carry_forward"] == TRANCHE_006_STATUS
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "prepare_v01_alpha_dependency_remediation_next_tranche_selection_packet_after_tranche_005_movement": "selected",
        "prepare_v01_alpha_dependency_remediation_tranche_006_execution_packet": "deferred",
        "pause_v01_alpha_release_readiness_due_to_retained_tranche_004_blocker": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        REVIEW_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW_20260515_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_005_blocker_movement_registration_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche005BlockerMovementRegistrationResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_005_blocker_movement_registration_result_review_does_not_promote_release"
        in index_text
    )
