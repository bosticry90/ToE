...
from formal.python.tools.qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_report import (
    ...
    build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_weak_strong_attempt_files_exist() -> None:
...
def test_qft_gr_weak_strong_attempt_consumes_review() -> None:
...
def test_qft_gr_weak_strong_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    review = _json(REVIEW_PATH)
    ...
    assert (
        attempt["weak_strong_conservation_comparison_scope_assumption_obstruction_identified"]
        is False
    )
    assert attempt["weak_strong_conservation_comparison_scope_assumption_inconclusive"] is False


def test_qft_gr_weak_strong_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["weak_strong_conservation_comparison_scope_reduction_contract"]
    ...
    assert attempt["selected_mathematical_regularity_assumption_row"] == SELECTED_ROW_ID
    assert attempt["selected_bounded_mathematical_regularity_assumption_row"] == SELECTED_ROW_ID
    assert attempt["selected_row_count"] == 1
    assert attempt["weak_strong_conservation_comparison_scope"] == SELECTED_ROW_OBJECT
    assert attempt["weak_conservation_scope"] == WEAK_CONSERVATION_SCOPE
    assert attempt["strong_conservation_scope"] == STRONG_CONSERVATION_SCOPE
    assert attempt["comparison_scope_boundaries"] == COMPARISON_SCOPE_BOUNDARIES
    assert contract["contract_id"] == WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_ID
    assert contract["assumption_id"] == SELECTED_ROW_ID
    assert contract["assumption_family"] == SELECTED_ASSUMPTION_FAMILY
    assert contract["regularity_condition"] == SELECTED_ROW_OBJECT
    assert contract["weak_scope"] == WEAK_CONSERVATION_SCOPE
    assert contract["strong_scope"] == STRONG_CONSERVATION_SCOPE
    assert contract["comparison_scope_boundaries"] == COMPARISON_SCOPE_BOUNDARIES
    assert contract["contract_status"] == (
        BOUNDED_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_STATUS
    )
    assert contract["required_future_proof_object"] == "weak_strong_conservation_comparison_regular_scope"
    assert len(attempt["execution_findings"]) == 4


def test_qft_gr_weak_strong_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert (
        attempt["weak_strong_conservation_comparison_scope_assumption_reduction_attempt_started"]
        is True
    )
    assert (
        attempt["weak_strong_conservation_comparison_scope_assumption_reduction_attempt_executed"]
        is True
    )
    assert (
        attempt["weak_strong_conservation_comparison_scope_assumption_reduced_by_attempt"]
        is True
    )
    assert attempt["weak_strong_conservation_comparison_scope_assumption_discharged"] is False
    assert (
        attempt[
            "weak_strong_conservation_comparison_scope_assumption_discharged_by_attempt"
        ]
        is False
    )
    assert attempt["weak_conservation_proved"] is False
    assert attempt["strong_conservation_proved"] is False
...
def test_qft_gr_weak_strong_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        "review_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result": "selected",
        "execute_qft_gr_distributional_pairing_regular_domain_assumption_reduction_attempt": "not_authorized_current_row",
        "execute_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_attempt": "not_authorized_current_row",
...
def test_qft_gr_weak_strong_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt(
