...
def test_bridge_toyhg_c6_pair_compatibility_feasibility_artifact_pointers_exist() -> None:
    repo_root = _repo_root_from_test_file(Path(__file__))

    artifact_rel = Path("formal/quarantine/feasibility/BRIDGE_TOYHG_C6_pair_compatibility_feasibility.json")
    artifact_path = repo_root / artifact_rel
    assert artifact_path.exists(), "Expected Toy-H/Toy-G C6 pair-compatibility feasibility artifact to exist"

    payload = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert payload.get("schema_version") == 1
    assert payload.get("bridge_id") == "BRIDGE_TICKET_TOYHG_0001"
    ...
    assert nodes, "Missing pytest_nodes evidence in pair-compatibility feasibility artifact"
    for node in nodes:
