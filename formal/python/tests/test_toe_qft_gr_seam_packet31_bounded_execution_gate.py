...
def test_qft_gr_seam_packet31_bounded_execution_authority_parity_and_invariance() -> None:
    ...
    for ref in refs:
        assert (ref in state_text) or (ref in inventory_text) or (ref in roadmap_text), (
            f"Missing packet31 execution pointer across authority surfaces: {ref}"
        )
        assert ref in roadmap_text, f"Missing packet31 execution pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_packet31 = _extract_token_from_surfaces(
        [state_text, inventory_text, roadmap_text],
        "TOE_QFT_GR_SEAM_PACKET31_STATUS_v0",
    )
    roadmap_packet31 = _extract_token_from_surfaces([roadmap_text], "TOE_QFT_GR_SEAM_PACKET31_STATUS_v0")
