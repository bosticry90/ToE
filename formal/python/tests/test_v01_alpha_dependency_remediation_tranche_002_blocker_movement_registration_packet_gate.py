...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_report import (
...
RESULT_REVIEW_PATH = (
    RELEASE_DIR
    / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_002_STATUS_ADJUDICATION_RESULT_REVIEW_20260515_v0.json"
)
PACKET_PATH = (
    RELEASE_DIR
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche002BlockerMovementRegistrationPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_consumes_status_result_review() -> None:
    packet = _json(PACKET_PATH)
    assert packet["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_002_BLOCKER_MOVEMENT_REGISTRATION_PACKET_20260515_v0"
    )
    ...
    assert packet["consumes_status_adjudication_result_review_pointer"] == (
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_002_STATUS_ADJUDICATION_RESULT_REVIEW_20260515_v0.json"
    )


def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_preserves_selected_dependency() -> None:
    packet = _json(PACKET_PATH)
    assert packet["packet_scope"] == (
        "PREPARE_TRANCHE_002_BLOCKER_MOVEMENT_REGISTRATION_PACKET_ONLY_NO_BLOCKER_MOVEMENT_OR_RELEASE_PROMOTION"
    )
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_preserves_candidate_and_proposes_movement() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert proposal["movement_scope"] == "tranche_002_only"
    assert proposal["tranche_001_status"] == TRANCHE_001_STATUS
    assert proposal["requires_result_review_before_execution"] is True


def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_preserves_evidence_policy_and_documentation() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_prepares_only() -> None:
    packet = _json(PACKET_PATH)
    proposal = packet["movement_proposal"]
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_remaining_blockers_tracked() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["next_action_scope"] == (
        "REVIEW_TRANCHE_002_BLOCKER_MOVEMENT_REGISTRATION_PACKET_RESULT_ONLY_NO_BLOCKER_MOVEMENT_OR_RELEASE_PROMOTION"
    )
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_result": "selected",
        "execute_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration": "deferred",
        "prepare_v01_alpha_release_readiness_adjudication_packet": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        PACKET_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_002_BLOCKER_MOVEMENT_REGISTRATION_PACKET_20260515_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_002_blocker_movement_registration_packet_gate.py",
        OUTCOME_ID,
        PROPOSED_MOVEMENT_TOKEN,
        NEXT_TARGET,
    ]
    ...
    assert "V01DependencyRemediationTranche002BlockerMovementRegistrationPacket" in index_text
    assert (
        "v01_dependency_remediation_tranche_002_blocker_movement_registration_packet_does_not_move_blocker"
        in index_text
    )
