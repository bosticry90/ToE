...
REPORT_PATH = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_RENORMALIZED_EXPECTATION_VALUE_SEMANTICS_RESULT_REVIEW_20260503_v0.json"
)
SOURCE_REPORT_PATH = (
    REPO_ROOT
    / "formal"
...
SURFACE_ID = "qft_gr_renormalized_expectation_value_semantics_result_review_v0"
PREVIOUS_TARGET = "review_qft_gr_renormalized_expectation_value_semantics_result"
SELECTED_NEXT_TARGET = "prepare_qft_gr_classical_source_admissibility_semantics_bounded_attack"
...
SOURCE_RESULT_TOKEN = "QFT_GR_RENORMALIZED_EXPECTATION_VALUE_SEMANTICS_SUPPLIED_ONLY"
REVIEW_RESULT_TOKEN = (
    "QFT_GR_RENORMALIZED_EXPECTATION_VALUE_SEMANTICS_RESULT_REVIEW_CONSUMED_SUPPLIED_ONLY"
)
REPORT_ID = "QFT_GR_RENORMALIZED_EXPECTATION_VALUE_SEMANTICS_RESULT_REVIEW_20260503_v0"
RETAINED_BLOCKER = (
...
def test_renormalized_expectation_value_result_review_surface_consumes_result() -> None:
    ...
    for token in {
        ...
        "QFTGRRenormalizedExpectationValueResultReviewDecision",
        "acceptSuppliedOnlyAndPrepareClassicalSourceAdmissibilitySemantics",
        "qft_gr_renorm_expectation_value_result_review_consumes_live_target_v0",
        "qft_gr_renorm_expectation_value_result_review_completed_v0",
        "qft_gr_renorm_expectation_value_result_review_accepts_supplied_only_v0",
        "qft_gr_renorm_expectation_value_result_review_state_only_refuted_v0",
        "qft_gr_renorm_expectation_value_result_review_retained_as_supplied_v0",
        "qft_gr_renorm_expectation_value_result_review_token_v0",
        "qft_gr_renorm_expectation_value_result_review_selected_decision_v0",
        "qft_gr_renorm_expectation_value_result_review_selected_next_target_v0",
    }:
        assert token in text


def test_renormalized_expectation_value_result_review_preserves_boundaries() -> None:
    text = _read(SURFACE_PATH)

    for theorem in {
        "qft_gr_renorm_expectation_value_result_review_same_lane_not_authorized_v0",
...
def test_renormalized_expectation_value_result_review_report_records_selection() -> None:
    ...
    assert report["review_result"] == (
        "supplied_only_renormalized_expectation_value_result_consumed"
    )
...
def test_registry_rotates_to_classical_source_admissibility_preparation() -> None:
    assert_current_target_consistent()
    assert_frontier_matches_registry()
    assert_forbidden_promotions_closed()
    assert_public_surfaces_match_registry()
    ...
    review = workstream("qft_gr_renormalized_expectation_value_semantics_result_review", payload)
    ...
    assert review["classical_source_admissibility_authorized"] == "no"
    ...
    assert active["latest_surface"] == "QFT_GR_CLASSICAL_SOURCE_ADMISSIBILITY_SEMANTICS_v0"
    assert active["renormalized_expectation_value_result_review_surface"] == SURFACE_EVIDENCE
    assert active["renormalized_expectation_value_result_review_report"] == REPORT_EVIDENCE
    assert active["renormalized_expectation_value_result_review_token"] == REVIEW_RESULT_TOKEN
    assert active["preparation_scope"] == (
        "classical_source_admissibility_semantics_interface_only"
    )
    assert active["classical_source_admissibility_semantics_authorized"] == (
    ...
    assert qft_gr["renormalized_expectation_value_semantics_result_review_status"] == (
        "completed"
    )
    assert qft_gr["renormalized_expectation_value_semantics_result_review_evidence"] == (
        SURFACE_EVIDENCE
    )
    assert qft_gr["renormalized_expectation_value_semantics_result_review_report"] == (
        REPORT_EVIDENCE
    )
    assert qft_gr["classical_source_admissibility_semantics_preparation_target"] == (
        SELECTED_NEXT_TARGET
    )
    assert qft_gr["classical_source_admissibility_semantics_authorized"] == (
        "supplied_only_retained"
    )
...
def test_public_surfaces_track_result_review_and_next_preparation() -> None:
    for path in [README_PATH, STATE_PATH, ROADMAP_PATH, STRICT_MAP_PATH]:
        ...
        assert "QFT_GR_RenormalizedExpectationValueSemanticsResultReview.lean" in text
    ...
    assert "INV-MATH-QFTGR-RENORMALIZED-EXPECTATION-VALUE-RESULT-REVIEW-v0" in (
        inventory_text
    )
    ...
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_renormalized_expectation_value_semantics_result_review_gate.py"
    )
