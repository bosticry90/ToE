...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_006_status_adjudication_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_status_adjudication_report.py"
)
...
LEAN_ADJUDICATION_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche006StatusAdjudication.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_consumes_result_review() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    assert adjudication["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_STATUS_ADJUDICATION_20260522_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_adjudicates_only_selected_dependency() -> None:
    adjudication = _json(ADJUDICATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_preserves_evidence() -> None:
    adjudication = _json(ADJUDICATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_preserves_policy_and_documentation_chain() -> None:
    adjudication = _json(ADJUDICATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_decides_narrow_status_question() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    ...
    assert adjudication["tranche_006_status_candidate"] == STATUS_DECISION
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_keeps_blocker_unmoved_pending_review() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    assert adjudication["tranche_006_release_blocker_status"] == (
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_carries_posture() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    ...
    assert adjudication["tranche_005_status"] == TRANCHE_005_STATUS
    assert adjudication["tranche_005_dependency"] == TRANCHE_005_DEPENDENCY
    assert adjudication["tranche_006_status"] == POLICY_CLASSIFICATION
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_remaining_blockers_tracked() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    rows = adjudication["release_blocking_obligations_carry_forward"]
    assert adjudication["release_blocking_obligation_count"] == 2
    ...
    assert adjudication["other_release_blocking_obligation_count"] == 1
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_forbidden_effects_false() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    ...
    assert adjudication["tranche_006_moved_or_cleared"] is False


def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_next_target() -> None:
    adjudication = _json(ADJUDICATION_PATH)
    ...
    assert adjudication["next_action_scope"] == (
        "REVIEW_TRANCHE_006_STATUS_ADJUDICATION_RESULT_ONLY_NO_BLOCKER_MOVEMENT_OR_RELEASE_PROMOTION"
    )
    assert {row["target"]: row["decision"] for row in adjudication["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_006_status_adjudication_result": "selected",
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_acceptance_and_determinism() -> None:
    adjudication = _json(ADJUDICATION_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_status_adjudication_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_status_adjudication_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006StatusAdjudication" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_status_adjudication_does_not_move_blocker"
        in index_text
    )
