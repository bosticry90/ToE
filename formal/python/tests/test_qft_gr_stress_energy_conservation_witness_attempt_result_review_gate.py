...
from formal.python.tools.qft_gr_stress_energy_conservation_witness_attempt_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_stress_energy_conservation_witness_attempt_result_review,
)
...
LEAN_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_StressEnergyConservationWitnessAttemptResultReview.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_stress_energy_conservation_witness_attempt_result_review_report.py"
)
...
def test_qft_gr_stress_energy_conservation_witness_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_stress_energy_conservation_witness_attempt_result_review_consumes_obstruction_attempt() -> None:
...
def test_qft_gr_stress_energy_conservation_witness_attempt_result_review_accepts_obstruction_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["attempt_result_reviewed"] is True
    assert review["conservation_obstruction_result_accepted"] is True
    assert review["obstruction_class"] == OBSTRUCTION_CLASS
    assert review["primary_missing_condition"] == "conservation"
    assert review["refinement_packet_preparation_authorized"] is True
    assert review["refinement_candidates"] == REFINEMENT_CANDIDATES


def test_qft_gr_stress_energy_conservation_witness_attempt_result_review_preserves_nonclaim_boundaries() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_stress_energy_conservation_witness_attempt_result_review_selects_one_refinement_packet_target() -> None:
    review = _json(DEFAULT_OUT)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_stress_energy_conservation_refinement_attempt": "deferred",
        "prepare_qft_gr_stress_energy_conservation_assumption_reduction_packet": "deferred",
        "close_qft_gr_seam": "not_authorized",
        "authorize_public_submission": "not_authorized",
    }


def test_qft_gr_stress_energy_conservation_witness_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_stress_energy_conservation_witness_attempt_result_review(
