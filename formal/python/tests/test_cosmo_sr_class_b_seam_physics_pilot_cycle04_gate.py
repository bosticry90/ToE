...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_sr_class_b_seam_physics_pilot_cycle04_v0.json"
PREV_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_sr_class_b_seam_physics_pilot_cycle03_v0.json"
...
def test_cosmo_sr_cycle04_artifacts_exist() -> None:
    assert DOC_PATH.exists(), "Missing COSMO-SR Cycle04 target doc."
    assert ARTIFACT_PATH.exists(), "Missing COSMO-SR Cycle04 artifact."
    assert PREV_ARTIFACT_PATH.exists(), "Missing COSMO-SR Cycle03 predecessor artifact."


def test_cosmo_sr_cycle04_doc_contains_required_tokens() -> None:
    text = _read(DOC_PATH)
    required_tokens = [
    ...
    assert not missing, "COSMO-SR Cycle04 doc missing required token(s): " + ", ".join(missing)


def test_cosmo_sr_cycle04_artifact_schema_and_predecessor_tieback() -> None:
    artifact = _json(ARTIFACT_PATH)
    assert artifact["artifact_id"] == "cosmo_sr_class_b_seam_physics_pilot_cycle04_v0"
    assert artifact["seam_id"] == "SEAM-COSMO-SR"
    assert artifact["class_token"] == "TOE_CK_CLASS_COMPATIBILITY_v0"
    assert artifact["status"] == "CRITERIA_AND_SEXTIC_EXCLUSION_PINNED_NONCLAIM"

    derived = artifact["derived_from"]
    assert derived["artifact_id"] == "cosmo_sr_class_b_seam_physics_pilot_cycle03_v0"
    assert derived["artifact_path"] == "formal/output/cosmo_sr_class_b_seam_physics_pilot_cycle03_v0.json"


def test_cosmo_sr_cycle04_low_z_sextic_improvement() -> None:
    artifact = _json(ARTIFACT_PATH)
    criteria = artifact["blocker_discharge_criteria"]

    assert criteria["token"] == "EXACT_SR_DOPPLER_MATCH_SEXTIC_IMPROVEMENT_REQUIRED"
    ...
    for sample in criteria["samples"]:
        ...
        assert beta_series4 == _series4(z)
        ...
        assert abs_delta_series6 <= abs_delta_series4
        assert abs_delta_series6 <= eps


def test_cosmo_sr_cycle04_high_z_exclusion_and_nonclaim_boundary() -> None:
    artifact = _json(ARTIFACT_PATH)
    exclusion = artifact["bounded_incompatibility_exclusion"]

    z = Fraction(exclusion["z"])
    beta_exact = Fraction(exclusion["beta_exact"])
    beta_series6 = Fraction(exclusion["beta_series6"])
    ...
    assert adjudication["token"] == "COSMO_SR_CLASS_B_SEAM_PHYSICS_PILOT_CYCLE04_ADJUDICATION"
