...
from formal.python.tools.v01_alpha_dependency_remediation_next_tranche_selection_packet_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_next_tranche_selection_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationNextTrancheSelectionPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_consumes_tranche_001_result_review() -> None:
    packet = _json(PACKET_PATH)
    assert packet["schema_id"] == "V01_ALPHA_DEPENDENCY_REMEDIATION_NEXT_TRANCHE_SELECTION_PACKET_20260515_v0"
    assert packet["packet_id"] == PACKET_ID
    assert packet["status"] == "ACTIVE_NONLIVE_NONCLAIM"
    assert packet["captured_at_utc"] == DEFAULT_CAPTURED_AT_UTC
    assert packet["accepted"] is True
    assert packet["outcome_id"] == OUTCOME_ID
    assert packet["consumes_tranche_001_result_review"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW_v0"
    )
    assert packet["consumes_tranche_001_result_review_pointer"] == (
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_preserves_tranche_001_status() -> None:
    packet = _json(PACKET_PATH)
    assert packet["tranche_001_status"] == TRANCHE_001_STATUS
    assert packet["tranche_001_formal_movement_accepted"] is True
    assert packet["tranche_001_dependency_policy_remediation_satisfied"] is True
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_tracks_remaining_five() -> None:
    packet = _json(PACKET_PATH)
    rows = packet["remaining_release_blocking_obligations"]
    assert packet["remaining_release_blocking_obligation_count"] == 5
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_selects_exactly_one_tranche() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_prepares_selection_only() -> None:
    packet = _json(PACKET_PATH)
    selected = packet["selected_next_remediation_tranche"]
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        ...
        "prepare_v01_alpha_dependency_remediation_tranche_002_execution_packet": "deferred",
        "prepare_v01_alpha_release_readiness_adjudication_packet": "deferred",
    }


def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_next_tranche_selection_packet_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_next_tranche_selection_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_next_tranche_selection_packet_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationNextTrancheSelectionPacket" in index_text
    assert (
        "v01_dependency_remediation_next_tranche_selection_packet_does_not_execute_remediation"
        in index_text
    )
