...
def _extract_target_id(text: str) -> str:
    match = re.search(r"\bTARGET-COSMO-BG-MICRO-11-[A-Z0-9-]+-v0\b", text)
    assert match is not None, "Missing Cycle-011 TARGET token in COSMO micro document."
    return match.group(0)


def _extract_assignment_value(text: str, token_name: str) -> str:
    match = re.search(rf"\b{re.escape(token_name)}\s*:\s*([^\n\r]+)", text)
    assert match is not None, f"Missing `{token_name}` assignment in COSMO micro-11 document."
...
def test_cosmo_target_references_micro11_and_gate() -> None:
    ...
    micro_text = _read(COSMO_MICRO11_PATH)
...
def test_cosmo_micro11_artifact_schema_and_token_alignment() -> None:
    micro_text = _read(COSMO_MICRO11_PATH)
    artifact_id = _extract_assignment_value(micro_text, "COSMO_BG_MICRO11_DRYRUN_RECONCILIATION_ARTIFACT_v0")
