...
ASSESSMENT_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET08_ASSESSMENT_v0.md"
...
def test_qft_gr_seam_packet09_bounded_execution_document_structure() -> None:
    ...
    for marker in required_markers:
        assert marker in text, f"Packet09 execution doc missing marker: {marker}"


def test_qft_gr_seam_packet09_bounded_execution_checkpoint_schema_and_alignment() -> None:
    artifact = _read_json(PACKET_CHECKPOINT_PATH)
    auth_artifact = _read_json(AUTH_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet09_bounded_execution_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1I_QFT_GR_SEAM_PACKET09_BOUNDED_EXECUTION"
    assert artifact.get("status") == "PACKET09_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"

    payload = artifact.get("payload", {})
    assert payload.get("packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET09_BOUNDED_EXECUTION_v0.md"
    ...
    assert auth_payload.get("authorization_decision", {}).get("decision_outcome") == "AUTHORIZE_PACKET09_BOUNDED_TARGET_v0"

    binding = payload.get("authorized_target_binding", {})
    assert binding.get("authorization_decision_outcome") == "AUTHORIZE_PACKET09_BOUNDED_TARGET_v0"
    ...
    assert binding.get("authorized_exact_target") == auth_payload.get("packet09_bounded_target", {}).get("exact_target")
    assert binding.get("execution_target_match") == "EXACT_MATCH_CONFIRMED_v0"

    agg = payload.get("aggregation_criterion", {})
    assert agg.get("aggregation_result") == "THRESHOLD_MET_v0"

    ac = payload.get("acceptance_criteria", {})
    assert ac.get("ac1_input_row_pass_state_coverage") == "PASS_v0"
    assert ac.get("ac2_aggregation_rule_explicitness") == "PASS_v0"
    assert ac.get("ac3_no_scalar_scope_expansion_or_backflow") == "PASS_v0"
...
def test_qft_gr_seam_packet09_bounded_execution_authority_parity_and_invariance() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet09 execution pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet09 execution pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_packet09 = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET09_STATUS_v0")
    roadmap_packet09 = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET09_STATUS_v0")
    assert state_packet09 == roadmap_packet09 == "EXECUTED_BOUNDED_TARGET_STEP_v0"
