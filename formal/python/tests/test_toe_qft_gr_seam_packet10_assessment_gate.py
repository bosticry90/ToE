...
ASSESSMENT_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet10_assessment_checkpoint_v0.json"
...
def test_qft_gr_seam_packet10_assessment_document_structure() -> None:
    ...
    required_markers = [
        ...
        "did packet10 satisfy its exact bounded target?",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet10 assessment doc missing marker: {marker}"


def test_qft_gr_seam_packet10_assessment_checkpoint_schema() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet10_assessment_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1M_QFT_GR_SEAM_PACKET10_ASSESSMENT"
    assert artifact.get("status") == "PACKET10_ASSESSMENT_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET10_ASSESSMENT_v0.md"
    assert payload.get("parent_packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET10_BOUNDED_EXECUTION_v0.md"
    ...
    assert packet_artifact.get("status") == "PACKET10_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"
    assert auth_artifact.get("status") == "PACKET10_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"
    ...
    packet11 = payload.get("packet11_decision", {})
    assert packet11.get("authorized") is True
    assert packet11.get("verdict") == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
...
def test_qft_gr_seam_packet10_assessment_chain_consistency_and_authority_parity() -> None:
