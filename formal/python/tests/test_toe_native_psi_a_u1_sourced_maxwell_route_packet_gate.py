...
from formal.python.tools.toe_native_psi_a_u1_sourced_maxwell_route_packet_report import (
    ...
    build_toe_native_psi_a_u1_sourced_maxwell_route_packet,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "toe_native_psi_a_u1_sourced_maxwell_route_packet_report.py"
)
...
def test_psi_a_u1_sourced_maxwell_route_packet_files_exist() -> None:
    for path in [
        CURRENT_CONSERVATION_PACKET_PATH,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_psi_a_u1_sourced_maxwell_route_packet_builds() -> None:
    consumed = _json(CURRENT_CONSERVATION_PACKET_PATH)
    packet = _json(DEFAULT_OUT)
    assert consumed["outcome_id"] == CURRENT_CONSERVATION_OUTCOME
    ...
    assert (
        packet["consumed_current_conservation_from_dirac_pair_packet_result"]
        == CURRENT_CONSERVATION_OUTCOME
    )
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == NEXT_TARGET_KIND
    assert build_toe_native_psi_a_u1_sourced_maxwell_route_packet() == packet


def test_psi_a_u1_sourced_maxwell_route_packet_records_bounded_route() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["A_variation_residual"].startswith("delta_A S_{psi A}")
    ...
    assert packet["conserved_source_condition"] == CONSERVED_SOURCE_CONDITION
    assert packet["sourced_maxwell_residual_zero"] == SOURCED_MAXWELL_RESIDUAL_ZERO
    assert packet["sourced_gauge_route"] == SOURCED_GAUGE_ROUTE
    assert packet["sourced_maxwell_route"] == SOURCED_GAUGE_ROUTE
    assert packet["bounded_route_shape"] == SOURCED_GAUGE_ROUTE
    assert packet["current_divergence_route"] == CURRENT_DIVERGENCE_ROUTE
    assert packet["sourced_gauge_route_status"] == SOURCED_GAUGE_ROUTE_STATUS
    assert packet["current_consistency_status"] == CURRENT_CONSISTENCY_STATUS
    assert packet["route_steps"] == ROUTE_STEPS
    assert packet["route_step_count"] == 4
    assert packet["assumptions"] == ASSUMPTIONS
    assert packet["assumption_count"] == 5
    assert packet["stress_energy_exchange_preview"] == STRESS_ENERGY_EXCHANGE_PREVIEW
    assert packet["possible_C_exchange_route"] == POSSIBLE_C_EXCHANGE_ROUTE
...
def test_psi_a_u1_sourced_maxwell_route_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    ...
    for phrase in [
        "bounded sourced-Maxwell-route packet only",
        "nabla_mu F^{mu nu} = J^nu",
        "J^nu = q psibar gamma^nu psi",
        "no full Maxwell closure",
        "no homogeneous Maxwell route beyond the existing F = dA context",
...
def test_psi_a_u1_sourced_maxwell_route_packet_rotates_to_exchange_obligation() -> None:
    ...
    assert consumed["sourced_maxwell_route_packet_result"] == OUTCOME_ID
    assert consumed["sourced_gauge_route"] == SOURCED_GAUGE_ROUTE
    assert consumed["source_current"] == SOURCE_CURRENT
    ...
    if is_current:
        ...
        assert active_row["consumed_sourced_maxwell_route_packet_result"] == OUTCOME_ID
        assert active_row["packet_result"] == "PENDING"
        assert active_row["stress_energy_and_exchange_obligation_packet_result"] == (
            "PENDING"
        )
        assert (
            active_row[
        ...
        assert active_row["stress_energy_and_exchange_obligation_packet_prepared"] == "no"
        assert active_row["sourced_maxwell_route_packet_result"] == OUTCOME_ID
        ...
        assert active_row["stress_energy_exchange_preview"] == STRESS_ENERGY_EXCHANGE_PREVIEW
        for key in [
            "sourced_maxwell_route_packet_prepared",
            "sourced_gauge_route_recorded",
            "current_consistent_sourced_gauge_route_recorded",
...
def test_psi_a_u1_sourced_maxwell_route_packet_lean_and_surface_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        OUTCOME_ID,
        PACKET_CLASSIFICATION,
        "ToeNativePsiAU1SourcedMaxwellRoutePacket",
        NEXT_TARGET,
...
def test_psi_a_u1_sourced_maxwell_route_packet_validation_policy_is_bounded() -> None:
    packet = _json(DEFAULT_OUT)
...
def test_psi_a_u1_sourced_maxwell_route_packet_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_toe_native_psi_a_u1_sourced_maxwell_route_packet_gate.py"
    )
