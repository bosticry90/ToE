...
SCORECARD_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_WORKSHEET_v0.md"
...
THRESHOLDS_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet44_reconsideration_numeric_thresholds_checkpoint_v0.json"
...
def test_packet44_reconsideration_scorecard_document_structure() -> None:
    text = _read(SCORECARD_DOC_PATH)
    required_markers = [
        "Worksheet ID:",
        "Parent packet44 numeric-threshold measurement protocol:",
        "## Canonical Inputs",
...
def test_packet44_reconsideration_scorecard_checkpoint_schema_and_alignment() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet44_reconsideration_scorecard_worksheet_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2ZA_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_WORKSHEET"
    assert artifact.get("status") == "PACKET44_RECONSIDERATION_SCORECARD_WORKSHEET_ACTIVE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("scorecard_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_WORKSHEET_v0.md"
    ...
    assert payload.get("parent_numeric_thresholds_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_NUMERIC_THRESHOLDS_v0.md"
...
def test_packet44_reconsideration_scorecard_authority_parity_and_freeze() -> None:
    ...
    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_STATUS_v0")
    ...
    state_outcome = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_OUTCOME_v0"
    )
    roadmap_outcome = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET44_RECONSIDERATION_SCORECARD_OUTCOME_v0")
    assert state_outcome == roadmap_outcome == "HOLD_RETAINED_PENDING_SCORECARD_EVIDENCE_v0"

    assert not PACKET44_AUTH_DOC_PATH.exists(), "Packet44 authorization doc must not exist under scorecard-governed hold"
    assert not PACKET44_AUTH_CHECKPOINT_PATH.exists(), "Packet44 authorization checkpoint must not exist under scorecard-governed hold"
