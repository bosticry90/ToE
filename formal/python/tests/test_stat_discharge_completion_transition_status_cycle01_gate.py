...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "stat_discharge_completion_transition_status_cycle01_v0.json"
COHERENCE_STATUS_ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "stat_failure_trigger_discharge_coherence_status_cycle01_v0.json"
)

EXPECTED_ARTIFACT_ID = "stat_discharge_completion_transition_status_cycle01_v0"
EXPECTED_COHERENCE_STATUS_ARTIFACT_ID = "stat_failure_trigger_discharge_coherence_status_cycle01_v0"
EXPECTED_COUPLING_GATE = "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
EXPECTED_STATUS_VALUE = "DISCHARGE_COMPLETION_TRANSITION_SCAFFOLD_PINNED_NONCLAIM"
ALLOWED_ADJUDICATION_STATUS_VALUES = {"NOT_PRESENT_v0", "ADJUDICATION_TRANSITION_SCAFFOLD_PINNED_NONCLAIM"}
EXPECTED_ARTIFACT_REL = "formal/output/stat_discharge_completion_transition_status_cycle01_v0.json"
EXPECTED_GATE_REL = "formal/python/tests/test_stat_discharge_completion_transition_status_cycle01_gate.py"
...
def test_stat_discharge_completion_transition_status_cycle01_gate() -> None:
    stat_text = _read(STAT_DOC_PATH)
    state_text = _read(STATE_PATH)
    roadmap_text = _read(ROADMAP_PATH)
    matrix = _read_json(MATRIX_PATH)
    artifact_json = _read_json(ARTIFACT_PATH)
    coherence_status_artifact_json = _read_json(COHERENCE_STATUS_ARTIFACT_PATH)

    assert "| `PILLAR-STAT` | `ACTIVE` |" in roadmap_text, (
        "STAT discharge-completion transition-status gate applies only after `PILLAR-STAT` activation."
    )
    stat_matrix = matrix.get("pillars", {}).get("PILLAR-STAT")
    assert isinstance(stat_matrix, dict), (
        "PILLAR-STAT matrix row must exist for discharge-completion transition-status gate."
    )
    ...
    assert artifact_json.get("payload_sha256") == computed_payload_sha, (
        "STAT discharge-completion transition-status cycle-01 payload_sha256 does not match canonical payload hash."
    )

    assert coherence_status_artifact_json.get("artifact_id") == EXPECTED_COHERENCE_STATUS_ARTIFACT_ID

    for token_name, expected in (
        ("STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_CYCLE01_ARTIFACT_v0", EXPECTED_ARTIFACT_ID),
        ("STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_CYCLE01_GATE_v0", EXPECTED_COUPLING_GATE),
        ("STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_v0", EXPECTED_STATUS_VALUE),
    ):
    ...
    for doc_text, doc_label in (
        ...
        adjudication_status = _extract_token(doc_text, "STAT_ADJUDICATION_TRANSITION_STATUS_v0")
        assert adjudication_status in ALLOWED_ADJUDICATION_STATUS_VALUES, (
            f"{doc_label} must keep the adjudication-transition status either pre-admission or adjudication-status pinned."
        )

    stat_sha = _extract_token(stat_text, "STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_CYCLE01_SHA256_v0")
    state_sha = _extract_token(state_text, "STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_CYCLE01_SHA256_v0")
    roadmap_sha = _extract_token(roadmap_text, "STAT_DISCHARGE_COMPLETION_TRANSITION_STATUS_CYCLE01_SHA256_v0")
    ...
    for doc_text, doc_label in (
        ...
        assert _extract_token(doc_text, "STAT_DERIVATION_COMPLETENESS_GATE_READINESS_PACKET_v0") == "PRESENT", (
            f"{doc_label} must preserve the readiness packet before discharge-completion transition-status admission."
        )
        assert _extract_token(
            ...
            f"{doc_label} must preserve the gate entry-status token before discharge-completion transition-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_SURFACE_STATUS_v0"
        ) == "ENTRY_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger discharge surface-status token before discharge-completion transition-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_THEOREM_SURFACE_STATUS_v0"
        ) == "THEOREM_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger theorem-surface status token before discharge-completion transition-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_OBJECT_SURFACE_STATUS_v0"
        ) == "OBJECT_SURFACE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger object-surface status token before discharge-completion transition-status admission."
        )
        assert _extract_token(
            doc_text, "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_v0"
        ) == "COHERENCE_SCAFFOLD_PINNED_NONCLAIM", (
            f"{doc_label} must preserve the failure-trigger coherence-status token before discharge-completion transition-status admission."
        )
        assert EXPECTED_ARTIFACT_REL in doc_text, (
            f"{doc_label} must pin the discharge-completion transition-status artifact path."
        )
        assert EXPECTED_GATE_REL in doc_text, (
            f"{doc_label} must pin the discharge-completion transition-status gate path."
        )

    for token_name, expected in (
        (
            "STAT_FAILURE_TRIGGER_DISCHARGE_COHERENCE_STATUS_CYCLE01_ARTIFACT_v0",
            EXPECTED_COHERENCE_STATUS_ARTIFACT_ID,
        ),
        (
    ...
    assert payload.get("checkpoint") == "stat_discharge_completion_transition_status_cycle01"
    assert payload.get("status") == "discharge_completion_transition_status_non_promotional"
    assert payload.get("transition_entry_scope") == [
        "discharge_completion_transition_status_placeholder_only",
        "failure_trigger_coherence_status_verified_before_transition_status",
        "adjudication_transition_scope_boundary_pinned_before_transition_status",
    ...
    assert payload.get("required_transition_status_inputs") == [
        ...
        "discharge_completion_transition_scope_boundary_pinned",
    ...
    assert payload.get("emitted_status_tokens") == [
        ...
        "STAT_ADJUDICATION_TRANSITION_STATUS_v0: NOT_PRESENT_v0",
    ]
    ...
    assert "- dependency ladder back to the failure-trigger coherence-status artifact is pinned before discharge-completion transition-status admission." in stat_text
