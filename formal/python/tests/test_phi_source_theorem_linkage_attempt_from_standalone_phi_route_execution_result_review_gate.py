...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_source_theorem_linkage_attempt_from_standalone_phi_route_execution_result_review_report.py"
)
...
def consumed_target() -> str:
    return "review_phi_source_theorem_linkage_attempt_from_standalone_phi_route_execution_result"


def test_phi_source_execution_result_review_files_exist() -> None:
    for path in [
        EXECUTION_OUT,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_phi_source_execution_result_review_accepts_local_linkage() -> None:
...
def test_phi_source_execution_result_review_records_route_and_boundaries() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["C_source_phi_zero_constructed"] is True
    assert review["C_source_phi_zero_derived"] is True
    assert review["C_source_phi_linkage_constructed"] is True
    assert review["lean_execution_marker_preserved"] is True
    assert review["json_execution_report_preserved"] is True
    assert review["focused_execution_gate_passed"] is True
...
def test_phi_source_execution_result_review_records_lean_status() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_source_execution_result_review_rotates_to_closeout_preparation() -> None:
    ...
    if is_current:
        ...
        assert closeout["review_result"] == OUTCOME_ID
        assert closeout["strict_review_result"] == STRICT_REVIEW_RESULT
        assert closeout["rule_promoted"] == "no"
        assert closeout["master_action_promoted"] == "no"


def test_phi_source_execution_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "PhiSourceTheoremLinkageAttemptFromStandalonePhiRouteExecutionResultReview",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        CLOSEOUT_OUTCOME,
        ...
        PLAIN_MEANING,
        LEAN_STATUS_WORDING_FOR_REVIEW,
        "PHI_SOURCE_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_ROUTE_EXECUTION_RESULT_REVIEW_OUTCOME_v0",
...
def test_phi_source_execution_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_source_theorem_linkage_attempt_from_standalone_phi_route_execution_result_review_gate.py"
    )
