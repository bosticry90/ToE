...
def test_stat_unlock_prerequisite_integrity_gate() -> None:
    ...
    assert stat_registry_entry.get("mode") == "CLOSED_HANDOFF", "PILLAR-STAT must remain in CLOSED_HANDOFF mode."
    assert stat_registry_entry.get("expected_matrix_status") == "CLOSED"
    assert stat_registry_entry.get("completion_token") == "PILLAR_STAT_FULL_DERIVATION_DISCHARGE_ADJUDICATION"
    assert stat_registry_entry.get("completion_value") == "DISCHARGED_v0_BOUNDED"
    assert stat_registry_entry.get("roadmap_proceed_token") == "PROCEED_GATE_STAT"
    assert stat_registry_entry.get("roadmap_proceed_prefix") == "ALLOWED_"
    assert stat_registry_entry.get("expected_global_focus") == "PILLAR-COSMO"
    assert stat_registry_entry.get("expected_global_lane") == "TARGET-COSMO-BG-PLAN"

    assert _extract_token(stat_plan_text, "PILLAR_STAT_FULL_DERIVATION_DISCHARGE_ADJUDICATION") == "DISCHARGED_v0_BOUNDED"
    assert _extract_token(roadmap_text, "PROCEED_GATE_STAT").startswith("ALLOWED_")
    assert _extract_token(state_text, "NEXT_PILLAR_FOCUS_v0") == "PILLAR-COSMO"
    assert _extract_token(state_text, "NEXT_PILLAR_PRIMARY_LANE_v0") == "TARGET-COSMO-BG-PLAN"
