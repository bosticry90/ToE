...
from formal.python.tools.qft_gr_renormalization_scope_assumption_reduction_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_renormalization_scope_assumption_reduction_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_renormalization_scope_assumption_reduction_packet_result_review_report.py"
)
...
def test_qft_gr_renormalization_scope_packet_result_review_files_exist() -> None:
...
def test_qft_gr_renormalization_scope_packet_result_review_consumes_packet() -> None:
    ...
    assert (
        review["consumes_qft_gr_renormalization_scope_assumption_reduction_packet"]
        == PACKET_ID
    )
...
def test_qft_gr_renormalization_scope_packet_result_review_confirms_row002() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["renormalization_scope_status_tokens"] == [
        "required",
        "missing",
        "candidate_reducible",
    ]
    assert review["renormalization_scope"] == RENORMALIZATION_SCOPE
    assert review["renormalization_scope_object"] == RENORMALIZATION_SCOPE
    assert review["renormalization_scope_status"] == RENORMALIZATION_SCOPE_STATUS
    assert review["scope_boundaries"] == SCOPE_BOUNDARIES
...
def test_qft_gr_renormalization_scope_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert review["renormalization_scope_assumption_discharged"] is False
    assert review["renormalization_scope_assumption_reduced_by_review"] is False
...
def test_qft_gr_renormalization_scope_packet_result_review_selects_one_target() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_renormalization_scope_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalization_scope_assumption_reduction_packet_result_review(
