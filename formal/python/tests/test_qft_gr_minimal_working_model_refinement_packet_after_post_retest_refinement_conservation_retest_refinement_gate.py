...
from formal.python.tools.qft_gr_minimal_working_model_refinement_packet_after_post_retest_refinement_conservation_retest_refinement_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_minimal_working_model_refinement_packet_after_post_retest_refinement_conservation_retest_refinement,
)
...
TOOL_PATH = (
    ...
    / (
        ...
        "refinement_conservation_retest_refinement_report.py"
    )
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    ...
    / (
        ...
        "ConservationRetestRefinement.lean"
    )
)
...
def test_post_retest_refinement_conservation_retest_refinement_packet_records_obstruction() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["obstruction_class"] == OBSTRUCTION_CLASS
    assert packet["repeated_inconclusive_signal_count"] == 3
    assert len(packet["why_refinement_not_immediate_retest_or_promotion"]) >= 5
    ...
    assert packet["current_weak_pairing_domain_id"] == "toy_weak_pairing_domain_v3_candidate"
    assert packet["current_regular_context_id"] == "toy_regular_context_v3_candidate"
    assert packet["current_test_function_class_id"] == (
        "toy_conservation_test_function_class_v2_candidate"
    )
    assert packet["current_candidate_source_definition_id"] == (
    ...
    assert packet["proposed_regular_context_revision"] == "toy_regular_context_v4_candidate"
    assert packet["proposed_test_function_class_revision"] == (
        "toy_conservation_test_function_class_v3_candidate"
    )
    assert packet["proposed_candidate_source_definition_revision"] == (
...
def test_post_retest_refinement_conservation_retest_refinement_packet_selects_review_only() -> None:
    ...
    assert (
        ...
        == "not_authorized_without_model_delta"
    )
    assert (
        decisions[
...
def test_post_retest_refinement_conservation_retest_refinement_packet_deterministic() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_working_model_refinement_packet_after_post_retest_refinement_conservation_retest_refinement(
...
def test_post_retest_refinement_conservation_retest_refinement_packet_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_working_model_refinement_packet_after_post_retest_refinement_conservation_retest_refinement_gate.py"
    )
