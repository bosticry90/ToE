...
REVIEW_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_REVIEW_v0.md"
REVIEW_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet50_eligibility_review_checkpoint_v0.json"
...
def test_qft_gr_seam_packet50_eligibility_review_document_structure() -> None:
    ...
    required_markers = [
        ...
        "stagnation_or_semantic_reencoding_risk: UNRESOLVED_PENDING_CONCRETE_PACKET50_TARGET_v0",
        "remaining_gap_still_narrower_than_objective: SATISFIED_v0",
        "## Packet49 Readiness Assessment",
        "current_packet50_gain_statement_status: MISSING_v0",
        "current_packet50_stagnation_clearance_status: NOT_YET_DEMONSTRATED_v0",
        "current_packet50_convergence_binding_status: INCOMPLETE_v0",
        "## Disposition Decision",
        "disposition_hold: ACTIVE",
        "review_decision_outcome: HOLD_PACKET50_PENDING_CONCRETE_SEAM_LEVEL_GAIN_EVIDENCE_v0",
        ...
        "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_GATE_v0: REQUIRED_PACKET50_ELIGIBILITY_REVIEW_SCHEMA_AND_FREEZE_ENFORCEMENT",
        "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_ARTIFACT_v0: toe_qft_gr_seam_packet50_eligibility_review_checkpoint_v0",
    ]
    for marker in required_markers:
        assert marker in text, f"Packet49 eligibility review doc missing marker: {marker}"


def test_qft_gr_seam_packet50_eligibility_review_checkpoint_schema_and_hold_disposition() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet50_eligibility_review_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2U_QFT_GR_SEAM_PACKET50_ELIGIBILITY_REVIEW"
    assert artifact.get("status") == "PACKET50_ELIGIBILITY_REVIEW_COMPLETE_HOLD_v0"

    payload = artifact.get("payload", {})
    assert payload.get("review_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_REVIEW_v0.md"
    ...
    assert questions.get("stagnation_or_semantic_reencoding_risk") == "UNRESOLVED_PENDING_CONCRETE_PACKET50_TARGET_v0"
    assert questions.get("remaining_gap_still_narrower_than_objective") == "SATISFIED_v0"

    readiness = payload.get("packet50_readiness_assessment", {})
    assert readiness.get("current_packet50_gain_statement_status") == "MISSING_v0"
    assert readiness.get("current_packet50_stagnation_clearance_status") == "NOT_YET_DEMONSTRATED_v0"
    assert readiness.get("current_packet50_convergence_binding_status") == "INCOMPLETE_v0"
    ...
    assert disposition.get("review_decision_outcome") == "HOLD_PACKET50_PENDING_CONCRETE_SEAM_LEVEL_GAIN_EVIDENCE_v0"
...
def test_qft_gr_seam_packet50_eligibility_review_authority_parity_and_freeze_enforcement() -> None:
    ...
    for ref in refs:
        assert ref in state_text or ref in inventory_text, (
            f"Missing packet50 eligibility review pointer in compact-State or central inventory: {ref}"
        )
        assert ref in roadmap_text, f"Missing packet50 eligibility review pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_STATUS_v0")
    ...
    state_disposition = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_DISPOSITION_v0"
    )
    roadmap_disposition = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET50_ELIGIBILITY_DISPOSITION_v0")
    assert state_disposition == roadmap_disposition == "HOLD_v0"

    assert not PACKET50_AUTH_DOC_PATH.exists(), "Packet49 authorization doc must not exist during hold disposition"
    assert not PACKET50_AUTH_CHECKPOINT_PATH.exists(), "Packet49 authorization checkpoint must not exist during hold disposition"
