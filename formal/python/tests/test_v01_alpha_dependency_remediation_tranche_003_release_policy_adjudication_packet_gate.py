...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche003ReleasePolicyAdjudicationPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_consumes_result_review() -> None:
    ...
    assert packet["consumes_audit_result_review_pointer"] == (
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_AUDIT_RESULT_REVIEW_20260515_v0.json"
    )
    assert packet["source_audit"] == "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_AUDIT_v0"


def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_preserves_selected_dependency() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_preserves_evidence() -> None:
    packet = _json(PACKET_PATH)
    assert packet["lean_audit_target"]["lean_target"] == LEAN_TARGET
    assert packet["lean_audit_target"]["command"] == (
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_defines_policy_question_without_deciding() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert "standard Lean axiom posture of tranche 003" in packet["expert_re_review_requirement"]
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_keeps_blockers_tracked() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["next_action_scope"] == (
        "REVIEW_TRANCHE_003_RELEASE_POLICY_ADJUDICATION_PACKET_ONLY_NO_POLICY_DECISION"
    )
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_result": "selected",
        "execute_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication": "deferred",
        "prepare_v01_alpha_release_readiness_adjudication_packet": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_003_release_policy_adjudication_packet_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche003ReleasePolicyAdjudicationPacket" in index_text
    assert (
        "v01_dependency_remediation_tranche_003_release_policy_adjudication_packet_does_not_make_policy_decision"
        in index_text
    )
