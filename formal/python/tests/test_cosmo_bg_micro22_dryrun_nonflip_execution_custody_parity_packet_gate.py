...
COSMO_MICRO22_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_22_DRYRUN_NONFLIP_EXECUTION_CUSTODY_PARITY_PACKET_v0.md"
COSMO_MICRO22_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro22_dryrun_nonflip_execution_custody_parity_packet_cycle01_v0.json"
...
def test_cosmo_micro22_doc_contains_required_headers_and_tokens() -> None:
    ...
    assert not missing_headers, "COSMO micro-22 document is missing required header(s): " + ", ".join(missing_headers)
    ...
    assert not missing_tokens, "COSMO micro-22 document is missing required token(s): " + ", ".join(missing_tokens)


def test_cosmo_micro22_artifact_schema_and_scope_boundary() -> None:
    payload = _read_json(COSMO_MICRO22_ARTIFACT_PATH)

    assert payload.get("artifact_id") == "cosmo_bg_micro22_dryrun_nonflip_execution_custody_parity_packet_cycle01_v0"
    ...
    assert boundary_statement == "DRYRUN_NONFLIP_LANE_ONLY_EXECUTION_CUSTODY_PARITY_NO_ADJUDICATION_FLIP_NO_COMPARATOR_AUTHORIZATION"


def test_cosmo_micro22_forbidden_tokens_not_present() -> None:
    doc_text = _read(COSMO_MICRO22_PATH)
    artifact_text = _read(COSMO_MICRO22_ARTIFACT_PATH)
    forbidden_tokens = [
...
def test_cosmo_micro22_cross_surface_pointers_are_complete() -> None:
    matrix = _read_json(MATRIX_PATH)
    cosmo = matrix.get("pillars", {}).get("PILLAR-COSMO")
    assert isinstance(cosmo, dict), "PILLAR-COSMO matrix row must exist."

    assert cosmo.get("dryrun_nonflip_execution_custody_parity_packet_doc") == "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_22_DRYRUN_NONFLIP_EXECUTION_CUSTODY_PARITY_PACKET_v0.md"
    assert cosmo.get("dryrun_nonflip_execution_custody_parity_packet_gate") == "formal/python/tests/test_cosmo_bg_micro22_dryrun_nonflip_execution_custody_parity_packet_gate.py"
    assert cosmo.get("dryrun_nonflip_execution_custody_parity_packet_policy") == "CYCLE08_09_10_11_12_13_14_15_16_17_18_19_20_21_DRYRUN_NONFLIP_EXECUTION_CUSTODY_PARITY_LOCK_REQUIRED_NO_STATUS_FLIP_NO_COMPARATOR_AUTHORIZATION"
    ...
    assert not missing_state, "State missing dryrun nonflip execution custody parity token(s): " + ", ".join(missing_state)
    ...
    assert not missing_target, "COSMO target missing dryrun nonflip execution custody parity token(s): " + ", ".join(missing_target)
    ...
    assert not missing_rollup, "Rollup gate missing dryrun nonflip execution custody parity cross-pin token(s): " + ", ".join(missing_rollup)