...
EVALUATION_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet53_reconsideration_scorecard_evaluation_cycle01_checkpoint_v0.json"
...
def test_packet53_scorecard_cycle01_checkpoint_schema_and_hold_result() -> None:
    artifact = _read_json(EVALUATION_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet53_reconsideration_scorecard_evaluation_cycle01_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_2ZB_QFT_GR_SEAM_PACKET53_RECONSIDERATION_SCORECARD_EVALUATION_CYCLE01"
    assert artifact.get("status") == "PACKET53_RECONSIDERATION_SCORECARD_EVALUATION_CYCLE01_COMPLETE_HOLD_v0"

    payload = artifact.get("payload", {})
    assert payload.get("cycle_id") == "packet53_reconsideration_cycle01_baseline_packet39_to_packet40"
    assert payload.get("formula_version") == "packet53_measurement_protocol_v0"
...
def test_packet53_scorecard_cycle01_authority_parity_and_freeze() -> None:
    ...
    state_status = _extract_token_from_compact_state_or_inventory(
        state_text, inventory_text, "TOE_QFT_GR_SEAM_PACKET53_RECONSIDERATION_SCORECARD_CYCLE01_STATUS_v0"
    )
    roadmap_status = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET53_RECONSIDERATION_SCORECARD_CYCLE01_STATUS_v0")
    assert state_status == roadmap_status == "EVALUATED_HOLD_RETAINED_MISSING_NUMERIC_INPUTS_v0"

    assert not PACKET53_AUTH_DOC_PATH.exists(), "Packet49 authorization doc must not exist after cycle01 scorecard evaluation"
    assert not PACKET53_AUTH_CHECKPOINT_PATH.exists(), "Packet49 authorization checkpoint must not exist after cycle01 scorecard evaluation"
