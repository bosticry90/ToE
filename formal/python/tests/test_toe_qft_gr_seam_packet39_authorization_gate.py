...
def test_qft_gr_seam_packet39_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert any(ref in text for text in (state_text, inventory_text, roadmap_text)), (
            f"Missing packet39 authorization pointer across authority surfaces: {ref}"
        )
    ...
    state_auth = _extract_token_from_surfaces(
        ...
        "TOE_QFT_GR_SEAM_PACKET39_AUTHORIZATION_STATUS_v0",
    )
    roadmap_auth = _extract_token_from_surfaces([roadmap_text], "TOE_QFT_GR_SEAM_PACKET39_AUTHORIZATION_STATUS_v0")
