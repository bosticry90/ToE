...
DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT69_v0.md"
)


def _read() -> str:
    assert DOC_PATH.exists(), f"Missing Increment69 doc: {DOC_PATH}"
    return DOC_PATH.read_text(encoding="utf-8")


def test_increment69_doc_exists_and_anchors() -> None:
    text = _read()
...
def test_increment69_status_and_nonclaim_tokens_present() -> None:
    text = _read()

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT69_STATUS_v0: T-CONDITIONAL_BOUNDED_NONCLAIM" in text
    assert f"QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT69_DELTA_v0: {REQUIRED_DELTA}" in text
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT69_ADJUDICATION: DISCHARGED_v0_BOUNDED" in text
...
def test_increment69_template_and_governance_pointers_present() -> None:
    text = _read()
