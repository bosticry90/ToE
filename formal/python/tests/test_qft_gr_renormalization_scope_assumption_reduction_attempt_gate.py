...
from formal.python.tools.qft_gr_renormalization_scope_assumption_reduction_attempt_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_renormalization_scope_assumption_reduction_attempt,
)
...
LEAN_ATTEMPT_PATH = (
    ...
    / "ToeFormal"
    / "Bridges"
    / "QFT_GR_RenormalizationScopeAssumptionReductionAttempt.lean"
)
TOOL_PATH = (
    REPO_ROOT
    / "formal"
    / "python"
    / "tools"
    / "qft_gr_renormalization_scope_assumption_reduction_attempt_report.py"
)
SURFACES_PATH = (
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_files_exist() -> None:
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_consumes_review() -> None:
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_records_one_classification() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["renormalization_scope_assumption_reduced_pending_result_review"] is True
    assert attempt["renormalization_scope_assumption_obstruction_identified"] is False
    assert attempt["renormalization_scope_assumption_inconclusive"] is False


def test_qft_gr_renormalization_scope_assumption_reduction_attempt_reduces_selected_row_only() -> None:
    attempt = _json(DEFAULT_OUT)
    contract = attempt["renormalization_scope_reduction_contract"]
    ...
    assert attempt["accepted_prior_renormalization_assumption_row"] == ACCEPTED_PRIOR_ROW
    assert attempt["selected_renormalization_assumption_row"] == SELECTED_ROW_ID
    assert contract["contract_id"] == RENORMALIZATION_SCOPE_CONTRACT_ID
    ...
    assert contract["renormalization_scope_object"] == RENORMALIZATION_SCOPE
    assert contract["scope_boundaries"] == SCOPE_BOUNDARIES
    assert contract["contract_status"] == BOUNDED_SCOPE_CONTRACT_STATUS
    assert contract["source_packet_scope_status"] == RENORMALIZATION_SCOPE_STATUS
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    assert attempt["renormalization_scope_assumption_reduced_by_attempt"] is True
    assert attempt["renormalization_scope_assumption_discharged"] is False
    assert attempt["renormalization_scope_assumption_discharged_by_attempt"] is False
    assert (
        attempt["renormalization_scope_assumption_reduced_or_discharged_by_implication"]
        is False
    )
    assert attempt["renormalization_scope_discharged_by_implication"] is False
    assert attempt["renormalization_scope_claimed_as_conservation_proof"] is False
    assert attempt["renormalization_scope_claimed_as_conservation_source"] is False
    assert attempt["renormalization_scope_claimed_as_source_admissibility"] is False
    assert attempt["renormalization_scope_claimed_as_bianchi_compatibility"] is False
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_selects_one_review_target() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        ...
        "prepare_qft_gr_renormalized_expectation_domain_assumption_reduction_packet": "deferred",
        "discharge_qft_gr_renormalization_scope_assumption": "not_authorized",
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_deterministic_and_pinned() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalization_scope_assumption_reduction_attempt(
