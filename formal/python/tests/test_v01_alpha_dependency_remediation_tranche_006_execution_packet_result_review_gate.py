...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_report import (
...
PACKET_PATH = (
    RELEASE_DIR / "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_EXECUTION_PACKET_20260515_v0.json"
)
RESULT_REVIEW_PATH = (
    RELEASE_DIR
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_report.py"
)
...
LEAN_RESULT_REVIEW_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche006ExecutionPacketResultReview.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_consumes_packet() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0"
    )
    ...
    assert review["consumes_tranche_006_execution_packet"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_EXECUTION_PACKET_v0"
    )
    assert review["consumes_tranche_006_execution_packet_pointer"] == (
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_EXECUTION_PACKET_20260515_v0.json"
    )


def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_preserves_posture() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert review["tranche_006_status"] == "lean_dependency_audit_scope_accepted_pending_execution"
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_selects_tranche_006_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_accepts_exact_audit_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    target = review["lean_dependency_audit_target"]
    required = review["required_evidence_surface"]
    assert review["tranche_006_execution_packet_accepted"] is True
    assert review["tranche_006_lean_dependency_audit_scope_accepted"] is True
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_tracks_current_ledger() -> None:
    review = _json(RESULT_REVIEW_PATH)
    ...
    assert selectable[0]["dependency_finding_id"] == SELECTED_FINDING_ID
    assert selectable[0]["dependency"] == SELECTED_DEPENDENCY
    assert review["tranche_006_obligation_carry_forward"]["dependency_finding_id"] == (
        SELECTED_FINDING_ID
    )


def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_authorizes_audit_only() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["execution_packet_result_review_classification"] == (
        RESULT_REVIEW_CLASSIFICATION
    )
    assert review["tranche_006_audit_execution_authorized"] is True
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_forbidden_effects_false() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_next_target() -> None:
    review = _json(RESULT_REVIEW_PATH)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selected_next_target_kind"] == "tranche_006_audit_execution_only"
    ...
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        "execute_v01_alpha_dependency_remediation_tranche_006_audit": "selected",
        "execute_v01_alpha_dependency_remediation_tranche_006": "deferred",
        "pause_v01_alpha_release_readiness_due_to_retained_tranche_004_blocker": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_acceptance_and_determinism() -> None:
    review = _json(RESULT_REVIEW_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_is_pinned() -> None:
    roadmap_text = _read(PHYSICS_ROADMAP_PATH)
    refs = [
        REVIEW_ID,
        "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_006_EXECUTION_PACKET_RESULT_REVIEW_20260515_v0.json",
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_006_execution_packet_result_review_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche006ExecutionPacketResultReview" in index_text
    assert (
        "v01_dependency_remediation_tranche_006_execution_packet_result_review_does_not_execute_audit"
        in index_text
    )
