...
from formal.python.tools.qft_gr_minimal_working_model_conservation_retest_attempt_report import (
    ...
    RETEST_STATUS,
    SCHEMA_ID,
    build_qft_gr_minimal_working_model_conservation_retest_attempt,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_minimal_working_model_conservation_retest_attempt_report.py"
)
LEAN_ATTEMPT_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Derivation"
    / "QFTGRMinimalWorkingModelConservationRetestAttempt.lean"
)
...
def test_minimal_working_model_conservation_retest_attempt_files_exist() -> None:
...
def test_minimal_working_model_conservation_retest_attempt_consumes_packet_review() -> None:
...
def test_minimal_working_model_conservation_retest_attempt_executes_inconclusive_retest() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["bounded_conservation_retest_attempt_executed"] is True
...
def test_minimal_working_model_conservation_retest_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
    for key in [
...
def test_minimal_working_model_conservation_retest_attempt_selects_result_review_only() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_minimal_working_model_conservation_retest_attempt_updates_live_target() -> None:
    ...
    assert attempt_workstream["bounded_conservation_retest_attempt_executed"] == "yes"
    ...
    assert active_workstream["conservation_retest_attempt_consumed"] == "yes"
    assert active_workstream["conservation_retest_attempt_result_review_executed"] == "no"
    assert active_workstream["conservation_retest_executed"] == "yes"
    assert active_workstream["retest_inconclusive"] == "yes"
...
def test_minimal_working_model_conservation_retest_attempt_deterministic() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_working_model_conservation_retest_attempt(
...
def test_minimal_working_model_conservation_retest_attempt_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_working_model_conservation_retest_attempt_gate.py"
    )
