...
from formal.python.tools.qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_report.py"
)
...
def test_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_files_exist() -> None:
...
def test_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_consumes_packet() -> None:
    ...
    assert review[
        "consumes_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet"
    ] == PACKET_ID
...
def test_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_accepts_blocker_only() -> None:
    review = _json(DEFAULT_OUT)
    assert review["selected_blocker"] == PRIMARY_BLOCKER
    assert review["primary_blocker"] == PRIMARY_BLOCKER
    assert review["insufficient_assumptions_blocker_accepted"] is True
    assert review["assumption_reduction_packet_preparation_authorized"] is True
...
def test_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
...
def test_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_selects_one_assumption_reduction_target() -> None:
    review = _json(DEFAULT_OUT)
    assert review["selected_next_target"] == NEXT_TARGET
    assert review["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in review["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "review_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result": "consumed",
...
def test_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_covariant_conservation_proof_object_obstruction_refinement_packet_result_review(
    ...
    for token in [REVIEW_ID, OUTCOME_ID, RESULT_REVIEW_CLASSIFICATION, PRIMARY_BLOCKER, NEXT_TARGET]:
