...
def test_redundancy_control_seam_family_index_shape() -> None:
    payload = _json(INDEX_PATH)

    assert payload.get("schema_id") == "REDUNDANCY_CONTROL_SEAM_FAMILY_INDEX_20260409_v0"
    assert payload.get("status") == "ACTIVE_NONLIVE_NONCLAIM_PILOT"
    assert payload.get("pilot_scope") == "ONE_SEAM_FAMILY"
    ...
    family = families[0]
    assert family.get("family_id") == "SEAM_QM_STAT_CLASS_B_PHYSICS_PILOT"
...
def test_redundancy_control_seam_family_index_state_tokens_present() -> None:
    ...
    required = [
        "REDUNDANCY_CONTROL_SEAM_PILOT_STATUS_v0: ACTIVE_ONE_FAMILY_NONLIVE_NONCLAIM",
        "REDUNDANCY_CONTROL_SEAM_PILOT_INDEX_v0: formal/output/reports/redundancy_control_seam_family_index_20260409_v0.json",
        "REDUNDANCY_CONTROL_SEAM_PILOT_RULE_v0: MISSING_OWNER_OR_RETENTION_OR_ARCHIVE_OR_PARITY_DEPENDENCIES_IS_HARD_FAIL",
