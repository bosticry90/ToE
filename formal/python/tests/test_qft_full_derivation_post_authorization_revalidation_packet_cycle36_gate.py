...
ARTIFACT_PATH = (
    ...
    / "formal"
    / "output"
    / "qft_full_derivation_post_authorization_revalidation_packet_cycle36_v0.json"
)

CYCLE36_PROGRESS_TOKEN = "QFT_FULL_DERIVATION_PROGRESS_CYCLE36_v0: POST_AUTHORIZATION_REVALIDATION_PACKET_LOCK_PINNED"
POST_AUTH_REVALIDATION_GATE_TOKEN = (
...
ARTIFACT_POINTER = "formal/output/qft_full_derivation_post_authorization_revalidation_packet_cycle36_v0.json"
CYCLE36_GATE_PATH = "formal/python/tests/test_qft_full_derivation_post_authorization_revalidation_packet_cycle36_gate.py"
...
def test_qft_cycle36_post_auth_revalidation_artifacts_exist() -> None:
    ...
    assert ARTIFACT_PATH.exists(), "Missing cycle-36 post-authorization revalidation packet artifact."


def test_qft_cycle36_post_auth_revalidation_tokens_are_pinned_in_qft_docs() -> None:
    evol_text = _read(QFT_EVOL_TARGET_PATH)
    discharge_text = _read(QFT_FULL_DISCHARGE_TARGET_PATH)

    for token in [
        CYCLE36_PROGRESS_TOKEN,
        POST_AUTH_REVALIDATION_GATE_TOKEN,
        POST_AUTH_REVALIDATION_SCOPE_TOKEN,
        CYCLE36_GATE_PATH,
    ]:
        assert token in evol_text, f"QFT evolution umbrella target missing cycle-36 token `{token}`."
        assert token in discharge_text, f"QFT full-derivation discharge target missing cycle-36 token `{token}`."

    for token in [ARTIFACT_TOKEN, ARTIFACT_POINTER]:
        assert token in discharge_text, f"QFT full-derivation discharge target missing cycle-36 packet token `{token}`."
...
def test_qft_cycle36_post_auth_revalidation_artifact_payload_is_consistent() -> None:
    ...
    assert payload["artifact_id"] == "qft_full_derivation_post_authorization_revalidation_packet_cycle36_v0"
    assert payload["target_id"] == "TARGET-QFT-FULL-DERIVATION-DISCHARGE-v0"
    assert payload["progress_token"] == CYCLE36_PROGRESS_TOKEN
    ...
    assert required_bundle_tokens.issubset(set(payload.get("bundle_tokens", []))), (
        "Cycle-36 artifact bundle payload must contain required dependency and adjudication-lock tokens."
    )


def test_qft_cycle36_post_auth_revalidation_does_not_flip_adjudication() -> None:
    discharge_text = _read(QFT_FULL_DISCHARGE_TARGET_PATH)
