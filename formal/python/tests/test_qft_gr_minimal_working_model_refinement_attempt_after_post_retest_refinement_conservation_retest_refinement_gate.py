...
from formal.python.tools.qft_gr_minimal_working_model_refinement_attempt_after_post_retest_refinement_conservation_retest_refinement_report import (
    ...
    SCHEMA_ID,
    build_qft_gr_minimal_working_model_refinement_attempt_after_post_retest_refinement_conservation_retest_refinement,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_minimal_working_model_refinement_attempt_after_post_retest_refinement_conservation_retest_refinement_report.py"
)
LEAN_ATTEMPT_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Derivation"
    / "QFTGRMinimalWorkingModelRefinementAttemptAfterPostRetestRefinementConservationRetestRefinement.lean"
)
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_files_exist() -> None:
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_consumes_review() -> None:
    ...
    assert review["review_id"] == PACKET_RESULT_REVIEW_ID
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_executes_authorized_obstruction_refinement_only() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert attempt["model_refinement_executed"] is True
    assert attempt["obstruction_class"] == OBSTRUCTION_CLASS
    assert attempt["repeated_inconclusive_signal_preserved"] is True
    ...
    assert attempt["current_weak_pairing_domain_id"] == "toy_weak_pairing_domain_v3_candidate"
    assert attempt["weak_pairing_domain_id"] == "toy_weak_pairing_domain_v4_candidate"
    assert attempt["current_regular_context_id"] == "toy_regular_context_v3_candidate"
    assert attempt["regularity_structure_id"] == "toy_regular_context_v4_candidate"
    assert attempt["current_test_function_class_id"] == (
    ...
    assert attempt["current_candidate_source_definition_id"] == (
    ...
    assert attempt["scope_restriction_id"] == "bounded_toy_weak_pairing_scope_v4"
    assert attempt["obstruction_accounting_id"] == (
        "repeated_inconclusive_obstruction_account_v3"
    )
    assert attempt["bounded_refinement_attempt_result_review_pending"] is True
    assert attempt["refinement_attempt_result_review_pending"] is True


def test_post_retest_refinement_conservation_retest_refinement_attempt_preserves_nonclaims() -> None:
    attempt = _json(DEFAULT_OUT)
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_validation_policy() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert (
        attempt["validation_posture"]["release_index_lean_path"]
        == "not_freshly_validated_preserved_caveat"
    )
    assert "not freshly Lean-validated" in attempt["validation_caveat"]


def test_post_retest_refinement_conservation_retest_refinement_attempt_selects_result_review_only() -> None:
    attempt = _json(DEFAULT_OUT)
    ...
    assert {row["target"]: row["decision"] for row in attempt["candidate_next_targets"]} == {
        ...
        "prepare_qft_gr_minimal_working_model_countermodel_packet_after_post_retest_refinement_conservation_retest_refinement": (
            "not_authorized_pending_attempt_result_review"
        ),
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_updates_live_target() -> None:
    ...
    assert attempt_workstream["model_refinement_executed"] == "yes"
    assert attempt_workstream["obstruction_class"] == OBSTRUCTION_CLASS
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_deterministic() -> None:
    attempt = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_working_model_refinement_attempt_after_post_retest_refinement_conservation_retest_refinement(
...
def test_post_retest_refinement_conservation_retest_refinement_attempt_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_working_model_refinement_attempt_after_post_retest_refinement_conservation_retest_refinement_gate.py"
    )
