...
SYNTHESIS_0150_PATH = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_50_SYNTHESIS_NOTE_v0.md"
)
...
def test_increment51_semantic_delta_decision_exists_and_anchors() -> None:
    decision = _read(DECISION_PATH)
    synthesis = _read(SYNTHESIS_0150_PATH)
    ...
    assert "formal/docs/release/QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_50_SYNTHESIS_NOTE_v0.md" in decision
    assert "formal/docs/paper/TOE_QFT_GR_SEAM_REACTIVATION_OBJECTIVE_v0.md" in decision


def test_increment51_semantic_delta_decision_tokens_present() -> None:
    decision = _read(DECISION_PATH)

    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT51_SEMANTIC_DELTA_STATUS_v0: "
        "DEFINED_ADDITIVE_DELTA_READY_FOR_BOUNDED_INCREMENT"
    ) in decision
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT51_OPEN_CONDITION_v0: "
        "SATISFIED_BY_EXPLICIT_PREFIX_TRANSITION_CURVATURE_LAPLACIAN_GRADIENT_MAGNITUDE_STABILITY_GRADIENT_SIGN_MAGNITUDE_DRIFT_BOUND_GRADIENT_SIGN_MAGNITUDE_STABILITY_GRADIENT_SIGN_MAGNITUDE_STABILITY_INVARIANCE_DEPENDENCY_CRITERION"
    ) in decision
    assert REQUIRED_HOLD_TOKEN in decision


def test_increment51_semantic_delta_explicit_nonredundancy_basis() -> None:
    decision = _read(DECISION_PATH)
    ...
    assert "canonical admissible transition-curvature-laplacian-gradient-magnitude-stability-gradient-sign-magnitude-drift-bound-gradient-sign-magnitude-stability-gradient-sign-magnitude profile" in decision
    assert "prefix-transition-curvature-laplacian-gradient-magnitude-stability-gradient-sign-magnitude-drift-bound-gradient-sign-magnitude-stability-gradient-sign-magnitude-stability invariance" in decision
    assert "canonical admissible transition-curvature-laplacian-gradient-magnitude-stability-gradient-sign-magnitude-drift-bound-gradient-sign-magnitude-stability-gradient-sign-magnitude-stability profile" in decision


def test_increment51_semantic_delta_nonclaim_and_no_auto_open() -> None:
    decision = _read(DECISION_PATH)
    ...
    assert "does not itself open Increment51" in decision
