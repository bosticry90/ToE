...
def test_ea02_exists_and_encodes_invariance_and_beta_non_inference():
    doc = (REPO_ROOT / "State_of_the_Theory.md").read_text(encoding="utf-8")

    ea02 = _extract_block(doc, block_id="EA-02")
    deps = _dependencies_line(ea02)

    # Required wiring
    assert "OV-02x" in deps
    ...
    assert not forbidden.search(ea02), "EA-02 must not contain β!=0 / detected curvature phrasing"
