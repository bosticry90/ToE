...
def test_cycle55_nonflip_execution_readiness_packet_tokens_in_discharge_doc() -> None:
    doc = _read(
        Path("formal/docs/paper/DERIVATION_TARGET_QFT_FULL_DERIVATION_DISCHARGE_v0.md")
    )
    assert (
        "QFT_FULL_DERIVATION_PROGRESS_CYCLE55_v0: "
        "NONFLIP_EXECUTION_READINESS_PACKET_LOCK_PINNED" in doc
    )
    assert (
        "QFT_FULL_DERIVATION_NONFLIP_EXECUTION_READINESS_PACKET_GATE_v0: "
        "REQUIRE_TWO_KEY_AUTHORIZED_REVALIDATION_CLOSURE_AND_NONFLIP_EXECUTION_PACKET" in doc
    )
    assert (
        "QFT_FULL_DERIVATION_NONFLIP_EXECUTION_READINESS_PACKET_SCOPE_v0: "
        "ADJUDICATION_REMAINS_NOT_YET_DISCHARGED_UNTIL_EXPLICIT_FLIP_AUTHORITY" in doc
    )
    assert (
        "QFT_FULL_DERIVATION_NONFLIP_EXECUTION_READINESS_PACKET_ARTIFACT_v0: "
        "qft_full_derivation_nonflip_execution_readiness_packet_cycle55_v0" in doc
    )
    assert "formal/output/qft_full_derivation_nonflip_execution_readiness_packet_cycle55_v0.json" in doc
...
def test_cycle55_nonflip_execution_readiness_packet_preserves_no_flip_tokens() -> None:
...
def test_cycle55_nonflip_execution_readiness_packet_artifact_json_exists_and_is_consistent() -> None:
    artifact = _read(
        Path(
            "formal/output/"
            "qft_full_derivation_nonflip_execution_readiness_packet_cycle55_v0.json"
        )
    )
    assert '"artifact_id": "qft_full_derivation_nonflip_execution_readiness_packet_cycle55_v0"' in artifact
    assert '"cycle": 55' in artifact
    assert '"status": "nonflip_execution_readiness_packet_locked"' in artifact
