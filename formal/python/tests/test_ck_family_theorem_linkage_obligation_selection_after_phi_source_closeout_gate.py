...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "ck_family_theorem_linkage_obligation_selection_after_phi_source_closeout_report.py"
)
...
def test_ck_family_selection_after_phi_source_closeout_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_ck_family_selection_after_phi_source_closeout_selects_C_bridge_phi() -> None:
...
def test_ck_family_selection_after_phi_source_closeout_preserves_bridge_registry_boundary() -> None:
    selection = _json(DEFAULT_OUT)
    ...
    assert selection["phi_bridge_registry_boundary"] == PHI_BRIDGE_REGISTRY_BOUNDARY
    assert selection["prior_phi_bridge_candidate_id"] == BRIDGE_CANDIDATE_ID
    assert selection["prior_phi_bridge_candidate_type"] == BRIDGE_CANDIDATE_TYPE
    assert selection["prior_phi_bridge_constraint_form"] == BRIDGE_CONSTRAINT_FORM
    assert (
        selection["prior_phi_bridge_constraint_equation"]
        == BRIDGE_CONSTRAINT_EQUATION
    )
    assert (
        selection["prior_phi_bridge_admissibility_constraint_form"]
        == BRIDGE_ADMISSIBILITY_CONSTRAINT_FORM
    )
...
def test_ck_family_selection_after_phi_source_closeout_records_lean_status() -> None:
    selection = _json(DEFAULT_OUT)
...
def test_ck_family_selection_after_phi_source_closeout_rotates_to_review() -> None:
    ...
    assert selector["A_source_route_imported"] == "no"
...
def test_ck_family_selection_after_phi_source_closeout_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_SELECTION_RESULT,
        PACKET_CLASSIFICATION,
        "CKFamilyTheoremLinkageObligationSelectionAfterPhiSourceCloseout",
        CONSUMED_TARGET,
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        FOLLOW_ON_TARGET_AFTER_REVIEW,
        ...
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PHI_SOURCE_CLOSEOUT_OUTCOME_v0",
...
def test_ck_family_selection_after_phi_source_closeout_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_ck_family_theorem_linkage_obligation_selection_after_phi_source_closeout_gate.py"
    )
