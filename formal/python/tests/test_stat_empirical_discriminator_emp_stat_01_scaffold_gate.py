...
DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_STAT_EMPIRICAL_DISCRIMINATOR_EMP_STAT_01_v0.md"
...
def test_stat_emp_stat_01_run_bundle_is_hash_and_pointer_pinned() -> None:
    doc_text = _read(DOC_PATH)
    artifact_bytes = ARTIFACT_PATH.read_bytes()
    artifact_hash = hashlib.sha256(artifact_bytes).hexdigest()

    assert _extract_token(doc_text, "EMP_STAT_01_DISCRIMINATOR_STATUS_v0") == "RUN_BOUNDED_v0_NONCLAIM"
    assert _extract_token(doc_text, "EMP_STAT_01_PRUNE_DECISION_v0") == "ELIMINATION_READY_BOUNDED_v0"
    assert _extract_token(doc_text, "EMP_STAT_01_PRUNE_RESULT_v0") == "PASS_AND_PRUNE_SIGNAL_PRESENT_v0"
    assert _extract_token(doc_text, "EMP_STAT_01_ARTIFACT_v0") == "stat_empirical_discriminator_emp_stat_01_run_cycle01_v0"
    assert _extract_token(doc_text, "EMP_STAT_01_GATE_v0") == "ARTIFACT_HASH_AND_CROSS_SURFACE_POINTERS_REQUIRED"
    assert _extract_token(doc_text, "EMP_STAT_01_ARTIFACT_SHA256_v0") == artifact_hash
    ...
    for path_ref in (
        ...
        assert path_ref in doc_text, f"STAT discriminator doc must pin `{path_ref}`."
