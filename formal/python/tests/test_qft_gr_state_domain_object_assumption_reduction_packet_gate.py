...
from formal.python.tools.qft_gr_state_domain_object_assumption_reduction_packet_report import (
    ...
    build_qft_gr_state_domain_object_assumption_reduction_packet,
)
...
def test_qft_gr_state_domain_object_packet_files_exist() -> None:
...
def test_qft_gr_state_domain_object_packet_consumes_result_review() -> None:
...
def test_qft_gr_state_domain_object_packet_selects_only_row001() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_domain_object_assumption"]
    ...
    assert packet["state_domain_object_status_tokens"] == selected["current_status"]


def test_qft_gr_state_domain_object_packet_prepares_requested_fields_only() -> None:
    packet = _json(DEFAULT_OUT)
    selected = packet["state_domain_object_assumption"]
    assert packet["state_domain_object"] == STATE_DOMAIN_OBJECT
    assert selected["state_domain_object"] == STATE_DOMAIN_OBJECT
    assert packet["state_admissibility_boundary"] == STATE_ADMISSIBILITY_BOUNDARY
    assert selected["state_admissibility_boundary"] == STATE_ADMISSIBILITY_BOUNDARY
    assert packet["state_object_compatibility_condition"] == (
        STATE_OBJECT_COMPATIBILITY_CONDITION
    )
    assert selected["state_object_compatibility_condition"] == (
        STATE_OBJECT_COMPATIBILITY_CONDITION
    )
    assert packet["state_domain_object_definition_status"] == (
        STATE_DOMAIN_OBJECT_DEFINITION_STATUS
    )
    assert selected["state_domain_object_definition_status"] == (
        STATE_DOMAIN_OBJECT_DEFINITION_STATUS
    )
    ...
    assert packet["state_domain_object_assumption_reduction_analysis_prepared"] is True
    assert packet["prepares_reduction_analysis_only"] is True


def test_qft_gr_state_domain_object_packet_preserves_nonclaims() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["state_domain_object_assumption_discharged"] is False
    assert packet["state_domain_object_assumption_reduced_or_discharged_by_preparation"] is False
...
def test_qft_gr_state_domain_object_packet_selects_one_target() -> None:
    packet = _json(DEFAULT_OUT)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selection_count"] == 1
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        NEXT_TARGET: "selected",
        "execute_qft_gr_state_domain_object_assumption_reduction_attempt": "deferred",
...
def test_qft_gr_state_domain_object_packet_deterministic_and_pinned() -> None:
    packet = _json(DEFAULT_OUT)
    generated = build_qft_gr_state_domain_object_assumption_reduction_packet(
