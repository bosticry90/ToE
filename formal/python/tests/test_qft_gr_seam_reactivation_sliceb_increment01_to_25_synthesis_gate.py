...
def test_synthesis_0125_exists_and_is_anchored() -> None:
    synthesis = _read(SYNTHESIS_0125_PATH)
    synthesis_0124 = _read(SYNTHESIS_0124_PATH)
    objective = _read(OBJECTIVE_PATH)
    increment25 = _read(INCREMENT25_PACKET_PATH)

    assert REQUIRED_QUESTION in objective
    assert REQUIRED_QUESTION in synthesis_0124
    assert REQUIRED_QUESTION in increment25
    ...
    assert "formal/docs/release/QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_24_SYNTHESIS_NOTE_v0.md" in synthesis


def test_synthesis_0125_contains_required_sections() -> None:
    synthesis = _read(SYNTHESIS_0125_PATH)
...
def test_synthesis_0125_tokens_present() -> None:
    synthesis = _read(SYNTHESIS_0125_PATH)
    ...
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_25_SYNTHESIS_STATUS_v0: "
        "SYNTHESIZED_BOUNDED_v0"
    ) in synthesis


def test_synthesis_0125_nonclaim_markers_present() -> None:
    synthesis = _read(SYNTHESIS_0125_PATH)
