...
from formal.python.tools.v01_alpha_dependency_remediation_tranche_003_execution_packet_report import (
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "v01_alpha_dependency_remediation_tranche_003_execution_packet_report.py"
)
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Release"
    / "V01DependencyRemediationTranche003ExecutionPacket.lean"
)
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_files_exist() -> None:
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_consumes_selection_result_review() -> None:
    packet = _json(PACKET_PATH)
    assert packet["schema_id"] == (
        "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_003_EXECUTION_PACKET_20260515_v0"
    )
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_preserves_prior_tranches() -> None:
    packet = _json(PACKET_PATH)
    assert packet["tranche_001_status"] == TRANCHE_001_STATUS
    assert packet["tranche_002_status"] == TRANCHE_002_STATUS
    assert packet["tranche_003_status"] == "selected_for_execution_packet_preparation"
    assert packet["tranche_003_cleared_for_global_release_readiness"] is False
    assert packet["global_release_readiness_still_blocked"] is True


def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_selects_only_tranche_003() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_tracks_four_blockers() -> None:
    packet = _json(PACKET_PATH)
    ...
    assert packet["nonselected_release_blocking_obligation_count"] == 3
    assert [
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_defines_lean_audit_surface() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_defines_review_requirements_and_criteria() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_prepares_without_execution() -> None:
    packet = _json(PACKET_PATH)
    assert packet["tranche_003_execution_packet_prepared"] is True
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_forbidden_effects_false() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_next_target() -> None:
    packet = _json(PACKET_PATH)
    assert packet["selected_next_target"] == NEXT_TARGET
    assert packet["selected_next_target_kind"] == (
        "tranche_003_execution_packet_result_review_only"
    )
    ...
    assert {row["target"]: row["decision"] for row in packet["candidate_next_targets"]} == {
        "review_v01_alpha_dependency_remediation_tranche_003_execution_packet_result": "selected",
        "execute_v01_alpha_dependency_remediation_tranche_003_audit": "deferred",
        "prepare_v01_alpha_release_readiness_adjudication_packet": "deferred",
    }


def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_acceptance_and_determinism() -> None:
    packet = _json(PACKET_PATH)
...
def test_v01_alpha_dependency_remediation_tranche_003_execution_packet_is_pinned() -> None:
    ...
    refs = [
        ...
        "formal/python/tools/v01_alpha_dependency_remediation_tranche_003_execution_packet_report.py",
        "formal/python/tests/test_v01_alpha_dependency_remediation_tranche_003_execution_packet_gate.py",
        OUTCOME_ID,
        NEXT_TARGET,
    ]
    for ref in refs:
    ...
    assert "V01DependencyRemediationTranche003ExecutionPacket" in index_text
    assert (
        "v01_dependency_remediation_tranche_003_execution_packet_does_not_execute_remediation"
        in index_text
    )
