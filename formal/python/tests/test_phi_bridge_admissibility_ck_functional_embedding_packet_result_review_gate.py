...
from formal.python.tools.phi_bridge_admissibility_ck_functional_embedding_packet_result_review_report import (
    ...
    build_phi_bridge_admissibility_ck_functional_embedding_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_bridge_admissibility_ck_functional_embedding_packet_result_review_report.py"
)
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_files_exist() -> None:
    for path in [
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_accepts_packet() -> None:
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_carries_forms() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_required_points() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert {row["row_id"] for row in review["review_criteria"]} == {
        ...
        "c_bridge_zero_preserved_as_rule",
        ...
        "no_bridge_proof_or_route_alignment_proof",
        "no_phi_generation_or_potential_derivation",
        "no_qft_gr_closure_or_master_action_promotion",
        "bridge_admissibility_rule_closeout_next_target_selected",
    }
    ...
    assert (
        review["second_phi_relevant_ck_admissibility_rule_candidate_classification"]
        == SECOND_RULE_CLASSIFICATION
    )


def test_phi_bridge_admissibility_ck_functional_embedding_result_review_blocks_shortcuts() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert "does not prove full bridge admissibility" in review["non_claim_boundary"]
    assert "does not close QFT-GR" in review["non_claim_boundary"]


def test_phi_bridge_admissibility_ck_functional_embedding_result_review_validation_policy() -> None:
    review = _json(DEFAULT_OUT)
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_rotates_to_closeout() -> None:
    ...
    assert consumed["bridge_admissibility_proved"] == "no"
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        CONSUMED_TARGET,
        NEXT_TARGET,
        "PhiBridgeAdmissibilityCKFunctionalEmbeddingPacketResultReview",
        "CURRENT_LIVE_NEXT_TARGET_v0: prepare_phi_bridge_admissibility_ck_admissibility_rule_closeout",
        BRIDGE_CANDIDATE_ID,
        BRIDGE_CONSTRAINT_FORM,
        BRIDGE_CONSTRAINT_EQUATION,
        BRIDGE_ROUTE_FIELD_EQUATION_MATCH,
        ...
        LAGRANGE_MULTIPLIER_ACTION_FORM,
        PENALTY_ACTION_FORM,
        "accepts the admissibility-only route as a bridge route-consistency rule only",
...
def test_phi_bridge_admissibility_ck_functional_embedding_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_bridge_admissibility_ck_functional_embedding_packet_result_review_gate.py"
    )
