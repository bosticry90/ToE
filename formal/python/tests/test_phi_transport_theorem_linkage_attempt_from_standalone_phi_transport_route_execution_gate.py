...
from formal.python.tools.phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_report import (
    BOUNDARY_ITEMS,
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_report.py"
)
...
def consumed_target() -> str:
    return "execute_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route"


def review_target() -> str:
    return "review_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_result"


def test_phi_transport_standalone_attempt_execution_files_exist() -> None:
    for path in [
        DEFAULT_OUT,
        TOOL_PATH,
        LEAN_PACKET_PATH,
...
def test_phi_transport_standalone_attempt_execution_constructs_transport_zero() -> None:
    ...
    assert execution["outcome_id"] == OUTCOME_ID == EXECUTION_RESULT
    ...
    assert execution["result_review_outcome"] == RESULT_REVIEW_OUTCOME
    assert execution["result_review_strict_outcome"] == STRICT_REVIEW_RESULT
...
def test_phi_transport_standalone_attempt_execution_preserves_component_route() -> None:
    execution = _json(DEFAULT_OUT)

    assert execution["standalone_phi_transport_route"] == STANDALONE_PHI_TRANSPORT_ROUTE
    assert execution["standalone_phi_transport_route_preserved"] is True
    assert execution["exact_five_component_transport_tuple_preserved"] is True
    assert execution["target_C_transport_phi_zero_preserved"] is True
    assert execution["transport_constraint_form"] == TRANSPORT_CONSTRAINT_FORM
    assert execution["transport_constraint_equation"] == TRANSPORT_CONSTRAINT_EQUATION
    assert execution["transport_admissibility_constraint_form"] == (
        TRANSPORT_ADMISSIBILITY_CONSTRAINT_FORM
    )
    assert execution["transport_component_count"] == len(TRANSPORT_COMPONENTS)
    assert execution["transport_component_forms"] == [
        row["component_form"] for row in TRANSPORT_COMPONENTS
    ]
    assert execution["transport_action_variation_zero_component"] == (
        TRANSPORT_ACTION_VARIATION_ZERO_COMPONENT
    )
    assert execution["transport_variation_bridge_zero_component"] == (
        TRANSPORT_VARIATION_BRIDGE_ZERO_COMPONENT
    )
    assert execution["transport_bridge_source_zero_component"] == (
        TRANSPORT_BRIDGE_SOURCE_ZERO_COMPONENT
    )
    assert execution["transport_source_residual_zero_component"] == (
        TRANSPORT_SOURCE_RESIDUAL_ZERO_COMPONENT
    )
    assert execution["transport_residual_regime_zero_component"] == (
        TRANSPORT_RESIDUAL_REGIME_ZERO_COMPONENT
    )
    assert execution["componentwise_zero_route"] == COMPONENTWISE_ZERO_ROUTE
    assert execution["execution_route_to_authorize"] == EXECUTION_ROUTE_TO_AUTHORIZE
    assert execution["executed_componentwise_route"] == EXECUTED_COMPONENTWISE_ROUTE
    assert execution["C_transport_tuple_zero"] == C_TRANSPORT_TUPLE_ZERO
    assert execution["target_conclusion"] == TARGET_CONCLUSION
    assert execution["plain_meaning"] == PLAIN_MEANING
    assert execution["known_phi_transport_chain_form"] == KNOWN_PHI_TRANSPORT_CHAIN_FORM
    assert execution["route_kind"] == "standalone_phi_transport_componentwise_zero_execution"
    assert execution["transport_action_variation_zero_component_used"] is True
    assert execution["transport_variation_bridge_zero_component_used"] is True
    assert execution["transport_bridge_source_zero_component_used"] is True
    assert execution["transport_source_residual_zero_component_used"] is True
    assert execution["transport_residual_regime_zero_component_used"] is True
    assert execution["componentwise_zero_route_constructed"] is True
    assert execution["C_transport_phi_tuple_zero_constructed"] is True
    assert execution["C_transport_phi_zero_constructed"] is True
    assert execution["C_transport_phi_zero_derived"] is True


def test_phi_transport_standalone_attempt_execution_preserves_boundaries() -> None:
    execution = _json(DEFAULT_OUT)
    route_text = " ".join(execution["executed_componentwise_route"])

    assert execution["route_purity_watch_items"] == TRANSPORT_ROUTE_PURITY_WATCH_ITEMS
    ...
    assert execution["C_transport_phi_discharged"] is True
    assert execution["C_transport_phi_linkage_constructed"] is True
    ...
    for flag in [
        ...
        assert execution[flag] is False, flag

    assert execution["lean_status_wording"] == LEAN_STATUS_WORDING_FOR_EXECUTION
    assert (
        execution["lean_status_wording_lines"]
        == LEAN_STATUS_WORDING_LINES_FOR_EXECUTION
    )
    assert (
    ...
    assert "full ToeFormal aggregate = PASSED_SERIAL_RERUN" not in json.dumps(
        execution
    )


def test_phi_transport_standalone_attempt_execution_rotates_to_result_review() -> None:
    ...
    review = _workstream(registry, review_target())
    ...
    assert consumed["review_result"] == RESULT_REVIEW_OUTCOME
    assert consumed["strict_review_result"] == STRICT_REVIEW_RESULT
    assert consumed["execution_result"] == EXECUTION_RESULT
    ...
    assert consumed["executed_componentwise_route"] == "; ".join(
        EXECUTED_COMPONENTWISE_ROUTE
    )
    assert consumed["C_transport_phi_discharged"] == "yes"
    ...
    if is_current:
        ...
        assert active["C_transport_phi_discharged"] == "yes"
...
def test_phi_transport_standalone_attempt_execution_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_EXECUTION_RESULT,
        PACKET_CLASSIFICATION,
        "PhiTransportTheoremLinkageAttemptFromStandalonePhiTransportRouteExecution",
        consumed_target(),
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        SUGGESTED_REVIEW_OUTCOME,
        ...
        *[row["component_form"] for row in TRANSPORT_COMPONENTS],
        ...
        TARGET_CONCLUSION,
        *LEAN_STATUS_WORDING_LINES_FOR_EXECUTION,
        "PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_TRANSPORT_ROUTE_EXECUTION_OUTCOME_v0",
        "PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_TRANSPORT_ROUTE_EXECUTION_NONCLAIM_BOUNDARY_v0",
...
def test_phi_transport_standalone_attempt_execution_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_gate.py"
    )
