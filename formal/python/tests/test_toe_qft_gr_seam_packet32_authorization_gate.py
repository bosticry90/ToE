...
ASSESSMENT_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet31_assessment_checkpoint_v0.json"
...
def test_qft_gr_seam_packet32_authorization_document_structure() -> None:
    ...
    required_markers = [
        ...
        "packet32_physics_quantity_tightened:",
        "packet32_discriminator_strengthening_requirement:",
        "packet32_ambiguity_reduction_requirement:",
        "packet32_non_repetition_clause:",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet32 authorization doc missing marker: {marker}"


def test_qft_gr_seam_packet32_authorization_checkpoint_schema_and_preconditions() -> None:
    artifact = _read_json(AUTH_CHECKPOINT_PATH)
    assessment_artifact = _read_json(ASSESSMENT_CHECKPOINT_PATH)

    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet32_authorization_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1AL_QFT_GR_SEAM_PACKET32_AUTHORIZATION"
    assert artifact.get("status") == "PACKET32_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("authorization_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET32_AUTHORIZATION_v0.md"
    assert payload.get("parent_assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET31_ASSESSMENT_v0.md"
    ...
    assessment_decision = assessment_artifact.get("payload", {}).get("packet32_decision", {})
    ...
    pre = payload.get("preconditions_from_packet31_assessment", {})
    ...
    assert decision.get("decision_outcome") == "AUTHORIZE_PACKET32_BOUNDED_TARGET_v0"
    assert decision.get("status") == "AUTHORIZED_WITH_SINGLE_BOUNDED_TARGET_v0"

    bounded = payload.get("packet32_bounded_target", {})
    ...
    assert bounded.get("discriminator_strengthening_requirement") == (
        "criterion_level_post_hardening_closure_resilience_discrimination_beyond_packet31_closure_hardening_witness_qualification_required"
    )
    assert bounded.get("ambiguity_reduction_requirement") == (
        "packet32_valid_only_if_residual_interface_ambiguity_reduced_via_new_closure_resilience_discriminative_content"
    )
    assert bounded.get("non_repetition_clause") == (
        "packet32_invalid_if_only_packet31_closure_hardening_witness_reencoded_without_new_closure_resilience_discriminative_content"
    )


def test_qft_gr_seam_packet32_authorization_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet32 authorization pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet32 authorization pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_auth = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET32_AUTHORIZATION_STATUS_v0")
    roadmap_auth = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET32_AUTHORIZATION_STATUS_v0")
