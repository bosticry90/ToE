...
COSMO_MICRO13_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_13_DRYRUN_CUSTODY_PACKET_v0.md"
COSMO_MICRO13_ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "cosmo_bg_micro13_dryrun_custody_packet_cycle01_v0.json"
...
def test_cosmo_micro13_artifacts_exist() -> None:
    assert COSMO_TARGET_PATH.exists(), "Missing COSMO background target document."
    assert COSMO_MICRO13_PATH.exists(), "Missing COSMO background Cycle-013 micro document."
    assert COSMO_MICRO13_ARTIFACT_PATH.exists(), "Missing COSMO background Cycle-013 artifact payload."


def test_cosmo_target_references_micro13_and_gate() -> None:
    ...
    assert not missing, "COSMO parent target is missing required micro-13 token(s): " + ", ".join(missing)


def test_cosmo_micro13_doc_contains_required_tokens() -> None:
    text = _read(COSMO_MICRO13_PATH)
    required_tokens = [
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "COSMO micro-13 document is missing required token(s): " + ", ".join(missing)


def test_cosmo_dryrun_custody_keeps_locked_queue_status_and_bundle_coherence() -> None:
    ...
    assert cosmo.get("dryrun_custody_doc") == "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_13_DRYRUN_CUSTODY_PACKET_v0.md"
    assert cosmo.get("dryrun_custody_gate") == "formal/python/tests/test_cosmo_bg_micro13_dryrun_custody_packet_gate.py"
    assert cosmo.get("dryrun_custody_policy") == "CYCLE08_09_10_11_12_CUSTODY_ATTESTATION_LOCK_REQUIRED_NO_STATUS_FLIP"
    ...
    assert not missing, "State missing dryrun custody policy token(s): " + ", ".join(missing)


def test_cosmo_micro13_artifact_schema_and_token_alignment() -> None:
    payload = _read_json(COSMO_MICRO13_ARTIFACT_PATH)

    assert payload.get("artifact_id") == "cosmo_bg_micro13_dryrun_custody_packet_cycle01_v0"
    assert payload.get("artifact_version") == "v0"
    assert payload.get("placeholder_template") is True

    body = payload.get("payload")
    assert isinstance(body, dict)
    assert body.get("checkpoint") == "cosmo_bg_micro13_dryrun_custody_packet_cycle01"
    assert body.get("status") == "placeholder_non_promotional"
    assert body.get("scope") == "dryrun_custody_packet_only_nonclaim_v0"

    policy_tokens = body.get("policy_tokens")
    assert isinstance(policy_tokens, list) and len(policy_tokens) == 3

    pointers = body.get("required_custody_bundle_pointers")
    assert isinstance(pointers, list) and len(pointers) == 5

    assert body.get("required_custody_bundle_hash") == "c08_c09_c10_c11_c12_pointer_bundle_cycle01_v0"