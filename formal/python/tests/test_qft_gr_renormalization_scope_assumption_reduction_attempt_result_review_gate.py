...
from formal.python.tools.qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review,
)
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_files_exist() -> None:
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_consumes_attempt() -> None:
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_accepts_row002() -> None:
    review = _json(DEFAULT_OUT)
    contract = review["renormalization_scope_reduction_contract"]
    assert review["blocker"] == BLOCKER
    assert review["selected_assumption_family"] == SELECTED_ASSUMPTION_FAMILY
    assert review["accepted_prior_renormalization_assumption_row"] == ACCEPTED_PRIOR_ROW
    ...
    assert review["accepted_contract_id"] == RENORMALIZATION_SCOPE_CONTRACT_ID
    assert review["bounded_scope_contract_status"] == BOUNDED_SCOPE_CONTRACT_STATUS
    ...
    assert (
        review["renormalization_scope_assumption_reduction_attempt_result_reviewed"]
        is True
    )
    assert review["renormalization_scope_assumption_reduction_accepted"] is True
    assert review["renormalization_scope_assumption_reduction_rejected"] is False


def test_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    assert (
        review["renormalization_scope_assumption_reduced_pending_result_review_accepted"]
        is True
    )
    assert review["renormalization_scope_assumption_discharged"] is False
    assert review["renormalization_scope_assumption_discharged_by_review"] is False
    assert review["renormalization_scope_discharged_by_implication"] is False
    assert review["renormalization_scope_assumption_reduced_or_discharged_by_review"] is False
    assert (
        review["renormalization_scope_assumption_reduced_or_discharged_by_implication"]
        is False
    )
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_selects_domain_packet() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["next_renormalization_assumption_row_object"] == RENORMALIZED_EXPECTATION_DOMAIN
    assert (
        review["next_renormalization_assumption_row_required_future_proof_object"]
        == NEXT_ROW_REQUIRED_FUTURE_PROOF_OBJECT
    )
    assert review["downstream_finiteness_regular_boundary_row"] == (
        "RN-ASSUMP-004-finiteness_regular_boundary"
    )
    assert review["downstream_finiteness_regular_boundary_object"] == (
        FINITENESS_REGULARITY_BOUNDARY
    )
...
def test_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review_deterministic_and_pinned() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_renormalization_scope_assumption_reduction_attempt_result_review(
