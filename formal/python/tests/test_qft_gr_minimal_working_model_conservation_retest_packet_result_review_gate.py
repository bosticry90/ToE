...
from formal.python.tools.qft_gr_minimal_working_model_conservation_retest_packet_report import (
    DEFAULT_OUT as CONSERVATION_RETEST_PACKET_PATH,
    OUTCOME_ID as CONSERVATION_RETEST_PACKET_OUTCOME,
    PACKET_CLASSIFICATION as CONSERVATION_RETEST_PACKET_CLASSIFICATION,
    PACKET_ID as CONSERVATION_RETEST_PACKET_ID,
    RETEST_CONDITION_ID,
    SCHEMA_ID as CONSERVATION_RETEST_PACKET_SCHEMA_ID,
)
from formal.python.tools.qft_gr_minimal_working_model_conservation_retest_packet_result_review_report import (
    ...
    REVIEW_ID,
    SCHEMA_ID,
    build_qft_gr_minimal_working_model_conservation_retest_packet_result_review,
)
...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "qft_gr_minimal_working_model_conservation_retest_packet_result_review_report.py"
)
LEAN_REVIEW_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Derivation"
    / "QFTGRMinimalWorkingModelConservationRetestPacketResultReview.lean"
)
...
def test_minimal_working_model_conservation_retest_packet_result_review_files_exist() -> None:
    assert CONSERVATION_RETEST_PACKET_PATH.exists()
    assert DEFAULT_OUT.exists()
    assert TOOL_PATH.exists()
    assert LEAN_REVIEW_PATH.exists()


def test_minimal_working_model_conservation_retest_packet_result_review_consumes_packet() -> None:
    review = _json(DEFAULT_OUT)
    packet = _json(CONSERVATION_RETEST_PACKET_PATH)
    ...
    assert (
        review["consumes_qft_gr_minimal_working_model_conservation_retest_packet"]
        == CONSERVATION_RETEST_PACKET_ID
    )
    assert packet["schema_id"] == CONSERVATION_RETEST_PACKET_SCHEMA_ID
    assert packet["packet_id"] == CONSERVATION_RETEST_PACKET_ID
    assert packet["outcome_id"] == CONSERVATION_RETEST_PACKET_OUTCOME
    assert packet["packet_classification"] == CONSERVATION_RETEST_PACKET_CLASSIFICATION
    assert packet["selected_next_target"] == CONSUMED_TARGET


def test_minimal_working_model_conservation_retest_packet_result_review_accepts_protocol() -> None:
    review = _json(DEFAULT_OUT)
    ...
    delta = review["refinement_delta_after_first_conservation_test"]
...
def test_minimal_working_model_conservation_retest_packet_result_review_authorizes_attempt_only() -> None:
    review = _json(DEFAULT_OUT)
...
def test_minimal_working_model_conservation_retest_packet_result_review_preserves_nonclaims() -> None:
    review = _json(DEFAULT_OUT)
    for key in [
...
def test_minimal_working_model_conservation_retest_packet_result_review_updates_live_target() -> None:
...
def test_minimal_working_model_conservation_retest_packet_result_review_deterministic() -> None:
    review = _json(DEFAULT_OUT)
    generated = build_qft_gr_minimal_working_model_conservation_retest_packet_result_review(
        packet_path=CONSERVATION_RETEST_PACKET_PATH,
...
def test_minimal_working_model_conservation_retest_packet_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_qft_gr_minimal_working_model_conservation_retest_packet_result_review_gate.py"
    )
