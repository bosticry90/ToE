...
ASSESSMENT_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET13_ASSESSMENT_v0.md"
ASSESSMENT_CHECKPOINT_PATH = REPO_ROOT / "formal" / "output" / "toe_qft_gr_seam_packet13_assessment_checkpoint_v0.json"
...
def test_qft_gr_seam_packet13_assessment_document_structure() -> None:
    ...
    required_markers = [
        ...
        "did packet13 satisfy its exact bounded target?",
        "target_satisfaction_verdict: YES_EXACT_TARGET_SATISFIED_v0",
        "physics_tightening_beyond_packet12_verdict: YES_REAL_TIGHTENING_VERIFIED_v0",
        "interface_decision_witness_ambiguity_reduction_verdict: YES_RESIDUAL_AMBIGUITY_REDUCED_v0",
        "remaining_gap_narrower_than_objective_verdict: YES_NARROWER_THAN_OBJECTIVE_v0",
        "packet14_progress_verdict: GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0",
        "packet14_ladder_extension_verdict: REJECT_IF_MOMENTUM_EXTENSION_v0",
        "packet14_authorization_verdict: JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0",
        "packet14_exact_bounded_target:",
        ...
        "TOE_QFT_GR_SEAM_PACKET13_ASSESSMENT_GATE_v0: REQUIRED_PACKET13_ASSESSMENT_SCHEMA_AND_AUTHORITY_PARITY",
        "TOE_QFT_GR_SEAM_PACKET13_ASSESSMENT_ARTIFACT_v0: toe_qft_gr_seam_packet13_assessment_checkpoint_v0",
    ...
    for marker in required_markers:
        assert marker in text, f"Packet13 assessment doc missing marker: {marker}"


def test_qft_gr_seam_packet13_assessment_checkpoint_schema() -> None:
    ...
    assert artifact.get("artifact_id") == "toe_qft_gr_seam_packet13_assessment_checkpoint_v0"
    assert artifact.get("phase") == "PHASE_1V_QFT_GR_SEAM_PACKET13_ASSESSMENT"
    assert artifact.get("status") == "PACKET13_ASSESSMENT_COMPLETE_v0"

    payload = artifact.get("payload", {})
    assert payload.get("assessment_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET13_ASSESSMENT_v0.md"
    assert payload.get("parent_packet_doc_path") == "formal/docs/paper/TOE_QFT_GR_SEAM_PACKET13_BOUNDED_EXECUTION_v0.md"
    ...
    assert packet_artifact.get("status") == "PACKET13_EXECUTED_UNDER_AUTHORIZED_BOUNDED_TARGET_v0"
    assert auth_artifact.get("status") == "PACKET13_AUTHORIZATION_EXPLICIT_DECISION_COMPLETE_v0"
    ...
    assert summary.get("physics_tightening_beyond_packet12_verdict") == "YES_REAL_TIGHTENING_VERIFIED_v0"
    assert summary.get("interface_decision_witness_ambiguity_reduction_verdict") == "YES_RESIDUAL_AMBIGUITY_REDUCED_v0"

    packet14 = payload.get("packet14_decision", {})
    assert packet14.get("authorized") is True
    assert packet14.get("progress_verdict") == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet14.get("ladder_extension_verdict") == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet14.get("verdict") == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet14.get("hold_refine_trigger_status") == "NOT_HIT_v0"
...
def test_qft_gr_seam_packet13_assessment_chain_consistency_and_authority_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet13 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet13 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"

    state_assessment = _extract_token(state_text, "TOE_QFT_GR_SEAM_PACKET13_ASSESSMENT_STATUS_v0")
    roadmap_assessment = _extract_token(roadmap_text, "TOE_QFT_GR_SEAM_PACKET13_ASSESSMENT_STATUS_v0")
