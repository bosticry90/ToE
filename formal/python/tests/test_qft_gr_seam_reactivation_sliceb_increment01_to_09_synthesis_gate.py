...
SYNTHESIS_0109_PATH = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_09_SYNTHESIS_NOTE_v0.md"
)
...
def test_synthesis_0109_exists_and_is_anchored() -> None:
    synthesis = _read(SYNTHESIS_0109_PATH)
    objective = _read(OBJECTIVE_PATH)
    increment09 = _read(INCREMENT09_PACKET_PATH)

    assert REQUIRED_QUESTION in objective
    assert REQUIRED_QUESTION in increment09
...
def test_synthesis_0109_contains_required_sections() -> None:
    synthesis = _read(SYNTHESIS_0109_PATH)
...
def test_synthesis_0109_tokens_present() -> None:
    synthesis = _read(SYNTHESIS_0109_PATH)

    assert REQUIRED_HOLD_TOKEN in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT10_DECISION_RULE_v0: "
        "REQUIRE_NEW_INCOMPATIBILITY_OR_DEPENDENCY_CRITERION_BEYOND_ORIGIN_PROVENANCE_EPOCH_BRANCH_IRREVERSIBILITY_FALLBACK_COMPLETENESS_STACK"
    ) in synthesis
    assert (
        "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT01_TO_09_SYNTHESIS_STATUS_v0: "
        "SYNTHESIZED_BOUNDED_v0"
    ) in synthesis


def test_synthesis_0109_nonclaim_markers_present() -> None:
    synthesis = _read(SYNTHESIS_0109_PATH)
