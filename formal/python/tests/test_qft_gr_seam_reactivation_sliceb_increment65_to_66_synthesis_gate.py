...
DOC_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT65_TO_66_SYNTHESIS_v0.md"
)


def _read() -> str:
    assert DOC_PATH.exists(), f"Missing Increment65-to-66 synthesis doc: {DOC_PATH}"
    return DOC_PATH.read_text(encoding="utf-8")


def test_increment65_to_66_synthesis_exists_and_anchors() -> None:
    text = _read()
...
def test_increment65_to_66_synthesis_tokens_and_nonclaim_present() -> None:
    text = _read()

    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT65_TO_66_SYNTHESIS_STATUS_v0: LOCKED_BOUNDED_REOPEN_READY_v0" in text
    assert f"QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT65_TO_66_DELTA_v0: {REQUIRED_DELTA}" in text
    assert "QFT_GR_SEAM_REACTIVATION_SLICEB_INCREMENT65_TO_66_ADJUDICATION: DISCHARGED_v0_BOUNDED" in text
...
def test_increment65_to_66_template_and_governance_pointers_present() -> None:
    text = _read()
