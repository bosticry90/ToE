...
TOOL_PATH = (
    ...
    / "python"
    / "tools"
    / "cexchange_theorem_linkage_obligation_closeout_result_review_report.py"
)
...
def test_cexchange_closeout_result_review_files_exist() -> None:
    for path in [
        CLOSEOUT_PATH,
        DEFAULT_OUT,
        TOOL_PATH,
...
def test_cexchange_closeout_result_review_accepts_local_closeout() -> None:
    ...
    assert build_cexchange_theorem_linkage_obligation_closeout_result_review() == review


def test_cexchange_closeout_result_review_preserves_boundary() -> None:
    review = _json(DEFAULT_OUT)
    ...
    assert review["local_cexchange_obligation_closed"] is True
...
def test_cexchange_closeout_result_review_records_lean_status() -> None:
    review = _json(DEFAULT_OUT)
...
def test_cexchange_closeout_result_review_rotates_to_selector() -> None:
    ...
    assert selector["authorization_evidence"] == (
        "formal/toe_formal/ToeFormal/Derivation/"
        "CKFamilyTheoremLinkageObligationSelectionAfterCExchangeCloseout.lean"
    )
    assert selector["report"] == (
        "formal/docs/release/"
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_CEXCHANGE_CLOSEOUT_20260627_v0.json"
    )
    assert selector["selection_result"] == (
    ...
    assert selector["selected_next_target"] == (
        "review_ck_family_theorem_linkage_obligation_selection_after_cexchange_closeout_result"
    )
    assert selector["selected_obligation"] == "psi-A total conservation theorem-linkage gap"
...
def test_cexchange_closeout_result_review_mirrors() -> None:
    joined = "\n".join(
        _read(path)
        for path in [
            TOOL_PATH,
    ...
    for token in [
        ...
        STRICT_REVIEW_RESULT,
        PACKET_CLASSIFICATION,
        "CExchangeTheoremLinkageObligationCloseoutResultReview",
        CONSUMED_TARGET,
        NEXT_TARGET,
        NEXT_TARGET_KIND,
        LIKELY_NEXT_OBLIGATION,
        ...
        C_EXCHANGE_TARGET_CONCLUSION,
        LEAN_STATUS_WORDING_FOR_REVIEW,
        "CEXCHANGE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_RESULT_REVIEW_OUTCOME_v0",
...
def test_cexchange_closeout_result_review_not_manifest_enrolled() -> None:
    assert_focused_gate_not_manifest_enrolled(
        "test_cexchange_theorem_linkage_obligation_closeout_result_review_gate.py"
    )
