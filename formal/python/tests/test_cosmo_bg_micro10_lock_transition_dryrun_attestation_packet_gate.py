...
COSMO_MICRO10_PATH = (
    ...
    / "docs"
    / "paper"
    / "DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_10_LOCK_TRANSITION_DRYRUN_ATTESTATION_PACKET_v0.md"
)
COSMO_MICRO10_ARTIFACT_PATH = (
    REPO_ROOT / "formal" / "output" / "cosmo_bg_micro10_lock_transition_dryrun_attestation_packet_cycle01_v0.json"
)
...
def _extract_target_id(text: str) -> str:
    match = re.search(r"\bTARGET-COSMO-BG-MICRO-10-[A-Z0-9-]+-v0\b", text)
    assert match is not None, "Missing Cycle-010 TARGET token in COSMO micro document."
    return match.group(0)


def _extract_assignment_value(text: str, token_name: str) -> str:
    match = re.search(rf"\b{re.escape(token_name)}\s*:\s*([^\n\r]+)", text)
    assert match is not None, f"Missing `{token_name}` assignment in COSMO micro-10 document."
...
def test_cosmo_micro10_artifacts_exist() -> None:
    assert COSMO_TARGET_PATH.exists(), "Missing COSMO background target document."
    assert COSMO_MICRO10_PATH.exists(), "Missing COSMO background Cycle-010 micro document."
    assert COSMO_MICRO10_ARTIFACT_PATH.exists(), "Missing COSMO background Cycle-010 artifact payload."


def test_cosmo_target_references_micro10_and_gate() -> None:
    text = _read(COSMO_TARGET_PATH)
    micro_text = _read(COSMO_MICRO10_PATH)
    ...
    assert not missing, "COSMO parent target is missing required micro-10 token(s): " + ", ".join(missing)


def test_cosmo_micro10_doc_contains_required_tokens() -> None:
    text = _read(COSMO_MICRO10_PATH)
    ...
    missing = [token for token in required_tokens if token not in text]
    assert not missing, "COSMO micro-10 document is missing required token(s): " + ", ".join(missing)


def test_cosmo_lock_transition_dryrun_attestation_keeps_locked_queue_status() -> None:
    ...
    assert cosmo.get("lock_transition_dryrun_attestation_doc") == (
        "formal/docs/paper/DERIVATION_TARGET_COSMOLOGY_BACKGROUND_MICRO_10_LOCK_TRANSITION_DRYRUN_ATTESTATION_PACKET_v0.md"
    )
    assert cosmo.get("lock_transition_dryrun_attestation_gate") == (
        "formal/python/tests/test_cosmo_bg_micro10_lock_transition_dryrun_attestation_packet_gate.py"
    )
    assert cosmo.get("lock_transition_dryrun_attestation_policy") == "DRYRUN_ATTESTATION_REQUIRED_NO_STATUS_FLIP"
    ...
    assert not missing, "State missing COSMO lock-transition dryrun attestation token(s): " + ", ".join(missing)


def test_cosmo_micro10_artifact_schema_and_token_alignment() -> None:
    micro_text = _read(COSMO_MICRO10_PATH)
    artifact_id = _extract_assignment_value(micro_text, "COSMO_BG_MICRO10_LOCK_TRANSITION_DRYRUN_ATTESTATION_ARTIFACT_v0")
    checkpoint = artifact_id.removesuffix("_v0")

    payload = _read_json(COSMO_MICRO10_ARTIFACT_PATH)
    ...
    assert body.get("scope") == "lock_transition_dryrun_attestation_packet_only_nonclaim_v0"

    policy_tokens = body.get("policy_tokens")
    assert isinstance(policy_tokens, list) and len(policy_tokens) == 3

    witnesses = body.get("required_dryrun_attestation_witnesses")
    assert isinstance(witnesses, list) and len(witnesses) == 5
