...
def test_review_accepts_exact_artifacts_and_independent_regeneration() -> None:
    ...
    assert verification["selected_next_target"] == (
        NONZERO_CURVATURE_GUARDRAIL_TARGET
    )


def test_review_accepts_only_level_three_locally_flat_connection_e_repro() -> None:
    ...
    assert report["equation_compendium_rows_activated"] == [PROPOSED_EQUATION_ID]
    assert report["equation_compendium_status"] == (
        "ACTIVE_CALCULATION_SURFACE_SCOPED_E_REPRO"
    )
    assert report["boundary"]["genuine_nonzero_curvature_validated"] is False
...
def test_review_verifies_geometry_thresholds_and_negative_control() -> None:
    verification = verify_calculation_result()
    assert verification["background_geometry_classification_match"] is True
    assert verification["all_six_thresholds_match"] is True
...
def test_review_and_nonzero_curvature_guardrail_are_preserved_after_rotation() -> None:
    ...
    assert active["claim_status"] == (
        "prepared_twelve_row_route_selection_result_review_pending"
    )
...
def test_successful_review_activates_only_covariant_identity_surface() -> None:
    ...
    assert PROPOSED_EQUATION_ID in compendium
...
def test_geometry_mismatch_is_localized_without_abort(tmp_path: Path) -> None:
    paths = _copy_artifacts(tmp_path)
    payload = json.loads(paths["output_path"].read_text(encoding="utf-8"))
    payload["background_geometry"]["scalar_curvature"] = 1.0
    paths["output_path"].write_bytes(canonical_json_bytes(payload))
    verification = verify_calculation_result(**paths)
    assert verification["accepted"] is False
    assert "geometry_classification_mismatch" in verification["mismatch_codes"]


def test_threshold_mismatch_is_localized_without_abort(tmp_path: Path) -> None:
    paths = _copy_artifacts(tmp_path)
    payload = json.loads(paths["output_path"].read_text(encoding="utf-8"))
...
def test_negative_control_mismatch_is_localized_without_abort(
    tmp_path: Path,
) -> None:
    paths = _copy_artifacts(tmp_path)
    payload = json.loads(paths["output_path"].read_text(encoding="utf-8"))
    payload["naive_partial_divergence_negative_control"]["failure_detected"] = False
    paths["output_path"].write_bytes(canonical_json_bytes(payload))
    verification = verify_calculation_result(**paths)
    assert verification["accepted"] is False
