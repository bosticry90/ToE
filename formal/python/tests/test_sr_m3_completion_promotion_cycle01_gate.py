...
TARGET_DOC_PATH = REPO_ROOT / "formal" / "docs" / "paper" / "DERIVATION_TARGET_SR_M3_COMPLETION_PROMOTION_v0.md"
...
ARTIFACT_PATH = REPO_ROOT / "formal" / "output" / "sr_m3_completion_promotion_cycle01_v0.json"

EXPECTED_ARTIFACT_ID = "sr_m3_completion_promotion_cycle01_v0"
...
def test_sr_m3_completion_promotion_cycle01_gate() -> None:
    ...
    assert ARTIFACT_PATH.exists(), "SR M3 completion promotion artifact is missing."
    ...
    m3_completion = sr_row.get("m3_completion", {})
    assert m3_completion.get("target_id") == "TARGET-SR-M3-COMPLETION-PROMOTION-v0"
    assert m3_completion.get("doc_path") == "formal/docs/paper/DERIVATION_TARGET_SR_M3_COMPLETION_PROMOTION_v0.md"
    assert m3_completion.get("artifact_path") == "formal/output/sr_m3_completion_promotion_cycle01_v0.json"
    assert m3_completion.get("gate_path") == "formal/python/tests/test_sr_m3_completion_promotion_cycle01_gate.py"

    for text in (target_text, sr_text, state_text, roadmap_text):
        assert _extract_token(text, "SR_M3_STATUS_v0") == "COMPLETE_BOUNDED_v0"
        assert _extract_token(text, "SR_M3_COMPLETION_ARTIFACT_v0") == EXPECTED_ARTIFACT_ID
        assert _extract_token(text, "SR_M3_COMPLETION_SHA256_v0") == artifact_hash
        assert _extract_token(text, "SR_M3_COMPLETION_GATE_v0") == EXPECTED_GATE
        assert _extract_token(text, "SR_M3_PROMOTION_READINESS_v0") == "FIRST_DISCRIMINATOR_CLOSED_AND_PROMOTED_v0"
