...
DOC = ROOT / "formal" / "docs" / "paper" / "TOE_QFT_GR_SEAM_PACKET33_ASSESSMENT_v0.md"
CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet33_assessment_checkpoint_v0.json"
)
PARENT_EXEC_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET33_BOUNDED_EXECUTION_v0.md"
)
PARENT_EXEC_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet33_bounded_execution_checkpoint_v0.json"
)
PARENT_AUTH_DOC = (
    ROOT
    / "formal"
    / "docs"
    / "paper"
    / "TOE_QFT_GR_SEAM_PACKET33_AUTHORIZATION_v0.md"
)
PARENT_AUTH_CHECKPOINT = (
    ROOT
    / "formal"
    / "output"
    / "toe_qft_gr_seam_packet33_authorization_checkpoint_v0.json"
)
...
def test_packet33_assessment_files_exist() -> None:
    assert DOC.exists(), f"Missing packet33 assessment doc: {DOC}"
    assert CHECKPOINT.exists(), f"Missing packet33 assessment checkpoint: {CHECKPOINT}"


def test_packet33_assessment_doc_required_markers() -> None:
    text = _read_text(DOC)
    required_markers = [
        ...
        "PACKET33_ASSESSMENT_COMPLETE_v0",
...
def test_packet33_assessment_checkpoint_schema_and_lineage() -> None:
    data = _load_json(CHECKPOINT)
    assert data["artifact_id"] == "toe_qft_gr_seam_packet33_assessment_checkpoint_v0"
    assert data["phase"] == "PHASE_2E_QFT_GR_SEAM_PACKET33_ASSESSMENT"
    assert data["scope"] == "packet33_target_satisfaction_assessment"
    assert data["status"] == "PACKET33_ASSESSMENT_COMPLETE_v0"

    payload = data["payload"]
    assert payload["assessment_doc_path"].endswith(
        "TOE_QFT_GR_SEAM_PACKET33_ASSESSMENT_v0.md"
    )
...
def test_packet33_assessment_target_satisfaction_and_packet34_rule() -> None:
    ...
    packet34 = payload["packet34_decision"]
    assert packet34["authorized"] is True
    assert packet34["progress_verdict"] == "GENUINE_PHYSICS_PROGRESS_IF_SINGLE_BOUNDED_TARGET_v0"
    assert packet34["ladder_extension_verdict"] == "REJECT_IF_MOMENTUM_EXTENSION_v0"
    assert packet34["verdict"] == "JUSTIFIED_CONDITIONAL_ON_SINGLE_BOUNDED_TARGET_v0"
    assert packet34["exact_bounded_target"] == EXPECTED_PACKET34_TARGET
    assert packet34["hold_refine_trigger_status"] == "NOT_HIT_v0"


def test_packet33_assessment_scalar_and_guardrail_invariants() -> None:
    payload = _load_json(CHECKPOINT)["payload"]
    ...
    assert guardrails["assessment_scope"] == "ASSESS_PACKET33_BEFORE_PACKET34_v0"


def test_packet33_assessment_authority_pointer_parity() -> None:
    ...
    for ref in refs:
        assert ref in state_text, f"Missing packet33 assessment pointer in State_of_the_Theory.md: {ref}"
        assert ref in roadmap_text, f"Missing packet33 assessment pointer in PHYSICS_ROADMAP_v0.md: {ref}"


def test_packet33_assessment_parent_chain_exists() -> None:
