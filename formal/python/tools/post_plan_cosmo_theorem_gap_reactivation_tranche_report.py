...
SCHEMA_ID = "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_REPORT_20260419_v0"
DEFAULT_DECLARATION_PATH = REPO_ROOT / "formal" / "docs" / "release" / "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_20260419_v0.json"
DEFAULT_OUT_PATH = REPO_ROOT / "formal" / "output" / "reports" / "post_plan_cosmo_theorem_gap_reactivation_tranche_20260419_v0.json"
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    dossier_path = REPO_ROOT / _text(required_inputs.get("cosmo_dossier_report"))
    prior_path = REPO_ROOT / _text(required_inputs.get("prior_cosmo_completion_tranche_report"))
    ...
    doc_path = REPO_ROOT / _text(required_inputs.get("cosmo_target_doc"))
    artifact_path = REPO_ROOT / _text(required_inputs.get("cosmo_artifact"))
    gate_path = REPO_ROOT / _text(required_inputs.get("cosmo_gate"))
    ...
    if not all([target_row, matrix_row]):
        terminal_outcome = "HOLD_PENDING_POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_REPAIR"
        next_action = "RESTORE_COSMO_REACTIVATION_INPUT_SHAPE_AND_RERUN"
    elif prerequisites_ok and row_truth_change_detected:
        terminal_outcome = "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_EXECUTED_AND_PROMOTED"
        next_action = "RERUN_THEOREM_GAP_RERANKING_AFTER_COSMO_BLOCKER_REDUCTION"
    elif prerequisites_ok and explicit_exhaustion_detected:
        terminal_outcome = "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_EXPLICITLY_EXHAUSTED"
        next_action = "RERUN_THEOREM_GAP_RERANKING_AFTER_COSMO_EXPLICIT_EXHAUSTION"
    else:
        terminal_outcome = "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_EVIDENCE_INCOMPLETE"
        next_action = "REPAIR_COSMO_REACTIVATION_EVIDENCE_OR_RETAIN_TERMINAL_HOLD"
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_OUTCOME",
            "no_loop_rule_declared": _text(contract.get("no_loop_rule"))
            == "ONE_POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
                "allowed_outcome_materialized": terminal_outcome in allowed_outcomes,
                "single_outcome_materialized": True,
                "promotion_only_if_row_truth_changed": (terminal_outcome != "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_EXECUTED_AND_PROMOTED") or row_truth_change_detected,
                "exhaustion_only_if_pruned_under_new_authorization": (terminal_outcome != "POST_PLAN_COSMO_THEOREM_GAP_REACTIVATION_TRANCHE_EXPLICITLY_EXHAUSTED") or explicit_exhaustion_detected,
            },
            "inputs": {
        ...
        "source_bundle": {
            ...
            "cosmo_dossier_report": _ptr(dossier_path),
            "prior_cosmo_completion_tranche_report": _ptr(prior_path),
        ...
        "non_claim_boundary": "Repository-local refreshed COSMO theorem-gap reactivation tranche only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the refreshed COSMO theorem-gap reactivation tranche report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "post_plan_cosmo_theorem_gap_reactivation_tranche_report: "
