...
ACCEPTED_REVIEW_FINDINGS = [
    ...
    "total conservation linked to accepted gauge/matter exchange halves",
    "exchange cancellation route preserved",
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "PSI_A_TOTAL_CONSERVATION_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_RESULT_REVIEW_20260627_v0.json"
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Derivation"
    / "PsiATotalConservationTheoremLinkageObligationCloseoutResultReview.lean"
)
...
def _consumed_closeout_valid(closeout: dict[str, Any]) -> bool:
    return (
        ...
        and closeout.get("local_psi_A_total_conservation_obligation_closed") is True
        and closeout.get("exchange_cancellation_route_constructed") is True
        and closeout.get("total_conservation_derived") is True
...
def build_psi_A_total_conservation_theorem_linkage_obligation_closeout_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        "local_total_conservation_closeout_accepted": (
            ...
            and closeout.get("T_total_definition_used") is True
        ),
        "exchange_cancellation_shape_preserved": (
            closeout.get("gauge_exchange_route") == GAUGE_EXCHANGE_ROUTE
            and closeout.get("matter_exchange_route") == MATTER_EXCHANGE_ROUTE
            and closeout.get("total_stress_energy_definition")
            == TOTAL_STRESS_ENERGY_DEFINITION
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PSI_A_TOTAL_CONSERVATION_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_RESULT_REVIEW"
    )
    payload: dict[str, Any] = {
        ...
        "packet_id": PACKET_ID,
        "status": (
            "ACTIVE_PSI_A_TOTAL_CONSERVATION_THEOREM_LINKAGE_OBLIGATION_"
        ...
        if accepted
        else "PSI_A_TOTAL_CONSERVATION_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_RESULT_REVIEW_REQUIRES_REMEDIATION",
        ...
        "follow_on_target_after_selector_review": FOLLOW_ON_TARGET_AFTER_SELECTOR_REVIEW,
        ...
        "non_claim_boundary": (
            "This result review accepts only the local psi-A total conservation "
            "theorem-linkage closeout: total conservation is linked to accepted "
            "gauge/matter exchange halves by exchange cancellation. It authorizes "
            "the next selector target only. It does not claim full Maxwell closure, "
        ...
        "critical_gate_fail_conditions": [
            "fail to consume review_psi_A_total_conservation_theorem_linkage_obligation_closeout_result",
            "fail to accept the local psi-A total conservation closeout",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PsiATotalConservationTheoremLinkageObligationCloseoutResultReview",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Review the local psi-A total conservation theorem-linkage closeout."
        )
    )
