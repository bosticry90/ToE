...
NEXT_TARGET = (
    "execute_qft_gr_renormalized_expectation_finiteness_assumption_reduction_"
    "attempt"
)
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_20260606_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The RN-ASSUMP-004 packet has been accepted; the next bounded "
                "step may attempt to reduce exactly the finiteness/regularity "
                "assumption."
            ),
        },
        {
            ...
            "reason": (
                "Packet result review does not discharge finiteness or "
...
def build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review(
    *,
    ...
    acceptance_criteria = {
        "consumes_expected_rn_assump_004_packet": packet.get("packet_id")
        == EXPECTED_PACKET_ID,
        ...
        is True,
        "prior_rn_assump_001_002_003_accepted": packet.get(
        ...
        "finiteness_regular_boundary_recorded": packet.get(
            "finiteness_regular_boundary"
        )
        == FINITENESS_REGULARITY_BOUNDARY
        and packet.get("finiteness_regular_boundary_condition")
        == FINITENESS_REGULARITY_BOUNDARY,
        "finiteness_regular_boundary_status_records_not_discharged": packet.get(
        ...
        "no_finiteness_regular_boundary_discharge": packet.get(
            "finiteness_regular_boundary_discharged"
        )
        is False
        and packet.get(
            "finiteness_regular_boundary_reduced_or_discharged_by_preparation"
        )
        is False
    ...
    return {
        ...
        if accepted
        else "QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION
        if accepted
        else "qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review_blocked",
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet": (
            EXPECTED_PACKET_ID
        ),
        "consumes_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "finiteness_regular_boundary_reduced_by_review": False,
        ...
        "authorized_attempt_scope": (
            "renormalized_expectation_finiteness_assumption_reduction_attempt_"
            "only_no_finiteness_regular_boundary_discharge_no_conservation_"
        ...
        if accepted
        else "REMEDIATE_QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW",
        "selected_next_target_kind": (
            "qft_gr_renormalized_expectation_finiteness_assumption_reduction_attempt_execution"
        ),
        "selected_route": (
        ...
        "non_claim_boundary": (
            "This result review accepts only the RN-ASSUMP-004 "
            "finiteness/regularity packet and authorizes one bounded reduction "
            "attempt. It does not discharge finiteness or regularity "
...
def write_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_result_review_report: "
