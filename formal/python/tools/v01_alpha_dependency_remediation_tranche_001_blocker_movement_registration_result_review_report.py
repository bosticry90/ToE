...
EXPECTED_REGISTRATION_OUTCOME = (
    "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_BLOCKER_MOVEMENT_REGISTERED_AS_"
...
REGISTERED_MOVEMENT = "release_blocking_to_documented_nonblocking_dependency"
...
def _other_obligations(registration: dict[str, Any]) -> list[dict[str, Any]]:
    return list(registration.get("other_release_blocking_obligations", []))


def _other_obligations_carried_forward(registration: dict[str, Any]) -> bool:
    rows = _other_obligations(registration)
    return (
        len(rows) == 5
...
def build_result_review(
    *,
    ...
    other_obligations = _other_obligations(registration)
    ...
    acceptance_criteria = {
        ...
        "registered_movement_tranche_001_only": registered_movement.get(
            "selected_remediation_finding_id"
        )
        == SELECTED_REMEDIATION_FINDING_ID
        and registered_movement.get("selected_dependency") == SELECTED_DEPENDENCY
        and registered_movement.get("movement_scope") == "tranche_001_only",
        ...
        is True
        and registration.get("tranche_001_formal_movement_accepted") is False,
    ...
    return {
        ...
        if accepted
        else "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW_BLOCKED",
        ...
        "tranche_001_formal_movement_accepted": accepted,
        "tranche_001_release_blocker_status": REGISTERED_BLOCKER_STATUS,
        "tranche_001_dependency_policy_remediation_satisfied": accepted,
        "tranche_001_cleared_for_global_release_readiness": False,
        "global_release_readiness_still_blocked": True,
        "release_blocking_obligation_count_after_review": len(other_obligations),
        ...
        if accepted
        else "REMEDIATE_V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_BLOCKER_MOVEMENT_REGISTRATION_RESULT_REVIEW",
        ...
        "candidate_next_targets": [
            {
                "target": NEXT_TARGET,
                "decision": "selected",
                "reason": "Tranche 001 movement is accepted, so select the next remediation tranche using the remaining five blockers.",
            },
            {
                "target": "execute_v01_alpha_dependency_remediation_tranche_002",
                "decision": "deferred",
                "reason": "Direct tranche 002 execution is deferred until a selection packet chooses the next tranche.",
            },
            {
                "target": "prepare_v01_alpha_release_readiness_adjudication_packet",
                "decision": "deferred",
                "reason": "Release-readiness adjudication remains blocked by five release-blocking obligations.",
            },
        ],
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "The v0.1-alpha dependency remediation tranche 001 blocker movement registration "
            "result review accepts the documented/nonblocking movement for V01-ALPHA-DEP-REM-001 "
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "v01_alpha_dependency_remediation_tranche_001_blocker_movement_registration_result_review_report: "
