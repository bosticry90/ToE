...
VALIDATION_PATH = (
    RELEASE_ROOT
    / "TOE_CURRENT_NATIVE_HYPOTHESIS_EVIDENCE_RECONCILIATION_OPEN_VALIDATION_v0.json"
)
...
KIND = "toe_current_native_hypothesis_evidence_reconciliation_stage_4_open_v0"
EVIDENCE = (
    "formal/toe_formal/ToeFormal/Derivation/"
    "ToeCurrentNativeHypothesisEvidenceReconciliationAttemptOpen.lean"
)
REPORT = (
...
OUTCOME = "CURRENT_HYPOTHESIS_EVIDENCE_RECONCILIATION_STAGE_4_OPEN"
STRICT_OUTCOME = (
    "STAGE_4_OPEN_RECONCILIATION_ONLY_NO_RECONCILIATION_RESULT_TRUTH_"
    "ADJUDICATION_EVIDENCE_PROMOTION_REPRESENTATION_ACTION_SEAM_CLOSURE_"
    "FRONTIER_SELECTION_OR_STAGE_5"
)
...
OPEN_EXACT_PATH_SET = [
    ...
    (
        "formal/docs/release/"
        "TOE_CURRENT_NATIVE_HYPOTHESIS_EVIDENCE_RECONCILIATION_"
...
def _check_authority() -> None:
    ...
    if authority["authorized_stage"] != {
        ...
        raise ValueError("Stage 4 OPEN authority does not match the manifest")
    binding = authority["stage_3_input_binding"]
    if binding["claim_count"] != 2673:
        raise ValueError("Stage 3 claim inventory is not bound")
    if binding["selected_source_count"] != 640:
        raise ValueError("Stage 3 selected-source count is not bound")
    if binding["source_count_outside_bounded_selection"] != 12923:
        raise ValueError("Stage 3 unreviewed-overflow count is not bound")
    limits = authority["workload_limits"]
    if limits["maximum_graph_nodes"] != 4096:
        raise ValueError("Stage 4 graph-node cap is not bound")
    if limits["maximum_graph_edges"] != 16384:
        raise ValueError("Stage 4 graph-edge cap is not bound")
    if authority["candidate_promotion_contract"][
        "promotion_decision_authorized"
    ]:
        raise ValueError("Stage 4 may not promote evidence")
    ...
    if review["decision"] != "AUTHORIZE_STAGE_4_OPEN":
        raise ValueError("Stage 4 OPEN authority review decision mismatch")
...
def _project_current_target(registry: dict, report_sha256: str) -> None:
    ...
    if len(active) != 1 or active[0]["workstream_id"] != TARGET:
        raise ValueError("expected the selected Stage 4 workstream to be active")
    workstream = active[0]
    workstream.update(
        {
            "queue_scope": (
                "Stage 4 current-hypothesis reconciliation is OPEN under "
                "immutable bounded-program authority; no reconciliation "
                "result exists"
            ),
            ...
            "consumed_target_kind": "closed_bounded_stage_3_scientific_authority",
            "claim_ceiling_level": 2,
            "claim_label": "B-BOUNDED",
            "claim_status": (
                "Stage 4 OPEN only for bounded current-hypothesis "
                "reconciliation; no result, truth adjudication, evidence "
                "promotion, representation, frontier selection, or Stage 5"
            ),
        }
    )
...
def _validation_payload(
    *,
    ...
    return {
        ...
        "authority_decision": "AUTHORIZE_STAGE_4_OPEN",
        ...
        "schema_id": (
            "toe.current_native_hypothesis_evidence_reconciliation."
            "stage_4_open_validation.v0"
        ),
        "scope_hash": EXPECTED_SCOPE_HASH,
        "scientific_output_at_open": {
            "canonical_evidence_promoted": False,
            "current_hypothesis_graph_produced": False,
            "native_frontier_selected": False,
            "reconciliation_performed": False,
            "reconciliation_result_produced": False,
            "representation_or_action_selected": False,
            "scientific_claim_adjudicated": False,
            "stage_5_output_produced": False,
        },
        "semantic_stage_id": SEMANTIC_STAGE_ID,
        "stage_3_input_binding": _read(AUTHORITY_PATH)["stage_3_input_binding"],
        ...
        "validation_checks": {
            ...
            "open_checkpoint_contains_no_reconciliation_result": True,
            "program_attempt_count_is_four": (
            ...
            "promotion_and_truth_adjudication_remain_prohibited": True,
            "reconciliation_only_boundary_preserved": True,
            "stage_3_close_is_bound": True,
            "stage_5_remains_prohibited": True,
...
def open_stage(*, opened_from_commit: str) -> str:
    ...
    if event["attempt_sequence_number"] != 4:
        raise ValueError("OPEN event attempt number mismatch")
    if event["event_sequence_number"] != 7:
