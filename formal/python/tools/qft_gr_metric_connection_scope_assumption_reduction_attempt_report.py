...
SCHEMA_ID = "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_20260527_v0"
ATTEMPT_ID = "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_v0"
OUTCOME_ID = (
    "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_EXECUTED_"
    "WITH_NO_CONSERVATION_WITNESS_OR_SEAM_CLOSURE"
)
CONSUMED_TARGET = EXPECTED_EXECUTION_TARGET
NEXT_TARGET = "review_qft_gr_metric_connection_scope_assumption_reduction_attempt_result"
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_20260527_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The bounded OD-ASSUMP-006 metric/connection-scope attempt "
                "must be result-reviewed before any conservation proof object, "
                "conservation witness, source admissibility, Bianchi "
                "compatibility, or seam-closure action."
            ),
        },
        {
            ...
            "reason": (
                "Reducing metric/connection scope does not authorize a "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Reducing metric/connection scope does not authorize source "
                "admissibility."
            ),
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "The bounded metric/connection-scope contract is not a Bianchi "
                "compatibility claim."
            ),
        },
        {
...
def build_qft_gr_metric_connection_scope_assumption_reduction_attempt(
    *,
    review_path: Path = DEFAULT_REVIEW_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    review = _read_json(review_path)
    selected_assumption = review.get("metric_connection_scope_assumption", {})
    ...
    classification_rows = [
        {
            ...
            "selected": True,
            "meaning": (
                "The metric/connection-scope assumption has been reduced to a "
                "bounded operator-domain metric/connection contract pending "
                "result review, without proving conservation."
            ),
        },
        {
            ...
            "meaning": (
                ...
                "metric/connection scope could be reduced."
            ),
        },
        {
    ...
    reduction_contract = {
        "contract_id": METRIC_CONNECTION_SCOPE_CONTRACT_ID,
        ...
        "typed_scope": (
            "bounded_metric_connection_scope_contract_for_selected_operator_"
            "domain_future_proof_object_only"
        ),
        "available_repo_evidence": selected_assumption.get("available_repo_evidence", []),
        "claim_ceiling": (
            "metric_connection_scope_reduced_pending_result_review_only_no_"
            "conservation_proof_no_conservation_witness_no_source_admissibility_"
            "no_bianchi_compatibility_no_qft_gr_seam_closure"
        ),
    }
    execution_findings = [
        "The attempt consumes the OD-ASSUMP-006 packet result review and keeps metric/connection scope as the only executed row.",
        "The bounded geometry domain is fixed for the selected operator domain without claiming source admissibility.",
        "The connection compatibility condition is scoped to the selected operator domain and is not a Bianchi compatibility claim.",
        "The reduction is pending result review and does not prove conservation or construct a conservation proof object.",
    ]
    acceptance_criteria = {
        ...
        is True,
        "prior_rows001_002_003_004_005_preserved": review.get(
        ...
        "prior_rows001_002_003_004_005_remain_accepted": review.get(
            "prior_rows001_002_003_004_005_remain_accepted"
        )
        is True,
        ...
        "metric_connection_scope_object_preserved": review.get(
            "metric_connection_scope_object"
        )
        == METRIC_CONNECTION_SCOPE_OBJECT
        and selected_assumption.get("metric_connection_scope_object")
        == METRIC_CONNECTION_SCOPE_OBJECT,
        "bounded_geometry_domain_preserved": review.get("bounded_geometry_domain")
        == BOUNDED_GEOMETRY_DOMAIN
        and selected_assumption.get("bounded_geometry_domain")
        == BOUNDED_GEOMETRY_DOMAIN,
        "connection_compatibility_condition_preserved": review.get(
            "connection_compatibility_condition"
        )
        == CONNECTION_COMPATIBILITY_CONDITION
        and selected_assumption.get("connection_compatibility_condition")
        == CONNECTION_COMPATIBILITY_CONDITION,
    ...
    return {
        ...
        if executed
        else "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_BLOCKED",
        "consumes_qft_gr_metric_connection_scope_assumption_reduction_packet_result_review": EXPECTED_REVIEW_ID,
        "consumes_qft_gr_metric_connection_scope_assumption_reduction_packet_result_review_pointer": _ptr(
            review_path
        ),
        ...
        "prior_accepted_conservation_form_scope_contract": CONSERVATION_FORM_SCOPE_CONTRACT_ID,
        ...
        "required_future_proof_object": REQUIRED_FUTURE_PROOF_OBJECT,
        "attempt_scope": (
            "EXECUTE_BOUNDED_QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_"
        ...
        "metric_connection_scope_reduction_contract": reduction_contract,
        "execution_findings": execution_findings,
        "metric_connection_scope_assumption_reduction_attempt_executed": executed,
        "metric_connection_scope_assumption_reduced_pending_result_review": executed,
        "metric_connection_scope_assumption_obstruction_identified": False,
        "metric_connection_scope_assumption_inconclusive": False,
        "metric_connection_scope_assumption_discharged": False,
        "metric_connection_scope_assumption_reduced_by_attempt": executed,
        ...
        "metric_connection_scope_claimed_as_source_admissibility": False,
        "bounded_geometry_domain_claimed_as_bianchi_compatibility": False,
        "connection_compatibility_claimed_as_bianchi_compatibility": False,
        ...
        if executed
        else "REMEDIATE_QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT",
        "selected_next_target_kind": (
            "qft_gr_metric_connection_scope_assumption_reduction_attempt_result_review"
        ),
        "selected_route": (
            "qft_gr_metric_connection_scope_assumption_reduction_attempt_result_"
            "review_after_execution"
        ),
        "selection_count": 1 if executed else 0,
        "next_action_scope": (
            "REVIEW_QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_"
            "RESULT_ONLY_NO_CONSERVATION_WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This execution reduces only OD-ASSUMP-006 metric/connection scope "
            "to a bounded operator-domain metric/connection contract pending "
            "result review. It does not prove conservation, construct a "
...
def write_qft_gr_metric_connection_scope_assumption_reduction_attempt(
    *,
    review_path: Path = DEFAULT_REVIEW_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_metric_connection_scope_assumption_reduction_attempt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_metric_connection_scope_assumption_reduction_attempt(
        review_path=review_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_metric_connection_scope_assumption_reduction_attempt_report: "
