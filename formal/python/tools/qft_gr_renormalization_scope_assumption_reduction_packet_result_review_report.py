...
SCHEMA_ID = "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_20260606_v0"
REVIEW_ID = "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_v0"
OUTCOME_ID = (
...
NEXT_TARGET = "execute_qft_gr_renormalization_scope_assumption_reduction_attempt"
AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS = [
    "qft_gr_renormalization_scope_assumption_reduced_pending_result_review",
    "qft_gr_renormalization_scope_assumption_obstruction_identified_requires_refinement",
    "qft_gr_renormalization_scope_assumption_inconclusive_requires_assumption_reduction",
]
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_20260606_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The RN-ASSUMP-002 packet has been accepted; the next bounded "
                "step may attempt to reduce exactly the renormalization scope "
                "assumption."
            ),
        },
        {
            ...
            "decision": "deferred",
            "reason": (
                "Renormalized expectation finiteness and regularity remain "
                "downstream of the bounded RN-ASSUMP-002 attempt."
            ),
        },
        {
            "target": "prepare_qft_gr_renormalized_expectation_domain_assumption_reduction_packet",
            "decision": "deferred",
            "reason": (
                "Further renormalized expectation domain work remains downstream "
                "of the selected scope row."
            ),
        },
        {
            "target": "discharge_qft_gr_renormalization_scope_assumption",
            "decision": "not_authorized",
            "reason": "Packet result review does not discharge renormalization scope assumptions.",
        },
        {
...
def build_qft_gr_renormalization_scope_assumption_reduction_packet_result_review(
    *,
    ...
    selected_status = packet.get("renormalization_scope_status_tokens", [])

    acceptance_criteria = {
        "consumes_expected_rn_assump_002_packet": packet.get("packet_id")
        == EXPECTED_PACKET_ID,
        ...
        is True
        and packet.get("prior_completed_family") == PRIOR_COMPLETED_FAMILY,
        "prior_rn_assump_001_accepted": packet.get(
        ...
        "renormalization_scope_object_recorded": packet.get(
            "renormalization_scope"
        )
        == RENORMALIZATION_SCOPE
        and packet.get("renormalization_scope_object") == RENORMALIZATION_SCOPE,
        "renormalization_scope_status_records_not_discharged": packet.get(
            "renormalization_scope_status"
        )
        == RENORMALIZATION_SCOPE_STATUS,
        "scope_boundaries_recorded": packet.get("scope_boundaries")
        == SCOPE_BOUNDARIES,
        "packet_preparation_only_confirmed": packet.get("prepared") is True
        and packet.get(
            "renormalization_scope_assumption_reduction_analysis_prepared"
        )
        is True
        and packet.get("prepares_reduction_analysis_only") is True,
        "no_renormalization_scope_discharge": packet.get(
    ...
    return {
        ...
        if accepted
        else "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION
        if accepted
        else "qft_gr_renormalization_scope_assumption_reduction_packet_result_review_blocked",
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_renormalization_scope_assumption_reduction_packet": (
            EXPECTED_PACKET_ID
        ),
        "consumes_qft_gr_renormalization_scope_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "renormalization_scope_assumption_reduced_by_review": False,
        ...
        "authorized_attempt_scope": (
            "renormalization_scope_assumption_reduction_attempt_only_no_"
            "renormalization_scope_discharge_no_conservation_witness_no_qft_gr_"
            "seam_closure"
        ),
        "authorized_attempt_result_classifications": (
            AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS
        ...
        if accepted
        else "REMEDIATE_QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW",
        "selected_next_target_kind": (
            "qft_gr_renormalization_scope_assumption_reduction_attempt_execution"
        ),
        "selected_route": (
            "qft_gr_renormalization_scope_assumption_reduction_attempt_after_"
            "packet_result_review"
        ),
        "selection_count": 1 if accepted else 0,
        "selected_next_target_count": selected_next_target_count if accepted else 0,
        "next_action_scope": (
            "EXECUTE_QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_"
            "ONLY_NO_RENORMALIZATION_SCOPE_DISCHARGE_CONSERVATION_WITNESS_OR_"
            "QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the RN-ASSUMP-002 "
            "renormalization-scope packet and authorizes one bounded reduction "
            "attempt. It does not discharge the renormalization scope "
...
def write_qft_gr_renormalization_scope_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_renormalization_scope_assumption_reduction_packet_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_renormalization_scope_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_renormalization_scope_assumption_reduction_packet_result_review_report: "
