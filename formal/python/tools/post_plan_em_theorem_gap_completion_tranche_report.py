...
SCHEMA_ID = "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_REPORT_20260418_v0"
DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_20260418_v0.json"
)
DEFAULT_OUT_PATH = (
    REPO_ROOT / "formal" / "output" / "reports" / "post_plan_em_theorem_gap_completion_tranche_20260418_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    doc_path = REPO_ROOT / str(required_inputs.get("em_target_doc", "")).strip()
    artifact_path = REPO_ROOT / str(required_inputs.get("em_artifact", "")).strip()
    gate_path = REPO_ROOT / str(required_inputs.get("em_gate", "")).strip()
    ...
    queue_ok = (
        ...
        and queue_rows.get(target_row_id, {}).get("queue_rank") == 5
        and queue_rows.get(target_row_id, {}).get("row_id") == str(policy.get("required_queue_follow_on_row", "")).strip()
    )
    qft_ok = (
        qft_report.get("summary", {}).get("terminal_outcome")
        == str(policy.get("required_prior_qft_outcome", "")).strip()
    )
    ...
    artifact_ok = (
        artifact.get("artifact_id") == "em_empirical_comparison_packet_04_v0"
    ...
    default_outcome = str(
        outcome_contract.get("default_outcome", "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE")
    ).strip()

    if not matrix_row or not target_row:
        terminal_outcome = "HOLD_PENDING_POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_REPAIR"
        next_action = "RESTORE_EM_TRANCHE_INPUT_SHAPE_AND_RERUN"
    elif all([queue_ok, qft_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and row_truth_change_detected:
        terminal_outcome = "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED"
        next_action = "REASSESS_SEAM_ROUTES_AND_MASTER_ACTION_WITH_CHANGED_EM_ROW_TRUTH"
    elif all([queue_ok, qft_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and explicit_exhaustion_detected:
        terminal_outcome = "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED"
        next_action = "PREPARE_POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_WITH_EM_EXHAUSTION_RECORDED"
    elif all([queue_ok, qft_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]):
        terminal_outcome = "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_NONPROMOTED"
        next_action = "PREPARE_POST_PLAN_SR_THEOREM_GAP_COMPLETION_TRANCHE_WITH_EM_NONMOVING_HISTORY_RECORDED"
    else:
        terminal_outcome = "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE"
        next_action = "REPAIR_EM_TRANCHE_EVIDENCE_AND_RERUN"
    ...
    return {
        ...
        "criteria": {
            ...
            "prior_qft_nonmoving_history_recorded": qft_ok,
            "target_map_materialized": target_map_ok,
            "em_doc_tokens_present": doc_ok,
            "em_artifact_alignment_ok": artifact_ok,
            "em_row_alignment_ok": row_ok,
            "em_live_theorem_gap_observation_present": contradiction_ok,
            "single_terminal_outcome_rule_declared": str(outcome_contract.get("single_terminal_outcome_rule", "")).strip()
            == "EXACTLY_ONE_ALLOWED_POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_OUTCOME",
            "no_loop_rule_declared": str(outcome_contract.get("no_loop_rule", "")).strip()
            == "ONE_POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
                "allowed_outcome_materialized": terminal_outcome in allowed_outcomes,
                "single_outcome_materialized": True,
                "promotion_only_if_row_truth_changed": (terminal_outcome != "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED") or row_truth_change_detected,
                "exhaustion_only_if_explicit_decision_present": (terminal_outcome != "POST_PLAN_EM_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED") or explicit_exhaustion_detected,
            },
            "inputs": {
                "target_row_id": target_row_id,
                "queue_rank": queue_rows.get(target_row_id, {}).get("queue_rank"),
                "prior_qft_outcome": qft_report.get("summary", {}).get("terminal_outcome"),
        ...
        "source_bundle": {
            ...
            "em_target_doc": _ptr(doc_path),
            "em_artifact": _ptr(artifact_path),
            "em_gate": _ptr(gate_path),
        },
        "non_claim_boundary": "Repository-local post-plan EM theorem-gap completion tranche measurement only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the post-plan EM theorem-gap completion tranche report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "post_plan_em_theorem_gap_completion_tranche_report: "
