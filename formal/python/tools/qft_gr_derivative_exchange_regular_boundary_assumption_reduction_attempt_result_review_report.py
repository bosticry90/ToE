...
from formal.python.tools.qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_report import (
...
SCHEMA_ID = (
    ...
    "ATTEMPT_RESULT_REVIEW_20260608_v0"
)
REVIEW_ID = (
    "QFT_GR_DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_ASSUMPTION_REDUCTION_"
...
NEXT_ROW_ID = "MR-ASSUMP-002-weak_strong_conservation_comparison_scope"
NEXT_ROW_OBJECT = WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE
...
DEFAULT_OUT = (
    ...
    / (
        "QFT_GR_DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_ASSUMPTION_REDUCTION_"
        "ATTEMPT_RESULT_REVIEW_20260608_v0.json"
    )
)
    ...
    return [
        {
            ...
            "reason": (
                ...
                "places MR-ASSUMP-002-weak_strong_conservation_comparison_scope "
                "after the accepted bounded MR-ASSUMP-001 reduction."
            ),
        },
        {
            "target": (
            ...
            "decision": "deferred",
            "reason": (
                "MR-ASSUMP-003 remains after MR-ASSUMP-002 in the packet row "
                "inventory."
            ),
        },
        {
            ...
            "decision": "deferred",
            "reason": (
                "MR-ASSUMP-004 remains after MR-ASSUMP-002 and MR-ASSUMP-003 "
                "in the packet row inventory."
            ),
        },
        {
            ...
            "reason": (
                ...
                "derivative-exchange regularity discharge."
            ),
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Accepting MR-ASSUMP-001 does not discharge the mathematical "
                "regularity family."
            ),
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "Derivative-exchange regularity is not a state-admissibility claim.",
        },
        {
            "target": "claim_qft_gr_source_admissibility",
            "decision": "not_authorized",
            "reason": "Derivative-exchange regularity is not source admissibility.",
        },
        {
...
def _global_nonclaims() -> dict[str, bool]:
    return {
        ...
        "derivative_exchange_regular_boundary_assumption_discharged_by_review": False,
        "derivative_exchange_regular_boundary_reduced_or_discharged_by_review": False,
        ...
        "derivative_exchange_regular_boundary_globally_solved_by_review": False,
...
def _acceptance_criteria(
    ...
    contract = attempt.get("derivative_exchange_regular_boundary_reduction_contract", {})
    rows = packet.get("candidate_reducible_assumptions", [])
    row_ids = [row.get("assumption_id") for row in rows]
    selected_index = row_ids.index(SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW)
    ...
    return {
        ...
        == SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW,
        "confirms_mr_assump_001_reduced_pending_review": attempt.get(
            "derivative_exchange_regular_boundary_assumption_reduced_pending_result_review"
        )
        is True
        and attempt.get("derivative_exchange_regular_boundary_assumption_reduced_by_attempt")
        is True
        and attempt.get("derivative_exchange_regular_boundary_assumption_obstruction_identified")
        is False
        and attempt.get("derivative_exchange_regular_boundary_assumption_inconclusive")
        is False,
        "preserves_bounded_derivative_exchange_contract": contract.get("contract_id")
        == DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_CONTRACT_ID
        and contract.get("contract_status")
        == BOUNDED_DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_CONTRACT_STATUS
        and contract.get("assumption_id")
        == SELECTED_BOUNDED_MATHEMATICAL_REGULARITY_ROW
        and contract.get("derivative_exchange_regular_boundary")
        == DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY,
        "does_not_globally_solve_derivative_exchange_regular_boundary": attempt.get(
            "derivative_exchange_regular_boundary_globally_solved"
        )
        is False
        and attempt.get("derivative_exchange_regular_boundary_globally_solved_by_attempt")
        is False,
...
def build_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review(
    *,
    ...
    return {
        ...
        "consumes_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt": (
            EXPECTED_ATTEMPT_ID
        ),
        "consumes_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "derivative_exchange_regular_boundary_reduction_contract": attempt.get(
            "derivative_exchange_regular_boundary_reduction_contract"
        ),
        "accepted_contract_id": DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_CONTRACT_ID,
        "bounded_derivative_exchange_regular_boundary_contract_status": (
            BOUNDED_DERIVATIVE_EXCHANGE_REGULAR_BOUNDARY_CONTRACT_STATUS
        ),
        "derivative_exchange_regular_boundary_assumption_reduction_attempt_result_reviewed": (
            accepted
        ),
        "derivative_exchange_regular_boundary_assumption_reduction_accepted": accepted,
        "derivative_exchange_regular_boundary_assumption_reduction_rejected": (
            not accepted
        ),
        "derivative_exchange_regular_boundary_assumption_reduced_pending_result_review_accepted": (
            accepted
        ),
        ...
        "weak_strong_conservation_comparison_scope": NEXT_ROW_OBJECT,
        "weak_strong_conservation_comparison_scope_required_future_proof_object": (
            NEXT_ROW_REQUIRED_FUTURE_PROOF_OBJECT
        ),
        ...
        **nonclaims,
        ...
        "next_action_scope": (
            ...
            "ASSUMPTION_REDUCTION_PACKET_ONLY_NO_CONSERVATION_WITNESS_OR_"
            "QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": criteria,
        "non_claim_boundary": nonclaims,
        "claim_ceiling": [
            "no global derivative-exchange regularity solve",
        ...
        "failure_mode_if_unresolved": (
            "mathematical-regularity progress remains blocked before weak/strong "
            "conservation comparison scope can be prepared"
        ),
    }


def write_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    packet_path: Path = DEFAULT_PACKET_PATH,
    ...
    payload = build_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "regular-boundary assumption-reduction attempt result review."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review(
        attempt_path=attempt_path,
        packet_path=packet_path,
        out=out,
    ...
    print(
        "qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review_report: "
