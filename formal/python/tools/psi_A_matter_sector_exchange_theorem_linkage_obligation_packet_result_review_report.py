...
SCHEMA_ID = (
    "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_RESULT_"
    "REVIEW_20260627_v0"
)
...
NEXT_TARGET = "prepare_psi_A_matter_sector_exchange_theorem_linkage_attempt_from_dirac_pair"
NEXT_TARGET_KIND = "psi_A_matter_sector_exchange_theorem_linkage_attempt_from_dirac_pair_preparation"
...
ACCEPTED_REVIEW_FINDINGS = [
    "matter-sector exchange obligation scoped",
    "target equation recorded",
    "T_psi policy preserved",
    "Dirac equation and adjoint equation context preserved",
...
DEFAULT_OUT = (
    ...
    / "release"
    / (
        "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_"
...
LEAN_PACKET_PATH = (
    ...
    / "ToeFormal"
    / "Derivation"
    / "PsiAMatterSectorExchangeTheoremLinkageObligationPacketResultReview.lean"
)
...
def build_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        "matter_exchange_target_scope_accepted": (
            theorem_shape["given"] == THEOREM_SHAPE_GIVEN
            and theorem_shape["then"] == TARGET
            and theorem_shape["watch_items"] == WATCH_ITEMS
        ),
        "dirac_context_preserved": (
            T_PSI_POLICY in theorem_shape["given"]
            and DIRAC_EQUATION in theorem_shape["given"]
            and ADJOINT_DIRAC_EQUATION in theorem_shape["given"]
            and CURRENT_DEFINITION in theorem_shape["given"]
        ),
        "watch_items_preserved": len(WATCH_ITEMS) == 10,
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_RESULT_REVIEW"
    )
    payload: dict[str, Any] = {
        ...
        "packet_id": PACKET_ID,
        "status": (
            "ACTIVE_PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_RESULT_REVIEW"
        ),
        ...
        if accepted
        else "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_RESULT_REVIEW_REQUIRES_REMEDIATION",
        ...
        "non_claim_boundary": (
            "This result review accepts only the psi-A matter-sector exchange "
            "theorem-linkage obligation packet scope. It records that the "
            "matter stress-energy policy, Dirac equation and adjoint equation "
            "context, current definition J^alpha = q psibar gamma^alpha psi, "
            "and watch items are preserved for a later Dirac-pair attempt. It "
            "does not execute any proof, discharge any theorem, discharge "
            "GAP-1 through GAP-8 globally, promote any C_k rule, embed C_k in "
            "an action, vary C_k, close a seam, claim empirical validation, or "
            "promote the master action."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume review_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_result",
            "fail to accept the matter-sector exchange target scope",
            "fail to preserve the T_psi policy",
            "fail to preserve the Dirac equation and adjoint equation context",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PsiAMatterSectorExchangeTheoremLinkageObligationPacketResultReview",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Review the psi-A matter-sector exchange theorem-linkage obligation packet."
        )
    )
    ...
    payload = build_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_result_review(
