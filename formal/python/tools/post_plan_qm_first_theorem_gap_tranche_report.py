...
SCHEMA_ID = "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_REPORT_20260418_v0"
DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_20260418_v0.json"
)
DEFAULT_OUT_PATH = (
    REPO_ROOT / "formal" / "output" / "reports" / "post_plan_qm_first_theorem_gap_tranche_20260418_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    doc_path = REPO_ROOT / str(required_inputs.get("qm_target_doc", "")).strip()
    artifact_path = REPO_ROOT / str(required_inputs.get("qm_artifact", "")).strip()
    gate_path = REPO_ROOT / str(required_inputs.get("qm_gate", "")).strip()
    ...
    target_map_ok = (
        ...
        and target_row.get("route_class") == str(policy.get("required_target_route_class", "")).strip()
    )
    phase2_ok = phase2.get("summary", {}).get("terminal_outcome") == str(policy.get("required_phase2_outcome", "")).strip()
    doc_ok = all(
        token in doc_text
        for token in [
            "DERIVATION_TARGET_QM_EMPIRICAL_COMPARISON_PACKET_04_v0",
            "QM_EMPIRICAL_PACKET_04_STATUS_v0: RUN_BOUNDED_v0_NONCLAIM",
            "QM_EMPIRICAL_PACKET_04_DECISION_v0: INCONCLUSIVE_v0",
    ...
    artifact_ok = (
        artifact.get("artifact_id") == "qm_empirical_comparison_packet_04_v0"
    ...
    default_outcome = str(outcome_contract.get("default_outcome", "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EVIDENCE_INCOMPLETE")).strip()

    if not matrix_row or not target_row:
        terminal_outcome = "HOLD_PENDING_POST_PLAN_QM_THEOREM_GAP_TRANCHE_REPAIR"
        next_action = "RESTORE_QM_PHASE3_INPUT_SHAPE_AND_RERUN"
    elif all([target_map_ok, phase2_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and row_truth_change_detected:
        terminal_outcome = "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EXECUTED_AND_PROMOTED"
        next_action = "REASSESS_SEAM_ROUTES_AND_MASTER_ACTION_WITH_CHANGED_QM_ROW_TRUTH"
    elif all([target_map_ok, phase2_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and explicit_exhaustion_detected:
        terminal_outcome = "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EXPLICITLY_EXHAUSTED"
        next_action = "SELECT_NEXT_THEOREM_GAP_ROW_WITH_EXPLICIT_QM_EXHAUSTION_RECORDED"
    elif all([target_map_ok, phase2_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]):
        terminal_outcome = "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EXECUTED_NONPROMOTED"
        next_action = "RETAIN_CURRENT_SEAM_AND_MASTER_ACTION_CLASSES_AND_SELECT_NEXT_THEOREM_GAP_TRANCHE"
    else:
        terminal_outcome = "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EVIDENCE_INCOMPLETE"
        next_action = "REPAIR_QM_PHASE3_EVIDENCE_AND_RERUN"
    ...
    return {
        ...
        "criteria": {
            ...
            "phase2_nonpromoted_measurement_recorded": phase2_ok,
            "qm_doc_tokens_present": doc_ok,
            "qm_artifact_alignment_ok": artifact_ok,
            "qm_row_alignment_ok": row_ok,
            "qm_live_theorem_gap_observation_present": contradiction_ok,
            "single_terminal_outcome_rule_declared": str(outcome_contract.get("single_terminal_outcome_rule", "")).strip()
            == "EXACTLY_ONE_ALLOWED_POST_PLAN_QM_THEOREM_GAP_TRANCHE_OUTCOME",
            "no_loop_rule_declared": str(outcome_contract.get("no_loop_rule", "")).strip()
            == "ONE_POST_PLAN_QM_THEOREM_GAP_TRANCHE_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
                "allowed_outcome_materialized": terminal_outcome in allowed_outcomes,
                "single_outcome_materialized": True,
                "promotion_only_if_row_truth_changed": (terminal_outcome != "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EXECUTED_AND_PROMOTED") or row_truth_change_detected,
                "exhaustion_only_if_explicit_decision_present": (terminal_outcome != "POST_PLAN_QM_FIRST_THEOREM_GAP_TRANCHE_EXPLICITLY_EXHAUSTED") or explicit_exhaustion_detected,
            },
            "inputs": {
        ...
        "source_bundle": {
            ...
            "qm_target_doc": _ptr(doc_path),
            "qm_artifact": _ptr(artifact_path),
            "qm_gate": _ptr(gate_path),
        },
        "non_claim_boundary": "Repository-local post-plan QM first theorem-gap tranche measurement only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the post-plan QM first theorem-gap tranche report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"post_plan_qm_first_theorem_gap_tranche_report: terminal_outcome={payload['summary']['terminal_outcome']} out={out}")
    return 0
