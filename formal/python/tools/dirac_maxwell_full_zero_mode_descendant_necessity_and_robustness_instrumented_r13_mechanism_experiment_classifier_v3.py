...
the strict v3 evidence assembler, and evaluates H_A--H_D only from metrics
...
CLASSIFIER_ID = (
    ...
    "INSTRUMENTED_R13_MECHANISM_EXPERIMENT_CLASSIFIER_v3"
)
SCRIPT_RELATIVE_PATH = (
    "formal/python/tools/dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_classifier_v3.py"
)
HYPOTHESES_A_TO_D = (
...
def _blocked(
    ...
    return {
        ...
        "assembler_id": raw_v3.ASSEMBLER_ID,
...
def _evaluate_h_b(
    ...
    if block_id not in raw_v3.BLOCK_IDS:
    ...
    if any(set(value) != set(raw_v3.BLOCK_IDS) for value in (loose_shares, tight_shares, neighbor_shares)):
...
def _classify_assembled(evidence: raw_v3.AssembledRawEvidence) -> dict[str, Any]:
...
def classify_from_raw_payloads(
    ...
    try:
        assembled = raw_v3.assemble_raw_evidence(
            repo_root,
        )
