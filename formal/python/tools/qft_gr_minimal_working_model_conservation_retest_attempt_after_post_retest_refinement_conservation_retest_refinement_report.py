...
SCHEMA_ID = (
    ...
    "RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_20260614_v0"
)
ATTEMPT_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
...
NEXT_TARGET_KIND = (
    "qft_gr_minimal_working_model_conservation_retest_attempt_after_post_"
    "retest_refinement_conservation_retest_refinement_result_review"
)
RETEST_RESULT = "inconclusive"
RETEST_STATUS = (
    "bounded_conservation_retest_attempt_after_post_retest_refinement_"
    "conservation_retest_refinement_executed_inconclusive_pending_result_review"
)
DEFAULT_OUT = (
    REPO_ROOT
    / "formal"
    ...
    / (
        "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
        "RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_20260614_v0.json"
    )
)
    ...
    return [
        {
            ...
            "selected": False,
            "meaning": (
                "The v3 conservation retest passed, still pending result "
        ...
        },
        {
            ...
            "selected": False,
            "meaning": (
                "The v3 conservation retest exposed an explicit obstruction "
        ...
        },
        {
            ...
            "selected": True,
            "meaning": (
                "The bounded v3 retest was executed, but the packet "
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The executed v3 conservation retest must be result-reviewed "
        ...
        },
        {
            ...
            "reason": "This bounded v3 retest execution target is consumed here.",
        },
        {
            "target": (
...
def _retest_execution_matrix(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "pass_criteria_evaluation": _criterion_rows(
            ...
            reason=(
                "The packet defines the v3 weak pairing domain, v3 regularity "
                "context, and v2 test class, but it supplies no zero "
        ...
        "fail_criteria_evaluation": _criterion_rows(
            ...
            reason=(
                ...
                "or limit/interchange step inside the v3 scope."
            ),
        ),
...
def _why_inconclusive(packet: dict[str, Any]) -> list[str]:
    ...
    return [
        (
            ...
            "v3 toy weak-pairing setting."
        ),
        (
            "toy_weak_pairing_domain_v3_candidate narrows admitted pairings, "
            "but does not establish full source-domain membership."
        ),
        (
            "toy_regular_context_v3_candidate narrows derivative, boundary, "
            "regularization, and limit/interchange use, but does not prove "
            "vanishing weak divergence for every admitted test vector."
        ),
        (
            "toy_conservation_test_function_class_v2_candidate narrows the "
            "test-vector scope but does not convert the toy candidate into an "
            "admissible source."
        ),
        (
            "toy_source_candidate_definition_v3_candidate remains a candidate "
...
def _what_was_tested(packet: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "defined weak pairings under toy_weak_pairing_domain_v3_candidate",
        (
            "derivative, divergence, regularization, boundary-term, and "
            "limit/interchange admission under toy_regular_context_v3_candidate"
        ),
        (
            "compact-support or admissible test-vector weak-divergence pairings "
            "in toy_conservation_test_function_class_v2_candidate"
        ),
        "explicit pass, fail, and inconclusive criteria from the v3 packet",
    ]
...
def build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement(
    *,
    ...
    packet_pointer = review.get(
        "consumes_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_pointer"
    )
    ...
    acceptance_criteria = {
        ...
        "bounded_v3_retest_executed_only": retest_condition.get("condition_id")
        == RETEST_CONDITION_ID
    ...
    return {
        ...
        if executed
            ...
            "retest_refinement_conservation_retest_refinement_requires_remediation"
        ),
        ...
        "consumes_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_result_review": (
            EXPECTED_PACKET_RESULT_REVIEW_ID
        ),
        "consumes_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_result_review_pointer": _ptr(
        ...
        if executed
            ...
            "ATTEMPT_AFTER_POST_RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT"
        ),
...
def write_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement(
    *,
    packet_result_review_path: Path = DEFAULT_PACKET_RESULT_REVIEW_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR minimal working model bounded v3 conservation-"
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement(
    ...
    print(
        "qft_gr_minimal_working_model_conservation_retest_attempt_after_post_"
        "retest_refinement_conservation_retest_refinement_report: "
