...
SCHEMA_ID = "QFT_GR_COVARIANT_CONSERVATION_PROOF_OBJECT_PACKET_RESULT_REVIEW_20260525_v0"
REVIEW_ID = "QFT_GR_COVARIANT_CONSERVATION_PROOF_OBJECT_PACKET_RESULT_REVIEW_v0"
OUTCOME_ID = (
...
NEXT_TARGET = "execute_qft_gr_covariant_conservation_proof_object_attempt"
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_COVARIANT_CONSERVATION_PROOF_OBJECT_PACKET_RESULT_REVIEW_20260525_v0.json"
)
    ...
    return [
        {
            "target": NEXT_TARGET,
            "decision": "selected",
            "reason": "Accepted proof-object packet preparation may authorize only the bounded proof-object attempt.",
        },
        {
            "target": CONSUMED_TARGET,
            "decision": "consumed",
            "reason": "The packet result review is consumed by this checkpoint.",
        },
        {
            "target": "execute_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt",
            "decision": "deferred",
            "reason": "The broader conservation witness attempt remains downstream of the proof-object attempt.",
        },
        {
            "target": "close_qft_gr_seam",
            "decision": "not_authorized",
            "reason": "Proof-object packet result review does not close QFT-GR.",
        },
        {
...
def build_qft_gr_covariant_conservation_proof_object_packet_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        == SELECTED_OBSTRUCTION,
        "target_proof_object_expected": packet.get("target_proof_object")
        == TARGET_PROOF_OBJECT,
        "proof_object_packet_prepared_only": packet.get(
            "prepares_proof_object_packet_only"
        )
        is True
    ...
    return {
        ...
        if accepted
        else "QFT_GR_COVARIANT_CONSERVATION_PROOF_OBJECT_PACKET_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION,
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_covariant_conservation_proof_object_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_covariant_conservation_proof_object_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "proof_object_packet_preparation_accepted": accepted,
        "bounded_proof_object_attempt_authorized": accepted,
        "proof_object_packet_prepared_only": True,
        ...
        if accepted
        else "REMEDIATE_QFT_GR_COVARIANT_CONSERVATION_PROOF_OBJECT_PACKET_RESULT_REVIEW",
        "selected_next_target_kind": (
            "qft_gr_covariant_conservation_proof_object_attempt_execution"
        ),
        "selection_count": 1 if accepted else 0,
        "next_action_scope": (
            "EXECUTE_QFT_GR_COVARIANT_CONSERVATION_PROOF_OBJECT_ATTEMPT_ONLY_"
            "NO_CONSERVATION_WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts proof-object packet preparation and "
            "authorizes only a bounded proof-object attempt. It does not "
            "construct the proof object or conservation witness, claim source "
...
def write_qft_gr_covariant_conservation_proof_object_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_covariant_conservation_proof_object_packet_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR covariant conservation proof-object packet result review."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_covariant_conservation_proof_object_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_covariant_conservation_proof_object_packet_result_review_report: "
