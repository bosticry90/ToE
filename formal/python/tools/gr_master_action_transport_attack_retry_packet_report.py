...
SCHEMA_ID = "GR_MASTER_ACTION_TRANSPORT_ATTACK_RETRY_PACKET_REPORT_20260412_v0"
...
DEFAULT_DECLARATION_PATH = (
    ...
    / "docs"
    / "release"
    / "GR_MASTER_ACTION_TRANSPORT_ATTACK_RETRY_PACKET_20260412_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    if not preconditions_ok:
        ...
        next_action = "PROMOTE_GR_MASTER_ACTION_TRANSPORT_AFTER_RETRY_MOVE"
    elif theorem_gap_delta == 0 and not target_row_success_incremented and row_success_count == 0:
        terminal_outcome = "GR_TRANSPORT_OBLIGATION_DECLARED_BUT_STILL_INSUFFICIENT"
        next_action = "ESCALATE_STRUCTURE_BEYOND_DECLARED_TRANSPORT_OBLIGATION"
    else:
        terminal_outcome = "GR_VALID_BUT_NONMOVING"
        next_action = "HOLD_GR_TRANSPORT_RETRY_RESULT_AND_RESELECT_IF_NEEDED"
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_GR_MASTER_ACTION_TRANSPORT_RETRY_OUTCOME",
            "no_loop_rule_declared": str(ruling_contract.get("no_loop_rule", "")).strip()
            == "ONE_GR_MASTER_ACTION_TRANSPORT_RETRY_PACKET_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "source_bundle": {
            ...
            "gr_master_action_transport_attack_packet_declaration": _ptr(prior_decl_path),
            "gr_master_action_transport_attack_packet_report": _ptr(prior_report_path),
        ...
        "non_claim_boundary": "Repository-local GR master-action transport retry packet report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate GR master-action transport attack retry packet report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "gr_master_action_transport_attack_retry_packet_report: "
