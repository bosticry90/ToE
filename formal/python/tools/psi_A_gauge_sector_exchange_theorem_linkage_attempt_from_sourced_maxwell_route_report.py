...
INPUT_ROUTE = "gauge stress-energy divergence identity plus sourced Maxwell route"
TARGET_RULE = TARGET
PROOF_EXECUTION_STATUS = "not yet"
RULE_PROMOTION_STATUS = "not authorized"

ROUTE_GIVEN = THEOREM_SHAPE_GIVEN
ROUTE_THEN = TARGET
PLANNED_PROOF_STEPS = [
    "start from nabla_mu T_A^{mu nu} = - F^nu{}_alpha nabla_mu F^{mu alpha}",
    "substitute nabla_mu F^{mu alpha} = J^alpha from the accepted sourced Maxwell route",
    "preserve the same F and J objects",
    "verify sign and index placement",
    "obtain - F^nu{}_alpha J^alpha",
]

ACCEPTED_PACKET_FINDINGS = [
    "attempt type: sourced-Maxwell gauge-sector exchange theorem-linkage attempt",
    "input route: gauge stress-energy divergence identity plus sourced Maxwell route",
    "target rule: nabla_mu T_A^{mu nu} = - F^nu{}_alpha J^alpha",
...
def build_psi_A_gauge_sector_exchange_theorem_linkage_attempt_from_sourced_maxwell_route(
    *,
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PSI_A_GAUGE_SECTOR_EXCHANGE_THEOREM_LINKAGE_ATTEMPT_FROM_SOURCED_MAXWELL_ROUTE"
    )
    payload: dict[str, Any] = {
        ...
        "packet_id": PACKET_ID,
        "status": (
            "ACTIVE_PSI_A_GAUGE_SECTOR_EXCHANGE_THEOREM_LINKAGE_ATTEMPT_FROM_SOURCED_MAXWELL_ROUTE_PREPARATION"
        ),
        ...
        if accepted
        else "PSI_A_GAUGE_SECTOR_EXCHANGE_THEOREM_LINKAGE_ATTEMPT_FROM_SOURCED_MAXWELL_ROUTE_REQUIRES_REMEDIATION",
        ...
        "non_claim_boundary": (
            "This packet prepares only the psi-A gauge-sector exchange theorem-linkage "
            "attempt from the accepted gauge stress-energy divergence identity and "
            "sourced Maxwell route. It indexes the planned substitution route toward "
            "nabla_mu T_A^{mu nu} = - F^nu{}_alpha J^alpha for later execution but "
        ...
        "critical_gate_fail_conditions": [
            "fail to consume prepare_psi_A_gauge_sector_exchange_theorem_linkage_attempt_from_sourced_maxwell_route",
            "fail to index the sourced-Maxwell gauge-sector exchange route",
            "fail to record the gauge stress-energy divergence identity",
            "fail to record the sourced Maxwell route",
            "fail to preserve the same F and J objects",
            "fail to preserve sign and index placement",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PsiAGaugeSectorExchangeTheoremLinkageAttemptFromSourcedMaxwellRoute",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare the psi-A gauge-sector exchange theorem-linkage attempt from the sourced Maxwell route."
        )
    )
    ...
    payload = build_psi_A_gauge_sector_exchange_theorem_linkage_attempt_from_sourced_maxwell_route(
