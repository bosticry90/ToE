...
SCHEMA_ID = "QFT_GR_BINDING_REFINEMENT_REPORT_20260412_v0"

DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "QFT_GR_BINDING_REFINEMENT_20260412_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    required_lane_selection_outcome = str(refinement_policy.get("required_lane_selection_outcome", "")).strip()
    required_preservation_outcome = str(refinement_policy.get("required_preservation_outcome", "")).strip()
    ...
    binding_lock = dict(refinement_policy.get("binding_lock", {}))
    ...
    lock_declared = (
        str(binding_lock.get("seam_id", "")).strip() == "QFT-GR"
        and str(binding_lock.get("comparator_id", "")).strip() == "EXT-COMP-QFT-GR-0001"
        and str(binding_lock.get("quantity_id", "")).strip() == "qft_gr_bridge_transport_interface_residual"
    )
    ...
    default_outcome = str(
        refinement_contract.get("default_outcome", "QFT_GR_BINDING_STILL_PARTIAL_HOLD")
    ).strip()

    if not preconditions_ok:
        terminal_outcome = "QFT_GR_REFINEMENT_REQUIRES_UNDECLARED_STRUCTURE"
        next_action = "RESTORE_QFT_GR_REFINEMENT_PRECONDITIONS"
    elif requires_undeclared_structure:
        terminal_outcome = "QFT_GR_REFINEMENT_REQUIRES_UNDECLARED_STRUCTURE"
        next_action = "DECLARE_MISSING_QFT_GR_REFINEMENT_STRUCTURE_BEFORE_NEXT_STEP"
    elif (
        ...
        next_action = "OPEN_ONE_BOUNDED_QFT_GR_PROBE_EXECUTION_LAYER"
    elif (
        refinement_executable
        and lock_declared
        and weakness_declared
        ...
        terminal_outcome = "QFT_GR_BINDING_CONFIRMED"
        next_action = "OPEN_QFT_GR_PROBE_READINESS_ADJUDICATION_LAYER"
    elif (
        refinement_executable
        and lock_declared
        and weakness_declared
        ...
        terminal_outcome = "QFT_GR_BINDING_STILL_PARTIAL_HOLD"
    ...
    return {
        ...
        "criteria": {
            ...
            "binding_lock_declared": lock_declared,
            ...
            == "EXACTLY_ONE_ALLOWED_QFT_GR_BINDING_REFINEMENT_OUTCOME",
            "no_loop_rule_declared": str(refinement_contract.get("no_loop_rule", "")).strip()
            == "ONE_QFT_GR_BINDING_REFINEMENT_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "non_claim_boundary": "Repository-local QFT-GR binding refinement report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate QFT-GR binding refinement report.")
    parser.add_argument("--declaration", type=Path, default=DEFAULT_DECLARATION_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "qft_gr_binding_refinement_20260412_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "qft_gr_binding_refinement_report: "
