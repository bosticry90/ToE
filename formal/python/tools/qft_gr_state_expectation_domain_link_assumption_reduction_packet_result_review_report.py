...
RESULT_REVIEW_CLASSIFICATION = (
    "qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review_"
    "accepts_packet_and_authorizes_bounded_reduction_attempt_only"
)
CONSUMED_TARGET = EXPECTED_REVIEW_TARGET
NEXT_TARGET = "execute_qft_gr_state_expectation_domain_link_assumption_reduction_attempt"

AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS = [
    "qft_gr_state_expectation_domain_link_assumption_reduced_pending_result_review",
    (
        "qft_gr_state_expectation_domain_link_assumption_obstruction_"
        "identified_requires_refinement"
    ),
    (
        "qft_gr_state_expectation_domain_link_assumption_inconclusive_"
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The OD-ASSUMP-003 packet is accepted as preparation-only, so "
                "the next bounded step may attempt the state-expectation "
                "domain-link reduction without claiming conservation or source "
                "admissibility."
            ),
        },
        {
            ...
            "reason": (
                ...
                "requires the OD-ASSUMP-003 attempt and result review first."
            ),
        },
        {
            ...
            "reason": (
                "Accepting the state-expectation domain-link packet does not "
...
def build_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    packet = _read_json(packet_path)
    selected_assumption = packet.get("state_expectation_domain_link_assumption", {})
    selected_status = packet.get("state_expectation_domain_link_status_tokens", [])
    prior_rows = packet.get("prior_accepted_operator_domain_assumption_rows", [])
    candidate_next_targets = _candidate_next_targets()

    acceptance_criteria = {
        "consumes_expected_state_expectation_domain_link_packet": packet.get(
            "packet_id"
        )
        == EXPECTED_PACKET_ID,
        ...
        "confirms_selected_row003": packet.get("selected_operator_domain_assumption_row")
        == SELECTED_ROW_ID
        and selected_assumption.get("assumption_id") == SELECTED_ROW_ID,
        "confirms_state_expectation_object": packet.get("state_expectation_object")
        == STATE_EXPECTATION_OBJECT
        ...
        and packet.get(
            "state_expectation_domain_link_assumption_reduction_analysis_prepared"
        )
        is True
        and packet.get("assumptions_reduced_or_discharged_by_preparation") is False,
        "no_state_expectation_domain_link_reduced_by_review_alone": packet.get(
            "state_expectation_domain_link_assumption_discharged"
        )
        is False
        and packet.get("state_expectation_domain_link_claimed_as_operator_domain_closed")
        is False,
    ...
    return {
        ...
        "consumes_qft_gr_state_expectation_domain_link_assumption_reduction_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_state_expectation_domain_link_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "prior_rows001_002_remain_accepted": accepted,
        "selected_operator_domain_assumption_row": SELECTED_ROW_ID,
        "state_expectation_domain_link_assumption": selected_assumption,
        ...
        "state_expectation_domain_link_packet_accepted_by_review": accepted,
        "state_expectation_domain_link_analysis_accepted": accepted,
        "state_expectation_domain_link_assumption_reduced_by_review": False,
        ...
        "authorized_attempt_scope": (
            "state_expectation_domain_link_assumption_reduction_attempt_only_"
        ...
        "selected_next_target_kind": (
            "qft_gr_state_expectation_domain_link_assumption_reduction_attempt_execution"
        ),
        "selected_route": (
        ...
        "next_action_scope": (
            "EXECUTE_QFT_GR_STATE_EXPECTATION_DOMAIN_LINK_ASSUMPTION_REDUCTION_"
            "ATTEMPT_ONLY_NO_SOURCE_ADMISSIBILITY_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the OD-ASSUMP-003 "
            "state-expectation domain-link packet and authorizes one bounded "
...
def write_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    ...
    payload = (
        build_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = (
        write_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review(
            packet_path=packet_path,
            out=out,
    ...
    print(
        "qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review_report: "
