...
AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS = [
    ...
    "qft_gr_distributional_pairing_regular_domain_assumption_obstruction_identified_requires_refinement",
    "qft_gr_distributional_pairing_regular_domain_assumption_inconclusive_requires_assumption_reduction",
]
    ...
    return [
        {
            ...
            "reason": (
                ...
                "the MR-ASSUMP-003 distributional-pairing regular-domain reduction "
        ...
        },
        {
            ...
            "reason": "MR-ASSUMP-004 remains downstream; only MR-ASSUMP-003 is authorized.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "Distributional-pairing regularity is not proved by packet result review.",
        },
        {
            "target": "claim_qft_gr_state_admissibility",
            "decision": "not_authorized",
            "reason": "MR-ASSUMP-003 result review does not imply state admissibility.",
        },
        {
            "target": "claim_qft_gr_source_admissibility",
            "decision": "not_authorized",
            "reason": "MR-ASSUMP-003 result review does not imply source admissibility.",
        },
        {
...
def build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review(
    *,
    ...
    selected_row = packet.get("distributional_pairing_regular_domain_assumption", {})
    ...
    acceptance_criteria = {
        ...
        == SELECTED_ASSUMPTION_FAMILY,
        "selected_row_is_mr_assump_003": packet.get(
        ...
        "selected_row_distributional_fields_preserved": packet.get(
            "distributional_pairing_regular_domain"
        )
        == SELECTED_ROW_OBJECT
        and selected_row.get("regularity_condition") == SELECTED_ROW_OBJECT
        and packet.get("distributional_pairing_domain_boundaries")
        == DISTRIBUTIONAL_PAIRING_DOMAIN_BOUNDARIES,
        ...
        is False,
        "no_distributional_pairing_proof": packet.get(
    ...
    selected_result_review_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_QFT_GR_DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW"
    )
    ...
    return {
        ...
        if accepted
        else "QFT_GR_DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_REJECTS_OR_REQUIRES_REMEDIATION",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION
        if accepted
        else "qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review_rejects_or_requires_remediation",
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        "consumed_target": CONSUMED_TARGET,
        "packet_selected_next_target": CONSUMED_TARGET,
        "result_review_selected_next_target": selected_result_review_next_target,
        "distributional_pairing_regular_domain_assumption_reduction_packet_selected_next_target": CONSUMED_TARGET,
        "distributional_pairing_regular_domain_assumption_reduction_packet_result_review_selected_next_target": selected_result_review_next_target,
        ...
        "distributional_pairing_regular_domain_assumption_reduction_packet_reviewed": accepted,
        "distributional_pairing_regular_domain_assumption_reduction_packet_accepted": accepted,
        "distributional_pairing_regular_domain_assumption_reduction_packet_rejected": not accepted,
        "distributional_pairing_regular_domain_assumption_reduction_packet_preparation_only": True,
        "packet_preparation_only_confirmed_by_review": accepted,
        "mr_assump_003_attempt_executed_by_review": False,
        "distributional_pairing_regular_domain_assumption_reduction_attempt_authorized": accepted,
        "distributional_pairing_regular_domain_assumption_reduction_attempt_executed": False,
        ...
        "distributional_pairing_regular_domain_authorized_attempt_result_classifications": AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS,
        ...
        "non_claim_boundary": (
            "This result review accepts only the MR-ASSUMP-003 distributional-pairing "
            "regular-domain packet and authorizes the bounded MR-ASSUMP-003 attempt. "
            "It does not execute that attempt, does not prove distributional-pairing "
            "regularity, does not reduce or discharge mathematical-regularity assumptions "
            "by implication, does not claim state/source admissibility or Bianchi "
            "compatibility, does not construct a conservation proof object or witness, "
            "does not derive the semiclassical Einstein equation, does not close QFT-GR, "
            "does not promote the master action, and does not authorize release assembly "
            "or public submission."
        ),
    }


def write_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR distributional-pairing regular-domain packet "
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet_result_review_report: "
