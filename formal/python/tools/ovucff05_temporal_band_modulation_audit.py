"""CLI for OV-UCFF-05 temporal band modulation audit scaffold.

Usage
  .\py.ps1 -m formal.python.tools.ovucff05_temporal_band_modulation_audit --report --demo modulated
  .\py.ps1 -m formal.python.tools.ovucff05_temporal_band_modulation_audit --report --pinned
  .\py.ps1 -m formal.python.tools.ovucff05_temporal_band_modulation_audit --write
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Use the canonical pinned input artifact for OV-UCFF-05. "
            "Ignored if --input-json is provided."
        ),
    )
    p.add_argument("--demo", default="modulated", choices=["constant", "modulated", "noise"], help="Demo input selection")
    p.add_argument("--n-bands", type=int, default=8, help="Number of rFFT bands")
    p.add_argument("--dt", type=float, default=None, help="Frame spacing (time step) for temporal harmonics")
    ...
    if args.write:
        out = write_ovucff05_temporal_band_modulation_artifact()
