...
from formal.python.tools.v01_alpha_retained_tranche_004_source_map_closure_adjudication_report import (
    ...
    CLOSURE_ADJUDICATION_RESULT_CLASSIFICATION as EXPECTED_EXECUTION_CLASSIFICATION,
    DEFAULT_OUT as DEFAULT_CLOSURE_ADJUDICATION_PATH,
    EXECUTION_ID as EXPECTED_CLOSURE_ADJUDICATION_ID,
    OUTCOME_ID as EXPECTED_CLOSURE_ADJUDICATION_OUTCOME,
    REFINED_AUTHORIZATION_ADJUDICATION_TARGET,
    SCHEMA_ID as EXPECTED_CLOSURE_ADJUDICATION_SCHEMA_ID,
)
...
OUTCOME_ID = (
    "V01_ALPHA_RETAINED_TRANCHE_004_SOURCE_MAP_CLOSURE_ADJUDICATION_"
    "RESULT_REVIEW_ACCEPTS_SOURCE_MAP_CLOSURE_AUTHORIZATION_AND_AUTHORIZES_"
    "CLOSURE_REGISTRATION_PREPARATION_ONLY"
)
RESULT_REVIEW_CLASSIFICATION = (
    "source_map_closure_authorization_accepted_closure_registration_packet_"
    "preparation_only"
)
CONSUMED_TARGET = (
...
FORBIDDEN_EFFECTS = [
    ...
    "closure_registration_packet_prepared",
...
def _reviewed_closure_requirements(
    execution: dict[str, Any],
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in execution.get("adjudicated_closure_requirements", []):
        rows.append(
            {
                ...
                "execution_adjudication_status": row.get(
                    "closure_adjudication_status"
                ),
                "result_review_status": (
                    "accepted_for_source_map_closure_registration_packet_"
                    "preparation_only"
                ),
                "closure_registration_status": (
                    "not_registered_by_result_review_alone"
                ),
                "closure_registration_packet_preparation_authorized": True,
            }
        )
    return rows
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The closure adjudication result review accepts the authorization "
                "only for preparing a separate closure-registration packet."
            ),
        },
        {
            ...
            "decision": "deferred",
            "reason": (
                "Blocker movement remains unavailable until a later governed "
                "registration and movement-control step."
            ),
        },
        {
            ...
            "decision": "deferred",
            "reason": (
                "Refinement remains available if closure-registration packet "
                "preparation exposes a gap."
            ),
        },
        {
...
def build_source_map_closure_adjudication_result_review(
    *,
    closure_adjudication_path: Path = DEFAULT_CLOSURE_ADJUDICATION_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    execution = _read_json(closure_adjudication_path)
    reviewed_closure_requirements = _reviewed_closure_requirements(execution)
    adjudicated_closure_requirements = list(
        execution.get("adjudicated_closure_requirements", [])
    )
    ...
    success_criteria = list(execution.get("closure_adjudication_success_criteria", []))
    failure_criteria = list(execution.get("closure_adjudication_failure_criteria", []))
    execution_boundary = list(execution.get("closure_adjudication_execution_boundary", []))
    execution_steps = list(execution.get("closure_adjudication_execution_steps", []))
    candidate_next_targets = _candidate_next_targets()
    forbidden_effect_status = {effect: False for effect in FORBIDDEN_EFFECTS}

    acceptance_criteria = {
        "consumes_expected_closure_adjudication_execution": execution.get(
            "execution_id"
        )
        == EXPECTED_CLOSURE_ADJUDICATION_ID,
        "closure_adjudication_schema_expected": execution.get("schema_id")
        == EXPECTED_CLOSURE_ADJUDICATION_SCHEMA_ID,
        "closure_adjudication_outcome_expected": execution.get("outcome_id")
        == EXPECTED_CLOSURE_ADJUDICATION_OUTCOME,
        "closure_adjudication_selected_this_review": execution.get(
            "selected_next_target"
        )
        == CONSUMED_TARGET,
        "classification_is_authorized_pending_review": execution.get(
            "source_map_closure_adjudication_result_classification"
        )
        == EXPECTED_EXECUTION_CLASSIFICATION
        and execution.get("closure_adjudication_result_classification_count") == 1,
        "closure_authorization_acceptance_is_explicit": execution.get("accepted")
        is True
        and execution.get("executed") is True
        and execution.get("source_map_closure_authorized_pending_result_review")
        is True
        and execution.get("closure_adjudication_answer")
        == CLOSURE_ADJUDICATION_ANSWER
        and execution.get("source_map_closure_authorization_result_review_required")
        is True,
        "all_closure_requirements_reviewed": len(reviewed_closure_requirements) == 7
        and len(adjudicated_closure_requirements) == 7
        and execution.get("adjudicated_closure_requirement_count") == 7
        and all(
            row.get("execution_adjudication_status")
            == "satisfies_source_map_closure_requirement_pending_result_review"
            for row in reviewed_closure_requirements
        )
        and all(
        ...
        "accepted_authorization_and_witness_material_carried": len(
        ...
        "no_final_closure_seam_or_blocker_movement_in_input": execution.get(
            "source_map_closure_claimed"
        )
        is False
        and execution.get("source_map_closure_registered") is False
        and execution.get("source_map_closure_result_claimed_as_final_closure")
        is False
        ...
        "closure_registration_packet_preparation_selected_only": sum(
    ...
    return {
        ...
        "consumes_source_map_closure_adjudication_execution": (
            EXPECTED_CLOSURE_ADJUDICATION_ID
        ),
        "consumes_source_map_closure_adjudication_execution_pointer": _ptr(
            closure_adjudication_path
        ),
        "consumed_source_map_closure_adjudication_schema_id": execution.get(
            "schema_id"
        ),
        "consumed_source_map_closure_adjudication_outcome_id": execution.get(
            "outcome_id"
        ),
        "consumed_source_map_closure_adjudication_result_classification": (
            execution.get("source_map_closure_adjudication_result_classification")
        ),
        "review_scope": (
        ...
        "source_map_closure_adjudication_result_reviewed": accepted,
        "source_map_closure_adjudication_result_accepted_by_review": accepted,
        "source_map_closure_authorization_decision_accepted_by_review": accepted,
        "source_map_closure_authorization_accepted_by_review": accepted,
        "source_map_closure_authorization_accepted_for_registration_packet_preparation_only": accepted,
        ...
        "closure_adjudication_question": execution.get("closure_adjudication_question"),
        "closure_adjudication_answer": execution.get("closure_adjudication_answer"),
        "closure_adjudication_answer_accepted_by_review": accepted,
        "source_map_closure_authorized_pending_result_review": (
            execution.get("source_map_closure_authorized_pending_result_review")
            is True
        ),
        "source_map_closure_authorization_result_review_required": False,
        "source_map_closure_registration_packet_preparation_authorized": accepted,
        "source_map_closure_registration_packet_preparation_only": accepted,
        "source_map_closure_registration_packet_prepared": False,
        ...
        "accepted_closure_requirement_count": len(reviewed_closure_requirements)
        if accepted
        else 0,
        "adjudicated_closure_requirements": adjudicated_closure_requirements,
        "adjudicated_closure_requirement_count": len(adjudicated_closure_requirements),
        "closure_adjudication_requirements": list(
            execution.get("closure_adjudication_requirements", [])
        ),
        "closure_adjudication_requirement_count": execution.get(
            "closure_adjudication_requirement_count"
        ),
        ...
        "source_map_authorization_adjudication_result_accepted": execution.get(
        ...
        "non_claim_boundary": (
            "The retained tranche 004 source-map closure adjudication result "
            "review accepts the closure authorization only for preparing a "
            "separate source-map closure-registration packet. It does not "
            "register or claim final source-map closure, close the QFT-GR seam, "
...
def write_source_map_closure_adjudication_result_review(
    *,
    closure_adjudication_path: Path = DEFAULT_CLOSURE_ADJUDICATION_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_source_map_closure_adjudication_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_CLOSURE_ADJUDICATION_PATH,
    )
...
def main(argv: list[str] | None = None) -> int:
    ns = _parse_args(argv)
    closure_adjudication_path = (
        ns.closure_adjudication
        if ns.closure_adjudication.is_absolute()
        else (REPO_ROOT / ns.closure_adjudication)
    )
    out = ns.out if ns.out.is_absolute() else (REPO_ROOT / ns.out)
    payload = write_source_map_closure_adjudication_result_review(
        closure_adjudication_path=closure_adjudication_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "v01_alpha_retained_tranche_004_source_map_closure_adjudication_result_review_report: "
