...
SCHEMA_ID = (
    "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_PACKET_RESULT_REVIEW_20260525_v0"
)
REVIEW_ID = "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_PACKET_RESULT_REVIEW_v0"
OUTCOME_ID = "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_PACKET_RESULT_REVIEW_ACCEPTS_PACKET_AND_AUTHORIZES_BOUNDED_WITNESS_ATTEMPT_ONLY"
RESULT_REVIEW_CLASSIFICATION = (
    "qft_gr_covariant_conservation_statement_witness_packet_result_review_accepts_"
    "packet_and_authorizes_bounded_witness_attempt_only"
)
CONSUMED_TARGET = POST_PACKET_REVIEW_TARGET
NEXT_TARGET = "execute_qft_gr_covariant_conservation_statement_witness_attempt"
EXECUTION_CLASSIFICATIONS = FUTURE_EXECUTION_CLASSIFICATIONS
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_PACKET_RESULT_REVIEW_20260525_v0.json"
)
...
def build_qft_gr_covariant_conservation_statement_witness_packet_result_review(
    *,
    ...
    candidate_next_targets = [
        ...
        {
            "target": "review_qft_gr_covariant_conservation_statement_witness_attempt_result",
            "decision": "deferred",
    ...
    acceptance_criteria = {
        ...
        == CONSUMED_TARGET,
        "primary_blocker_preserved": packet.get("primary_blocker")
        == PRIMARY_MISSING_CONDITION
        ...
        "post_packet_target_expected": packet.get("packet_fields", {}).get(
            "post_packet_review_target"
        )
        == POST_PACKET_REVIEW_TARGET,
    ...
    return {
        ...
        if accepted
        else "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_PACKET_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION,
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_covariant_conservation_statement_witness_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_covariant_conservation_statement_witness_packet_pointer": _ptr(
            packet_path
        ),
        ...
        if accepted
        else "REMEDIATE_QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_PACKET_RESULT_REVIEW",
        ...
        "next_action_scope": (
            "EXECUTE_QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITNESS_ATTEMPT_ONLY_"
            "NO_QFT_GR_SEAM_CLOSURE_OR_EMPIRICAL_VALIDATION"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts the covariant-conservation statement "
            "witness packet and authorizes only a bounded witness attempt. It "
            "does not construct the witness, claim stress-energy source "
...
def write_qft_gr_covariant_conservation_statement_witness_packet_result_review(
    *,
    packet_path: Path = PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_covariant_conservation_statement_witness_packet_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR covariant conservation statement witness packet result review."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_covariant_conservation_statement_witness_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_covariant_conservation_statement_witness_packet_result_review_report: "
