"""Open gravitational-requirement inventory Stage 1 without producing science."""
...
VALIDATION_PATH = (
    RELEASE_ROOT
    / "TOE_NATIVE_GRAVITATIONAL_REQUIREMENT_INVENTORY_OPEN_VALIDATION_v0.json"
)
PROGRAM_ID = (
...
TARGET = "inventory_toe_native_gravitational_requirements_v0"
KIND = "toe_native_gravitational_requirement_inventory_stage_1_open_v0"
EVIDENCE = (
    "formal/toe_formal/ToeFormal/Derivation/"
    "ToeNativeGravitationalRequirementInventoryAttemptOpen.lean"
)
REPORT = (
    "formal/docs/release/bounded_program_events/"
    "TOE_NATIVE_GRAVITATIONAL_REQUIREMENTS_AND_CANDIDATE_ACTION_FAMILY_"
    "SURVEY_V0_ATTEMPT_01_OPEN_v0.json"
)
OUTCOME = "NATIVE_GRAVITATIONAL_REQUIREMENT_INVENTORY_STAGE_1_OPEN"
STRICT_OUTCOME = (
    "STAGE_1_OPEN_NO_REQUIREMENT_ADJUDICATION_ACTION_FAMILY_COMPARISON_"
    "EVIDENCE_PROMOTION_GRAVITY_PRINCIPLE_ACTION_CALCULATION_OR_STAGE_2"
)
...
def _check_authority() -> None:
    ...
    if len(authority["requirement_ids"]) != 10:
        raise ValueError("Stage 1 authority must bind ten requirement rows")
...
def _project_current_target(registry: dict, report_sha256: str) -> None:
    ...
    if projection["current_target"] != EXPECTED_PREVIOUS_TARGET:
        raise ValueError("survey preparation target is not current")
    projection.update(
        {
            "active_lane": TARGET,
    ...
    if len(active) != 1 or active[0]["workstream_id"] != EXPECTED_PREVIOUS_TARGET:
        raise ValueError("active workstream is not the survey preparation target")
    workstream = active[0]
    workstream.update(
        {
            "workstream_id": TARGET,
            ...
            "queue_scope": (
                "Native gravitational requirement inventory Stage 1 is OPEN; "
                "the OPEN checkpoint contains no scientific inventory result"
            ),
            "claim_status": (
                "Stage 1 OPEN only; no requirement adjudication, action-family "
                "comparison, promotion, gravitational principle, action, "
                "calculation, or Stage 2"
            ),
        }
    )
...
def open_stage(*, opened_from_commit: str) -> str:
    ...
    try:
        ...
        stage = _stage()
        validation = {
            "artifact_id": "TOE_NATIVE_GRAVITATIONAL_REQUIREMENT_INVENTORY_OPEN_VALIDATION_v0",
            "attempt_sequence_number": 1,
            "atomic_open_commit_expected_paths": stage["prospective_envelope"]["open_commit_exact_path_set"],
            "authority_decision": "AUTHORIZE_NATIVE_GRAVITATIONAL_REQUIREMENT_INVENTORY_STAGE_1_OPEN",
            "captured_at_utc": "2026-07-31T14:30:00Z",
            ...
            "schema_id": "toe.native_gravitational_requirement_inventory.stage_1_open_validation.v0",
            ...
            "scientific_output_at_open": {
                ...
                "requirement_rows_adjudicated": 0,
