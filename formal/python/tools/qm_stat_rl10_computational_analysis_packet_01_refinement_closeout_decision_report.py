...
SCHEMA_ID = "QM_STAT_RL10_COMPUTATIONAL_ANALYSIS_PACKET_01_REFINEMENT_CLOSEOUT_DECISION_REPORT_20260416_v0"

DEFAULT_BASELINE_DECISION_PATH = REPO_ROOT / "formal" / "output" / "reports" / "qm_stat_rl10_computational_analysis_packet_01_decision_20260416_v0.json"
DEFAULT_REFINEMENT_REPORT_PATH = REPO_ROOT / "formal" / "output" / "reports" / "qm_stat_rl10_computational_analysis_packet_01_refinement_01_20260416_v0.json"
...
def build_report(*, baseline_decision_path: Path, refinement_report_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    baseline_authorizes_refinement = (
        ...
        and str(baseline_summary.get("authorized_follow_on", "")).strip() == "ONE_BOUNDED_PACKET01_REFINEMENT_ONLY"
    )
    ...
    refinement_boundary_preserved = (
        ...
        and not bool(refinement_criteria.get("restart_implication", False))
    ...
    if not baseline_authorizes_refinement:
        ...
        str(refinement_summary.get("stability_classification", "")).strip() == "STABLE_v0"
        and str(refinement_summary.get("comparator_classification", "")).strip() == "COMPARATOR_SENSITIVE_v0"
        and str(refinement_summary.get("discriminator_classification", "")).strip() == "DISCRIMINATIVE_v0"
        and str(refinement_summary.get("subordinate_disposition", "")).strip() == "RETAIN_v0"
    ):
        decision = "RETAIN_REFINEMENT_v0"
        basis = "REFINEMENT_PRESERVED_SIGNAL_UNDER_TIGHTER_COMPARATOR_MARGIN"
        next_action = "STOP_PACKET01_FAMILY_AND_PRESERVE_REFINED_BASELINE"
    elif str(refinement_summary.get("stability_classification", "")).strip() == "STABLE_v0":
        decision = "RETAIN_BASELINE_v0"
        basis = "REFINEMENT_DID_NOT_OUTPERFORM_BASELINE_BUT_BASELINE_REMAINS_VALID"
        next_action = "STOP_PACKET01_FAMILY_AND_PRESERVE_BASELINE"
    elif str(refinement_summary.get("subordinate_disposition", "")).strip() == "PRUNE_v0":
        decision = "RETIRE_REFINEMENT_v0"
        basis = "REFINEMENT_SIGNAL_DEGRADED_OR_BECAME_ARTIFACT_DEPENDENT"
        next_action = "STOP_PACKET01_FAMILY_AND_DROP_REFINEMENT"
    else:
        decision = "STOP_PACKET01_FAMILY_v0"
    ...
    return {
        "schema_id": SCHEMA_ID,
        "report_id": "QM_STAT_RL10_COMPUTATIONAL_ANALYSIS_PACKET_01_REFINEMENT_CLOSEOUT_DECISION_REPORT_v0",
        ...
        "non_claim_boundary": "Repository-local Packet-01 refinement closeout decision report only; no Packet-02 authorization, restart implication, blocker movement, lane reopen, or external-truth claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the QM-STAT RL10 computational-analysis Packet-01 refinement closeout decision report.")
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "qm_stat_rl10_computational_analysis_packet_01_refinement_closeout_20260416_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "qm_stat_rl10_computational_analysis_packet_01_refinement_closeout_decision_report: "
