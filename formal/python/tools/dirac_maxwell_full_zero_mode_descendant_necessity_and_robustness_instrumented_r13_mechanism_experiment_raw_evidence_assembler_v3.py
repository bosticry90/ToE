...
v3 matrix, identity manifest, freeze packet, review anchor, and completed output
...
ASSEMBLER_ID = (
    ...
    "INSTRUMENTED_R13_MECHANISM_EXPERIMENT_RAW_EVIDENCE_ASSEMBLER_v3"
)
SCRIPT_RELATIVE_PATH = (
    "formal/python/tools/dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_raw_evidence_assembler_v3.py"
)
...
CLASSIFIER_V3_MODULE = (
    "formal.python.tools.dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_classifier_v3"
)
ASSEMBLER_V3_MODULE = (
    "formal.python.tools.dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_raw_evidence_assembler_v3"
)
...
def _validate_frozen_documents(
    ...
    for ordinal, (record, run_id) in enumerate(zip(records, EXPECTED_RUN_IDS, strict=True)):
        ...
        # V3 also binds the role-resolved executable metric configuration.
        try:
            closure_digest = authority["runtime_source_closure_sha256"]
            ...
            resolved = executor_v2.resolve_frozen_metric_configuration(
                record,
                record["partial_metric_configuration"],
                record["role_resolution_overlay"],
            )
            resolved_hash = _sha256(
                executor_v2.canonical_json_bytes(resolved)
            )
        ...
        if (
            ...
            or not complete_hash
            or not resolved_hash
            or not full_hash
            or record.get("resolved_metric_configuration") != resolved
            or record.get("resolved_metric_configuration_sha256")
            != resolved_hash
        ):
...
def _validate_auxiliary_result(
    ...
    expected_runtime_anchor = {
        ...
        "resolved_metric_configuration_sha256_by_run_id": authority[
            "expected_resolved_metric_configuration_sha256_by_run_id"
        ],
    }
...
def assemble_raw_evidence(repo_root: str | Path) -> AssembledRawEvidence:
    """Authoritative path-closed assembler for the frozen v3 contract.
