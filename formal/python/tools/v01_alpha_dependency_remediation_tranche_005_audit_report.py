...
SCHEMA_ID = "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_AUDIT_20260515_v0"
AUDIT_ID = "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_AUDIT_v0"
OUTCOME_ID = (
...
EXPECTED_RESULT_REVIEW_OUTCOME = (
    "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_EXECUTION_PACKET_RESULT_REVIEW_"
    "ACCEPTS_LEAN_DEPENDENCY_AUDIT_SCOPE_AND_AUTHORIZES_TRANCHE_005_AUDIT_EXECUTION_ONLY"
)
...
LEAN_IMPORT_MODULE = "ToeFormal.Bridges.EM_QFT_InterfaceAlignmentSemanticBridge"
LEAN_AUDIT_COMMAND = (
...
LEAN_AXIOM_PRINT_OUTPUT = (
    "'ToeFormal.Bridges.EMQFTInterfaceAlignmentSemanticBridge."
    "supplied_interface_alignment_semantics_construct_bridge_package_v0' depends on axioms: "
    "[propext,\n Classical.choice,\n Quot.sound]"
)
LEAN_AXIOMS_USED = ["propext", "Classical.choice", "Quot.sound"]
PROJECT_AXIOMS_USED: list[str] = []
NEXT_TARGET = "review_v01_alpha_dependency_remediation_tranche_005_audit_result"
...
def _other_obligations(rows: list[dict[str, Any]], tranche_006: dict[str, Any]) -> list[dict[str, Any]]:
    other = []
    for row in rows:
        ...
        carry_row = {
            ...
            "modified_by_tranche_005": False,
        }
        if row.get("dependency_finding_id") == TRANCHE_004_FINDING_ID:
            carry_row["status_carry_forward"] = TRANCHE_004_STATUS
        elif row.get("dependency_finding_id") == "V01-ALPHA-DEP-REM-006":
            carry_row["status_carry_forward"] = tranche_006.get(
                "status_carry_forward",
                "tracked_unmodified_not_audited_in_tranche_004",
            )
        else:
            carry_row["status_carry_forward"] = row.get("status_carry_forward")
        other.append(carry_row)
    return other
...
def build_audit(
    *,
    ...
    other_obligations = _other_obligations(release_blockers, tranche_006)
    ...
    acceptance_criteria = {
        ...
        == "tracked_unresolved"
        and tranche_006.get("dependency_finding_id") == "V01-ALPHA-DEP-REM-006"
        and tranche_006.get("status_carry_forward")
        == "tracked_unmodified_not_audited_in_tranche_004",
        "audit_is_only_for_tranche_005": result_review.get("selected_tranche_id")
        == SELECTED_TRANCHE_ID
        ...
        "lean_audit_was_authorized": result_review.get(
            "tranche_005_audit_execution_authorized"
        )
        is True
        ...
        "depends_only_on_standard_lean_axioms": PROJECT_AXIOMS_USED == []
        and LEAN_AXIOMS_USED == ["propext", "Classical.choice", "Quot.sound"],
        "evidence_surface_exists": True,
        "release_blocking_obligations_carried_forward": len(release_blockers) == 3
        and len(other_obligations) == 2
        and all(row["modified_by_tranche_005"] is False for row in other_obligations),
    ...
    return {
        ...
        if accepted
        else "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_AUDIT_BLOCKED",
        "consumes_tranche_005_execution_packet_result_review": EXPECTED_RESULT_REVIEW_ID,
        "consumes_tranche_005_execution_packet_result_review_pointer": _ptr(
            result_review_path
        ),
        "consumed_result_review_schema_id": result_review.get("schema_id"),
        "audit_scope": (
            "EXECUTE_TRANCHE_005_LEAN_DEPENDENCY_AUDIT_ONLY_NO_REMEDIATION_OR_RELEASE_PROMOTION"
        ),
        ...
        "selected_obligation_status_after_audit": (
            "release_blocking_pending_tranche_005_audit_result_review"
        ),
        ...
        "lean_evidence": {
            ...
            "standard_lean_or_mathlib_axioms_used": LEAN_AXIOMS_USED,
        ...
        "evidence_surfaces_produced_or_updated": [
            {
                "surface": "formal/docs/release/V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_AUDIT_20260515_v0.json",
                "kind": "tranche_005_audit_result_packet",
                "status": "produced",
            },
            {
        ...
        if accepted
        else "REMEDIATE_V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_005_AUDIT",
        "selected_next_target_kind": "tranche_005_audit_result_review_only",
        "selection_count": 1 if accepted else 0,
        "next_action_scope": (
            "REVIEW_TRANCHE_005_LEAN_AUDIT_EVIDENCE_ONLY_NO_REMEDIATION_CLOSURE_OR_RELEASE_PROMOTION"
        ),
        "candidate_next_targets": [
            {
                ...
                "decision": "selected",
                "reason": (
                    "The tranche 005 audit produced exact Lean dependency evidence that must "
                    "be result-reviewed before policy, documentation, or blocker movement."
                ),
            },
            {
                "target": (
                    "prepare_v01_alpha_dependency_remediation_tranche_005_release_policy_"
                    "adjudication_packet"
                ),
                "decision": "deferred",
            ...
            },
            {
                ...
                "decision": "deferred",
                "reason": "Release readiness remains blocked by retained tranche 004 and tracked obligations.",
            },
        ],
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "The v0.1-alpha dependency remediation tranche 005 audit captures exact Lean "
            "dependency evidence for supplied_interface_alignment_semantics_construct_bridge_package_v0 "
            "only. It separates standard Lean/mathlib axioms from project-local axioms and "
            "records that no project-local axioms were used. It carries tranche 004 as retained/"
            "release-blocking and keeps tranche 006 tracked. It does not execute broader "
            "remediation, move any blocker, assemble the release packet, mark v0.1-alpha "
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the v0.1-alpha dependency remediation tranche 005 audit."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "v01_alpha_dependency_remediation_tranche_005_audit_report: "
