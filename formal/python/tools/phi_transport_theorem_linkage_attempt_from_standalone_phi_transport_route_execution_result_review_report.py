...
SCHEMA_ID = (
    ...
    "EXECUTION_RESULT_REVIEW_20260701_v0"
)
...
NEXT_TARGET = "prepare_phi_transport_theorem_linkage_obligation_closeout"
NEXT_TARGET_KIND = "phi_transport_theorem_linkage_obligation_closeout_preparation"
CLOSEOUT_OUTCOME = (
...
ACCEPTED_REVIEW_FINDINGS = [
    "phi-transport theorem-linkage execution accepted",
    ...
    return [
        {
            ...
            "assessment": "The bounded phi-transport execution result is consumed by review.",
        },
        {
            "row_id": "five_component_tuple_preserved",
            "status": "accepted",
            "evidence": TRANSPORT_CONSTRAINT_FORM,
            "assessment": "The five-component C_transport^phi tuple definition is unchanged.",
        },
        {
            "row_id": "transport_zero_components_preserved",
            "status": "accepted",
            "evidence": [
                TRANSPORT_ACTION_VARIATION_ZERO_COMPONENT,
                TRANSPORT_VARIATION_BRIDGE_ZERO_COMPONENT,
            ...
            "assessment": "All action-to-regime transport zero components are preserved.",
        },
        {
            "row_id": "componentwise_zero_route_constructed",
            "status": "accepted",
            "evidence": COMPONENTWISE_ZERO_ROUTE,
            "assessment": "The componentwise zero route constructs C_transport^phi = 0.",
        },
        {
            "row_id": "execution_artifacts_preserved",
            "status": "accepted",
            "evidence": [_ptr(EXECUTION_LEAN_PACKET_PATH), _ptr(EXECUTION_PATH)],
            "assessment": "The Lean execution marker and JSON execution report remain preserved.",
        },
        {
            "row_id": "focused_execution_gates_passed",
            "status": "accepted",
            "evidence": "test_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_gate.py",
            "assessment": "Focused execution gates were preserved for this route.",
        },
        {
...
def build_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        "tuple_definition_preserved": (
            execution.get("transport_constraint_form") == TRANSPORT_CONSTRAINT_FORM
            and execution.get("transport_constraint_equation")
            == TRANSPORT_CONSTRAINT_EQUATION
            and execution.get("transport_admissibility_constraint_form")
            == TRANSPORT_ADMISSIBILITY_CONSTRAINT_FORM
            and execution.get("transport_component_count") == len(TRANSPORT_COMPONENTS)
        ),
        ...
        "zero_components_preserved": (
            execution.get("transport_action_variation_zero_component")
            == TRANSPORT_ACTION_VARIATION_ZERO_COMPONENT
            and execution.get("transport_variation_bridge_zero_component")
            == TRANSPORT_VARIATION_BRIDGE_ZERO_COMPONENT
            and execution.get("transport_bridge_source_zero_component")
            == TRANSPORT_BRIDGE_SOURCE_ZERO_COMPONENT
            and execution.get("transport_source_residual_zero_component")
            == TRANSPORT_SOURCE_RESIDUAL_ZERO_COMPONENT
            and execution.get("transport_residual_regime_zero_component")
            == TRANSPORT_RESIDUAL_REGIME_ZERO_COMPONENT
        ),
        "C_transport_phi_zero_locally_constructed": (
            execution.get("C_transport_tuple_zero") == C_TRANSPORT_TUPLE_ZERO
            and execution.get("target_conclusion") == TARGET_CONCLUSION
            and execution.get("C_transport_phi_zero_constructed") is True
            and execution.get("C_transport_phi_zero_derived") is True
            and execution.get("C_transport_phi_discharged") is True
    ...
    selected_next_target = (
        ...
        if accepted
            ...
            "REMEDIATE_PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_"
            "TRANSPORT_ROUTE_EXECUTION_RESULT_REVIEW"
        )
    )
    payload: dict[str, Any] = {
        ...
        "packet_id": PACKET_ID,
        "status": (
            "ACTIVE_PHI_TRANSPORT_THEOREM_LINKAGE_ATTEMPT_FROM_STANDALONE_PHI_"
            "TRANSPORT_ROUTE_EXECUTION_RESULT_REVIEW"
        ),
        ...
        "route_kind": "standalone_phi_transport_componentwise_zero_execution_review",
        ...
        "same_standalone_phi_transport_registry_tuple": accepted,
        "same_component_order": accepted,
        ...
        "proof_execution_status": "already executed; not re-executed by review",
        ...
        "non_claim_boundary": (
            "This result review accepts only the local standalone phi-transport "
            "execution result: C_transport^phi is reduced componentwise from "
            ...
            "Transport_RESIDUAL_REGIME^phi = 0, yielding "
            "C_transport^phi = (0, 0, 0, 0, 0) and C_transport^phi = 0. It "
            "authorizes only closeout preparation. It claims no phi-sector "
            "closure, no scalar/QFT closure, no QFT-GR closure, no EM-QFT "
            "closure, no seam closure, no general C_k closure, no C_k "
            "promotion, no action embedding, no variation, no empirical "
            "validation, and no master-action promotion."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume review_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_result",
            ...
            "fail to preserve C_transport^phi = (0, 0, 0, 0, 0)",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PhiTransportTheoremLinkageAttemptFromStandalonePhiTransportRouteExecutionResultReview",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Review the executed standalone phi-transport C_transport^phi "
            "componentwise zero theorem-linkage route."
        )
    )
    ...
    payload = (
        build_phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_result_review(
