...
AUTHORIZED_CONSERVATION_RETEST_ATTEMPT_CLASSIFICATIONS = [
    (
        ...
        "retest_refinement_executed_pending_result_review"
    ),
    (
        "qft_gr_minimal_working_model_post_retest_refinement_conservation_"
        "retest_pass_candidate_only_not_source_admissibility"
    ),
    (
        "qft_gr_minimal_working_model_post_retest_refinement_conservation_"
        "retest_fail_requires_countermodel_or_scope_refinement"
    ),
    (
        "qft_gr_minimal_working_model_post_retest_refinement_conservation_"
        "retest_inconclusive_requires_further_refinement_or_countermodel_packet"
    ),
]
DEFAULT_OUT = (
    REPO_ROOT
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The post-retest-refinement conservation-retest packet is "
                "accepted as a bounded protocol, so the next action may "
                "execute only that retest attempt."
            ),
        },
        {
...
def build_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    packet = _read_json(packet_path)
    refinement_delta = packet.get("post_retest_refinement_delta", {})
    ...
    packet_validation_policy = packet.get("validation_policy", {})

    acceptance_criteria = {
        "consumes_expected_post_retest_refinement_retest_packet": (
        ...
        "post_retest_refinement_delta_defined": len(delta_changes) == 7
        ...
        is True
        and packet_validation_policy.get("full_pytest_required") is False
        and packet_validation_policy.get("full_governance_suite_required") is False
        and packet_validation_policy.get("full_aggregate_lean_required") is False,
    ...
    selected_next_target = (
        ...
        if accepted
            ...
            "PACKET_AFTER_POST_RETEST_REFINEMENT_RESULT_REVIEW"
        )
    )
    ...
    return {
        ...
        "post_retest_refinement_conservation_retest_packet_result_review_accepted": (
            accepted
        ),
        ...
        "refined_artifact_status": packet.get("refined_artifact_status"),
        ...
        "validation_posture": {
            ...
            "targeted_lean_result_review_frontier_index": (
                "required_for_checkpoint"
            ),
        ...
        "validation_caveat": (
            ...
            "packet-result-review checkpoint; inherited caveat: "
            f"{packet.get('validation_caveat')}"
        ),
        ...
        "next_action_scope": (
            ...
            "AFTER_POST_RETEST_REFINEMENT_ONLY_NO_SOURCE_ADMISSIBILITY_"
        ...
        "non_claim_boundary": (
            ...
            "conservation-retest packet and authorizes one bounded conservation "
            "retest attempt. It does not execute the retest and preserves no "
            "source admissibility, no conservation claim, no conservation proof "
...
def write_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "conservation-retest packet result review."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_result_review_report: "
