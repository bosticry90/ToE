...
from formal.python.tools.qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review_report import (
    ...
    SELECTED_ROW_ID as ACCEPTED_PRIOR_ROW_ID_002,
)
...
from formal.python.tools.qft_gr_state_domain_object_assumption_reduction_attempt_result_review_report import (
    SELECTED_ROW_ID as ACCEPTED_PRIOR_ROW_ID_001,
)
...
PACKET_ID = "QFT_GR_STATE_EXPECTATION_COMPATIBILITY_ASSUMPTION_REDUCTION_PACKET_v0"
OUTCOME_ID = (
    "QFT_GR_STATE_EXPECTATION_COMPATIBILITY_ASSUMPTION_REDUCTION_PACKET_"
...
ACCEPTED_PRIOR_ROW_IDS = [
    ACCEPTED_PRIOR_ROW_ID_001,
    ACCEPTED_PRIOR_ROW_ID_002,
]
NEXT_TARGET = (
    "review_qft_gr_state_expectation_compatibility_assumption_reduction_"
    "packet_result"
)
NEXT_TARGET_KIND = (
    "qft_gr_state_expectation_compatibility_assumption_reduction_packet_"
...
STATE_EXPECTATION_COMPATIBILITY_CONDITION = STATE_EXPECTATION_COMPATIBILITY
...
CANDIDATE_REDUCTION_ROUTE = (
    ...
    "operator-domain and renormalization-domain reductions without asserting "
    "a conserved source"
)
FAILURE_MODE_IF_UNRESOLVED = (
    "operator-domain and renormalization reductions may not share a common "
    "state-expectation domain for a future conservation proof"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The prepared SD-ASSUMP-003 packet must be result-reviewed "
                "before any state-expectation compatibility reduction attempt "
                "is authorized."
            ),
        },
        {
            "target": (
                "execute_qft_gr_state_expectation_compatibility_assumption_"
                "reduction_attempt"
            ),
            "decision": "deferred",
            "reason": (
                "A bounded SD-ASSUMP-003 attempt requires packet result review "
                "first."
            ),
        },
        {
            ...
            "reason": (
                "State-expectation compatibility packet preparation is not a "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Compatibility between state expectation and accepted domains "
                "is not stress-energy source admissibility."
            ),
        },
        {
...
def build_qft_gr_state_expectation_compatibility_assumption_reduction_packet(
    *,
    ...
    if selected_row:
        selected_row_payload = {
            ...
            "claim_ceiling": (
                "state_expectation_compatibility_packet_only_no_state_"
    ...
    acceptance_criteria = {
        ...
        == CONSUMED_TARGET,
        "accepted_prior_rows001002": attempt_result_review.get(
            "accepted_state_domain_assumption_rows"
        )
        == ACCEPTED_PRIOR_ROW_IDS
        and attempt_result_review.get("accepted_state_domain_assumption_row")
        == ACCEPTED_PRIOR_ROW_ID_002,
        "selects_only_row003": selected_row is not None
        ...
        "state_expectation_compatibility_condition_recorded": (
            selected_row_payload.get("state_expectation_compatibility_condition")
            == STATE_EXPECTATION_COMPATIBILITY_CONDITION
        ),
        ...
        "does_not_claim_state_expectation_compatibility": True,
        "does_not_claim_state_admissibility": attempt_result_review.get(
    ...
    return {
        ...
        if prepared
        else "QFT_GR_STATE_EXPECTATION_COMPATIBILITY_ASSUMPTION_REDUCTION_PACKET_BLOCKED",
        "packet_classification": PACKET_CLASSIFICATION,
        "packet_classification_count": 1 if prepared else 0,
        "consumes_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review": (
            EXPECTED_ATTEMPT_RESULT_REVIEW_ID
        ),
        "consumes_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review_pointer": _ptr(
        ...
        "state_expectation_compatibility_assumption": selected_row_payload,
        ...
        "claim_ceiling": (
            "state_expectation_compatibility_assumption_reduction_packet_"
        ...
        **_nonclaims(),
        ...
        "next_action_scope": (
            "REVIEW_QFT_GR_STATE_EXPECTATION_COMPATIBILITY_ASSUMPTION_REDUCTION_"
            "PACKET_RESULT_ONLY_NO_STATE_ADMISSIBILITY_SOURCE_ADMISSIBILITY_"
            "CONSERVATION_WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This packet prepares only SD-ASSUMP-003 state-expectation "
            "compatibility reduction analysis. It records the compatibility "
            "condition, available repo evidence, required future proof object, "
            "candidate reduction route, claim ceiling, and failure mode. It "
...
def write_qft_gr_state_expectation_compatibility_assumption_reduction_packet(
    *,
    attempt_result_review_path: Path = DEFAULT_ATTEMPT_RESULT_REVIEW_PATH,
    state_domain_packet_path: Path = DEFAULT_STATE_DOMAIN_PACKET_PATH,
    ...
    payload = build_qft_gr_state_expectation_compatibility_assumption_reduction_packet(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR SD-ASSUMP-003 state-expectation "
            "compatibility assumption-reduction packet."
        )
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_state_expectation_compatibility_assumption_reduction_packet(
        attempt_result_review_path=attempt_result_review_path,
        state_domain_packet_path=state_domain_packet_path,
        out=out,
    ...
    print(
        "qft_gr_state_expectation_compatibility_assumption_reduction_packet_report: "
