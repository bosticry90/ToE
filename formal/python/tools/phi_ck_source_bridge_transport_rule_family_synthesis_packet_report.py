...
SCHEMA_ID = "PHI_CK_SOURCE_BRIDGE_TRANSPORT_RULE_FAMILY_SYNTHESIS_PACKET_20260619_v0"
ARTIFACT_ID = SCHEMA_ID
PACKET_ID = "PHI_CK_SOURCE_BRIDGE_TRANSPORT_RULE_FAMILY_SYNTHESIS_PACKET_v0"
PACKET_RESULT = (
...
CONSUMED_TARGET = "prepare_phi_ck_source_bridge_transport_rule_family_synthesis_packet"
NEXT_TARGET = "review_phi_ck_source_bridge_transport_rule_family_synthesis_packet_result"
NEXT_TARGET_KIND = "phi_ck_source_bridge_transport_rule_family_synthesis_packet_result_review"
REVIEW_OUTCOME_HINT = (
...
SOURCE_RULE_DISPLAY_FORM = "C_source^phi = 0"
...
BRIDGE_RULE_PLAIN_MEANING = (
    "phi route is valid only if the master-action route matches the scalar "
    "witness/source route"
)
TRANSPORT_RULE_ID = "phi_transport_consistency_ck_rule"
TRANSPORT_RULE_ROLE = "transport consistency"
TRANSPORT_RULE_DISPLAY_FORM = "C_transport^phi = 0"
TRANSPORT_RULE_PLAIN_MEANING = (
    "phi route is valid only if it remains coherent across the derivation chain"
)
RULE_FAMILY_CLASSIFICATION = "three phi-relevant C_k admissibility-rule candidates"
...
def _rule_family_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "family_display_form": BRIDGE_ADMISSIBILITY_CONSTRAINT_FORM,
    ...
    return [
        {
            ...
            "assessment": "The packet consumes the transport admissibility closeout.",
        },
        {
        ...
        {
            "row_id": "three_concrete_phi_ck_roles_recorded",
            "status": "accepted",
            "evidence": [SOURCE_RULE_ROLE, BRIDGE_RULE_ROLE, TRANSPORT_RULE_ROLE],
            "assessment": "C_k now has three bounded phi-relevant rule roles.",
        },
        {
            "row_id": "admissibility_only_family_classification",
            "status": "accepted",
            "evidence": RULE_EPISTEMIC_STATUS,
            "assessment": "All three rules remain admissibility-only candidates.",
        },
        {
        ...
        },
        {
            ...
            "assessment": "Transport proof and full route-alignment proof remain blocked.",
        },
        {
            "row_id": "no_qft_gr_closure_semiclassical_coupling_or_master_promotion",
            "status": "accepted",
            "evidence": [
...
def _validation_policy() -> dict[str, Any]:
    return {
        ...
        "checkpoint_type": "phi_ck_source_bridge_transport_rule_family_synthesis_packet",
...
def build_phi_ck_source_bridge_transport_rule_family_synthesis_packet(
    *,
    ...
    acceptance_criteria = {
        ...
        "source_rule_form_preserved": (
            ...
            == SOURCE_CANDIDATE_CONSTRAINT_FORM
            and transport_closeout.get("source_candidate_constraint_equation")
            == SOURCE_CANDIDATE_CONSTRAINT_EQUATION
            and transport_closeout.get("source_admissibility_constraint_form")
            == SOURCE_ADMISSIBILITY_CONSTRAINT_FORM
        ),
        "bridge_rule_form_preserved": (
            transport_closeout.get("bridge_constraint_form") == BRIDGE_CONSTRAINT_FORM
            and transport_closeout.get("bridge_constraint_equation")
            == BRIDGE_CONSTRAINT_EQUATION
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PHI_CK_SOURCE_BRIDGE_TRANSPORT_RULE_FAMILY_SYNTHESIS_PACKET"
    )
    return {
        "artifact_id": ARTIFACT_ID,
        "schema_id": SCHEMA_ID,
        "packet_id": PACKET_ID,
        "status": "ACTIVE_PHI_CK_SOURCE_BRIDGE_TRANSPORT_RULE_FAMILY_SYNTHESIS_PACKET",
        "captured_at_utc": captured_at_utc,
        "prepared": accepted,
        "accepted": accepted,
        "outcome_id": OUTCOME_ID
        if accepted
        else "PHI_CK_SOURCE_BRIDGE_TRANSPORT_RULE_FAMILY_SYNTHESIS_PACKET_REQUIRES_REMEDIATION",
        ...
        "three_phi_relevant_ck_admissibility_rule_candidates_synthesized": True,
        ...
        "c_k_acquired_three_concrete_phi_relevant_rule_roles": True,
        "source_rule_decides_phi_source_permission": True,
        "bridge_rule_decides_phi_route_consistency": True,
        "transport_rule_decides_derivation_chain_coherence": True,
        ...
        "mathematical_statement": (
            "The phi/C_k branch now has a three-rule admissibility family: "
            "C_source^phi = 0 for source admissibility, C_bridge^phi = 0 "
            "for bridge admissibility, and C_transport^phi = 0 for "
            "derivation-chain transport consistency. The exact source form "
            "remains C_source^nu[g, phi] := nabla_mu T_phi^{mu nu} with "
            "C_source^nu[g, phi] = 0."
        ),
        "non_claim_boundary": (
            "This synthesis packet records the source, bridge, and transport "
            "phi/C_k admissibility-rule family only. The three rules are "
            "admissibility-only rule candidates, not action terms, not "
            "dynamical laws, not native phi derivations, not V(phi) "
            "derivations, not QFT-GR closure, and not master-action "
            "promotion. It does not execute C_k variation, does not embed "
            "any rule in an action, does not prove transport consistency, "
            "does not prove full route alignment, does not prove source "
            "admissibility, does not prove bridge admissibility, does not "
            "prove new conservation, does not claim semiclassical coupling, "
        ...
        "critical_gate_fail_conditions": [
            ...
            "claim any rule is a new dynamical law",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PhiCKSourceBridgeTransportRuleFamilySynthesisPacket",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build the phi/C_k source-bridge-transport rule-family synthesis packet."
    )
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--captured-at-utc", default=DEFAULT_CAPTURED_AT_UTC)
    args = parser.parse_args()

    packet = build_phi_ck_source_bridge_transport_rule_family_synthesis_packet(
