...
SCHEMA_ID = (
    ...
    "CONSERVATION_CLOSEOUT_20260627_v0"
)
...
FOLLOW_ON_TARGET_KIND = "psi_A_matter_sector_exchange_theorem_linkage_obligation_packet"

SELECTED_OBLIGATION = "psi-A matter-sector exchange theorem-linkage gap"
SELECTED_OBLIGATION_RANK = 3
PREVIOUS_CLOSED_OBLIGATION = "psi-A total conservation theorem-linkage gap"
DEPENDENCY_CHAIN = (
...
DEFAULT_OUT = (
    ...
    / (
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PSI_A_TOTAL_"
        "CONSERVATION_CLOSEOUT_20260627_v0.json"
    )
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    ...
    / "ToeFormal"
    / "Derivation"
    / "CKFamilyTheoremLinkageObligationSelectionAfterPsiATotalConservationCloseout.lean"
)
...
def build_ck_family_theorem_linkage_obligation_selection_after_psi_A_total_conservation_closeout(
    *,
    ...
    acceptance_criteria = {
        ...
        "previous_obligation_locally_closed": (
            review.get("local_psi_A_total_conservation_obligation_closed") is True
        ...
        "selects_third_priority_matter_exchange_obligation": (
            SELECTED_OBLIGATION == REVIEW_LIKELY_NEXT_OBLIGATION
            and SELECTED_OBLIGATION_RANK == 3
        ),
        "selector_records_watch_items_without_execution": (
            WATCH_ITEMS
            == [
                "T_psi definition",
                "Dirac pair",
            ...
            and MATTER_EXCHANGE_TARGET_RULE
            == "nabla_mu T_psi^{mu nu} = + F^nu{}_alpha J^alpha"
        ),
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PSI_A_TOTAL_CONSERVATION_CLOSEOUT"
    )
    payload: dict[str, Any] = {
        ...
        if accepted
        else "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PSI_A_TOTAL_CONSERVATION_CLOSEOUT_REQUIRES_REMEDIATION",
        ...
        "non_claim_boundary": (
            ...
            "obligation after the local psi-A total conservation closeout. It "
            "selects the psi-A matter-sector exchange theorem-linkage gap and "
            "records the watch items, but it does not execute a proof, discharge "
            "a theorem, discharge GAP-1 through GAP-8 globally, promote C_k, "
            "embed C_k in an action, vary C_k, select a multiplier or penalty "
            "route, make a direct dynamical-law claim, close full Maxwell, "
            "EM-QFT, QFT-GR, or GR-QM, claim empirical validation, or promote "
            "the master action."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume select_next_ck_family_theorem_linkage_obligation_after_psi_A_total_conservation_closeout",
            "fail to select psi-A matter-sector exchange theorem-linkage gap",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.CKFamilyTheoremLinkageObligationSelectionAfterPsiATotalConservationCloseout",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Select the next C_k theorem-linkage obligation after psi-A total "
    ...
    payload = build_ck_family_theorem_linkage_obligation_selection_after_psi_A_total_conservation_closeout(
