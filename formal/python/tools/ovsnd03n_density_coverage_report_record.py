"""CLI wrapper for OV-SND-03N-COV density coverage report lock."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    rec = ovsnd03n_density_coverage_report_record(date=str(args.date))

    if not args.no_write:
        out = write_ovsnd03n_coverage_lock(
    ...
    if args.report:
        print("OV-SND-03N-COV report")
        print(f"  schema: {rec.schema}")
        print(f"  date: {rec.date}")
        print(f"  n_density_conditions_supported: {rec.decision['n_density_conditions_supported']}")
        print(f"  n_speed_conditions_supported: {rec.decision['n_speed_conditions_supported']}")
        print(f"  recommended_path: {rec.decision['recommended_path']}")
        print(f"  blockers: {rec.decision['blockers']}")
