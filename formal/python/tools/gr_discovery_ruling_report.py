...
from formal.python.tools.gr_discovery_discriminator_tranche_report import build_report as build_execution_report


REPO_ROOT = find_repo_root(Path(__file__))
SCHEMA_ID = "GR_DISCOVERY_RULING_REPORT_20260411_v0"
...
DEFAULT_DECLARATION_PATH = (
    ...
    / "docs"
    / "release"
    / "GR_DISCOVERY_RULING_20260411_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    next_route_alignment = bool(execution_summary.get("next_route_alignment", False))
    ...
    if (
        execution_classification == "DISCOVERY_TRANCHE_EXECUTABLE"
        and terminal_outcome_allowed
        and next_route_alignment
    ...
    return {
        ...
        "non_claim_boundary": "Repository-local GR discovery ruling report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate GR discovery ruling report.")
    parser.add_argument("--declaration", type=Path, default=DEFAULT_DECLARATION_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "gr_discovery_ruling_report_20260411_v0.json",
    )
