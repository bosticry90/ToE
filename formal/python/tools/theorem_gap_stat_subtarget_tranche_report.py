...
SCHEMA_ID = "THEOREM_GAP_STAT_SUBTARGET_TRANCHE_20260411_v0"

DEFAULT_TRANCHE_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "THEOREM_GAP_STAT_SUBTARGET_TRANCHE_20260411_v0.json"
)
...
def build_report(captured_at_utc: str | None, tranche_path: Path) -> dict[str, Any]:
    ...
    criteria = {
        "tranche_declares_stat_target_and_blocker": target_row == "ROW-PILLAR-STAT-001" and blocker_class == "THEOREM_GAP",
    ...
    objective_criteria = {
        "stat_subtarget_success_observed": success_observed,
    ...
    return {
        ...
        "objective_quality": {
            ...
            "summary": {
                ...
                "next_action": (
                    ...
                    else "DEFER_OR_RECLASSIFY_STAT_NEAR_TERM_BLOCKER_BURN_LANE"
                ),
            },
        },
        "summary": {
            ...
            "next_action": (
                "CONTINUE_STAT_SUBTARGET_BLOCKER_MOVING_REWORK"
                if all(criteria.values())
                else "RESTORE_STAT_TRANCHE_CONTRACT"
            ),
        },
        "source_bundle": {
        ...
        "non_claim_boundary": "Repository-local STAT sub-target theorem-gap tranche artifact only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate theorem-gap STAT sub-target tranche report.")
    parser.add_argument("--tranche", type=Path, default=DEFAULT_TRANCHE_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "theorem_gap_stat_subtarget_tranche_20260411_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"theorem_gap_stat_subtarget_tranche_report: phase_status={payload['summary']['phase_status']} out={out}")
    return 0
