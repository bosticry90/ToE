...
SCHEMA_ID = "SIMULATION_FIRST_FALSIFICATION_CAMPAIGN_DECISION_20260411_v0"
...
def build_report(*, packet_report_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    packet_outcome = str(packet.get("summary", {}).get("packet_outcome", "INCONCLUSIVE"))
    ...
    if scientific_state_change:
        decision = "SIMULATION_FIRST_CAMPAIGN_PRODUCTIVE"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate simulation-first falsification campaign decision report.")
    parser.add_argument("--packet-report", type=Path, default=DEFAULT_PACKET_REPORT_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "simulation_first_falsification_campaign_decision_20260411_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "simulation_first_falsification_campaign_decision_report: "
