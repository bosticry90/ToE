...
from formal.python.tools.qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_report import (
...
SCHEMA_ID = (
    ...
    "RETEST_REFINEMENT_RESULT_REVIEW_20260613_v0"
)
REVIEW_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
...
NEXT_TARGET_KIND = (
    ...
    "refinement_conservation_retest_preparation_only"
)
...
DEFAULT_OUT = (
    ...
    / (
        "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
        "RETEST_REFINEMENT_RESULT_REVIEW_20260613_v0.json"
    )
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The accepted retest result is inconclusive, so the single "
        ...
        },
        {
            ...
            "reason": (
                "This post-retest-refinement conservation-retest attempt "
                "result-review target is consumed here."
            ),
        },
        {
    ...
    return [
        {
            ...
            "selected": True,
            "reason": (
                "The second bounded retest is inconclusive and points to "
                "weak pairing-domain, regularity, source-definition, or "
                "scope refinement."
            ),
        },
        {
    ...
    return [
        {
            ...
            "selected": True,
            "reason": (
                "The v2 weak pairing domain and regular context still do not "
                "decide zero versus nonzero weak divergence."
            ),
        },
        {
            "refinement_target": "construct_countermodel_for_failed_retest",
            "selected": False,
...
def build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_result_review(
    *,
    ...
    acceptance_criteria = {
        "consumes_expected_post_retest_refinement_attempt": (
        ...
        "why_inconclusive_recorded": len(attempt.get("why_inconclusive", [])) >= 6,
    ...
    selected_next_target = (
        ...
        if accepted
            ...
            "ATTEMPT_AFTER_POST_RETEST_REFINEMENT_RESULT_REVIEW"
        )
    )
    ...
    return {
        ...
        if accepted
            ...
            "POST_RETEST_REFINEMENT_RESULT_REVIEW_REQUIRES_REMEDIATION"
        ),
        ...
        "consumes_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement": (
            EXPECTED_ATTEMPT_ID
        ),
        "consumes_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_pointer": _ptr(
            attempt_path
        ),
        ...
        "conservation_retest_attempt_after_post_retest_refinement_result_review_accepted": (
            accepted
        ),
        "conservation_retest_attempt_after_post_retest_refinement_consumed": accepted,
        "conservation_retest_attempt_after_post_retest_refinement_executed": (
            attempt.get("attempt_executed") is True
        ),
        ...
        "validation_caveat": (
            ...
            "result-review checkpoint; inherited caveat: "
            f"{attempt.get('validation_caveat')}"
        ),
        ...
        "next_action_scope": (
            ...
            "POST_RETEST_REFINEMENT_CONSERVATION_RETEST_ONLY_NO_RETEST_RERUN_"
            "NO_CONSERVATION_PROOF_WITNESS_SOURCE_ADMISSIBILITY_BIANCHI_"
            "SEMICLASSICAL_EINSTEIN_QFT_GR_CLOSURE_EMPIRICAL_VALIDATION_OR_"
            "PUBLIC_SUBMISSION"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts the bounded post-retest-refinement "
            "conservation-retest attempt as inconclusive and authorizes "
            "exactly one next bounded target: a model-refinement packet. It "
            "does not convert the inconclusive result into a pass or failure, "
            "does not rerun conservation, does not claim conservation, does "
            "not construct a conservation proof object, constructs no "
            "conservation witness, does not claim source admissibility, does "
            "not claim Bianchi compatibility, does not derive the "
            "semiclassical Einstein equation, does not close QFT-GR, does "
            "not validate empirically, does not authorize public submission, "
            "and does not promote the master action."
        ),
    }


def write_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR minimal working model post-retest-refinement "
            "conservation-retest attempt result review."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_result_review(
        attempt_path=attempt_path,
        out=out,
    ...
    print(
        "qft_gr_minimal_working_model_conservation_retest_attempt_after_post_"
        "retest_refinement_result_review_report: "
