    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The RN-ASSUMP-001 packet has been accepted; the next bounded "
                "step may attempt to reduce exactly the renormalized "
                "stress-energy object assumption."
            ),
        },
        {
            "target": (
            ...
            "decision": "deferred",
            "reason": (
                "Renormalization scope remains downstream of the RN-ASSUMP-001 "
                "bounded reduction attempt."
            ),
        },
        {
            "target": (
...
def build_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_result_review(
    *,
    ...
    selected_status = packet.get("renormalized_stress_energy_object_status_tokens", [])

    acceptance_criteria = {
        "consumes_expected_rn_assump_001_packet": packet.get("packet_id")
        == EXPECTED_PACKET_ID,
        ...
        "candidate_object_recorded": packet.get("candidate_stress_energy_object")
        == CANDIDATE_STRESS_ENERGY_OBJECT
        and packet.get("renormalized_stress_energy_object")
        == CANDIDATE_STRESS_ENERGY_OBJECT,
        ...
        "no_final_object_definition_or_discharge": packet.get(
            "renormalized_stress_energy_object_final_definition_claimed"
        )
        is False
        and packet.get("renormalized_stress_energy_object_defined_as_final")
        is False
        and packet.get("renormalized_stress_energy_object_assumption_discharged")
        is False
    ...
    return {
        ...
        "consumes_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet": (
            EXPECTED_PACKET_ID
        ),
        "consumes_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "renormalized_stress_energy_object_assumption_reduced_by_review": False,
        ...
        "authorized_attempt_scope": (
            "renormalized_stress_energy_object_assumption_reduction_attempt_"
            "only_no_final_object_definition_no_conservation_witness_no_qft_gr_"
            "seam_closure"
        ),
        "authorized_attempt_result_classifications": (
            AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS
        ...
        "next_action_scope": (
            ...
            "REDUCTION_ATTEMPT_ONLY_NO_FINAL_OBJECT_DEFINITION_CONSERVATION_"
            "WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the RN-ASSUMP-001 packet and "
            "authorizes one bounded reduction attempt. It does not define or "
            "discharge the renormalized stress-energy object as final, "
...
def write_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    ...
    payload = (
        build_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "object assumption-reduction packet result review."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_renormalized_stress_energy_object_assumption_reduction_packet_result_review_report: "
