...
SCHEMA_ID = (
    ...
    "RETEST_RESULT_REVIEW_20260613_v0"
)
REVIEW_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_ATTEMPT_AFTER_CONSERVATION_"
    "RETEST_RESULT_REVIEW_v0"
)
OUTCOME_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_ATTEMPT_AFTER_CONSERVATION_RETEST_"
    "RESULT_REVIEW_ACCEPTS_REFINED_CANDIDATE_AND_AUTHORIZES_BOUNDED_"
    "CONSERVATION_RETEST_PACKET_ONLY"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The post-retest refinement attempt is accepted only as a "
                "refined candidate, so the next bounded action may prepare a "
                "conservation-retest packet for that refined candidate. This "
        ...
        },
        {
            ...
            "reason": (
                "This post-retest refinement-attempt result-review target is "
                "consumed here."
            ),
        },
        {
        ...
        },
        {
            ...
            "reason": (
                "A retest retry remains downstream of a prepared and reviewed "
                "post-retest conservation-retest packet."
            ),
        },
        {
            "target": (
            ...
            "reason": (
                "The prior retest remains inconclusive and the refinement "
                "attempt result review does not convert it into a failure."
            ),
        },
        {
...
def build_qft_gr_minimal_working_model_refinement_attempt_after_conservation_retest_result_review(
    *,
    ...
    acceptance_criteria = {
        "consumes_expected_post_retest_refinement_attempt": attempt.get("schema_id")
        ...
        == CONSUMED_TARGET,
        "bounded_post_retest_attempt_executed": attempt.get(
            "bounded_refinement_attempt_executed"
        )
        is True
        and attempt.get("post_retest_refinement_attempt_executed") is True
        ...
        "post_retest_refined_components_confirmed": component_scopes
        >= {
            "weak_pairing_domain",
    ...
    selected_next_target = (
        ...
        if accepted
            ...
            "CONSERVATION_RETEST_RESULT_REVIEW"
        )
    )
    ...
    return {
        ...
        if accepted
            ...
            "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_ATTEMPT_AFTER_CONSERVATION_"
            "RETEST_RESULT_REVIEW_REQUIRES_REMEDIATION"
        ),
        ...
        "consumes_qft_gr_minimal_working_model_refinement_attempt_after_retest": (
            EXPECTED_ATTEMPT_ID
        ),
        "consumes_qft_gr_minimal_working_model_refinement_attempt_after_retest_pointer": _ptr(
            attempt_path
        ),
        ...
        "refinement_attempt_after_retest_result_review_accepted": accepted,
        "post_retest_refinement_attempt_result_review_accepted": accepted,
        "refinement_attempt_executed_confirmed": accepted,
        "bounded_refinement_attempt_executed": attempt.get(
            "bounded_refinement_attempt_executed"
        )
        is True,
        "post_retest_refinement_attempt_executed": attempt.get(
        ...
        "refined_artifact_status": (
            "post_retest_refined_toy_candidate_accepted_pending_conservation_"
            "retest_packet"
            if accepted
            else "requires_remediation"
        ),
        ...
        "validation_caveat": (
            ...
            "attempt-result-review checkpoint; inherited caveat: "
        ...
        "next_action_scope": (
            ...
            "AFTER_POST_RETEST_REFINEMENT_ONLY_NO_RETEST_EXECUTION_SOURCE_"
        ...
        "non_claim_boundary": (
            "This result review accepts only the bounded post-retest refined "
            "candidate and authorizes conservation-retest packet preparation "
            "only. It does not prepare the packet, execute a retest, claim "
            "source admissibility, claim conservation, construct a conservation "
            "proof object or witness, claim Bianchi compatibility, derive the "
...
def write_qft_gr_minimal_working_model_refinement_attempt_after_conservation_retest_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_refinement_attempt_after_conservation_retest_result_review(
        attempt_path=attempt_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_minimal_working_model_refinement_attempt_after_conservation_retest_result_review_report: "
