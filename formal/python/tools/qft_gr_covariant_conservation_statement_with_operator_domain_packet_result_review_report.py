...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_PACKET_RESULT_REVIEW_20260525_v0.json"
)
    ...
    return [
        {
            "target": NEXT_TARGET,
            "decision": "selected",
            "reason": "Accepted statement formulation may feed only a bounded witness attempt next.",
        },
        {
            "target": "prepare_qft_gr_renormalized_expectation_domain_conservation_packet",
            "decision": "deferred",
            "reason": "Expectation-domain refinement is not selected by this result review.",
        },
        {
            "target": "derive_semiclassical_einstein_equation",
            "decision": "not_authorized",
            "reason": "Statement formulation review does not derive the semiclassical Einstein equation.",
        },
        {
            "target": "close_qft_gr_seam",
            "decision": "not_authorized",
            "reason": "Statement formulation review does not close QFT-GR.",
        },
        {
...
def build_qft_gr_covariant_conservation_statement_with_operator_domain_packet_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        == CONSUMED_TARGET,
        "statement_formulated_under_operator_domain": packet.get(
            "covariant_conservation_statement_formulated"
        )
        is True
        and packet.get("covariant_conservation_statement_prepared") is True
        and packet.get("primary_blocker") == PRIMARY_BLOCKER
        and packet.get("primary_blocker_addressed_at_preparation_level") is True,
    ...
    return {
        ...
        if accepted
        else "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_PACKET_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION,
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_covariant_conservation_statement_with_operator_domain_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_covariant_conservation_statement_with_operator_domain_packet_pointer": _ptr(
            packet_path
        ),
        "consumed_packet_outcome_id": packet.get("outcome_id"),
        "consumed_packet_classification": packet.get("packet_classification"),
        "primary_blocker": PRIMARY_BLOCKER,
        "statement_formulation_accepted": accepted,
        "statement_formulated_under_accepted_operator_domain": packet.get(
            "covariant_conservation_statement_formulated"
        )
        is True,
        ...
        if accepted
        else "REMEDIATE_QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_PACKET_RESULT_REVIEW",
        ...
        "non_claim_boundary": (
            "This result review accepts only the formulated covariant "
            "conservation statement under the accepted operator-domain "
            "structure and authorizes a bounded witness attempt. It does not "
            "construct the witness, claim stress-energy source admissibility "
...
def write_qft_gr_covariant_conservation_statement_with_operator_domain_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    ...
    payload = (
        build_qft_gr_covariant_conservation_statement_with_operator_domain_packet_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR covariant conservation statement with operator-domain packet result review."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_covariant_conservation_statement_with_operator_domain_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_covariant_conservation_statement_with_operator_domain_packet_result_review_report: "
