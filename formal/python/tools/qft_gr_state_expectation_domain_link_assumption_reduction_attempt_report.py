...
from formal.python.tools.qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review_report import (
...
ATTEMPT_ID = "QFT_GR_STATE_EXPECTATION_DOMAIN_LINK_ASSUMPTION_REDUCTION_ATTEMPT_v0"
OUTCOME_ID = (
    "QFT_GR_STATE_EXPECTATION_DOMAIN_LINK_ASSUMPTION_REDUCTION_ATTEMPT_"
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The bounded state-expectation domain-link attempt must be "
                "reviewed before any downstream operator-domain row or source "
                "admissibility question is prepared."
            ),
        },
        {
            "target": (
            ...
            "reason": (
                ...
                "requires result-review acceptance for this attempt first."
            ),
        },
        {
            ...
            "reason": (
                "The state-expectation domain-link attempt does not authorize "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Reducing state-expectation domain linkage does not authorize "
...
def build_qft_gr_state_expectation_domain_link_assumption_reduction_attempt(
    *,
    review_path: Path = DEFAULT_REVIEW_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    review = _read_json(review_path)
    selected_assumption = review.get("state_expectation_domain_link_assumption", {})
    ...
    classification_rows = [
        {
            ...
            "selected": True,
            "meaning": (
                "The state-expectation domain-link assumption has been reduced "
                "to a bounded operator-domain link contract pending result "
                "review, without claiming source admissibility."
            ),
        },
        {
            ...
            "meaning": (
                ...
                "state-expectation domain linkage could be reduced."
            ),
        },
        {
    ...
    reduction_contract = {
        "contract_id": STATE_EXPECTATION_DOMAIN_LINK_CONTRACT_ID,
        ...
        "typed_scope": (
            "bounded_state_expectation_operator_domain_link_contract_for_"
        ...
        "claim_ceiling": (
            "state_expectation_domain_link_reduced_pending_result_review_only_"
            "no_source_admissibility_no_conservation_witness_no_qft_gr_seam_closure"
        ),
    }
    execution_findings = [
        "The attempt consumes the OD-ASSUMP-003 packet result review and keeps state-expectation domain linkage as the only executed row.",
        "The QFT state-expectation functional is pinned only to the prepared operator-domain link condition.",
        "The reduction is pending result review and does not discharge source admissibility, conservation, or Bianchi obligations.",
        "Renormalized-expectation linkage, source admissibility, conservation proof objects, and QFT-GR seam closure remain downstream.",
    ]
    acceptance_criteria = {
        ...
        is True,
        "prior_rows001_002_preserved": review.get(
            "prior_accepted_operator_domain_assumption_rows"
        )
        == [PRIOR_ACCEPTED_ROW001, PRIOR_ACCEPTED_ROW002],
        "prior_row001_contract_recorded_for_attempt": review.get(
            "prior_rows001_002_remain_accepted"
        )
        is True,
        "prior_row002_contract_recorded_for_attempt": review.get(
            "prior_rows001_002_remain_accepted"
        )
        is True,
        ...
        "state_expectation_object_preserved": review.get("state_expectation_object")
        == STATE_EXPECTATION_OBJECT
        and selected_assumption.get("state_expectation_object")
        == STATE_EXPECTATION_OBJECT,
    ...
    return {
        ...
        if executed
        else "QFT_GR_STATE_EXPECTATION_DOMAIN_LINK_ASSUMPTION_REDUCTION_ATTEMPT_BLOCKED",
        "consumes_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review": EXPECTED_REVIEW_ID,
        "consumes_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result_review_pointer": _ptr(
            review_path
        ),
        ...
        "operator_domain_link_condition": OPERATOR_DOMAIN_LINK_CONDITION,
        "attempt_scope": (
            "EXECUTE_BOUNDED_QFT_GR_STATE_EXPECTATION_DOMAIN_LINK_ASSUMPTION_"
        ...
        "state_expectation_domain_link_reduction_contract": reduction_contract,
        "execution_findings": execution_findings,
        "state_expectation_domain_link_assumption_reduction_attempt_executed": executed,
        "state_expectation_domain_link_assumption_reduced_pending_result_review": executed,
        "state_expectation_domain_link_assumption_obstruction_identified": False,
        "state_expectation_domain_link_assumption_inconclusive": False,
        "state_expectation_domain_link_assumption_discharged": False,
        "state_expectation_domain_link_assumption_reduced_by_attempt": executed,
        ...
        "non_claim_boundary": (
            "This execution reduces only OD-ASSUMP-003 state-expectation "
...
def write_qft_gr_state_expectation_domain_link_assumption_reduction_attempt(
    *,
    review_path: Path = DEFAULT_REVIEW_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_state_expectation_domain_link_assumption_reduction_attempt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_state_expectation_domain_link_assumption_reduction_attempt(
        review_path=review_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_state_expectation_domain_link_assumption_reduction_attempt_report: "
