...
from formal.python.tools.qft_gr_state_expectation_compatibility_assumption_reduction_packet_result_review_report import (
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The bounded SD-ASSUMP-003 state-expectation compatibility "
                "reduction attempt must be result-reviewed before any "
                "state admissibility, source admissibility, conservation, "
        ...
        },
        {
            ...
            "reason": (
                "A bounded state-expectation compatibility contract pending "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "The compatibility contract is not a stress-energy source-"
                "admissibility claim."
            ),
        },
        {
...
def build_qft_gr_state_expectation_compatibility_assumption_reduction_attempt(
    *,
    ...
    classification_rows = [
        {
            ...
            "selected": True,
            "meaning": (
                "The SD-ASSUMP-003 state-expectation compatibility assumption "
                "has been reduced to a bounded repo-local compatibility "
                "contract pending result review, without claiming state "
                "admissibility, source admissibility, Bianchi compatibility, "
                "conservation, or seam closure."
            ),
        },
        {
            ...
            "meaning": (
                ...
                "the state-expectation compatibility assumption could be reduced."
            ),
        },
        {
    ...
    reduction_contract = {
        "contract_id": STATE_EXPECTATION_COMPATIBILITY_CONTRACT_ID,
        ...
        "contract_status": BOUNDED_STATE_EXPECTATION_COMPATIBILITY_CONTRACT_STATUS,
        ...
        "available_repo_evidence": review.get(
            "state_expectation_compatibility_status_tokens", []
        ),
        "claim_ceiling": (
            "state_expectation_compatibility_contract_pending_result_review_only_"
    ...
    execution_findings = [
        "The attempt consumes the SD-ASSUMP-003 packet result review and keeps state_expectation_compatibility as the only executed row.",
        "The bounded repo-local contract links the state-expectation functional to the accepted operator-domain and renormalization-domain reductions.",
    ...
    acceptance_criteria = {
        ...
        == PRIOR_COMPLETED_FAMILIES,
        "prior_state_domain_rows_confirmed": review.get(
        ...
        "state_expectation_compatibility_preserved": review.get(
            "state_expectation_compatibility_condition"
        )
        == STATE_EXPECTATION_COMPATIBILITY_CONDITION
        and review.get("state_expectation_compatibility")
        == STATE_EXPECTATION_COMPATIBILITY_CONDITION,
        ...
        == 1,
        "no_state_expectation_compatibility_discharge": review.get(
            "state_expectation_compatibility_claimed"
        )
        is False
        and review.get("state_expectation_compatibility_discharged") is False
        and review.get("state_expectation_compatibility_satisfied") is False,
    ...
    return {
        ...
        "consumes_qft_gr_state_expectation_compatibility_assumption_reduction_packet_result_review": (
            EXPECTED_REVIEW_ID
        ),
        "consumes_qft_gr_state_expectation_compatibility_assumption_reduction_packet_result_review_pointer": _ptr(
            review_path
        ),
        ...
        "state_expectation_compatibility_status_tokens": review.get(
            "state_expectation_compatibility_status_tokens", []
        ),
        "required_future_proof_object": REQUIRED_FUTURE_PROOF_OBJECT,
        "candidate_reduction_route": CANDIDATE_REDUCTION_ROUTE,
        "failure_mode_if_unresolved": FAILURE_MODE_IF_UNRESOLVED,
        "attempt_scope": (
            "EXECUTE_BOUNDED_QFT_GR_STATE_EXPECTATION_COMPATIBILITY_ASSUMPTION_"
        ...
        "state_expectation_compatibility_reduction_contract": reduction_contract,
        ...
        "state_expectation_compatibility_assumption_obstruction_identified": False,
        "state_expectation_compatibility_assumption_inconclusive": False,
        "state_expectation_compatibility_assumption_reduced_by_attempt": executed,
        ...
        "non_claim_boundary": (
            "This execution reduces only SD-ASSUMP-003 to a bounded repo-local "
            "state-expectation-compatibility contract pending result review. "
            "It does not claim state admissibility, claim source admissibility, "
...
def write_qft_gr_state_expectation_compatibility_assumption_reduction_attempt(
    *,
    review_path: Path = DEFAULT_REVIEW_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_state_expectation_compatibility_assumption_reduction_attempt(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_state_expectation_compatibility_assumption_reduction_attempt(
        review_path=review_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_state_expectation_compatibility_assumption_reduction_attempt_report: "
