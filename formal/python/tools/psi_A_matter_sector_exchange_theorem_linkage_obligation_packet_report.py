...
SCHEMA_ID = "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_20260627_v0"
ARTIFACT_ID = SCHEMA_ID
PACKET_ID = "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_v0"
PACKET_RESULT = (
...
NEXT_TARGET = "review_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_result"
NEXT_TARGET_KIND = "psi_A_matter_sector_exchange_theorem_linkage_obligation_packet_result_review"
...
THEOREM_TARGET_ID = "psi_A_matter_sector_exchange_from_dirac_pair"
THEOREM_TARGET_NAME = "psi-A matter-sector exchange theorem-linkage from Dirac pair"

T_PSI_POLICY = "T_psi^{mu nu} policy"
DIRAC_EQUATION = "Dirac equation"
ADJOINT_DIRAC_EQUATION = "adjoint Dirac equation"
...
PLAIN_MEANING = NEXT_PACKET_PLAIN_MEANING
...
ACCEPTED_PACKET_FINDINGS = [
    "obligation: psi-A matter-sector exchange theorem-linkage gap",
    "basis: accepted psi-A matter stress-energy policy, Dirac pair, current definition, and shared compatibility assumptions",
    "proof style: Dirac-pair stress-energy divergence route with current definition and compatibility assumptions",
    "target: nabla_mu T_psi^{mu nu} = + F^nu{}_alpha J^alpha",
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_20260627_v0.json"
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Derivation"
    / "PsiAMatterSectorExchangeTheoremLinkageObligationPacket.lean"
)
...
def _validation_policy() -> dict[str, Any]:
    return {
        "checkpoint_type": "psi_A_matter_sector_exchange_theorem_linkage_obligation_packet",
...
def build_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet(
    *,
    ...
    acceptance_criteria = {
        ...
        "matter_sector_exchange_obligation_selected": (
            review.get("selected_obligation") == OBLIGATION
            and review.get("selected_obligation_rank") == 3
        ),
        "dirac_matter_exchange_target_scoped": (
        ...
        "basis_and_proof_style_recorded": (
            BASIS
            == "accepted psi-A matter stress-energy policy, Dirac pair, current definition, and shared compatibility assumptions"
            and PROOF_STYLE
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET"
    )
    payload: dict[str, Any] = {
        "artifact_id": ARTIFACT_ID,
        "schema_id": SCHEMA_ID,
        "packet_id": PACKET_ID,
        "status": "ACTIVE_PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET",
        "captured_at_utc": captured_at_utc,
        "prepared": accepted,
        "accepted": accepted,
        "outcome_id": OUTCOME_ID
        if accepted
        else "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_OBLIGATION_PACKET_REQUIRES_REMEDIATION",
        ...
        "non_claim_boundary": (
            "This packet scopes only the psi-A matter-sector exchange "
            "theorem-linkage target. It records that the accepted matter "
            "stress-energy policy, Dirac equation, adjoint Dirac equation, "
            "current definition J^alpha = q psibar gamma^alpha psi, shared "
            "compatibility assumptions, and shared domain/boundary assumptions "
            "are the inputs for the target nabla_mu T_psi^{mu nu} = + "
            "F^nu{}_alpha J^alpha. It does not execute any proof, discharge "
            "any theorem, discharge GAP-1 through GAP-8 globally, promote any "
            "C_k rule, embed C_k in an action, vary C_k, close full Maxwell, "
            "close EM-QFT, close QFT-GR, close GR-QM, claim empirical "
            "validation, or promote the master action."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume prepare_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet",
            "fail to scope the psi-A matter-sector exchange theorem target",
            "fail to record the T_psi policy",
            "fail to record the Dirac equation",
            "fail to record the adjoint Dirac equation",
            "fail to record J^alpha = q psibar gamma^alpha psi",
            "fail to preserve gamma/spin/tetrad/metric compatibility assumptions",
            "execute a proof",
            "discharge a theorem",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PsiAMatterSectorExchangeTheoremLinkageObligationPacket",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare the psi-A matter-sector exchange theorem-linkage obligation packet."
        )
    )
    ...
    payload = build_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet(
