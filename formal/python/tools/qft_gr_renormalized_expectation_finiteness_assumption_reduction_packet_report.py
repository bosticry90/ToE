...
from formal.python.tools.qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_result_review_report import (
    ...
    NEXT_ROW_ID as EXPECTED_SELECTED_FINITENESS_ROW,
...
from formal.python.tools.qft_gr_renormalized_expectation_domain_assumption_reduction_packet_report import (
    SELECTED_ROW_ID as ACCEPTED_PRIOR_DOMAIN_ROW,
)
...
PACKET_ID = "QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_v0"
OUTCOME_ID = (
    "QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_"
...
SELECTED_ROW_ID = EXPECTED_SELECTED_FINITENESS_ROW
ACCEPTED_PRIOR_ROWS = [
    ACCEPTED_PRIOR_OBJECT_ROW,
    ACCEPTED_PRIOR_SCOPE_ROW,
    ACCEPTED_PRIOR_DOMAIN_ROW,
]
NEXT_TARGET = (
...
FINITENESS_REGULARITY_BOUNDARY_SCOPE = [
    "repo_local_finiteness_regular_boundary_only",
    "accepted_renormalized_expectation_domain_contract_only",
    "no_finiteness_regular_boundary_discharge",
...
CANDIDATE_REDUCTION_ROUTE = (
    "bound the finiteness and regularity preconditions needed before testing "
    "covariant-divergence compatibility"
)
FAILURE_MODE_IF_UNRESOLVED = (
    "the conservation blocker remains mixed with finiteness or regularity failures"
)
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_20260606_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The prepared RN-ASSUMP-004 finiteness/regularity packet must "
                "be result-reviewed before any bounded reduction attempt."
            ),
        },
        {
            "target": (
        ...
        },
        {
            ...
            "reason": "Packet preparation does not discharge finiteness or regularity.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "The finiteness packet does not claim source admissibility.",
        },
        {
...
def build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet(
    *,
    ...
    if selected_row:
        selected_row_payload = {
            ...
            "claim_ceiling": (
                "renormalized_expectation_finiteness_packet_only_no_assumption_"
    ...
    acceptance_criteria = {
        "consumes_expected_rn_assump_003_result_review": result_review.get(
            "review_id"
        )
        ...
        == PRIOR_COMPLETED_FAMILY,
        "prior_rn_rows_accepted": result_review.get(
            "accepted_renormalization_assumption_rows"
        )
        == ACCEPTED_PRIOR_ROWS,
        "selects_only_finiteness_regular_boundary_row": selected_row is not None
        ...
        "finiteness_regular_boundary_condition_recorded": bool(
            FINITENESS_REGULARITY_BOUNDARY
        ),
        "scope_boundaries_recorded": len(FINITENESS_REGULARITY_BOUNDARY_SCOPE) == 7,
        "prepares_reduction_analysis_only": bool(selected_row_payload),
        "does_not_discharge_finiteness_regular_boundary": True,
    ...
    return {
        ...
        if prepared
        else "QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET_BLOCKED",
        "packet_classification": PACKET_CLASSIFICATION,
        "packet_classification_count": 1 if prepared else 0,
        "consumes_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_result_review": (
            EXPECTED_ATTEMPT_RESULT_REVIEW_ID
        ),
        "consumes_qft_gr_renormalized_expectation_domain_assumption_reduction_attempt_result_review_pointer": _ptr(
        ...
        "scope_boundaries": FINITENESS_REGULARITY_BOUNDARY_SCOPE,
        "renormalized_expectation_finiteness_assumption": selected_row_payload,
        "renormalized_expectation_finiteness_status_tokens": selected_status,
        ...
        "claim_ceiling": (
            "renormalized_expectation_finiteness_assumption_reduction_packet_"
        ...
        "renormalized_expectation_finiteness_assumption_reduction_analysis_prepared": (
            prepared
        ),
        "prepares_reduction_analysis_only": prepared,
        "finiteness_regular_boundary_discharged": False,
        "finiteness_regular_boundary_reduced_or_discharged_by_preparation": False,
        ...
        if prepared
        else "REMEDIATE_QFT_GR_RENORMALIZED_EXPECTATION_FINITENESS_ASSUMPTION_REDUCTION_PACKET",
        ...
        "non_claim_boundary": (
            "This packet prepares only RN-ASSUMP-004 finiteness/regularity "
            "reduction analysis. It does not discharge finiteness or "
            "regularity assumptions, construct a conservation proof object or "
...
def write_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet(
    *,
    attempt_result_review_path: Path = DEFAULT_ATTEMPT_RESULT_REVIEW_PATH,
    renormalization_packet_path: Path = DEFAULT_RENORMALIZATION_PACKET_PATH,
    ...
    payload = build_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR renormalized-expectation finiteness/"
            "regularity assumption-reduction packet."
        )
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet(
        attempt_result_review_path=attempt_result_review_path,
        renormalization_packet_path=renormalization_packet_path,
        out=out,
    ...
    print(
        "qft_gr_renormalized_expectation_finiteness_assumption_reduction_packet_report: "
