...
SCHEMA_ID = (
    "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_PACKET_"
...
NEXT_TARGET = "execute_qft_gr_metric_connection_scope_assumption_reduction_attempt"

AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS = [
    "qft_gr_metric_connection_scope_assumption_reduced_pending_result_review",
    (
        "qft_gr_metric_connection_scope_assumption_obstruction_identified_"
        "requires_refinement"
    ),
    (
        "qft_gr_metric_connection_scope_assumption_inconclusive_requires_"
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The OD-ASSUMP-006 packet is accepted as preparation-only, so "
                "the next bounded step may attempt metric/connection-scope "
                "reduction without constructing a conservation proof object, "
                "conservation witness, source admissibility claim, Bianchi "
                "compatibility claim, or QFT-GR seam closure."
            ),
        },
        {
            ...
            "reason": (
                "Accepting the metric/connection-scope packet does not "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "The packet review authorizes only a bounded assumption-row "
                "reduction attempt, not witness construction."
            ),
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Accepting a metric/connection-scope packet does not authorize "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Packet review confirms bounded metric/connection preparation "
                "only; it does not claim Bianchi compatibility."
            ),
        },
        {
...
def build_qft_gr_metric_connection_scope_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    packet = _read_json(packet_path)
    selected_assumption = packet.get("metric_connection_scope_assumption", {})
    selected_status = packet.get("metric_connection_scope_status_tokens", [])
    prior_rows = packet.get("prior_accepted_operator_domain_assumption_rows", [])
    candidate_next_targets = _candidate_next_targets()

    acceptance_criteria = {
        "consumes_expected_metric_connection_scope_packet": packet.get("packet_id")
        == EXPECTED_PACKET_ID,
        ...
        and bool(packet.get("prior_accepted_renormalized_expectation_domain_link_contract"))
        and bool(packet.get("prior_accepted_conservation_form_scope_contract")),
        "confirms_selected_row006": packet.get("selected_operator_domain_assumption_row")
        == SELECTED_ROW_ID
        and selected_assumption.get("assumption_id") == SELECTED_ROW_ID,
        "metric_connection_scope_object_confirmed": packet.get(
            "metric_connection_scope_object"
        )
        == METRIC_CONNECTION_SCOPE_OBJECT
        and selected_assumption.get("metric_connection_scope_object")
        == METRIC_CONNECTION_SCOPE_OBJECT,
        "bounded_geometry_domain_confirmed": packet.get("bounded_geometry_domain")
        == BOUNDED_GEOMETRY_DOMAIN
        and selected_assumption.get("bounded_geometry_domain")
        == BOUNDED_GEOMETRY_DOMAIN,
        "connection_compatibility_condition_confirmed": packet.get(
            "connection_compatibility_condition"
        )
        == CONNECTION_COMPATIBILITY_CONDITION
        and selected_assumption.get("connection_compatibility_condition")
        == CONNECTION_COMPATIBILITY_CONDITION,
        "required_future_proof_object_confirmed": packet.get(
            "required_future_proof_object"
        )
        == REQUIRED_FUTURE_PROOF_OBJECT
        ...
        and packet.get("metric_connection_scope_assumption_reduction_analysis_prepared")
        is True
        and packet.get("assumptions_reduced_or_discharged_by_preparation") is False,
        "no_metric_connection_assumption_reduced_by_review_alone": packet.get(
            "metric_connection_scope_assumption_discharged"
        )
        is False
        and packet.get("metric_connection_scope_claimed_as_bianchi_compatibility")
        is False
        and packet.get("metric_connection_scope_claimed_as_conservation_proof")
        is False
        and packet.get("metric_connection_scope_claimed_as_source_admissibility")
        is False
        and packet.get("bounded_geometry_domain_claimed_as_bianchi_compatibility")
        is False
        and packet.get("connection_compatibility_claimed_as_bianchi_compatibility")
        is False,
    ...
    return {
        ...
        "consumes_qft_gr_metric_connection_scope_assumption_reduction_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_metric_connection_scope_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "prior_rows001_002_003_004_005_remain_accepted": accepted,
        "selected_operator_domain_assumption_row": SELECTED_ROW_ID,
        "metric_connection_scope_assumption": selected_assumption,
        "metric_connection_scope_status_tokens": selected_status,
        ...
        "metric_connection_scope_packet_accepted_by_review": accepted,
        "metric_connection_scope_analysis_accepted": accepted,
        "metric_connection_scope_assumption_reduced_by_review": False,
        ...
        "authorized_attempt_scope": (
            "metric_connection_scope_assumption_reduction_attempt_only_no_"
        ...
        "selected_next_target_kind": (
            "qft_gr_metric_connection_scope_assumption_reduction_attempt_execution"
        ),
        "selected_route": (
            "qft_gr_metric_connection_scope_assumption_reduction_attempt_after_"
            "packet_result_review"
        ),
        "selection_count": 1 if accepted else 0,
        "next_action_scope": (
            "EXECUTE_QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_"
            "ATTEMPT_ONLY_NO_CONSERVATION_WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the OD-ASSUMP-006 "
            "metric/connection-scope packet and authorizes one bounded "
            "reduction attempt. It does not prove conservation, construct a "
...
def write_qft_gr_metric_connection_scope_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_metric_connection_scope_assumption_reduction_packet_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_metric_connection_scope_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_metric_connection_scope_assumption_reduction_packet_result_review_report: "
