...
def _other_obligations_tracked(packet: dict[str, Any]) -> bool:
...
def build_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        "policy_question_prepared": packet.get("policy_question") == EXPECTED_POLICY_QUESTION,
        ...
        "tranche_001_remains_release_blocking": packet.get(
        ...
        "other_five_obligations_tracked": _other_obligations_tracked(packet),
        "expert_re_review_requirement_preserved": packet.get("expert_re_review_required")
        is True,
    ...
    return {
        ...
        if accepted
        else "V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_RELEASE_POLICY_ADJUDICATION_PACKET_RESULT_REVIEW_BLOCKED",
        "consumes_packet": EXPECTED_PACKET_ID,
        "consumes_packet_pointer": _ptr(packet_path),
        "consumed_packet_schema_id": packet.get("schema_id"),
        "source_tranche_execution_result_review": packet.get("consumes_result_review"),
        "review_scope": (
            "REVIEW_TRANCHE_001_RELEASE_POLICY_ADJUDICATION_PACKET_ONLY_"
        ...
        if accepted
        else "REMEDIATE_V01_ALPHA_DEPENDENCY_REMEDIATION_TRANCHE_001_POLICY_PACKET_RESULT_REVIEW",
        ...
        "candidate_next_targets": [
            {
                "target": NEXT_TARGET,
                "decision": "selected",
                "reason": "The policy question packet is complete, so the narrow policy adjudication execution can be authorized.",
            },
            {
                "target": "execute_v01_alpha_dependency_remediation_tranche_002",
                "decision": "deferred",
                "reason": "The next remediation tranche remains deferred until tranche 001 policy meaning is adjudicated.",
            },
            {
                "target": "prepare_v01_alpha_release_readiness_adjudication_packet",
                "decision": "deferred",
                "reason": "Release-readiness adjudication remains blocked by the pending policy decision and five other blockers.",
            },
        ],
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "The v0.1-alpha dependency remediation tranche 001 release-policy adjudication packet "
            "result review accepts the policy-question preparation and authorizes only narrow policy "
            "adjudication execution. It does not decide the policy question, clear the blocker, assemble "
            "the release packet, mark v0.1-alpha readiness, discharge Lean theorem debt, reduce "
            "axiom/spec-backed proof debt, discharge retained assumptions, authorize Phase 2, close seams, "
            "validate empirically, promote the master action, promote claims, or make an external-truth claim."
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "v01_alpha_dependency_remediation_tranche_001_release_policy_adjudication_packet_result_review_report: "
