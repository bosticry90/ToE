...
SCHEMA_ID = "QM_STAT_RL10_DISCRETE_TRANSITION_BRIDGE_PROBE_RULING_REPORT_20260412_v0"
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    default_outcome = str(contract.get("default_outcome", "PROBE_SIGNAL_INCONCLUSIVE")).strip()
    ...
    if terminal_outcome == "PROBE_SIGNAL_CONFIRMED":
        next_action = "OPEN_BOUNDED_PROBE_SIGNIFICANCE_REVIEW"
    elif terminal_outcome == "PROBE_SIGNAL_NONDISCRIMINATIVE":
        next_action = "HOLD_PROBE_AND_REVIEW_SIGNAL_DISCRIMINATION"
    elif terminal_outcome == "PROBE_SIGNAL_INCONCLUSIVE":
        next_action = "REPEAT_ONLY_IF_GOVERNANCE_AUTHORIZES_ONE_MORE_BOUNDED_PASS"
    else:
        next_action = "RETIRE_PROBE_PATH_AND_RECORD_FALSIFICATION"
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_PROBE_RULING_OUTCOME",
            "no_loop_rule_declared": str(contract.get("no_loop_rule", "")).strip()
            == "ONE_BRIDGE_PROBE_RULING_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "non_claim_boundary": "Repository-local bridge probe ruling report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QM-STAT RL10 bridge probe ruling report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "qm_stat_rl10_discrete_transition_bridge_probe_ruling_report: "
