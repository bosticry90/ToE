...
from formal.python.tools.qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_report import (
    DEFAULT_OUT as DEFAULT_ATTEMPT_RESULT_REVIEW_PATH,
    NEXT_ROW_ID as EXPECTED_SELECTED_SCOPE_ROW,
...
SCHEMA_ID = "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_20260606_v0"
PACKET_ID = "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_v0"
OUTCOME_ID = (
...
SELECTED_ROW_ID = EXPECTED_SELECTED_SCOPE_ROW
NEXT_TARGET = "review_qft_gr_renormalization_scope_assumption_reduction_packet_result"
...
SCOPE_BOUNDARIES = [
    "repo_local_candidate_stress_energy_expectation_scope_only",
    "no_scheme_independence_claim",
...
CANDIDATE_REDUCTION_ROUTE = (
    ...
    "scheme-independence, source admissibility, Bianchi compatibility, and "
    "conservation"
)
FAILURE_MODE_IF_UNRESOLVED = (
    "renormalized expectation claims remain scope-ambiguous before the "
    "conservation proof-object route"
)
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_20260606_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The prepared RN-ASSUMP-002 renormalization-scope packet must be "
                "result-reviewed before any reduction attempt or downstream "
                "renormalization row is authorized."
            ),
        },
        {
            "target": "execute_qft_gr_renormalization_scope_assumption_reduction_attempt",
            "decision": "deferred",
        ...
        },
        {
            ...
            "decision": "deferred",
            "reason": "Finiteness/domain work remains downstream of RN-ASSUMP-002 review.",
        },
        {
            "target": "discharge_qft_gr_renormalization_scope_assumption",
            "decision": "not_authorized",
            "reason": "Packet preparation does not discharge renormalization scope assumptions.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "The scope packet does not claim source admissibility.",
        },
        {
...
def build_qft_gr_renormalization_scope_assumption_reduction_packet(
    *,
    ...
    if selected_row:
        selected_row_payload = {
            ...
            "renormalization_scope_status": RENORMALIZATION_SCOPE_STATUS,
            ...
            "claim_ceiling": (
                "renormalization_scope_packet_only_no_assumption_discharge_no_"
    ...
    acceptance_criteria = {
        "consumes_expected_rn_assump_001_result_review": result_review.get(
            "review_id"
        )
        ...
        == PRIOR_COMPLETED_FAMILY,
        "prior_rn_assump_001_accepted": result_review.get(
            "accepted_renormalization_assumption_row"
        )
        == ACCEPTED_PRIOR_ROW
        and result_review.get("accepted_renormalization_assumption_rows")
        == [ACCEPTED_PRIOR_ROW],
        "source_renormalization_packet_available": renormalization_packet.get(
            "packet_id"
        )
        == EXPECTED_RENORMALIZATION_PACKET_ID,
        "selects_only_renormalization_scope_row": selected_row is not None
        ...
        "renormalization_scope_object_recorded": bool(RENORMALIZATION_SCOPE),
        "scope_boundaries_recorded": len(SCOPE_BOUNDARIES) == 6,
        "prepares_reduction_analysis_only": bool(selected_row_payload),
        "does_not_discharge_renormalization_scope_assumption": True,
    ...
    return {
        ...
        if prepared
        else "QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_BLOCKED",
        "packet_classification": PACKET_CLASSIFICATION,
        "packet_classification_count": 1 if prepared else 0,
        "consumes_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review": (
            EXPECTED_ATTEMPT_RESULT_REVIEW_ID
        ),
        "consumes_qft_gr_renormalized_stress_energy_object_assumption_reduction_attempt_result_review_pointer": _ptr(
        ...
        "accepted_prior_row": ACCEPTED_PRIOR_ROW,
        ...
        "renormalization_scope_status": RENORMALIZATION_SCOPE_STATUS,
        "scope_boundaries": SCOPE_BOUNDARIES,
        "renormalization_scope_assumption": selected_row_payload,
        "renormalization_scope_status_tokens": selected_status,
        ...
        "candidate_reduction_route": CANDIDATE_REDUCTION_ROUTE,
        "claim_ceiling": (
            "renormalization_scope_assumption_reduction_packet_preparation_only_"
            "no_assumption_discharge_no_conservation_witness_no_qft_gr_seam_"
            "closure"
        ),
        "failure_mode_if_unresolved": FAILURE_MODE_IF_UNRESOLVED,
        "renormalization_scope_assumption_reduction_analysis_prepared": prepared,
        "prepares_reduction_analysis_only": prepared,
        "renormalization_scope_assumption_discharged": False,
        "renormalization_scope_assumption_reduced_or_discharged_by_preparation": (
            False
        ),
        ...
        if prepared
        else "REMEDIATE_QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET",
        "selected_next_target_kind": (
            "qft_gr_renormalization_scope_assumption_reduction_packet_result_review"
        ),
        "selected_route": (
            "qft_gr_renormalization_scope_assumption_reduction_packet_result_review_after_preparation"
        ),
        "selection_count": 1 if prepared else 0,
        "next_action_scope": (
            "REVIEW_QFT_GR_RENORMALIZATION_SCOPE_ASSUMPTION_REDUCTION_PACKET_"
            "RESULT_ONLY_NO_CONSERVATION_WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This packet prepares only RN-ASSUMP-002 renormalization-scope "
            "reduction analysis. It does not discharge renormalization scope "
...
def write_qft_gr_renormalization_scope_assumption_reduction_packet(
    *,
    attempt_result_review_path: Path = DEFAULT_ATTEMPT_RESULT_REVIEW_PATH,
    renormalization_packet_path: Path = DEFAULT_RENORMALIZATION_PACKET_PATH,
    ...
    payload = build_qft_gr_renormalization_scope_assumption_reduction_packet(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR renormalization-scope assumption-reduction packet."
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_renormalization_scope_assumption_reduction_packet(
        attempt_result_review_path=attempt_result_review_path,
        renormalization_packet_path=renormalization_packet_path,
        out=out,
    ...
    print(
        "qft_gr_renormalization_scope_assumption_reduction_packet_report: "
