...
SCHEMA_ID = (
    ...
    "RETAINED_BLOCKER_DECLARATION_RESULT_REVIEW_20260515_v0"
)
REVIEW_ID = (
    "V01_ALPHA_DEPENDENCY_REMEDIATION_NEXT_TRANCHE_SELECTION_PACKET_AFTER_TRANCHE_004_"
...
NEXT_TARGET = "prepare_v01_alpha_dependency_remediation_tranche_005_execution_packet"
...
def _unresolved_non_tranche_004(packet: dict[str, Any]) -> list[dict[str, Any]]:
    return list(packet.get("unresolved_non_tranche_004_obligations", []))
...
def _release_blockers_tracked(rows: list[dict[str, Any]]) -> bool:
    return (
        len(rows) == 3
        and [row.get("dependency_finding_id") for row in rows] == ALL_RELEASE_BLOCKER_IDS
...
def build_result_review(
    *,
    ...
    unresolved_non_tranche_004 = _unresolved_non_tranche_004(packet)
    selected_row = _selected_row(packet)
    retained_tranche_004 = dict(packet.get("retained_tranche_004_carry_forward", {}))
    ...
    acceptance_criteria = {
        ...
        and packet.get("retained_tranche_004_release_blocker_carry_forward_required") is True
        and packet.get("release_readiness_blocked_by_tranche_004") is True,
        "tranche_004_identity_and_reason_preserved": retained_tranche_004.get(
        ...
        and retained_tranche_004.get("construction_attempt_classification")
        ...
        "selection_method_stable_first_unresolved": selected_row.get("selection_method")
        == SELECTION_METHOD
        and packet.get("selection_policy", {}).get("method") == SELECTION_METHOD
        and packet.get("selection_policy", {}).get("eligible_finding_ids")
        == UNRESOLVED_NON_TRANCHE_004_IDS
        and packet.get("selection_policy", {}).get("selected_finding_id")
        == SELECTED_FINDING_ID,
    ...
    return {
        ...
        if accepted
            ...
            "V01_ALPHA_DEPENDENCY_REMEDIATION_NEXT_TRANCHE_SELECTION_AFTER_TRANCHE_004_"
            "RETAINED_BLOCKER_RESULT_REVIEW_BLOCKED"
        ),
        ...
        "tranche_005_selection_accepted": accepted,
        "tranche_005_execution_packet_preparation_authorized": accepted,
        ...
        if accepted
            ...
            "TRANCHE_004_RETAINED_BLOCKER_RESULT_REVIEW"
        ),
        "selected_next_target_kind": "tranche_005_execution_packet_preparation_only",
        "next_action_scope": (
        ...
        "candidate_next_targets": [
            {
                ...
                "decision": "selected",
                "reason": (
                    "The selection review accepts tranche 005 and authorizes only "
            ...
            },
            {
                ...
                "decision": "deferred",
                "reason": (
                    "Actual tranche 005 remediation execution requires an execution packet "
                    "and result review first."
                ),
            },
            {
                ...
                "decision": "deferred",
                "reason": (
                    "Release readiness remains blocked by tranche 004, but this review "
                    "continues the remaining remediation queue."
                ),
            },
        ],
        ...
        "non_claim_boundary": (
            ...
            "review after tranche 004 retained-blocker declaration accepts only the "
            "tranche 005 selection for V01-ALPHA-DEP-REM-005 / "
            "supplied_interface_alignment_semantics_construct_bridge_package_v0 and "
            "authorizes only tranche 005 execution-packet preparation. It carries "
            "tranche 004 as retained/release-blocking and does not execute remediation, "
            "prepare the execution packet during review, move tranche 004, register "
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "result review after tranche 004 retained-blocker declaration."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "v01_alpha_dependency_remediation_next_tranche_selection_packet_after_tranche_004_retained_blocker_declaration_result_review_report: "
