...
"""Feasibility + design pre-gate for BRIDGE_TICKET_TOYH_0004 (Toy-H current-anchor)."""
...
def build_bridge_toyh_c6_current_anchor_feasibility(*, repo_root: Path) -> dict:
    selected_target = "mismatch_toyh_current_only"
    primary_probe = "stress_toyh_current_control"
    ...
    if block_reasons:
        return {
            "schema_version": 1,
            "bridge_id": "BRIDGE_TICKET_TOYH_0004",
    ...
    has_grad = "grad2_2d" in surface_text
    ...
    resolved_by_current_anchor_count = int(targeted_resolution.get("n_current_control_fail_resolved_by_current_anchor", 0))
    ...
    selected_present = bool(target_reason_count > 0 or resolved_by_current_anchor_count > 0)

    has_primary_probe = primary_probe in target_probe_ids

    def _current_reason(it: dict) -> str:
        channels = dict(it.get("channels", {}))
        current = dict(channels.get("toyh_current", {}))
        return str(current.get("reason_code", ""))

    target_has_current_control_fail = any(
        _current_reason(it) == "FAIL_CURRENT_CHANNEL_CONTROL_MIN" for it in target_items
    )
    ...
    # Post-resolution fallback: target mismatch can disappear after implementation.
    if (not has_primary_probe) and (orth_primary is not None) and selected_present:
        ...
        current_payload = dict(orth_primary.get("toyh_current", {}))
        target_has_current_control_fail = (
            str(current_payload.get("reason_code", "")) == "FAIL_CURRENT_CHANNEL_CONTROL_MIN"
        )
    ...
    current_control_min = shared_probe_set.get("current_channel_control_min")

    has_pinned_grid = list(grid_sizes) == [7, 9, 11]
    has_tolerance_bundle = toyh_tol is not None and current_control_min is not None

    if not has_grid_type:
        block_reasons.append("C6 surface lacks required typed grid/parameter structures.")
    if not has_grad:
        block_reasons.append("C6 surface lacks deterministic gradient operator required for current-anchor checks.")
    if not selected_present:
        block_reasons.append(
            "Selected target region mismatch_toyh_current_only is absent from mismatch summary and has no resolved-by-anchor count."
        )
    if not has_primary_probe:
        block_reasons.append("Primary targeted probe stress_toyh_current_control is absent from selected mismatch region.")
    if not target_has_current_control_fail:
        block_reasons.append("Selected mismatch region does not contain FAIL_CURRENT_CHANNEL_CONTROL_MIN on Toy-H current channel.")
    ...
    if not has_tolerance_bundle:
        block_reasons.append("Pinned tolerance bundle (toyh_tolerance, current_channel_control_min) is incomplete.")
    ...
    payload = {
        ...
        "target_region": {
            ...
            "resolved_by_current_anchor_count": int(resolved_by_current_anchor_count),
        ...
        "pinned_plan": {
            ...
            "local_phase_shear_alpha_values": {"admissible": [1e-6], "inadmissible": [0.0]},
            "amplitude_scales": {"admissible": [1.0], "inadmissible": [1.1]},
            "tolerances": {
                "toyh_tolerance": float(toyh_tol) if toyh_tol is not None else None,
                "current_channel_control_min": float(current_control_min) if current_control_min is not None else None,
        ...
        "checks": {
            ...
            "has_gradient_tooling": has_grad,
            "selected_target_present": bool(selected_present),
            "target_reason_count": int(target_reason_count),
            "selected_target_resolved_by_current_anchor_count": int(resolved_by_current_anchor_count),
            "primary_probe_present": has_primary_probe,
            "target_has_current_control_fail_reason": target_has_current_control_fail,
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Feasibility + design pre-gate for BRIDGE_TICKET_TOYH_0004.")
    parser.add_argument(
        "--out",
        default="formal/quarantine/feasibility/BRIDGE_TOYH_C6_current_anchor_feasibility.json",
        help=(
    ...
    payload = build_bridge_toyh_c6_current_anchor_feasibility(repo_root=repo_root)
