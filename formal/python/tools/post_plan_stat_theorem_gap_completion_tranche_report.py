...
SCHEMA_ID = "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_REPORT_20260418_v0"
DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_20260418_v0.json"
)
DEFAULT_OUT_PATH = (
    REPO_ROOT / "formal" / "output" / "reports" / "post_plan_stat_theorem_gap_completion_tranche_20260418_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    cosmo_path = REPO_ROOT / str(required_inputs.get("post_plan_cosmo_tranche_report", "")).strip()
    ...
    doc_path = REPO_ROOT / str(required_inputs.get("stat_target_doc", "")).strip()
    artifact_path = REPO_ROOT / str(required_inputs.get("stat_artifact", "")).strip()
    gate_path = REPO_ROOT / str(required_inputs.get("stat_gate", "")).strip()

    queue_report = _read_json(queue_path)
    cosmo_report = _read_json(cosmo_path)
    ...
    queue_ok = (
        ...
        and queue_report.get("summary", {}).get("second_active_row") == str(policy.get("required_queue_second_active_row", "")).strip()
    )
    cosmo_ok = cosmo_report.get("summary", {}).get("terminal_outcome") == str(policy.get("required_cosmo_outcome", "")).strip()
    ...
    default_outcome = str(outcome_contract.get("default_outcome", "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE")).strip()

    if not matrix_row or not target_row:
        terminal_outcome = "HOLD_PENDING_POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_REPAIR"
        next_action = "RESTORE_STAT_TRANCHE_INPUT_SHAPE_AND_RERUN"
    elif all([queue_ok, cosmo_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and row_truth_change_detected:
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED"
        next_action = "REASSESS_SEAM_ROUTES_AND_MASTER_ACTION_WITH_CHANGED_STAT_ROW_TRUTH"
    elif all([queue_ok, cosmo_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and explicit_exhaustion_detected:
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED"
        next_action = "PREPARE_GR_DORMANT_NEW_STRUCTURE_COMPLETION_PACKAGE_WITH_STAT_EXHAUSTION_RECORDED"
    elif all([queue_ok, cosmo_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]):
        ...
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE"
        next_action = "REPAIR_STAT_TRANCHE_EVIDENCE_AND_RERUN"
    ...
    return {
        ...
        "criteria": {
            ...
            "cosmo_upstream_tranche_recorded": cosmo_ok,
            "target_map_materialized": target_map_ok,
            "stat_doc_tokens_present": doc_ok,
            "stat_artifact_alignment_ok": artifact_ok,
            "stat_row_alignment_ok": row_ok,
            "stat_live_theorem_gap_observation_present": contradiction_ok,
            "single_terminal_outcome_rule_declared": str(outcome_contract.get("single_terminal_outcome_rule", "")).strip()
            == "EXACTLY_ONE_ALLOWED_POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_OUTCOME",
            "no_loop_rule_declared": str(outcome_contract.get("no_loop_rule", "")).strip()
            == "ONE_POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
                "allowed_outcome_materialized": terminal_outcome in allowed_outcomes,
                "single_outcome_materialized": True,
                "promotion_only_if_row_truth_changed": (terminal_outcome != "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED") or row_truth_change_detected,
                "exhaustion_only_if_explicit_decision_present": (terminal_outcome != "POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED") or explicit_exhaustion_detected,
            },
            "inputs": {
        ...
        "source_bundle": {
            ...
            "post_plan_cosmo_tranche_report": _ptr(cosmo_path),
        ...
        "non_claim_boundary": "Repository-local post-plan STAT theorem-gap completion tranche measurement only; no scientific adequacy claim."
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the post-plan STAT theorem-gap completion tranche report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "post_plan_stat_theorem_gap_completion_tranche_report: "
