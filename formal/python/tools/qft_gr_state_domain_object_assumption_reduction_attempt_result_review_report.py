...
NEXT_ROW_ID = "SD-ASSUMP-002-state_admissibility_boundary"
NEXT_TARGET = "prepare_qft_gr_state_admissibility_boundary_assumption_reduction_packet"
NEXT_TARGET_KIND = (
...
def _row_by_id(rows: list[dict[str, Any]], row_id: str) -> dict[str, Any]:
    for row in rows:
        if row.get("assumption_id") == row_id:
            return row
    raise KeyError(f"Missing state-domain assumption row: {row_id}")
...
def _global_nonclaims() -> dict[str, bool]:
    return {
        ...
        "state_domain_object_assumption_discharged_by_review": False,
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The repo-authoritative state-domain row inventory places "
                "SD-ASSUMP-002-state_admissibility_boundary after the accepted "
                "SD-ASSUMP-001 bounded reduction."
            ),
        },
        {
            ...
            "decision": "deferred",
            "reason": (
                "State-expectation compatibility remains after the "
                "state-admissibility boundary row in the current inventory."
            ),
        },
        {
            "target": "discharge_qft_gr_state_domain_assumptions",
            "decision": "not_authorized",
            "reason": "Accepting SD-ASSUMP-001 does not discharge the state-domain family.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "State-domain object acceptance is not a source-admissibility claim.",
        },
        {
...
def _acceptance_criteria(
    ...
    contract = attempt.get("state_domain_object_reduction_contract", {})
    ...
    return {
        ...
        == PRIOR_COMPLETED_FAMILIES,
        "confirms_selected_row001_only": attempt.get(
            "selected_state_domain_assumption_row"
        )
        == SELECTED_ROW_ID
        and contract.get("assumption_id") == SELECTED_ROW_ID,
        "confirms_state_domain_object_contract": contract.get("contract_id")
        == STATE_DOMAIN_OBJECT_CONTRACT_ID
        and contract.get("contract_status") == BOUNDED_STATE_DOMAIN_OBJECT_CONTRACT_STATUS
        and contract.get("state_domain_object") == STATE_DOMAIN_OBJECT
        and contract.get("state_admissibility_boundary") == STATE_ADMISSIBILITY_BOUNDARY
        and contract.get("state_object_compatibility_condition")
        == STATE_OBJECT_COMPATIBILITY_CONDITION
        and contract.get("state_domain_object_definition_status")
        == STATE_DOMAIN_OBJECT_DEFINITION_STATUS
        and contract.get("required_future_proof_object") == REQUIRED_FUTURE_PROOF_OBJECT
        ...
        is True
        and attempt.get("state_domain_object_assumption_reduced_by_attempt") is True
        and attempt.get("state_domain_object_assumption_obstruction_identified")
        is False
        and attempt.get("state_domain_object_assumption_inconclusive") is False,
        "preserves_attempt_nonclaim_boundary": all(
            attempt.get(key) is value for key, value in _attempt_nonclaims().items()
        ),
        "repo_authoritative_inventory_selects_next_row002": row_ids.index(NEXT_ROW_ID)
        == row_ids.index(SELECTED_ROW_ID) + 1
        and next_row.get("assumption_family") == SELECTED_ASSUMPTION_FAMILY
        and next_row.get("current_status") == ["required", "missing", "candidate_reducible"],
        "selects_exactly_one_next_row_packet_target": sum(
...
def build_qft_gr_state_domain_object_assumption_reduction_attempt_result_review(
    *,
    ...
    inventory = state_domain_packet.get("candidate_reducible_assumptions", [])
    row_ids = _row_ids(inventory)
    next_row = _row_by_id(inventory, NEXT_ROW_ID)
    candidate_next_targets = _candidate_next_targets()
    criteria = _acceptance_criteria(attempt, row_ids, next_row, candidate_next_targets)
    ...
    return {
        ...
        if accepted
        else "QFT_GR_STATE_DOMAIN_OBJECT_ASSUMPTION_REDUCTION_ATTEMPT_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION
        if accepted
        else (
            "qft_gr_state_domain_object_assumption_reduction_attempt_result_"
            "review_blocked"
        ),
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_state_domain_object_assumption_reduction_attempt": (
            EXPECTED_ATTEMPT_ID
        ),
        "consumes_qft_gr_state_domain_object_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "accepted_state_domain_assumption_rows": [SELECTED_ROW_ID],
        ...
        "state_domain_object_reduction_contract": attempt.get(
            "state_domain_object_reduction_contract"
        ),
        "accepted_contract_id": STATE_DOMAIN_OBJECT_CONTRACT_ID,
        "bounded_state_domain_object_contract_status": (
            BOUNDED_STATE_DOMAIN_OBJECT_CONTRACT_STATUS
        ),
        ...
        "state_domain_object_assumption_reduction_attempt_result_reviewed": accepted,
        "state_domain_object_assumption_reduction_accepted": accepted,
        "state_domain_object_assumption_reduction_rejected": not accepted,
        "state_domain_object_assumption_reduced_pending_result_review_accepted": (
            accepted
        ),
        "state_domain_object_assumption_accepted": accepted,
        "repo_authoritative_state_domain_row_inventory": row_ids,
        "repo_authoritative_state_domain_row_inventory_count": len(row_ids),
        "next_state_domain_assumption_row": NEXT_ROW_ID if accepted else None,
        "next_state_domain_assumption_row_status_tokens": next_row.get("current_status"),
        "next_state_domain_assumption_row_required_future_proof_object": next_row.get(
            "required_future_proof_object"
        ),
        "next_state_domain_assumption_row_candidate_reduction_route": next_row.get(
            "candidate_reduction_route"
        ),
        "state_admissibility_boundary_assumption_packet_preparation_authorized": (
            accepted
        ),
        "state_admissibility_boundary_assumption_packet_target": selected_target,
        **_global_nonclaims(),
        "candidate_next_targets": candidate_next_targets,
        "selected_next_target": selected_target
        if accepted
        else (
            "REMEDIATE_QFT_GR_STATE_DOMAIN_OBJECT_ASSUMPTION_REDUCTION_ATTEMPT_"
            "RESULT_REVIEW"
        ),
        "selected_next_target_kind": NEXT_TARGET_KIND,
        "selected_route": (
            "qft_gr_state_admissibility_boundary_assumption_reduction_packet_"
            "preparation_after_sd_assump_001_result_review"
        ),
        ...
        "next_action_scope": (
            "PREPARE_QFT_GR_STATE_ADMISSIBILITY_BOUNDARY_ASSUMPTION_REDUCTION_"
            "PACKET_ONLY_NO_STATE_ADMISSIBILITY_DISCHARGE_CONSERVATION_WITNESS_"
            "OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": criteria,
        "non_claim_boundary": (
            "This result review accepts only the bounded SD-ASSUMP-001 "
            "state-domain object reduction contract and authorizes preparation "
            "of the next state-domain row packet. It does not discharge "
            "state-domain assumptions by implication, discharge state "
        ...
        "claim_ceiling": [
            "no state-domain family discharge",
            "no state-admissibility discharge",
...
def write_qft_gr_state_domain_object_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    state_domain_packet_path: Path = DEFAULT_STATE_DOMAIN_PACKET_PATH,
    ...
    payload = build_qft_gr_state_domain_object_assumption_reduction_attempt_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_state_domain_object_assumption_reduction_attempt_result_review(
        attempt_path=attempt_path,
        state_domain_packet_path=state_domain_packet_path,
        out=out,
    ...
    print(
        "qft_gr_state_domain_object_assumption_reduction_attempt_result_review_report: "
        f"accepted={payload['accepted']} row={payload['accepted_state_domain_assumption_row']} "
