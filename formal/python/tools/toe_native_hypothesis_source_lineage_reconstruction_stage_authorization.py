...
VALIDATION_PATH = (
    RELEASE_ROOT
    / "TOE_NATIVE_HYPOTHESIS_SOURCE_LINEAGE_RECONSTRUCTION_OPEN_VALIDATION_v0.json"
)
...
KIND = "toe_native_hypothesis_source_lineage_reconstruction_stage_2_open_v0"
EVIDENCE = (
    "formal/toe_formal/ToeFormal/Derivation/"
    "ToeNativeHypothesisSourceLineageReconstructionAttemptOpen.lean"
)
REPORT = (
...
OUTCOME = "SOURCE_LINEAGE_RECONSTRUCTION_STAGE_2_OPEN"
STRICT_OUTCOME = (
    "STAGE_2_OPEN_LINEAGE_ONLY_NO_LINEAGE_RESULT_CLAIM_EXTRACTION_"
    "SCIENTIFIC_ADJUDICATION_EVIDENCE_PROMOTION_FRONTIER_SELECTION_"
    "OR_STAGE_3"
)
PREVIOUS_STAGE_TARGET = "inventory_toe_repository_wide_native_hypothesis_sources_v0"
EXPECTED_SCOPE_HASH = (
...
OPEN_EXACT_PATH_SET = [
    ...
    (
        "formal/docs/release/"
        "TOE_NATIVE_HYPOTHESIS_SOURCE_LINEAGE_RECONSTRUCTION_"
...
def _check_authority() -> None:
    ...
    if authority["authorized_stage"] != {
        ...
        raise ValueError("Stage 2 OPEN authority does not match the manifest")
    if authority["stage_1_input_binding"]["exact_duplicate_group_count"] != 421:
        raise ValueError("Stage 1 duplicate-group input is not bound")
    if authority["workload_limits"]["maximum_claims_extracted"] != 0:
        raise ValueError("claim extraction is not permitted in Stage 2")
    if authority["workload_limits"]["maximum_evidence_promotions"] != 0:
        raise ValueError("evidence promotion is not permitted in Stage 2")
    ...
    if review["decision"] != "AUTHORIZE_STAGE_2_OPEN":
        raise ValueError("Stage 2 OPEN authority review decision mismatch")
...
def _project_current_target(registry: dict, report_sha256: str) -> None:
    ...
    if len(active) != 1 or active[0]["workstream_id"] != TARGET:
        raise ValueError("expected the selected Stage 2 workstream to be active")
    workstream = active[0]
    workstream.update(
        {
            "queue_scope": (
                "Stage 2 source-lineage reconstruction is OPEN under immutable "
                "bounded-program authority; no lineage result exists"
            ),
            ...
            "consumed_target_kind": "closed_bounded_stage_1_scientific_authority",
            "claim_ceiling_level": 2,
            "claim_label": "B-BOUNDED",
            "claim_status": (
                "Stage 2 OPEN only for source-lineage reconstruction; no "
                "lineage result, claim extraction, scientific adjudication, "
                "evidence promotion, frontier selection, or Stage 3 output"
            ),
        }
    )
...
def _validation_payload(
    *,
    ...
    return {
        ...
        "authority_decision": "AUTHORIZE_STAGE_2_OPEN",
        ...
        "schema_id": (
            "toe.native_hypothesis_source_lineage_reconstruction."
        ...
        "scientific_output_at_open": {
            ...
            "lineage_result_produced": False,
            "native_frontier_selected": False,
            "scientific_claim_adjudicated": False,
            "stage_3_output_produced": False,
        },
        "semantic_stage_id": SEMANTIC_STAGE_ID,
        "stage_1_input_binding": _read(AUTHORITY_PATH)["stage_1_input_binding"],
        ...
        "validation_checks": {
            ...
            "lineage_only_boundary_preserved": True,
            "open_checkpoint_contains_no_lineage_result": True,
            "program_attempt_count_is_two": (
            ...
            "stage_1_close_is_bound": True,
            "stage_3_remains_prohibited": True,
...
def open_stage(*, opened_from_commit: str) -> str:
    ...
    if event["attempt_sequence_number"] != 2:
        raise ValueError("OPEN event attempt number mismatch")
    if event["event_sequence_number"] != 3:
