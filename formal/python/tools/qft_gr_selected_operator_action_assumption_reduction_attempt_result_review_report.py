...
DEFAULT_CAPTURED_AT_UTC = "2026-05-27T00:00:00Z"
SCHEMA_ID = (
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_SELECTED_OPERATOR_ACTION_ASSUMPTION_REDUCTION_ATTEMPT_RESULT_REVIEW_20260527_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The selected-operator/action reduction is accepted only as a "
                "bounded contract, so the next operator-domain row may be "
                "prepared without claiming conservation or source admissibility."
            ),
        },
        {
            ...
            "reason": (
                ...
                "downstream of candidate source-domain membership."
            ),
        },
        {
        ...
        },
        {
            ...
            "reason": (
                "Accepting the bounded operator/action contract does not "
...
def build_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    attempt = _read_json(attempt_path)
    contract = attempt.get("selected_operator_action_reduction_contract", {})
    ...
    acceptance_criteria = {
        ...
        "reduced_pending_review_result_accepted": attempt.get(
            "operator_action_assumption_reduced_pending_result_review"
        )
        is True
        and attempt.get("operator_action_assumption_obstruction_identified") is False
        and attempt.get("operator_action_assumption_inconclusive") is False,
        "reduction_not_treated_as_discharge": attempt.get(
            "operator_action_assumption_discharged"
        )
        is False
        ...
        is False,
        "no_source_admissibility_or_bianchi_claim": attempt.get(
        ...
        "next_row_is_second_operator_domain_row": row_sequence[1]
        == NEXT_OPERATOR_DOMAIN_ASSUMPTION_ROW,
    ...
    return {
        ...
        if accepted
            ...
            "QFT_GR_SELECTED_OPERATOR_ACTION_ASSUMPTION_REDUCTION_ATTEMPT_"
            "RESULT_REVIEW_REJECTS_OR_REQUIRES_REMEDIATION"
        ),
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION
        if accepted
        else (
            "qft_gr_selected_operator_action_assumption_reduction_attempt_result_"
            "review_rejects_or_requires_remediation"
        ),
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_selected_operator_action_assumption_reduction_attempt": EXPECTED_ATTEMPT_ID,
        "consumes_qft_gr_selected_operator_action_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "selected_operator_action_reduction_contract": contract,
        "accepted_contract_id": SELECTED_OPERATOR_ACTION_CONTRACT_ID
        if accepted
        else "",
        "contract_acceptance_scope": (
            "bounded_operator_action_contract_only_no_full_assumption_discharge"
        ),
        ...
        "operator_action_assumption_reduction_attempt_result_reviewed": accepted,
        "operator_action_assumption_reduction_accepted": accepted,
        "operator_action_assumption_reduction_rejected": not accepted,
        "operator_action_assumption_reduced_pending_result_review_accepted": accepted,
        "operator_action_assumption_discharged": False,
        "operator_action_assumption_discharged_by_review": False,
        ...
        if accepted
            ...
            "REMEDIATE_QFT_GR_SELECTED_OPERATOR_ACTION_ASSUMPTION_REDUCTION_"
        ...
        "next_action_scope": (
            "PREPARE_QFT_GR_CANDIDATE_SOURCE_DOMAIN_MEMBERSHIP_ASSUMPTION_"
            "REDUCTION_PACKET_ONLY_NO_ASSUMPTION_DISCHARGE_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the reduced selected-operator/action "
            "contract for OD-ASSUMP-001 and authorizes preparation of the next "
            "operator-domain row, OD-ASSUMP-002 candidate source-domain "
            "membership. It does not discharge the assumption as final, "
...
def write_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    ...
    payload = (
        build_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_selected_operator_action_assumption_reduction_attempt_result_review(
        attempt_path=attempt_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_selected_operator_action_assumption_reduction_attempt_result_review_report: "
