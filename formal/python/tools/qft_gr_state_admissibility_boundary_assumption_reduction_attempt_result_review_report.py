...
from formal.python.tools.qft_gr_state_admissibility_boundary_assumption_reduction_attempt_report import (
...
NEXT_ROW_ID = "SD-ASSUMP-003-state_expectation_compatibility"
NEXT_TARGET = (
...
def _global_nonclaims() -> dict[str, bool]:
    return {
        ...
        "state_admissibility_boundary_assumption_discharged_by_review": False,
        "state_admissibility_boundary_assumption_reduced_or_discharged_by_implication": False,
    ...
    return [
        {
            ...
            "reason": (
                ...
                "SD-ASSUMP-003-state_expectation_compatibility after the "
                "accepted SD-ASSUMP-002 bounded reduction."
            ),
        },
        {
            ...
            "reason": (
                "Accepting SD-ASSUMP-002 does not discharge the state-domain "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "A reviewed state-admissibility boundary contract is not a "
                "state-admissibility discharge."
            ),
        },
        {
            "target": "claim_qft_gr_source_admissibility",
            "decision": "not_authorized",
            "reason": "State-domain boundary acceptance is not source admissibility.",
        },
        {
...
def _acceptance_criteria(
    ...
    contract = attempt.get("state_admissibility_boundary_reduction_contract", {})
    ...
    return {
        ...
        == PRIOR_COMPLETED_FAMILIES,
        "confirms_sd_assump_001_remains_accepted": attempt.get(
            "accepted_prior_state_domain_assumption_row"
        )
        == ACCEPTED_PRIOR_ROW_ID
        and attempt.get("accepted_state_domain_assumption_rows")
        == [ACCEPTED_PRIOR_ROW_ID],
        "confirms_selected_row002_only": attempt.get(
            "selected_state_domain_assumption_row"
        )
        == SELECTED_ROW_ID
        and contract.get("assumption_id") == SELECTED_ROW_ID,
        "confirms_state_admissibility_boundary_contract": contract.get("contract_id")
        == STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_ID
        and contract.get("contract_status")
        == BOUNDED_STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_STATUS
        and contract.get("state_admissibility_boundary")
        == STATE_ADMISSIBILITY_BOUNDARY_CONDITION
        and contract.get("state_admissibility_boundary_condition")
        == STATE_ADMISSIBILITY_BOUNDARY_CONDITION
        ...
        is True
        and attempt.get("state_admissibility_boundary_assumption_reduced_by_attempt")
        is True
        and attempt.get("state_admissibility_boundary_assumption_obstruction_identified")
        is False
        and attempt.get("state_admissibility_boundary_assumption_inconclusive")
        is False,
        "preserves_attempt_nonclaim_boundary": all(
            attempt.get(key) is value for key, value in _attempt_nonclaims().items()
        ),
        "repo_authoritative_inventory_selects_next_row003": row_ids.index(NEXT_ROW_ID)
...
def build_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review(
    *,
    ...
    return {
        ...
        "consumes_qft_gr_state_admissibility_boundary_assumption_reduction_attempt": (
            EXPECTED_ATTEMPT_ID
        ),
        "consumes_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "accepted_state_domain_assumption_row_count": 2,
        "state_admissibility_boundary": STATE_ADMISSIBILITY_BOUNDARY_CONDITION,
        "state_admissibility_boundary_condition": (
            STATE_ADMISSIBILITY_BOUNDARY_CONDITION
        ),
        "state_admissibility_boundary_reduction_contract": attempt.get(
            "state_admissibility_boundary_reduction_contract"
        ),
        "accepted_contract_id": STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_ID,
        "bounded_state_admissibility_boundary_contract_status": (
            BOUNDED_STATE_ADMISSIBILITY_BOUNDARY_CONTRACT_STATUS
        ),
        ...
        "state_admissibility_boundary_assumption_reduction_attempt_result_reviewed": (
            accepted
        ),
        "state_admissibility_boundary_assumption_reduction_accepted": accepted,
        "state_admissibility_boundary_assumption_reduction_rejected": not accepted,
        "state_admissibility_boundary_assumption_reduced_pending_result_review_accepted": (
            accepted
        ),
        "state_admissibility_boundary_assumption_accepted": accepted,
        ...
        "state_expectation_compatibility_assumption_packet_preparation_authorized": (
            accepted
        ),
        "state_expectation_compatibility_assumption_packet_target": selected_target,
        **_global_nonclaims(),
        ...
        "selected_route": (
            "qft_gr_state_expectation_compatibility_assumption_reduction_packet_"
            "preparation_after_sd_assump_002_result_review"
        ),
        ...
        "next_action_scope": (
            ...
            "REDUCTION_PACKET_ONLY_NO_STATE_ADMISSIBILITY_SOURCE_ADMISSIBILITY_"
            "CONSERVATION_WITNESS_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": criteria,
        "non_claim_boundary": (
            "This result review accepts only the bounded SD-ASSUMP-002 "
            "state-admissibility-boundary reduction contract and authorizes "
            "preparation of the next state-domain row packet. It does not "
...
def write_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    state_domain_packet_path: Path = DEFAULT_STATE_DOMAIN_PACKET_PATH,
    ...
    payload = (
        build_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = (
        write_qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review(
            attempt_path=attempt_path,
            state_domain_packet_path=state_domain_packet_path,
            out=out,
    ...
    print(
        "qft_gr_state_admissibility_boundary_assumption_reduction_attempt_result_review_report: "
