...
SCHEMA_ID = (
    ...
    "CONSERVATION_CLOSEOUT_RESULT_REVIEW_20260627_v0"
)
...
NEXT_TARGET = "prepare_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet"
NEXT_TARGET_KIND = "psi_A_matter_sector_exchange_theorem_linkage_obligation_packet"
...
REVIEW_ACCEPTANCE_SUMMARY = [
    ...
    "psi-A matter-sector exchange theorem-linkage gap selected",
...
DEFAULT_OUT = (
    ...
    / (
        "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PSI_A_TOTAL_"
        "CONSERVATION_CLOSEOUT_RESULT_REVIEW_20260627_v0.json"
    )
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    ...
    / "ToeFormal"
    / "Derivation"
    / "CKFamilyTheoremLinkageObligationSelectionAfterPsiATotalConservationCloseoutResultReview.lean"
)
...
def _validation_policy() -> dict[str, Any]:
    return {
        "checkpoint_type": (
            ...
            "psi_A_total_conservation_closeout_result_review"
        ),
...
def build_ck_family_theorem_linkage_obligation_selection_after_psi_A_total_conservation_closeout_result_review(
    *,
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PSI_A_TOTAL_CONSERVATION_CLOSEOUT_RESULT_REVIEW"
    )
    payload: dict[str, Any] = {
        ...
        if accepted
        else "CK_FAMILY_THEOREM_LINKAGE_OBLIGATION_SELECTION_AFTER_PSI_A_TOTAL_CONSERVATION_CLOSEOUT_RESULT_REVIEW_REQUIRES_REMEDIATION",
        ...
        "matter_exchange_target_rule": MATTER_EXCHANGE_TARGET_RULE,
        "gauge_exchange_dependency": GAUGE_EXCHANGE_DEPENDENCY,
        ...
        "next_packet_scope": (
            "prepare the psi-A matter-sector exchange theorem-linkage "
        ...
        "non_claim_boundary": (
            ...
            "psi-A total conservation closeout. It accepts that the psi-A "
            "matter-sector exchange theorem-linkage gap was selected and "
            "preserves preparation of that obligation packet as the next target. "
            "It does not execute any proof, discharge any theorem, discharge "
            "GAP-1 through GAP-8, promote any C_k rule, embed C_k in an action, "
            "vary C_k, claim full Maxwell closure, close EM-QFT, close QFT-GR, "
            "close GR-QM, claim empirical validation, or promote the master action."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume review_ck_family_theorem_linkage_obligation_selection_after_psi_A_total_conservation_closeout_result",
            "fail to accept the psi-A matter-sector exchange theorem-linkage gap selection",
            "fail to select prepare_psi_A_matter_sector_exchange_theorem_linkage_obligation_packet",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.CKFamilyTheoremLinkageObligationSelectionAfterPsiATotalConservationCloseoutResultReview",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Review the post-psi-A total conservation C_k theorem-linkage "
    ...
    payload = build_ck_family_theorem_linkage_obligation_selection_after_psi_A_total_conservation_closeout_result_review(
