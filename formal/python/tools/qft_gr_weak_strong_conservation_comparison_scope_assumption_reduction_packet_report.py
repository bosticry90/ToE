...
from formal.python.tools.qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review_report import (
    DEFAULT_OUT as DEFAULT_MR001_RESULT_REVIEW_PATH,
    ...
    OUTCOME_ID as EXPECTED_MR001_RESULT_REVIEW_OUTCOME,
    RESULT_REVIEW_CLASSIFICATION as EXPECTED_MR001_RESULT_REVIEW_CLASSIFICATION,
    REVIEW_ID as EXPECTED_MR001_RESULT_REVIEW_ID,
    SCHEMA_ID as EXPECTED_MR001_RESULT_REVIEW_SCHEMA_ID,
)
...
ACCEPTED_PRIOR_ROW_ID = "MR-ASSUMP-001-derivative_exchange_regular_boundary"
...
WEAK_STRONG_COMPARISON_BOUNDARY = (
    "weak_strong_conservation_comparison_scope_for_future_conservation_"
    "proof_object"
)
COMPARISON_SCOPE_BOUNDARIES = [
    "weak_conservation_scope_recorded_for_future_distributional_or_test_pairing",
    "strong_conservation_scope_recorded_for_future_covariant_divergence_form",
    "comparison_scope_only_no_weak_conservation_proof",
    "comparison_scope_only_no_strong_conservation_proof",
    "no_weak_strong_equivalence_claim_without_future_proof_object",
    "no_source_admissibility_claim",
    "no_bianchi_compatibility_claim",
    "no_qft_gr_seam_closure",
]
CANDIDATE_REDUCTION_ROUTE = (
    "separate strong, weak, and distributional comparison requirements before "
    "any conservation proof-object attempt"
)
FAILURE_MODE_IF_UNRESOLVED = (
    "the project may conflate weak and strong conservation forms without the "
    "regularity needed to compare them"
)
...
DEFAULT_OUT = (
    ...
    / (
        "QFT_GR_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_ASSUMPTION_"
        "REDUCTION_PACKET_20260608_v0.json"
    )
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The prepared MR-ASSUMP-002 weak/strong conservation comparison "
                "scope packet must be result-reviewed before any bounded "
                "reduction attempt is authorized."
            ),
        },
        {
            "target": (
            ...
            "decision": "deferred",
            "reason": "A bounded MR-ASSUMP-002 attempt requires packet result review first.",
        },
        {
            "target": (
                "prepare_qft_gr_distributional_pairing_regular_domain_"
                "assumption_reduction_packet"
            ),
            "decision": "deferred",
            "reason": "MR-ASSUMP-003 remains downstream of MR-ASSUMP-002 packet review.",
        },
        {
            "target": "prove_qft_gr_weak_conservation",
            "decision": "not_authorized",
            "reason": "Packet preparation records scope only; weak conservation is not proved.",
        },
        {
            "target": "prove_qft_gr_strong_conservation",
            "decision": "not_authorized",
            "reason": "Packet preparation records scope only; strong conservation is not proved.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "Weak/strong comparison scope is not state admissibility.",
        },
        {
            "target": "claim_qft_gr_source_admissibility",
            "decision": "not_authorized",
            "reason": "Weak/strong comparison scope is not source admissibility.",
        },
        {
...
def _nonclaims() -> dict[str, bool]:
    return {
        ...
        "weak_strong_conservation_equivalence_claimed": False,
        "weak_strong_conservation_comparison_scope_discharged": False,
        "weak_strong_conservation_comparison_scope_reduced_or_discharged_by_preparation": False,
...
def build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet(
    *,
    mr001_result_review_path: Path = DEFAULT_MR001_RESULT_REVIEW_PATH,
    mathematical_regularity_packet_path: Path = DEFAULT_MATHEMATICAL_REGULARITY_PACKET_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    result_review = _read_json(mr001_result_review_path)
    ...
    accepted_row = _row_by_id(rows, ACCEPTED_PRIOR_ROW_ID)
    ...
    if selected_row:
        selected_row_payload = {
            ...
            "comparison_boundary": WEAK_STRONG_COMPARISON_BOUNDARY,
            ...
            "claim_ceiling": (
                "weak_strong_conservation_comparison_scope_packet_only_no_"
                "weak_conservation_proof_no_strong_conservation_proof_no_"
    ...
    accepted_index = (
        row_ids.index(ACCEPTED_PRIOR_ROW_ID) if ACCEPTED_PRIOR_ROW_ID in row_ids else -1
    )
    acceptance_criteria = {
        "consumes_expected_mr001_result_review": result_review.get("review_id")
        == EXPECTED_MR001_RESULT_REVIEW_ID,
        "mr001_result_review_schema_expected": result_review.get("schema_id")
        == EXPECTED_MR001_RESULT_REVIEW_SCHEMA_ID,
        "mr001_result_review_outcome_expected": result_review.get("outcome_id")
        == EXPECTED_MR001_RESULT_REVIEW_OUTCOME,
        "mr001_result_review_classification_expected": result_review.get(
            "result_review_classification"
        )
        == EXPECTED_MR001_RESULT_REVIEW_CLASSIFICATION,
        "mr001_result_review_authorized_this_packet": result_review.get(
        ...
        == PRIOR_COMPLETED_FAMILIES,
        "accepted_mr001_before_this_packet": result_review.get(
            "accepted_mathematical_regularity_assumption_row"
        )
        == ACCEPTED_PRIOR_ROW_ID
        and result_review.get(
            "derivative_exchange_regular_boundary_assumption_reduction_accepted"
        )
        is True,
        "selects_only_mr002": selected_row is not None
        and result_review.get("next_mathematical_regularity_assumption_row")
        == SELECTED_ROW_ID
        and selected_index == accepted_index + 1,
        ...
        == CANDIDATE_REDUCTION_ROUTE,
        "distinguishes_weak_and_strong_conservation_scope": bool(
            WEAK_CONSERVATION_SCOPE and STRONG_CONSERVATION_SCOPE
        )
        and WEAK_CONSERVATION_SCOPE != STRONG_CONSERVATION_SCOPE,
        "comparison_boundaries_recorded": selected_row_payload.get(
            "comparison_scope_boundaries"
        )
        == COMPARISON_SCOPE_BOUNDARIES,
        ...
        "does_not_claim_either_conservation_form": result_review.get(
            "conservation_proved"
        )
        is False
    ...
    return {
        ...
        "consumes_qft_gr_derivative_exchange_regular_boundary_assumption_reduction_attempt_result_review_pointer": _ptr(
            mr001_result_review_path
        ),
        ...
        "accepted_prior_mathematical_regularity_assumption_row": ACCEPTED_PRIOR_ROW_ID,
        "accepted_mathematical_regularity_assumption_rows": [ACCEPTED_PRIOR_ROW_ID],
        ...
        "selected_row_is_repo_authoritative_next_row": (
            selected_index == accepted_index + 1
        ),
        "weak_strong_conservation_comparison_scope_assumption": selected_row_payload,
        "weak_strong_conservation_comparison_scope": SELECTED_ROW_OBJECT,
        "weak_conservation_scope": WEAK_CONSERVATION_SCOPE,
        "strong_conservation_scope": STRONG_CONSERVATION_SCOPE,
        "weak_strong_comparison_boundary": WEAK_STRONG_COMPARISON_BOUNDARY,
        "comparison_scope_boundaries": COMPARISON_SCOPE_BOUNDARIES,
        "weak_strong_conservation_comparison_scope_status_tokens": selected_status,
        ...
        "claim_ceiling": (
            "weak_strong_conservation_comparison_scope_assumption_reduction_"
            "packet_preparation_only_no_weak_or_strong_conservation_proof_no_"
            "conservation_proof_object_no_conservation_witness_no_state_or_"
        ...
        **nonclaims,
        ...
        if prepared
            ...
            "REMEDIATE_QFT_GR_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_"
            "ASSUMPTION_REDUCTION_PACKET"
        ),
        "selected_next_target_kind": NEXT_TARGET_KIND,
        "selected_route": (
            "qft_gr_weak_strong_conservation_comparison_scope_assumption_"
            "reduction_packet_result_review_after_preparation"
        ),
        ...
        "next_action_scope": (
            ...
            "ASSUMPTION_REDUCTION_PACKET_RESULT_ONLY_NO_CONSERVATION_WITNESS_"
            "OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This packet prepares only MR-ASSUMP-002 weak/strong conservation "
            "comparison scope reduction analysis. It distinguishes weak and "
            "strong conservation scopes for a future proof object, preserves "
            "insufficient_assumptions_for_conservation, and does not prove "
            "either conservation form, construct a conservation proof object "
            "or witness, claim state/source admissibility or Bianchi "
...
def write_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet(
    *,
    mr001_result_review_path: Path = DEFAULT_MR001_RESULT_REVIEW_PATH,
    mathematical_regularity_packet_path: Path = DEFAULT_MATHEMATICAL_REGULARITY_PACKET_PATH,
    ...
    payload = build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR MR-ASSUMP-002 weak/strong conservation "
            "comparison scope assumption-reduction packet."
        )
    )
    parser.add_argument(
        "--mr001-result-review",
        type=Path,
        default=DEFAULT_MR001_RESULT_REVIEW_PATH,
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ns = _parse_args(argv)
    mr001_result_review_path = (
        ns.mr001_result_review
        if ns.mr001_result_review.is_absolute()
        else (REPO_ROOT / ns.mr001_result_review)
    )
    mathematical_regularity_packet_path = (
    ...
    payload = write_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet(
        mr001_result_review_path=mr001_result_review_path,
        mathematical_regularity_packet_path=mathematical_regularity_packet_path,
        out=out,
    ...
    print(
        "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_report: "
