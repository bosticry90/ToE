...
SCHEMA_ID = (
    ...
    "EXECUTION_RESULT_REVIEW_20260628_v0"
)
...
NEXT_TARGET = "prepare_psi_A_matter_sector_exchange_theorem_linkage_obligation_closeout"
NEXT_TARGET_KIND = (
...
ACCEPTED_REVIEW_FINDINGS = [
    ...
    "T_psi policy used",
    "current definition J^alpha = q psibar gamma^alpha psi preserved",
...
def _review_criteria(execution: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "status": "accepted",
            "evidence": execution.get("matter_exchange_route_constructed"),
            "assessment": "The matter-side exchange route was constructed.",
        },
        {
            "row_id": "dirac_pair_used",
            "status": "accepted",
            "evidence": [DIRAC_EQUATION_SHAPE, ADJOINT_DIRAC_EQUATION_SHAPE],
            "assessment": "The Dirac and adjoint Dirac equations were used.",
        },
        {
            "row_id": "tpsi_policy_used",
            "status": "accepted",
            "evidence": T_PSI_POLICY,
            "assessment": "The selected T_psi policy is preserved.",
        },
        {
            "row_id": "current_definition_preserved",
            "status": "accepted",
            "evidence": CURRENT_DEFINITION,
            "assessment": "The current definition is unchanged.",
        },
        {
            "row_id": "compatibility_assumptions_preserved",
            "status": "accepted",
            "evidence": COMPATIBILITY_ASSUMPTIONS,
            "assessment": "Spin, tetrad, gamma, metric, domain, and boundary assumptions remain explicit.",
        },
        {
            "row_id": "watch_items_preserved",
            "status": "accepted",
            "evidence": WATCH_ITEMS,
            "assessment": "The same T_psi, F, J, sign, index, derivative, and domain watch items are preserved.",
        },
        {
...
def build_psi_A_matter_sector_exchange_theorem_linkage_attempt_from_dirac_pair_execution_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        "matter_exchange_route_constructed": (
            execution.get("matter_exchange_route_constructed") is True
            and execution.get("matter_exchange_derived") is True
        ...
        "dirac_pair_and_current_preserved": (
            execution.get("T_psi_policy") == T_PSI_POLICY
            and execution.get("dirac_equation_shape") == DIRAC_EQUATION_SHAPE
            and execution.get("adjoint_dirac_equation_shape")
            == ADJOINT_DIRAC_EQUATION_SHAPE
            and execution.get("current_definition") == CURRENT_DEFINITION
        ),
        "compatibility_and_watch_items_preserved": (
            execution.get("compatibility_assumptions") == COMPATIBILITY_ASSUMPTIONS
            and execution.get("watch_items") == WATCH_ITEMS
            and execution.get("domain_boundary_assumptions")
            == DOMAIN_BOUNDARY_ASSUMPTIONS
        ),
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_ATTEMPT_FROM_DIRAC_PAIR_EXECUTION_RESULT_REVIEW"
    )
    payload: dict[str, Any] = {
        ...
        "packet_id": PACKET_ID,
        "status": (
            "ACTIVE_PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_ATTEMPT_FROM_DIRAC_PAIR_EXECUTION_RESULT_REVIEW"
        ),
        ...
        if accepted
        else "PSI_A_MATTER_SECTOR_EXCHANGE_THEOREM_LINKAGE_ATTEMPT_FROM_DIRAC_PAIR_EXECUTION_RESULT_REVIEW_REQUIRES_REMEDIATION",
        ...
        "non_claim_boundary": (
            "This result review accepts only that the psi-A matter-sector "
            "exchange theorem-linkage attempt has been executed from the Dirac "
            "pair: matter gains exactly the energy-momentum transferred from "
            "the gauge field. It authorizes only closeout preparation. It does "
        ...
        "critical_gate_fail_conditions": [
            "fail to consume the psi-A matter exchange execution result",
            "fail to accept the executed matter exchange route",
            ...
            "fail to preserve compatibility and watch items",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PsiAMatterSectorExchangeTheoremLinkageAttemptFromDiracPairExecutionResultReview",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Review the executed psi-A matter-sector exchange theorem-linkage "
            "attempt from the Dirac pair."
        )
    )
