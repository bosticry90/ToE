...
def _load_origin_dataset(csv_path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray, str]:
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as handle:
        ...
        if reader.fieldnames is None:
            raise ValueError("CT-07 origin CSV has no header.")
        fields = set(reader.fieldnames)
        required = {"k_um_inv", "omega_over_2pi_kHz", "sigma_omega_over_2pi_kHz"}
        if not required.issubset(fields):
            raise ValueError(f"CT-07 origin CSV missing required columns: {sorted(required - fields)}")
        ...
        for row in reader:
            ...
            if not (math.isfinite(k) and math.isfinite(omega) and math.isfinite(sigma)):
                continue
            if sigma <= 0.0:
                raise ValueError("CT-07 sigma_omega_over_2pi_kHz must be > 0")
            rows.append((k, omega, sigma))

    if len(rows) < 6:
        raise ValueError("CT-07 requires at least 6 finite source rows.")
...
def _apply_lowk_slice(
    ...
    *,
    ...
    if not (0.0 < float(k_quantile) <= 1.0):
        raise ValueError("CT-07 k_quantile must be in (0, 1].")

    k_max = float(np.quantile(k_um_inv, float(k_quantile)))
    mask = k_um_inv <= k_max
    k_slice = k_um_inv[mask]
    omega_slice = omega_khz[mask]
    sigma_slice = sigma_khz[mask]
    if int(k_slice.size) < 6:
        raise ValueError("CT-07 low-k slice has fewer than 6 points; adjust k_quantile.")
    return k_slice, omega_slice, sigma_slice, k_max
...
def build_ct07_reports(
    *,
    ...
    k_slice, omega_slice, sigma_slice, k_max_um_inv = _apply_lowk_slice(
        k_um_inv,
        omega_khz,
        sigma_khz,
    ...
    c_s2_negative = float(c_s2_fit) * float(c_s2_scale_negative)
    omega_negative = _predict_omega(k_slice, c_s2=c_s2_negative, alpha=alpha_fit)
    ...
    params: dict[str, float | str | bool | int] = {
        ...
        "preprocessing_tag": "finite_filter_plus_k_sort_plus_lowk_quantile_slice",
        "observable_id": "omega_over_2pi_kHz_vs_k_um_inv",
        "fit_model": "omega2=c_s2*k^2+alpha*k^4",
        "k_quantile": float(k_quantile),
        "k_max_um_inv": float(k_max_um_inv),
        "n_slice_points": int(k_slice.size),
        "c_s2_scale_negative": float(c_s2_scale_negative),
    ...
    report = CT07ExternalAnchorLowKReport(
        ...
        config_tag="ct07_external_anchor_dispersion_lowk_slice_v0",
        regime_tag="CT07_External_Anchor_Dispersion_LowK_Slice",
        domain_tag="ct07_external_anchor_dispersion_lowk_domain_01",
        params=params,
        boundary="digitized_dispersion_lowk_slice",
        cases=[
            CT07ExternalAnchorLowKCase(
                case_id="POSITIVE",
            ...
            CT07ExternalAnchorLowKCase(
                ...
                c_s2_kHz2_um2=float(c_s2_negative),
                alpha_kHz2_um4=float(alpha_fit),
                c_s2_scale=float(c_s2_scale_negative),
...
def main() -> None:
    ...
    (out_dir / "ct07_reference_report.json").write_text(
        json.dumps(report.to_jsonable(), indent=2, sort_keys=True), encoding="utf-8"
    )
    (out_dir / "ct07_candidate_report.json").write_text(
