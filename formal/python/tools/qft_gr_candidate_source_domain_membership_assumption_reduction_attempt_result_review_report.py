    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The candidate source-domain membership reduction is accepted "
                "only as a bounded operator-domain membership contract, so the "
                "next operator-domain row may be prepared without claiming "
                "source admissibility or conservation."
            ),
        },
        {
            "target": (
        ...
        },
        {
            ...
            "reason": (
                "Accepting candidate source-domain membership does not "
                "authorize full source admissibility."
            ),
        },
        {
...
def build_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    attempt = _read_json(attempt_path)
    contract = attempt.get("candidate_source_domain_membership_reduction_contract", {})
    ...
    acceptance_criteria = {
        ...
        == CANDIDATE_SOURCE_DOMAIN_MEMBERSHIP_CONTRACT_ID,
        "candidate_source_object_preserved": attempt.get("candidate_source_object")
        == CANDIDATE_SOURCE_OBJECT
        and contract.get("candidate_source_object") == CANDIDATE_SOURCE_OBJECT,
        "operator_domain_membership_condition_preserved": attempt.get(
            "operator_domain_membership_condition"
        )
        == OPERATOR_DOMAIN_MEMBERSHIP_CONDITION
        and contract.get("operator_domain_membership_condition")
        == OPERATOR_DOMAIN_MEMBERSHIP_CONDITION,
        ...
        is True
        and attempt.get("candidate_source_domain_membership_assumption_obstruction_identified")
        is False
        and attempt.get("candidate_source_domain_membership_assumption_inconclusive")
        is False,
        ...
        "next_row_is_third_operator_domain_row": row_sequence[2]
        == NEXT_OPERATOR_DOMAIN_ASSUMPTION_ROW,
    ...
    return {
        ...
        "consumes_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt": EXPECTED_ATTEMPT_ID,
        "consumes_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "candidate_source_domain_membership_reduction_contract": contract,
        "accepted_contract_id": CANDIDATE_SOURCE_DOMAIN_MEMBERSHIP_CONTRACT_ID
        if accepted
        else "",
        "contract_acceptance_scope": (
            "bounded_candidate_source_domain_membership_contract_only_no_full_"
        ...
        "candidate_source_domain_membership_assumption_reduction_attempt_result_reviewed": accepted,
        "candidate_source_domain_membership_assumption_reduction_accepted": accepted,
        "candidate_source_domain_membership_assumption_reduction_rejected": not accepted,
        "candidate_source_domain_membership_assumption_reduced_pending_result_review_accepted": accepted,
        "candidate_source_domain_membership_assumption_discharged": False,
        "candidate_source_domain_membership_assumption_discharged_by_review": False,
        ...
        "next_action_scope": (
            "PREPARE_QFT_GR_STATE_EXPECTATION_DOMAIN_LINK_ASSUMPTION_REDUCTION_"
            "PACKET_ONLY_NO_SOURCE_ADMISSIBILITY_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the reduced OD-ASSUMP-002 "
            "candidate source-domain membership contract and authorizes "
            "preparation of the next operator-domain row, OD-ASSUMP-003 "
            "state-expectation domain link. It does not discharge the "
            "assumption as final, claim source admissibility, construct a "
            "conservation proof object or conservation witness, claim Bianchi "
...
def write_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_result_review(
        attempt_path=attempt_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_candidate_source_domain_membership_assumption_reduction_attempt_result_review_report: "
