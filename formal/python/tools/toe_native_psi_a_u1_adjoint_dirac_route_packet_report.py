...
from formal.python.tools.toe_native_psi_a_u1_psi_variation_dirac_route_packet_report import (
    ...
    DEFAULT_OUT as PSI_VARIATION_DIRAC_ROUTE_PATH,
    ...
    OUTCOME_ID as PSI_VARIATION_DIRAC_ROUTE_OUTCOME,
    PACKET_ID as PSI_VARIATION_DIRAC_ROUTE_PACKET_ID,
    PACKET_RESULT as PSI_VARIATION_DIRAC_ROUTE_RESULT,
    PSI_EQUATION_ROUTE,
    PSIBAR_VARIATION_ROUTE,
    SCHEMA_ID as PSI_VARIATION_DIRAC_ROUTE_SCHEMA_ID,
...
SCHEMA_ID = "TOE_NATIVE_PSI_A_U1_ADJOINT_DIRAC_ROUTE_PACKET_20260624_v0"
ARTIFACT_ID = SCHEMA_ID
PACKET_ID = "TOE_NATIVE_PSI_A_U1_ADJOINT_DIRAC_ROUTE_PACKET_v0"
OUTCOME_ID = (
...
NEXT_TARGET = "prepare_toe_native_psi_A_u1_current_conservation_from_dirac_pair_packet"
NEXT_TARGET_KIND = (
    "toe_native_psi_A_u1_current_conservation_from_dirac_pair_packet_preparation"
)

PRIMARY_VARIATION_VARIABLE = "psi"
ADJOINT_DERIVATIVE_POLICY = "D_mu psibar = nabla_mu psibar - i q A_mu psibar"
ADJOINT_VARIATION_ROUTE = (
    "delta_psi S_{psi A} -> i (D_mu psibar) gamma^mu + m psibar = 0"
)
ADJOINT_EQUATION_ROUTE = "i (D_mu psibar) gamma^mu + m psibar = 0"
...
PLAIN_MEANING = (
    "The project records the matched adjoint-side route needed before the "
    "current-conservation attempt; it does not yet prove that the current is "
    "conserved."
)
...
INDEXED_FUTURE_ROUTES = [
    {
        ...
        "route_shape": CURRENT_CONSERVATION_ROUTE_PREVIEW,
        "status": "indexed_requires_future_conservation_proof",
        "proof_claimed": False,
    },
    {
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "TOE_NATIVE_PSI_A_U1_ADJOINT_DIRAC_ROUTE_PACKET_20260624_v0.json"
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    / "formal"
...
def _review_criteria(psi_variation: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "row_id": "psi_variation_dirac_route_packet_consumed",
            "status": "accepted",
            "evidence": psi_variation.get("outcome_id"),
            "assessment": "The psi-variation Dirac route packet is the consumed input.",
        },
        {
        ...
        {
            "row_id": "psi_equation_route_preserved",
            "status": "accepted",
            "evidence": [PSIBAR_VARIATION_ROUTE, PSI_EQUATION_ROUTE],
            "assessment": "The psibar-variation psi equation route remains recorded.",
        },
        {
            "row_id": "opposite_sign_adjoint_derivative_indexed",
            "status": "accepted",
            "evidence": ADJOINT_DERIVATIVE_POLICY,
            "assessment": "The adjoint-side derivative carries the opposite U(1) gauge sign.",
        },
        {
            "row_id": "adjoint_equation_route_recorded",
            "status": "accepted",
            "evidence": [ADJOINT_VARIATION_ROUTE, LEFT_ACTING_ADJOINT_NOTATION],
            "assessment": "The bounded adjoint equation route is recorded as a route record only.",
        },
        {
            "row_id": "dirac_pair_conservation_attempt_selected_next",
            "status": "accepted",
            "evidence": NEXT_TARGET,
            "assessment": "The current-conservation-from-Dirac-pair packet is selected next.",
        },
        {
...
def _validation_policy() -> dict[str, Any]:
    return {
        ...
        "checkpoint_type": "toe_native_psi_A_u1_adjoint_dirac_route_packet",
...
def build_toe_native_psi_a_u1_adjoint_dirac_route_packet(
    *,
    psi_variation_dirac_route_path: Path = PSI_VARIATION_DIRAC_ROUTE_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    psi_variation = _read_json(psi_variation_dirac_route_path)
    review_criteria = _review_criteria(psi_variation)
    acceptance_criteria = {
        "consumes_expected_psi_variation_dirac_route_packet": (
            psi_variation.get("schema_id") == PSI_VARIATION_DIRAC_ROUTE_SCHEMA_ID
            and psi_variation.get("packet_id") == PSI_VARIATION_DIRAC_ROUTE_PACKET_ID
            and psi_variation.get("outcome_id") == PSI_VARIATION_DIRAC_ROUTE_OUTCOME
            and psi_variation.get("packet_result") == PSI_VARIATION_DIRAC_ROUTE_RESULT
            and psi_variation.get("selected_next_target") == CONSUMED_TARGET
            and psi_variation.get("accepted") is True
        ),
        "action_block_and_conventions_preserved": (
            psi_variation.get("action_block_statement") == ACTION_BLOCK_STATEMENT
            and psi_variation.get("covariant_derivative_policy")
            == COVARIANT_DERIVATIVE_POLICY
            and psi_variation.get("field_strength_policy") == FIELD_STRENGTH_POLICY
            and psi_variation.get("gauge_transformation_policy")
            == GAUGE_TRANSFORMATION_POLICY
        ),
        "psi_equation_route_preserved": (
            psi_variation.get("psibar_variation_route") == PSIBAR_VARIATION_ROUTE
            and psi_variation.get("psi_equation_route") == PSI_EQUATION_ROUTE
            and psi_variation.get("dirac_route_equation") == DIRAC_ROUTE_EQUATION
        ),
        "adjoint_route_recorded": (
            PRIMARY_VARIATION_VARIABLE == "psi"
            and ADJOINT_DERIVATIVE_POLICY.endswith("- i q A_mu psibar")
            and ADJOINT_VARIATION_ROUTE.endswith(ADJOINT_EQUATION_ROUTE)
            and LEFT_ACTING_ADJOINT_NOTATION.startswith("psibar")
        ),
        "future_routes_indexed_without_proof": (
            len(INDEXED_FUTURE_ROUTES) == 3
            and all(route["proof_claimed"] is False for route in INDEXED_FUTURE_ROUTES)
        ),
        "current_conservation_from_dirac_pair_selected_next": (
            NEXT_TARGET
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_TOE_NATIVE_PSI_A_U1_ADJOINT_DIRAC_ROUTE_PACKET"
    )
    ...
    return {
        ...
        "schema_id": SCHEMA_ID,
        "packet_id": PACKET_ID,
        "status": "ACTIVE_TOE_NATIVE_PSI_A_U1_ADJOINT_DIRAC_ROUTE_PACKET",
        "captured_at_utc": captured_at_utc,
        "prepared": accepted,
        "accepted": accepted,
        "outcome_id": OUTCOME_ID
        if accepted
        else "TOE_NATIVE_PSI_A_U1_ADJOINT_DIRAC_ROUTE_PACKET_REQUIRES_REMEDIATION",
        ...
        "left_acting_adjoint_notation": LEFT_ACTING_ADJOINT_NOTATION,
        "adjoint_equation_route_status": ADJOINT_EQUATION_ROUTE_STATUS,
        "current_conservation_from_pair_preview": CURRENT_CONSERVATION_FROM_PAIR_PREVIEW,
        "current_conservation_route_preview": CURRENT_CONSERVATION_ROUTE_PREVIEW,
        "sourced_maxwell_compatibility_route_preview": (
            SOURCED_MAXWELL_COMPATIBILITY_ROUTE_PREVIEW
        ),
        "exchange_route_preview": EXCHANGE_ROUTE_PREVIEW,
        "proof_pair_status": PROOF_PAIR_STATUS,
        ...
        "adjoint_dirac_route_packet_prepared": accepted,
        "psi_variation_adjoint_route_recorded": accepted,
        "adjoint_equation_route_recorded": accepted,
        "opposite_gauge_sign_adjoint_derivative_indexed": accepted,
        "left_acting_adjoint_notation_recorded": accepted,
        "psi_and_adjoint_pair_indexed": accepted,
        "current_conservation_from_dirac_pair_packet_selected": accepted,
        "current_conservation_from_dirac_pair_packet_preparation_authorized": accepted,
        ...
        "critical_gate_fail_conditions": [
            "treat adjoint route recording as an adjoint Dirac derivation closeout",
        ...
        "mathematical_statement": (
            "This packet records the bounded psi-variation adjoint route for the "
            "selected psi-A U(1) action block: D_mu psibar = nabla_mu psibar - "
            "i q A_mu psibar and delta_psi S_{psi A} -> i (D_mu psibar) "
            "gamma^mu + m psibar = 0. It selects the current-conservation-from-"
            "Dirac-pair packet next."
        ),
        "plain_meaning": PLAIN_MEANING,
        "non_claim_boundary": (
            "This is an adjoint Dirac route packet only; it records "
            "D_mu psibar = nabla_mu psibar - i q A_mu psibar and delta_psi "
            "S_{psi A} -> i (D_mu psibar) gamma^mu + m psibar = 0 as a "
            "bounded adjoint equation route. It records no current conservation "
            "proof, no sourced Maxwell closure, no stress-energy derivation, no "
            "exchange identity, no total conservation proof, no C_exchange "
            "closeout, no EM-QFT closure, no QFT-GR closure, no quantized "
            "electromagnetism, no anomaly analysis, no Phase 2 authorization, "
        ...
        "full_toeformal_aggregate_timed_out": False,
        "source_inputs": {
            "psi_variation_dirac_route_json": _ptr(psi_variation_dirac_route_path),
            "psi_variation_dirac_route_outcome": PSI_VARIATION_DIRAC_ROUTE_OUTCOME,
        },
        "generated_outputs": {
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Prepare the ToE-native psi-A U(1) adjoint Dirac route packet."
    )
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument(
        "--psi-variation-dirac-route",
        type=Path,
        default=PSI_VARIATION_DIRAC_ROUTE_PATH,
    )
    parser.add_argument("--captured-at-utc", default=DEFAULT_CAPTURED_AT_UTC)
    args = parser.parse_args(argv)

    payload = build_toe_native_psi_a_u1_adjoint_dirac_route_packet(
        psi_variation_dirac_route_path=args.psi_variation_dirac_route,
