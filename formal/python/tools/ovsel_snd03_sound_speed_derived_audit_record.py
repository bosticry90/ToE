"""CLI: write OV-SEL-SND-03 derived sound-speed audit lock."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    rec = ovsel_snd03_sound_speed_derived_audit_record(status_date=str(args.status_date))

    if not args.no_write:
        path = write_ovsel_snd03_lock(status_date=str(args.status_date))
        if args.report:
            print(f"Wrote lock: {path}")

    if args.report:
        print("OV-SEL-SND-03 report")
