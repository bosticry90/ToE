...
the strict v1 evidence assembler, and evaluates H_A--H_D only from metrics
...
CLASSIFIER_ID = (
    ...
    "INSTRUMENTED_R13_MECHANISM_EXPERIMENT_CLASSIFIER_v1"
)
SCRIPT_RELATIVE_PATH = (
    "formal/python/tools/dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_classifier_v1.py"
)
HYPOTHESES_A_TO_D = (
...
def _blocked(
    ...
    return {
        ...
        "assembler_id": raw_v1.ASSEMBLER_ID,
...
def _evaluate_h_b(
    ...
    if block_id not in raw_v1.BLOCK_IDS:
    ...
    if any(set(value) != set(raw_v1.BLOCK_IDS) for value in (loose_shares, tight_shares, neighbor_shares)):
...
def _classify_assembled(evidence: raw_v1.AssembledRawEvidence) -> dict[str, Any]:
    ...
    return {
        ...
        "scientific_input_closure_digest": (
            evidence.scientific_input_closure_digest
        ),
...
def classify_from_raw_payloads(
    ...
    try:
        assembled = raw_v1.assemble_raw_evidence(
            repo_root,
        )
    except raw_v1.RawEvidenceError as error:
