...
SCHEMA_ID = "THEOREM_GAP_QM_SUBTARGET_TRANCHE_20260411_v0"

DEFAULT_TRANCHE_PATH = REPO_ROOT / "formal" / "docs" / "release" / "THEOREM_GAP_QM_SUBTARGET_TRANCHE_20260411_v0.json"
...
R5_REPORT_PATH = REPO_ROOT / "formal" / "output" / "reports" / "theorem_gap_qm_rework_tranche_20260411_v0.json"
...
def build_report(captured_at_utc: str | None, tranche_path: Path) -> dict[str, Any]:
    ...
    r5_report = _read_json(R5_REPORT_PATH)
    ...
    r5_success_baseline = int(
        r5_report.get("objective_quality", {}).get("inputs", {}).get("target_row_success_count", 0) or 0
    )
    target_row_success_incremented = target_row_success_count > r5_success_baseline
    ...
    criteria = {
        "tranche_declares_qm_target_and_blocker": target_row == "ROW-PILLAR-QM-001" and blocker_class == "THEOREM_GAP",
        "tranche_declares_subproblem": bool(str(tranche.get("sub_problem", "")).strip()),
        "tranche_declares_measurable_success": measurable_success in {
            "THEOREM_GAP_DELTA_NE_0_OR_TARGET_ROW_SUCCESS_COUNT_INCREMENT",
            "THEOREM_GAP_DELTA_LT_0_OR_TARGET_ROW_SUCCESS_COUNT_INCREMENT",
        },
        "tranche_declares_expected_transition": expected_transition == "THEOREM_GAP_REDUCED_BY_AT_LEAST_ONE_OR_ROW_SUCCESS_INCREMENTED",
        "evidence_bundle_exists": evidence_bundle_exists,
        "linkage_entry_present_for_r6": len(linkage_entries) == 1,
    ...
    no_change_observed = (theorem_delta == 0) and (not target_row_success_incremented)
    no_change_route_satisfied = (
        ...
        or (
            (
                ...
                and bool(str(linkage_entry.get("failure_diagnosis", "")).strip())
            ...
            or fail_closed_route_ok
        )
    )

    objective_criteria = {
        "qm_subtarget_success_observed": success_observed,
        "theorem_gap_delta_changed": theorem_delta != 0,
    ...
    return {
        ...
        "objective_quality": {
            ...
            "inputs": {
                ...
                "r5_target_row_success_baseline": r5_success_baseline,
            ...
            "summary": {
                ...
                "next_action": (
                    ...
                    else "CONTINUE_QM_SUBTARGET_BLOCKER_MOVING_REWORK"
                ),
            },
        },
        "summary": {
            ...
            "next_action": (
                "CONTINUE_QM_SUBTARGET_BLOCKER_MOVING_REWORK"
                if all(criteria.values())
                else "RESTORE_R6_TRANCHE_CONTRACT"
            ),
        },
        "source_bundle": {
            ...
            "r5_report": _ptr(R5_REPORT_PATH),
            "ledger": _ptr(LEDGER_PATH),
        },
        "non_claim_boundary": "Repository-local QM sub-target theorem-gap tranche artifact only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate theorem-gap QM sub-target tranche report.")
    parser.add_argument(
        "--tranche",
        type=Path,
        default=DEFAULT_TRANCHE_PATH,
        help="Path to QM subtarget tranche declaration JSON.",
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "theorem_gap_qm_subtarget_tranche_20260411_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"theorem_gap_qm_subtarget_tranche_report: phase_status={payload['summary']['phase_status']} out={out}")
    return 0
