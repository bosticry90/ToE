...
SCHEMA_ID = "SHARED_MODEL_CLASS_POST_REFINEMENT_DECISION_REPORT_20260412_v0"

DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "SHARED_MODEL_CLASS_POST_REFINEMENT_DECISION_20260412_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    refinement_path = REPO_ROOT / str(
        required_inputs.get("shared_model_class_binding_refinement_report", "")
    ).strip()
    ...
    required_proposal_outcome = str(decision_policy.get("required_proposal_outcome", "")).strip()
    ...
    default_outcome = str(
        decision_contract.get(
            "default_outcome",
            "HOLD_SHARED_MODEL_CLASS_AS_EXTERNALLY_COMPARABLE_BUT_NOT_PROBE_READY",
        )
    ).strip()

    if not preconditions_ok or path_falsified:
        terminal_outcome = "PATH_FALSIFIED"
        next_action = "CLOSE_SHARED_MODEL_CLASS_LANE_AND_REASSESS"
    elif higher_level_comparator_policy_required:
        terminal_outcome = "REQUIRES_HIGHER_LEVEL_COMPARATOR_POLICY"
        next_action = "OPEN_HIGHER_LEVEL_COMPARATOR_POLICY_LAYER"
        ...
        next_action = "OPEN_ONE_FINAL_BOUNDED_REFINEMENT_LAYER"
        ...
        next_action = "MAINTAIN_LIMITED_HOLD_AND_WAIT_FOR_STRONGER_POLICY_OR_EVIDENCE"
    else:
        terminal_outcome = "AUTHORIZE_ONE_MORE_BOUNDED_REFINEMENT"
        next_action = "OPEN_ONE_FINAL_BOUNDED_REFINEMENT_LAYER"
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_SHARED_MODEL_CLASS_POST_REFINEMENT_DECISION_OUTCOME",
            "no_loop_rule_declared": str(decision_contract.get("no_loop_rule", "")).strip()
            == "ONE_SHARED_MODEL_CLASS_POST_REFINEMENT_DECISION_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "source_bundle": {
            ...
            "shared_model_class_binding_refinement_report": _ptr(refinement_path),
        ...
        "non_claim_boundary": "Repository-local shared model-class post-refinement decision report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate shared model-class post-refinement decision report.")
    parser.add_argument("--declaration", type=Path, default=DEFAULT_DECLARATION_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "shared_model_class_post_refinement_decision_20260412_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "shared_model_class_post_refinement_decision_report: "
