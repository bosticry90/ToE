...
SCHEMA_ID = (
    "QFT_GR_CONSERVATION_FORM_SCOPE_ASSUMPTION_REDUCTION_PACKET_"
    "RESULT_REVIEW_20260527_v0"
)
REVIEW_ID = "QFT_GR_CONSERVATION_FORM_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_v0"
OUTCOME_ID = (
    "QFT_GR_CONSERVATION_FORM_SCOPE_ASSUMPTION_REDUCTION_PACKET_"
    "RESULT_REVIEW_ACCEPTS_PACKET_AND_AUTHORIZES_BOUNDED_REDUCTION_ATTEMPT_ONLY"
)
RESULT_REVIEW_CLASSIFICATION = (
    "qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_"
    "accepts_packet_and_authorizes_bounded_reduction_attempt_only"
)
CONSUMED_TARGET = EXPECTED_REVIEW_TARGET
NEXT_TARGET = "execute_qft_gr_conservation_form_scope_assumption_reduction_attempt"

AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS = [
    "qft_gr_conservation_form_scope_assumption_reduced_pending_result_review",
    (
        "qft_gr_conservation_form_scope_assumption_obstruction_identified_"
        "requires_refinement"
    ),
    (
        "qft_gr_conservation_form_scope_assumption_inconclusive_requires_"
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_CONSERVATION_FORM_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_20260527_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The OD-ASSUMP-005 packet is accepted as preparation-only, so "
                "the next bounded step may attempt the conservation-form-scope "
                "reduction without constructing a conservation proof object, "
                "conservation witness, source admissibility claim, or QFT-GR "
                "seam closure."
            ),
        },
        {
            ...
            "reason": (
                "Accepting the conservation-form-scope packet does not "
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Accepting a conservation-form-scope packet does not authorize "
...
def build_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    packet = _read_json(packet_path)
    selected_assumption = packet.get("conservation_form_scope_assumption", {})
    selected_status = packet.get("conservation_form_scope_status_tokens", [])
    prior_rows = packet.get("prior_accepted_operator_domain_assumption_rows", [])
    candidate_next_targets = _candidate_next_targets()

    acceptance_criteria = {
        "consumes_expected_conservation_form_scope_packet": packet.get("packet_id")
        == EXPECTED_PACKET_ID,
        ...
        "confirms_selected_row005": packet.get("selected_operator_domain_assumption_row")
        == SELECTED_ROW_ID
        and selected_assumption.get("assumption_id") == SELECTED_ROW_ID,
        "conservation_form_options_confirmed": packet.get("conservation_form_options")
        == CONSERVATION_FORM_OPTIONS
        and selected_assumption.get("conservation_form_options")
        == CONSERVATION_FORM_OPTIONS,
        "selected_bounded_conservation_form_confirmed": packet.get(
            "selected_bounded_conservation_form"
        )
        == SELECTED_BOUNDED_CONSERVATION_FORM
        ...
        and packet.get("conservation_form_scope_assumption_reduction_analysis_prepared")
        is True
        and packet.get("assumptions_reduced_or_discharged_by_preparation") is False,
        "no_conservation_form_reduced_by_review_alone": packet.get(
            "conservation_form_scope_assumption_discharged"
        )
        is False
        and packet.get("conservation_form_scope_claimed_as_conservation_proof")
        is False
    ...
    return {
        ...
        "consumes_qft_gr_conservation_form_scope_assumption_reduction_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_conservation_form_scope_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        ...
        "prior_rows001_002_003_004_remain_accepted": accepted,
        "selected_operator_domain_assumption_row": SELECTED_ROW_ID,
        "conservation_form_scope_assumption": selected_assumption,
        ...
        "conservation_form_scope_packet_accepted_by_review": accepted,
        "conservation_form_scope_analysis_accepted": accepted,
        "conservation_form_scope_assumption_reduced_by_review": False,
        ...
        "authorized_attempt_scope": (
            "conservation_form_scope_assumption_reduction_attempt_only_no_"
        ...
        "selected_next_target_kind": (
            "qft_gr_conservation_form_scope_assumption_reduction_attempt_execution"
        ),
        "selected_route": (
        ...
        "non_claim_boundary": (
            "This result review accepts only the OD-ASSUMP-005 "
            "conservation-form-scope packet and authorizes one bounded "
...
def write_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_conservation_form_scope_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_conservation_form_scope_assumption_reduction_packet_result_review_report: "
