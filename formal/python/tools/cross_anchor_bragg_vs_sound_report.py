...
def _canonical_bragg_pairing_key(condition_key: str) -> str:
    """Map report-local Bragg condition keys to OV-BR-SND-03 bragg_key namespace."""
    k = (condition_key or "").strip()
    if k == "condition_A":
        return "br04a_conditionA"
    if k == "condition_B":
        return "br04b_conditionB"
    return k
...
def _load_bragg_sound_pairing_map(
    *,
    ...
    for t in tuples_raw:
        ...
        # Back-compat: early versions used file paths, keyed by condition_key.
        ck = t.get("bragg_condition_key")
        sl = t.get("sound_lock_path") or t.get("sound_lock_relpath")
        if isinstance(ck, str) and isinstance(sl, str):
            ck = ck.strip()
        ...
        # v4 schema (linted): uses allowed keys bragg_key + sound_key.
        bragg_key = t.get("bragg_key")
        sound_key = t.get("sound_key")
        if not isinstance(bragg_key, str) or not isinstance(sound_key, str):
            continue
        bragg_key = bragg_key.strip()
        sound_key = sound_key.strip()
        if not bragg_key or not sound_key:
            continue

        # Map sound_key -> the lock relpath(s) used by this report.
        sound_lock_rel: str | None = None
        if sound_key == "snd02_single":
            sound_lock_rel = "formal/markdown/locks/observables/OV-SND-02_sound_speed_from_propagation.md"
        elif sound_key == "snd02b_seriesB":
            sound_lock_rel = "formal/markdown/locks/observables/OV-SND-02B_sound_speed_from_propagation_seriesB.md"

        if sound_lock_rel:
            out.setdefault(bragg_key, set()).add(_norm_relpath(sound_lock_rel))
...
def build_report(
    *,
    ...
    for b in bragg:
        bkey = b.key or ""
        bkey_map = _canonical_bragg_pairing_key(bkey)
        for s in sound:
            ...
            # OV-BR-SND-03 audit reports comparability.status="established" when at least
            # one explicitly paired row is computed and passes.
            if comparability_status not in {"present", "established"}:
            ...
            allowed_sound_locks = None
            if bkey_map:
                allowed_sound_locks = pairing_map.get(bkey_map)
            if not allowed_sound_locks and bkey:
                allowed_sound_locks = pairing_map.get(bkey)
            ...
            if reasons:
                suppressed.append(
                    PairingDecision(
                        bragg_condition_key=bkey_map or "(missing)",
