...
from formal.python.tools.qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_report import (
...
REVIEW_ID = "QFT_GR_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_ASSUMPTION_REDUCTION_ATTEMPT_RESULT_REVIEW_v0"
OUTCOME_ID = (
    "QFT_GR_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_ASSUMPTION_REDUCTION_"
...
NEXT_ROW_ID = "MR-ASSUMP-003-distributional_pairing_regular_domain"
NEXT_ROW_OBJECT = DISTRIBUTIONAL_PAIRING_REGULAR_DOMAIN
    ...
    return [
        {
            ...
            "reason": (
                ...
                "places MR-ASSUMP-003-distributional_pairing_regular_domain "
                "after the accepted bounded MR-ASSUMP-002 reduction."
            ),
        },
        {
            "target": (
            ...
            "decision": "deferred",
            "reason": (
                "MR-ASSUMP-004 remains after MR-ASSUMP-003 in the packet row "
                "inventory."
            ),
        },
        {
            "target": "discharge_qft_gr_weak_strong_conservation_comparison_scope_assumption",
            "decision": "not_authorized",
            "reason": (
                "This review accepts a bounded reduction only, not global weak/strong "
                "conservation comparison-scope discharge."
            ),
        },
        {
            ...
            "decision": "not_authorized",
            "reason": (
                "Accepting MR-ASSUMP-002 does not discharge the mathematical "
                "regularity family."
            ),
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "Weak/strong scope regularity is not a state-admissibility claim.",
        },
        {
            "target": "claim_qft_gr_source_admissibility",
            "decision": "not_authorized",
            "reason": "Weak/strong scope regularity is not source admissibility.",
        },
        {
...
def _global_nonclaims() -> dict[str, bool]:
    return {
        ...
        "weak_strong_conservation_comparison_scope_assumption_discharged_by_review": False,
        "weak_strong_conservation_comparison_scope_reduced_or_discharged_by_review": False,
        ...
        "weak_strong_conservation_scope_globally_solved_by_review": False,
...
def _acceptance_criteria(
    ...
    contract = attempt.get("weak_strong_conservation_comparison_scope_reduction_contract", {})
    rows = packet.get("candidate_reducible_assumptions", [])
    row_ids = [row.get("assumption_id") for row in rows]
    selected_index = row_ids.index("MR-ASSUMP-002-weak_strong_conservation_comparison_scope")
    ...
    return {
        ...
        "confirms_mr_assump_002_reduced_pending_review": attempt.get(
            "weak_strong_conservation_comparison_scope_assumption_reduced_pending_result_review"
        )
        is True
        and attempt.get("weak_strong_conservation_comparison_scope_assumption_reduced_by_attempt")
        is True
        and attempt.get("weak_strong_conservation_comparison_scope_assumption_obstruction_identified")
        is False
        and attempt.get("weak_strong_conservation_comparison_scope_assumption_inconclusive")
        is False,
        "preserves_bounded_weak_strong_comparison_contract": contract.get("contract_id")
        == WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_ID
        and contract.get("contract_status")
        == BOUNDED_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_STATUS
        and contract.get("assumption_id")
        == "MR-ASSUMP-002-weak_strong_conservation_comparison_scope"
        and contract.get("regularity_condition")
        == WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE,
        "does_not_globally_solve_weak_strong_scope": attempt.get(
            "weak_strong_conservation_scope_globally_solved"
        )
        is False
        and attempt.get("weak_strong_conservation_scope_globally_solved_by_attempt")
        is False,
...
def build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review(
    *,
    ...
    return {
        ...
        "consumes_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt": (
            EXPECTED_ATTEMPT_ID
        ),
        "consumes_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "weak_strong_conservation_comparison_scope": WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE,
        "weak_strong_conservation_comparison_scope_reduction_contract": attempt.get(
            "weak_strong_conservation_comparison_scope_reduction_contract"
        ),
        "accepted_contract_id": WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_ID,
        "bounded_weak_strong_conservation_comparison_scope_contract_status": (
            BOUNDED_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_CONTRACT_STATUS
        ),
        "weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_reviewed": (
            accepted
        ),
        "weak_strong_conservation_comparison_scope_assumption_reduction_accepted": accepted,
        "weak_strong_conservation_comparison_scope_assumption_reduction_rejected": (
            not accepted
        ),
        "weak_strong_conservation_comparison_scope_assumption_reduced_pending_result_review_accepted": (
            accepted
        ),
        ...
        "distributional_pairing_regular_domain": NEXT_ROW_OBJECT,
        "distributional_pairing_regular_domain_required_future_proof_object": (
            NEXT_ROW_REQUIRED_FUTURE_PROOF_OBJECT
        ),
        ...
        **nonclaims,
        ...
        "claim_ceiling": [
            "no global weak/strong conservation-comparison scope solve",
        ...
        "failure_mode_if_unresolved": (
            ...
            "distributional-pairing regular-domain reduction can be prepared"
        ),
    }


def write_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    packet_path: Path = DEFAULT_PACKET_PATH,
    ...
    payload = build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "comparison-scope assumption-reduction attempt result review."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review(
        attempt_path=attempt_path,
        packet_path=packet_path,
        out=out,
    ...
    print(
        "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_attempt_result_review_report: "
