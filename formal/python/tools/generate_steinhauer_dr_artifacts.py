"""Generate frozen DR window artifacts from the Steinhauer Fig. 3a digitization.

This is a helper for regenerating (or extending) the evidence artifacts stored under:
  formal/external_evidence/bec_bragg_steinhauer_2001/

Design goals:
- Deterministic: no RNG, stable ordering.
- Evidence-friendly: emits the same fields the repository's frozen artifacts use.
- Minimal dependencies: uses the in-repo DR/DQ/BR helpers, plus stdlib.

This script does NOT overwrite existing files unless --force is provided.

Typical usage (example):
    python -m formal.python.tools.generate_steinhauer_dr_artifacts window-counts

    python -m formal.python.tools.generate_steinhauer_dr_artifacts generate \
    --kmax-um-inv 2.5 \
    --out-linear formal/external_evidence/bec_bragg_steinhauer_2001/dr04c_fit_artifact.json \
    --out-curved formal/external_evidence/bec_bragg_steinhauer_2001/dr04c_fit_artifact_curved.json \
        --tag DR04c_Steinhauer_Fig3a_lowk_linearization_kmax_2p5
"""
...
def _ensure_repo_root_on_syspath() -> None:
    # When executing this file as a script (python path/to/file.py), Python puts the
    # script's directory on sys.path, not necessarily the repo root. Since this repo
    # relies on namespace-package imports like `formal.python.*`, we add the repo root.
...
REPO_REL_CSV = Path("formal/external_evidence/bec_bragg_steinhauer_2001/omega_k_data.csv")


def _load_fig3a_rows(csv_path: Path) -> list[dict[str, str]]:
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        rows = [r for r in reader if (r.get("figure", "").strip() == "Fig3a")]
    if not rows:
        raise RuntimeError(f"No Fig3a rows found in {csv_path}")
    return rows


def _sample_kw_for_kmax(csv_path: Path, *, kmax_um_inv: float) -> tuple[tuple[float, float], ...]:
    rows = _load_fig3a_rows(csv_path)
...
def _metric_1d_fields_from_dr_fit(dr01_fit: DR01Fit1D) -> dict[str, float]:
    ...
    # Emit the same scalar fields the existing frozen artifacts use.
    return {
...
def cmd_window_counts(args: argparse.Namespace) -> None:
    ...
    print(f"Fig3a points: N={len(ks)}")
    print("k values (um^-1):")
    print(", ".join(f"{k:.6g}" for k in ks))

    thresholds = [1.6, 1.65, 1.7, 1.75, 1.8, 2.1, 2.5, 3.2]
    print("\nCounts by kmax:")
    for t in thresholds:
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    sample_kw = _sample_kw_for_kmax(csv_path, kmax_um_inv=float(args.kmax_um_inv))
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_counts = sub.add_parser("window-counts", help="Print Fig3a k-values and counts by kmax")
    p_counts.add_argument("--csv", default=str(REPO_REL_CSV))
    ...
    p_gen.add_argument("--csv", default=str(REPO_REL_CSV))
    ...
    p_gen.add_argument(
        "--source-ref",
        default="Steinhauer PRL 88, 120407 (2002), Fig 3a; digitized from arXiv 0111438v1; formal/external_evidence/bec_bragg_steinhauer_2001/omega_k_data.csv",
    )
    p_gen.add_argument(
        "--source-ref-curved",
        default=None,
        help="Optional curved artifact source_ref override (defaults to --source-ref)",
    )
    p_gen.add_argument(
        "--fit-method-tag",
        default="low-k through-origin omega~=c_s*k; u fixed to 0",
    )
    p_gen.add_argument(
        "--fit-method-tag-curved",
        default="low-k curved omega/k = c0 + beta*k^2; u fixed to 0",
    )
