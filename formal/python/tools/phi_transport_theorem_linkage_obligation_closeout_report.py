...
from formal.python.tools.phi_transport_theorem_linkage_attempt_from_standalone_phi_transport_route_execution_result_review_report import (
...
SCHEMA_ID = "PHI_TRANSPORT_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_20260701_v0"
ARTIFACT_ID = SCHEMA_ID
PACKET_ID = "PHI_TRANSPORT_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_v0"
...
NEXT_TARGET = "review_phi_transport_theorem_linkage_obligation_closeout_result"
NEXT_TARGET_KIND = "phi_transport_theorem_linkage_obligation_closeout_result_review"
...
LOCAL_CLOSEOUT_ROUTE = COMPONENTWISE_ZERO_ROUTE
...
def _consumed_review_valid(review: dict[str, Any]) -> bool:
    return (
        ...
        and review.get("C_transport_tuple_zero") == C_TRANSPORT_TUPLE_ZERO
        and review.get("componentwise_zero_route") == COMPONENTWISE_ZERO_ROUTE
        and review.get("executed_componentwise_route") == EXECUTED_COMPONENTWISE_ROUTE
        and review.get("C_transport_phi_zero_derived") is True
        and review.get("C_transport_phi_linkage_constructed") is True
...
def _validation_policy() -> dict[str, Any]:
    return {
        "checkpoint_type": "phi_transport_theorem_linkage_obligation_closeout",
...
def build_phi_transport_theorem_linkage_obligation_closeout(
    *,
    ...
    acceptance_criteria = {
        ...
        "local_route_preserved": (
            review.get("transport_constraint_form") == TRANSPORT_CONSTRAINT_FORM
            and review.get("transport_constraint_equation")
            == TRANSPORT_CONSTRAINT_EQUATION
            and review.get("target_conclusion") == TARGET_CONCLUSION
            and review.get("C_transport_tuple_zero") == C_TRANSPORT_TUPLE_ZERO
        ),
        "componentwise_transport_zero_reviewed": (
            review.get("transport_action_variation_zero_component")
            == TRANSPORT_ACTION_VARIATION_ZERO_COMPONENT
            and review.get("transport_variation_bridge_zero_component")
            == TRANSPORT_VARIATION_BRIDGE_ZERO_COMPONENT
            and review.get("transport_bridge_source_zero_component")
            == TRANSPORT_BRIDGE_SOURCE_ZERO_COMPONENT
            and review.get("transport_source_residual_zero_component")
            == TRANSPORT_SOURCE_RESIDUAL_ZERO_COMPONENT
            and review.get("transport_residual_regime_zero_component")
            == TRANSPORT_RESIDUAL_REGIME_ZERO_COMPONENT
        ),
        "constructed_and_reviewed": (
            review.get("C_transport_phi_zero_derived") is True
            and review.get("C_transport_phi_linkage_constructed") is True
            and review.get("reviewed") is True
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PHI_TRANSPORT_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT"
    )
    payload: dict[str, Any] = {
        "artifact_id": ARTIFACT_ID,
        "schema_id": SCHEMA_ID,
        "packet_id": PACKET_ID,
        "status": "ACTIVE_PHI_TRANSPORT_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT",
        ...
        if accepted
        else "PHI_TRANSPORT_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_REQUIRES_REMEDIATION",
        ...
        "route_kind": "standalone_phi_transport_action_to_regime_transport_match",
        ...
        "local_phi_transport_theorem_linkage_obligation_closed": accepted,
        "phi_transport_theorem_linkage_obligation_locally_closed": accepted,
        "phi_transport_theorem_linkage_obligation_discharged": accepted,
        ...
        "componentwise_zero_route_reviewed": accepted,
        ...
        "non_claim_boundary": (
            "This closeout records only that the local C_transport^phi "
            "theorem-linkage obligation is closed by the standalone "
            "action-to-regime componentwise transport match: "
            ...
            "claims no phi-sector closure, no scalar/QFT closure, no QFT-GR "
            "closure, no EM-QFT closure, no seam closure, no general C_k "
            "closure, no C_k promotion, no action embedding, no variation, "
            "no empirical validation, and no master-action promotion."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume prepare_phi_transport_theorem_linkage_obligation_closeout",
            "fail to close the local C_transport^phi theorem-linkage obligation",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PhiTransportTheoremLinkageObligationCloseout",
    ...
    payload["phi_transport_theorem_linkage_obligation_discharged"] = accepted
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Close out the local standalone phi-transport theorem-linkage obligation."
    )
    ...
    payload = build_phi_transport_theorem_linkage_obligation_closeout(
        ...
        json.dumps(
            {
                ...
                "phi_transport_theorem_linkage_obligation_locally_closed": payload[
