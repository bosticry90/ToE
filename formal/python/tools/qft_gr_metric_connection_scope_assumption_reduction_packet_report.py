...
SCHEMA_ID = "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_PACKET_20260527_v0"
PACKET_ID = "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_PACKET_v0"
OUTCOME_ID = (
...
PRIOR_ACCEPTED_ROW005 = "OD-ASSUMP-005-conservation_form_scope"
SELECTED_ROW_ID = NEXT_OPERATOR_DOMAIN_ASSUMPTION_ROW
NEXT_TARGET = "review_qft_gr_metric_connection_scope_assumption_reduction_packet_result"
METRIC_CONNECTION_SCOPE_OBJECT = (
    "bounded_metric_connection_scope_for_selected_operator_domain"
)
BOUNDED_GEOMETRY_DOMAIN = "selected_operator_domain_bounded_geometry_domain"
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_PACKET_20260527_v0.json"
)
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The metric/connection-scope packet must be reviewed before "
                "any bounded reduction attempt, conservation proof object, "
                "Bianchi compatibility claim, source admissibility question, "
                "or seam closure action."
            ),
        },
        {
            "target": "execute_qft_gr_metric_connection_scope_assumption_reduction_attempt",
            "decision": "deferred",
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "Metric/connection scope preparation does not authorize source admissibility.",
        },
        {
            "target": "claim_qft_gr_bianchi_compatibility",
            "decision": "not_authorized",
            "reason": "A bounded metric/connection scope is below Bianchi compatibility.",
        },
        {
...
def build_qft_gr_metric_connection_scope_assumption_reduction_packet(
    *,
    ...
    if selected_row:
        selected_row_payload = {
            ...
            "claim_ceiling": (
                "metric_connection_scope_packet_only_no_conservation_proof_"
                "object_no_conservation_witness_no_source_admissibility_no_"
    ...
    acceptance_criteria = {
        ...
        and result_review.get("completed_operator_domain_row") == PRIOR_ACCEPTED_ROW005,
        "prior_conservation_form_scope_contract_accepted": result_review.get(
            "accepted_contract_id"
        )
        == CONSERVATION_FORM_SCOPE_CONTRACT_ID,
        "prior_rows001_002_003_004_preserved": result_review.get(
        ...
        "selects_only_metric_connection_scope_row": selected_row is not None
        ...
        "metric_connection_scope_object_recorded": selected_row_payload.get(
            "metric_connection_scope_object"
        )
        == METRIC_CONNECTION_SCOPE_OBJECT,
        "bounded_geometry_domain_recorded": selected_row_payload.get(
            "bounded_geometry_domain"
        )
        == BOUNDED_GEOMETRY_DOMAIN,
        "connection_compatibility_condition_recorded": selected_row_payload.get(
            "connection_compatibility_condition"
        )
        == CONNECTION_COMPATIBILITY_CONDITION,
        ...
        "does_not_prove_conservation": result_review.get("conservation_proved")
        is False
        and result_review.get("covariant_conservation_statement_proved") is False,
        ...
        is False
        and result_review.get("metric_connection_scope_treated_as_bianchi_compatibility")
        is False,
    ...
    return {
        ...
        if prepared
        else "QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_PACKET_BLOCKED",
        "packet_classification": PACKET_CLASSIFICATION,
        "packet_classification_count": 1 if prepared else 0,
        "consumes_qft_gr_conservation_form_scope_assumption_reduction_attempt_result_review": EXPECTED_RESULT_REVIEW_ID,
        "consumes_qft_gr_conservation_form_scope_assumption_reduction_attempt_result_review_pointer": _ptr(
            result_review_path
        ),
        ...
        "metric_connection_scope_assumption_reduction_analysis_prepared": prepared,
        "metric_connection_scope_assumption": selected_row_payload,
        "metric_connection_scope_object": METRIC_CONNECTION_SCOPE_OBJECT,
        "bounded_geometry_domain": BOUNDED_GEOMETRY_DOMAIN,
        "connection_compatibility_condition": CONNECTION_COMPATIBILITY_CONDITION,
        "required_future_proof_object": REQUIRED_FUTURE_PROOF_OBJECT,
        ...
        "claim_ceiling": (
            "metric_connection_scope_assumption_reduction_packet_only_no_"
        ...
        if prepared
        else "REMEDIATE_QFT_GR_METRIC_CONNECTION_SCOPE_ASSUMPTION_REDUCTION_PACKET",
        ...
        "non_claim_boundary": (
            "This packet prepares only OD-ASSUMP-006 metric/connection-scope "
            "reduction analysis and records the bounded geometry domain and "
            "connection compatibility condition needed for future proof-object "
            "work. It does not prove conservation, construct a conservation "
...
def write_qft_gr_metric_connection_scope_assumption_reduction_packet(
    *,
    result_review_path: Path = DEFAULT_RESULT_REVIEW_PATH,
    operator_domain_packet_path: Path = DEFAULT_OPERATOR_DOMAIN_PACKET_PATH,
    ...
    payload = build_qft_gr_metric_connection_scope_assumption_reduction_packet(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR metric/connection-scope assumption-reduction packet."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_metric_connection_scope_assumption_reduction_packet(
        result_review_path=result_review_path,
        operator_domain_packet_path=operator_domain_packet_path,
        out=out,
    ...
    print(
        "qft_gr_metric_connection_scope_assumption_reduction_packet_report: "
        f"prepared={payload['prepared']} row={payload['selected_operator_domain_assumption_row']} "
        f"scope={payload['metric_connection_scope_object']} "
