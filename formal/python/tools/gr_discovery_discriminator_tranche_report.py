...
SCHEMA_ID = "GR_DISCOVERY_DISCRIMINATOR_TRANCHE_REPORT_20260411_v0"
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    queue_rescoring_path = REPO_ROOT / str(required_inputs.get("queue_rescoring_report", "")).strip()
    contract_path = REPO_ROOT / str(required_inputs.get("discovery_tranche_ruling_contract", "")).strip()

    tranche = _read_json(tranche_path)
    queue_rescoring = _read_json(queue_rescoring_path)
    contract = _read_json(contract_path)

    required_selected_route = str(execution_policy.get("required_selected_route", "")).strip()
    required_rank3_candidate = str(execution_policy.get("required_rank3_candidate", "")).strip()

    rescoring_summary = dict(queue_rescoring.get("summary", {}))
    selected_route = str(rescoring_summary.get("selected_next_route", "")).strip()
    rank3_candidate = str(rescoring_summary.get("rank3_candidate", "")).strip()
    ...
    next_route_alignment = (
        selected_route == required_selected_route
        and rank3_candidate == required_rank3_candidate
        and target_row == required_rank3_candidate
    )
    ...
    return {
        ...
        "summary": {
            ...
            "rank3_candidate": rank3_candidate,
        ...
        "source_bundle": {
            ...
            "queue_rescoring_report": _ptr(queue_rescoring_path),
            "discovery_tranche_ruling_contract": _ptr(contract_path),
        },
        "non_claim_boundary": "Repository-local GR discovery tranche execution report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate GR discovery discriminator tranche execution report.")
    parser.add_argument("--declaration", type=Path, default=DEFAULT_DECLARATION_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "gr_discovery_discriminator_tranche_report_20260411_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "gr_discovery_discriminator_tranche_report: "
