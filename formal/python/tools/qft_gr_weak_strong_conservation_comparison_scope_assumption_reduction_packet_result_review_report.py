...
AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS = [
    ...
    "qft_gr_weak_strong_conservation_comparison_scope_assumption_obstruction_identified_requires_refinement",
    "qft_gr_weak_strong_conservation_comparison_scope_assumption_inconclusive_requires_assumption_reduction",
]
    ...
    return [
        {
            ...
            "reason": (
                ...
                "the MR-ASSUMP-002 weak/strong comparison-scope reduction attempt. "
                "This review does not execute that attempt."
            ),
        },
        {
        ...
        {
            "target": "prepare_qft_gr_distributional_pairing_regular_domain_assumption_reduction_packet",
            "decision": "not_authorized_current_row",
            "reason": "MR-ASSUMP-003 remains downstream; only MR-ASSUMP-002 is authorized.",
        },
        {
            "target": "prepare_qft_gr_limit_interchange_regularization_boundary_assumption_reduction_packet",
            "decision": "not_authorized_current_row",
            "reason": "MR-ASSUMP-004 remains downstream; only MR-ASSUMP-002 is authorized.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "Weak conservation is not proved by packet result review.",
        },
        {
            "target": "prove_qft_gr_strong_conservation",
            "decision": "not_authorized",
            "reason": "Strong conservation is not proved by packet result review.",
        },
        {
            "target": "claim_qft_gr_state_admissibility",
            "decision": "not_authorized",
            "reason": "MR-ASSUMP-002 result review does not imply state admissibility.",
        },
        {
            "target": "claim_qft_gr_source_admissibility",
            "decision": "not_authorized",
            "reason": "MR-ASSUMP-002 result review does not imply source admissibility.",
        },
        {
...
def build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review(
    *,
    ...
    selected_row = packet.get("weak_strong_conservation_comparison_scope_assumption", {})
    ...
    acceptance_criteria = {
        ...
        == SELECTED_ASSUMPTION_FAMILY,
        "selected_row_is_mr_assump_002": packet.get(
            "selected_mathematical_regularity_assumption_row"
        )
        == SELECTED_ROW_ID
        and selected_row.get("assumption_id") == SELECTED_ROW_ID
        and selected_row.get("assumption_family") == SELECTED_ASSUMPTION_FAMILY,
        "selected_row_scope_fields_preserved": packet.get(
            "weak_strong_conservation_comparison_scope"
        )
        == SELECTED_ROW_OBJECT
        and selected_row.get("regularity_condition") == SELECTED_ROW_OBJECT
        and packet.get("weak_conservation_scope") == WEAK_CONSERVATION_SCOPE
        and packet.get("strong_conservation_scope") == STRONG_CONSERVATION_SCOPE
        and packet.get("comparison_scope_boundaries") == COMPARISON_SCOPE_BOUNDARIES,
    ...
    selected_result_review_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_QFT_GR_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW"
    )
    ...
    return {
        ...
        if accepted
        else "QFT_GR_WEAK_STRONG_CONSERVATION_COMPARISON_SCOPE_ASSUMPTION_REDUCTION_PACKET_RESULT_REVIEW_REJECTS_OR_REQUIRES_REMEDIATION",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION
        if accepted
        else "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review_rejects_or_requires_remediation",
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet": EXPECTED_PACKET_ID,
        "consumes_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_pointer": _ptr(
            packet_path
        ),
        "consumed_target": CONSUMED_TARGET,
        "packet_selected_next_target": CONSUMED_TARGET,
        "result_review_selected_next_target": selected_result_review_next_target,
        "weak_strong_conservation_comparison_scope_assumption_reduction_packet_selected_next_target": CONSUMED_TARGET,
        "weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review_selected_next_target": selected_result_review_next_target,
        ...
        "weak_strong_conservation_comparison_scope_assumption_reduction_packet_reviewed": accepted,
        "weak_strong_conservation_comparison_scope_assumption_reduction_packet_accepted": accepted,
        "weak_strong_conservation_comparison_scope_assumption_reduction_packet_rejected": not accepted,
        "weak_strong_conservation_comparison_scope_assumption_reduction_packet_preparation_only": True,
        "packet_preparation_only_confirmed_by_review": accepted,
        "mr_assump_002_attempt_executed_by_review": False,
        "weak_strong_conservation_comparison_scope_assumption_reduction_attempt_authorized": accepted,
        "weak_strong_conservation_comparison_scope_assumption_reduction_attempt_executed": False,
        ...
        "weak_strong_conservation_comparison_scope_authorized_attempt_result_classifications": AUTHORIZED_ATTEMPT_RESULT_CLASSIFICATIONS,
        ...
        "non_claim_boundary": (
            "This result review accepts only the MR-ASSUMP-002 weak/strong "
            "conservation-comparison scope packet and authorizes the bounded "
            "MR-ASSUMP-002 attempt. It does not execute that attempt, does not "
            "prove weak or strong conservation, does not reduce or discharge "
            "mathematical-regularity assumptions by implication, does not claim "
            "state/source admissibility or Bianchi compatibility, does not "
            "construct a conservation proof object or witness, does not derive "
            "the semiclassical Einstein equation, does not close QFT-GR, does not "
            "promote the master action, and does not authorize release assembly "
            "or public submission."
        ),
    }


def write_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR weak/strong conservation comparison scope packet "
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_weak_strong_conservation_comparison_scope_assumption_reduction_packet_result_review_report: "
