"""CLI wrapper for OV-SND-03 sound speed density scaling lock."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    rec = ovsnd03_sound_speed_density_scaling_record(date=str(args.date))

    if not args.no_write:
        out = write_ovsnd03_lock(
    ...
    if args.report:
        print("OV-SND-03 report")
        print(f"  schema: {rec.schema}")
        print(f"  date: {rec.date}")
        print(f"  record_fingerprint: {rec.fingerprint()}")
        print(f"  c_m_per_s: {rec.inputs['OV-SND-02']['c_m_per_s']}")
        print(f"  n0_cm3: {rec.inputs['OV-SND-03N']['n0_cm3']}")
