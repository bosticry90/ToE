...
SCHEMA_ID = (
    ...
    "RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_REFINEMENT_20260614_v0"
)
ATTEMPT_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
    "RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_REFINEMENT_v0"
)
OUTCOME_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_CONSERVATION_RETEST_ATTEMPT_AFTER_POST_"
    "RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_REFINEMENT_EXECUTED_WITH_NO_"
    "CONSERVATION_PROOF_OR_SOURCE_ADMISSIBILITY"
)
RESULT_CLASSIFICATION = (
    "qft_gr_minimal_working_model_conservation_retest_after_post_retest_"
    "refinement_conservation_retest_refinement_refinement_inconclusive_requires_model_"
    "refinement"
)
ALLOWED_RESULT_CLASSIFICATIONS = [
    (
        "qft_gr_minimal_working_model_conservation_retest_after_post_retest_"
        "refinement_conservation_retest_refinement_refinement_passed_pending_result_review"
    ),
    (
        "qft_gr_minimal_working_model_conservation_retest_after_post_retest_"
        "refinement_conservation_retest_refinement_refinement_failed_requires_countermodel_"
        "or_scope_refinement"
    ),
    RESULT_CLASSIFICATION,
]
...
NEXT_TARGET = (
    ...
    "post_retest_refinement_conservation_retest_refinement_refinement_result"
)
NEXT_TARGET_KIND = (
    "qft_gr_minimal_working_model_conservation_retest_attempt_after_post_"
    "retest_refinement_conservation_retest_refinement_refinement_result_review"
)
RETEST_RESULT = "inconclusive"
RETEST_STATUS = (
    "bounded_conservation_retest_attempt_after_post_retest_refinement_"
    "conservation_retest_refinement_refinement_executed_inconclusive_pending_result_review"
)
DEFAULT_OUT = (
    REPO_ROOT
    / "formal"
    ...
    return [
        {
            ...
            "selected": False,
            "meaning": (
                "The v4 conservation retest passed, still pending result "
                "review and still candidate-only."
            ),
        },
        {
            ...
            "selected": False,
            "meaning": (
                "The v4 conservation retest exposed an explicit obstruction "
                "requiring a countermodel or scope refinement."
            ),
        },
        {
            ...
            "selected": True,
            "meaning": (
                "The bounded v4 retest was executed, but the packet "
                "assumptions still do not decide zero versus nonzero weak "
                "divergence."
            ),
        },
    ]
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The executed v4 conservation retest must be result-reviewed "
                "before any pass, fail, model-refinement, or countermodel "
                "route is selected."
            ),
        },
        {
            ...
            "reason": "This bounded v4 retest execution target is consumed here.",
        },
        {
            "target": (
...
def _delta_changes(packet: dict[str, Any]) -> dict[str, dict[str, Any]]:
    delta = packet.get("newest_refinement_delta", {})
    return {
...
def _retest_execution_matrix(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "newest_refinement_delta": packet.get("newest_refinement_delta", {}),
        ...
        "pass_criteria_evaluation": _criterion_rows(
            ...
            reason=(
                "The packet defines the v4 weak pairing domain, v4 regularity "
                "context, and v3 test class, but it supplies no zero "
                "weak-divergence derivation for every admitted pairing."
            ),
        ),
        "fail_criteria_evaluation": _criterion_rows(
            ...
            reason=(
                ...
                "pairing, derivative, divergence, regularization, boundary, "
                "or limit/interchange step inside the v4 scope."
            ),
        ),
...
def _why_inconclusive(packet: dict[str, Any]) -> list[str]:
    ...
    return [
        (
            ...
            "v4 toy weak-pairing setting."
        ),
        (
            "toy_weak_pairing_domain_v4_candidate narrows admitted pairings, "
            "but does not establish full source-domain membership."
        ),
        (
            "toy_regular_context_v4_candidate narrows derivative, boundary, "
            "regularization, and limit/interchange use, but does not prove "
            "vanishing weak divergence for every admitted test vector."
        ),
        (
            "toy_conservation_test_function_class_v3_candidate narrows the "
            "test-vector scope but does not convert the toy candidate into an "
            "admissible source."
        ),
        (
            "toy_source_candidate_definition_v4_candidate remains a candidate "
...
def _what_was_tested(packet: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "defined weak pairings under toy_weak_pairing_domain_v4_candidate",
        (
            "derivative, divergence, regularization, boundary-term, and "
            "limit/interchange admission under toy_regular_context_v4_candidate"
        ),
        (
            "compact-support or admissible test-vector weak-divergence pairings "
            "in toy_conservation_test_function_class_v3_candidate"
        ),
        "explicit pass, fail, and inconclusive criteria from the v4 packet",
    ]
...
def build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement(
    *,
    ...
    acceptance_criteria = {
        "consumes_expected_packet_result_review": (
            review.get("schema_id") == EXPECTED_PACKET_RESULT_REVIEW_SCHEMA_ID
        ...
        is True
        and review.get(
            "bounded_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement_authorized"
        )
        is True,
        ...
        "bounded_v4_retest_executed_only": retest_condition.get("condition_id")
        == RETEST_CONDITION_ID
        ...
        "newest_refinement_delta_consumed": len(delta_changes) == 7
        ...
        "why_inconclusive_recorded": len(why_inconclusive) >= 7,
        ...
        and review.get("aggregate_lean_timeout_caveat_preserved") is True
    ...
    return {
        ...
        if executed
            ...
            "POST_RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_REFINEMENT_REQUIRES_"
            "REMEDIATION"
        ),
        ...
        if executed
            ...
            "retest_refinement_conservation_retest_refinement_refinement_requires_remediation"
        ),
        ...
        "consumes_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement_result_review_pointer": _ptr(
        ...
        "consumes_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement": (
            EXPECTED_PACKET_ID
        ),
        "consumes_qft_gr_minimal_working_model_conservation_retest_packet_after_post_retest_refinement_conservation_retest_refinement_refinement_pointer": _ptr(
            packet_path
        ),
        ...
        "validation_caveat": (
            ...
            "required for this routine bounded retest-attempt checkpoint. The "
            "release-index path remains not freshly Lean-validated, aggregate "
            "Lean is not run, and no aggregate Lean health claim is made. "
            f"Inherited caveat: {review.get('validation_caveat')}"
        ),
        ...
        if executed
            ...
            "ATTEMPT_AFTER_POST_RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_REFINEMENT"
        ),
        ...
        "next_action_scope": (
            ...
            "AFTER_POST_RETEST_REFINEMENT_CONSERVATION_RETEST_REFINEMENT_REFINEMENT_"
        ...
        "non_claim_boundary": (
            "This bounded post-retest-refinement conservation-retest-refinement "
            "attempt records an inconclusive weak-conservation retest result "
            "pending review. It preserves no conservation claim, no "
            "conservation proof object, no conservation witness, no source "
...
def write_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement(
    *,
    packet_result_review_path: Path = DEFAULT_PACKET_RESULT_REVIEW_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR minimal working model bounded v4 conservation-"
            "retest attempt report after post-retest-refinement conservation-"
            "retest refinement."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement(
    ...
    print(
        "qft_gr_minimal_working_model_conservation_retest_attempt_after_post_"
        "retest_refinement_conservation_retest_refinement_refinement_report: "
