...
SCHEMA_ID = "QM_STAT_RL10_DISCRETE_TRANSITION_BRIDGE_EXTERNAL_COMPARATOR_BINDING_RULING_REPORT_20260412_v0"
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    default_outcome = str(contract.get("default_outcome", "COMPARATOR_BINDING_PARTIAL_HOLD")).strip()
    ...
    if terminal_outcome == "BRIDGE_SIGNAL_PROBE_READY":
        next_action = "AUTHORIZE_SINGLE_BOUNDED_EXTERNAL_PROBE_RUN"
    elif terminal_outcome == "EXTERNAL_COMPARATOR_BINDING_CONFIRMED":
        next_action = "PREPARE_BOUNDED_PROBE_READINESS_REVIEW_PACKET"
    elif terminal_outcome == "COMPARATOR_BINDING_PARTIAL_HOLD":
        next_action = "MAINTAIN_HOLD_AND_TIGHTEN_BINDING_EVIDENCE"
    else:
        next_action = "RECORD_FALSIFICATION_AND_RETIRE_BINDING_PATH"
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_BINDING_RULING_OUTCOME",
            "no_loop_rule_declared": str(contract.get("no_loop_rule", "")).strip()
            == "ONE_EXTERNAL_COMPARATOR_BINDING_RULING_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "source_bundle": {
            ...
            "bridge_external_comparator_binding_execution_report": _ptr(execution_path),
        },
        "non_claim_boundary": "Repository-local bridge external comparator binding ruling report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QM-STAT RL10 bridge external comparator binding ruling report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "qm_stat_rl10_discrete_transition_bridge_external_comparator_binding_ruling_report: "
