...
v1 matrix, identity manifest, freeze packet, review anchor, and completed output
...
ASSEMBLER_ID = (
    ...
    "INSTRUMENTED_R13_MECHANISM_EXPERIMENT_RAW_EVIDENCE_ASSEMBLER_v1"
)
SCRIPT_RELATIVE_PATH = (
    "formal/python/tools/dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_raw_evidence_assembler_v1.py"
)
DEFAULT_RUN_MATRIX_RELATIVE_PATH = executor_custody_v1.RUN_MATRIX_RELATIVE_PATH
DEFAULT_IDENTITY_MANIFEST_RELATIVE_PATH = (
    executor_custody_v1.IDENTITY_MANIFEST_RELATIVE_PATH
)
DEFAULT_FREEZE_PACKET_RELATIVE_PATH = executor_custody_v1.FREEZE_PACKET_RELATIVE_PATH
EXPECTED_OUTPUT_ROOT_RELATIVE_PATH = (
    executor_custody_v1.EXPERIMENT_OUTPUT_ROOT_RELATIVE_PATH
)
EXPECTED_CANONICAL_ROOT_RELATIVE_PATH = (
    executor_custody_v1.CANONICAL_OUTPUT_ROOT_RELATIVE_PATH
)
...
CLASSIFIER_V1_MODULE = (
    "formal.python.tools.dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_classifier_v1"
)
ASSEMBLER_V1_MODULE = (
    "formal.python.tools.dirac_maxwell_full_zero_mode_descendant_necessity_and_"
    "robustness_instrumented_r13_mechanism_experiment_raw_evidence_assembler_v1"
)
...
REQUIRED_IMPLEMENTATION_MODULE_NAMES = {
    ...
    CLASSIFIER_V1_MODULE,
    SEMANTIC_V1_MODULE,
    executor_v1.__name__,
    executor_custody_v1.__name__,
...
EXECUTION_STARTED_SCHEMA_ID = executor_v1.EXECUTION_STARTED_SCHEMA_ID
MATRIX_RESULT_SCHEMA_ID = executor_v1.MATRIX_RESULT_SCHEMA_ID
EXPECTED_RUN_IDS = executor_custody_v1.EXACT_RUN_IDS
PAIR_IDS = (
...
class AssembledRawEvidence:
    ...
    scientific_input_closure_digest: str
...
def _attest_authority_file_binding(
    ...
    *,
    ...
    required = {"relative_path", "sha256", "git_blob_oid"}
    ...
    if (
        ...
        or not _is_lower_hex(binding["git_blob_oid"], 40)
        or _git_blob_oid(contents) != binding["git_blob_oid"]
    ):
        _fail("BLOCKED_CUSTODY", diagnostic, relative_path)
...
def _load_and_attest_reviewed_authority(
    ...
    anchor_path = repo_root / executor_custody_v1.REVIEW_ANCHOR_RELATIVE_PATH
    anchor = _load_json_object(anchor_path, missing_result="BLOCKED_CUSTODY")
    if anchor.get("verdict") != executor_custody_v1.EXPECTED_REVIEW_VERDICT:
        _fail("BLOCKED_CUSTODY", "FREEZE_REVIEW_VERDICT_NOT_ACCEPTED")
    authority = anchor.get(executor_custody_v1.REVIEW_AUTHORITY_FIELD)
    ...
    try:
        loaded_authority, anchor_report = executor_v1._load_reviewed_authority(
            repo_root
        )
    ...
    if (
        ...
        != executor_custody_v1.REVIEW_ANCHOR_RELATIVE_PATH
        or anchor_report.get("verdict")
        != executor_custody_v1.EXPECTED_REVIEW_VERDICT
        or anchor_report.get("sha256") != anchor_sha256
        or anchor_report.get("git_blob_oid")
        != _git_blob_oid(anchor_path.read_bytes())
        or anchor_report.get("head_bytes_exact") is not True
        or anchor_report.get("runtime_execution_authority_sha256")
        != _sha256(executor_v1.canonical_json_bytes(authority))
    ):
        _fail("BLOCKED_CUSTODY", "REVIEW_ANCHOR_ATTESTATION_MISMATCH")
    if (
        authority["executor_id"] != executor_v1.EXECUTOR_ID
    ...
    if not _is_lower_hex(closure_digest, 64):
        _fail("BLOCKED_CUSTODY", "SCIENTIFIC_INPUT_CLOSURE_DIGEST_INVALID")
    ...
    if (
        ...
        or set(artifact_bindings) != set(executor_custody_v1.REQUIRED_ARTIFACT_PATHS)
    ):
        _fail("BLOCKED_CUSTODY", "ARTIFACT_BINDING_CLOSURE_INVALID")
    artifact_paths: list[str] = []
    for artifact_name, expected_relative_path in (
        executor_custody_v1.REQUIRED_ARTIFACT_PATHS.items()
    ):
    ...
    if set(artifact_paths) != set(executor_custody_v1.REQUIRED_ARTIFACT_PATHS.values()):
        _fail("BLOCKED_CUSTODY", "ARTIFACT_BINDING_CLOSURE_INVALID")
    implementation_closure = authority["implementation_closure"]
    if (
        not isinstance(implementation_closure, Mapping)
        or not isinstance(implementation_closure.get("modules"), list)
    ):
        _fail("BLOCKED_CUSTODY", "IMPLEMENTATION_CLOSURE_INVALID")
    if (
        _sha256(executor_v1.canonical_json_bytes(implementation_closure))
        != closure_digest
    ):
        _fail("BLOCKED_CUSTODY", "SCIENTIFIC_INPUT_CLOSURE_DIGEST_MISMATCH")
    module_names: list[str] = []
    for binding in implementation_closure["modules"]:
    ...
    if tuple(module_names) != executor_custody_v1.REQUIRED_MODULE_NAMES or set(
        module_names
    ) != REQUIRED_IMPLEMENTATION_MODULE_NAMES:
        _fail("BLOCKED_CUSTODY", "IMPLEMENTATION_CLOSURE_INVALID")
...
def _validate_frozen_documents(
    ...
    try:
        matrix_diagnostics = executor_v1.strict_validate_matrix(matrix, authority)
    ...
    for ordinal, (record, run_id) in enumerate(zip(records, EXPECTED_RUN_IDS, strict=True)):
        ...
        # V1 identity is positive inclusion plus a strict full-record hash.
        # The historical exclusion-list input_hash is retained only as a
        # legacy record field and never controls this assembler.
        try:
            closure_digest = authority["scientific_input_closure_digest"]
            physical_core = executor_v1.build_physical_configuration_core(
                record, closure_digest
            )
            scientific_core = executor_v1.build_scientific_input_core(
                record, closure_digest
            )
            physical_hash = executor_v1.physical_configuration_hash(
                physical_core
            )
            scientific_hash = executor_v1.scientific_input_hash(scientific_core)
        ...
        if not physical_hash or not scientific_hash or not full_hash:
    ...
    for instrumented_id, control_id in PAIR_IDS:
        closure_digest = authority["scientific_input_closure_digest"]
        instrumented_core = executor_v1.build_physical_configuration_core(
            by_id[instrumented_id], closure_digest
        )
        control_core = executor_v1.build_physical_configuration_core(
            by_id[control_id], closure_digest
        )
        if executor_v1.physical_configuration_hash(
            instrumented_core
        ) != executor_v1.physical_configuration_hash(control_core):
            _fail(
                "BLOCKED_RUN_IDENTITY",
...
def _validate_closure_event(
    ...
    # advisory schema fields only in v1 and cannot block or support H_C.
    recomputed = {
        "p_previous": p0,
        "p_current": p1,
        "rho_previous": rho0,
...
def _validate_auxiliary_result(
    ...
    if (
        ...
        or review_anchor_report.get("head_bytes_exact") is not True
    ):
        _fail(
            "BLOCKED_CUSTODY",
    ...
    expected_runtime_anchor = {
        "runtime_execution_authority_sha256": _sha256(
            executor_v1.canonical_json_bytes(authority)
        ),
    ...
    if (
        ...
        or loaded_attestation.get("scientific_input_closure_digest")
        != authority["scientific_input_closure_digest"]
...
def assemble_raw_evidence(repo_root: str | Path) -> AssembledRawEvidence:
    """Authoritative path-closed assembler for the frozen v1 contract.
