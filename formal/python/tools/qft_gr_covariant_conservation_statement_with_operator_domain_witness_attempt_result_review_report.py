...
OUTCOME_ID = (
    "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_WITNESS_ATTEMPT_"
    "RESULT_REVIEW_ACCEPTS_OBSTRUCTION_AND_AUTHORIZES_REFINEMENT_PACKET_PREPARATION_ONLY"
)
...
NEXT_TARGET = (
    "prepare_qft_gr_covariant_conservation_statement_with_operator_domain_"
    "obstruction_refinement_packet"
)
REFINED_OBSTRUCTION_CLASS = "post_operator_domain_statement_missing_conservation_proof_object"
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_WITNESS_ATTEMPT_RESULT_REVIEW_20260525_v0.json"
)
    ...
    return [
        {
            "target": NEXT_TARGET,
            "decision": "selected",
            "reason": "The accepted obstruction should be refined into one bounded packet before another attempt.",
        },
        {
            "target": "execute_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt",
            "decision": "deferred",
            "reason": "Another attempt requires a later refinement packet and review.",
        },
        {
            "target": "prepare_qft_gr_renormalized_expectation_domain_conservation_packet",
            "decision": "deferred",
            "reason": "Expectation-domain refinement may be selected only after obstruction refinement.",
        },
        {
            "target": "close_qft_gr_seam",
            "decision": "not_authorized",
            "reason": "Accepting an obstruction does not close QFT-GR.",
        },
        {
...
def build_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt_result_review(
    *,
    ...
    acceptance_criteria = {
        ...
        is False
        and attempt.get(
            "covariant_conservation_statement_with_operator_domain_witness_constructed"
        )
        is False,
    ...
    return {
        ...
        if accepted
        else "QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_WITNESS_ATTEMPT_RESULT_REVIEW_BLOCKED",
        "result_review_classification": RESULT_REVIEW_CLASSIFICATION,
        "result_review_classification_count": 1 if accepted else 0,
        "consumes_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt": EXPECTED_ATTEMPT_SCHEMA_ID,
        "consumes_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt_pointer": _ptr(
            attempt_path
        ),
        "consumed_attempt_outcome_id": attempt.get("outcome_id"),
        "consumed_attempt_classification": attempt.get("result_classification"),
        "refined_obstruction_class": REFINED_OBSTRUCTION_CLASS,
        ...
        if accepted
        else "REMEDIATE_QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_WITNESS_ATTEMPT_RESULT_REVIEW",
        "selected_next_target_kind": (
            "qft_gr_covariant_conservation_statement_with_operator_domain_obstruction_refinement_packet_preparation"
        ),
        "selection_count": 1 if accepted else 0,
        "next_action_scope": (
            "PREPARE_QFT_GR_COVARIANT_CONSERVATION_STATEMENT_WITH_OPERATOR_DOMAIN_"
            "OBSTRUCTION_REFINEMENT_PACKET_ONLY_NO_QFT_GR_SEAM_CLOSURE_OR_EMPIRICAL_VALIDATION"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts the post-operator-domain witness "
            "attempt obstruction and authorizes only obstruction-refinement "
            "packet preparation. It does not construct a conservation witness, "
...
def write_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt_result_review(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR covariant conservation statement with operator-domain witness attempt result review."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt_result_review(
        attempt_path=attempt_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_covariant_conservation_statement_with_operator_domain_witness_attempt_result_review_report: "
