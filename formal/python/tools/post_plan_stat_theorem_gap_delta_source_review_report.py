...
SCHEMA_ID = "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_REPORT_20260419_v0"
DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_20260419_v0.json"
)
DEFAULT_OUT_PATH = (
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    policy = dict(declaration.get("delta_source_policy", {}))
    ...
    review_path = REPO_ROOT / _text(required_inputs.get("stat_packet05_lane_eligibility_review_report"))
    ...
    protocol_packet04_cap_ok = _text(policy.get("required_protocol_packet04_cap_token")) in protocol_text
    ...
    packet05_review_visible = all(
        [
            review_summary.get("target_row_id") == target_row_id,
            review_summary.get("lane_key") == lane_key,
        ]
    )
    packet05_review_not_eligible = review_summary.get("terminal_outcome") == _text(
        policy.get("required_packet05_review_not_eligible_outcome")
    )
    packet05_review_eligible = review_summary.get("terminal_outcome") == _text(
        policy.get("required_packet05_review_eligible_outcome")
    )
    ...
    higher_packet_path_visible = stat_packet05_lane_enabled or stat_packet05_row_declared or stat_packet05_ledger_visible
    packet04_cap_confirmed = all(
        [
            evidence_pending,
            evidence_target_ok,
            matrix_row_ok,
            packet04_matrix_ok,
            protocol_packet04_cap_ok,
            ...
            doc_and_gate_ok,
            artifact_ok,
            not higher_packet_path_visible,
        ]
    )
    common_ready = all(
        [
            ...
            doc_and_gate_ok,
            artifact_ok,
            packet05_review_visible,
        ]
    )

    allowed_outcomes = set(contract.get("allowed_outcomes", []))
    if higher_packet_path_visible and common_ready and packet05_review_eligible:
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_HIGHER_PACKET_PATH_VISIBLE"
        next_action = "EXECUTE_ONE_BOUNDED_STAT_PACKET05_LANE_ENABLEMENT_ONLY_AND_RERUN_STAT_EVIDENCE_STACK"
    elif packet04_cap_confirmed and packet05_review_not_eligible:
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_PACKET04_CAP_CONFIRMED"
        next_action = "RETAIN_STAT_PACKET04_FAIL_CLOSED_AND_REFRESH_PACKET05_LANE_ELIGIBILITY_ONLY_IF_BOOTSTRAP_CHANGES"
    elif packet04_cap_confirmed:
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_PACKET04_CAP_CONFIRMED"
        next_action = "MATERIALIZE_OR_REPAIR_STAT_PACKET05_LANE_ELIGIBILITY_REVIEW_AND_RERUN_DELTA_SOURCE"
    else:
        terminal_outcome = "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_EVIDENCE_INCOMPLETE"
        next_action = "REPAIR_STAT_DELTA_SOURCE_REVIEW_INPUTS_AND_RERUN"
    ...
    return {
        ...
        "criteria": {
            ...
            "protocol_packet04_cap_declared": protocol_packet04_cap_ok,
            ...
            "packet05_lane_eligibility_review_visible": packet05_review_visible,
            "single_terminal_outcome_rule_declared": _text(contract.get("single_terminal_outcome_rule"))
            == "EXACTLY_ONE_ALLOWED_POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_OUTCOME",
            "no_loop_rule_declared": _text(contract.get("no_loop_rule"))
            == "ONE_POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
                "allowed_outcome_materialized": terminal_outcome in allowed_outcomes,
                "single_outcome_materialized": True,
                "packet04_cap_confirmation_requires_no_higher_packet_path": (
                    terminal_outcome != "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_PACKET04_CAP_CONFIRMED"
                )
                or not higher_packet_path_visible,
                "higher_packet_path_outcome_requires_visibility": (
                    terminal_outcome != "POST_PLAN_STAT_THEOREM_GAP_DELTA_SOURCE_REVIEW_HIGHER_PACKET_PATH_VISIBLE"
                )
                or higher_packet_path_visible,
            },
            "inputs": {
                ...
                "packet05_lane_eligibility_review_outcome": review_summary.get("terminal_outcome"),
        ...
        "summary": {
            ...
            "evidence_surface_outcome": evidence_summary.get("terminal_outcome"),
            "packet05_lane_eligibility_review_outcome": review_summary.get("terminal_outcome"),
            "packet04_baseline_cap_active": packet04_cap_confirmed,
            "higher_packet_path_visible": higher_packet_path_visible,
        ...
        "source_bundle": {
            ...
            "stat_packet05_lane_eligibility_review_report": _ptr(review_path),
        ...
        "non_claim_boundary": "Repository-local STAT theorem-gap delta-source review only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the post-plan STAT theorem-gap delta-source review report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "post_plan_stat_theorem_gap_delta_source_review_report: "
