...
SCHEMA_ID = "PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_20260630_v0"
ARTIFACT_ID = SCHEMA_ID
PACKET_ID = "PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_v0"
...
NEXT_TARGET = "review_phi_source_theorem_linkage_obligation_closeout_result"
NEXT_TARGET_KIND = "phi_source_theorem_linkage_obligation_closeout_result_review"
...
DEFAULT_OUT = (
    ...
    / "docs"
    / "release"
    / "PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_20260630_v0.json"
)
LEAN_PACKET_PATH = (
    REPO_ROOT
    / "formal"
    ...
    / "ToeFormal"
    / "Derivation"
    / "PhiSourceTheoremLinkageObligationCloseout.lean"
)
...
def _consumed_review_valid(review: dict[str, Any]) -> bool:
    return (
        ...
        and review.get("on_shell_condition") == ON_SHELL_CONDITION
        and review.get("target_conclusion") == TARGET_CONCLUSION
        and review.get("execution_route") == EXECUTION_ROUTE
        and review.get("execution_reduction_route") == EXECUTION_REDUCTION_ROUTE
        and review.get("C_source_phi_zero_constructed") is True
        and review.get("C_source_phi_zero_derived") is True
...
def _validation_policy() -> dict[str, Any]:
    return {
        "checkpoint_type": "phi_source_theorem_linkage_obligation_closeout",
...
def build_phi_source_theorem_linkage_obligation_closeout(
    *,
    ...
    acceptance_criteria = {
        ...
        "standalone_phi_route_preserved": (
            review.get("C_source_phi_residual_definition")
            == C_SOURCE_PHI_RESIDUAL_DEFINITION
            and review.get("residual_identity_form") == RESIDUAL_IDENTITY_FORM
            and review.get("on_shell_residual_form") == ON_SHELL_RESIDUAL_FORM
            and review.get("on_shell_condition") == ON_SHELL_CONDITION
        ...
        "constructed_and_reviewed": (
            review.get("C_source_phi_zero_constructed") is True
            and review.get("C_source_phi_zero_derived") is True
            and review.get("reviewed") is True
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT"
    )
    payload: dict[str, Any] = {
        "artifact_id": ARTIFACT_ID,
        "schema_id": SCHEMA_ID,
        "packet_id": PACKET_ID,
        "status": "ACTIVE_PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT",
        ...
        if accepted
        else "PHI_SOURCE_THEOREM_LINKAGE_OBLIGATION_CLOSEOUT_REQUIRES_REMEDIATION",
        ...
        "plain_meaning": PLAIN_MEANING,
        "local_phi_source_theorem_linkage_obligation_closed": accepted,
        ...
        "non_claim_boundary": (
            "This closeout records only that the local phi-source theorem-linkage "
            "obligation is closed by the standalone scalar/on-shell residual "
            "route: C_source^nu[g, phi] is defined as nabla_mu T_phi^{mu nu}, "
            "rewritten as sum_i R_i^phi nabla^nu phi_i with R_i^phi := "
            "Box_g phi_i + partial_i V(phi), and reduced to zero under the "
            "on-shell condition R_i^phi = 0. It claims no phi-sector "
            "completion, no full scalar/QFT closure, no QFT-GR source "
            "admissibility, no EM-QFT closure, no general C_k closure, no C_k "
            "rule promotion, no action embedding, no variation, no empirical "
            "validation, no seam closure, and no master-action promotion."
        ),
        "critical_gate_fail_conditions": [
            "fail to consume prepare_phi_source_theorem_linkage_obligation_closeout",
            "fail to close the local phi-source theorem-linkage obligation",
        ...
        "lane_level_lean_targets": [
            "ToeFormal.Derivation.PhiSourceTheoremLinkageObligationCloseout",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Close out the local standalone phi-source theorem-linkage obligation."
    )
    ...
    payload = build_phi_source_theorem_linkage_obligation_closeout(
        ...
        json.dumps(
            {
                ...
                "phi_source_theorem_linkage_obligation_locally_closed": payload[
                    "phi_source_theorem_linkage_obligation_locally_closed"
                ],
