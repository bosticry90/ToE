...
SCHEMA_ID = "QM_STAT_RL10_DISCRETE_TRANSITION_BRIDGE_ROBUSTNESS_REFINEMENT_RULING_REPORT_20260412_v0"
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    default_outcome = str(contract.get("default_outcome", "ROBUSTNESS_REFINEMENT_INCONCLUSIVE")).strip()
    ...
    if terminal_outcome == "BRIDGE_SIGNAL_PROBE_READY":
        next_action = "AUTHORIZE_SINGLE_PROBE_LANE_GATEWAY_REVIEW"
    elif terminal_outcome == "BRIDGE_SIGNAL_COMPARATOR_BOUND_BUT_HOLD":
        next_action = "RETAIN_HOLD_AFTER_REFINEMENT"
    elif terminal_outcome == "BRIDGE_SIGNAL_PATH_FALSIFIED":
        next_action = "RETIRE_PATH_AND_RECORD_FALSIFICATION"
    else:
        next_action = "REVIEW_REFINEMENT_PACKET_FOR_INCONCLUSIVE_CAUSE"
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_REFINEMENT_RULING_OUTCOME",
            "no_loop_rule_declared": str(contract.get("no_loop_rule", "")).strip()
            == "ONE_BRIDGE_ROBUSTNESS_REFINEMENT_RULING_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "source_bundle": {
            ...
            "bridge_robustness_refinement_execution_report": _ptr(execution_path),
        },
        "non_claim_boundary": "Repository-local bridge robustness refinement ruling report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QM-STAT RL10 bridge robustness refinement ruling report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "qm_stat_rl10_discrete_transition_bridge_robustness_refinement_ruling_report: "
