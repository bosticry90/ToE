"""CLI: write OV-SEL-SND-02 sound anchor interaction stress test lock."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    rec = ovsel_snd02_sound_anchor_interaction_stress_test_record(status_date=str(args.status_date))

    if not args.no_write:
        path = write_ovsel_snd02_lock(status_date=str(args.status_date))
        if args.report:
            print(f"Wrote lock: {path}")

    if args.report:
        print("OV-SEL-SND-02 report")
