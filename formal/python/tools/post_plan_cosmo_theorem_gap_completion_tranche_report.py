...
SCHEMA_ID = "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_REPORT_20260418_v0"
DEFAULT_DECLARATION_PATH = (
    REPO_ROOT / "formal" / "docs" / "release" / "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_20260418_v0.json"
)
DEFAULT_OUT_PATH = (
    REPO_ROOT / "formal" / "output" / "reports" / "post_plan_cosmo_theorem_gap_completion_tranche_20260418_v0.json"
)
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    doc_path = REPO_ROOT / str(required_inputs.get("cosmo_target_doc", "")).strip()
    artifact_path = REPO_ROOT / str(required_inputs.get("cosmo_artifact", "")).strip()
    gate_path = REPO_ROOT / str(required_inputs.get("cosmo_gate", "")).strip()
    ...
    queue_ok = (
        ...
        and queue_report.get("summary", {}).get("first_active_row") == str(policy.get("required_queue_first_active_row", "")).strip()
    )
    target_map_ok = target_row.get("route_class") == str(policy.get("required_target_route_class", "")).strip()
    doc_ok = all(
        token in doc_text
        for token in [
            "DERIVATION_TARGET_COSMO_EMPIRICAL_COMPARISON_PACKET_04_v0",
            "COSMO_EMPIRICAL_PACKET_04_STATUS_v0: RUN_BOUNDED_v0_NONCLAIM",
            "COSMO_EMPIRICAL_PACKET_04_DECISION_v0: INCONCLUSIVE_v0",
    ...
    artifact_ok = (
        artifact.get("artifact_id") == "cosmo_empirical_comparison_packet_04_v0"
    ...
    default_outcome = str(outcome_contract.get("default_outcome", "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE")).strip()

    if not matrix_row or not target_row:
        terminal_outcome = "HOLD_PENDING_POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_REPAIR"
        next_action = "RESTORE_COSMO_TRANCHE_INPUT_SHAPE_AND_RERUN"
    elif all([queue_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and row_truth_change_detected:
        terminal_outcome = "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED"
        next_action = "REASSESS_SEAM_ROUTES_AND_MASTER_ACTION_WITH_CHANGED_COSMO_ROW_TRUTH"
    elif all([queue_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]) and explicit_exhaustion_detected:
        terminal_outcome = "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED"
        next_action = "PREPARE_POST_PLAN_STAT_THEOREM_GAP_COMPLETION_TRANCHE_WITH_COSMO_EXHAUSTION_RECORDED"
    elif all([queue_ok, target_map_ok, doc_ok, artifact_ok, row_ok, contradiction_ok]):
        ...
        terminal_outcome = "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_EVIDENCE_INCOMPLETE"
        next_action = "REPAIR_COSMO_TRANCHE_EVIDENCE_AND_RERUN"
    ...
    return {
        ...
        "criteria": {
            ...
            "cosmo_doc_tokens_present": doc_ok,
            "cosmo_artifact_alignment_ok": artifact_ok,
            "cosmo_row_alignment_ok": row_ok,
            "cosmo_live_theorem_gap_observation_present": contradiction_ok,
            "single_terminal_outcome_rule_declared": str(outcome_contract.get("single_terminal_outcome_rule", "")).strip()
            == "EXACTLY_ONE_ALLOWED_POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_OUTCOME",
            "no_loop_rule_declared": str(outcome_contract.get("no_loop_rule", "")).strip()
            == "ONE_POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_LAYER_ONLY",
        },
        "objective_quality": {
            "criteria": {
                "allowed_outcome_materialized": terminal_outcome in allowed_outcomes,
                "single_outcome_materialized": True,
                "promotion_only_if_row_truth_changed": (terminal_outcome != "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_EXECUTED_AND_PROMOTED") or row_truth_change_detected,
                "exhaustion_only_if_explicit_decision_present": (terminal_outcome != "POST_PLAN_COSMO_THEOREM_GAP_COMPLETION_TRANCHE_EXPLICITLY_EXHAUSTED") or explicit_exhaustion_detected,
            },
            "inputs": {
        ...
        "source_bundle": {
            ...
            "cosmo_target_doc": _ptr(doc_path),
            "cosmo_artifact": _ptr(artifact_path),
            "cosmo_gate": _ptr(gate_path),
        },
        "non_claim_boundary": "Repository-local post-plan COSMO theorem-gap completion tranche measurement only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the post-plan COSMO theorem-gap completion tranche report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "post_plan_cosmo_theorem_gap_completion_tranche_report: "
