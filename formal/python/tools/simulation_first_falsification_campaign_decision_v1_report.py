...
SCHEMA_ID = "SIMULATION_FIRST_FALSIFICATION_CAMPAIGN_DECISION_20260411_v1"
...
def build_report(*, packet_report_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    packet_outcome = str(packet.get("summary", {}).get("packet_outcome", "INCONCLUSIVE_NO_COMPARISON_ROWS"))
    scientific_state_change = bool(packet.get("summary", {}).get("scientific_state_change_observed", False))
    major_dead_end_elimination = bool(packet.get("summary", {}).get("major_dead_end_elimination_observed", False))
    ...
    if blocker_facing_movement:
        ...
        next_action = "CONTINUE_SIMULATION_FIRST_CAMPAIGN_WITH_BLOCKER_FOCUS"
    elif scientific_state_change and major_dead_end_elimination:
        decision = "SIMULATION_FIRST_CAMPAIGN_PRODUCTIVE_WITH_ROUTE_ELIMINATION"
        next_action = "CONTINUE_SIMULATION_FIRST_CAMPAIGN_WITH_NEXT_BOUNDED_PACKET"
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate simulation-first falsification campaign v1 decision report.")
    parser.add_argument("--packet-report", type=Path, default=DEFAULT_PACKET_REPORT_PATH)
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "simulation_first_falsification_campaign_decision_20260411_v1.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "simulation_first_falsification_campaign_decision_v1_report: "
