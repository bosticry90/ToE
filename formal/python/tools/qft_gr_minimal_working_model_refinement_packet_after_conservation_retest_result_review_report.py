...
SCHEMA_ID = (
    ...
    "PACKET_RESULT_REVIEW_20260613_v0"
)
REVIEW_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_AFTER_CONSERVATION_RETEST_"
...
OUTCOME_ID = (
    "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_AFTER_CONSERVATION_RETEST_"
    "PACKET_RESULT_REVIEW_ACCEPTS_PACKET_AND_AUTHORIZES_BOUNDED_REFINEMENT_"
    "ATTEMPT_ONLY"
)
RESULT_REVIEW_CLASSIFICATION = (
    "qft_gr_minimal_working_model_refinement_after_conservation_retest_packet_"
    "result_review_accepts_packet_and_authorizes_bounded_refinement_attempt_only"
)
CONSUMED_TARGET = EXPECTED_CONSUMED_TARGET
NEXT_TARGET = (
    "execute_qft_gr_minimal_working_model_refinement_attempt_after_conservation_"
    "retest"
)
NEXT_TARGET_KIND = (
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The post-retest refinement packet is accepted as a preparation "
                "artifact, so the next bounded action may execute only the "
                "post-retest refinement attempt scoped to weak pairing domain, "
                "regularity assumptions, test-function class, candidate source "
                "definition, and scope restriction."
            ),
        },
        {
            ...
            "reason": (
                "This post-retest refinement-packet result-review target is "
                "consumed here."
            ),
        },
        {
            ...
            "reason": "The review authorizes refinement attempt execution only.",
        },
        {
            "target": (
            ...
            "reason": "The accepted packet preserves an inconclusive retest, not a failed retest.",
        },
        {
...
def _validation_policy() -> dict[str, Any]:
    return {
        "checkpoint_type": "routine_packet_result_review",
...
def build_qft_gr_minimal_working_model_refinement_packet_after_conservation_retest_result_review(
    *,
    ...
    acceptance_criteria = {
        "consumes_expected_refinement_after_retest_packet": packet.get("schema_id")
        ...
        "post_retest_refinement_dimensions_confirmed": {
            row.get("scope") for row in refinement_dimensions
        }
        >= {
            "weak_pairing_domain",
        ...
        "standing_aggregate_lean_caveat_preserved": packet.get(
    ...
    selected_next_target = (
        ...
        if accepted
            ...
            "CONSERVATION_RETEST_PACKET_RESULT_REVIEW"
        )
    )
    ...
    return {
        ...
        if accepted
            ...
            "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_AFTER_CONSERVATION_RETEST_"
            "PACKET_RESULT_REVIEW_REQUIRES_REMEDIATION"
        ),
        ...
        "consumes_qft_gr_minimal_working_model_refinement_after_retest_packet": (
            EXPECTED_PACKET_ID
        ),
        "consumes_qft_gr_minimal_working_model_refinement_after_retest_packet_pointer": (
            _ptr(packet_path)
        ),
        ...
        "refinement_after_retest_packet_result_review_accepted": accepted,
        ...
        "bounded_post_retest_refinement_attempt_authorized": accepted,
        ...
        "validation_posture": {
            "focused_packet_current_target_registry_gates": (
                "accepted_reported_22_passed"
            ),
            "adjacent_minimal_model_nonclaim_gates": (
                "accepted_reported_29_passed_3_expected_historical_skips"
            ),
            "targeted_lean_packet_frontier_index": "accepted_reported_passed",
            "git_diff_check": "accepted_reported_passed_lf_crlf_warnings_only",
        ...
        "next_action_scope": (
            "EXECUTE_QFT_GR_MINIMAL_WORKING_MODEL_POST_RETEST_REFINEMENT_"
            "ATTEMPT_ONLY_NO_CONSERVATION_RERUN_COUNTERMODEL_PACKET_SOURCE_"
        ...
        "non_claim_boundary": (
            "This result review accepts only the prepared post-retest refinement "
            "packet and authorizes one bounded refinement attempt. It does not "
            "execute the refinement attempt, does not rerun conservation, does "
            "not prepare a countermodel packet, does not claim source "
            ...
            "no QFT-GR seam, validates nothing empirically, authorizes no public "
            "submission, and promotes no master action. Boundary shorthand: no "
...
def write_qft_gr_minimal_working_model_refinement_packet_after_conservation_retest_result_review(
    *,
    packet_path: Path = DEFAULT_PACKET_PATH,
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the QFT-GR minimal working model post-retest "
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_refinement_packet_after_conservation_retest_result_review(
        packet_path=packet_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_minimal_working_model_refinement_packet_after_conservation_retest_result_review_report: "
