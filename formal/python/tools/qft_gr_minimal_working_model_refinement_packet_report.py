...
SCHEMA_ID = "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_PACKET_20260613_v0"
PACKET_ID = "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_PACKET_v0"
OUTCOME_ID = (
...
NEXT_TARGET = "review_qft_gr_minimal_working_model_refinement_packet_result"
NEXT_TARGET_KIND = "qft_gr_minimal_working_model_refinement_packet_result_review"
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The refinement packet must be reviewed before any refinement "
                "attempt, conservation retest, source-admissibility claim, or "
        ...
        },
        {
            ...
            "reason": "This model-refinement packet preparation target is consumed here.",
        },
        {
            "target": "execute_qft_gr_minimal_working_model_refinement_attempt",
            "decision": "not_authorized_before_packet_result_review",
            "reason": "The packet prepares obligations only; it does not execute refinement.",
        },
        {
            "target": "retry_qft_gr_minimal_working_model_conservation_test",
            "decision": "not_authorized_before_refinement_review",
            "reason": "The packet does not rerun or upgrade the inconclusive test.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "The packet is a refinement plan and contains no conservation proof.",
        },
        {
        ...
        },
        {
            ...
            "decision": "not_authorized",
            "reason": "QFT-GR closure remains outside this refinement packet.",
        },
        {
...
def _refinement_obligations() -> list[dict[str, Any]]:
    return [
        {
            "obligation_id": "weak_pairing_domain_for_toy_candidate",
            "scope": "weak_pairing_domain",
            "required_packet_output": (
                "Identify the admitted test-vector domain and dual pairing "
                "domain needed to state the toy candidate's weak divergence "
                "pairings without asserting source-domain membership."
            ),
            "source_admissibility_claimed": False,
            "conservation_claimed": False,
        },
        {
            "obligation_id": "regularity_budget_for_toy_candidate_pairing",
            "scope": "regularity",
            "required_packet_output": (
                "Record the minimal regularity, derivative-exchange, boundary, "
                "and limit/interchange assumptions needed for the packet-level "
                "weak pairing operations."
            ),
            "source_admissibility_claimed": False,
            "conservation_claimed": False,
        },
        {
            "obligation_id": "inconclusive_test_obstruction_accounting",
            "scope": "obstruction_accounting",
            "required_packet_output": (
                "Map each inconclusive-test obstruction to either weak-pairing "
                "domain refinement or regularity refinement, with no pass/fail "
                "conversion."
            ),
            "source_admissibility_claimed": False,
            "conservation_claimed": False,
        ...
        {
            "obligation_id": "nonpromotion_review_gate",
            "scope": "governance_boundary",
            "required_packet_output": (
                "Require packet result review before any refinement attempt, "
                "conservation retest, source-admissibility claim, or promotion."
            ),
            "source_admissibility_claimed": False,
            "conservation_claimed": False,
        },
...
def _review_gate_requirements() -> list[str]:
    return [
        ...
        "confirm no conservation test is retried",
...
def build_qft_gr_minimal_working_model_refinement_packet(
    *,
    ...
    refinement_obligations = _refinement_obligations()
    selected_refinement_objectives = [REFINEMENT_OBJECTIVE]

    acceptance_criteria = {
        "consumes_expected_conservation_test_attempt_result_review": review.get(
            "schema_id"
        )
        ...
        == CONSUMED_TARGET,
        "consumed_attempt_classification_preserved": review.get(
            "consumed_attempt_classification"
        )
        == "qft_gr_minimal_working_model_conservation_test_inconclusive_requires_model_refinement",
        "inconclusive_not_converted_to_pass": review.get(
            "inconclusive_not_converted_to_pass"
        )
        is True
        and review.get("test_inconclusive") is True
        and review.get("test_passed") is False,
        ...
        == [REFINEMENT_OBJECTIVE],
        "weak_pairing_domain_and_regularity_obligations_recorded": {
            row["scope"] for row in refinement_obligations
        }
        >= {"weak_pairing_domain", "regularity"},
        "obstruction_accounting_recorded": any(
            row["scope"] == "obstruction_accounting"
            for row in refinement_obligations
        ),
        "review_gate_requirements_recorded": len(_review_gate_requirements()) >= 8,
    ...
    selected_next_target = (
        NEXT_TARGET
        if accepted
        else "REMEDIATE_QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_PACKET"
    )
    ...
    return {
        ...
        if accepted
        else "QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_PACKET_REQUIRES_REMEDIATION",
        ...
        "consumes_qft_gr_minimal_working_model_conservation_test_attempt_result_review_pointer": _ptr(
            result_review_path
        ),
        ...
        "consumed_attempt_classification": review.get(
            "consumed_attempt_classification"
        ),
        "conservation_test_attempt_result": review.get("test_result"),
        "accepted_inconclusive_result": review.get("accepted_inconclusive_result")
        is True,
        ...
        "refinement_obligations": refinement_obligations,
        ...
        "conservation_test_executed_by_packet": False,
        ...
        "next_action_scope": (
            "REVIEW_QFT_GR_MINIMAL_WORKING_MODEL_REFINEMENT_PACKET_RESULT_ONLY_"
            "NO_REFINEMENT_ATTEMPT_CONSERVATION_RETRY_SOURCE_ADMISSIBILITY_"
        ...
        "non_claim_boundary": (
            "This packet prepares only a weak-pairing-domain and regularity "
            "refinement packet for the toy candidate. It does not retry the "
            "conservation test, claim conservation, construct a conservation "
            "proof object, construct a conservation witness, claim source "
...
def write_qft_gr_minimal_working_model_refinement_packet(
    *,
    result_review_path: Path = DEFAULT_RESULT_REVIEW_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_minimal_working_model_refinement_packet(
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QFT-GR minimal working model refinement packet."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_minimal_working_model_refinement_packet(
        result_review_path=result_review_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_minimal_working_model_refinement_packet_report: "
