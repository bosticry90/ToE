...
SCHEMA_ID = "THEOREM_GAP_STAT_BOUNDED_STOP_RULE_DECISION_20260411_v0"
...
TARGET_ROW = "ROW-PILLAR-STAT-001"
MAX_NO_CHANGE_ATTEMPTS = 1
EXCLUDED_ROWS = {"ROW-PILLAR-QM-001", "ROW-PILLAR-GR-001", "ROW-PILLAR-STAT-001"}
PREFERRED_ORDER = [
...
def build_report(captured_at_utc: str | None, consume_attempt: bool) -> dict[str, Any]:
    stat = _read_json(STAT_SUBTARGET_REPORT_PATH)
    closure_map = _read_json(CLOSURE_MAP_PATH)

    stat_inputs = stat.get("objective_quality", {}).get("inputs", {})
    theorem_gap_delta = int(stat_inputs.get("theorem_gap_delta", 0) or 0)
    target_row_success_incremented = bool(stat_inputs.get("target_row_success_count_incremented", False))
    ...
    if stop_rule_triggered:
        decision = "DEFER_OR_RECLASSIFY_STAT_NEAR_TERM_BLOCKER_BURN_LANE"
        next_action = "SELECT_NEXT_NON_QM_NON_GR_NON_STAT_BLOCKER_BEARING_LANE"
    else:
        decision = "CONTINUE_STAT_ON_NARROWER_SUBPROBLEM"
        next_action = "RUN_STAT_NARROW_SUBPROBLEM_BLOCKER_MOVING_TRANCHE"

    return {
        "schema_id": SCHEMA_ID,
        "status": "ACTIVE_NONLIVE_NONCLAIM",
        "captured_at_utc": _ts(captured_at_utc),
        "target": "STAT_ACTIVE_LANE_PROBATION_AND_BOUNDED_STOP_RULE",
        "inputs": {
        ...
        "summary": {
            "decision": decision,
            "stat_continuation_earned": movement_observed,
        ...
        "source_bundle": {
            "stat_subtarget_report": _ptr(STAT_SUBTARGET_REPORT_PATH),
            "closure_map": _ptr(CLOSURE_MAP_PATH),
        },
        "non_claim_boundary": "Repository-local STAT bounded stop-rule decision artifact; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate theorem-gap STAT bounded stop-rule decision report.")
    parser.add_argument("--consume-attempt", action="store_true")
    parser.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "formal" / "output" / "reports" / "theorem_gap_stat_bounded_stop_rule_decision_20260411_v0.json",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "theorem_gap_stat_bounded_stop_rule_decision_report: "
