...
SCHEMA_ID = "QM_STAT_RL10_DISCRETE_TRANSITION_BRIDGE_PROBE_READINESS_ROBUSTNESS_RULING_REPORT_20260412_v0"
...
def build_report(*, declaration_path: Path, captured_at_utc: str | None) -> dict[str, Any]:
    ...
    default_outcome = str(contract.get("default_outcome", "BRIDGE_SIGNAL_COMPARATOR_BOUND_BUT_HOLD")).strip()
    ...
    if terminal_outcome == "BRIDGE_SIGNAL_PROBE_READY":
        next_action = "OPEN_SINGLE_PROBE_LANE_AUTHORIZATION_REVIEW"
    elif terminal_outcome == "BRIDGE_SIGNAL_COMPARATOR_BOUND_BUT_HOLD":
        next_action = "MAINTAIN_HOLD_AND_RUN_NEXT_BOUNDED_ROBUSTNESS_CHECK"
    elif terminal_outcome == "BRIDGE_SIGNAL_ROBUSTNESS_FAILURE":
        next_action = "REMEDIATE_ROBUSTNESS_FAILURE_BEFORE_ANY_PROBE"
    else:
    ...
    return {
        ...
        "criteria": {
            ...
            == "EXACTLY_ONE_ALLOWED_ROBUSTNESS_RULING_OUTCOME",
            "no_loop_rule_declared": str(contract.get("no_loop_rule", "")).strip()
            == "ONE_BRIDGE_PROBE_READINESS_ROBUSTNESS_RULING_ONLY",
        },
        "objective_quality": {
            "criteria": {
        ...
        "source_bundle": {
            ...
            "bridge_probe_readiness_robustness_execution_report": _ptr(execution_path),
        },
        "non_claim_boundary": "Repository-local bridge probe-readiness robustness ruling report only; no scientific adequacy claim.",
    }


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the QM-STAT RL10 bridge probe-readiness robustness ruling report."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "qm_stat_rl10_discrete_transition_bridge_probe_readiness_robustness_ruling_report: "
