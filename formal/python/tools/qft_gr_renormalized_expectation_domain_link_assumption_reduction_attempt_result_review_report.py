...
NEXT_OPERATOR_DOMAIN_ASSUMPTION_ROW = "OD-ASSUMP-005-conservation_form_scope"
NEXT_TARGET = "prepare_qft_gr_conservation_form_scope_assumption_reduction_packet"
    ...
    return [
        {
            ...
            "decision": "selected",
            "reason": (
                "The OD-ASSUMP-004 renormalized-expectation domain-link "
                "reduction is accepted only as a bounded operator-domain link "
                "contract, so the next operator-domain row may be prepared "
                "without claiming source admissibility or conservation."
            ),
        },
        {
            ...
            "reason": (
                "Accepting renormalized-expectation domain linkage does not "
...
def build_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    attempt = _read_json(attempt_path)
    contract = attempt.get("renormalized_expectation_domain_link_reduction_contract", {})
    ...
    acceptance_criteria = {
        ...
        "prior_rows001_002_003_preserved": attempt.get(
        ...
        == RENORMALIZED_EXPECTATION_DOMAIN_LINK_CONTRACT_ID,
        "renormalized_expectation_object_preserved": attempt.get(
            "renormalized_expectation_object"
        )
        == RENORMALIZED_EXPECTATION_OBJECT
        and contract.get("renormalized_expectation_object")
        == RENORMALIZED_EXPECTATION_OBJECT,
        ...
        is False
        and attempt.get("renormalized_expectation_domain_link_assumption_inconclusive")
        is False,
        ...
        "next_row_is_fifth_operator_domain_row": row_sequence[4]
        == NEXT_OPERATOR_DOMAIN_ASSUMPTION_ROW,
    ...
    return {
        ...
        "consumes_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt": EXPECTED_ATTEMPT_ID,
        "consumes_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt_pointer": _ptr(
            attempt_path
        ),
        ...
        "renormalized_expectation_domain_link_reduction_contract": contract,
        "accepted_contract_id": RENORMALIZED_EXPECTATION_DOMAIN_LINK_CONTRACT_ID
        if accepted
        else "",
        "contract_acceptance_scope": (
            "bounded_renormalized_expectation_domain_link_contract_only_no_full_"
        ...
        "renormalized_expectation_domain_link_assumption_reduction_attempt_result_reviewed": accepted,
        "renormalized_expectation_domain_link_assumption_reduction_accepted": accepted,
        "renormalized_expectation_domain_link_assumption_reduction_rejected": not accepted,
        "renormalized_expectation_domain_link_assumption_reduced_pending_result_review_accepted": accepted,
        "renormalized_expectation_domain_link_assumption_discharged": False,
        "renormalized_expectation_domain_link_assumption_discharged_by_review": False,
        ...
        "selected_next_target_kind": (
            "qft_gr_conservation_form_scope_assumption_reduction_packet_preparation"
        ),
        "selection_count": 1 if accepted else 0,
        "next_action_scope": (
            "PREPARE_QFT_GR_CONSERVATION_FORM_SCOPE_ASSUMPTION_REDUCTION_"
            "PACKET_ONLY_NO_SOURCE_ADMISSIBILITY_OR_QFT_GR_SEAM_CLOSURE"
        ),
        "acceptance_criteria": acceptance_criteria,
        "non_claim_boundary": (
            "This result review accepts only the reduced OD-ASSUMP-004 "
            "renormalized-expectation domain-link contract and authorizes "
            "preparation of the next operator-domain row, OD-ASSUMP-005 "
            "conservation-form scope. It does not discharge the assumption as "
...
def write_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt_result_review(
    *,
    attempt_path: Path = DEFAULT_ATTEMPT_PATH,
    out: Path = DEFAULT_OUT,
    captured_at_utc: str = DEFAULT_CAPTURED_AT_UTC,
) -> dict[str, Any]:
    payload = build_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt_result_review(
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt_result_review(
        attempt_path=attempt_path,
        out=out,
        captured_at_utc=str(ns.captured_at_utc),
    )
    print(
        "qft_gr_renormalized_expectation_domain_link_assumption_reduction_attempt_result_review_report: "
