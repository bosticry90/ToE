...
def test_rl13_surface_contract_top_level_keys_frozen():
    rec = rl13_velocity_addition_group_law_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-13_velocity_addition_group_law_comparator/v0"
    assert payload["observable_id"] == "OV-RL-13"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-13"
...
def test_rl13_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl13_velocity_addition_group_law_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_rl13_scope_limits_include_discriminative_candidate():
    rec = rl13_velocity_addition_group_law_v0_record(date="2026-02-09", tolerance_profile="pinned")
