...
def test_rl07_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "OV-RL-07_energy_noether_conservation_v0.md"

    rec = rl07_energy_noether_conservation_v0_record(date="2026-02-09", tolerance_profile="pinned")
    expected = render_rl07_lock_markdown(rec)
