...
from formal.python.toe.comparators.ct07_external_anchor_dispersion_lowk_slice_v0 import (
    _load_ct07_report_artifact,
    ct07_external_anchor_dispersion_lowk_slice_v0_record,
)


def test_ct07_pinned_artifacts_exist_and_unblock() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    artifact_dir = repo_root / "formal" / "external_evidence" / "ct07_external_anchor_dispersion_lowk_domain_01"
    ref_path = artifact_dir / "ct07_reference_report.json"
    cand_path = artifact_dir / "ct07_candidate_report.json"
    metadata_path = artifact_dir / "dataset_metadata.json"
    citation_path = artifact_dir / "source_citation.md"

    assert ref_path.exists(), f"Missing CT-07 reference artifact: {ref_path}"
    assert cand_path.exists(), f"Missing CT-07 candidate artifact: {cand_path}"
    assert metadata_path.exists(), f"Missing CT-07 dataset metadata: {metadata_path}"
    assert citation_path.exists(), f"Missing CT-07 source citation: {citation_path}"
    ...
    _, ref_ok = _load_ct07_report_artifact(ref_path)
    _, cand_ok = _load_ct07_report_artifact(cand_path)
    assert ref_ok is True
    assert cand_ok is True

    rec = ct07_external_anchor_dispersion_lowk_slice_v0_record(date="2026-02-10", tolerance_profile="pinned")
