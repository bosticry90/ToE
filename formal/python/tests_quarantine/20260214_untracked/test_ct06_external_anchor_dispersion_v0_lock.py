...
def test_ct06_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "CT-06_external_anchor_dispersion_v0.md"

    rec = ct06_external_anchor_dispersion_v0_record(date="2026-02-10", tolerance_profile="pinned")
    expected = render_ct06_lock_markdown(rec)
