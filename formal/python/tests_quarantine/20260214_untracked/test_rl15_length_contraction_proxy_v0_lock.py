...
def test_rl15_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "OV-RL-15_length_contraction_proxy_v0.md"

    rec = rl15_length_contraction_proxy_v0_record(date="2026-02-09", tolerance_profile="pinned")
    expected = render_rl15_lock_markdown(rec)
