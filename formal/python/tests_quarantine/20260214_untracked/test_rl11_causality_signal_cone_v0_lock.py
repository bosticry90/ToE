...
def test_rl11_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "OV-RL-11_causality_signal_cone_v0.md"

    rec = rl11_causality_signal_cone_v0_record(date="2026-02-09", tolerance_profile="pinned")
    expected = render_rl11_lock_markdown(rec)
