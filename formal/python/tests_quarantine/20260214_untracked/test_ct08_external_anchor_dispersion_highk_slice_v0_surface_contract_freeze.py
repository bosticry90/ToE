...
def test_ct08_surface_contract_top_level_keys_frozen():
    ...
    assert payload["schema"] == "CT-08_external_anchor_dispersion_highk_slice_comparator/v0"
    assert payload["observable_id"] == "CT-08"
    assert payload["domain_id"] == "CT-DOMAIN-08"
...
def test_ct08_surface_contract_row_keys_and_reason_codes_frozen():
    rec = ct08_external_anchor_dispersion_highk_slice_v0_record(date="2026-02-10", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_ct08_scope_limits_include_non_independent_token():
    rec = ct08_external_anchor_dispersion_highk_slice_v0_record(date="2026-02-10", tolerance_profile="pinned")
