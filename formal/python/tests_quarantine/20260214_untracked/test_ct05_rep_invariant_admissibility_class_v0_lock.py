...
def test_ct05_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "CT-05_rep_invariant_admissibility_class_v0.md"

    rec = ct05_rep_invariant_admissibility_class_v0_record(date="2026-02-09", tolerance_profile="pinned")
    expected = render_ct05_lock_markdown(rec)
