...
def test_rl09_surface_contract_top_level_keys_frozen():
    rec = rl09_dispersion_topology_bridge_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-09_dispersion_topology_bridge_comparator/v0"
    assert payload["observable_id"] == "OV-RL-09"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-09"
...
def test_rl09_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl09_dispersion_topology_bridge_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_rl09_scope_limits_include_discriminative_candidate():
    rec = rl09_dispersion_topology_bridge_v0_record(date="2026-02-09", tolerance_profile="pinned")
