...
ALLOWED_REASON_CODES = {
    ...
    "FAIL_ADMISSIBILITY_MISMATCH",
    "FAIL_ADMISSIBILITY_NOT_DETECTED",
    "FAIL_BOUND_STATUS_MISMATCH",
    "FAIL_BOUND_RESIDUAL",
    "FAIL_REP_BREAK_NOT_DETECTED",
    "FAIL_NONDETERMINISTIC_FINGERPRINT",
}


def test_ct05_surface_contract_top_level_keys_frozen():
    rec = ct05_rep_invariant_admissibility_class_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "CT-05_rep_invariant_admissibility_class_comparator/v0"
    assert payload["observable_id"] == "CT-05"
    assert payload["domain_id"] == "CT-DOMAIN-05"
...
def test_ct05_surface_contract_row_keys_and_reason_codes_frozen():
    rec = ct05_rep_invariant_admissibility_class_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 3
...
def test_ct05_scope_limits_include_discriminative_candidate():
    rec = ct05_rep_invariant_admissibility_class_v0_record(date="2026-02-09", tolerance_profile="pinned")
