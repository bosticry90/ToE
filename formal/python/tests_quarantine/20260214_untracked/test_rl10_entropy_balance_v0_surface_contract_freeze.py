...
def test_rl10_surface_contract_top_level_keys_frozen():
    rec = rl10_entropy_balance_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-10_entropy_balance_comparator/v0"
    assert payload["observable_id"] == "OV-RL-10"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-10"
...
def test_rl10_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl10_entropy_balance_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_rl10_scope_limits_include_discriminative_candidate():
    rec = rl10_entropy_balance_v0_record(date="2026-02-09", tolerance_profile="pinned")
