...
def test_cx01_lock_markdown_matches_record() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    lock_path = repo_root / "formal" / "markdown" / "locks" / "observables" / "CX-01_update_contractivity_v0.md"

    rec = cx01_update_contractivity_v0_record(date="2026-02-11", tolerance_profile="pinned")
    expected = render_cx01_lock_markdown(rec)
