...
from formal.python.toe.comparators.rl04_continuity_v0 import rl04_continuity_v0_record
...
def test_rl04_surface_contract_top_level_keys_frozen():
    rec = rl04_continuity_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-04_continuity_comparator/v0"
    assert payload["observable_id"] == "OV-RL-04"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-04"
...
def test_rl04_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl04_continuity_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 1
...
def test_rl04_scope_limits_include_discriminative_candidate():
    rec = rl04_continuity_v0_record(date="2026-02-09", tolerance_profile="pinned")
