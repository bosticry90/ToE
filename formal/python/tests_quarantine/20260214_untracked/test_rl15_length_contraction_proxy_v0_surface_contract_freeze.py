...
ALLOWED_REASON_CODES = {
    ...
    "FAIL_LENGTH_CONTRACTION",
    "FAIL_BREAK_NOT_DETECTED",
    "FAIL_NONDETERMINISTIC_FINGERPRINT",
}


def test_rl15_surface_contract_top_level_keys_frozen():
    rec = rl15_length_contraction_proxy_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-15_length_contraction_proxy_comparator/v0"
    assert payload["observable_id"] == "OV-RL-15"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-15"
...
def test_rl15_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl15_length_contraction_proxy_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_rl15_scope_limits_include_discriminative_candidate():
    rec = rl15_length_contraction_proxy_v0_record(date="2026-02-09", tolerance_profile="pinned")
