...
def test_ct02_surface_contract_top_level_keys_frozen():
    rec = ct02_energy_causality_update_bounds_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "CT-02_energy_causality_update_bounds_comparator/v0"
    assert payload["observable_id"] == "CT-02"
    assert payload["domain_id"] == "CT-DOMAIN-02"
...
def test_ct02_surface_contract_row_keys_and_reason_codes_frozen():
    rec = ct02_energy_causality_update_bounds_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_ct02_scope_limits_include_discriminative_candidate():
    rec = ct02_energy_causality_update_bounds_v0_record(date="2026-02-09", tolerance_profile="pinned")
