...
def test_rl08_surface_contract_top_level_keys_frozen():
    rec = rl08_gauge_redundancy_invariance_v0_record(date="2026-02-09", tolerance_profile="pinned")
    payload = rec.to_jsonable()

    assert payload["schema"] == "OV-RL-08_gauge_redundancy_invariance_comparator/v0"
    assert payload["observable_id"] == "OV-RL-08"
    assert payload["domain_id"] == "DRBR-DOMAIN-RL-08"
...
def test_rl08_surface_contract_row_keys_and_reason_codes_frozen():
    rec = rl08_gauge_redundancy_invariance_v0_record(date="2026-02-09", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_rl08_scope_limits_include_discriminative_candidate():
    rec = rl08_gauge_redundancy_invariance_v0_record(date="2026-02-09", tolerance_profile="pinned")
