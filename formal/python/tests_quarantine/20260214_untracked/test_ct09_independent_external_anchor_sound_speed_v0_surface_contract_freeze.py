...
def test_ct09_surface_contract_top_level_keys_frozen():
    ...
    assert payload["schema"] == "CT-09_independent_external_anchor_sound_speed_comparator/v0"
    assert payload["observable_id"] == "CT-09"
    assert payload["domain_id"] == "CT-DOMAIN-09"
...
def test_ct09_surface_contract_row_keys_and_reason_codes_frozen():
    rec = ct09_independent_external_anchor_sound_speed_v0_record(date="2026-02-11", tolerance_profile="pinned")
    rows = list(rec.rows)
    assert len(rows) == 2
...
def test_ct09_scope_limits_include_external_anchor_only():
    rec = ct09_independent_external_anchor_sound_speed_v0_record(date="2026-02-11", tolerance_profile="pinned")
    ...
    assert "external_anchor_only" in limits
