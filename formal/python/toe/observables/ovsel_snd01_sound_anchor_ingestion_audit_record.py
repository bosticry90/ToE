"""OV-SEL-SND-01: sound-anchor ingestion audit (computed lock).

Purpose
- Audit that adding the OV-SND-01 (symbolic) and OV-SND-01N (digitized) anchor
  does not trigger unintended policy/regime/selection consequences.

Design
- Deterministic.
- Checks:
  - OV-SND-01 and OV-SND-01N locks are lock==computed.
  - OV-SND-01N CSV/metadata exist.
  - Canonical governance locks remain lock==computed (OV-SEL-01/02, OV-DQ-03).
  - Negative check: governance/selection locks do not reference OV-SND-*.
...
class OVSELSND01SoundAnchorIngestionAuditRecord:
    schema: str
    status_date: str
...
def ovsel_snd01_sound_anchor_ingestion_audit_record(
    *,
    status_date: str = "2026-01-24",
) -> OVSELSND01SoundAnchorIngestionAuditRecord:
    ...
    checks: dict[str, Any] = {
        "OV-SND-01 lock": _check_lock_matches(
            repo_root=repo_root,
        ...
        "OV-SND-01N lock": _check_lock_matches(
            repo_root=repo_root,
        ...
        "No OV-SND token in OV-SEL-01": _check_no_token(
            repo_root=repo_root,
            lock_rel_path="formal/markdown/locks/observables/OV-SEL-01_selection_status.md",
            token="OV-SND-",
        ),
        "No OV-SND token in OV-XD-04": _check_no_token(
            repo_root=repo_root,
    ...
    anchor = {
        ...
        "digitization_id": snd01n.digitization_id,
        ...
        "digitized_csv_relpath": snd01n.dataset["csv_relpath"],
        "digitized_csv_sha256": snd01n.dataset["csv_sha256"],
        "benchmark_only": True,
    }
    ...
    narrative = (
        ...
        f"\nSelf-checks (lock==computed + file existence) all_ok={bool(all_ok)}."
    )

    return OVSELSND01SoundAnchorIngestionAuditRecord(
...
def render_ovsel_snd01_lock_markdown(record: OVSELSND01SoundAnchorIngestionAuditRecord) -> str:
...
def write_ovsel_snd01_lock(*, lock_path: Path | None = None, status_date: str = "2026-01-24") -> Path:
    repo_root = _find_repo_root(Path(__file__))
    out = lock_path
    if out is None:
        out = (
            ...
            / "locks"
            / "observables"
            / "OV-SEL-SND-01_sound_anchor_ingestion_audit.md"
        )

    rec = ovsel_snd01_sound_anchor_ingestion_audit_record(status_date=str(status_date))

    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(render_ovsel_snd01_lock_markdown(rec), encoding="utf-8")
    return out
