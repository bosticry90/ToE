"""OV-SEL-SND-05: multi-density constancy check audit (computed lock).

Purpose
- Audit that adding the multi-condition scaffolding records (coverage report,
  cross-source mapping record, and OV-SND-04 constancy check) does not trigger
  unintended policy/regime/selection consequences.

Design
- Deterministic.
- Checks:
  - New locks are lock==computed.
  - Canonical governance locks remain lock==computed (OV-SEL-01/02, OV-DQ-03).
  - Negative checks: governance/selection locks do not reference OV-SND-04 or OV-BR-SND-02.
...
class OVSELSND05MultiDensityConstancyAuditRecord:
    schema: str
    status_date: str
...
def ovsel_snd05_multi_density_constancy_audit_record(
    *,
    ...
    snd04 = ovsnd04_sound_speed_density_constancy_record(date=str(status_date))
    ...
    checks: dict[str, Any] = {
        "OV-SND-03N-COV": _check_lock_matches(
            repo_root=repo_root,
            lock_rel_path="formal/markdown/locks/observables/OV-SND-03N_density_coverage_report.md",
            computed_payload=cov.to_jsonable(),
        ),
        "OV-BR-SND-02": _check_lock_matches(
            repo_root=repo_root,
            lock_rel_path="formal/markdown/locks/observables/OV-BR-SND-02_cross_source_density_mapping.md",
            computed_payload=br.to_jsonable(),
        ),
        "OV-SND-04": _check_lock_matches(
            repo_root=repo_root,
            lock_rel_path="formal/markdown/locks/observables/OV-SND-04_sound_speed_density_constancy.md",
            computed_payload=snd04.to_jsonable(),
        ),
        ...
        "No OV-SND-04 token in OV-SEL-01": _check_no_token(
            repo_root=repo_root,
            lock_rel_path="formal/markdown/locks/observables/OV-SEL-01_selection_status.md",
            token="OV-SND-04",
        ),
        "No OV-BR-SND-02 token in OV-SEL-01": _check_no_token(
            repo_root=repo_root,
            lock_rel_path="formal/markdown/locks/observables/OV-SEL-01_selection_status.md",
            token="OV-BR-SND-02",
        ),
        "No OV-SND-04 token in OV-XD-04": _check_no_token(
            repo_root=repo_root,
            lock_rel_path=str(sel01.ovxd04_overlap_only_comparison["lock_path"]),
            token="OV-SND-04",
        ),
        "No OV-BR-SND-02 token in OV-XD-04": _check_no_token(
            repo_root=repo_root,
            lock_rel_path=str(sel01.ovxd04_overlap_only_comparison["lock_path"]),
            token="OV-BR-SND-02",
        ),
    ...
    return OVSELSND05MultiDensityConstancyAuditRecord(
...
def render_ovsel_snd05_lock_markdown(record: OVSELSND05MultiDensityConstancyAuditRecord) -> str:
...
def write_ovsel_snd05_lock(*, lock_path: Path | None = None, status_date: str = "2026-01-24") -> Path:
    ...
    rec = ovsel_snd05_multi_density_constancy_audit_record(status_date=str(status_date))

    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(render_ovsel_snd05_lock_markdown(rec), encoding="utf-8")
    return out
