...
CT07_TOLERANCE_PROFILE_ENV = "TOE_CT07_TOLERANCE_PROFILE"

CT07_TOLERANCE_PROFILES: dict[str, dict[str, float]] = {
    "pinned": {
        "eps_rmse_kHz": 0.10,
    ...
    "portable": {
        "eps_rmse_kHz": 0.14,
        "eps_max_abs_error_kHz": 0.28,
        "eps_reduced_chi2": 1.6,
    },
}
...
def _default_artifact_dir(repo_root: Path) -> Path:
    return repo_root / "formal" / "external_evidence" / "ct07_external_anchor_dispersion_lowk_domain_01"
...
class CT07ExternalAnchorLowKCase:
    case_id: str
    kind: str
    model_tag: str
    c_s2_kHz2_um2: float
    ...
    def to_jsonable(self) -> dict[str, Any]:
        return {
            ...
            "c_s2_scale": float(self.c_s2_scale),
...
@dataclass(frozen=True)
class CT07ExternalAnchorLowKReport:
    schema: str
    config_tag: str
    regime_tag: str
    domain_tag: str
    ...
    boundary: str
    cases: list[CT07ExternalAnchorLowKCase]
...
def _load_ct07_report_artifact(path: Path) -> tuple[CT07ExternalAnchorLowKReport, bool]:
    payload = _load_json(path)
    if str(payload.get("schema", "")) != "CT-07/external_anchor_dispersion_lowk_front_door_report/v1":
        raise ValueError(f"Unexpected CT-07 report schema in {path}: {payload.get('schema')!r}")
    ...
    report = CT07ExternalAnchorLowKReport(
        schema=str(payload["schema"]),
        config_tag=str(payload["config_tag"]),
        regime_tag=str(payload["regime_tag"]),
        domain_tag=str(payload["domain_tag"]),
        params={
            "origin_dataset_id": str(payload["params"]["origin_dataset_id"]),
            "origin_dataset_csv_relpath": str(payload["params"]["origin_dataset_csv_relpath"]),
            "origin_dataset_csv_sha256": str(payload["params"]["origin_dataset_csv_sha256"]),
            ...
            "k_quantile": float(payload["params"]["k_quantile"]),
            "k_max_um_inv": float(payload["params"]["k_max_um_inv"]),
            "n_slice_points": int(payload["params"]["n_slice_points"]),
            "c_s2_scale_negative": float(payload["params"]["c_s2_scale_negative"]),
            ...
            "non_independent_of_ct06": bool(payload["params"]["non_independent_of_ct06"]),
        },
...
def ct07_v0_tolerance_profile_from_env(env: Mapping[str, str] | None = None) -> str:
    src = env if env is not None else os.environ
    profile = str(src.get(CT07_TOLERANCE_PROFILE_ENV, "pinned")).strip().lower()
    if profile not in CT07_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(CT07_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported {CT07_TOLERANCE_PROFILE_ENV} profile '{profile}'. Allowed: {allowed}")
    return profile


def ct07_v0_tolerances(profile: str) -> dict[str, float]:
    p = str(profile).strip().lower()
    if p not in CT07_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(CT07_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported tolerance profile '{profile}'. Allowed: {allowed}")
    return dict(CT07_TOLERANCE_PROFILES[p])


def ct07_compare_surfaces(
    reference: CT07ExternalAnchorLowKReport,
    candidate: CT07ExternalAnchorLowKReport,
    *,
    tolerances: Mapping[str, float],
    deterministic_ok: bool = True,
) -> list[dict[str, Any]]:
    if not isinstance(reference, CT07ExternalAnchorLowKReport):
        raise TypeError("reference must be a CT07ExternalAnchorLowKReport")
    if not isinstance(candidate, CT07ExternalAnchorLowKReport):
        raise TypeError("candidate must be a CT07ExternalAnchorLowKReport")
    ...
    for ref_case, cand_case in zip(reference.cases, candidate.cases, strict=True):
        ...
        if cand_case.kind == "positive_control":
            ...
            if not reasons:
                reasons = ["ct07_external_anchor_lowk_fit_ok"]
                passed = True
            else:
                passed = False
            ...
            if not reasons:
                reasons = ["ct07_external_anchor_lowk_failure_detected"]
                passed = True
            else:
                passed = False
        else:
        ...
        rows.append(
            {
                "artifact_id": f"CT07_EXTERNAL_ANCHOR_LOWK_{cand_case.case_id}",
                "source": {
                ...
                "metric_vector": {
                    ...
                    "c_s2_scale": float(cand_case.c_s2_scale),
...
@dataclass(frozen=True)
class CT07ExternalAnchorLowKV0Record:
    schema: str
    date: str
    observable_id: str
    domain_id: str
...
def ct07_external_anchor_dispersion_lowk_slice_v0_record(
    *,
    date: str = "2026-02-10",
    tolerance_profile: str | None = None,
    artifact_dir: Path | None = None,
    env: Mapping[str, str] | None = None,
) -> CT07ExternalAnchorLowKV0Record:
    profile = str(tolerance_profile).strip().lower() if tolerance_profile is not None else ct07_v0_tolerance_profile_from_env(env)
    tolerances = ct07_v0_tolerances(profile)

    repo_root = _find_repo_root(Path(__file__))
    data_dir = (artifact_dir or _default_artifact_dir(repo_root)).resolve()
    ref_path = data_dir / "ct07_reference_report.json"
    cand_path = data_dir / "ct07_candidate_report.json"
    missing = [str(p) for p in [ref_path, cand_path] if not p.exists()]
    if missing:
        return CT07ExternalAnchorLowKV0Record(
    ...
    reference, ref_deterministic = _load_ct07_report_artifact(ref_path)
    candidate, cand_deterministic = _load_ct07_report_artifact(cand_path)
    deterministic_ok = bool(ref_deterministic and cand_deterministic)

    rows = ct07_compare_surfaces(
        reference,
        candidate,
...
def render_ct07_lock_markdown(record: CT07ExternalAnchorLowKV0Record) -> str:
    ...
    return (
        ...
        "- Outputs: `formal/external_evidence/ct07_external_anchor_dispersion_lowk_domain_01/ct07_reference_report.json`, "
