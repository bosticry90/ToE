...
RL15_TOLERANCE_PROFILE_ENV = "TOE_RL15_TOLERANCE_PROFILE"

RL15_TOLERANCE_PROFILES: dict[str, dict[str, float]] = {
    "pinned": {
        "eps_contraction": 1e-8,
        "eps_break": 1e-3,
    },
    "portable": {
        "eps_contraction": 1e-6,
        "eps_break": 1e-3,
    },
}
...
def _default_artifact_dir(repo_root: Path) -> Path:
    return repo_root / "formal" / "external_evidence" / "rl15_length_contraction_proxy_domain_01"
...
class RL15ContractionCase:
    ...
    def to_jsonable(self) -> dict[str, Any]:
        return {
            ...
            "L0": float(self.L0),
            "L": float(self.L),
            "contraction_ratio": float(self.contraction_ratio),
            "contraction_residual": float(self.contraction_residual),
        }


@dataclass(frozen=True)
class RL15ContractionReport:
    schema: str
    config_tag: str
    regime_tag: str
    domain_tag: str
    ...
    boundary: str
    cases: list[RL15ContractionCase]
...
def _load_rl15_report_artifact(path: Path) -> tuple[RL15ContractionReport, bool]:
    payload = _load_json(path)
    if str(payload.get("schema", "")) != "RL/length_contraction_front_door_report/v1":
        raise ValueError(f"Unexpected RL15 report schema in {path}: {payload.get('schema')!r}")
    ...
    report = RL15ContractionReport(
        ...
        params={
            ...
            "L0": float(payload["params"]["L0"]),
            "eps_contraction": float(payload["params"]["eps_contraction"]),
...
def rl15_v0_tolerance_profile_from_env(env: Mapping[str, str] | None = None) -> str:
    src = env if env is not None else os.environ
    profile = str(src.get(RL15_TOLERANCE_PROFILE_ENV, "pinned")).strip().lower()
    if profile not in RL15_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(RL15_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported {RL15_TOLERANCE_PROFILE_ENV} profile '{profile}'. Allowed: {allowed}")
    return profile


def rl15_v0_tolerances(profile: str) -> dict[str, float]:
    p = str(profile).strip().lower()
    if p not in RL15_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(RL15_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported tolerance profile '{profile}'. Allowed: {allowed}")
    return dict(RL15_TOLERANCE_PROFILES[p])


def rl15_compare_surfaces(
    reference: RL15ContractionReport,
    candidate: RL15ContractionReport,
    *,
    tolerances: Mapping[str, float],
    deterministic_ok: bool = True,
) -> list[dict[str, Any]]:
    if not isinstance(reference, RL15ContractionReport):
        raise TypeError("reference must be a RL15ContractionReport")
    if not isinstance(candidate, RL15ContractionReport):
        raise TypeError("candidate must be a RL15ContractionReport")
    ...
    for ref_case, cand_case in zip(reference.cases, candidate.cases, strict=True):
        ...
        contraction_residual = abs(cand_case.contraction_ratio - expected_ratio)
        if abs(contraction_residual - float(cand_case.contraction_residual)) > 1e-12:
            reasons.append("FAIL_DOMAIN_PARAMETER_INCONSISTENT")

        eps_contraction = float(tolerances["eps_contraction"])
        eps_break = float(tolerances["eps_break"])
        if cand_case.kind == "positive_control":
            if contraction_residual > eps_contraction:
                reasons.append("FAIL_LENGTH_CONTRACTION")
            if not reasons:
                reasons = ["rl15_length_contraction_ok"]
                passed = True
            else:
                passed = False
        else:
            if contraction_residual < eps_break:
                reasons.append("FAIL_BREAK_NOT_DETECTED")
            if not reasons:
                reasons = ["rl15_length_contraction_break_detected"]
                passed = True
            else:
                passed = False
        ...
        rows.append(
            {
                "artifact_id": f"RL15_CONTRACTION_{cand_case.case_id}",
                "source": {
                ...
                "metric_vector": {
                    ...
                    "L0": float(cand_case.L0),
                    "L": float(cand_case.L),
                    "contraction_ratio": float(cand_case.contraction_ratio),
                    "expected_ratio": float(expected_ratio),
                    "contraction_residual": float(contraction_residual),
...
@dataclass(frozen=True)
class RL15LengthContractionV0Record:
    schema: str
    date: str
    observable_id: str
    domain_id: str
...
def rl15_length_contraction_proxy_v0_record(
    *,
    date: str = "2026-02-09",
    tolerance_profile: str | None = None,
    artifact_dir: Path | None = None,
    env: Mapping[str, str] | None = None,
) -> RL15LengthContractionV0Record:
    profile = str(tolerance_profile).strip().lower() if tolerance_profile is not None else rl15_v0_tolerance_profile_from_env(env)
    tolerances = rl15_v0_tolerances(profile)

    repo_root = _find_repo_root(Path(__file__))
    data_dir = (artifact_dir or _default_artifact_dir(repo_root)).resolve()
    ref_path = data_dir / "rl15_reference_report.json"
    cand_path = data_dir / "rl15_candidate_report.json"
    missing = [str(p) for p in [ref_path, cand_path] if not p.exists()]
    if missing:
        return RL15LengthContractionV0Record(
    ...
    reference, ref_deterministic = _load_rl15_report_artifact(ref_path)
    candidate, cand_deterministic = _load_rl15_report_artifact(cand_path)
    deterministic_ok = bool(ref_deterministic and cand_deterministic)

    rows = rl15_compare_surfaces(
        reference,
        candidate,
...
def render_rl15_lock_markdown(record: RL15LengthContractionV0Record) -> str:
    ...
    return (
        ...
        "- Outputs: `formal/external_evidence/rl15_length_contraction_proxy_domain_01/rl15_reference_report.json`, "
