...
def _load_ct01_report_artifact(path: Path) -> tuple[CT01PropagationReport, bool]:
    payload = _load_json(path)
    if str(payload.get("schema", "")) != "CT-01/no_superluminal_propagation_front_door_report/v1":
        raise ValueError(f"Unexpected CT-01 report schema in {path}: {payload.get('schema')!r}")
    ...
    report = CT01PropagationReport(
        ...
        params={
            ...
            "c_cone": float(payload["params"]["c_cone"]),
            "amplitude": float(payload["params"]["amplitude"]),
            "sigma": float(payload["params"]["sigma"]),
            "x0": float(payload["params"]["x0"]),
            "delta_x": float(payload["params"]["delta_x"]),
            "support_radius": float(payload["params"]["support_radius"]),
            "u_threshold": float(payload["params"]["u_threshold"]),
            "eps_ct01": float(payload["params"]["eps_ct01"]),
...
def ct01_v0_tolerance_profile_from_env(env: Mapping[str, str] | None = None) -> str:
    ...
    profile = str(src.get(CT01_TOLERANCE_PROFILE_ENV, "pinned")).strip().lower()
    if profile not in CT01_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(CT01_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported {CT01_TOLERANCE_PROFILE_ENV} profile '{profile}'. Allowed: {allowed}")
    return profile


def ct01_v0_tolerances(profile: str) -> dict[str, float]:
    p = str(profile).strip().lower()
    if p not in CT01_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(CT01_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported tolerance profile '{profile}'. Allowed: {allowed}")
    return dict(CT01_TOLERANCE_PROFILES[p])
...
def ct01_compare_surfaces(
    ...
    *,
    ...
    if not isinstance(reference, CT01PropagationReport):
        raise TypeError("reference must be a CT01PropagationReport")
    if not isinstance(candidate, CT01PropagationReport):
        raise TypeError("candidate must be a CT01PropagationReport")
    ...
    for ref_case, cand_case in zip(reference.cases, candidate.cases, strict=True):
        ...
        c_cone = float(reference.params["c_cone"])

        if cand_case.kind == "positive_control":
            if not cand_case.crossed:
                reasons.append("FAIL_NO_CROSSING")
            if cand_case.v_emp > c_cone + eps_ct01:
                reasons.append("FAIL_SUPERLUMINAL")
            if not reasons:
                reasons = ["ct01_within_cone"]
                passed = True
            else:
                passed = False
        else:
            if cand_case.v_emp < c_cone + eps_break:
                reasons.append("FAIL_BREAK_NOT_DETECTED")
            if not reasons:
                reasons = ["ct01_superluminal_detected"]
                passed = True
            else:
                passed = False
...
@dataclass(frozen=True)
class CT01NoSuperluminalV0Record:
    schema: str
    date: str
    observable_id: str
    domain_id: str
...
def ct01_no_superluminal_v0_record(
    *,
...
) -> CT01NoSuperluminalV0Record:
    profile = str(tolerance_profile).strip().lower() if tolerance_profile is not None else ct01_v0_tolerance_profile_from_env(env)
    tolerances = ct01_v0_tolerances(profile)

    repo_root = _find_repo_root(Path(__file__))
    data_dir = (artifact_dir or _default_artifact_dir(repo_root)).resolve()
    ref_path = data_dir / "ct01_reference_report.json"
    cand_path = data_dir / "ct01_candidate_report.json"
    missing = [str(p) for p in [ref_path, cand_path] if not p.exists()]
    if missing:
        return CT01NoSuperluminalV0Record(
    ...
    reference, ref_deterministic = _load_ct01_report_artifact(ref_path)
    candidate, cand_deterministic = _load_ct01_report_artifact(cand_path)
    deterministic_ok = bool(ref_deterministic and cand_deterministic)

    rows = ct01_compare_surfaces(
        reference,
        candidate,
...
def render_ct01_lock_markdown(record: CT01NoSuperluminalV0Record) -> str:
