...
CT08_TOLERANCE_PROFILE_ENV = "TOE_CT08_TOLERANCE_PROFILE"

CT08_TOLERANCE_PROFILES: dict[str, dict[str, float]] = {
    "pinned": {
        "eps_rmse_kHz": 0.25,
...
class CT08ExternalAnchorHighKCase:
    case_id: str
    kind: str
    model_tag: str
    c_s2_kHz2_um2: float
...
@dataclass(frozen=True)
class CT08ExternalAnchorHighKReport:
    schema: str
    config_tag: str
    regime_tag: str
    domain_tag: str
    ...
    boundary: str
    cases: list[CT08ExternalAnchorHighKCase]
...
def _load_ct08_report_artifact(path: Path) -> tuple[CT08ExternalAnchorHighKReport, bool]:
    payload = _load_json(path)
    if str(payload.get("schema", "")) != "CT-08/external_anchor_dispersion_highk_front_door_report/v1":
        raise ValueError(f"Unexpected CT-08 report schema in {path}: {payload.get('schema')!r}")
    ...
    cases = [
        CT08ExternalAnchorHighKCase(
            ...
            alpha_scale=float(case["alpha_scale"]),
    ...
    report = CT08ExternalAnchorHighKReport(
        ...
        params={
            ...
            "k_min_um_inv": float(payload["params"]["k_min_um_inv"]),
...
def ct08_v0_tolerance_profile_from_env(env: Mapping[str, str] | None = None) -> str:
    src = env if env is not None else os.environ
    profile = str(src.get(CT08_TOLERANCE_PROFILE_ENV, "pinned")).strip().lower()
    if profile not in CT08_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(CT08_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported {CT08_TOLERANCE_PROFILE_ENV} profile '{profile}'. Allowed: {allowed}")
    return profile


def ct08_v0_tolerances(profile: str) -> dict[str, float]:
    p = str(profile).strip().lower()
    if p not in CT08_TOLERANCE_PROFILES:
        allowed = ", ".join(sorted(CT08_TOLERANCE_PROFILES.keys()))
        raise ValueError(f"Unsupported tolerance profile '{profile}'. Allowed: {allowed}")
    return dict(CT08_TOLERANCE_PROFILES[p])


def ct08_compare_surfaces(
    reference: CT08ExternalAnchorHighKReport,
    candidate: CT08ExternalAnchorHighKReport,
    *,
    tolerances: Mapping[str, float],
    deterministic_ok: bool = True,
) -> list[dict[str, Any]]:
    if not isinstance(reference, CT08ExternalAnchorHighKReport):
        raise TypeError("reference must be a CT08ExternalAnchorHighKReport")
    if not isinstance(candidate, CT08ExternalAnchorHighKReport):
        raise TypeError("candidate must be a CT08ExternalAnchorHighKReport")
    ...
    for ref_case, cand_case in zip(reference.cases, candidate.cases, strict=True):
        ...
        if cand_case.kind == "positive_control":
            ...
            if not reasons:
                reasons = ["ct08_external_anchor_highk_fit_ok"]
                passed = True
            else:
                passed = False
            ...
            if not reasons:
                reasons = ["ct08_external_anchor_highk_failure_detected"]
                passed = True
            else:
                passed = False
        else:
        ...
        rows.append(
            {
                "artifact_id": f"CT08_EXTERNAL_ANCHOR_HIGHK_{cand_case.case_id}",
                "source": {
                ...
                "metric_vector": {
                    ...
                    "alpha_scale": float(cand_case.alpha_scale),
...
@dataclass(frozen=True)
class CT08ExternalAnchorHighKV0Record:
    schema: str
    date: str
    observable_id: str
    domain_id: str
...
def ct08_external_anchor_dispersion_highk_slice_v0_record(
    *,
    date: str = "2026-02-10",
    tolerance_profile: str | None = None,
    artifact_dir: Path | None = None,
    env: Mapping[str, str] | None = None,
) -> CT08ExternalAnchorHighKV0Record:
    profile = str(tolerance_profile).strip().lower() if tolerance_profile is not None else ct08_v0_tolerance_profile_from_env(env)
    tolerances = ct08_v0_tolerances(profile)

    repo_root = _find_repo_root(Path(__file__))
    data_dir = (artifact_dir or _default_artifact_dir(repo_root)).resolve()
    ref_path = data_dir / "ct08_reference_report.json"
    cand_path = data_dir / "ct08_candidate_report.json"
    missing = [str(p) for p in [ref_path, cand_path] if not p.exists()]
    if missing:
        return CT08ExternalAnchorHighKV0Record(
    ...
    reference, ref_deterministic = _load_ct08_report_artifact(ref_path)
    candidate, cand_deterministic = _load_ct08_report_artifact(cand_path)
    deterministic_ok = bool(ref_deterministic and cand_deterministic)

    rows = ct08_compare_surfaces(
        reference,
        candidate,
...
def render_ct08_lock_markdown(record: CT08ExternalAnchorHighKV0Record) -> str:
    ...
    return (
        ...
        "- Outputs: `formal/external_evidence/ct08_external_anchor_dispersion_highk_domain_01/ct08_reference_report.json`, "
