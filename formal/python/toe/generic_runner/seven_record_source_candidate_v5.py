...
def compute(root=c.norm.ROOT):
    ...
    for row in rv:
        key=row['record_id']
        stage(key+'.PHASE_DERIVATION','SOURCE_FEYNMAN_PHASES',['RV.BOUND_SOURCE'],row['phase'])
        ...
        stage(key+'.COEFFICIENT_DERIVATION','COVARIANT_CONTRACTION_AND_IDENTITY_NORMALIZATION',
              ['RV.BOUND_SOURCE',key+'.GROUP_DERIVATION',key+'.SPINOR_DERIVATION',key+'.PHASE_DERIVATION'],row['normalization'])
    ...
    return dict(schema_id='SEVEN_RECORD_SOURCE_EXECUTION_COMPONENT_PACKET_v5',
