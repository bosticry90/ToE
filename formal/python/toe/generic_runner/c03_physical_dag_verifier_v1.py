"""Separate operation implementation for the C03 physical fragment.
...
def operation(key, parents):
    suffix = key.removeprefix(p.PREFIX)
    detail = {}
...
def verify_material(packet, material, source_reads):
    """Core verifier. Production entry below loads its own trusted material.

    Explicit alternate material is reserved for decoded-input unit tests; it
    cannot confer frozen-source admission or a full source-sensitivity gate.
    """
    require(set(packet) == {'schema_id', 'graph', 'outputs', 'scope'} and packet['schema_id'] == p.SCHEMA
            and packet['scope'] == 'C03_PHYSICAL_FRAGMENT_ONLY', 'PACKET_SCHEMA')
    graph = packet['graph']
    require(set(graph) == {'nodes', 'edges', 'node_count', 'edge_count', 'canonical_digest'}, 'GRAPH_SCHEMA')
    nodes, order = x.graph_structure(graph, p.structural_contract())
    specs = p.derived_specs()
    require(set(nodes) == set(material) | set(specs), 'EXACT_NODE_PROFILE')
    values, receipts = {}, []
    fields = set(p.structural_contract()['node_schema']['required_fields'])
    for key in order:
        node = nodes[key]
        require(set(node) == fields, 'UNDECLARED_NODE_FIELD', key)
        detail = {}
        if key in material:
            expected = material[key]
            require(not node['parents'], 'SOURCE_HAS_PARENTS', key)
            for field in ('kind', 'operation', 'semantic_type', 'evidence_refs', 'typed_value'):
                require(x.canonical(node[field]) == x.canonical(expected[field]), 'SOURCE_BINDING_OR_VALUE_MISMATCH', key + ':' + field)
            require(node['epistemic_status'] == 'SOURCE_BOUND', 'SOURCE_EPISTEMIC_STATUS')
            raw = expected['typed_value']
            actual = x.typed_decode(expected['semantic_type'], raw) if expected['semantic_type'] in x.SCALARS else raw
        else:
            for field, expected in specs[key].items():
                require(node[field] == expected, 'OPERATION_SIGNATURE_MISMATCH', key + ':' + field)
            require(node['evidence_refs'] == [] and node['epistemic_status'] == 'DERIVED_CHECKABLE', 'DERIVED_EVIDENCE_OR_STATUS')
            actual, detail = operation(key, [values[parent] for parent in node['parents']])
            require(x.exact_equal(actual, x.typed_decode(node['semantic_type'], node['typed_value'])), 'RECOMPUTED_INTERMEDIATE_MISMATCH', key)
        values[key] = actual
        receipts.append(dict(node_id=key, evaluated_value_digest=x.digest(encode(actual), 'C03_RECOMPUTED_VALUE_v1'),
                             status='RESOLVED_OR_RECOMPUTED_AND_EQUAL', detail=detail))
    require(set(packet['outputs']) == {p.ROOT_ID} and x.exact_equal(values[p.ROOT_ID], E(packet['outputs'][p.ROOT_ID])), 'EMITTED_COEFFICIENT_MISMATCH')
    return dict(status='C03_PHYSICAL_FRAGMENT_RECOMPUTED', node_count=len(nodes), edge_count=graph['edge_count'],
                receipts=receipts, source_reads=source_reads,
                shared_premises='Pinned source definitions, exact algebra library, analytic UV master identity and I/O profile.',
                candidate_routines_called=False, non_author_review=False,
                full_seven_record_dag=False, scientific_requalification=False)


def verify(packet, root=p.norm.ROOT):
    material, reads = p.source_material(root)
    return verify_material(packet, material, reads)
