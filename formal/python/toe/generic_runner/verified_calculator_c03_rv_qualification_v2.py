"""Execute the amended v6 exact-evidence lineage without scientific promotion."""
...
from formal.python.toe.generic_runner.verified_calculator.dependency_closure import generate_dependency_closure_v2, validate_dependency_closure_v2
from formal.python.toe.generic_runner.verified_calculator.errors import require
from formal.python.toe.generic_runner.verified_calculator.independent import run_julia_independent, run_lean_qualification_envelope_checker
from formal.python.toe.generic_runner.verified_calculator_c03_rv_candidate_v2 import candidate
from formal.python.toe.generic_runner.verified_calculator_c03_rv_census_v1 import scientific_authority_binding


REPOSITORY_ROOT = Path(__file__).resolve().parents[4]
DEFAULT_FREEZE_DIRECTORY = REPOSITORY_ROOT / "formal" / "docs" / "release" / "verified_calculator" / "c03_rv_exact_v6"
...
def qualify(destination: Path = DEFAULT_FREEZE_DIRECTORY) -> dict[str, Any]:
    ...
    require(len(run.evaluation.receipts) == len(run.evaluation.type_signature_receipts) == 207 and len(run.evaluation.outputs) == 16, "C03_RV_V6_QUALIFICATION_CENSUS")
    julia = run_julia_independent(run)
    specs = mandatory_challenge_specs()
    challenge_results = api.run_challenges(run, specs)
    lean = run_lean_qualification_envelope_checker(run, julia, specs, challenge_results)
    receipt = api.verify_run(
        run, challenge_results=challenge_results, challenge_specs=specs,
        julia_evidence=julia, lean_evidence=lean,
    )
    ...
    closure = generate_dependency_closure_v2(REPOSITORY_ROOT)
    validate_dependency_closure_v2(closure)
    bundle = api.assemble_evidence_bundle(
        run, receipt, challenge_specs=specs, julia_evidence=julia,
        lean_evidence=lean, dependency_manifests=(closure,),
        authority_bindings=(authority,), authority_attachments=(attachment,),
    )
    ...
    return {
        "schema_id": "C03RVExactQualificationReplayV2",
        ...
        "lean_qualification_envelope_hash": lean.accepted_envelope_hash,
        ...
        "type_signature_receipt_count": len(run.evaluation.type_signature_receipts),
        ...
        "dependency_custody_observation_hash": closure["custody_observation_hash"],
