...
/- Flat token mirror for substring-based EM U1 micro-gates. The executable
Lean token values below use line continuations in a few long strings; these
comments pin the exact one-line audit tokens without changing semantics.
