...
/-
  Local structural scaffolding for Parent P1.
  This file’s goal is to lock symbolic structure (not analytic correctness),
  so we provide minimal placeholders and plane-wave action rules.
-/

abbrev FieldC : Type := ℝ → ℝ → ℂ   -- (t, x) ↦ ℂ

-- Abstract derivative operators (placeholders)
opaque Dt : FieldC → FieldC
opaque Dxx : FieldC → FieldC
opaque Dxxxx : FieldC → FieldC
opaque Dxxxxxx : FieldC → FieldC

-- 1D plane-wave ansatz ψ(t,x) = exp(i(kx - ωt)) (amplitude can be added later if needed)
def planeWave (k ω : ℝ) : FieldC :=
  fun t x => Complex.exp (Complex.I * ((k * x - ω * t : ℝ) : ℂ))

-- Locked “eigenfunction” action rules for the abstract operators on plane waves.
-- These are axioms in this structural layer.
axiom Dt_planeWave (k ω : ℝ) :
  Dt (planeWave k ω) = fun t x =>
    (-(Complex.I) * (ω : ℂ)) * (planeWave k ω t x)

axiom Dxx_planeWave (k ω : ℝ) :
  Dxx (planeWave k ω) = fun t x =>
    (-(k ^ 2 : ℝ) : ℂ) * (planeWave k ω t x)

axiom Dxxxx_planeWave (k ω : ℝ) :
  Dxxxx (planeWave k ω) = fun t x =>
    ((k ^ 4 : ℝ) : ℂ) * (planeWave k ω t x)

axiom Dxxxxxx_planeWave (k ω : ℝ) :
  Dxxxxxx (planeWave k ω) = fun t x =>
    (-(k ^ 6 : ℝ) : ℂ) * (planeWave k ω t x)
...
theorem dispersion_linear_P1 (p : P1Params) (k ω : ℝ)
  ...
  have hc : (ω : ℂ) = (omega_linear_P1 p k : ℂ) := by
    have h0'' := h0
    simp only [hDt00, hDxx00, hDxxxx00, hDxxxxxx00, hψ00] at h0''
    have h0' :
        (ω : ℂ) =
          (2⁻¹ : ℂ) * (k : ℂ) ^ 2
          + (p.lam : ℂ) * (k : ℂ) ^ 4
          + -((p.beta : ℂ) * (k : ℂ) ^ 6) := by
      simpa [hII] using h0''
    simpa [omega_linear_P1, sub_eq_add_neg, hg] using h0'
  exact_mod_cast hc
