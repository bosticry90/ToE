...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR covariant conservation proof-object attempt executed and identified an obstruction requiring refinement, with no proof object construction, conservation witness construction, source admissibility claim, Bianchi compatibility, empirical validation, or QFT-GR seam closure"
      retained_blocker :=
...
/-- Previous live target consumed by the QFT-GR covariant conservation proof-object attempt. -/
def previousLiveNextStrictTargetV0 : String :=
  "execute_qft_gr_covariant_conservation_proof_object_attempt"

/-- Current live target after QFT-GR covariant conservation proof-object attempt. -/
def currentLiveNextStrictTargetV0 : String :=
  "review_qft_gr_covariant_conservation_proof_object_attempt_result"
