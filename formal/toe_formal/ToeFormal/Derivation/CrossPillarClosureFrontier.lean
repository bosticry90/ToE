...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR state-expectation domain-link assumption-reduction packet prepared OD-ASSUMP-003 analysis only and selected packet result review, with no source admissibility claim, proof object construction, conservation witness construction, Bianchi compatibility, empirical validation, or QFT-GR seam closure"
      retained_blocker :=
...
/-- Previous live target consumed by the QFT-GR state-expectation domain-link packet. -/
def previousLiveNextStrictTargetV0 : String :=
  "prepare_qft_gr_state_expectation_domain_link_assumption_reduction_packet"

/-- Current live target after the QFT-GR state-expectation domain-link packet. -/
def currentLiveNextStrictTargetV0 : String :=
  "review_qft_gr_state_expectation_domain_link_assumption_reduction_packet_result"
