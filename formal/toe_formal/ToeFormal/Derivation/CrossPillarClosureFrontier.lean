...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR minimal working model bounded refinement attempt after repeated-inconclusive v3 conservation retest executed the v4 candidate refinement, recorded repeated_weak_divergence_undecided_under_candidate_pairing_domain_v3 as the remaining obstruction class, selected only attempt-result review, preserved candidate-only status and bounded validation-only discipline including standing aggregate Lean and release-index caveats, and preserved no conservation proof object or witness, no source admissibility, no Bianchi compatibility, no semiclassical Einstein equation, no empirical validation, no master-action promotion, no release claim, no public submission, and no QFT-GR closure"
      retained_blocker :=
