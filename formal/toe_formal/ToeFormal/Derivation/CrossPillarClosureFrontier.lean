...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR minimal working model post-retest refinement attempt executed only the authorized weak pairing domain, regularity assumptions, test-function class, candidate source definition, scope restriction, and obstruction-accounting dimensions; selected only the attempt-result review; preserved bounded validation-only discipline and the aggregate Lean caveat; and preserved no conservation rerun, no conservation claim, no conservation proof object or witness, no source admissibility, no Bianchi compatibility, no semiclassical Einstein equation, no empirical validation, no master-action promotion, no release claim, no public submission, and no QFT-GR seam closure"
      retained_blocker :=
...
/-- Previous live target consumed by the post-retest refinement attempt. -/
def previousLiveNextStrictTargetV0 : String :=
  "execute_qft_gr_minimal_working_model_refinement_attempt_after_conservation_retest"

/-- Current live target after the post-retest refinement attempt. -/
def currentLiveNextStrictTargetV0 : String :=
  "review_qft_gr_minimal_working_model_refinement_attempt_after_conservation_retest_result"
