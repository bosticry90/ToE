...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR RN-ASSUMP-002 renormalization-scope bounded attempt executed with reduced-pending-result-review classification only, with no renormalization scope discharge, conservation proof object, conservation witness, source admissibility, Bianchi compatibility, empirical validation, or QFT-GR seam closure"
      retained_blocker :=
...
/-- Previous live target consumed by the QFT-GR RN-ASSUMP-002 attempt. -/
def previousLiveNextStrictTargetV0 : String :=
  "execute_qft_gr_renormalization_scope_assumption_reduction_attempt"

/-- Current live target after the QFT-GR RN-ASSUMP-002 attempt. -/
def currentLiveNextStrictTargetV0 : String :=
  "review_qft_gr_renormalization_scope_assumption_reduction_attempt_result"
