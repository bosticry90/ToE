...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR minimal positive conservation witness attempt result review accepted the strict toy local weak-conservation bridge witness and authorized only a witness maturation packet; the accepted bridge remains local to residual zero plus supplied divergence identity plus allowed weak pairing plus compact-support/no-boundary assumptions implying weak conservation against allowed tests, carries weak_pairing_domain_obstruction as an unresolved dominant obstruction candidate with canonical id repeated_weak_divergence_undecided_under_candidate_pairing_domain_v3, blocks immediate retest, countermodel selection, source-map ladder selection, and ordinary model refinement at this checkpoint, preserves bounded validation-only discipline including standing aggregate Lean and release-index caveats, and preserves no broad QFT-GR conservation claim, no source admissibility, no Bianchi compatibility, no semiclassical Einstein equation, no empirical validation, no master-action promotion, no release claim, no public submission, and no QFT-GR closure"
      retained_blocker :=
  ...
  , { row := .masterAction
      current_strongest_surface :=
        "QFT-GR minimal positive conservation witness attempt result review accepted the strict toy local weak-conservation bridge witness and selected witness maturation packet preparation only; it preserves the unresolved dominant weak-pairing obstruction candidate, strict toy scope, candidate-only status, and bounded validation-only discipline, and makes no source-admissibility claim, broad conservation pass claim, broad conservation proof-object claim, broad conservation witness claim, Bianchi compatibility claim, semiclassical Einstein equation claim, QFT-GR closure, public-submission claim, scientific-validation claim, or master-action promotion"
      retained_blocker :=
...
/-- Previous live target consumed by the strict toy positive witness attempt result review. -/
def previousLiveNextStrictTargetV0 : String :=
  "review_qft_gr_minimal_positive_conservation_witness_attempt_under_strict_toy_assumptions_result"

/-- Current live target after the strict toy positive witness attempt result review. -/
def currentLiveNextStrictTargetV0 : String :=
  "prepare_qft_gr_minimal_positive_conservation_witness_maturation_packet"
