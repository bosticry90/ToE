...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .masterAction
      current_strongest_surface :=
        "Post-sampleRep32 axiom-audit selector consumes the 59-axiom audit review and returns to full-pillar target-map selection without continuing FNRep by momentum"
      retained_blocker :=
...
/-- Previous live target consumed by the post-sampleRep32 axiom-audit selector. -/
def previousLiveNextStrictTargetV0 : String :=
  "select_next_post_samplerep32_axiom_audit_bounded_attack"

/-- Current live target after the post-sampleRep32 axiom-audit selector. -/
def currentLiveNextStrictTargetV0 : String :=
  "return_to_full_pillar_target_map_next_lane_selection"
