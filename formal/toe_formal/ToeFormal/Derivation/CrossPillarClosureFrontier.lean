...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      ...
      master_action_dependency := .required_for_coherence
      next_strict_slice :=
        "review_ck_family_theorem_linkage_obligation_selection_after_psi_A_matter_exchange_closeout_result"
      status := .retained }
  , { row := .grQMSeam
      current_strongest_surface :=
...
/-- Previous live target consumed by the matter-exchange selector. -/
def previousLiveNextStrictTargetV0 : String :=
  "select_next_ck_family_theorem_linkage_obligation_after_psi_A_matter_exchange_closeout"

/-- Current live target after the matter-exchange selector. -/
def currentLiveNextStrictTargetV0 : String :=
  "review_ck_family_theorem_linkage_obligation_selection_after_psi_A_matter_exchange_closeout_result"
