...
def crossPillarClosureFrontierV0 :
  ...
  , { row := .qftGRSeam
      current_strongest_surface :=
        "QFT-GR minimal working model bounded conservation-retest attempt after latest repeated-inconclusive refinement executed the v4 weak-distributional retest protocol, recorded the result as inconclusive pending result review, did not claim pass or failure, preserved candidate-only status and bounded validation-only discipline including standing aggregate Lean and release-index caveats, and preserved no conservation proof object or witness, no source admissibility, no Bianchi compatibility, no semiclassical Einstein equation, no empirical validation, no master-action promotion, no release claim, no public submission, and no QFT-GR closure"
      retained_blocker :=
...
/-- Previous live target consumed by the latest v4 conservation-retest attempt. -/
def previousLiveNextStrictTargetV0 : String :=
...
/-- Current live target after the latest v4 conservation-retest attempt. -/
def currentLiveNextStrictTargetV0 : String :=
  "review_qft_gr_minimal_working_model_conservation_retest_attempt_after_post_retest_refinement_conservation_retest_refinement_refinement_result"
