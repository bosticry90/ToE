...
def dpi_coupled
    (rho0 lambda_chi beta_chi gamma : ℝ)
    (phi : FieldC) (chi : FieldR) (_pi : FieldR) : FieldR :=
  fun t x =>
    - (1 / 2 : ℝ) * (DxxR chi t x)
...
theorem reduction_property
    ...
    (hAlpha : alpha = 0)
    (hGamma : gamma = 0)
    (hChi : ∀ (_t _x : ℝ), chi _t _x = 0)
    (hPi : ∀ (_t _x : ℝ), pi _t _x = 0) :
    (dphi_coupled g_eff rho0 alpha phi chi = dphi_single g_eff rho0 phi)
    ∧ (dchi_coupled pi = (fun _ _ => 0))
    ∧ (dpi_coupled rho0 lambda_chi beta_chi gamma phi chi pi = (fun _ _ => 0)) := by
  ...
  · -- dphi_coupled = dphi_single
    funext _ _
    dsimp [dphi_coupled, dphi_single]
    simp [hAlpha]
  · constructor
    · -- dchi_coupled pi = 0
      funext _ _
    ...
    · -- dpi_coupled = 0 (gamma = 0, chi = 0 ⇒ derivative terms vanish structurally)
      ...
      dsimp [dpi_coupled]
      simp [hGamma, DxxR_zero, DxxxxR_zero, DxxxxxxR_zero]
