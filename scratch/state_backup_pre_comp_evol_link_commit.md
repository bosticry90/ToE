...
- Phase 5 — Comparator expansion (after Phases 1-4)
	Status: Deferred
	Actions: implement UCFF dispersion comparator; revisit canonical surface diversification; consider new domains only after Phases 1-4.
	Notes: Design-only until Phases 1-4 complete.
...
Status: Design-frozen / deferred
Owner ticket/module: formal/quarantine/bridge_tickets/BRIDGE_TICKET_COMPARATOR_CLASS_0001_dispersion_lane_ucff.md
Evidence path: formal/quarantine/bridge_tickets/BRIDGE_TICKET_COMPARATOR_CLASS_0001_dispersion_lane_ucff.md
Exit criteria: Explicit authorization after COMP-01 and COMP-02 exit criteria are met.
...
Evidence path: State_of_the_Theory.md
Exit criteria: Lift only after COMP-01 + COMP-02 exit criteria are met.
...
Notes: CT-01 probe/dispersion and solver-level ω-shift lanes are front-door-backed on pinned PTC cases (including conservative baseline, dissipative break case, and conservative amplitude/coupling/k variants). SYM01/PAR01/BC01 are now front-door-backed via deterministic B1 hooks artifacts (phase/pass + conjugation/fail, parity/pass + advection/fail, periodic-vs-Dirichlet boundary residuals). TR01 is report-backed via `time_reversal_error`; EN01 is report-backed via regime-specific `energy_delta` behavior. ST01 is front-door-backed via pinned bright-soliton cases and report shape metrics (`shape_residual`, `shape_peak_delta`, `shape_peak_ratio`) and now includes an n64 -> n128 resolution/convergence lane as a numerical adequacy check. TG01 is front-door-backed via pinned harmonic-trap ground-state cases and report trap metrics (`trap_shape_residual`, `trap_peak_ratio`, `trap_m2_delta`). OV-CV-01 is now dual-lane and front-door-backed on typed DR-01 artifacts: v0 integrity-only contract checks plus v1 discriminative-candidate cross-artifact residual checks under pinned/portable tolerance profiles. OV-CV-BR-01 adds explicit CV01 reason-code to pruning-atom mapping with deterministic lock records (canonical + negative-control) and attribution tests. OV-CV-02 pins a second comparator domain lane (DRBR-DOMAIN-02) with integrity-only front-door checks and deterministic blocked/negative-control behavior.
Falsifier families: dispersion/probe (CT01), solver consequence (ω-shift), symmetry (SYM01, PAR01, TR01), boundary handling (BC01), conservation/monotonicity (EN01), soliton structure/shape preservation plus resolution/convergence adequacy (ST01), trapped ground-state structure/shape stability under harmonic confinement (TG01), BEC-Bragg comparator v0/v1 lanes (OV-CV-01), explicit CV01-to-pruning bridge lane (OV-CV-BR-01), and second-domain comparator lane (OV-CV-02).
...
Comparator-expansion block rule (policy): No new comparator implementations unless (i) variational scaffold exists, (ii) at least one Noether invariant lemma exists, (iii) SYM-01 gate suite is pinned.
