...
try {
	...
	# Bragg lane validation guardrail (must not mutate locks).
	./py.ps1 -m formal.python.tools.regen_canonical_locks --bragg-only --validate-only --report

	# Sound lane validation guardrail (must not mutate locks).
	./py.ps1 -m formal.python.tools.regen_canonical_locks --snd-only --validate-only --report
