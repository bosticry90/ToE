param(
  [string[]]$FocusedTests = @(
    ...
    "formal/python/tests/test_research_mode_qm_stat_reentry_review_queue_packet_report.py",
    ...
    "formal/python/tests/test_research_mode_qm_stat_reentry_review_execution_report.py",
    "formal/python/tests/test_research_mode_qm_stat_reentry_post_review_adjudication_report.py"
  ),
  [switch]$SkipGate,
...
if (Test-Path $ResearchPath) {
  Write-Host "[3/22] Materialize retained pilot artifacts" -ForegroundColor Cyan
  ./py.ps1 -m formal.python.research.pilot_pack --write
  if ($LASTEXITCODE -ne 0) { throw "Research pilot materialization failed" }

  Write-Host "[4/22] Materialize Step 14 acceptance review" -ForegroundColor Cyan
  ...
  Write-Host "[5/22] Materialize sandbox candidacy bridge" -ForegroundColor Cyan
  ...
  Write-Host "[6/22] Materialize harder live QM-STAT target" -ForegroundColor Cyan
  ...
  Write-Host "[7/22] Materialize QM-STAT sandbox payload record" -ForegroundColor Cyan
  ...
  Write-Host "[8/22] Materialize QM-STAT payload versus harder-target comparison" -ForegroundColor Cyan
  ...
  Write-Host "[9/22] Materialize QM-STAT governed review wrapper" -ForegroundColor Cyan
  ...
  Write-Host "[10/22] Materialize QM-STAT sandbox governed intake execution" -ForegroundColor Cyan
  ...
  Write-Host "[11/22] Materialize QM-STAT sandbox review execution packet" -ForegroundColor Cyan
  ...
  Write-Host "[12/22] Materialize QM-STAT sandbox review execution" -ForegroundColor Cyan
  ...
  Write-Host "[13/22] Materialize QM-STAT post-review adjudication" -ForegroundColor Cyan
  ...
  Write-Host "[14/22] Materialize QM-STAT stronger live-target or authority evidence" -ForegroundColor Cyan
  ...
  Write-Host "[15/22] Materialize QM-STAT re-entry support artifact" -ForegroundColor Cyan
  ...
  Write-Host "[16/22] Materialize QM-STAT re-entry eligibility review" -ForegroundColor Cyan
  ...
  Write-Host "[17/22] Materialize QM-STAT re-entry review-cycle queue" -ForegroundColor Cyan
  ...
  Write-Host "[18/22] Materialize QM-STAT re-entry review queue packet" -ForegroundColor Cyan
  ./py.ps1 -m formal.python.tools.research_mode_qm_stat_reentry_review_queue_packet_report
  if ($LASTEXITCODE -ne 0) { throw "Research QM-STAT re-entry review queue packet materialization failed" }

  Write-Host "[19/22] Materialize QM-STAT re-entry downstream intake decision" -ForegroundColor Cyan
  ./py.ps1 -m formal.python.tools.research_mode_qm_stat_reentry_downstream_intake_decision_report
  if ($LASTEXITCODE -ne 0) { throw "Research QM-STAT re-entry downstream intake decision materialization failed" }

  Write-Host "[20/22] Materialize QM-STAT re-entry review execution packet" -ForegroundColor Cyan
  ./py.ps1 -m formal.python.tools.research_mode_qm_stat_reentry_review_execution_packet_report
  if ($LASTEXITCODE -ne 0) { throw "Research QM-STAT re-entry review execution packet materialization failed" }

  Write-Host "[21/22] Materialize QM-STAT re-entry review execution" -ForegroundColor Cyan
  ./py.ps1 -m formal.python.tools.research_mode_qm_stat_reentry_review_execution_report
  if ($LASTEXITCODE -ne 0) { throw "Research QM-STAT re-entry review execution materialization failed" }

  Write-Host "[22/22] Materialize QM-STAT re-entry post-review adjudication" -ForegroundColor Cyan
  ./py.ps1 -m formal.python.tools.research_mode_qm_stat_reentry_post_review_adjudication_report
  if ($LASTEXITCODE -ne 0) { throw "Research QM-STAT re-entry post-review adjudication materialization failed" }
}
