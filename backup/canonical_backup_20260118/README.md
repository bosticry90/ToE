...
1. The idea cannot be clearly stated
   ...
   If I can�t say what the system is in a clean way, it isn�t viable.
...
3. Nothing meaningful ever emerges
   If the system only produces trivial, frozen, or empty behavior�and never generates patterns, structure, or organization�the hypothesis fails.
...
To run those modules outside pytest without relying on implicit sys.path behavior, prefer module invocation from the repo root:

python -m formal.python.toe.bridges.br01_dispersion_to_metric
